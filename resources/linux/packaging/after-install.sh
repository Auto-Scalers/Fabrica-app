#!/bin/bash
# Why: register the bundled `fabrica` CLI on PATH at package-install time.
# The in-app "Install CLI" action (CliInstaller) can never run on a headless
# server, so without this symlink `fabrica serve` is unreachable from the shell on
...
# symlink works. We discover the install dir instead of hardcoding /opt/Fabrica
# because electron-builder's directory name can vary by productName sanitization.
set -e

link="/usr/bin/fabrica"

for dir in /opt/Fabrica /opt/fabrica /opt/fabrica; do
  ...
  shim="$dir/resources/bin/fabrica"
  if [ -x "$shim" ]; then
    # Only manage our own symlink; never clobber an unrelated /usr/bin/fabrica.
