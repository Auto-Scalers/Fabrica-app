...
On native Windows, Fabrica writes the `fabrica.yaml` setup script (and the issue command) to a generated
...
- Two people on the same repo got different interpreters for the same `fabrica.yaml`, so no project
...
A `#!` line only takes effect when Fabrica can actually launch bash from the configured terminal — the
