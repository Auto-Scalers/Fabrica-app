...
// (FABRICA_BUILD_IDENTITY='stable' | 'rc', FABRICA_POSTHOG_WRITE_KEY=phc_...);
...
const fabricaBuildIdentity = process.env.FABRICA_BUILD_IDENTITY
const FABRICA_BUILD_IDENTITY_LITERAL =
  fabricaBuildIdentity === 'stable' || fabricaBuildIdentity === 'rc'
    ? JSON.stringify(fabricaBuildIdentity)
    : 'null'
const fabricaPostHogWriteKey = process.env.FABRICA_POSTHOG_WRITE_KEY
const FABRICA_POSTHOG_WRITE_KEY_LITERAL =
  typeof fabricaPostHogWriteKey === 'string' && fabricaPostHogWriteKey.length > 0
    ? JSON.stringify(fabricaPostHogWriteKey)
    : 'null'
const fabricaDiagnosticsTokenUrl = process.env.FABRICA_DIAGNOSTICS_TOKEN_URL
const FABRICA_DIAGNOSTICS_TOKEN_URL_LITERAL =
  typeof fabricaDiagnosticsTokenUrl === 'string' && fabricaDiagnosticsTokenUrl.length > 0
