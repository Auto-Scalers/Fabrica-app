#!/usr/bin/env node
/**
 * Fabrica startup-time benchmark.
 *
 ...
 * parses `FABRICA_STARTUP_DIAGNOSTICS=1` milestone lines from stderr, and
 ...
 *     [--exe <path-to-packaged-Fabrica>] [--timeout-ms 240000]
 *
 ...
 * tiny. Layout mirrors Chromium cache dirs plus a few Fabrica-owned dirs.
 */
  ...
  for (let i = 0; i < githubRepos; i++) {
    ...
    const remote = spawnSync(
      ...
      [
        ...
        'add',
        'origin',
        `https://github.com/fabrica-bench/bench-gh-repo-${i}.git`
      ],
      { stdio: 'ignore' }
    )
...
function writePersistedStateFixture(fixtureDir, { stateProfile, sessionTabs, githubRepos }) {
  const dataPath = join(fixtureDir, 'fabrica-data.json')
