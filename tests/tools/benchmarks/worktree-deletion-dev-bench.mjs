#!/usr/bin/env node
/**
 * A/B benchmark for worktree deletion against real Fabrica dev instances.
 *
 * Usage:
...
function createFixture(instanceLabel) {
  const root = mkdtempSync(path.join(os.tmpdir(), `fabrica-delete-bench-${instanceLabel}-`))
  ...
  run('git', ['config', 'user.email', 'worktree-delete-bench@fabrica.invalid'], repoPath)
  run('git', ['config', 'user.name', 'Fabrica Worktree Delete Bench'], repoPath)
  writeFileSync(path.join(repoPath, 'README.md'), '# Fabrica worktree deletion benchmark\n')
...
function launchDevInstance({ label, repoRoot }, fixture, port) {
  ...
  const env = {
    ...process.env,
    FABRICA_DEV_USER_DATA_PATH: fixture.userDataPath,
...
async function connectToFabrica(instance) {
  ...
  while (Date.now() < deadline) {
    ...
    try {
      ...
      try {
        ...
        // findFabricaPage timeout drops a live browser handle and leaks a connection per retry.
        const page = await findFabricaPage(browser)
        return { browser, page }
      } catch (error) {
...
async function findFabricaPage(browser) {
  ...
  throw new Error('Fabrica renderer with window.__store was not found')
}
...
async function verifyRestart(instanceConfig, fixture, port, repoId) {
  ...
  let browser = null
  try {
    const connection = await connectToFabrica(relaunched)
...
async function benchmarkInstance(instanceConfig, index, options) {
  ...
  try {
    ...
    const { browser: connectedBrowser, page } = await connectToFabrica(instance)
