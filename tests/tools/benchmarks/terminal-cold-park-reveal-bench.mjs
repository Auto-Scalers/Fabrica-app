...
// FABRICA_E2E_TERMINAL_PARKING_DELAY_MS override to shrink the 30s hysteresis and
...
function createLocalRepoFixture() {
  ...
  git(repoPath, 'config', 'user.email', 'bench@fabrica.local')
  git(repoPath, 'config', 'user.name', 'Fabrica Bench')
...
async function setupWorkspaces(page, fixture) {
  return await runWithTimeout(
    'fixture registration in Fabrica',
    () =>
      page.evaluate(
