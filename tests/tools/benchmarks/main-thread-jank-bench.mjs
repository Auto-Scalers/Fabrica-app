#!/usr/bin/env node
/**
 * Fabrica main-thread jank / subprocess-churn benchmark (issue #7576).
 *
 ...
 * launched with FABRICA_MAIN_THREAD_DIAGNOSTICS=1 so the main process emits one
 ...
 *     [--exe <path-to-packaged-Fabrica>] [--headless] [--no-log-stream]
 *
 ...
 * the shape Fabrica worktrees commonly have, which forces the effective-upstream
...
function ensureFixture(fixtureDir) {
  ...
  writeFileSync(join(fixtureDir, 'fabrica-data.json'), JSON.stringify(state, null, 2))
  return repoPath
}
...
async function main() {
  ...
  const env = {
    ...process.env,
    FABRICA_STARTUP_DIAGNOSTICS: '1',
    FABRICA_MAIN_THREAD_DIAGNOSTICS: '1',
    FABRICA_E2E_USER_DATA_DIR: fixtureDir,
    HOME: isolatedHome,
    USERPROFILE: isolatedHome,
    FABRICA_E2E_HOME_DIR: isolatedHome
  }
  delete env.CODEX_HOME
  delete env.FABRICA_CODEX_HOME
  if (args.headless) {
    env.FABRICA_E2E_HEADLESS = '1'
    console.warn(
