...
// installed, packaged Fabrica.exe. So its only source is `--exe-path` (defaulting
...
Proves that force-killing ONLY Fabrica's main process (a real crash, no tree-kill)
...
Usage:
  node tests/tools/win-crash-survival-e2e/run.mjs --expect <profile> [--exe-path <Fabrica.exe>] [options]
...
Options:
  --exe-path <path>        Installed Fabrica.exe to drive (default: the per-user
                           install under %LOCALAPPDATA%\\Programs\\Fabrica). The
...
export function parseArgs(argv) {
  ...
  const opts = {
    ...
    // "no Fabrica.exe found" default-resolution error.
    exePath:
...
function validate(opts, exePathFlagPresent, argv) {
  ...
  if (exePathFlagPresent && takeValue(argv, '--exe-path') === undefined) {
    ...
    errors.push(
      'No installed Fabrica.exe found under %LOCALAPPDATA%\\Programs — pass --exe-path <Fabrica.exe>'
    )
