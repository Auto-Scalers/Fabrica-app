...
//
// GitHub #7742: on Windows, when Fabrica's main/renderer process crashed, open
...
// fabrica-terminal-daemon.exe that survives main death (src/main/daemon/
...
const SENTINEL_ENV = 'FABRICA_CRASH_SENTINEL'
...
async function main() {
  ...
  // clear platform message, not a confusing "no Fabrica.exe found" default-resolution
  // failure.
  ...
  const canary = `FABRICA-CRASH-SENTINEL-${runId}`
  const runDir = mkdtempSync(path.join(tmpdir(), `fabrica-win-crash-e2e-${runId}-`))
  ...
  const diagDir = process.env.FABRICA_E2E_DIAG_DIR || path.join(runDir, 'diag')
  let passed = false
  try {
 ...
 * daemons on the machine (including the developer's live Fabrica) are ignored.
 ...
 * touches any other Fabrica on the box (a live user instance uses a different
