...
async function callFabrica(cli, args, options = {}) {
...
function buildLongSpec(sizeKb, marker) {
  ...
  const header = [
    `FABRICA_LONG_PROMPT_REPRO_START ${marker}`,
  ...
  let index = 0
  while (
    Buffer.byteLength(`${lines.join('\n')}\nFABRICA_LONG_PROMPT_REPRO_END ${marker}`, 'utf8') <
    targetBytes
  ) {
    lines.push(
  ...
  lines.push(`FABRICA_LONG_PROMPT_REPRO_END ${marker}`)
  return lines.join('\n')
}
...
async function tryCloseTerminal(cli, handle, cwd) {
  ...
  try {
    await callFabrica(cli, ['terminal', 'close', '--terminal', handle], { cwd })
  } catch {
...
async function parentMain() {
  const cli = argValue('cli', process.env.FABRICA_REPRO_CLI ?? 'fabrica')
  ...
  const tempDir = path.join(tmpdir(), `fabrica-orchestration-long-prompt-${process.pid}-${Date.now()}`)
  await mkdir(tempDir, { recursive: true })
  const workerReportPath = path.join(tempDir, 'worker-report.json')

  console.log(`runtime: ${cli} status`)
  await callFabrica(cli, ['status'], { cwd })

  const coordinator = await callFabrica(
    cli,
    [
      'terminal',
      'create',
  ...
  const worker = await callFabrica(
    cli,
    [
      'terminal',
      'create',
  ...
  try {
    ...
    const dispatch = await callFabrica(
      cli,
      [
        'orchestration',
        'dispatch',
...
async function fakeWorkerMain() {
  ...
  const finish = async (reason) => {
    ...
    process.stdout.write(
      `\nFABRICA_REPRO_REPORT ${report.contractOk ? 'ok' : 'failed'} ${reportPath}\n`
    )
...
The parent mode requires a running Fabrica runtime and creates temporary Fabrica
terminals. The fake worker records whether orchestration dispatch delivered a
long prompt in a safe agent-input contract.

Options:
  --cli <path>         Fabrica CLI command (default: FABRICA_REPRO_CLI or fabrica)
