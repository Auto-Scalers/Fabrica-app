...
const DEFAULT_EXECUTABLE = '/Applications/Fabrica.app/Contents/MacOS/Fabrica'
const DEFAULT_PROFILE = path.join(os.homedir(), 'Library', 'Application Support', 'fabrica')
const URL = 'https://example.com/fabrica-terminal-garble-repro'
...
const runRoot = mkdtempSync(path.join(os.tmpdir(), 'fabrica-terminal-garble-'))
const userDataDir = path.join(runRoot, 'user-data')
const evidenceDir = path.join(runRoot, 'evidence')
mkdirSync(userDataDir, { recursive: true })
mkdirSync(evidenceDir, { recursive: true })
function sanitizeProfile() {
  const sourceDataPath = path.join(sourceProfile, 'fabrica-data.json')
  ...
  writeFileSync(path.join(userDataDir, 'fabrica-data.json'), serialized)
...
try {
  ...
  app = await electron.launch({
    ...
    env: {
      ...
      ...(mainPath ? { NODE_ENV: 'development', FABRICA_E2E_HEADFUL: '1' } : {})
    }
  })
