// Launches the copied Fabrica.exe as the daemon host (ELECTRON_RUN_AS_NODE=1) and
// waits for its {type:'ready'} IPC signal. Mirrors the real fork() options in
// src/main/daemon/daemon-init.ts (detached, ipc channel, FABRICA_USER_DATA_PATH).
...
export function launchDaemonHost(options) {
  ...
  const env = {
    ...
    FABRICA_USER_DATA_PATH: workDir
  }
  ...
  // if run against a build that carries the FABRICA_NODE_PTY_NATIVE_DIR patch.
  if (nodePtyNativeDir) {
    env.FABRICA_NODE_PTY_NATIVE_DIR = nodePtyNativeDir
  }
