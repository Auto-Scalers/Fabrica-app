...
function makeSyntheticInventory() {
  ...
  return {
    appDir: 'C:\\App',
    unpackedRoot,
    hostExe: { name: 'Fabrica.exe', ...f('C:\\App\\Fabrica.exe') },
    runtimeData: [
...
function run() {
  ...
  for (const tier of TIERS) {
    ...
    check(`${tier}: has exe`, dests.includes('Fabrica.exe'))
  ...
  // ── Module-path filter ──────────────────────────────────────────────
  const appDir = 'C:\\Users\\me\\AppData\\Local\\Programs\\fabrica'
  const modules = [
    'C:\\Users\\me\\AppData\\Local\\Programs\\fabrica\\Fabrica.exe',
    'C:\\Windows\\System32\\kernel32.dll',
    'C:\\Users\\me\\AppData\\Local\\fabrica-daemon-host\\Fabrica.exe'
  ]
  const resident = findAppDirResidentModules(modules, appDir)
  check('detects app-dir module', resident.length === 1)
  check('detects the right module', resident[0].toLowerCase().includes('programs\\fabrica\\fabrica.exe'))
  // Sibling-prefix must NOT match (C:\...\fabrica vs C:\...\fabrica-daemon-host).
  check(
    'sibling prefix not matched',
    findAppDirResidentModules(['C:\\a\\fabrica-daemon-host\\x.dll'], 'C:\\a\\fabrica').length === 0
  )
  // Forward/back-slash + case normalization.
  check(
    'slash + case normalized',
    findAppDirResidentModules(['c:/a/FABRICA/x.dll'], 'C:\\A\\fabrica').length === 1
  )
  check(
    'relocated host has zero app-dir modules',
    findAppDirResidentModules(
      ['C:\\work\\daemon-host\\Fabrica.exe', 'C:\\Windows\\System32\\ntdll.dll'],
      appDir
    ).length === 0
  )
