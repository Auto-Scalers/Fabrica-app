...
/**
 * Read-only discovery of the machine's current Fabrica install registry state.
...
export function discoverInstallRegistryState() {
  const ps = [
    ...
    `  if ($dn -eq 'Fabrica' -or ($il -and $il -match '\\\\Programs\\\\fabrica')) {`,
    ...
    `  if ($il -and ($sn -eq 'Fabrica' -or $il -match 'fabrica')) {`,
...
/** The two directories NSIS writes Fabrica shortcuts into: Start Menu + Desktop. */
...
/** Read-only: list Fabrica *.lnk shortcuts under the given dirs (recursive). */
export function discoverFabricaShortcuts(dirs = shortcutDirs()) {
  const found = []
  for (const dir of dirs) {
    collectFabricaLnks(dir, found)
  }
  return found
}

function collectFabricaLnks(dir, out) {
  if (!existsSync(dir)) {
    return
  }
  let entries = []
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      collectFabricaLnks(full, out)
    } else if (entry.isFile() && /\.lnk$/i.test(entry.name) && /fabrica/i.test(entry.name)) {
      out.push(full)
    }
  }
}
...
/**
 * Snapshot the shared install registry keys + Fabrica shortcuts before an isolated
...
export function backupInstallState(runDir) {
  ...
  let i = 0
  for (const lnk of discoverFabricaShortcuts(dirs)) {
 ...
 * Pre-existing shortcuts are copied back; test-created Fabrica shortcuts are removed.
...
function restoreShortcuts(manifest, result) {
  ...
  for (const lnk of discoverFabricaShortcuts(manifest.shortcutDirs)) {
...
function printMismatchWarning(manifest, expected, actual) {
  ...
  console.error('!! Your REAL Fabrica install pointer may be hijacked to the test directory.')
  console.error('!! The next real Fabrica update could install into the test location.')
...
if (process.argv[1] && path.resolve(process.argv[1]) === import.meta.filename) {
  ...
  console.log('[registry-shortcut-backup] Fabrica shortcuts that would be backed up:')
  console.log(JSON.stringify(discoverFabricaShortcuts(), null, 2))
}
