...
//
// Fabrica ships a per-user oneClick NSIS installer (electron-builder defaults:
// oneClick=true, perMachine=false) named fabrica-windows-setup.exe. One-click
// silent mode is `<setup.exe> /S`; the app installs under
// %LOCALAPPDATA%\Programs\<dir> and the exe is Fabrica.exe. The install dir casing
// is not guaranteed (observed lowercase "fabrica" on a dev box), so the exe is
...
const PRODUCT_NAME = 'Fabrica'
const EXE_NAME = 'Fabrica.exe'
...
export function resolveInstaller({ localPath, releaseTag, assetPattern }) {
  ...
  const outDir = mkdtempSync(path.join(tmpdir(), 'fabrica-e2e-installer-'))
...
export function silentInstall(setupExe, { timeoutMs = 180_000, installDir = null } = {}) {
  ...
  // On update runs the old Fabrica.exe already exists, so wait for the exe whose
...
/**
 * Locate the installed Fabrica.exe. In isolated mode (`installDir` set), the exe is
 * at a known fixed path (<installDir>\Fabrica.exe). Otherwise it is discovered
 ...
 * developer's REAL Fabrica. It additionally refuses to run against the default
  ...
  if (!allowDefaultLocation && pathsEqual(resolved, path.join(programsRoot(), PRODUCT_NAME))) {
    throw new Error(
      `Refusing to uninstall the default install location "${resolved}" — this is where a ` +
        `developer's REAL Fabrica lives. Isolated mode must target a separate --install-dir.`
    )
  }
