...
//
// Given two Fabrica Windows installers (version N and N+1), proves what happens to
...
async function main() {
  ...
  const canary = `FABRICA-E2E-CANARY-${runId}`
  const runDir = mkdtempSync(path.join(tmpdir(), `fabrica-win-update-e2e-${runId}-`))
...
function resolveScopedDaemon(userDataDir) {
  ...
  return {
    ...
    // install-dir Fabrica.exe — the key survival signal.
...
async function teardown({
  ...
  if (hadPreexistingInstall) {
    console.log(
      '\n*** NOTE: an Fabrica install existed before this run. It was OVERWRITTEN and the\n' +
