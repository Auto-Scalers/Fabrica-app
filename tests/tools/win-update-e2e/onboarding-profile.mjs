// Persistence seed for a fresh packaged Fabrica profile: dismisses onboarding and
...
// fabrica-data.json` BEFORE launch dismisses it. The app derives the Projects list
...
//
// IMPORTANT: seed only BEFORE the first launch. The app rewrites fabrica-data.json
 ...
 * for it. A real checkout (init + one commit) is required — Fabrica treats a
...
export function createSeededRepo(dir) {
  ...
  git('init', '-b', 'main')
  git('config', 'user.email', 'win-update-e2e@fabrica.test')
...
export function seedFreshProfile(userDataDir, profile) {
  ...
  writeFileSync(path.join(userDataDir, 'fabrica-data.json'), `${JSON.stringify(profile, null, 2)}\n`)
}
