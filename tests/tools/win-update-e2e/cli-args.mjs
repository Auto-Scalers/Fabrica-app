...
Installer source (version N, then N+1) — path or release tag on each side:
  --from <path>            Local fabrica-windows-setup.exe for the base version (N)
  --to <path>              Local fabrica-windows-setup.exe for the update (N+1)
...
Options:
  --install-dir <path>     Isolated-install mode: install the test build into
                           ...
                           leaving a developer's REAL Fabrica install untouched.
  ...
  --allow-existing-install Proceed even if an Fabrica install already exists. The
                           ...
                           developer's real Fabrica. Clean machines (CI/VM) never
...
/** Default per-user oneClick install location: %LOCALAPPDATA%\Programs\Fabrica. */
function defaultInstallDir() {
  const localAppData =
    process.env.LOCALAPPDATA ?? path.join(process.env.USERPROFILE ?? '', 'AppData', 'Local')
  return path.join(localAppData, 'Programs', 'Fabrica')
}
...
function looksLikeHarnessInstall(dir) {
  return existsSync(path.join(dir, 'Fabrica.exe')) && existsSync(path.join(dir, 'Uninstall Fabrica.exe'))
}
  ...
  if (/\s/.test(installDir)) {
    errors.push(
      `--install-dir must not contain spaces (got "${installDir}"). The NSIS installer's ` +
        ...
        `be passed. Choose a spaces-free location (e.g. C:\\FabricaE2E).`
    )
  }
    ...
    if (entries.length > 0 && !looksLikeHarnessInstall(installDir)) {
      errors.push(
        `--install-dir "${installDir}" is a non-empty directory that does not look like a ` +
          `prior harness install (no Fabrica.exe + "Uninstall Fabrica.exe"). Refusing to overwrite ` +
