...
test('the tab X button applies the same running-process confirmation as Cmd+W', async ({
  ...
  await execInTerminal(fabricaPage, ptyId, 'echo repro-10142-ready')
  await waitForTerminalOutput(fabricaPage, 'repro-10142-ready', 20_000)
  await execInTerminal(fabricaPage, ptyId, 'sleep 300')
  ...
  await expect
    .poll(
      async () =>
        (await fabricaPage.evaluate((id) => window.api.pty.inspectProcess(id), ptyId))
          .foregroundProcess,
      { timeout: 20_000, message: 'sleep 300 never became the foreground process' }
    )
    .toBe('sleep')

  const busyTabId = (await getActiveTabId(fabricaPage))!
  ...
  expect(
    {
      confirmDialogVisible: await closeDialogTitle(fabricaPage).isVisible(),
      tabStillPresent: (await countRenderedTabs(fabricaPage)) === tabsBefore
    },
