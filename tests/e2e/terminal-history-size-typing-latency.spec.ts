...
test.describe('Terminal typing latency vs scrollback history size', () => {
  test('typing stays responsive as terminal history grows', async ({
    ...
    const scriptPath = path.join(testRepoPath, `.FABRICA-history-benchmark-${runId}.mjs`)
    ...
    try {
      ...
      commandSent = true
      await waitForRecentTerminalMarker(fabricaPage, `HIST_READY_${runId}`, 10_000)
      await focusActiveTerminalInput(fabricaPage)

      const phases: PhaseLatency[] = []
      let seq = 0

      const baseline = await measureTypingLatency(fabricaPage, runId, 'empty history', seq)
