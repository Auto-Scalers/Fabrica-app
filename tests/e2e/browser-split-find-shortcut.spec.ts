...
function browserAddressBar(page: Page, browserTabId: string) {
  return page.locator(
    `[data-browser-overlay-tab-id="${browserTabId}"] [data-FABRICA-browser-address-bar="true"]`
  )
}
...
async function focusBrowserAddressBar(page: Page, browserTabId: string): Promise<void> {
  ...
  const addressBarForm = browserOverlay.locator(
    'form:has(> [data-FABRICA-browser-address-bar="true"])'
  )
...
test.describe('browser split Find shortcut', () => {
  ...
  test('routes repeated Find shortcuts to the focused terminal or browser split', async ({
    ...
    const fixture = await createTerminalBrowserSplit(fabricaPage)

    await fabricaPage.evaluate(({ terminalGroupId }) => {
      const state = window.__store?.getState()
      const worktreeId = state?.activeWorktreeId
      if (state && worktreeId) {
    ...
    await waitForFocusedGroup(fabricaPage, fixture.terminalGroupId)
    await fabricaPage.keyboard.press(`${modifier}+f`)
    await expect(terminalFindInput(fabricaPage)).toBeFocused()
    await expect(browserFindInput(fabricaPage)).toBeHidden()
    await fabricaPage.keyboard.press('Escape')

    await focusBrowserGroup(fabricaPage, fixture.browserGroupId)
    await focusBrowserAddressBar(fabricaPage, fixture.browserTabId)
    await fabricaPage.keyboard.press(`${modifier}+f`)
    await expect(browserFindInput(fabricaPage)).toBeFocused()
    await expect(terminalFindInput(fabricaPage)).toBeHidden()
    await browserFindCloseButton(fabricaPage).click()
    await expect(browserFindInput(fabricaPage)).toBeHidden()
    ...
    await fabricaPage.evaluate(({ browserTabId }) => {
      window.__store?.getState().closeBrowserTab(browserTabId)
    }, fixture)
    await expect(
      fabricaPage.locator(`[data-browser-overlay-tab-id="${fixture.browserTabId}"]`)
    ).toHaveCount(0)
  ...
  test('opens Find only in the browser split whose guest owns the shortcut', async ({
    ...
    const fixture = await createBrowserSplit(fabricaPage)

    await pressFindInBrowserGuest(fabricaPage, fixture.firstBrowserTabId, fixture.firstBrowserPageId)

    await expect(browserSplitFindInput(fabricaPage, fixture.firstBrowserTabId)).toBeVisible()
    await expect(browserSplitFindInput(fabricaPage, fixture.secondBrowserTabId)).toBeHidden()
    await expect
      .poll(() =>
  ...
  test('keeps browser Find available when split focus state is temporarily missing', async ({
    ...
    const fixture = await createTerminalBrowserSplit(fabricaPage)
    await focusBrowserGroup(fabricaPage, fixture.browserGroupId)
    const addressBar = browserAddressBar(fabricaPage, fixture.browserTabId)
    await focusBrowserAddressBar(fabricaPage, fixture.browserTabId)
  ...
  test('keeps browser Find available when the focused split ID is stale', async ({ fabricaPage }) => {
    const fixture = await createTerminalBrowserSplit(fabricaPage)
    await focusBrowserGroup(fabricaPage, fixture.browserGroupId)
    const addressBar = browserAddressBar(fabricaPage, fixture.browserTabId)
    await focusBrowserAddressBar(fabricaPage, fixture.browserTabId)
