...
test.describe('Workspace Space git status checks', () => {
  test('checks every scanned deletable row, including rows after the first 50', async ({
    ...
    const worktreeParent = realpathSync(
      mkdtempSync(path.join(os.tmpdir(), 'FABRICA-space-git-status-'))
    )
    ...
    try {
      ...
      const repoId = await fabricaPage.evaluate((testRepoPath) => {
      ...
      await loadWorktreesUntilPathsPresent(fabricaPage, repoId, registeredWorktreePaths)
      ...
      await expect(fabricaPage.getByText('Keep: git not checked')).toHaveCount(0)
    } finally {
      for (const worktreePath of worktreePaths) {
        try {
