...
test('closing a parked terminal tab retires its exact PTY session', async ({ fabricaPage }) => {
  ...
  const tab = fabricaPage.locator(`[data-testid="sortable-tab"][data-tab-id="${tabId}"]`)
  await expect(tab).toBeVisible()

  const marker = `PARKED_CLOSE_READY_${randomUUID()}`
  const keepAliveScript = `process.stdout.write(${JSON.stringify(`${marker}\n`)}); setInterval(() => {}, 1000)`
  await execInTerminal(fabricaPage, ptyId, nodeTerminalCommand(['-e', keepAliveScript]))
  await expect
    .poll(() => getTerminalContent(fabricaPage), {
      timeout: 10_000,
      message: 'long-lived terminal child did not print its ready marker'
    })
    .toContain(marker)
  await expect.poll(() => hasPtySession(fabricaPage, ptyId), { timeout: 10_000 }).toBe(true)
  ...
  await expect
    .poll(() => hasPtySession(fabricaPage, ptyId), {
      timeout: 15_000,
