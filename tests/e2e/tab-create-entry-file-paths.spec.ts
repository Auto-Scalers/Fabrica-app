...
const relativeFilePath =
  'packages/FABRICA/src/renderer/src/components/navigation/worktree/secondary-nav/SecondaryNav.tsx'
...
test('new-tab file results prioritize the filename and reveal the full path on hover', async ({
  ...
  const input = fabricaPage.locator('input[role="combobox"][aria-controls="tab-create-entry-results"]')
  await input.fill('secondaryNav')

  const row = fabricaPage.locator('[role="option"]').filter({ hasText: 'Open file' }).first()
  await expect(row).toBeVisible()
  await expect(row).toContainText('SecondaryNav.tsx')
  await expect(row).toContainText('packages/FABRICA/src/renderer/src/components/navigation/')
  const rowText = await row.textContent()
  expect(rowText?.indexOf('SecondaryNav.tsx')).toBeLessThan(
    rowText?.indexOf('packages/FABRICA/src/renderer/src/components/navigation/') ?? -1
  )
  ...
  await expect(
    fabricaPage.locator('[data-slot="tooltip-content"]').filter({ hasText: relativeFilePath })
  ).toBeVisible()

  const proofPath = process.env.FABRICA_STA3424_PROOF_PATH
  if (proofPath) {
