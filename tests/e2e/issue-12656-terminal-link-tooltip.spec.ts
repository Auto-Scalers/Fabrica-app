...
test.describe('Issue #12656 terminal link tooltip', () => {
  test('clears hover state without permanently shrinking the terminal', async ({
    ...
    const url = `https://example.com/FABRICA-issue-12656-${randomUUID().slice(0, 8)}`
    await sendToTerminal(
      fabricaPage,
      ptyId,
      `printf 'issue-12656-output-%02d\\n' $(seq 1 64); printf '${url}\\n'\r`
    )
    await waitForTerminalOutput(fabricaPage, url)

    let probe: LinkProbe | null = null
    await expect
      .poll(
        async () => {
          probe = await locateUrl(fabricaPage, url)
          return probe
        },
    ...
    const idle = await readTooltipState(fabricaPage, probe.tabId)
    expect(Math.abs(idle.paneBottom - idle.terminalBottom)).toBeLessThanOrEqual(1)
    await expect
      .poll(async () => {
        await moveToLink(fabricaPage, probe)
        return readTooltipState(fabricaPage, probe.tabId)
      })
      .toMatchObject({ display: '', currentLinkText: url })

    const hovered = await readTooltipState(fabricaPage, probe.tabId)
    ...
    await captureProof(fabricaPage, testInfo, 'issue-12656-fixed-hover.png')

    await fabricaPage.evaluate(() => window.dispatchEvent(new Event('blur')))
    await expect
      .poll(() => readTooltipState(fabricaPage, probe.tabId))
      .toMatchObject({ display: 'none', currentLinkText: null, cursor: 'text' })
    const cleared = await readTooltipState(fabricaPage, probe.tabId)
    expect(Math.abs(cleared.paneBottom - cleared.terminalBottom)).toBeLessThanOrEqual(1)
    await captureProof(fabricaPage, testInfo, 'issue-12656-fixed-after-blur.png')

    await expect.poll(() => getTerminalContent(fabricaPage)).toContain(url)
  })
})
