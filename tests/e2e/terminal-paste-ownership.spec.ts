...
async function installClipboardReadTerminalBlurRepro(app: ElectronApplication): Promise<void> {
  await app.evaluate(({ BrowserWindow, ipcMain }) => {
    ...
    const global = globalThis as unknown as {
      __FABRICAOriginalClipboardReadTextHandler?: ClipboardReadHandler
    }
    ...
    if (!invokeHandlers || !handler || global.__FABRICAOriginalClipboardReadTextHandler) {
      return
    }
    global.__FABRICAOriginalClipboardReadTextHandler = handler
    invokeHandlers.set('clipboard:readText', async (event, ...args) => {
      ...
      return global.__FABRICAOriginalClipboardReadTextHandler!(event, ...args)
    })
  })
}
...
async function restoreClipboardReadTerminalBlurRepro(app: ElectronApplication): Promise<void> {
  await app.evaluate(({ ipcMain }) => {
    ...
    if (invokeHandlers && global.__FABRICAOriginalClipboardReadTextHandler) {
      invokeHandlers.set('clipboard:readText', global.__FABRICAOriginalClipboardReadTextHandler)
      delete global.__FABRICAOriginalClipboardReadTextHandler
    }
  })
}
...
test.describe('terminal paste ownership', () => {
  test('keyboard paste shortcuts send clipboard text to the focused terminal exactly once', async ({
    ...
    const scriptPath = path.join(testRepoPath, `.FABRICA-paste-ownership-${runId}.mjs`)
    ...
    try {
      ...
      scriptStarted = true
      await waitForTerminalOutput(fabricaPage, `PASTE_READY_${runId}`, 10_000)

      for (const [index, chord] of keyboardPasteChords().entries()) {
        const payload = `FABRICA_E2E_PASTE_${runId}_${index}`
        const encodedPayload = Buffer.from(payload, 'utf8').toString('base64')
        await clearTerminalPtyWriteLog(electronApp)
        await fabricaPage.evaluate((text) => window.api.ui.writeClipboardText(text), payload)
        await focusActiveTerminalInput(fabricaPage)

        await fabricaPage.keyboard.press(chord)
        await waitForTerminalOutput(fabricaPage, encodedPayload, 10_000, 12_000)
  ...
  test('keyboard paste survives transient terminal blur during clipboard read', async ({
    ...
    const ptyId = await waitForActivePanePtyId(fabricaPage)
    const runId = randomUUID()
    const scriptPath = path.join(testRepoPath, `.FABRICA-paste-blur-${runId}.mjs`)
    writeFileSync(scriptPath, pasteEchoScript(runId))
    let scriptStarted = false
    ...
    try {
      ...
      const payload = `FABRICA_E2E_TRANSIENT_BLUR_PASTE_${runId}`
      const encodedPayload = Buffer.from(payload, 'utf8').toString('base64')
      await clearTerminalPtyWriteLog(electronApp)
      await fabricaPage.evaluate((text) => window.api.ui.writeClipboardText(text), payload)
      await focusActiveTerminalInput(fabricaPage)
      ...
      await fabricaPage.keyboard.press(keyboardPasteChords()[0])
      await waitForTerminalOutput(fabricaPage, encodedPayload, 10_000, 12_000)
  ...
  test('terminal context-menu Paste sends clipboard text exactly once', async ({
    ...
    const ptyId = await waitForActivePanePtyId(fabricaPage)
    const runId = randomUUID()
    const scriptPath = path.join(testRepoPath, `.FABRICA-paste-context-menu-${runId}.mjs`)
    writeFileSync(scriptPath, pasteEchoScript(runId))
    let scriptStarted = false
    ...
    try {
      ...
      const payload = `FABRICA_E2E_CONTEXT_MENU_PASTE_${runId}`
      const encodedPayload = Buffer.from(payload, 'utf8').toString('base64')
      await clearTerminalPtyWriteLog(electronApp)
      await fabricaPage.evaluate((text) => window.api.ui.writeClipboardText(text), payload)
      await focusActiveTerminalInput(fabricaPage)
      ...
      await fabricaPage.getByRole('menuitem', { name: /Paste/ }).click()
      await waitForTerminalOutput(fabricaPage, encodedPayload, 10_000, 12_000)
  ...
  test('Windows multiline keyboard paste normalizes terminal newlines with one PTY owner', async ({
    ...
    const ptyId = await waitForActivePanePtyId(fabricaPage)
    const runId = randomUUID()
    const sentinel = `FABRICA_E2E_MULTILINE_DONE_${runId}`
    const payload = [
      `FABRICA_E2E_MULTILINE_${runId}`,
    ...
    // FABRICA's direct Windows bracketed-paste path must produce the same bytes.
    const terminalText = payload.replace(/\r?\n/g, '\r')
    const scriptPath = path.join(testRepoPath, `.FABRICA-paste-multiline-${runId}.mjs`)
    ...
    try {
      ...
      await fabricaPage.keyboard.press('Control+V')
      await waitForTerminalOutput(fabricaPage, `PASTE_COMPLETE_${runId}:MATCH`, 10_000, 12_000)
  ...
  test('right-click paste sends clipboard text to the focused terminal exactly once', async ({
    ...
    const ptyId = await waitForActivePanePtyId(fabricaPage)
    const runId = randomUUID()
    const scriptPath = path.join(testRepoPath, `.FABRICA-paste-right-click-${runId}.mjs`)
    writeFileSync(scriptPath, pasteEchoScript(runId))
    let scriptStarted = false
    ...
    try {
      ...
      const payload = `FABRICA_E2E_RIGHT_CLICK_PASTE_${runId}`
      const encodedPayload = Buffer.from(payload, 'utf8').toString('base64')
      await clearTerminalPtyWriteLog(electronApp)
      await fabricaPage.evaluate((text) => window.api.ui.writeClipboardText(text), payload)
      await focusActiveTerminalInput(fabricaPage)

      await rightClickActiveTerminalSurface(fabricaPage)
      await waitForTerminalOutput(fabricaPage, encodedPayload, 10_000, 12_000)
