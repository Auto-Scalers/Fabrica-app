...
test.describe('Docker SSH relay perf', () => {
  test.skip(!RUN_DOCKER_SSH, 'Set FABRICA_E2E_SSH_DOCKER=1 to run Docker-backed SSH relay perf.')
  ...
  test('keeps remote typing responsive while the Linux relay streams TUI output', async ({
    ...
    try {
      ...
      await execInTerminal(fabricaPage, ptyId, `node -e ${shellQuote(remoteTypingLoadScript(runId))}`)
      await waitForTerminalOutput(fabricaPage, `REMOTE_TUI_READY_${runId}`, 30_000, 80_000)
      const measurement = await measureRemoteTyping(fabricaPage, ptyId, runId)
      ...
      await stopRemoteLoad(fabricaPage, ptyId)
    } finally {
  ...
  test('keeps active remote typing responsive while a background SSH PTY stream is ACK-stalled', async ({
    ...
    try {
      ...
      await ensureTerminalVisible(fabricaPage, 45_000)
      await waitForActiveTerminalManager(fabricaPage, 60_000)
      backgroundPtyId = await waitForActivePanePtyId(fabricaPage, 60_000)
      ...
      await waitForTerminalOutput(fabricaPage, `REMOTE_ACK_FLOOD_READY_${runId}`, 30_000, 80_000)
      await holdSshPtyAckGate(fabricaPage, [backgroundPtyId])
      await fabricaPage.evaluate((ptyId) => window.api.pty.write(ptyId, 'g'), backgroundPtyId)

      await splitActiveTerminalPane(fabricaPage, 'vertical')
      await focusLastTerminalPane(fabricaPage)
      activePtyId = await waitForActivePanePtyId(fabricaPage, 60_000)
      ...
      await waitForTerminalOutput(fabricaPage, `REMOTE_TUI_READY_${activeRunId}`, 30_000, 80_000)
      const heldAckPressure = expect.poll(
        async () => (await readSshPtyAckGate(fabricaPage))?.heldAckChars ?? 0,
        {
          timeout: 30_000,
      ...
      const measurement = await measureRemoteTyping(fabricaPage, activePtyId, activeRunId)
      const ackGate = await readSshPtyAckGate(fabricaPage)
      const summary = `median=${measurement.medianLatencyMs.toFixed(
        1
      ...
      await releaseSshPtyAckGate(fabricaPage)
      const releasedAckGate = await readSshPtyAckGate(fabricaPage)
      expect(releasedAckGate?.heldAckChars ?? 0).toBe(0)
    } finally {
      await releaseSshPtyAckGate(fabricaPage).catch(() => undefined)
      if (activePtyId) {
        await stopRemoteLoad(fabricaPage, activePtyId).catch(() => undefined)
      }
      if (backgroundPtyId) {
        await stopRemoteLoad(fabricaPage, backgroundPtyId).catch(() => undefined)
      }
      cleanupDockerSshRelayTarget(target)
    }
  })
  ...
  test('keeps remote typing responsive while relay file streams and git churn are active', async ({
    ...
    try {
      ...
      await waitForSessionReady(fabricaPage)
      await waitForActiveWorktree(fabricaPage)
      const remote = await connectDockerSshRelayTarget(fabricaPage, target)
      await ensureTerminalVisible(fabricaPage, 45_000)
      await waitForActiveTerminalManager(fabricaPage, 60_000)
      const ptyId = await waitForActivePanePtyId(fabricaPage, 60_000)
      ...
      const loadFile = `/tmp/FABRICA-relay-load-${runId}.png`
      ...
      await waitForTerminalOutput(fabricaPage, `LOAD_FILES_READY_${runId}`, 60_000, 80_000)
      ...
      const measurement = await measureRemoteTyping(fabricaPage, ptyId, runId)
      const load = await fabricaPage.evaluate(() => {
        const state = (
  ...
  test('keeps an SSH workspace terminal usable after disconnect and reconnect', async ({
    ...
    try {
      ...
      await ensureTerminalVisible(fabricaPage, 45_000)
      await waitForActiveTerminalManager(fabricaPage, 60_000)
      const beforePtyId = await waitForActivePanePtyId(fabricaPage, 60_000)
      ...
      await execInTerminal(fabricaPage, beforePtyId, beforeCommand)
      await waitForTerminalOutput(fabricaPage, beforeMarker, 20_000, 60_000)
      ...
      await execInTerminal(fabricaPage, beforePtyId, recoveryCommand)
      await waitForTerminalOutput(fabricaPage, recoveryStartedMarker, 30_000, 80_000)

      await reconnectDockerSshRelayTarget(fabricaPage, remote.targetId)
      await ensureTerminalVisible(fabricaPage, 45_000)
      await waitForActiveTerminalManager(fabricaPage, 60_000)
      const afterPtyId = await waitForActivePanePtyId(fabricaPage, 60_000)
      await waitForTerminalOutput(fabricaPage, recoveryMarker, 30_000, 80_000)
      ...
      await execInTerminal(fabricaPage, afterPtyId, afterCommand)
      await waitForTerminalOutput(fabricaPage, afterMarker, 20_000, 60_000)
