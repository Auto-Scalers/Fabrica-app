...
test.describe('Codex hidden startup composer background', () => {
  test('restores the input background when a Codex worktree first becomes visible', async ({
    ...
    const hiddenTabId = await fabricaPage.evaluate(
    ...
    const hiddenPtyId = await waitForHiddenTabPtyId(fabricaPage, hiddenTabId)
    await expect
      .poll(() => mainSnapshotContains(fabricaPage, hiddenPtyId, marker), {
        timeout: 20_000,
    ...
    await expect
      .poll(
        async () => {
          try {
            const nextTarget = await readCodexStartupBackgroundTarget(fabricaPage, marker)
            target = nextTarget
    ...
    const visibleBackgroundPixels = await countVisibleBackgroundPixels(fabricaPage, target)
