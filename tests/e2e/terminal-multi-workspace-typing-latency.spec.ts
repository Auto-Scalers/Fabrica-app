 ...
 *
 * Scenarios are gated behind FABRICA_TYPING_BENCH=1 (they are benchmarks that
...
const BENCH_ENABLED = process.env.FABRICA_TYPING_BENCH === '1'
...
const LOAD_PANES = readPositiveInt('FABRICA_TYPING_BENCH_LOAD_PANES', 4)
const LOAD_RATE_KBPS = readPositiveInt('FABRICA_TYPING_BENCH_RATE_KBPS', 256)
const KEY_COUNT = readPositiveInt('FABRICA_TYPING_BENCH_KEYS', 32)
const KEY_CADENCE_MS = readPositiveInt('FABRICA_TYPING_BENCH_KEY_CADENCE_MS', 250)
const CPU_WORKERS = readPositiveInt('FABRICA_TYPING_BENCH_CPU_WORKERS', 0)
const BENCH_LABEL = process.env.FABRICA_TYPING_BENCH_LABEL ?? 'dev'
...
test.describe('Multi-workspace sustained typing latency bench', () => {
  ...
  test('baseline: paced typing with no agent load', async ({
    ...
    const probePath = path.join(testRepoPath, `.FABRICA-mwt-probe-${runId}.mjs`)
    const sidecarPath = path.join(testRepoPath, `.FABRICA-mwt-arrivals-${runId}.jsonl`)
    writeTypingEchoProbeScript(probePath, runId, sidecarPath)
    try {
      await resetDeliveryDebug(fabricaPage)
      await startTypingProbe(fabricaPage, typingPtyId, probePath, runId)
      const measurement = await measurePacedTyping(fabricaPage, runId, sidecarPath)
      writeBenchReport(
        testInfo,
        'baseline',
        measurement,
      ...
      await stopPtysQuietly(fabricaPage, [typingPtyId])
  ...
  test('typing under sustained hidden multi-workspace agent load', async ({
    ...
    test.skip(!BENCH_ENABLED, 'Bench-only: run via pnpm bench:multi-workspace-typing')
    await waitForSessionReady(fabricaPage)
    const typingWorktreeId = await waitForActiveWorktree(fabricaPage)
    const loadWorktreeId = (await getAllWorktreeIds(fabricaPage)).find((id) => id !== typingWorktreeId)
    ...
    const runId = randomUUID()
    const loadPath = path.join(testRepoPath, `.FABRICA-mwt-load-${runId}.mjs`)
    const probePath = path.join(testRepoPath, `.FABRICA-mwt-probe-${runId}.mjs`)
    const sidecarPath = path.join(testRepoPath, `.FABRICA-mwt-arrivals-${runId}.jsonl`)
    ...
    try {
      await switchToWorktree(fabricaPage, loadWorktreeId)
      loadPanes = await ensureActiveWorktreePaneLoad(fabricaPage, LOAD_PANES)
      await startSustainedLoadInPanes(fabricaPage, loadPanes, loadPath, runId, testRepoPath)

      await switchToWorktree(fabricaPage, typingWorktreeId)
      await expect
        .poll(() => getActiveWorktreeId(fabricaPage), { timeout: 10_000 })
      ...
      await switchToWorktree(fabricaPage, loadWorktreeId).catch(() => undefined)
      await stopPtysQuietly(
        fabricaPage,
        loadPanes.map((pane) => pane.ptyId)
      )
      await switchToWorktree(fabricaPage, typingWorktreeId).catch(() => undefined)
  ...
  test('typing under sustained visible split agent load', async ({
    ...
    try {
      ...
      await resetDeliveryDebug(fabricaPage)
      await startTypingProbe(fabricaPage, typingPane.ptyId, probePath, runId)
      const measurement = await measurePacedTyping(fabricaPage, runId, sidecarPath)
      writeBenchReport(
        testInfo,
