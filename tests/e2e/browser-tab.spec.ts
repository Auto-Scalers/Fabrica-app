...
test.describe('Browser Tab', () => {
   ...
   * - Browser works and also retains state when switching tabs etc.
   */
  test('creating a browser tab adds it and activates browser view', async ({ fabricaPage }) => {
    const worktreeId = (await getActiveWorktreeId(fabricaPage))!
    const browserTabsBefore = await getBrowserTabs(fabricaPage, worktreeId)

    await createBrowserTab(fabricaPage, worktreeId)

    // Wait for the browser tab to appear in the store
    await expect
      .poll(async () => (await getBrowserTabs(fabricaPage, worktreeId)).length, { timeout: 5_000 })
      .toBe(browserTabsBefore.length + 1)

    // The active tab type should switch to 'browser'
    await expect.poll(async () => getActiveTabType(fabricaPage), { timeout: 3_000 }).toBe('browser')
  })
   ...
   * - Browser works and also retains state when switching tabs etc.
   */
  test('browser tab is created and active in the store', async ({ fabricaPage }) => {
    const worktreeId = (await getActiveWorktreeId(fabricaPage))!

    await createBrowserTab(fabricaPage, worktreeId)
    await expect.poll(async () => getActiveTabType(fabricaPage), { timeout: 5_000 }).toBe('browser')

    // Verify the browser tab exists in the store
    const browserTabs = await getBrowserTabs(fabricaPage, worktreeId)
    ...
    const activeBrowserTabId = await fabricaPage.evaluate(() => {
      const store = window.__store
   ...
   * - Browser works and also retains state when switching tabs etc.
   */
  test('browser tab retains state when switching to terminal and back', async ({ fabricaPage }) => {
    const worktreeId = (await getActiveWorktreeId(fabricaPage))!
    ...
    await switchToTerminalTab(fabricaPage, worktreeId)
    await expect.poll(async () => getActiveTabType(fabricaPage), { timeout: 3_000 }).toBe('terminal')

    // Switch back to browser tab
    await switchToBrowserTab(fabricaPage, worktreeId, browserTabId!)
    await expect.poll(async () => getActiveTabType(fabricaPage), { timeout: 3_000 }).toBe('browser')

    // The browser tab should still exist with the same ID
    const browserTabsAfter = await getBrowserTabs(fabricaPage, worktreeId)
  ...
  test('browser webview form state survives switching between browser tabs', async ({
    ...
    try {
      ...
      await writeBrowserInputValue(fabricaPage, firstTab!.id, 'first typed value')
      ...
      await writeBrowserInputValue(fabricaPage, secondTab!.id, 'second typed value')
      ...
      await switchToBrowserTab(fabricaPage, worktreeId, firstTab!.id)
      await expect
        .poll(async () => readBrowserInputValue(fabricaPage, firstTab!.id), { timeout: 5_000 })
        .toBe('first typed value')

      await switchToBrowserTab(fabricaPage, worktreeId, secondTab!.id)
      await expect
        .poll(async () => readBrowserInputValue(fabricaPage, secondTab!.id), { timeout: 5_000 })
  ...
  test('browser page reload restores the configured 100% zoom', async ({ fabricaPage }) => {
    const formServer = await startBrowserFormServer()
    try {
      const worktreeId = (await getActiveWorktreeId(fabricaPage))!
      ...
      await expect
        .poll(async () => readBrowserInputValue(fabricaPage, browserTab!.id), { timeout: 5_000 })
        .not.toBeNull()

      const zoomLevels = await fabricaPage.evaluate(async (browserTabId) => {
  ...
  test('Cmd/Ctrl+0 resets a zoomed browser page to 100%', async ({ fabricaPage }) => {
    const formServer = await startBrowserFormServer()
    try {
      const worktreeId = (await getActiveWorktreeId(fabricaPage))!
      const browserTab = await createBrowserTab(
      ...
      await fabricaPage.evaluate(
        async ({ browserTabId, browserPageId, modifier }) => {
          ...
          window.dispatchEvent(
            new CustomEvent('FABRICA:browser-page-zoom', {
      ...
      await expect
        .poll(() =>
          fabricaPage.evaluate((browserTabId) => {
            const slot = document.querySelector(`[data-browser-overlay-tab-id="${browserTabId}"]`)
  ...
  test('reloading one browser tab does not adopt another tab zoom', async ({ fabricaPage }) => {
    ...
    try {
      const worktreeId = (await getActiveWorktreeId(fabricaPage))!
      const tabA = await createBrowserTab(fabricaPage, worktreeId, formServerA.url('Zoom A'), 'Zoom A')
      const tabB = await createBrowserTab(fabricaPage, worktreeId, formServerB.url('Zoom B'), 'Zoom B')
      expect(tabA?.id).toBeTruthy()
      expect(tabB?.id).toBeTruthy()
      for (const tab of [tabA, tabB]) {
        await expect
          .poll(async () => readBrowserInputValue(fabricaPage, tab!.id), { timeout: 5_000 })
          .not.toBeNull()
      }

      const levels = await fabricaPage.evaluate(
  ...
  test('plain links stay current while explicit new-tab gestures activate FABRICA tabs', async ({
    electronApp,
    fabricaPage
  }) => {
    ...
    try {
      ...
      const baseTabCount = await fabricaPage.locator('[data-tab-id]').count()
      await clickBrowserLink(fabricaPage, sourceTab!.id, '#external-link')

      const sourceTabLocator = fabricaPage.locator(`[data-tab-id="${sourceTab!.id}"]`)
      await expect(sourceTabLocator).toContainText('Linked destination', { timeout: 10_000 })
      await expect(fabricaPage.locator('[data-tab-id]')).toHaveCount(baseTabCount)

      await clickBrowserLink(fabricaPage, sourceTab!.id, '#return-link')
      await expect(sourceTabLocator).toContainText('Source page', { timeout: 10_000 })
      await clickBrowserLink(fabricaPage, sourceTab!.id, '#frame-link', {
      ...
      await clickBrowserLink(fabricaPage, sourceTab!.id, '#frame-modifier-link', {
        frameSelector: '#link-frame',
        modifiers: process.platform === 'darwin' ? ['meta'] : ['control']
      })
      await expectBrowserTabActive(fabricaPage, 'Frame modifier destination')
      await switchToBrowserTab(fabricaPage, worktreeId, sourceTab!.id)
      await clickBrowserLink(fabricaPage, sourceTab!.id, '#frame-middle-link', {
        button: 'middle',
        frameSelector: '#link-frame'
      })
      await expectBrowserTabActive(fabricaPage, 'Frame middle destination')
      await switchToBrowserTab(fabricaPage, worktreeId, sourceTab!.id)

      await clickBrowserLink(fabricaPage, sourceTab!.id, '#modifier-link', {
        modifiers: process.platform === 'darwin' ? ['meta'] : ['control']
      })
      await expectBrowserTabActive(fabricaPage, 'Modifier destination')
      await switchToBrowserTab(fabricaPage, worktreeId, sourceTab!.id)

      const tabCountBeforeCancelledClick = await fabricaPage.locator('[data-tab-id]').count()
      await clickBrowserLink(fabricaPage, sourceTab!.id, '#cancelled-link')
      await expect(
        fabricaPage.locator('[data-tab-id]').filter({ hasText: 'Click handled in page' })
      ).toBeVisible({ timeout: 10_000 })
      await expect(fabricaPage.locator('[data-tab-id]')).toHaveCount(tabCountBeforeCancelledClick)

      await clickBrowserLink(fabricaPage, sourceTab!.id, '#middle-link', { button: 'middle' })
      await expectBrowserTabActive(fabricaPage, 'Middle-click destination')
      await expect
  ...
  test('blocked window.close in a link-created tab does not break tab switching', async ({
    ...
    try {
      ...
      await clickBrowserLink(fabricaPage, sourceTab!.id, '#window-close-link')
      let closeTabId: string | null = null
      await expect
        .poll(async () => {
          const tabs = await getBrowserTabs(fabricaPage, worktreeId)
          closeTabId = tabs.find((tab) => tab.url === closeServer.url)?.id ?? null
          return closeTabId
        })
        .not.toBeNull()

      await fabricaPage.locator(`[data-tab-id="${neighboringTab!.id}"]`).click()
      await expect.poll(async () => getActiveTabType(fabricaPage), { timeout: 5_000 }).toBe('browser')
      await expect
        .poll(() => readBrowserWindowCloseStatus(fabricaPage, closeTabId!), { timeout: 5_000 })
  ...
  test('directly created browser tabs block window.close and remain usable', async ({
    ...
    try {
      ...
      await expect
        .poll(() => readBrowserWindowCloseStatus(fabricaPage, directTab!.id), { timeout: 5_000 })
        .toContain('window.close() was blocked')

      await expect
        .poll(
          () =>
            fabricaPage.evaluate(async (targetBrowserTabId) => {
   ...
   * - Browser works and also retains state when switching tabs etc.
   */
  test('browser tab retains state when switching worktrees and back', async ({ fabricaPage }) => {
