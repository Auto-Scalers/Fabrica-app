...
test.describe('Markdown add-review-note shortcut', () => {
  ...
  test('opens the review-note composer for the current selection in the rich editor', async ({
    ...
    try {
      ...
      const editor = await waitForRichMarkdownEditor(fabricaPage)
      await editor.click()
      await fabricaPage.keyboard.press('ControlOrMeta+A')

      await fabricaPage.keyboard.press('ControlOrMeta+Shift+A')

      await expect(fabricaPage.getByPlaceholder('Add note for the AI')).toBeVisible({
        timeout: 5_000
      })
    } finally {
  ...
  test('opens the composer for the current selection in the Monaco source editor', async ({
    ...
    try {
      ...
      const monaco = fabricaPage.locator('.monaco-editor').first()
  ...
  test('opens the inline composer for the selected block in the markdown preview', async ({
    ...
    try {
      ...
      await expect(fabricaPage.locator('[data-annotation-block-key]').first()).toBeVisible({
        timeout: 25_000
      })
  ...
  test('does not open the composer without a text selection', async ({ fabricaPage }, testInfo) => {
    const context = await getActiveWorktreeContext(fabricaPage)
    let filePath: string | null = null
    ...
    try {
      ...
      await expect(fabricaPage.getByPlaceholder('Add note for the AI')).toHaveCount(0)
    } finally {
      await cleanupMarkdownFixture(filePath)
    }
  })
