...
async function seedUntrackedFile(page: Page): Promise<SeededUntrackedFile> {
  return page.evaluate(async () => {
    ...
    const fileName = `FABRICA-discard-confirm-${Date.now()}.txt`
...
test.describe('Source Control discard confirmation', () => {
  ...
  test('deletes an untracked file without confirmation', async ({ fabricaPage }) => {
    const seededFile = await seedUntrackedFile(fabricaPage)
    await openSourceControl(fabricaPage)

    const row = fabricaPage
      .locator('[data-testid="source-control-entry"]')
      .filter({ hasText: seededFile.fileName })
    await expect(row).toBeVisible()

    await deleteUntrackedFileFromRow(row)
    await confirmPendingDelete(fabricaPage)

    await expect(
      fabricaPage.getByRole('dialog', { name: `Delete "${seededFile.fileName}"?` })
    ).toHaveCount(0)
    await expect(row).toHaveCount(0, { timeout: 10_000 })

    await refreshGitStatus(fabricaPage)
    await expect(
      fabricaPage.locator('[data-testid="source-control-entry"]').filter({
