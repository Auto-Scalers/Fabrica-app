...
async function openInstrumentedGitHubTasksPage(
  ...
  await page.evaluate(() => {
    ...
    const existingIssue = {
      ...
      state: 'open' as const,
      url: 'https://github.com/FABRICA/e2e/issues/999',
      labels: [],
    ...
    store.setState({
      repos: state.repos.map((candidate) =>
        candidate.id === repo.id
          ? { ...candidate, upstream: { owner: 'FABRICA', repo: 'e2e' } }
          : candidate
      ),
      settings: {
        ...state.settings,
...
test.describe('Tasks page', () => {
  ...
  test('opening the tasks view renders the tasks UI', async ({ fabricaPage }) => {
    await openTasksPage(fabricaPage)

    await expect
      .poll(async () => getStoreState<string>(fabricaPage, 'activeView'), { timeout: 5_000 })
      .toBe('tasks')

    await expect(fabricaPage.getByRole('button', { name: 'Close tasks' })).toBeVisible({
      timeout: 10_000
    })
    ...
    await expect
      .poll(
        async () => {
          renderedSources = await getRenderedTaskSources(fabricaPage)
    ...
    if (renderedSources.some((source) => source.source === 'github' && source.active)) {
      await expect(fabricaPage.getByRole('button', { name: 'Issues', exact: true })).toBeVisible()
      await expect(fabricaPage.getByRole('button', { name: 'PRs', exact: true })).toBeVisible()
      await expect(fabricaPage.getByRole('button', { name: 'Projects', exact: true })).toBeVisible()
      await expect(fabricaPage.getByPlaceholder(/Search GitHub (issues|PRs)/i)).toBeVisible()
    }
  })

  test('closing the tasks page returns to the previous view', async ({ fabricaPage }) => {
    const previousView = await getStoreState<string>(fabricaPage, 'activeView')
    ...
    await fabricaPage.getByRole('button', { name: 'Close tasks' }).click()
    ...
    await expect(fabricaPage.getByRole('button', { name: 'Close tasks' })).toHaveCount(0)
    if (previousView === 'terminal') {
      await expect(fabricaPage.locator('.xterm').first()).toBeVisible({ timeout: 5_000 })
    }
  })

  test('reopening restores the GitHub page and scroll position', async ({ fabricaPage }) => {
    await openMockedPaginatedGitHubTasks(fabricaPage)

    await fabricaPage.getByRole('button', { name: 'Page 28', exact: true }).click()
    await expect(fabricaPage.getByText('Issue page 28 item 1', { exact: true })).toBeVisible()

    const list = fabricaPage.locator('[data-task-list-scroll="github"]')
    ...
    const clampedRowsStyle = await fabricaPage.addStyleTag({
      content:
        '[data-task-list-scroll="github"] > .divide-y { max-height: 0 !important; overflow: hidden !important; }'
    })
    await openTasksPage(fabricaPage)

    await expect(fabricaPage.getByRole('button', { name: 'Page 28', exact: true })).toHaveAttribute(
      'aria-current',
      'page'
    )
    const restoredList = fabricaPage.locator('[data-task-list-scroll="github"]')
    ...
    await fabricaPage.getByText('Issue page 28 item 12', { exact: true }).click()
    await expect(restoredList).toHaveCount(0)
    await expect
      .poll(async () => {
        const position = await getStoreState<{ scrollTop: number }>(fabricaPage, 'taskListPosition')
        return position.scrollTop
      })
      .toBeGreaterThan(300)
    await fabricaPage.getByRole('button', { name: 'GitHub list', exact: true }).click()
    await expect(fabricaPage.getByText('Issue page 28 item 1', { exact: true })).toBeVisible()
    await expect
      .poll(() => restoredList.evaluate((element) => element.scrollTop))
      .toBeGreaterThan(300)
    ...
    await fabricaPage.getByRole('button', { name: 'Close tasks' }).click()
    const pendingRestoreStyle = await fabricaPage.addStyleTag({
      content:
        '[data-task-list-scroll="github"] > .divide-y { max-height: 0 !important; overflow: hidden !important; }'
    })
    await openTasksPage(fabricaPage)
    ...
    await fabricaPage.getByRole('button', { name: 'Page 1', exact: true }).click()
    await pendingRestoreStyle.evaluate((element) => element.remove())
    await expect(fabricaPage.getByRole('button', { name: 'Page 1', exact: true })).toHaveAttribute(
      'aria-current',
      'page'
    )
    await expect
    ...
    const permanentlyClampedRowsStyle = await fabricaPage.addStyleTag({
      content:
        '[data-task-list-scroll="github"] > .divide-y { max-height: 0 !important; overflow: hidden !important; }'
    })
    await openTasksPage(fabricaPage)
    ...
    await fabricaPage.waitForTimeout(5_500)
    await fabricaPage.getByRole('button', { name: 'Close tasks' }).click()
    await expect
      .poll(async () => {
        const position = await getStoreState<{ scrollTop: number }>(fabricaPage, 'taskListPosition')
  ...
  test('GitHub search waits for idle, keeps rows visible, and Enter does not double-fetch', async ({
    ...
    await openInstrumentedGitHubTasksPage(fabricaPage)

    const input = fabricaPage.getByPlaceholder('Search GitHub issues...')
    const existingIssue = fabricaPage.getByText('Existing GitHub issue', { exact: true })
    await expect(input).toBeVisible()
    await expect(existingIssue).toBeVisible()

    await input.fill('')
    await fabricaPage.waitForTimeout(800)
    await resetTaskSearchRequestProbe(fabricaPage)

    await input.pressSequentially('rate', { delay: 400 })

    expect(await readTaskSearchRequestProbe(fabricaPage)).toEqual({
      countQueries: [],
      fetchQueries: []
    })
    await expect(existingIssue).toBeVisible()
    ...
    await expect
      .poll(async () => readTaskSearchRequestProbe(fabricaPage), { timeout: 2_000 })
