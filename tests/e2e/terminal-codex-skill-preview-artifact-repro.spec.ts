...
const RUN_REPRO = process.env.FABRICA_E2E_CODEX_SKILL_PREVIEW_REPRO === '1'
const EXPECT_NO_ARTIFACTS = process.env.FABRICA_E2E_EXPECT_NO_CODEX_SKILL_PREVIEW_ARTIFACTS === '1'
const FULLSCREEN_MIN_SIZE = { width: 1200, height: 760 }
const MIN_REPRO_DIFF_RATIO = 0.006
const MAX_CLEAN_DIFF_RATIO = 0.0015
const FABRICA_REPO_PATH = realpathSync(process.cwd())
...
const CODEX_SKILL_PREVIEW_RE = /Press enter to insert|esc to close|electron|FABRICA-cli|FABRICA-emulator/i
const SETUP_PANE_ACTIVITY_RE = /install-FABRICA-skills|pnpm|Progress:|Packages:|Lockfile/i
...
async function addRealFABRICARepo(page: Page, repoPath: string): Promise<string> {
  return page.evaluate(async (repoPath) => {
    ...
    if (!repo) {
      throw new Error(`Real FABRICA repo did not load: ${repoPath}`)
    }
    ...
    if (!worktree) {
      throw new Error(`Real FABRICA worktree did not load: ${repoPath}`)
    }
...
async function createWorkspaceThroughComposer(page: Page, workspaceName: string): Promise<string> {
  ...
  await expect
    .poll(
      ...
      {
        timeout: 60_000,
        message: `Workspace ${workspaceName} did not appear in the real FABRICA repo`
      }
    )
    .not.toBeNull()
  ...
  await expect
    .poll(() => getActiveWorktreeId(page), {
      timeout: 30_000,
      message: 'Created real FABRICA workspace did not become active'
    })
    .toBe(createdId)
...
test.describe('Codex skill preview terminal artifact repro @headful', () => {
  ...
  test.skip(!RUN_REPRO, 'Set FABRICA_E2E_CODEX_SKILL_PREVIEW_REPRO=1 to run this repro.')
  ...
  test('captures the real FABRICA repo setup-split Codex skill preview overpaint before any click', async ({
    electronApp,
    fabricaPage
  }, testInfo) => {
    ...
    await setStableFullscreenWindow(electronApp, fabricaPage)
    await waitForSessionReady(fabricaPage)
    await addRealFABRICARepo(fabricaPage, FABRICA_REPO_PATH)
    ...
    const worktreeId = await createWorkspaceThroughComposer(fabricaPage, workspaceName)
    createdWorktreeIds.push(worktreeId)

    await ensureTerminalVisible(fabricaPage, 30_000)
    await waitForActiveTerminalManager(fabricaPage, 30_000)
    await waitForPaneCount(fabricaPage, 2, 60_000)
    await waitForPaneIdentitySnapshot(fabricaPage, 2)
    const webglActive = await forceTerminalWebgl(fabricaPage)
    test.skip(!webglActive, 'Codex skill preview artifact repro needs WebGL rendering')

    const leftPane = await focusLeftTerminalPane(fabricaPage)
    const rightPane = await getRightTerminalPane(fabricaPage)
    ...
    await dismissCodexPromptsIfPresent(fabricaPage, leftPane)
    await waitForPaneContent(fabricaPage, leftPane.tabId, leftPane.ptyId, CODEX_READY_RE, 60_000)
    await waitForPaneContent(
      fabricaPage,
      leftPane.tabId,
      leftPane.ptyId,
    ...
    await fabricaPage.waitForTimeout(CODEX_READY_SETTLE_MS)
    await waitForPaneVisibleContentChanges(fabricaPage, rightPane, 1, 8_000)

    await focusLeftTerminalPane(fabricaPage)
    await fabricaPage.keyboard.type('test $e', { delay: 70 })
    await waitForPaneContent(
      fabricaPage,
      leftPane.tabId,
      leftPane.ptyId,
    ...
    const evidence = await captureClickEvidence(fabricaPage, leftPane, testInfo)
