...
function largePastePayload(runId: string): string {
  return `FABRICA_LARGE_PASTE_${runId}_0123456789abcdef`.repeat(4096)
}
...
test.describe('large terminal paste responsiveness', () => {
  test('chunked keyboard paste keeps the renderer responsive while PTY writes are pending', async ({
    ...
    const scriptPath = path.join(testRepoPath, `.FABRICA-large-paste-${runId}.cjs`)
    ...
    try {
      ...
      scriptStarted = true
      await waitForTerminalOutput(fabricaPage, `LARGE_PASTE_READY_${runId}`, 10_000)
      ...
      await installRendererHeartbeat(fabricaPage)
      await focusActiveTerminalInput(fabricaPage)

      const pasteKey = fabricaPage.keyboard.press(keyboardPasteChord())
      await expect
        .poll(
          async () =>
      ...
      const heartbeatBefore = await readRendererHeartbeat(fabricaPage)
      await fabricaPage.waitForTimeout(150)
      const heartbeatAfter = await readRendererHeartbeat(fabricaPage)
      expect(heartbeatAfter).toBeGreaterThan(heartbeatBefore)

      await pasteKey
      await waitForTerminalOutput(fabricaPage, doneLine, 20_000, 12_000)
      ...
      await stopRendererHeartbeat(fabricaPage).catch(() => undefined)
      if (scriptStarted) {
