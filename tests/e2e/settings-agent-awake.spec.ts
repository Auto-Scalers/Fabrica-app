...
async function installPowerSaveBlockerProbe(electronApp: ElectronApplication): Promise<void> {
  await electronApp.evaluate(({ powerSaveBlocker }) => {
    ...
    if (root.__FABRICAAwakePowerProbe) {
      root.__FABRICAAwakePowerProbe.starts = []
      root.__FABRICAAwakePowerProbe.stops = []
      return
    }
    ...
    powerSaveBlocker.start = ((type) => {
      ...
      root.__FABRICAAwakePowerProbe?.starts.push({ type, id })
      return id
    }) as typeof powerSaveBlocker.start

    powerSaveBlocker.stop = ((id) => {
      root.__FABRICAAwakePowerProbe?.stops.push({ id })
      originalStop(id)
...
test.describe('Agent awake setting', () => {
  ...
  test('can be changed from Agents settings and persists through IPC', async ({ fabricaPage }) => {
    await openSettings(fabricaPage)
    await dismissTransientAnnouncement(fabricaPage)
    await fabricaPage.getByPlaceholder('Search settings').fill('awake')

    await expect(fabricaPage.getByText('Keep computer awake').first()).toBeVisible()

    const keepAwakeModes = fabricaPage.getByRole('radiogroup', {
    ...
    await expect
      .poll(async () => (await getSettings(fabricaPage)).computerAwakeMode, {
        timeout: 5_000,
  ...
  test('keeps the OS awake only while a hook-reported agent is working', async ({
    ...
    await setKeepAwake(fabricaPage, true)
