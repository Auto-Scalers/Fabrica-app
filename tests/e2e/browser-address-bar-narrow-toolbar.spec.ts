...
function addressBarInput(page: Page): Locator {
  return page.locator('[data-FABRICA-browser-address-bar="true"]')
}

function addressBarOverlay(page: Page): Locator {
  return page.locator('[data-FABRICA-browser-address-bar-overlay="true"]')
}
...
test.describe('Browser address bar in a narrow toolbar', () => {
  ...
  test('focusing the squeezed address bar expands a typable field that navigates', async ({
    ...
    try {
      ...
      await createBlankBrowserTab(fabricaPage, worktreeId)
      await settleToSqueezedRestingState(fabricaPage, electronApp)

      const overlay = addressBarOverlay(fabricaPage)
      // The bug: the inline field is squeezed away entirely.
      await expect.poll(() => addressBarInputWidth(fabricaPage), { timeout: 10_000 }).toBeLessThan(40)

      await fabricaPage.locator('form:has(> [data-FABRICA-browser-address-bar="true"])').click()

      await expect(overlay).toBeVisible()
      await expect
        .poll(() => addressBarInputWidth(fabricaPage), { timeout: 5_000 })
        .toBeGreaterThan(BROWSER_ADDRESS_BAR_MIN_INLINE_WIDTH / 2)

      await addressBarInput(fabricaPage).fill(destination.url)
      await addressBarInput(fabricaPage).press('Enter')

      await expect
        .poll(async () => (await getBrowserTabs(fabricaPage, worktreeId)).at(-1)?.url ?? null, {
          timeout: 15_000
        })
        .toContain('/typed')
    } finally {
