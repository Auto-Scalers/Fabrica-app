  ...
  for (const layout of LAYOUT_VARIANTS) {
    ...
    for (const arm of REMAP_ARMS) {
      ...
      test(`sends ${committed} for the ${layout.label} backquote key when ${arm.name}`, async ({
        ...
        await applyImePlatformPolicy(fabricaPage, 'mac')
        await expectImePlatformPolicy(fabricaPage, 'mac')
        const arena = await openTerminalImePaneArena(fabricaPage)
        const reader = createTerminalImeByteReader(testRepoPath, 1)
        let completed = false
        try {
          await startTerminalImeByteReader(fabricaPage, arena.ptyId, reader)
          ...
          const trace = await readTerminalImeBoundaryTrace(fabricaPage)
          ...
          const received = await waitForTerminalImeBytes(fabricaPage, reader)
