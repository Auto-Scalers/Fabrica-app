...
const SAME_WORKSPACE_PANES = readPositiveInt(
  'FABRICA_E2E_OPENCODE_SAME_WORKSPACE_PANES',
  DEFAULT_SAME_WORKSPACE_PANES
)
const CROSS_WORKSPACE_PANES_PER_WORKTREE = readPositiveInt(
  'FABRICA_E2E_OPENCODE_CROSS_WORKSPACE_PANES',
  DEFAULT_CROSS_WORKSPACE_PANES_PER_WORKTREE
)
const PRESSURE_BACKGROUND_PANES = readPositiveInt(
  'FABRICA_E2E_OPENCODE_PRESSURE_BACKGROUND_PANES',
  DEFAULT_PRESSURE_BACKGROUND_PANES
)
const PRESSURE_OUTPUT_CHARS = readPositiveInt(
  'FABRICA_E2E_OPENCODE_PRESSURE_OUTPUT_CHARS',
  DEFAULT_PRESSURE_OUTPUT_CHARS
)
const HIDDEN_PRESSURE_PANES = readPositiveInt(
  'FABRICA_E2E_OPENCODE_HIDDEN_PRESSURE_PANES',
  DEFAULT_HIDDEN_PRESSURE_PANES
)
const FRAME_COUNT = readPositiveInt('FABRICA_E2E_OPENCODE_FRAME_COUNT', DEFAULT_FRAME_COUNT)
const FRAME_INTERVAL_MS = readPositiveInt(
  'FABRICA_E2E_OPENCODE_FRAME_INTERVAL_MS',
  DEFAULT_FRAME_INTERVAL_MS
)
const SCALE_SAME_WORKSPACE_PANES = readPositiveIntList('FABRICA_E2E_OPENCODE_SCALE_PANES')
const SCALE_CROSS_WORKSPACE_PANES = readPositiveIntList(
  'FABRICA_E2E_OPENCODE_SCALE_CROSS_WORKSPACE_PANES'
)
const SCALE_PRESSURE_PANES = readPositiveIntList('FABRICA_E2E_OPENCODE_SCALE_PRESSURE_PANES')
const SCALE_HIDDEN_PRESSURE_PANES = readPositiveIntList(
  'FABRICA_E2E_OPENCODE_SCALE_HIDDEN_PRESSURE_PANES'
)
...
async function measureCrossWorkspaceTypingDuringHiddenLoad({
  ...
  const hiddenPanes = await ensureActiveWorktreePaneLoad(fabricaPage, hiddenPaneCount)

  await switchToWorktree(fabricaPage, firstWorktreeId)
  await expect.poll(() => getActiveWorktreeId(fabricaPage), { timeout: 10_000 }).toBe(firstWorktreeId)
  await ensureTerminalVisible(fabricaPage)
  await waitForActiveTerminalManager(fabricaPage, 30_000)
  const typingPtyId = await waitForActivePanePtyId(fabricaPage)

  const runId = randomUUID()
  const scriptPath = path.join(testRepoPath, `.FABRICA-opencode-cross-${hiddenPaneCount}-${runId}.mjs`)
  writeInteractivePromptScript(scriptPath, runId)
  await resetTerminalPtyOutputDebug(fabricaPage)
  ...
  try {
    const measurement = await measureTypingDuringLoad(fabricaPage, scriptPath, typingPtyId, runId)
    const debug = await readTerminalPtyOutputDebug(fabricaPage)
    const scheduler = await readTerminalOutputSchedulerDebug(fabricaPage)
    const mainPressure = await readMainPtyPressureDebug(fabricaPage)
    ...
    await load.stop()
    await sendToTerminal(fabricaPage, typingPtyId, '\x03').catch(() => undefined)
...
test.describe('Artificial OpenCode terminal load', () => {
  ...
  test('measures baseline typing responsiveness with one active terminal', async ({
    ...
    const runId = randomUUID()
    const scriptPath = path.join(testRepoPath, `.FABRICA-opencode-baseline-typing-${runId}.mjs`)
    writeInteractivePromptScript(scriptPath, runId)
    await resetTerminalPtyOutputDebug(fabricaPage)
    try {
      const measurement = await measureTypingDuringLoad(fabricaPage, scriptPath, typingPtyId, runId)
  ...
  test('keeps typing responsive while same-workspace panes redraw simultaneously', async ({
    ...
    await waitForSessionReady(fabricaPage)
    await waitForActiveWorktree(fabricaPage)
    const panes = await ensureActiveWorktreePaneLoad(fabricaPage, SAME_WORKSPACE_PANES)
    const [typingPane, ...loadPanes] = panes
    await focusPane(fabricaPage, typingPane.paneKey)

    const runId = randomUUID()
    const scriptPath = path.join(testRepoPath, `.FABRICA-opencode-typing-${runId}.mjs`)
    writeInteractivePromptScript(scriptPath, runId)
    await resetTerminalPtyOutputDebug(fabricaPage)
    const load = await startSyntheticOpenCodeInjection({
      frameCount: FRAME_COUNT,
    ...
    try {
      ...
      annotateTypingMeasurement(
        ...
        panes.length,
        measurement,
        await readTerminalPtyOutputDebug(fabricaPage),
        await readTerminalOutputSchedulerDebug(fabricaPage),
        await readMainPtyPressureDebug(fabricaPage)
      )
      expect(measurement.medianLatencyMs).toBeLessThan(MAX_MEDIAN_KEY_LATENCY_MS)
      expect(measurement.worstLatencyMs).toBeLessThan(MAX_WORST_KEY_LATENCY_UNDER_LOAD_MS)
      expect(measurement.maxTimerDriftMs).toBeLessThan(MAX_TIMER_DRIFT_UNDER_LOAD_MS)
      ...
      await load.stop()
      await sendToTerminal(fabricaPage, typingPane.ptyId, '\x03').catch(() => undefined)
      rmSync(scriptPath, { force: true })
    }
  })
  ...
  for (const paneCount of SCALE_SAME_WORKSPACE_PANES) {
    test(`keeps typing responsive at ${paneCount} same-workspace OpenCode panes`, async ({
      ...
      await waitForSessionReady(fabricaPage)
      await waitForActiveWorktree(fabricaPage)
      const panes = await ensureActiveWorktreePaneLoad(fabricaPage, paneCount)
      const [typingPane, ...loadPanes] = panes
      await focusPane(fabricaPage, typingPane.paneKey)

      const runId = randomUUID()
      const scriptPath = path.join(testRepoPath, `.FABRICA-opencode-scale-${paneCount}-${runId}.mjs`)
      writeInteractivePromptScript(scriptPath, runId)
      await resetTerminalPtyOutputDebug(fabricaPage)
      const load = await startSyntheticOpenCodeInjection({
        frameCount: FRAME_COUNT,
  ...
  for (const hiddenPressureCase of hiddenPressureCases) {
    test(hiddenPressureCase.title, async ({ fabricaPage, testRepoPath }, testInfo) => {
