...
function createCombinedDiffScrollRepo(): CombinedDiffScrollRepo {
  const repoPath = realpathSync(mkdtempSync(path.join(os.tmpdir(), 'FABRICA-combined-diff-scroll-')))
...
test.describe('Combined diff scroll restore', () => {
  ...
  test('keeps the visible section anchored after switching tabs', async ({ fabricaPage }) => {
    ...
    try {
      ...
      const diffTabId = await openCombinedDiff(fabricaPage, worktreeId, fixture.repoPath)
      await expect(fabricaPage.locator('.combined-diff-scroll-container')).toBeVisible()
      await expect(fabricaPage.getByText(`${FILE_COUNT} changed files`)).toBeVisible()

      await scrollCombinedDiffDeep(fabricaPage)
      await waitForStableViewportAnchor(fabricaPage)
      const activeScrollSamples = await wheelCombinedDiffDown(fabricaPage)
      ...
      const beforeSwitch = await waitForStableViewportAnchor(fabricaPage)
      ...
      await expect(fabricaPage.locator('.combined-diff-scroll-container')).toHaveCount(0)

      await fabricaPage.locator(`[data-tab-id="${diffTabId}"]`).click({ force: true })
      await expect(fabricaPage.locator('.combined-diff-scroll-container')).toBeVisible()
      const afterSwitch = await waitForRestoredViewportAnchor(fabricaPage, beforeSwitch)

      expect(afterSwitch.key).toBe(beforeSwitch.key)
      expect(Math.abs(afterSwitch.top - beforeSwitch.top)).toBeLessThan(80)

      await clickVisibleDiffLine(fabricaPage)
      const afterLineClick = await waitForStableViewportAnchor(fabricaPage)
