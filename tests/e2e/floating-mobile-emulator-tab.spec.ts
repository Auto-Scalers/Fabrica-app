...
test('floating Mobile Emulator tab renders content and closes from the tab strip', async ({
  ...
  const tab = await seedFloatingSimulatorTab(fabricaPage)
  await openFloatingPanelIfNeeded(fabricaPage)

  const openPanel = fabricaPage.locator(OPEN_PANEL_SELECTOR).first()
  ...
  await attachPanelScreenshot(fabricaPage, testInfo)
