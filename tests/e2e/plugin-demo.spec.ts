...
async function openDemoPanel(page: Page): Promise<void> {
  ...
  const panelButton = page.getByRole('button', { name: 'Hello FABRICA', exact: true })
...
test('runs hello-FABRICA panel, command, and event behind visible consent', async ({ fabricaPage }) => {
  const tempRoot = await mkdtemp(join(tmpdir(), 'FABRICA-hello-plugin-e2e-'))
  const pluginRoot = join(tempRoot, 'hello-FABRICA')
  let createdWorktreeId: string | null = null
  await cp(join(process.cwd(), 'examples', 'plugins', 'hello-FABRICA'), pluginRoot, {
    recursive: true
  })

  try {
    const installed = await fabricaPage.evaluate(async (sourcePath) => {
    ...
    await openPluginSettings(fabricaPage)
    await fabricaPage.getByRole('tab', { name: /^Installed/ }).click()
    const row = fabricaPage.locator(`[data-plugin-key="${installed.pluginKey}"]`)
    await expect(row).toContainText('Needs review')
    await row.getByRole('button', { name: 'Review & enable' }).click()
    const consent = fabricaPage.getByRole('dialog', { name: 'Review permissions' })
    ...
    const commandResults = await fabricaPage.evaluate(async (pluginKey) => {
    ...
    createdWorktreeId = await createWorktree(fabricaPage, `plugin-event-${Date.now()}`)
    await expect
      .poll(
        () =>
