...
test.describe('Default branch visibility', () => {
  ...
  test('keeps the default branch visible when sleeping workspaces are hidden', async ({
    ...
    const { defaultBranchId, featureId } = await seedSidebarVisibilityScenario(fabricaPage)
    const defaultBranchRow = worktreeRow(fabricaPage, defaultBranchId)
    const featureRow = worktreeRow(fabricaPage, featureId)
