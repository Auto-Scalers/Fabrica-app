...
export const TEST_REPO_PATH_FILE = path.join(os.tmpdir(), 'FABRICA-e2e-test-repo-path.txt')
...
export default function globalSetup(): void {
  ...
  if (
    ...
    // Why: the SSH specs deploy FABRICA's relay from out/relay. The
  ...
  if (process.env.FABRICA_E2E_SSH_DOCKER === '1' || process.env.FABRICA_E2E_NESTED_RUNTIME_SSH === '1') {
  ...
  const testRepoDir = realpathSync(mkdtempSync(path.join(os.tmpdir(), 'FABRICA-e2e-repo-')))
  ...
  const worktreeDir = path.join(testRepoDir, '..', `FABRICA-e2e-worktree-${randomUUID()}`)
