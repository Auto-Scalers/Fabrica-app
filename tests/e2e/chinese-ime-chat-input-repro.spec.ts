...
function emitState(reason) {
  ...
  process.stdout.write('\\x1b]1337;FABRICAImeState=' + Buffer.from(JSON.stringify({
    reason,
    model,
    cursor,
    submitted
...
async function installImeEventProbe(page: Page): Promise<void> {
  await page.evaluate(() => {
    const targetWindow = window as unknown as { __FABRICAImeEventLog?: ImeEventLogEntry[] }
    targetWindow.__FABRICAImeEventLog = []
    ...
    const record = (event: Event): void => {
      ...
      targetWindow.__FABRICAImeEventLog!.push({
        type: event.type,
        at: performance.now(),
...
async function readImeEventLog(page: Page): Promise<ImeEventLogEntry[]> {
  return page.evaluate(() => {
    const targetWindow = window as unknown as { __FABRICAImeEventLog?: ImeEventLogEntry[] }
    return targetWindow.__FABRICAImeEventLog ?? []
  })
}
...
async function dispatchSogouEmptyCompositionUpdate(page: Page): Promise<void> {
  ...
  // popup is still open (#6765); FABRICA's tracker must not flip inactive on it.
  await page.evaluate(() => {
    const active = document.activeElement
    if (!(active instanceof HTMLTextAreaElement)) {
      throw new Error('xterm helper textarea is not focused')
...
async function dispatchSogouPostCompositionEnd(page: Page, data: string): Promise<void> {
  ...
  // compositionend; target the terminal element so FABRICA's tracker sees the end
...
test.describe('Chinese IME terminal chat input repro', () => {
  test('keeps composed Chinese text, cursor movement, and Backspace stable in the agent input surface', async ({
    ...
    const scriptPath = path.join(testRepoPath, `.FABRICA-chinese-ime-harness-${runId}.cjs`)
    writeFileSync(scriptPath, terminalImeHarnessScript(runId))
    const session = await fabricaPage.context().newCDPSession(fabricaPage)

    try {
      await sendToTerminal(fabricaPage, ptyId, `node ${JSON.stringify(scriptPath)}\r`)
      await waitForTerminalOutput(fabricaPage, `IME_HARNESS_READY_${runId}`, 10_000, 20_000)
      ...
      await expect
        .poll(async () => (await readPromptState(fabricaPage))?.submitted.at(-1) ?? null, {
          timeout: 5_000,
      ...
      await attachImeEvidence(fabricaPage, testInfo, 'final-ime-evidence').catch(() => undefined)
      await session.detach().catch(() => undefined)
      await sendToTerminal(fabricaPage, ptyId, '\x03').catch(() => undefined)
  ...
  test('commits the active Pinyin preedit when Shift toggles the Windows IME', async ({
    ...
    await reloadWithWindowsImePolicy(fabricaPage)
    await waitForSessionReady(fabricaPage)
    await waitForActiveWorktree(fabricaPage)
    await ensureTerminalVisible(fabricaPage)
    await waitForActiveTerminalManager(fabricaPage, 30_000)
    ...
    const ptyId = await waitForActivePanePtyId(fabricaPage)
    const runId = randomUUID()
    const scriptPath = path.join(testRepoPath, `.FABRICA-windows-shift-ime-${runId}.cjs`)
    const inputLogPath = path.join(testRepoPath, `.FABRICA-windows-shift-ime-${runId}.jsonl`)
    ...
    try {
      ...
      await waitForLivePrompt(fabricaPage, 's')
      await expect.poll(() => readActiveCompositionText(fabricaPage)).toBe('')
      await expect.poll(async () => (await readPromptState(fabricaPage))?.submitted).toEqual([])
      await expect
        .poll(() => readPtyInputs(inputLogPath).slice(inputCountBeforeShift))
        .toEqual(['s'])
    } finally {
      await attachImeEvidence(fabricaPage, testInfo, 'windows-shift-ime-evidence').catch(
        () => undefined
      )
      await session.detach().catch(() => undefined)
      await sendToTerminal(fabricaPage, ptyId, '\x03').catch(() => undefined)
  ...
  test('keeps Sogou-style candidate selection keys out of the PTY while committing Chinese text', async ({
    ...
    await reloadWithLinuxImePolicy(fabricaPage)
    await waitForSessionReady(fabricaPage)
    await waitForActiveWorktree(fabricaPage)
    await ensureTerminalVisible(fabricaPage)
    await waitForActiveTerminalManager(fabricaPage, 30_000)
    ...
    const ptyId = await waitForActivePanePtyId(fabricaPage)
    const runId = randomUUID()
    const scriptPath = path.join(testRepoPath, `.FABRICA-sogou-ime-harness-${runId}.cjs`)
    ...
    try {
      ...
      writeFileSync(scriptPath, terminalImeHarnessScript(runId))
      session = await fabricaPage.context().newCDPSession(fabricaPage)
      await sendToTerminal(fabricaPage, ptyId, `node ${JSON.stringify(scriptPath)}\r`)
      harnessStarted = true
      await waitForTerminalOutput(fabricaPage, `IME_HARNESS_READY_${runId}`, 10_000, 20_000)
      await focusActiveTerminalInput(fabricaPage)
      ...
      const postCompositionLogStart = (await readImeEventLog(fabricaPage)).length
      ...
      await attachImeEvidence(fabricaPage, testInfo, 'sogou-after-post-composition-digit-commit')
      await fabricaPage.keyboard.press('Enter')
      await expect
        .poll(async () => (await readPromptState(fabricaPage))?.submitted.at(-1) ?? null, {
          timeout: 5_000,
      ...
      await expect
        ...
        .toBe('')
      await attachImeEvidence(fabricaPage, testInfo, 'sogou-after-orphan-keyup-digit-suppression')

      const promptState = await readPromptState(fabricaPage)
      expect(
  ...
  test('keeps composed Chinese text stable in the real Codex TUI input @real-codex-ime', async ({
    ...
    test.skip(
      process.env.FABRICA_E2E_REAL_CODEX_IME !== '1',
      'Set FABRICA_E2E_REAL_CODEX_IME=1 to exercise the locally installed Codex TUI'
    )
    ...
    try {
      await launchCodexTui(fabricaPage, ptyId)
      await installImeEventProbe(fabricaPage)
      ...
      await attachImeEvidence(fabricaPage, testInfo, 'codex-after-composition-backspace', {
        cleanTerminal: stripTerminalControls(await getTerminalContent(fabricaPage, 20_000))
      })

      const cleanTerminal = stripTerminalControls(await getTerminalContent(fabricaPage, 20_000))
      expect(
        cleanTerminal,
      ...
      await session.detach().catch(() => undefined)
      await sendToTerminal(fabricaPage, ptyId, '\x03/quit\r').catch(() => undefined)
    }
  })
})
