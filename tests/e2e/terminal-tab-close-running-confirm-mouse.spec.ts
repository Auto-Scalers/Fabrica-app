...
test('middle-clicking a busy tab prompts, and cancelling keeps the tab', async ({ fabricaPage }) => {
  test.setTimeout(120_000)
  const busyTabId = await startBusyTerminal(fabricaPage)
  const busyTab = fabricaPage.locator(`${SORTABLE_TAB}[data-tab-id="${busyTabId}"]`).first()
  const tabsBefore = await fabricaPage.locator(SORTABLE_TAB).count()

  await busyTab.click({ button: 'middle' })

  await expect(closeDialogTitle(fabricaPage)).toBeVisible({ timeout: 15_000 })
  await fabricaPage.getByRole('button', { name: /^Cancel$/ }).click()
  await expect(closeDialogTitle(fabricaPage)).toBeHidden()
  await expect(busyTab).toBeVisible()
  expect(await fabricaPage.locator(SORTABLE_TAB).count()).toBe(tabsBefore)
})

test('confirming the X-button prompt closes the busy tab', async ({ fabricaPage }) => {
  test.setTimeout(120_000)
  const busyTabId = await startBusyTerminal(fabricaPage)
  const busyTab = fabricaPage.locator(`${SORTABLE_TAB}[data-tab-id="${busyTabId}"]`).first()
  ...
  await expect(closeDialogTitle(fabricaPage)).toBeVisible({ timeout: 15_000 })
  await fabricaPage.getByRole('button', { name: /^Stop and Close$/ }).click()

  await expect(busyTab).toHaveCount(0, { timeout: 15_000 })
  await expect(closeDialogTitle(fabricaPage)).toBeHidden()
})

test('Cmd+W on a busy single-pane tab raises exactly one dialog', async ({ fabricaPage }) => {
  test.setTimeout(120_000)
  const busyTabId = await startBusyTerminal(fabricaPage)
  const busyTab = fabricaPage.locator(`${SORTABLE_TAB}[data-tab-id="${busyTabId}"]`).first()

  await focusActiveTerminalInput(fabricaPage)
  await fabricaPage.keyboard.press(process.platform === 'darwin' ? 'Meta+w' : 'Control+w')
  await expect(closeDialogTitle(fabricaPage)).toBeVisible({ timeout: 15_000 })
  await fabricaPage.getByRole('button', { name: /^Stop and Close$/ }).click()
