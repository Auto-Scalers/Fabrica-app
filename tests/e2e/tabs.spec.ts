...
test.describe('Tabs', () => {
   ...
   */
  test('clicking "+" then "New Terminal" creates a new terminal tab', async ({ fabricaPage }) => {
    const tabsBefore = await countRenderedTabs(fabricaPage)
    ...
    await fabricaPage.getByRole('button', { name: 'New tab' }).click({ force: true })
    // Why: the "+" dropdown uses Radix <DropdownMenuItem>, which exposes the
    // label text as the accessible name once the menu is open.
    const newTerminalMenuItem = fabricaPage.getByRole('menuitem', { name: /New Terminal/i }).first()
    ...
    await expect(tabLocator(fabricaPage, storeActiveId!)).toBeVisible()
    await expect.poll(() => getDomActiveTabId(fabricaPage), { timeout: 3_000 }).toBe(storeActiveId)
    await expect
      .poll(() => getFocusedTerminalTabId(fabricaPage), {
        timeout: 5_000,
   ...
   * - New tab works
   */
  test('Cmd/Ctrl+T creates a new terminal tab', async ({ fabricaPage }) => {
  ...
  test('Cmd/Ctrl+Shift+] and Cmd/Ctrl+Shift+[ switch between tabs', async ({ fabricaPage }) => {
    ...
    if ((await countRenderedTabs(fabricaPage)) < 2) {
      ...
      await fabricaPage
        ...
        .click()
      await expect
        .poll(() => countRenderedTabs(fabricaPage), { timeout: 5_000 })
        .toBeGreaterThanOrEqual(2)
    }

    const firstTabId = await getActiveTabId(fabricaPage)
    const orderedTabs = await getWorktreeTabs(fabricaPage, worktreeId)
    ...
    await expect.poll(() => getDomActiveTabId(fabricaPage), { timeout: 3_000 }).toBe(firstTabId)
  })
   ...
   */
  test('dragging a tab to a new position reorders it', async ({ fabricaPage }) => {
    const worktreeId = (await getActiveWorktreeId(fabricaPage))!
    ...
    const domOrderBefore = await fabricaPage.$$eval(SORTABLE_TAB, (nodes) =>
    ...
    await expect
      .poll(
        async () =>
          fabricaPage.$$eval(SORTABLE_TAB, (nodes) =>
            nodes.map((n) => (n as HTMLElement).dataset.tabId ?? '')
          ),
  ...
  test('clicking tabs still switches after dragging a terminal tab to reorder', async ({
    ...
    await tabLocator(fabricaPage, firstTabId).click({ force: true })
    await expect.poll(() => getDomActiveTabId(fabricaPage), { timeout: 3_000 }).toBe(firstTabId)

    const firstTabBox = await tabLocator(fabricaPage, firstTabId).boundingBox()
    const secondTabBox = await tabLocator(fabricaPage, secondTabId).boundingBox()
    ...
    await fabricaPage.mouse.move(endX, endY, { steps: 8 })
    ...
    await tabLocator(fabricaPage, firstTabId).click({ force: true })
    await expect.poll(() => getDomActiveTabId(fabricaPage), { timeout: 3_000 }).toBe(firstTabId)
    await tabLocator(fabricaPage, secondTabId).click({ force: true })
    await expect
      .poll(() => getDomActiveTabId(fabricaPage), {
        timeout: 5_000,
  ...
  test('Cmd/Ctrl+Shift+[ walks tabs in drag-reordered order', async ({ fabricaPage }) => {
    ...
    const initialOrder = await getTabBarOrder(fabricaPage, worktreeId)
    ...
    await expect
      .poll(async () => getTabBarOrder(fabricaPage, worktreeId), { timeout: 3_000 })
      .toEqual([b, c, a])
    ...
    await expect.poll(() => getDomActiveTabId(fabricaPage), { timeout: 3_000 }).toBe(a)

    await fabricaPage.keyboard.press(`${mod}+Shift+BracketLeft`)
    await expect.poll(() => getDomActiveTabId(fabricaPage), { timeout: 3_000 }).toBe(c)
    await expect(tabLocator(fabricaPage, c)).toHaveAttribute('data-active', 'true')

    await fabricaPage.keyboard.press(`${mod}+Shift+BracketLeft`)
    await expect.poll(() => getDomActiveTabId(fabricaPage), { timeout: 3_000 }).toBe(b)
    await expect(tabLocator(fabricaPage, b)).toHaveAttribute('data-active', 'true')
  })
   ...
   * re-render.
   */
  test('closing a tab removes it from the tab bar', async ({ fabricaPage }) => {
    const worktreeId = (await getActiveWorktreeId(fabricaPage))!
    ...
    const tabsBefore = await countRenderedTabs(fabricaPage)
    const activeId = await getActiveTabId(fabricaPage)
    expect(activeId).not.toBeNull()
    const activeTab = tabLocator(fabricaPage, activeId!)
  ...
  test('closing the active tab activates a neighbor tab', async ({ fabricaPage }) => {
    ...
    const activeTabBefore = await getActiveTabId(fabricaPage)
    expect(activeTabBefore).not.toBeNull()

    const activeTab = tabLocator(fabricaPage, activeTabBefore!)
    await activeTab.hover()
    await activeTab.getByRole('button', { name: /^Close tab /i }).click()
    ...
    await expect.poll(() => getDomActiveTabId(fabricaPage), { timeout: 5_000 }).not.toBeNull()
  })
})
