...
test('mobile subscribe mounts overlay; collapse → chip; Take back dismisses', async ({
  ...
  const overlay = fabricaPage.locator('.mobile-driver-banner')
  await expect(overlay).toHaveCount(0)

  // Fire the IPC events main emits when a mobile client subscribes in 'auto'
  // mode (handleMobileSubscribe in src/main/runtime/FABRICA-runtime.ts). The
  ...
  await expectExpandedOverlayLeavesPaneReadable(fabricaPage, ptyId)
  ...
  await expectChipIsCompactInPane(fabricaPage, ptyId)

  await captureAttachment(fabricaPage, testInfo, 'overlay-collapsed.png')
...
test('held phone-fit state mounts restore overlay without collapse', async ({
  ...
  await captureAttachment(fabricaPage, testInfo, 'overlay-held-fit.png')
...
test('restore this terminal refits the active restored pane', async ({ fabricaPage, electronApp }) => {
  await waitForSessionReady(fabricaPage)
  await waitForActiveWorktree(fabricaPage)
  await ensureTerminalVisible(fabricaPage)
  await waitForActiveTerminalManager(fabricaPage)
  ...
  await expect(fabricaPage.locator('.mobile-driver-banner')).toHaveCount(1, { timeout: 15_000 })
  await expect
    .poll(() => getPaneTerminalCols(fabricaPage, ptyId), {
...
test('restore all refits non-focused restored terminal panes', async ({
  ...
  const ptyIds = await waitForVisiblePanePtyIds(fabricaPage, 2)
  const focusPtyId = await waitForActivePanePtyId(fabricaPage)
  ...
  await expect(fabricaPage.locator('.mobile-driver-banner')).toHaveCount(2, { timeout: 15_000 })

  await forcePaneToOneColumn(fabricaPage, inactivePtyId)
  await expect
    .poll(() => getPaneTerminalCols(fabricaPage, inactivePtyId), {
...
test('restore all recovers a hidden workspace held at narrow terminal geometry', async ({
  ...
  await ensureTerminalVisible(fabricaPage)
  await waitForActiveTerminalManager(fabricaPage)
  const hiddenWorkspacePtyId = await waitForActivePanePtyId(fabricaPage)
  await sendHeldPhoneFitIpc(electronApp, { ptyId: hiddenWorkspacePtyId, cols: 45, rows: 20 })
  await expect(fabricaPage.locator('.mobile-driver-banner')).toHaveCount(1, { timeout: 15_000 })

  await forcePaneToOneColumnAndSwitchWorktree(fabricaPage, hiddenWorkspacePtyId, secondWorktreeId)
  await expect
    .poll(() => getActiveWorktreeId(fabricaPage), {
      timeout: 5_000,
      message: 'second worktree should become active before restore-all'
    })
    .toBe(secondWorktreeId)
  await expect
    .poll(() => getPaneTerminalCols(fabricaPage, hiddenWorkspacePtyId), {
      message: 'test harness should hold workspace 1 in the bad narrow state'
    })
    .toBeLessThanOrEqual(2)
  await ensureTerminalVisible(fabricaPage)
  await waitForActiveTerminalManager(fabricaPage)
  const activeWorkspacePtyId = await waitForActivePanePtyId(fabricaPage)
  await installRestoreTerminalFitAutoRestoreRecorder(electronApp)
  await sendHeldPhoneFitIpc(electronApp, { ptyId: activeWorkspacePtyId, cols: 45, rows: 20 })
  await expect(
    fabricaPage.locator(`[data-pty-id="${activeWorkspacePtyId}"] .mobile-driver-banner`)
