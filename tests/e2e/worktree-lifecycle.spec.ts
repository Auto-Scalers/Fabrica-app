...
test.describe('Worktree Lifecycle', () => {
  ...
  test('removing a worktree clears its tabs, open files, and browser tabs', async ({
    ...
    const originalWorktreeId = await waitForActiveWorktree(fabricaPage)

    createdWorktreeId = await createIsolatedWorktree(fabricaPage)
    const newWorktreeId = createdWorktreeId
    await switchToWorktree(fabricaPage, newWorktreeId)
    await expect
    ...
    await fabricaPage.evaluate((worktreeId) => {
      const store = window.__store
      if (!store) {
        return
      }
    ...
    await clickFileInExplorer(fabricaPage, ['README.md', 'package.json'])

    // Baseline: the new worktree now has tabs/browser tabs/open files.
    expect((await getWorktreeTabs(fabricaPage, newWorktreeId)).length).toBeGreaterThan(0)
    ...
    await expect
      .poll(async () => (await getWorktreeTabs(fabricaPage, newWorktreeId)).length, {
        timeout: 10_000,
        message: 'tabsByWorktree still holds entries for the removed worktree'
      })
      .toBe(0)
    await expect
      .poll(async () => (await getBrowserTabs(fabricaPage, newWorktreeId)).length, { timeout: 5_000 })
      .toBe(0)
    await expect
      .poll(async () => (await getOpenFiles(fabricaPage, newWorktreeId)).length, { timeout: 5_000 })
      .toBe(0)
  ...
  test('switching worktrees preserves per-worktree state across a round-trip', async ({
    ...
    const originalState = await fabricaPage.evaluate((wId) => {
      const store = window.__store
      if (!store) {
    ...
    const afterRoundTrip = await fabricaPage.evaluate((wId) => {
      const store = window.__store
      if (!store) {
  ...
  test('terminal tabs stay scoped to the worktree that created them', async ({ fabricaPage }) => {
    ...
    const worktreeA = await waitForActiveWorktree(fabricaPage)
    ...
    await expect
      .poll(async () => (await getWorktreeTabs(fabricaPage, worktreeA)).length, { timeout: 5_000 })
      .toBeGreaterThanOrEqual(2)

    // Switch to B and create a tab there too.
    await switchToWorktree(fabricaPage, worktreeB)
    await expect
      .poll(async () => getActiveWorktreeId(fabricaPage), { timeout: 10_000 })
      .toBe(worktreeB)
    await ensureTerminalVisible(fabricaPage)
    ...
    await expect
      .poll(async () => (await getWorktreeTabs(fabricaPage, worktreeB)).length, { timeout: 5_000 })
      .toBeGreaterThanOrEqual(2)

    const tabsA = await getWorktreeTabs(fabricaPage, worktreeA)
    const tabsB = await getWorktreeTabs(fabricaPage, worktreeB)
