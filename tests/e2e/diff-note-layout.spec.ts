...
async function assertCardClearsFollowingLine(page: Page): Promise<void> {
  const card = page.locator('.FABRICA-diff-comment-card').first()
...
test.describe('Diff note layout', () => {
  ...
  test('saved notes reserve their rendered height in both diff layouts', async ({
    ...
    await fabricaPage.setViewportSize({ width: 1200, height: 800 })
    ...
    const added = await fabricaPage.evaluate(
    ...
    await expect(fabricaPage.locator('button:has(svg.lucide-rows-2)')).toBeVisible()
    await assertCardClearsFollowingLine(fabricaPage)
    await attachDiffScreenshot(fabricaPage, testInfo, 'side-by-side-diff-note-layout')

    await fabricaPage.evaluate(() =>
      window.__store?.getState().updateSettings({ diffDefaultView: 'inline' })
    )
    await expect(fabricaPage.locator('button:has(svg.lucide-columns-2)')).toBeVisible()
    await assertCardClearsFollowingLine(fabricaPage)
    await attachDiffScreenshot(fabricaPage, testInfo, 'inline-diff-note-layout')
  })
})
