...
test.describe('Activity Agent Pane Isolation', () => {
  test.beforeEach(async ({ fabricaPage }) => {
    ...
    await enableActivityAgentsView(fabricaPage)
  ...
  test('selecting agent rows isolates the matching split pane by stable leaf id', async ({
    ...
    const [first, second] = await seedActivityThreadsForSplitPanes(fabricaPage, snapshot)

    await agentsSidebarButton(fabricaPage).click()
    await expect(fabricaPage.getByText(first.prompt)).toBeVisible()
    await expect(fabricaPage.getByText(second.prompt)).toBeVisible()

    await fabricaPage.getByRole('button').filter({ hasText: first.prompt }).first().click()
    await expect
      .poll(async () => readActivityPaneVisibility(fabricaPage), {
        timeout: 10_000,
    ...
    await fabricaPage.getByRole('button').filter({ hasText: second.prompt }).first().click()
    await expect
      .poll(async () => readActivityPaneVisibility(fabricaPage), {
        timeout: 10_000,
  ...
  test('acknowledged stable pane keys clear the Agents unread badge', async ({ fabricaPage }) => {
    await splitActiveTerminalPane(fabricaPage, 'vertical')
    await waitForPaneCount(fabricaPage, 2)
    const snapshot = await waitForPaneIdentitySnapshot(fabricaPage, 2)
    ...
    await expect(agentsSidebarButton(fabricaPage)).toHaveAccessibleName(/^Agents\s+1$/)

    await fabricaPage.evaluate((paneKey) => {
      const store = window.__store
      if (!store) {
        throw new Error('window.__store is not available')
      }
    ...
    await expect(agentsSidebarButton(fabricaPage)).toHaveAccessibleName(/^Agents$/)
    await expect(fabricaPage.getByRole('button', { name: /^Agents\s+1$/ })).toHaveCount(0)
  })

  test('workspace card agent rows focus the matching terminal split pane', async ({ fabricaPage }) => {
    await splitActiveTerminalPane(fabricaPage, 'vertical')
    await waitForPaneCount(fabricaPage, 2)
    const snapshot = await waitForPaneIdentitySnapshot(fabricaPage, 2)
    const [first, second] = await seedActivityThreadsForSplitPanes(fabricaPage, snapshot)

    await enableInlineAgentCards(fabricaPage)

    await clickWorkspaceCardAgentRow(fabricaPage, first.prompt)
    await expect
      .poll(async () => readActivePaneSelection(fabricaPage), {
        timeout: 10_000,
    ...
    await clickWorkspaceCardAgentRow(fabricaPage, second.prompt)
    await expect
      .poll(async () => readActivePaneSelection(fabricaPage), {
        timeout: 10_000,
  ...
  test('workspace card agent rows reveal terminal logs from a non-terminal surface', async ({
    ...
    await waitForPaneCount(fabricaPage, 2)
    const snapshot = await waitForPaneIdentitySnapshot(fabricaPage, 2)
    const [first] = await seedActivityThreadsForSplitPanes(fabricaPage, snapshot)

    await enableInlineAgentCards(fabricaPage)
    await expect(terminalPaneForLeaf(fabricaPage, first.leafId)).toBeVisible()
    ...
    await expect(await clickFileInExplorer(fabricaPage, ['README.md'])).toBe('README.md')
    await expect
      .poll(() => readActivePaneSelection(fabricaPage))
      .toMatchObject({
        activeTabType: 'editor',
        activeTabId: snapshot.tabId
      })
    await expect(terminalPaneForLeaf(fabricaPage, first.leafId)).toBeHidden()
  ...
  test('workspace card agent rows focus the matching split-group terminal pane', async ({
    ...
    await splitActiveTerminalPane(fabricaPage, 'vertical')
    await waitForPaneCount(fabricaPage, 2)
    const firstGroupSnapshot = await waitForPaneIdentitySnapshot(fabricaPage, 2)
    const [first, second] = await seedActivityThreadsForSplitPanes(fabricaPage, firstGroupSnapshot)

    const splitGroup = await createTerminalInNewSplitGroup(fabricaPage)
    await waitForActiveTerminalManager(fabricaPage, 30_000)
    await waitForPaneCount(fabricaPage, 1, 30_000)
    const secondGroupSnapshot = await waitForPaneIdentitySnapshot(fabricaPage, 1)
    ...
    await clickWorkspaceCardAgentRow(fabricaPage, splitGroupThread.prompt)
    await expect
      .poll(async () => readActivePaneSelection(fabricaPage), {
        timeout: 10_000,
