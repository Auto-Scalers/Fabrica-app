...
test('keeps long repository names inside the cross-project confirmation dialog', async ({
  ...
  const tempRoot = mkdtempSync(path.join(os.tmpdir(), 'FABRICA-e2e-cross-project-dialog-'))
  const secondRepositoryPath = path.join(tempRoot, LONG_REPOSITORY_NAME)
  createGitRepository(secondRepositoryPath)

  try {
    const secondRepositoryId = await fabricaPage.evaluate(async (repositoryPath) => {
    ...
    const currentRepositoryId = await fabricaPage.evaluate(() => {
      const store = window.__store
    ...
    await fabricaPage.setViewportSize({ width: 640, height: 720 })
    ...
    const confirmation = fabricaPage.getByRole('dialog', { name: 'Switch project?' })
    await expect(confirmation).toBeVisible()
    await expect(confirmation).toContainText(LONG_REPOSITORY_NAME)

    if (process.env.FABRICA_VISUAL_PROOF === '1') {
