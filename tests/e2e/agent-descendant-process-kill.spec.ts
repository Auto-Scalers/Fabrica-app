...
test('killing an agent PTY terminates its detached-pgid descendants', async ({ fabricaPage }) => {
  ...
  const stage = mkdtempSync(join(tmpdir(), 'FABRICA-agent-descendant-'))
  ...
  try {
    ...
    const ptyId = await fabricaPage.evaluate(
    ...
    await fabricaPage.evaluate((id) => window.api.pty.kill(id), ptyId)
