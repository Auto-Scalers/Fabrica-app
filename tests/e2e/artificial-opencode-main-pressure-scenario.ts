...
>({
  ...
  await deps.waitForSessionReady(fabricaPage)
  await deps.waitForActiveWorktree(fabricaPage)
  const panes = await deps.ensureActiveWorktreePaneLoad(fabricaPage, backgroundPaneCount + 1)
  const [typingPane, ...loadPanes] = panes
  await deps.focusPane(fabricaPage, typingPane.paneKey)

  const runId = randomUUID()
  const scrollRunId = randomUUID()
  const typingScriptPath = path.join(testRepoPath, `.FABRICA-opencode-pressure-typing-${runId}.mjs`)
  const pressureScriptPath = path.join(testRepoPath, `.FABRICA-opencode-pressure-load-${runId}.mjs`)
  await seedActiveTerminalScrollback(fabricaPage, typingPane.ptyId, scrollRunId)
  ...
  try {
    ...
    const pressureBeforeTyping = await deps.waitForMainPtyPressureBacklog(fabricaPage)
    ...
    const mainPressure = await deps.readMainPtyPressureDebug(fabricaPage)
    ...
    deps.annotateTypingMeasurement(
      ...
      panes.length,
      measurement,
      await deps.readTerminalPtyOutputDebug(fabricaPage),
      scheduler,
      mainPressure,
      ackGate
    )
...
>({
  ...
  const scrollMeasurement = await measureActiveTerminalWheelScroll(fabricaPage)
  const mainPressureAfterScroll = await deps.readMainPtyPressureDebug(fabricaPage)
  const ackGateAfterScroll = await deps.readTerminalAckGateDebug(fabricaPage)
  annotateScrollMeasurement(
    testInfo,
  ...
  await scrollActiveTerminalToBottom(fabricaPage)
}
