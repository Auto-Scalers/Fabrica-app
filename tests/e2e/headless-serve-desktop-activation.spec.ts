...
import { _electron as electron, type ElectronApplication } from '@autoscalers/playwright-test'
import { test, expect, forwardElectronProcessLogs } from './helpers/fabrica-app'
import { TEST_REPO_PATH_FILE } from './global-setup'
import { getE2ECompletedOnboardingProfile } from './helpers/e2e-completed-onboarding-profile'
import { getFABRICAElectronLaunchArgs } from './helpers/electron-launch-args'
...
function createHeadlessLaunchIsolation(userDataDir: string): ElectronHomeIsolation {
  ...
  return createElectronHomeIsolation({
    ...
    launchEnv: {
      ...
      FABRICA_E2E_ENFORCE_SINGLE_INSTANCE_LOCK: '1'
    },
    extraEnv: {},
    userDataDir
  })
...
function readPersistedPromotionBinding(
  ...
  try {
    const persisted = JSON.parse(
      readFileSync(
        path.join(userDataDir, 'profiles', DEFAULT_LOCAL_FABRICA_PROFILE_ID, 'FABRICA-data.json'),
        'utf8'
      )
    ) as {
      workspaceSession?: {
...
{}, testInfo) => {
  ...
  const userDataDir = mkdtempSync(path.join(os.tmpdir(), 'FABRICA-e2e-serve-promotion-'))
  ...
  try {
    serveApp = await electron.launch({
      args: [...getFABRICAElectronLaunchArgs(mainPath, false), '--serve', '--serve-no-pairing'],
      env
    })
    ...
    const forwardAppLogs = process.env.FABRICA_E2E_FORWARD_APP_LOGS === '1'
    activatingProcess = spawn(electronPath, getFABRICAElectronLaunchArgs(mainPath, false), {
      env,
