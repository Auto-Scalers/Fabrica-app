...
async function captureEvidence(page: Page, name: string, locator?: Locator): Promise<void> {
  if (process.env.FABRICA_CAPTURE_EVIDENCE !== '1') {
    return
  }
...
test.describe('Worktree Lineage', () => {
  ...
  test('renders existing child lineage in the sidebar', async ({ fabricaPage }) => {
    const { parentId, childId } = await seedLineageScenario(fabricaPage)
    const parentRow = worktreeOption(fabricaPage, parentId)
    const childRow = worktreeOption(fabricaPage, childId)
    ...
    const positions = await fabricaPage.evaluate(
      ({ parentId, childId }) => {
    ...
    await fabricaPage.evaluate(async (childId) => {
  ...
  test('renders legacy-only inline lineage when side-map hydration is absent', async ({
    ...
    const { parentId, childId } = await seedLineageScenario(fabricaPage, { inlineOnly: true })
    const parentRow = worktreeOption(fabricaPage, parentId)
    const childRow = worktreeOption(fabricaPage, childId)
    ...
    await expect
      ...
      .toBe(true)
    await captureSidebarEvidence(fabricaPage, 'legacy-inline-lineage-nested.png')
  })
  ...
  test('updates nested child preview status when the child terminal sleeps', async ({
    ...
    const childTabId = await seedWorkspaceLiveTerminal(fabricaPage, childId)
    await expect(childRow).toContainText('Active')

    await markWorkspaceTerminalSlept(fabricaPage, { worktreeId: childId, tabId: childTabId })
  ...
  test('sleeps a workspace and every descendant from the parent context menu', async ({
    ...
    const { parentId, childId } = await seedLineageScenario(fabricaPage)
    await fabricaPage.evaluate((parentId) => {
      const store = window.__store
      if (!store) {
        throw new Error('window.__store is not available')
      }
    ...
    const parentTabId = await seedWorkspaceLiveTerminal(fabricaPage, parentId)
    const childTabId = await seedWorkspaceLiveTerminal(fabricaPage, childId)
    ...
    await worktreeOption(fabricaPage, parentId).click({ button: 'right' })
    const sleepSubtree = fabricaPage.getByRole('menuitem', {
  ...
  test('shows parent and child agent rows while the parent workspace is active', async ({
    ...
    const parentAgentPrompt = await seedWorkspaceAgentStatus(fabricaPage, parentId, 'PARENT')
    const childAgentPrompt = await seedWorkspaceAgentStatus(fabricaPage, childId, 'CHILD')
