...
async function commitRepository(
  ...
  await runGit(
    ...
    [
      '-c',
      'user.name=FABRICA Test',
      '-c',
      'user.email=FABRICA-test@example.invalid',
      'commit',
      '--quiet',
      '-m',
      'fixture'
...
async function configureFixtureGit(home: string, repositories: string): Promise<NodeJS.ProcessEnv> {
  ...
  const entries = [
    [`url.${repositoryBaseUrl}.insteadOf`, 'https://github.com/Auto-Scalers/'],
...
async function createMarketplaceFixture(): Promise<MarketplaceFixture> {
  const root = await mkdtemp(join(tmpdir(), 'FABRICA-marketplace-e2e-'))
  ...
  await copyLaunchPlugin(
    repositories,
    'FABRICA-multipass-recipes',
    'autoscalers.FABRICA-multipass-recipes',
    gitEnvironment
  )
  await copyLaunchPlugin(
    repositories,
    'FABRICA-navigation-shortcuts',
    'autoscalers.FABRICA-navigation-shortcuts',
    gitEnvironment
  )

  const marketplaceRepository = join(repositories, 'FABRICA-plugins.git')
  await mkdir(marketplaceRepository, { recursive: true })
  await writeFile(
    join(marketplaceRepository, 'FABRICA-marketplace.json'),
    `${JSON.stringify(
      {
        name: 'FABRICA Plugins',
        owner: 'autoscalers',
        plugins: [
          ['autoscalers.FABRICA-portuguese', 'FABRICA-portuguese', 'languages'],
          ['autoscalers.FABRICA-multipass-recipes', 'FABRICA-multipass-recipes', 'vm-recipes'],
          ['autoscalers.FABRICA-navigation-shortcuts', 'FABRICA-navigation-shortcuts', 'keybindings']
        ].map(([id, repository, category]) => ({
          id,
          source: {
            kind: 'git',
            url: `https://github.com/Auto-Scalers/${repository}.git`,
            ref: 'v1.0.0'
          },
          categories: [category]
        }))
...
async function applyInstalledLanguage(page: Page): Promise<void> {
  const languageId = 'plugin:autoscalers.FABRICA-portuguese/pt-BR'
  await page.evaluate(() => {
    const state = window.__store?.getState()
    if (!state) {
      throw new Error('store unavailable')
  ...
  await expect
    ...
    .toMatchObject({
      ...
      listings: expect.arrayContaining([
        expect.objectContaining({ pluginKey: 'autoscalers.FABRICA-portuguese', official: true }),
        expect.objectContaining({ pluginKey: 'autoscalers.FABRICA-multipass-recipes', official: true }),
        expect.objectContaining({
          pluginKey: 'autoscalers.FABRICA-navigation-shortcuts',
          official: true
        })
      ])
    })
