...
const EMOJI_TABLE_MARKER = 'FABRICA_EMOJI_TABLE_RENDER_DONE'
...
test.describe('OpenCode emoji table terminal rendering', () => {
  test('keeps emoji table output visually sane and restores the cursor', async ({
    ...
    await waitForPtyShellEcho(fabricaPage, ptyId, 20_000)
    const runId = randomUUID()
    const marker = `${EMOJI_TABLE_MARKER}_${runId}`
    const scriptPath = path.join(testRepoPath, `.FABRICA-opencode-emoji-table-${runId}.mjs`)
    ...
    try {
      ...
      await forceCursorProbeTheme(fabricaPage)
      await fabricaPage.waitForTimeout(50)

      const renderState = await readActiveTerminalRenderState(fabricaPage)
      const blinkSamples = await sampleCursorBlink(fabricaPage)
  ...
  test('local real OpenCode demo keeps table rendering and cursor visible', async ({
    ...
    test.skip(
      process.env.FABRICA_E2E_REAL_OPENCODE !== '1',
      'Set FABRICA_E2E_REAL_OPENCODE=1 to exercise the locally installed OpenCode TUI'
    )
    ...
    try {
      await waitForTerminalOutput(fabricaPage, 'Give me markdown table', 15_000)
      await waitForTerminalOutput(fabricaPage, 'Emoji', 60_000)
      await waitForTerminalOutput(fabricaPage, 'Alice', 60_000)
      await fabricaPage.waitForTimeout(1_500)

      await testInfo.attach('real-opencode-demo-table', {
        body: await fabricaPage.screenshot({ fullPage: true }),
