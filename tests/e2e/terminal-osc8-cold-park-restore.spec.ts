...
test('restores and opens an OSC 8 link after its terminal is cold-parked', async ({ fabricaPage }) => {
  ...
  const url = `https://example.com/FABRICA-osc8-${randomUUID()}`
  ...
  await expect.poll(() => getTerminalContent(fabricaPage, 4_000)).toContain(label)

  const baselineProbe = await locateLink(fabricaPage, label)
  await fabricaPage.mouse.move(baselineProbe.clientX, baselineProbe.clientY)
  await expect
    .poll(() => readLinkState(fabricaPage, tabId, label))
    .toMatchObject({
  ...
  const restoredProbe = await locateLink(fabricaPage, label)
  await fabricaPage.mouse.move(restoredProbe.clientX, restoredProbe.clientY)
  await expect
    .poll(() => readLinkState(fabricaPage, tabId, label))
    .toMatchObject({
      bufferType: 'alternate',
  ...
  await fabricaPage.keyboard.down(modifier)
  await fabricaPage.mouse.down()
  await fabricaPage.mouse.up()
  await fabricaPage.keyboard.up(modifier)
  await expect
    .poll(async () => (await getBrowserTabs(fabricaPage, worktreeId)).some((tab) => tab.url === url))
    .toBe(true)
})
