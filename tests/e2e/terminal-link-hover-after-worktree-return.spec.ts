 ...
 * FABRICA's file-path provider resolves link candidates asynchronously.
 */
...
test.describe('Terminal link hover after worktree return', () => {
  ...
  test('re-establishes a URL link on hover after the pointer leaves the terminal', async ({
    ...
    const url = `https://example.com/FABRICA-link-${randomUUID()}`
    await sendToTerminal(fabricaPage, ptyId, `echo ${url}\r`)
    await expect
      .poll(() => getTerminalContent(fabricaPage, 4000), {
        timeout: 10_000,
    ...
    const probe = await locateHoverProbe(fabricaPage, url)
    await expect
      .poll(() => hoverAndReadActiveLinkText(fabricaPage, probe), {
        timeout: 5_000,
        message: 'baseline hover never established the URL link'
      })
      .toContain(url)

    await dispatchScreenMouseLeave(fabricaPage, probe.tabId)
    await expect.poll(() => readActiveLinkText(fabricaPage, probe.tabId)).toBeNull()
    await expect.poll(() => readTerminalCursor(fabricaPage, probe.tabId)).not.toBe('pointer')
    ...
    await expect
      ...
      .toContain(url)
    await expect.poll(() => readTerminalCursor(fabricaPage, probe.tabId)).toBe('pointer')
  })
  ...
  test('re-establishes a file-path link on hover after switching worktrees and back', async ({
    ...
    const worktreePath = await activeWorktreePath(fabricaPage)
    const fileName = `FABRICA-linkfile-${randomUUID().slice(0, 8)}.txt`
    const filePath = path.join(worktreePath, fileName)
    writeFileSync(filePath, 'FABRICA file link target\n')
    const needle = `./${fileName}`

    try {
      await sendToTerminal(fabricaPage, ptyId, `echo ${needle}\r`)
      await expect
        .poll(() => getTerminalContent(fabricaPage, 4000), {
          timeout: 10_000,
      ...
      const probe = await assertLinkRecoversAfterReturn(fabricaPage, {
        firstWorktreeId,
        secondWorktreeId,
        needle,
        expectContains: fileName
      })
      await activateHoveredLink(fabricaPage, probe)
      ...
      await expect(fabricaPage.locator('.editor-header-path').first()).toContainText(fileName, {
        timeout: 20_000
      })
    } finally {
      await fabricaPage.evaluate((filePath) => {
        const state = window.__store?.getState()
  ...
  test('re-establishes a URL link on hover after switching worktrees and back', async ({
    ...
    await assertLinkRecoversAfterReturn(fabricaPage, {
      firstWorktreeId,
      secondWorktreeId,
      needle: url,
      expectContains: url
