...
test.describe('SSH terminal hidden view parking', () => {
  ...
  test('parks a hidden SSH tab and restores its scrollback on reveal', async ({
    ...
    try {
      ...
      const sshPtyId = await waitForActivePanePtyId(fabricaPage, 60_000)
      const sshTabId = await getActiveTabId(fabricaPage)
      if (!sshTabId) {
      ...
      await parkHiddenTabBehindDecoy(fabricaPage, remote.worktreeId, sshTabId, {
      ...
      await expect
        .poll(() => getTerminalContent(fabricaPage, 2_000_000), {
          timeout: 15_000,
