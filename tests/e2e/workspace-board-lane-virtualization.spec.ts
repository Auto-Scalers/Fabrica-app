...
test.describe('Workspace board lane virtualization', () => {
  ...
  test('mounts a window of cards for a large lane and keeps lane indexes', async ({ fabricaPage }) => {
    ...
    await fabricaPage.getByRole('button', { name: 'Workspace board' }).click()

    const cards = fabricaPage.locator('[data-workspace-board-card-id]')
  ...
  test('renders later lane indexes after the lane scrolls', async ({ fabricaPage }) => {
    await fabricaPage.evaluate((count) => {
      const store = window.__store
      if (!store) {
        throw new Error('window.__store is not available')
  ...
  test('bounds mounted lanes and cards while preserving a 21-status workflow', async ({
    ...
    const board = fabricaPage.locator('[data-workspace-board-selection-surface]')
    ...
    await expect
      .poll(() => fabricaPage.evaluate(() => window.__store?.getState().workspaceBoardColumnWidth))
      .toBe(328)
  ...
  test('selects the full lane across a single large marquee scroll jump', async ({ fabricaPage }) => {
    ...
    const lane = fabricaPage.locator(`[data-workspace-status="${statusId}"]`)
    ...
    const emptyLaneScroll = fabricaPage.locator(
    ...
    await fabricaPage.mouse.move(startPoint.x, startPoint.y)
    await fabricaPage.mouse.down()
    await fabricaPage.mouse.move(box.x + box.width - 18, box.y + 80, { steps: 4 })
    ...
    await expect(fabricaPage.locator('[data-workspace-board-selection-rect]')).toBeVisible()
    await expect
