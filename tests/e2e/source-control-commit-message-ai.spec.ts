...
test.describe('Source Control AI commit messages', () => {
  ...
  for (const { label, linkedIssue, expected } of [
    ...
    test(`${label} the commit-message recipe`, async ({ fabricaPage, testRepoPath }) => {
      ...
      try {
        ...
        await openSourceControlForWorktree(fabricaPage, testRepoPath, worktreePath)
            ...
            await store.getState().updateSettings({
              ...
              sourceControlAi: {
                ...
                actions: {
                  commitMessage: {
                    agentId: 'custom' as const,
                    commandInputTemplate: 'FABRICA_E2E_ISSUE={linkedIssue}\n\n{basePrompt}'
                  }
                }
              }
            })
        ...
        const textarea = fabricaPage.getByRole('textbox', { name: 'Commit message' })
        await expect(textarea).toBeVisible({ timeout: 10_000 })

        const generate = fabricaPage.getByRole('button', { name: 'Generate commit message with AI' })
