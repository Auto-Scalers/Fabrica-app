...
test.describe('Source Control Create PR intent worktree switching', () => {
  ...
  test('keeps Create PR intent running after switching worktrees', async ({
        ...
        store.setState((current) => ({
          ...
          createHostedReview: async (repoPath, input) => {
            ...
            return {
              ok: true as const,
              number: 74,
              url: 'https://github.com/acme/FABRICA/pull/74'
            }
          },
          gitStatusByWorktree: {
    ...
    const createPr = fabricaPage.getByRole('button', { name: 'Create PR' }).first()
    ...
    const completedWhileSwitchedEvidence = await fabricaPage.evaluate(() => {
  ...
  test('carries unavailable dirty intent through push to the final create preflight', async ({
    ...
    await waitForSessionReady(fabricaPage)
    await waitForActiveWorktree(fabricaPage)
    const { prWorktreeId, prWorktreePath } = await seedCreatePrComposer(fabricaPage)
    const remoteRoot = mkdtempSync(path.join(os.tmpdir(), 'FABRICA-e2e-create-pr-remote-'))
          ...
          repos: current.repos.map((repo) =>
            repo.id === worktree.repoId
              ? {
                  ...repo,
                  gitRemoteIdentity: {
                    canonicalKey: 'github.com/acme/FABRICA',
                    remoteName: 'origin',
    ...
    await expect
      ...
      .toBe(true)
    await expect(fabricaPage.getByText(finalCreateError)).toBeVisible({ timeout: 10_000 })
  })
})
