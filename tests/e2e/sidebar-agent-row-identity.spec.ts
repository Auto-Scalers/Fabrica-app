...
test('sidebar keeps a Cursor pane visible and an OpenCode pane out of Claude Code hands', async ({
  ...
  await useFullAgentActivityRows(fabricaPage)

  const openCode = await openAgentTab(fabricaPage, worktreeId, 'opencode')
  ...
  await expect
    .poll(() => paneTitles(fabricaPage, openCode.tabId), {
      timeout: 15_000,
      message: 'the OpenCode task title never reached the renderer'
    })
    .toContain(OPENCODE_TASK_OSC_TITLE)

  const cursor = await openAgentTab(fabricaPage, worktreeId, 'cursor')
  ...
  const agentList = worktreeRow(fabricaPage, worktreeId).locator('[aria-label="Agents"]')
  ...
  expect(await settledSidebarAgentRowIdentities(fabricaPage, agentListSelector)).toEqual([
    'Cursor',
    'OpenCode'
  ])
