 ...
 *   FABRICA_LARGE_FILE_COUNT=9500 npx playwright test \
...
async function addAndActivateRepo(fabricaPage: Page, repoPath: string): Promise<string> {
  const repoId = await fabricaPage.evaluate(async (pathToRepo: string) => {
  ...
  await expect
    .poll(
      () =>
        fabricaPage.evaluate(async (targetRepoId: string) => {
          const store = window.__store
          if (!store) {
            return 0
          }
  ...
  const worktreeId = await fabricaPage.evaluate(
  ...
  const sourceControlButton = fabricaPage.getByRole('button', { name: /^Source Control/ })
  await expect(sourceControlButton).toBeVisible()
  await expect
    .poll(() => fabricaPage.evaluate(() => window.__store?.getState().rightSidebarTab))
    .toBe('source-control')
  await expect(fabricaPage.getByRole('button', { name: 'Filter files by name' })).toBeVisible()
...
async function unregisterLargeFileCountRepos(
  ...
  for (const repoPath of repoPaths) {
    await fabricaPage.evaluate(async (pathToRepo) => {
      const store = window.__store
...
async function measureSourceControlLoad(
  ...
  return await fabricaPage.evaluate(async ({ worktreeId, repoPath, expectedRows, pollCycles }) => {
    const store = window.__store
    if (!store) {
      throw new Error('window.__store is not available')
    }
...
test.describe('Source Control large file count (#8013)', () => {
  ...
  test('a large untracked set under the status cap stays responsive', async ({
    ...
    const untrackedFiles = Number(process.env.FABRICA_LARGE_FILE_COUNT ?? '950')
    // Why: FABRICA_LARGE_FILE_BYTES gives untracked files realistic sizes so the
    ...
    const untrackedFileBytes = Number(process.env.FABRICA_LARGE_FILE_BYTES ?? '0')
    ...
    try {
      ...
      const worktreeId = await addAndActivateRepo(fabricaPage, fixture.repoPath)
      const workingSetBeforeMb = await readRendererWorkingSetMb(electronApp)
      const measurement = await measureSourceControlLoad(fabricaPage, {
        worktreeId,
      ...
      await unregisterLargeFileCountRepos(fabricaPage, [fixture.repoPath])
    }
  })
  ...
  test('a large modified set under the status cap stays responsive', async ({
    ...
    test.setTimeout(600_000)
    const modifiedFiles = Number(process.env.FABRICA_LARGE_FILE_COUNT ?? '750')
  ...
  test('a change set over the status cap degrades to the too-many-changes state', async ({
    ...
    try {
      ...
      const activationMaxLagMs = await fabricaPage.evaluate(() => {
      ...
      const tooManyChangesBanner = fabricaPage.getByText('Too many changes detected.', {
        exact: false
      })
      await expect(tooManyChangesBanner).toBeVisible()
      if (process.env.FABRICA_LARGE_FILE_SCREENSHOT_PATH) {
        await fabricaPage.screenshot({ path: process.env.FABRICA_LARGE_FILE_SCREENSHOT_PATH })
      }
      ...
      const hugeState = await fabricaPage.evaluate(
      ...
      await expect(tooManyChangesBanner).toBeVisible()
      await fabricaPage.getByRole('button', { name: 'Retry' }).click()
  ...
  test('untracked line-stat cache stays effective up to the status cap', async ({
    ...
    try {
      ...
      const warmRescanPerFileMs = async (repoPath: string, files: number): Promise<number> => {
        const worktreeId = await addAndActivateRepo(fabricaPage, repoPath)
        const measurement = await measureSourceControlLoad(fabricaPage, {
          worktreeId,
          repoPath,
          expectedRows: files,
      ...
      await unregisterLargeFileCountRepos(fabricaPage, [
        smallRepo.repoPath,
  ...
  test('a large clean repo (tracked files only) loads instantly', async ({
    ...
    test.setTimeout(600_000)
    const trackedFiles = Number(process.env.FABRICA_LARGE_FILE_COUNT ?? '15000')
