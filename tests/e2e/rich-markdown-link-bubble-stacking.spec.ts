...
test.describe('Rich markdown link bubble stacking', () => {
  test('link actions stay above the right Explorer', async ({ fabricaPage }, testInfo) => {
    await fabricaPage.setViewportSize({ width: 1920, height: 1080 })
    await waitForSessionReady(fabricaPage)
    await waitForActiveWorktree(fabricaPage)

    const context = await getActiveWorktreeContext(fabricaPage)
    ...
    try {
      ...
      const explorer = fabricaPage.locator('[data-FABRICA-explorer-shell]')
      const link = fabricaPage.locator(`.rich-markdown-editor a[href="${LINK_HREF}"]`)
      await expect(explorer).toBeVisible()
      await expect(link).toBeVisible()

      await link.click()

      const bubble = fabricaPage.locator('.rich-markdown-link-bubble')
      await expect(bubble).toBeVisible()
      await expect(bubble.locator('.rich-markdown-link-url')).toContainText('https://example.com')

      const overlap = await fabricaPage.evaluate((): OverlapHitTest => {
        const bubble = document.querySelector<HTMLElement>('.rich-markdown-link-bubble')
        const explorer = document.querySelector<HTMLElement>('[data-FABRICA-explorer-shell]')
        if (!bubble || !explorer) {
      ...
      await fabricaPage.getByRole('heading', { name: 'Rich markdown link overlay repro' }).click()
      await link.click()
      await expect(bubble).toBeVisible()
      ...
      await expect(bubble).toHaveCount(0)
      await fabricaPage.evaluate((zoom) => {
