...
export function startRemoteHttpListener(
  ...
  execDockerSshRelayTargetCommand(
    target,
    [
      `nohup node -e ${shellQuote(script)} >/tmp/FABRICA-http-${port}.log 2>&1 < /dev/null &`,
      `echo $! >/tmp/FABRICA-http-${port}.pid`
    ].join(' ')
  )
  return Number(execDockerSshRelayTargetCommand(target, `cat /tmp/FABRICA-http-${port}.pid`))
}
...
export function readRemoteListenerIdentity(
  ...
  const pid = Number(execDockerSshRelayTargetCommand(target, `cat /tmp/FABRICA-http-${port}.pid`))
  return {
    pid,
