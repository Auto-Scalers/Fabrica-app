...
const PAIRING_URL_PATTERN = /FABRICA:\/\/[^\s"\\]+/g
...
export function parseHeadlessPairedRuntimePairingOffer(
  ...
  if (
    readiness.type !== 'fabrica_server_ready' ||
...
function redactPairingMaterial(value: string): string {
  return value
    .replace(PAIRING_URL_PATTERN, 'FABRICA://[redacted]')
...
export async function launchHeadlessPairedRuntimeHost(): Promise<HeadlessPairedRuntimeHost> {
  const userDataDir = mkdtempSync(path.join(os.tmpdir(), 'FABRICA-e2e-headless-paired-'))
  ...
  try {
    ...
    app = await electron.launch({
      args: [
        ...getFABRICAElectronLaunchArgs(mainPath, false),
        '--serve',
        '--serve-json',
        '--serve-port',
        '0',
