...
export function createPairedWebClientUrl(
  ...
  if (options.disableRemoteTerminalStallRecovery) {
    clientUrl.searchParams.set('FABRICAE2EDisableRemoteTerminalStallRecovery', '1')
  }
  if (options.terminalParkingDelayMs !== undefined) {
    clientUrl.searchParams.set('FABRICAE2ETerminalParkingDelayMs', `${options.terminalParkingDelayMs}`)
  }
  if (options.terminalRetentionLimit !== undefined) {
    clientUrl.searchParams.set('FABRICAE2ETerminalRetentionLimit', `${options.terminalRetentionLimit}`)
  }
