...
const fakeCliDir = mkdtempSync(path.join(os.tmpdir(), 'FABRICA-e2e-retired-worker-'))
const lifecycleLedgerPath = path.join(fakeCliDir, 'codex-lifecycle.jsonl')
const fakeCodexSource = `
const { appendFileSync } = require('node:fs')
const ledger = process.env.FABRICA_E2E_CODEX_LIFECYCLE_LEDGER
...
process.stdin.on('data', (chunk) => {
  ...
  if (input.includes('FABRICA_E2E_EXIT_AFTER_DONE')) {
...
export const completedWorkerLaunchEnv = {
  ...
  FABRICA_E2E_CODEX_LIFECYCLE_LEDGER: lifecycleLedgerPath
}
...
export function runBuiltFABRICACli(
  args: string[],
  options: { userDataDir: string; cwd: string }
): unknown {
  const {
    FABRICA_ENVIRONMENT: _environment,
    FABRICA_PAIRING_CODE: _pairingCode,
    FABRICA_USER_DATA_PATH: _userDataPath,
    ...cleanEnv
  } = process.env
  void _environment
  void _pairingCode
  ...
  const output = execFileSync(
    ...
    {
      cwd: options.cwd,
      env: { ...cleanEnv, FABRICA_USER_DATA_PATH: options.userDataDir },
      encoding: 'utf8',
      timeout: 30_000
    }
  )
