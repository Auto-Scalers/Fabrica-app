...
type RestartSession = {
  ...
  launch: (options?: LaunchOptions) => Promise<LaunchedFABRICA>
...
export function createRestartSession(
  ...
  const userDataDir = mkdtempSync(path.join(os.tmpdir(), 'FABRICA-e2e-restart-'))
  ...
  const launch = async (options?: LaunchOptions): Promise<LaunchedFABRICA> => {
    runtimeWsPort ??= await reserveRestartRuntimeWsPort()
    const app = await electron.launch({
      args: getFABRICAElectronLaunchArgs(mainPath, headful),
      env: {
        ...homeIsolation.env,
        ...options?.extraEnv,
  ...
  const dispose = async (): Promise<void> => {
    ...
    if (process.env.FABRICA_E2E_PRESERVE_RESTART_PROFILE === '1') {
      ...
      () =>
        readRestartRendererState(() =>
          page.evaluate(async (repoId) => {
            ...
            // worktree is external to FABRICA's workspace root after the visibility rollout.
