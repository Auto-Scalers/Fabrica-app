  ...
  const child = fork(
    ...
    ],
    {
      ...
      env: {
        ...
        FABRICA_USER_DATA_PATH: runtime.userDataDir
      },
...
export async function spawnGenerationCanary(options: {
  ...
  const sessionId = `${worktreeId}@@FABRICA-9749-${label}-${randomUUID().slice(0, 8)}`
  ...
  await waitForCondition(`${label} canary readiness`, () =>
    output.includes(`FABRICA_GENERATION_CANARY_READY ${label} ${nonce}`)
  )
  const match = new RegExp(`FABRICA_GENERATION_CANARY_READY ${label} ${nonce} (\\d+)`).exec(output)
...
export async function pingGenerationCanary(canary: GenerationCanary, nonce: string): Promise<void> {
  ...
  const expected = `FABRICA_GENERATION_CANARY_ACK ${label} ${nonce}`
