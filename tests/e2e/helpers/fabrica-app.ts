...
type FABRICATestFixtures = {
  ...
  FABRICAAppExtraEnv: Record<string, string>
  ...
  // developer's shell or already-running FABRICA instance.
  launchEnv: NodeJS.ProcessEnv
}

type FABRICAWorkerFixtures = {
  /** Absolute path to the test git repo created by globalSetup. */
  testRepoPath: string
}

// Why: parse + warn at module scope so a bad FABRICA_E2E_SLOWMO_MS value logs once
// per worker instead of once per test (otherwise hundreds of lines per CI run).
const FABRICA_E2E_SLOWMO_MS_RAW = process.env.FABRICA_E2E_SLOWMO_MS
const FABRICA_E2E_SLOWMO_MS = ((): number => {
  if (FABRICA_E2E_SLOWMO_MS_RAW === undefined) {
    return 0
  }
  const parsed = Number(FABRICA_E2E_SLOWMO_MS_RAW)
  if (!Number.isFinite(parsed)) {
    console.warn(
      `[FABRICA-e2e] FABRICA_E2E_SLOWMO_MS="${FABRICA_E2E_SLOWMO_MS_RAW}" is not a number; ignoring (using 0).`
    )
    return 0
  }
...
function shouldLaunchHeadful(testInfo: TestInfo): boolean {
  // Why: FABRICA_E2E_FORCE_HEADFUL lets a developer watch any spec in a real
  // window without retagging it `@headful` or switching projects.
  if (process.env.FABRICA_E2E_FORCE_HEADFUL === '1') {
    return true
  }
  return testInfo.project.metadata.fabricaHeadful === true
}
...
// same FABRICA_E2E_FORWARD_APP_LOGS-gated stdout/stderr capture.
export function forwardElectronProcessLogs(app: ElectronApplication, testInfo: TestInfo): void {
  if (process.env.FABRICA_E2E_FORWARD_APP_LOGS !== '1') {
    return
  }
 ...
 */
export const test = base.extend<FABRICATestFixtures, FABRICAWorkerFixtures>({
  ...
  electronApp: async (
    ...
    const userDataDir = mkdtempSync(path.join(os.tmpdir(), 'FABRICA-e2e-userdata-'))
    ...
    const homeIsolation = createElectronHomeIsolation({
      ...
      launchEnv,
      extraEnv: FABRICAAppExtraEnv,
      userDataDir
    })
    // Why: FABRICA_E2E_SLOWMO_MS adds a pause between every Playwright action so a
    // developer running with FABRICA_E2E_FORCE_HEADFUL=1 can actually watch what
    ...
    const recordVideoDir = process.env.FABRICA_E2E_RECORD_VIDEO === '1' ? testInfo.outputDir : null
    if (recordVideoDir) {
      mkdirSync(recordVideoDir, { recursive: true })
    }
    const app = await electron.launch({
      args: [...FABRICAAppExtraArgs, ...getFABRICAElectronLaunchArgs(mainPath, headful)],
      ...(slowMo > 0 ? { slowMo } : {}),
      ...(recordVideoDir ? { recordVideo: { dir: recordVideoDir } } : {}),
      // Why: keep NODE_ENV=development so window.__store is exposed and
      // dev-only helpers activate. FABRICA_E2E_USER_DATA_DIR overrides the usual
      // shared dev profile so every spec gets a clean persistence root.
      // Why: FABRICA_E2E_HEADLESS suppresses mainWindow.show() so the app
      // window stays hidden during test runs, avoiding focus stealing and
      // screen clutter. Playwright interacts via CDP regardless.
      // Why: FABRICA_E2E_HEADLESS suppresses mainWindow.show() for CI/headless
      // runs. FABRICA_E2E_HEADFUL overrides this for tests that need a visible
      ...
      env: {
        ...
        ...((process.env.FABRICA_E2E_SSH_LOCALHOST === '1' ||
          process.env.FABRICA_E2E_SSH_DOCKER === '1' ||
          process.env.FABRICA_E2E_NESTED_RUNTIME_SSH === '1') &&
        !cleanEnv.FABRICA_RELAY_PATH
          ? { FABRICA_RELAY_PATH: path.join(process.cwd(), 'out', 'relay') }
          : {}),
        ...(headful ? { FABRICA_E2E_HEADFUL: '1' } : { FABRICA_E2E_HEADLESS: '1' })
      }
    })
    try {
  ...
  FABRICAAppExtraEnv: [{}, { option: true }],
  FABRICAAppExtraArgs: [[], { option: true }],
  ...
  fabricaPage: async ({ sharedPage }, provideFixture) => {
