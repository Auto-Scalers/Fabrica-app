...
export const DOCKER_SSH_SECOND_HUB_REMOTE_REPO_PATH = '/tmp/FABRICA-docker-second-hub-repo'
...
function seedRemoteRepo(target: DockerSshRelayTarget, repoPath: string): void {
  ...
  execDockerSshRelayTargetCommand(
    ...
    [
      ...
      'git config user.name "FABRICA Docker SSH E2E"',
      `printf '%s\\n' ${shellQuote(sentinel)} > .FABRICA-e2e-destination-id`,
      `printf '%s\\n' ${shellQuote(`remote relay ${sentinel}`)} > README.md`,
      'git add README.md .FABRICA-e2e-destination-id',
...
export function startDockerSshRelayTarget(testInfo: TestInfo): DockerSshRelayTarget {
  const host = process.env.FABRICA_E2E_SSH_TARGET_HOST?.trim() || '127.0.0.1'
  if (host === 'localhost' || host === '::1' || host.startsWith('127.')) {
    if (process.env.FABRICA_E2E_SSH_TARGET_HOST) {
      throw new Error(`FABRICA_E2E_SSH_TARGET_HOST must be non-loopback: ${host}`)
    }
  }
  const bindHost = host === '127.0.0.1' ? host : '0.0.0.0'
  const tempDir = mkdtempSync(path.join(os.tmpdir(), 'FABRICA-ssh-docker-'))
  ...
  const containerName = `FABRICA-ssh-e2e-${testInfo.workerIndex}-${Date.now()}-${randomUUID().slice(0, 8)}`
    ...
    run(
      ...
      [
        ...
        [
          ...
          'git config --global user.name "FABRICA Docker SSH E2E"',
