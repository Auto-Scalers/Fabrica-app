...
import { forwardElectronProcessLogs } from './FABRICA-app'
import {
...
const DIRECT_SSH_PROBE_CANARY_TARGET_ID = '__FABRICA_e2e_direct_ssh_probe_canary__'
...
export async function launchPairedElectronClient(
  ...
  const userDataDir = mkdtempSync(path.join(os.tmpdir(), 'FABRICA-e2e-paired-desktop-'))
  ...
  const app = await electron.launch({
    args: getFABRICAElectronLaunchArgs(mainPath, false),
    env: {
      ...homeIsolation.env,
      NODE_ENV: 'development',
      FABRICA_E2E_HEADLESS: '1',
      FABRICA_E2E_FORBID_LOCAL_SSH_CONNECT_PROBE: directSshProbePath
    }
  })
