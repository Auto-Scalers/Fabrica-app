...
function createUserDataDir(): string {
  const tempDir = mkdtempSync(path.join(os.tmpdir(), 'FABRICA-home-isolation-test-'))
  ...
  it('strips ambient home and Codex state before forcing a disposable home', () => {
    ...
    const isolation = createElectronHomeIsolation({
      inheritedEnv: {
        ...
        FABRICA_CODEX_HOME: '/real/FABRICA-codex',
    ...
    expect(isolation.env.FABRICA_CODEX_HOME).toBeUndefined()
  ...
  it('rejects generic fixture overlays that could escape the boundary', () => {
    ...
    expect(() =>
      createElectronHomeIsolation({
        inheritedEnv: {},
        launchEnv: {},
        extraEnv: { FABRICA_E2E_USER_DATA_DIR: '/unsafe' },
        userDataDir: createUserDataDir(),
        realHome: '/real/home'
      })
    ).toThrow(/FABRICAAppExtraEnv\.FABRICA_E2E_USER_DATA_DIR/)
  })
