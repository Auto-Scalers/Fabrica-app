...
const TEMP_PREFIX = 'FABRICA-9749-dg-'
...
function knownFABRICAUserDataDirs(): string[] {
  if (process.platform === 'darwin') {
    const appSupport = path.join(homedir(), 'Library', 'Application Support')
    return [path.join(appSupport, 'FABRICA'), path.join(appSupport, 'FABRICA-dev')]
  }
  if (process.platform === 'win32') {
    const roaming = process.env.APPDATA ?? path.join(homedir(), 'AppData', 'Roaming')
    return [path.join(roaming, 'FABRICA'), path.join(roaming, 'FABRICA-dev')]
  }
  const config = process.env.XDG_CONFIG_HOME ?? path.join(homedir(), '.config')
  return [path.join(config, 'FABRICA'), path.join(config, 'FABRICA-dev')]
}
...
function assertDisposableRoot(rootDir: string): void {
  ...
  for (const userDataDir of knownFABRICAUserDataDirs()) {
    if (isEqualToOrInside(rootDir, userDataDir)) {
      throw new Error('Refusing daemon-generation fixture inside real FABRICA user data')
    }
  }
}
