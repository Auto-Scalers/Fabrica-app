...
export async function waitForRestoredTerminalInputReady(
  ...
  while (Date.now() < deadline) {
    const marker = `FABRICA_RESTORED_INPUT_READY_${randomUUID().replaceAll('-', '')}`
        ...
        ({ expectedPtyId, input, pendingAttempts }) => {
          for (const manager of window.__paneManagers?.values() ?? []) {
            ...
            const container = pane.container as HTMLElement & {
              __FABRICAE2eTerminalInputReadinessInstanceId?: string
            }
            container.__FABRICAE2eTerminalInputReadinessInstanceId ??= crypto.randomUUID()
            const paneInstanceId = container.__FABRICAE2eTerminalInputReadinessInstanceId
