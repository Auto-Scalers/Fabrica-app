 ...
 *
 * Run:
 *   FABRICA_E2E_SSH_DOCKER=1 pnpm run test:e2e:ssh-docker-bulk-open-freeze
 */
...
test.describe('R2 Docker SSH bulk-open freeze', () => {
  test.skip(!RUN_DOCKER_SSH, 'Set FABRICA_E2E_SSH_DOCKER=1 to run Docker SSH freeze repro')
  ...
  test('bulk-open many flooding SSH terminals and measure renderer lag @freeze-repro', async ({
    ...
    try {
      ...
      await execInTerminal(fabricaPage, continuousFloodCommand(runId, 0))
      await waitForTerminalOutput(fabricaPage, `READY:SSH_BULK_${runId}_0`, 60_000)

      for (let i = 1; i < SESSION_SPLITS; i += 1) {
        await splitActiveTerminalPane(fabricaPage)
        await focusLastTerminalPane(fabricaPage)
        await waitForActivePanePtyId(fabricaPage, 30_000)
        await execInTerminal(fabricaPage, continuousFloodCommand(runId, i))
        await waitForTerminalOutput(fabricaPage, `READY:SSH_BULK_${runId}_${i}`, 60_000)
      }

      // Leave the workspace view so panes go inactive while flooding.
      await fabricaPage.evaluate(() => window.__store?.getState().setActiveView('tasks'))
      await fabricaPage.waitForTimeout(4_000)

      const hiddenProbe = await startRendererLagProbe(fabricaPage)
      ...
      const openProbe = await startRendererLagProbe(fabricaPage)
      await fabricaPage.evaluate(() => window.__store?.getState().setActiveView('terminal'))
      for (let pass = 0; pass < 3; pass += 1) {
        for (let i = 0; i < SESSION_SPLITS; i += 1) {
          await fabricaPage.keyboard.press(process.platform === 'darwin' ? 'Meta+]' : 'Control+]')
      ...
      const interactionProbeMs = await fabricaPage.evaluate(async () => {
