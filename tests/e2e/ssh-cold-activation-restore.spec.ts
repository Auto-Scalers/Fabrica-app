...
test.describe('SSH cold activation restore', () => {
  ...
  test('eagerly remounts every restored remote terminal after renderer reload', async ({
    ...
    try {
      ...
      while ((await readRemoteTerminalTabs(fabricaPage, remote.worktreeId)).length < TAB_COUNT) {
        await createRemoteTerminalTab(fabricaPage, remote.worktreeId)
      }
      const beforeReload = await readRemoteTerminalTabs(fabricaPage, remote.worktreeId)
      ...
      const afterReload = await readRemoteTerminalTabs(fabricaPage, remote.worktreeId)
      ...
      await fabricaPage.getByRole('button', { name: /^Terminal 1 Close tab Terminal 1/ }).click()
      await expect
        .poll(() => fabricaPage.evaluate(() => window.__store?.getState().activeTabId ?? null), {
          timeout: 10_000
        })
        .toBe(firstTabId)
      ...
      const proofFile = '/tmp/FABRICA-ssh-restore-proof'
      await focusActiveTerminalInput(fabricaPage)
      await fabricaPage.keyboard.type(`printf '${marker}' > ${proofFile} && printf '${marker}\\n'`)
  ...
  {}, testInfo) => {
    ...
    try {
      ...
      const beforeProofPath = `/tmp/FABRICA-ssh-restart-before-${Date.now()}`
      const afterProofPath = `/tmp/FABRICA-ssh-restart-after-${Date.now()}`

      await focusActiveTerminalInput(firstLaunch.page)
      await firstLaunch.page.keyboard.type(
        `export FABRICA_RESTART_TOKEN=${token}; cd /tmp; (while :; do sleep 60; done) & export FABRICA_BG_PID=$!; printf '%s|%s|%s|%s\\n' "$$" "$FABRICA_BG_PID" "$FABRICA_RESTART_TOKEN" "$PWD" > ${beforeProofPath}`
      )
      ...
      await secondLaunch.page.keyboard.type(
        `printf '%s|%s|%s|%s\\n' "$$" "$FABRICA_BG_PID" "$FABRICA_RESTART_TOKEN" "$PWD" > ${afterProofPath}; printf '${restoredMarker}\\n'`
      )
