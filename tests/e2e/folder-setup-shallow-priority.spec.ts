...
async function createShallowPriorityTruncationFixture(): Promise<{
  ...
  const parentPath = realpathSync(
    await mkdtemp(path.join(os.tmpdir(), 'FABRICA-e2e-shallow-priority-'))
  )
...
async function createCancellableScanFixture(): Promise<{
  ...
  const parentPath = realpathSync(
    await mkdtemp(path.join(os.tmpdir(), 'FABRICA-e2e-cancellable-scan-'))
  )
  tempRoots.push(parentPath)
...
test('prioritizes shallow sibling repositories in a bounded nested scan', async ({
  ...
  await fabricaPage
    ...
    .first()
    .click()
  const dialog = fabricaPage.getByRole('dialog', { name: /Add a project/i })
  await expect(dialog).toBeVisible()
  await dialog.getByRole('button', { name: /Browse folder/i }).click()

  const importDialog = fabricaPage.getByRole('dialog', {
  ...
  await expect(fabricaPage.getByText(fixture.groupName)).toBeVisible()
  await expect(fabricaPage.getByText('z-web-client').first()).toBeVisible()
})
