...
test('refreshes the visible tree after external Windows file changes', async ({ fabricaPage }) => {
  await waitForSessionReady(fabricaPage)
  await waitForActiveWorktree(fabricaPage)
  await fabricaPage.evaluate(() => window.__store?.getState().setRightSidebarOpen(false))
  await expect
    .poll(() => fabricaPage.evaluate(() => window.__store?.getState().rightSidebarOpen))
    .toBe(false)
  await openFileExplorer(fabricaPage)

  const worktreePath = await fabricaPage.evaluate(() => {
  ...
  try {
    ...
    writeFileSync(originalPath, 'created outside FABRICA\n')
