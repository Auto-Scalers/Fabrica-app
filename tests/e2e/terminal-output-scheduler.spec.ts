...
test.describe('Terminal output scheduler', () => {
  test('background tab output bursts use the shared drain while the active tab renders', async ({
    ...
    const ptyIdsByTabId: Record<string, string> = {
      [firstTabId]: await waitForTabPtyId(fabricaPage, firstTabId)
    }

    while (tabIds.length < TAB_COUNT) {
      const tabId = await createTerminalTab(fabricaPage)
      await waitForActiveTerminalManager(fabricaPage, 30_000)
      tabIds.push(tabId)
      ptyIdsByTabId[tabId] = await waitForTabPtyId(fabricaPage, tabId)
    }

    await tabLocator(fabricaPage, firstTabId).click()
    await expect
    ...
    await resetSchedulerDebug(fabricaPage)
    ...
    await sendPtyCommands(fabricaPage, [
      {
    ...
    await expect
      .poll(async () => (await getTerminalContent(fabricaPage)).includes(foregroundMarker), {
        timeout: 5_000,
    ...
    await tabLocator(fabricaPage, firstBackgroundTabId).click()
    await expect
      .poll(() => getDomActiveTabId(fabricaPage), {
        timeout: 5_000,
    ...
    await expect
      .poll(async () => (await getTerminalContent(fabricaPage)).includes(firstBackground.marker), {
        timeout: 5_000,
  ...
  test('visible bulk output uses the high-priority drain instead of synchronous xterm writes', async ({
    ...
    const activeTabId = await createTerminalTab(fabricaPage)
    if (!activeTabId) {
      throw new Error('Expected a fresh terminal tab')
    }
    const ptyId = await waitForTabPtyId(fabricaPage, activeTabId)
    await resetSchedulerDebug(fabricaPage)
    ...
    await sendPtyCommands(fabricaPage, [{ ptyId, command: floodCommand }])

    await expect
      .poll(async () => (await getTerminalContent(fabricaPage, 12_000)).includes(marker), {
        timeout: 30_000,
  ...
  test('hidden overflow restores from main-owned terminal state when the tab becomes visible', async ({
    ...
    const foregroundTabId = await getActiveTabId(fabricaPage)
    if (!foregroundTabId) {
      throw new Error('Expected an initial terminal tab')
    }
    const hiddenTabId = await createTerminalTab(fabricaPage)
    await waitForActiveTerminalManager(fabricaPage, 30_000)
    const hiddenPtyId = await waitForTabPtyId(fabricaPage, hiddenTabId)

    await tabLocator(fabricaPage, foregroundTabId).click()
    await expect
      .poll(() => getDomActiveTabId(fabricaPage), {
        timeout: 5_000,
    ...
    await sendPtyCommands(fabricaPage, [{ ptyId: hiddenPtyId, command: floodCommand }])

    await expect
      .poll(async () => mainSnapshotContains(fabricaPage, hiddenPtyId, marker), {
        timeout: 30_000,
        message: 'Main-owned terminal snapshot did not capture the hidden flood marker'
      })
      .toBe(true)

    await tabLocator(fabricaPage, hiddenTabId).click()
    await expect
      .poll(() => getDomActiveTabId(fabricaPage), {
        timeout: 5_000,
    ...
    expect(await getTerminalContent(fabricaPage)).not.toContain('FABRICA skipped hidden terminal output')
  })
})
