...
function nativeRepetitions(): number {
  const parsed = Number(process.env.FABRICA_E2E_NATIVE_IBUS_REPETITIONS ?? DEFAULT_REPETITIONS)
...
function nativeKeyDelayMs(): number {
  const parsed = Number(process.env.FABRICA_E2E_NATIVE_IBUS_KEY_DELAY_MS ?? DEFAULT_KEY_DELAY_MS)
  return Number.isInteger(parsed) && parsed >= 0
...
async function focusNativeTerminalWindow(page: Page): Promise<string> {
  ...
  const title = `FABRICA_NATIVE_IBUS_${randomUUID()}`
...
test.describe('Native IBus Hangul terminal input @headful', () => {
  test.skip(
    process.env.FABRICA_E2E_NATIVE_IBUS_HANGUL !== '1',
