...
test.describe('Terminal raw emoji table scroll restore repro', () => {
  ...
  test('uses WebGL by default for ordinary terminal output when available @terminal-rendering-golden', async ({
    ...
    const marker = `FABRICA_AUTO_WEBGL_SMOKE_${randomUUID()}`

    await sendToTerminal(fabricaPage, ptyId, `printf ${JSON.stringify(`${marker}\\n`)}\r`)
    await waitForTerminalOutput(fabricaPage, marker, 10_000)

    const expectedWebgl = await expectAutoWebgl(fabricaPage)
    ...
    let diagnostics = await readTerminalRenderDiagnostics(fabricaPage)
    await expect
      .poll(
        async () => {
          diagnostics = await readTerminalRenderDiagnostics(fabricaPage)
  ...
  test('keeps raw emoji box table aligned after restore and scroll @terminal-rendering-golden', async ({
    ...
    await ensureTerminalVisible(fabricaPage)
    await waitForActiveTerminalManager(fabricaPage, 30_000)
    await setWideRenderedTableViewport(fabricaPage)
    await waitForActiveTerminalColumns(fabricaPage, RAW_EMOJI_BOX_TABLE_WIDTH)
    const ptyId = await waitForActivePanePtyId(fabricaPage)
    const runId = randomUUID()
    const scriptPath = path.join(testRepoPath, `.FABRICA-raw-emoji-fixture-table-${runId}.mjs`)
    ...
    try {
      ...
      await waitForTerminalOutput(fabricaPage, completionMarker, 20_000, 30_000)
      await expect
      ...
      await scrollActiveTerminalToText(fabricaPage, 'Singer')
      await closeFeatureTips(fabricaPage)
      const expectedWebgl = await expectAutoWebgl(fabricaPage)
      ...
      const singerGeometry = await readVisibleSingerRowGeometry(fabricaPage)
