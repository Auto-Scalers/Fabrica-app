...
test.describe('terminal stuck-occlusion recovery', () => {
  ...
  test('a keystroke unlatches the hidden-delivery gate wedged by stale visibilityState', async ({
    ...
    await expect
      .poll(async () => getTerminalContent(fabricaPage), { timeout: 15_000 })
    ...
    await expect
      ...
      .toBeGreaterThan(0)
    expect(
      (await getDeliverySnapshot(fabricaPage)).hiddenDeliveryGatedVisiblePtyCount
    ).toBeGreaterThan(0)

    // The freeze repro: output produced now is dropped by main, not painted.
    const droppedBefore = (await getDeliverySnapshot(fabricaPage)).hiddenDeliveryDroppedChars
    await execInTerminal(fabricaPage, ptyId, 'echo occluded-$((70+8))')
    await expect
      .poll(async () => (await getDeliverySnapshot(fabricaPage)).hiddenDeliveryDroppedChars, {
        timeout: 15_000
      })
    ...
    await expect
      .poll(async () => (await getDeliverySnapshot(fabricaPage)).hiddenDeliveryGatedVisiblePtyCount, {
        timeout: 15_000
      })
      .toBe(0)

    // Live delivery continues under the override.
    await execInTerminal(fabricaPage, ptyId, 'echo live-after-$((200+56))')
    await expect
      .poll(async () => getTerminalContent(fabricaPage), { timeout: 15_000 })
    ...
    const report = await fabricaPage.evaluate(() =>
      (
        window as Window & {
          __FABRICATerminalFreezeReport?: () => Promise<{
