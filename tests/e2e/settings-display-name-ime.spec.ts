...
test.describe('Repository Display Name IME composition', () => {
  test('keeps Hangul syllables composed while typing slowly', async ({ fabricaPage }) => {
    await waitForSessionReady(fabricaPage)

    const repos = await getStoreState<Repo[]>(fabricaPage, 'repos')
    expect(repos.length).toBeGreaterThan(0)
    const repo = repos[0]

    await openRepoSettings(fabricaPage, repo.id)

    const repoSection = fabricaPage.locator(`[data-settings-section="repo-${repo.id}"]`)
    ...
    await typeHangulGanadaSlowly(session, fabricaPage, displayNameInput)
    ...
    await expect
      .poll(
        async () => {
          const current = await getStoreState<Repo[]>(fabricaPage, 'repos')
