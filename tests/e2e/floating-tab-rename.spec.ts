...
async function openFloatingPanel(page: Page): Promise<void> {
  ...
  if (!alreadyOpen) {
    await page.evaluate(() => window.dispatchEvent(new Event('FABRICA-toggle-floating-terminal')))
  }
...
test('Electron serializes native Unicode rename aliases', async ({ fabricaPage }) => {
