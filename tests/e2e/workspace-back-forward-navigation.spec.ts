...
test.describe('Workspace Back/Forward Navigation', () => {
  ...
  test('buttons are hidden outside the terminal view', async ({ fabricaPage }) => {
    await expect(await getBackButton(fabricaPage)).toBeVisible()
    await expect(await getForwardButton(fabricaPage)).toBeVisible()
    ...
    await expect(await getBackButton(fabricaPage)).toHaveCount(0)
    await expect(await getForwardButton(fabricaPage)).toHaveCount(0)
  ...
  test('both buttons disabled at cold start with a single history entry', async ({ fabricaPage }) => {
    ...
    const activeId = await getActiveWorktreeId(fabricaPage)
    expect(activeId).not.toBeNull()

    await resetNavHistory(fabricaPage)
    await seedVisit(fabricaPage, activeId!)

    const back = await getBackButton(fabricaPage)
    const forward = await getForwardButton(fabricaPage)
    await expect(back).toBeDisabled()
    await expect(forward).toBeDisabled()
  })

  test('clicking Back and Forward walks the history stack', async ({ fabricaPage }) => {
    const worktreeIds = await getAllWorktreeIds(fabricaPage)
    ...
    await resetNavHistory(fabricaPage)
    await seedVisit(fabricaPage, primaryId)
    await seedVisit(fabricaPage, secondaryId)
    ...
    const primaryRow = worktreeRow(fabricaPage, primaryId)
    const secondaryRow = worktreeRow(fabricaPage, secondaryId)

    await back.click()
    await expect
      .poll(async () => getActiveWorktreeId(fabricaPage), {
  ...
  test('re-activating the current worktree is a no-op (dedupe)', async ({ fabricaPage }) => {
    const activeId = await getActiveWorktreeId(fabricaPage)
    expect(activeId).not.toBeNull()
    ...
    const snapshot = await getNavHistorySnapshot(fabricaPage)
    expect(snapshot.history).toEqual([activeId])
    expect(snapshot.index).toBe(0)
    await expect(await getBackButton(fabricaPage)).toBeDisabled()
  })

  test('new navigation after going back truncates the forward stack', async ({ fabricaPage }) => {
    const worktreeIds = await getAllWorktreeIds(fabricaPage)
    ...
    await expect(await getForwardButton(fabricaPage)).toBeDisabled()
  })

  test(`${isMac ? 'Cmd' : 'Ctrl'}+Alt+Left/Right shortcuts walk history`, async ({ fabricaPage }) => {
    const worktreeIds = await getAllWorktreeIds(fabricaPage)
    ...
    await fabricaPage.keyboard.press(`${mod}+Alt+ArrowLeft`)
    await expect
      .poll(async () => getActiveWorktreeId(fabricaPage), {
    ...
    await fabricaPage.keyboard.press(`${mod}+Alt+ArrowRight`)
    await expect
      .poll(async () => getActiveWorktreeId(fabricaPage), {
  ...
  test('shortcut is a no-op in settings view', async ({ fabricaPage }) => {
    const worktreeIds = await getAllWorktreeIds(fabricaPage)
    ...
    await expect
      .poll(async () => fabricaPage.evaluate(() => window.__store!.getState().activeView))
      .toBe('settings')

    const idBefore = await getActiveWorktreeId(fabricaPage)
    await fabricaPage.evaluate(() => document.body.focus())
    await fabricaPage.keyboard.press(`${mod}+Alt+ArrowLeft`)

    // Give any erroneous nav a beat to land, then assert the active worktree
    // and the slice index both stayed put.
    await fabricaPage.waitForTimeout(150)
    expect(await getActiveWorktreeId(fabricaPage)).toBe(idBefore)
    const snapshot = await getNavHistorySnapshot(fabricaPage)
    expect(snapshot.index).toBe(1)
  })
})
