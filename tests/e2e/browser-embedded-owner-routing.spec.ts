...
test('stale helper cannot take goto or eval away from the real embedded webview', async ({
  ...
  try {
    ...
    const browserTab = await createBrowserTab(fabricaPage, worktreeId, server.sourceUrl)

    await expect
      .poll(() => readEmbeddedPage(fabricaPage, browserTab.id), { timeout: 10_000 })
      .toMatchObject({ marker: 'source-webview', title: 'Owned source', url: server.sourceUrl })

    const snapshot = await callBrowserRuntime(fabricaPage, 'browser.snapshot', {
      page: browserTab.pageId
    })
    expect(snapshot, JSON.stringify(snapshot)).toMatchObject({ ok: true })

    const sessionName = `FABRICA-tab-${browserTab.pageId}`
    await expect
    ...
    const navigation = await callBrowserRuntime(fabricaPage, 'browser.goto', {
      page: browserTab.pageId,
    ...
    await expect(fabricaPage.locator(`[data-tab-id="${browserTab.id}"]`)).toContainText(
      'Owned destination'
    )

    const evaluation = await callBrowserRuntime(fabricaPage, 'browser.eval', {
      page: browserTab.pageId,
