...
const REMOTE_IMAGE_PATH = '/tmp/FABRICA-ssh-external-preview.png'
const IMAGE_BASE64 =
...
test.describe('SSH external image preview', () => {
  ...
  test('opens an image outside the worktree from a terminal link', async ({
    ...
    try {
      ...
      await sendToTerminal(fabricaPage, ptyId, `printf '%s\\n' ${shellQuote(REMOTE_IMAGE_PATH)}\r`)
      await expect
        .poll(() => getTerminalContent(fabricaPage, 30_000), {
          timeout: 15_000,
      ...
      const probe = await findTerminalLink(fabricaPage, REMOTE_IMAGE_PATH)
      await activateTerminalLink(fabricaPage, probe, REMOTE_IMAGE_PATH)

      const preview = fabricaPage.locator(`img[alt="${REMOTE_IMAGE_PATH.split('/').at(-1)}"]`)
      ...
      await expect(fabricaPage.getByText('Unable to load file', { exact: true })).toHaveCount(0)

      const state = await fabricaPage.evaluate((filePath) => {
