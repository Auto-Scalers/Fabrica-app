...
test.describe('Terminal inactive cursor rendering', () => {
  ...
  test('keeps an unfocused prompt cursor rendered as one block outline', async ({ fabricaPage }) => {
    await splitActiveTerminalPane(fabricaPage, 'vertical')
    await waitForPaneCount(fabricaPage, 2)
    await placeInactiveCursorAtPrompt(fabricaPage)

    const fixedBehavior = await renderInactiveCursor(fabricaPage)
    ...
    const oldBehavior = await renderInactiveCursor(fabricaPage, 'outline')
