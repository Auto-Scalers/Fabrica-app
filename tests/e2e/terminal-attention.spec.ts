...
test.describe('Terminal attention', () => {
  ...
  test('background terminal attention marks a tab unread and clears on focus', async ({
    ...
    const secondTabId = await createTerminalTab(fabricaPage)
    await waitForActiveTerminalManager(fabricaPage, 30_000)
    ...
    await expect
      .poll(async () => (await getUnreadTerminalTabIds(fabricaPage)).includes(secondTabId), {
        timeout: 10_000,
        message: 'Background tab did not become unread after BEL'
      })
      .toBe(true)

    const secondTabBell = fabricaPage
      .locator(
  ...
  test('a BEL on the focused tab raises, then clears on click', async ({ fabricaPage }) => {
    ...
    const activeTabId = await getActiveTabId(fabricaPage)
    if (!activeTabId) {
      throw new Error('Expected an active terminal tab')
    }
    const activePtyId = await waitForActivePanePtyId(fabricaPage)
    await installRendererTitleLog(fabricaPage)
    ...
    await expect
      .poll(async () => (await getUnreadTerminalTabIds(fabricaPage)).includes(activeTabId), {
        timeout: 5_000,
  ...
  test('a BEL on the focused tab raises, then clears on plain Escape', async ({ fabricaPage }) => {
    await waitForSessionReady(fabricaPage)
    await waitForActiveWorktree(fabricaPage)
    await ensureTerminalVisible(fabricaPage)
    await waitForActiveTerminalManager(fabricaPage, 30_000)
    ...
    const activePaneKey = await getActivePaneKey(fabricaPage, activeTabId)
    const activePtyId = await waitForActivePanePtyId(fabricaPage)
    await installRendererTitleLog(fabricaPage)
    ...
    await expect
      .poll(async () => (await getUnreadTerminalPaneKeys(fabricaPage)).includes(activePaneKey), {
        timeout: 5_000,
    ...
    await focusActiveXterm(fabricaPage, activeTabId)
  ...
  // after restarting FABRICA with a Claude Code session open, clicking between
  ...
  test('mode bits replayed into xterm do not leak focus escapes to the shell', async ({
    ...
    try {
      ...
      // shell-startup BELs.
      const emittedFromXterm = await fabricaPage.evaluate(
        () =>
