...
test.describe('Hidden terminal TUI visual restore', () => {
  test('restores hidden full-screen TUI output without visible corruption', async ({
    ...
    const hiddenSnapshot = await waitForPaneIdentitySnapshot(fabricaPage, 1)
    ...
    const scriptPath = path.join(testRepoPath, `.FABRICA-hidden-tui-visual-${runId}.mjs`)
    ...
    await expect
      .poll(() => readMainSnapshotSource(fabricaPage, hiddenPane.ptyId!), {
        timeout: 10_000,
  ...
  test('keeps newer live output correct after plain hidden output restores', async ({
    ...
    const scriptPath = path.join(testRepoPath, `.FABRICA-low-risk-hidden-${runId}.mjs`)
    ...
    await ensureTerminalVisible(fabricaPage)
    await waitForActiveTerminalManager(fabricaPage, 30_000)
    await injectPaneData(fabricaPage, paneKey, liveFrame, {
  ...
  test('restores rich synchronized TUI output from the headless model', async ({
    ...
    const runId = randomUUID()
    const finalMarker = `VISUAL_RESTORE_FINAL_${runId}_24`
    const scriptPath = path.join(testRepoPath, `.FABRICA-hidden-rich-model-${runId}.mjs`)
    writeHiddenFrameScript(scriptPath, runId)
    await resetHiddenDebug(fabricaPage)
    try {
      await writeHiddenFrames(fabricaPage, hiddenPane.ptyId, scriptPath)
  ...
  test('keeps hidden terminal side effects live while hidden output may restore', async ({
    ...
    await resetHiddenDebug(fabricaPage)
    await writeHiddenSideEffectBurst(fabricaPage, hiddenPane.ptyId, hiddenTitle, marker)

    await expect
      .poll(() => getRuntimePaneTitle(fabricaPage, hiddenSnapshot.tabId, hiddenPane.numericPaneId), {
        timeout: 10_000,
        message: 'hidden OSC title did not update renderer-visible model state'
      })
      .toBe(hiddenTitle)
    await expect
      .poll(async () => (await getUnreadTerminalTabIds(fabricaPage)).includes(hiddenSnapshot.tabId), {
        timeout: 10_000,
