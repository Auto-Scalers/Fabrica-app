...
test('Linear filter chips keep readable names after the dropdown closes', async ({
  ...
  await expect
    .poll(() => getStoreState<string>(fabricaPage, 'activeView'), { timeout: 5_000 })
    .toBe('tasks')
  const filtersButton = fabricaPage.getByRole('button', { name: 'Filters', exact: true })
  await expect(filtersButton).toBeVisible()
  await expect(fabricaPage.getByText(FIXTURE.issue.title, { exact: true })).toBeVisible()

  await filtersButton.click()
  const popover = fabricaPage.locator('[data-slot="popover-content"]')
