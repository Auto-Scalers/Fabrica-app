...
test.describe('Windows project runtime smoke', () => {
  test('keeps a Windows-host project and WSL project available side by side', async ({
    ...
    const wsl = await fabricaPage.evaluate(async () => ({
    ...
    const wslRepoPath = await createGitRepo('FABRICA-e2e-project-runtime-', 'wsl-runtime-project')

    const smoke = await fabricaPage.evaluate(
    ...
    await openRepoSettings(fabricaPage, smoke.hostRepoId)
    const hostSection = fabricaPage.locator(`[data-settings-section="repo-${smoke.hostRepoId}"]`)
    ...
    await chooseProjectRuntime(fabricaPage, smoke.hostRepoId, 'WSL')
    await expect(
      hostSection.getByText(`This project runs in ${smoke.wslDistro} via WSL.`)
    ).toBeVisible()
    const hostAfterWslUiSwitch = await fabricaPage.evaluate((hostRepoId) => {
      const state = window.__store!.getState()
      const hostProject = state.projects.find((project) =>
    ...
    await chooseProjectRuntime(fabricaPage, smoke.hostRepoId, 'Windows')
    await expect(hostSection.getByText('This project runs on Windows.')).toBeVisible()
    const hostAfterWindowsUiSwitch = await fabricaPage.evaluate((hostRepoId) => {
      const state = window.__store!.getState()
      const hostProject = state.projects.find((project) =>
        project.sourceRepoIds.includes(hostRepoId)
      )
    ...
    await openRepoSettings(fabricaPage, smoke.wslRepoId)
    const wslSection = fabricaPage.locator(`[data-settings-section="repo-${smoke.wslRepoId}"]`)
    ...
    await fabricaPage.evaluate(async (repoId) => {
