...
test.describe('terminal document visibility WebGL recovery', () => {
  test('preserves the WebGL atlas and keeps terminal text painted after document visibility resumes', async ({
    ...
    const webglActive = await forceWebgl(fabricaPage)
    test.skip(!webglActive, 'WebGL was not active in this Electron environment')

    await writeStableTerminalContent(fabricaPage)
    expect(await patchAtlasCounter(fabricaPage)).toBe(true)
    expect(await countPatchedWebglAddons(fabricaPage)).toBeGreaterThanOrEqual(2)
    const baseline = await terminalScreenshots(fabricaPage)
    ...
    try {
      ...
      await resetAtlasResetCount(fabricaPage)
      ...
      if (!browserWindowVisibilityWorked) {
        await resetAtlasResetCount(fabricaPage)
        await dispatchDocumentVisibilityCycle(fabricaPage)
      }

      await waitForTerminalPaint(fabricaPage)
      expect(
        await readAtlasResetCount(fabricaPage),
        'ordinary document visibility resume cleared the shared WebGL atlas'
      ).toBe(0)

      const afterResume = await terminalScreenshots(fabricaPage)
