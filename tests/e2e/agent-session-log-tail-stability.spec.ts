...
test.describe('Agent Session History live log', () => {
  test.use({
    FABRICAAppExtraArgs: ['--js-flags=--expose-gc', '--enable-precise-memory-info']
  })
  ...
  test('keeps a long View Log viewport stable while the transcript grows', async ({
    ...
    await fabricaPage.setViewportSize({ width: 520, height: 720 })
    await openSessionHistory(fabricaPage)
    const title = fabricaPage.getByText(fixture.title, { exact: true }).first()
    await expect(title).toBeVisible({ timeout: 30_000 })
    await title.click()
    await fabricaPage.getByText('View Log', { exact: true }).click()
    await expect
      .poll(() => readProbeOrNull(fabricaPage), { timeout: 120_000 })
      .toMatchObject({ filePath: fixture.filePath, valueLength: fixture.initialLength })
    await fabricaPage.setViewportSize({ width: 900, height: 720 })
    ...
    let baseline = await restoreAnchorState(fabricaPage, true)
    ...
    for (let batch = 0; batch < 3; batch++) {
      ...
      replacementMemory.push(await measureLegacyControl(electronApp, fabricaPage, baseline, batch))
      baseline = await restoreAnchorState(fabricaPage, baseline.find.open, equivalentState.scrollTop)
      ...
      await forceGcAndSettle(fabricaPage)
      const before = await readMemory(electronApp, fabricaPage)
      appendFileSync(fixture.filePath, suffix)
      fixture.initialLength += suffix.length
      await fabricaPage.waitForTimeout(APPEND_CADENCE_MS)
      await expect
        .poll(() => readProbe(fabricaPage), { timeout: 30_000 })
        .toMatchObject({ valueLength: fixture.initialLength })
      const after = await readMemory(electronApp, fabricaPage)
      await forceGcAndSettle(fabricaPage)
      const settled = await readMemory(electronApp, fabricaPage)
      suffixMemory.push({ before, after, settled })
      const current = await readProbe(fabricaPage)
      ...
      expect(await fabricaPage.evaluate(() => 2 + 2)).toBe(4)
      expect((await readMainProcessCrashProbe(electronApp)).processGone).toBeNull()
      baseline = current
      if (batch === 0) {
        await fabricaPage.keyboard.press('Escape')
        await expect(fabricaPage.locator('.monaco-editor .find-widget')).toHaveAttribute(
          'aria-hidden',
          'true'
        )
        baseline = await readProbe(fabricaPage)
    ...
    await testInfo.attach('synthetic-live-log-stable', {
      body: await fabricaPage.screenshot(),
