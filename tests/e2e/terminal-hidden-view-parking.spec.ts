...
test.describe('Terminal hidden view parking', () => {
  test('parks a hidden terminal tab and restores rich TUI output on reveal', async ({
    ...
    const setup = await setUpParkableTabA(fabricaPage)
    const { worktreeId, tabAId, tabAPtyId } = setup

    const runId = randomUUID()
    const finalMarker = `PARKED_RESTORE_FINAL_${runId}_${PARKED_FRAME_COUNT - 1}`
    const scriptPath = path.join(testRepoPath, `.FABRICA-parked-rich-tui-${runId}.mjs`)
    writeParkedFrameScript(scriptPath, runId)
    try {
      await sendToTerminal(fabricaPage, tabAPtyId, `node ${JSON.stringify(scriptPath)}\r`)
      await expect
        .poll(() => getTerminalContent(fabricaPage, 12_000), {
          timeout: 15_000,
      ...
      const parkDetectedAfterMs = await parkHiddenTabBehindDecoy(fabricaPage, worktreeId, tabAId, {
        parkDelayMs: PARKING_DELAY_MS
      })
      const wiring = await readParkingWiring(fabricaPage)
      ...
      await activateTerminalTab(fabricaPage, tabAId)
      await waitForActiveTerminalManager(fabricaPage, 30_000)
      const revealedSnapshot = await waitForPaneIdentitySnapshot(fabricaPage, 1)
      ...
      await fabricaPage.screenshot({ path: screenshotPath, fullPage: true })
  ...
  test('keeps bell and title side effects live while parked', async ({ fabricaPage }) => {
    await waitForSessionReady(fabricaPage)
    const setup = await setUpParkableTabA(fabricaPage)
    const { worktreeId, tabAId, tabAPtyId } = setup

    await createActiveTerminalTab(fabricaPage, worktreeId)
    await parkHiddenTabBehindDecoy(fabricaPage, worktreeId, tabAId, {
      parkDelayMs: PARKING_DELAY_MS
    })
    ...
    await expect
      .poll(() => getTerminalTabTitle(fabricaPage, worktreeId, tabAId), {
        timeout: 10_000,
        message: 'parked OSC 0 title did not update the tab title in the store'
      })
      .toBe(parkedTitle)
    await expect
      .poll(async () => (await getUnreadTerminalTabIds(fabricaPage)).includes(tabAId), {
        timeout: 10_000,
        message: 'parked BEL did not mark the terminal tab unread'
      })
      .toBe(true)
    await expect
      .poll(() => isWorktreeUnread(fabricaPage, worktreeId), {
        timeout: 10_000,
  ...
  test('does not park excluded tabs', async ({ fabricaPage }) => {
    await waitForSessionReady(fabricaPage)
    const setup = await setUpParkableTabA(fabricaPage)
    ...
    const tabCId = await createActiveTerminalTab(fabricaPage, worktreeId)
    ...
    expect(await hasPendingStartupCommand(fabricaPage, tabCId)).toBe(true)

    // Tab B on top hides both A and C.
    const tabBId = await createActiveTerminalTab(fabricaPage, worktreeId)
    await expect
      .poll(() => getActiveTabId(fabricaPage), {
        timeout: 5_000,
    ...
    expect(await hasPendingStartupCommand(fabricaPage, tabCId)).toBe(true)
    const tabCState = await readTerminalTabViewState(fabricaPage, tabCId)
  ...
  test('reproduces a static frame byte-for-byte across 25 park/reveal cycles', async ({
    ...
    const scriptPath = path.join(testRepoPath, `.FABRICA-cycle-reference-${runId}.mjs`)
    ...
    try {
      ...
      const runOneParkRevealCycle = async (cycle: number): Promise<string[]> => {
        ...
        await activateTerminalTab(fabricaPage, decoyTabId)
        await waitForTabParked(fabricaPage, tabAId, { parkDelayMs: PARKING_DELAY_MS })
        await activateTerminalTab(fabricaPage, tabAId)
        await waitForActiveTerminalManager(fabricaPage, 30_000)
        const revealed = await waitForPaneIdentitySnapshot(fabricaPage, 1)
        ...
        await expect
          ...
          .toContain(marker)
        const rows = terminalContentRows(await getTerminalContent(fabricaPage, 12_000))
        // Garble sentinel: the hidden-skip banner must never appear.
        expect(rows.join('\n')).not.toContain('FABRICA skipped hidden terminal output')
        return rows
      }
