...
// scoped to this spec's app launches via FABRICAAppExtraEnv (same pattern as
...
test.use({
  FABRICAAppExtraEnv: { FABRICA_E2E_TERMINAL_PARKING_DELAY_MS: String(PARKING_DELAY_MS) },
...
test.describe('Terminal parked memory', () => {
  test('releases renderer terminal memory when hidden tabs park', async ({
    ...
    const scriptPath = path.join(testRepoPath, `.FABRICA-parked-memory-${runId}.mjs`)
    writeScrollbackFillScript(scriptPath, runId)
    try {
      const { worktreeId, scrollbackTabs } = await setUpScrollbackTabs(fabricaPage, scriptPath, runId)
      ...
      // the other 7 park.
      const visibleTab = await createActiveTerminalTab(fabricaPage, worktreeId)
      ...
      const metrics = await sampleParkedMemoryMetrics(fabricaPage)
      ...
      for (const tab of parkableTabs) {
        expect((await readTerminalTabViewState(fabricaPage, tab.tabId)).hasManager).toBe(false)
      }
      expect((await readTerminalTabViewState(fabricaPage, lastActiveTab.tabId)).hasManager).toBe(true)
  ...
  test('retains terminal views when parking is disabled', async ({
    ...
    await expect
      .poll(
        () =>
          fabricaPage.evaluate(() => window.__store?.getState().settings?.terminalHiddenViewParking),
    ...
    try {
      ...
      await fabricaPage.waitForTimeout(PARKING_DELAY_MS * 3)
      expect(await countMountedPaneManagers(fabricaPage, scrollbackTabIds)).toBe(SCROLLBACK_TAB_COUNT)
      ...
      for (const tab of scrollbackTabs) {
        const state = await readTerminalTabViewState(fabricaPage, tab.tabId)
        expect(state.hasManager).toBe(true)
        expect(state.paneCount).toBeGreaterThan(0)
      }
      expect((await readTerminalTabViewState(fabricaPage, visibleTab.tabId)).hasManager).toBe(true)
...
test.describe('Terminal hidden worktree retention budget', () => {
  ...
  test('releases un-parkable hidden worktree buffers only once the retention budget engages', async ({
    ...
    await waitForSessionReady(fabricaPage)
    const victimWorktreeId = await waitForActiveWorktree(fabricaPage)
    await skipUnlessParkingWired(fabricaPage)

    const decoyWorktreeId = (await getAllWorktreeIds(fabricaPage)).find(
    ...
    await updateTerminalSettings(fabricaPage, {
      terminalHiddenWorktreeRetentionBudget: false,
      terminalScrollbackRows: RETENTION_SCROLLBACK_ROWS
    })
    await waitForRetentionBudgetSetting(fabricaPage, false)

    const runId = randomUUID()
    const scriptPath = path.join(testRepoPath, `.FABRICA-retention-memory-${runId}.mjs`)
    ...
    try {
      ...
      const baselineSnapshot = await waitForPaneIdentitySnapshot(fabricaPage, 1)
      const baselinePtyId = baselineSnapshot.panes[0]?.ptyId
      if (!baselinePtyId) {
      ...
      for (let tabIndex = 0; tabIndex < RETENTION_TAB_COUNT; tabIndex += 1) {
        if (tabIndex > 0) {
          victimTabs.push(await createActiveTerminalTab(fabricaPage, victimWorktreeId))
        }
        ...
        await stageUnparkableWorktreeTabs(fabricaPage, victimWorktreeId)
      }
      ...
      await switchToWorktree(fabricaPage, decoyWorktreeId)
      await expect
        .poll(() => fabricaPage.evaluate(() => window.__store?.getState().activeWorktreeId), {
          timeout: 5_000,
      ...
      const decoySnapshot = await waitForPaneIdentitySnapshot(fabricaPage, 1)
      const decoyTabIds = [decoySnapshot.tabId]
      expect(await countMountedPaneManagers(fabricaPage, decoyTabIds)).toBe(1)
      ...
      await stageUnparkableWorktreeTabs(fabricaPage, victimWorktreeId)
      await stageUnparkableWorktreeTabs(fabricaPage, decoyWorktreeId)
      await waitForUnparkableWorktreeTabs(fabricaPage, victimWorktreeId)
      await waitForUnparkableWorktreeTabs(fabricaPage, decoyWorktreeId)
      ...
      await expect
        .poll(
          async () => {
            await stageUnparkableWorktreeTabs(fabricaPage, victimWorktreeId)
            await stageUnparkableWorktreeTabs(fabricaPage, decoyWorktreeId)
            const victimMounted = await countMountedPaneManagers(fabricaPage, victimTabIds)
            const decoyMounted = await countMountedPaneManagers(fabricaPage, decoyTabIds)
      ...
      const before = await readRetentionMemorySample(fabricaPage)
      expect(before.bufferMb).toBeGreaterThan(MIN_STAGED_BUFFER_MB)

      const flipStartedAt = Date.now()
      await updateTerminalSettings(fabricaPage, { terminalHiddenWorktreeRetentionBudget: true })
      await waitForRetentionBudgetSetting(fabricaPage, true)
      await expect
        .poll(() => countMountedPaneManagers(fabricaPage, victimTabIds), {
          timeout: 30_000,
      ...
      const after = await readRetentionMemorySample(fabricaPage)
      testInfo.annotations.push({
