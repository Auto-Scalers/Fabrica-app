...
test.describe('Inline TUI reveal convergence', () => {
  test('hidden-but-mounted tab reveal converges while the inline TUI streams', async ({
    ...
    try {
      ...
      await fabricaPage.waitForTimeout(Math.max(50, Math.min(PARKING_DELAY_MS / 2, 200)))
      expect(
        await isTerminalPaneMounted(fabricaPage, setup.tabId),
        'hidden-mounted scenario cold-parked before reveal'
      ).toBe(true)

      await activateTerminalTab(fabricaPage, setup.tabId)
      await waitForActiveTerminalManager(fabricaPage, 30_000)
      await assertRevealConvergence(fabricaPage, testInfo, setup, 'hidden-mounted-reveal')
    } finally {
      await setup.stop()
    }
  })
  ...
  test('worktree switch reveal converges after a hidden-time window resize', async ({
    ...
    try {
      ...
      const otherWorktreeId = await switchToOtherWorktree(fabricaPage, setup.worktreeId)
      ...
      await fabricaPage.waitForTimeout(2_500)

      await switchToWorktree(fabricaPage, setup.worktreeId)
      await activateTerminalTab(fabricaPage, setup.tabId)
      await waitForActiveTerminalManager(fabricaPage, 30_000)
      await assertRevealConvergence(fabricaPage, testInfo, setup, 'worktree-resize-reveal')
    } finally {
  ...
  test('parked tab reveal converges across repeated cycles while the inline TUI streams heavily', async ({
    ...
    try {
      ...
      for (let cycle = 0; cycle < CYCLES; cycle += 1) {
        ...
        await withCpuThrottle(fabricaPage, 6, async () => {
          await activateTerminalTab(fabricaPage, setup.tabId)
          await waitForActiveTerminalManager(fabricaPage, 30_000)
        ...
        await assertRevealConvergence(fabricaPage, testInfo, setup, `parked-heavy-reveal-c${cycle}`)
      }
    } finally {
      await setup.stop()
    }
  ...
  test('rapid tab hide/reveal flapping never wedges delivery for the streaming inline TUI', async ({
    ...
    try {
      ...
      for (let flap = 0; flap < 12; flap += 1) {
        await activateTerminalTab(fabricaPage, tabBId)
        await fabricaPage.waitForTimeout(50 + (flap % 3) * 120)
        await activateTerminalTab(fabricaPage, setup.tabId)
        await fabricaPage.waitForTimeout(50 + ((flap * 7) % 5) * 90)
      }
      await waitForActiveTerminalManager(fabricaPage, 30_000)
      await assertRevealConvergence(fabricaPage, testInfo, setup, 'tab-flapping-reveal')
    } finally {
      await setup.stop()
    }
  })
    ...
    try {
      ...
      await withCpuThrottle(fabricaPage, 6, async () => {
        for (let flap = 0; flap < 10; flap += 1) {
          ...
          await fabricaPage.waitForTimeout(60 + (flap % 4) * 110)
          await switchToWorktree(fabricaPage, setup.worktreeId)
          await fabricaPage.waitForTimeout(60 + ((flap * 5) % 4) * 130)
        }
      })
      await activateTerminalTab(fabricaPage, setup.tabId)
      await waitForActiveTerminalManager(fabricaPage, 30_000)
      await assertRevealConvergence(fabricaPage, testInfo, setup, 'worktree-flapping-reveal')
    } finally {
      await setup.stop()
    }
  })
  ...
  test('parked tab reveal converges after a parked-time window resize', async ({
    ...
    try {
      await createActiveTerminalTab(fabricaPage, setup.worktreeId)
      await createActiveTerminalTab(fabricaPage, setup.worktreeId)
      await waitForTabParked(fabricaPage, setup.tabId, { parkDelayMs: PARKING_DELAY_MS })
      ...
      const revealed = await waitForPaneIdentitySnapshot(fabricaPage, 1)
      expect(revealed.panes[0]?.ptyId).toBe(setup.ptyId)
      await assertRevealConvergence(fabricaPage, testInfo, setup, 'parked-resize-reveal')
    } finally {
      await resizeAppWindow(electronApp, 180, 120).catch(() => {})
      await setup.stop()
    }
