...
test.describe('Settings sidebar search on the Shortcuts pane', () => {
  test('pane-title-only query keeps rows visible and local search usable', async ({ fabricaPage }) => {
    ...
    const searchInput = fabricaPage.getByPlaceholder('Search settings')
    ...
    await expect(fabricaPage.getByRole('heading', { name: 'Shortcuts', exact: true })).toBeVisible()

    // Regression: a pane-title-only query used to blank the whole list (0/112).
    await expect(fabricaPage.getByText('Go to File', { exact: true })).toBeVisible()
    await expect(fabricaPage.getByText('No shortcuts match those filters.')).not.toBeVisible()
    ...
    const localSearch = fabricaPage.getByPlaceholder('Search command or keys')
    await localSearch.fill('go to')
    await expect(fabricaPage.getByText('Go to File', { exact: true })).toBeVisible()
    await expect(fabricaPage.getByText('Force Reload', { exact: true })).not.toBeVisible()

    // A row-matching global query still narrows the list as before.
    await localSearch.clear()
    await searchInput.fill('worktree')
    await expect(fabricaPage.getByText('Create worktree', { exact: true })).toBeVisible()
    await expect(fabricaPage.getByText('Go to File', { exact: true })).not.toBeVisible()
  })
})
