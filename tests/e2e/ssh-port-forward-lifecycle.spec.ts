...
const FORCE_SYSTEM_SSH = process.env.FABRICA_SSH_FORCE_SYSTEM_TRANSPORT === '1'
...
test.describe('Docker SSH port-forward lifecycle', () => {
  ...
  test('keeps a user-forwarded listener live across scan refresh @headful', async ({
    ...
    const marker = `FABRICA_FORWARD_${Date.now()}`
    ...
    try {
      ...
      await openPortsPanel(fabricaPage)
      ...
      await expect
        ...
        .toBe(remotePid)
      await expect(fabricaPage.getByText(`:${REMOTE_PORT}`, { exact: true })).toBeVisible()
      await expect
        .poll(
          () =>
            fabricaPage.evaluate(
        ...
        .toBe(true)
      await expect(fabricaPage.getByText(`:${REFRESH_BARRIER_PORT}`, { exact: true })).toBeVisible()
      ...
      await waitForSessionReady(fabricaPage, 60_000)
      await expect
        .poll(() => waitForActiveWorktree(fabricaPage), { timeout: 60_000 })
      ...
      await installRendererForwardCapture(fabricaPage)
      await openPortsPanel(fabricaPage)
      ...
      const postHydrationRoundTripForwards = await fabricaPage.evaluate(
      ...
      await expect(fabricaPage.getByText(`:${REFRESH_BARRIER_PORT}`, { exact: true })).toBeVisible()
      const staleSnapshotEvidence = await readPortForwardEvidence(fabricaPage, remote.targetId)
      ...
      await expect
        ...
        .toBe(true)
      await expect(fabricaPage.getByText(`:${SCAN_REFRESH_PORT}`, { exact: true })).toBeVisible()

      await unrelatedLocalPortReservation.release()
      const unrelatedForward = await addPortForward(fabricaPage, {
      ...
      await forceDockerSshRelayChannelReconnect(fabricaPage, target, remote.targetId)
      await expectForwardEvidence(fabricaPage, remote.targetId, [
        { localPort, remotePort: REMOTE_PORT },
        { localPort: unrelatedLocalPort, remotePort: REFRESH_BARRIER_PORT }
      ])
      ...
      const authorityBeforeTransportReconnect = await fabricaPage.evaluate(
      ...
      await expect
        .poll(
          async () => {
            const state = await fabricaPage.evaluate(
              (targetId) => window.__store?.getState().sshConnectionStates.get(targetId),
              remote.targetId
            )
            return (
      ...
      try {
        const collisionResult = await fabricaPage.evaluate(
      ...
      const evidence = await readPortForwardEvidence(fabricaPage, remote.targetId)
      ...
      const relayReconnectStates = await readSshStateCapture(fabricaPage)
