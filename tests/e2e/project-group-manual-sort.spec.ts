...
async function createProjectHeaderSortFixture(): Promise<string[]> {
  ...
  const root = realpathSync(await mkdtemp(path.join(os.tmpdir(), 'FABRICA-e2e-project-sort-')))
  tempRoots.push(root)
...
test.describe('Project Group manual sorting', () => {
  test('dragging a project header body reorders the visible project headers', async ({
    ...
    const projects = await seedProjectHeaderSortScenario(fabricaPage, repoPaths)

    await expect
      .poll(() => getProjectHeaderOrder(fabricaPage, projects), {
        timeout: 12_000,
  ...
  test('dragging a duplicate-ranked Project Group header reorders the visible headers', async ({
    ...
    await waitForSessionReady(fabricaPage)
    const groups = await seedDuplicateTabOrderProjectGroups(fabricaPage)

    await expect
      .poll(() => getProjectGroupHeaderOrder(fabricaPage, groups), {
        timeout: 12_000,
