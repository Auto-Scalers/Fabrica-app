...
test.describe('Terminal long table scroll restore repro', () => {
  test('reproduces long markdown table artifacts after workspace switch and scroll', async ({
    ...
    await fabricaPage.evaluate(() => {
      window.__store
        ?.getState()
        .markFeatureTipsSeen(['FABRICA-cli', 'cmd-j-palette', 'voice-dictation'])
    })
    ...
    const scriptPath = path.join(testRepoPath, `.FABRICA-long-table-${runId}.mjs`)
    writeFileSync(scriptPath, longMarkdownTableScript(runId))

    try {
      await sendToTerminal(fabricaPage, ptyId, `${nodeTerminalCommand([scriptPath])}\r`)
      ...
      await waitForActiveTerminalManager(fabricaPage, 30_000)
      await expect
        .poll(() => getTerminalContent(fabricaPage, 30_000), {
          timeout: 10_000,
          message: 'long table marker did not survive workspace switch'
        })
        .toContain(marker)

      await scrollActiveTerminalLikeUser(fabricaPage)
      await closeFeatureTips(fabricaPage)
      const diagnostics = await readTerminalRenderDiagnostics(fabricaPage)
  ...
  test('keeps narrow wrapped signer markdown table coherent after restore and scroll', async ({
    ...
    await setRenderedTableViewport(fabricaPage)
    await forceDarkTerminalRendererPath(fabricaPage)
    await ensureTerminalVisible(fabricaPage)
    await waitForActiveTerminalManager(fabricaPage, 30_000)
    const ptyId = await waitForActivePanePtyId(fabricaPage)
    await waitForPtyShellEcho(fabricaPage, ptyId, 15_000)
    ...
    const scriptPath = path.join(testRepoPath, `.FABRICA-narrow-signer-table-${runId}.mjs`)
    ...
    try {
      ...
      const content = await getTerminalContent(fabricaPage, 30_000)
  ...
  test('keeps real emoji markdown table right edge clean after restore and scroll', async ({
    ...
    await ensureTerminalVisible(fabricaPage)
    await waitForActiveTerminalManager(fabricaPage, 30_000)
    await setNarrowTerminalViewport(fabricaPage)
    const renderedTableTerminalCols = await waitForRenderedTerminalColumnsAtMost(
      fabricaPage,
      NARROW_TERMINAL_MAX_COLS
    )
    const ptyId = await waitForActivePanePtyId(fabricaPage)
    await waitForPtyColumnsAtMost(fabricaPage, ptyId, renderedTableTerminalCols)
    const runId = randomUUID()
    const marker = `EMOJI_FIXTURE_TABLE_RESTORE_${runId}`
    const scriptPath = path.join(testRepoPath, `.FABRICA-emoji-fixture-table-${runId}.mjs`)
    ...
    try {
      ...
      await waitForActiveTerminalManager(fabricaPage, 30_000)
      await setNarrowTerminalViewport(fabricaPage)
      await waitForRenderedTerminalColumnsAtMost(fabricaPage, NARROW_TERMINAL_MAX_COLS)
      await expect
        .poll(() => getTerminalContent(fabricaPage, 30_000), {
          timeout: 10_000,
        ...
        .toContain(marker)
      const generatedWidthContent = await getTerminalContent(fabricaPage, 30_000)
      ...
      await scrollActiveTerminalToText(fabricaPage, retainedEmojiCell)
      await closeFeatureTips(fabricaPage)
      await expect
        .poll(() => readActiveTerminalVisibleText(fabricaPage), {
          timeout: 5_000,
          message: `${retainedEmojiCell} row fragment should be visible before screenshot`
        })
        .toContain(retainedEmojiCell)
      const diagnostics = await readTerminalRenderDiagnostics(fabricaPage)
      const overpaint = await readTerminalRightEdgeOverpaint(fabricaPage)
      const wrapDiagnostics = await readTerminalBoxTableWrapDiagnostics(fabricaPage)
