...
const fakeGhDir = mkdtempSync(path.join(os.tmpdir(), 'FABRICA-e2e-fake-gh-'))
  ...
  launchEnv: [
    {
      ...
      FABRICA_GH_EXEC_TIMEOUT_MS: '1000'
    },
    { option: true }
  ]
})
...
test('GitHub Tasks drawer recovers when gh stalls on issue details', async ({
  ...
  const { repoId } = await fabricaPage.evaluate((repoPath) => {
  ...
  const detailHeading = fabricaPage.getByRole('heading', {
  ...
  await expect(fabricaPage.getByText('Unable to load details for this GitHub item.')).toBeVisible({
    timeout: 5_000
  })
