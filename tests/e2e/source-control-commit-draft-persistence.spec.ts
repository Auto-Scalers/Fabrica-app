...
test.describe('Source Control commit draft persistence', () => {
  test('preserves a typed draft when the sidebar tab remounts', async ({
    ...
    try {
      ...
      await openSourceControlForWorktree(fabricaPage, testRepoPath, firstWorktree.worktreePath)
      ...
      await expect
        .poll(
          async () => fabricaPage.evaluate(() => window.__store?.getState().rightSidebarTab ?? null),
          { timeout: 5_000 }
        )
        .toBe('explorer')
      ...
      await openSourceControlForWorktree(fabricaPage, testRepoPath, secondWorktree.worktreePath)
      await expect(textarea).toBeVisible({ timeout: 10_000 })
