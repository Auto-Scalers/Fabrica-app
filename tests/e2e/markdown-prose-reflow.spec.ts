...
test.describe('Markdown prose reflow', () => {
  ...
  test('hard-wrapped prose reflows as one document paragraph', async ({ fabricaPage }, testInfo) => {
    let filePath: string | null = null

    try {
      filePath = await openHardWrappedFixture(fabricaPage, testInfo)
      const metrics = await getGoalParagraphMetrics(fabricaPage)
  ...
  test('deleting an inserted empty paragraph keeps hard-wrapped prose reflowing', async ({
    ...
    try {
      filePath = await openHardWrappedFixture(fabricaPage, testInfo)
      await placeCaretAtGoalParagraphEnd(fabricaPage)
  ...
  test('deleting slash text then the empty paragraph keeps hard-wrapped prose reflowing', async ({
    ...
    try {
      ...
      await placeCaretAtGoalParagraphEnd(fabricaPage)
      await fabricaPage.keyboard.press('Enter')
      await fabricaPage.keyboard.type('/')
      await fabricaPage.keyboard.press('Backspace')
      await fabricaPage.keyboard.press('Backspace')
  ...
  test('deleting the block boundary before a heading keeps hard-wrapped prose reflowing', async ({
    ...
    try {
      filePath = await openHardWrappedFixture(fabricaPage, testInfo)
      await placeCaretAtHeadingStart(fabricaPage)
      await fabricaPage.keyboard.press('Backspace')
