...
async function dispatchCtrlCToActiveTerminalTextarea(
  ...
  return page.evaluate((dispatchOptions) => {
    ...
    // synthetic DOM events still exercise FABRICA's installed xterm boundary.
...
test.describe('Terminal Shortcuts', () => {
  ...
  test('Shift+Enter follows the pane Kitty keyboard state', async ({ fabricaPage, electronApp }) => {
    await installMainProcessPtyWriteSpy(electronApp)
    const ptyId = await waitForActivePanePtyId(fabricaPage)

    await pressAndExpectWrite(fabricaPage, electronApp, 'Shift+Enter', '\x1b\r')
    ...
    await execInTerminal(fabricaPage, ptyId, "printf '\\033[>1u'")
    await expect.poll(() => getKittyKeyboardFlags(fabricaPage)).toBe(1)
    await pressAndExpectWrite(fabricaPage, electronApp, 'Shift+Enter', '\x1b[13;2u')
    ...
    await sendToTerminal(fabricaPage, ptyId, '\x15\x03')
    await execInTerminal(fabricaPage, ptyId, "printf '\\033[=0u'")
    await expect.poll(() => getKittyKeyboardFlags(fabricaPage)).toBe(0)
    await pressAndExpectWrite(fabricaPage, electronApp, 'Shift+Enter', '\x1b\r')
  })
  ...
  test('Droid gets CSI-u Shift+Enter on Windows without changing Antigravity', async ({
    ...
    await installMainProcessPtyWriteSpy(electronApp)
    await waitForActivePanePtyId(fabricaPage)
    const paneKey = await setActivePaneForegroundAgent(fabricaPage, 'droid')
    try {
      await pressAndExpectWrite(fabricaPage, electronApp, 'Shift+Enter', '\x1b[13;2u', 2)
      await setActivePaneForegroundAgent(fabricaPage, 'antigravity')
      await pressAndExpectWrite(fabricaPage, electronApp, 'Shift+Enter', '\x1b\r')
    } finally {
  ...
  test('Windows forwards genuine Ctrl+Alt text chords to the PTY', async ({
    ...
    await pressAndExpectWrite(fabricaPage, electronApp, 'Control+Alt+u', '\x1b\x15')
    await pressAndExpectWrite(fabricaPage, electronApp, 'Control+Alt+2', '\x1b2')
    await pressAndExpectWrite(fabricaPage, electronApp, 'Control+Alt+;', '\x1b;')
  })
  ...
  test('Ctrl+Enter protects local ConPTY shells without breaking trusted TUI chords', async ({
    ...
    if (process.platform === 'win32') {
      await pressAndExpectWrite(fabricaPage, electronApp, 'Control+Enter', '\r')
      const paneKey = await setActivePaneForegroundAgent(fabricaPage, 'droid')
      try {
        // Droid queries CSI-u without activating live flags; trusted process evidence preserves cue/queue.
        await pressAndExpectWrite(fabricaPage, electronApp, 'Control+Enter', '\x1b[13;5u')
      } finally {
        await fabricaPage.evaluate(
          (key) => window.__store?.getState().clearPaneForegroundAgent(key),
          paneKey
  ...
  test('plain Ctrl+C sends ETX under kitty keyboard reporting', async ({
    ...
    await installMainProcessPtyWriteSpy(electronApp)
    await waitForActivePanePtyId(fabricaPage)
    await enableKittyKeyboardReporting(fabricaPage, 31)
    await clearPtyWriteLog(electronApp)
    await focusActiveTerminalInput(fabricaPage)
    await fabricaPage.keyboard.down('Control')
    await fabricaPage.keyboard.up('Control')
    expect((await getPtyWrites(electronApp)).join('')).toBe('')
    await clearPtyWriteLog(electronApp)

    expect(await dispatchCtrlCToActiveTerminalTextarea(fabricaPage, { keyupCtrlKey: false })).toEqual({
      keydownDefaultPrevented: false,
    ...
    await expect
      .poll(async () => await getKittyKeyboardFlags(fabricaPage), {
        timeout: 5_000,
    ...
    await clearPtyWriteLog(electronApp)
    await focusActiveTerminalInput(fabricaPage)
    await fabricaPage.keyboard.type('x')
    await expect
  ...
  test('@headful Codex-like background output stays visible without disabling WebGL in auto mode', async ({
    ...
    const hasPane = await fabricaPage.evaluate(() => {
      const state = window.__store?.getState()
      const worktreeId = state?.activeWorktreeId
      const tabId =
        state?.activeTabType === 'terminal'
    ...
    const webglActive = await fabricaPage
      .waitForFunction(
        () => {
          const state = window.__store?.getState()
          const worktreeId = state?.activeWorktreeId
    ...
    await execInTerminal(fabricaPage, ptyId, `printf '\\033[48;2;52;52;52m  ${marker}  \\033[0m\\n'`)
    await waitForTerminalOutput(fabricaPage, marker)

    await expect
      .poll(
        () =>
          fabricaPage.evaluate((expectedMarker) => {
            const state = window.__store?.getState()
            const worktreeId = state?.activeWorktreeId
            const tabId =
              state?.activeTabType === 'terminal'
  ...
  test('floating terminal owns tab switch shortcuts while focused', async ({ fabricaPage }) => {
    const scenario = await seedFloatingTerminalTabSwitchScenario(fabricaPage)
    await fabricaPage.evaluate(async () => {
      ...
      if (!document.querySelector('[data-floating-terminal-panel][aria-hidden="false"]')) {
        window.dispatchEvent(new CustomEvent('FABRICA-toggle-floating-terminal'))
      }
    })
    await expect(
      fabricaPage.locator('[data-floating-terminal-panel][aria-hidden="false"]')
    ).toBeVisible()
    await focusFloatingTerminal(fabricaPage)

    await fabricaPage.keyboard.press(`${mod}+Shift+BracketRight`)
    await expect
      .poll(() => getActiveFloatingTerminalTabId(fabricaPage), {
        timeout: 5_000,
        message: 'floating terminal did not switch to the next tab'
      })
      .toBe(scenario.floatingSecondTabId)
    await expect
      .poll(() => getActiveBackgroundTerminalTabId(fabricaPage), {
        timeout: 1_000,
    ...
    await focusFloatingTerminal(fabricaPage)
    await fabricaPage.keyboard.press(`${mod}+Shift+BracketLeft`)
    await expect
      .poll(() => getActiveFloatingTerminalTabId(fabricaPage), {
        timeout: 5_000,
    ...
    await expect(getActiveBackgroundTerminalTabId(fabricaPage)).resolves.toBe(
  ...
  test('all terminal chords reach the PTY or fire their action', async ({
    ...
    await focusActiveTerminalInput(fabricaPage)
    await fabricaPage.keyboard.press(`${mod}+k`)
    await expect
      .poll(async () => (await getTerminalContent(fabricaPage)).includes(marker), {
        timeout: 5_000,
    ...
    await waitForPaneCount(fabricaPage, panesBeforeSplit + 1)
    ...
    await focusActiveTerminalInput(fabricaPage)
    await fabricaPage.keyboard.press(`${mod}+BracketRight`)
    await focusActiveTerminalInput(fabricaPage)
    await fabricaPage.keyboard.press(`${mod}+BracketLeft`)
    expect(await countVisibleTerminalPanes(fabricaPage)).toBe(panesBeforeSplit + 1)
    ...
    await focusActiveTerminalInput(fabricaPage)
    await fabricaPage.keyboard.press(`${mod}+Shift+Enter`)
    await expect
    ...
    await waitForPaneCount(fabricaPage, panesBeforeHSplit + 1)
    await waitForActivePanePtyId(fabricaPage)
    await closeActivePaneAndSettle(fabricaPage, panesBeforeHSplit)

    // Cmd/Ctrl+F toggles the search overlay.
    await focusActiveTerminalInput(fabricaPage)
    await fabricaPage.keyboard.press(`${mod}+f`)
    const searchInput = fabricaPage.locator('[data-terminal-search-root] input').first()
    ...
    await expect(fabricaPage.locator('[data-terminal-search-root]').first()).toBeHidden({
      timeout: 3_000
    })
  })
  ...
  test('Cmd+Up/Down scrolls terminal viewport without writing to the PTY on macOS', async ({
    ...
    await execInTerminal(fabricaPage, ptyId, `for i in {1..120}; do echo ${marker}_$i; done`)
    await waitForTerminalOutput(fabricaPage, `${marker}_120`)

    await expect
      .poll(
        async () => {
          const viewport = await getActiveTerminalViewport(fabricaPage)
    ...
    await clearPtyWriteLog(electronApp)
    await focusActiveTerminalInput(fabricaPage)
    await fabricaPage.keyboard.press('Meta+ArrowUp')
    await expect
      .poll(async () => getActiveTerminalViewport(fabricaPage), {
        timeout: 5_000,
    ...
    await focusActiveTerminalInput(fabricaPage)
    await fabricaPage.keyboard.press('Meta+ArrowDown')
    await expect
      .poll(
        async () => {
          const viewport = await getActiveTerminalViewport(fabricaPage)
  ...
  test('Shift with Russian layout text reaches the PTY as Cyrillic under kitty keyboard reporting', async ({
    ...
    const dispatch = await pressShiftedRussianLayoutKey(fabricaPage)
