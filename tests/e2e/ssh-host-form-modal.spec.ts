...
test.describe('SSH host add/edit modal', () => {
  ...
  test.afterEach(async ({ fabricaPage }) => {
    const ids = await listTargetIdsByLabelPrefix(fabricaPage, HOST_PREFIX)
    if (ids.length > 0) {
      await removeSshTargets(fabricaPage, ids)
    }
  })
  ...
  test('opens add/edit form in a viewport-stable dialog over a long host list', async ({
    ...
    const seeded = await seedSshTargets(fabricaPage, 10)
    await openSshHostSettings(fabricaPage)

    const sshSection = fabricaPage.locator('[data-settings-section="ssh"]')
    ...
    const editDialog = fabricaPage.getByRole('dialog', { name: 'Edit SSH host' })
    ...
    await fabricaPage.locator('[data-slot="dialog-overlay"]').click({ position: { x: 8, y: 8 } })
    await expect(editDialog).toBeVisible()
    ...
    await createdCard.getByRole('button', { name: 'Edit target' }).click()
    const reopened = fabricaPage.getByRole('dialog', { name: 'Edit SSH host' })
  ...
  test('add-ssh-host settings intent opens the same modal dialog', async ({ fabricaPage }) => {
    ...
    await expect(fabricaPage.getByPlaceholder('Search settings')).toBeVisible({ timeout: 10_000 })
    await dismissTransientAnnouncement(fabricaPage)

    const dialog = fabricaPage.getByRole('dialog', { name: 'Add SSH host' })
