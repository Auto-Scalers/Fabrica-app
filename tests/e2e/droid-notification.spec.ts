...
async function getRendererFABRICAchedAgentStatuses(page: Page): Promise<AgentStatusSummary[]> {
...
test.describe('Droid notifications', () => {
  test('Codex hook completion dispatches while its worktree is inactive', async ({
    ...
    await sendToTerminal(fabricaPage, ptyId, `printf '${readyMarker}\\n'\r`)
    await waitForTerminalOutput(fabricaPage, readyMarker)

    const { paneKey, worktreeId } = await waitForActivePaneHookDescriptor(fabricaPage)
    ...
    await expect
      .poll(
        async () =>
          (await getRendererFABRICAchedAgentStatuses(fabricaPage)).some(
            (status) =>
    ...
    await switchToOtherExistingWorktree(fabricaPage)
    ...
    await expect
      .poll(
        async () =>
          (await getAgentStatuses(fabricaPage)).some(
            (status) =>
    ...
    await expect
      .poll(async () => isWorktreeUnread(fabricaPage, worktreeId), {
        timeout: 10_000,
  ...
  test('recognized agent title completion dispatches one task-complete notification', async ({
    ...
    await emitOscTitle(fabricaPage, ptyId, 'Codex working')
    await expect
      .poll(async () => (await getRendererTitleLog(fabricaPage)).includes('Codex working'), {
        timeout: 10_000,
        message: 'Codex working title did not reach the renderer before completion'
      })
      .toBe(true)

    await emitOscTitle(fabricaPage, ptyId, 'Codex done')
    await expect
      .poll(async () => (await getRendererTitleLog(fabricaPage)).includes('Codex done'), {
        timeout: 10_000,
  ...
  test('Factory Droid needs-input native title does not dispatch a task-complete notification', async ({
    ...
    await expect
      .poll(
        async () => (await getRendererTitleLog(fabricaPage)).includes('Factory Droid needs input'),
        {
          timeout: 10_000,
