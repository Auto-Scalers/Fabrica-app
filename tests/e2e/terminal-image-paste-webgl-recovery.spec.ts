...
test.describe('terminal image paste WebGL recovery @headful', () => {
  test('clears the WebGL atlas after a real image clipboard paste', async ({
    ...
    const scriptPath = path.join(testRepoPath, `.FABRICA-image-paste-redraw-${marker}.mjs`)
    ...
    try {
      ...
      await waitForTerminalOutput(fabricaPage, `READY_${marker}`, 10_000)

      await forceWebgl(fabricaPage)
      ...
      expect(await patchAtlasCounter(fabricaPage)).toBe(true)

      await fabricaPage.locator('.xterm-helper-textarea').first().focus()
      await fabricaPage.evaluate(
        (dataUrl) => window.api.ui.writeClipboardImage(dataUrl),
        CLIPBOARD_IMAGE_DATA_URL
      )
      await fabricaPage.keyboard.press(process.platform === 'darwin' ? 'Meta+V' : 'Control+V')
      await waitForTerminalOutput(fabricaPage, `DONE_${marker}`, 10_000)

      await expect.poll(() => readAtlasResetCount(fabricaPage), { timeout: 2_000 }).toBeGreaterThan(0)
    } finally {
