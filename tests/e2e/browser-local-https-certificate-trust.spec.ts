...
test.describe('local HTTPS certificate trust', () => {
  ...
  test('approves one exact local certificate endpoint without trusting sibling tabs or ports', async ({
    ...
    try {
      ...
      const firstTab = await createBrowserTab(fabricaPage, worktreeId, firstServer.schemeLessUrl)
      const firstSlot = browserSlot(fabricaPage, firstTab.id)
      ...
      await expect
        .poll(() => readBrowserHeading(fabricaPage, firstTab.id), { timeout: 10_000 })
        .toBe('Local HTTPS request 1')
      await expect
        .poll(() => readBrowserState(fabricaPage, firstTab.id, '__localTlsState'))
      ...
      const secondTab = await createBrowserTab(fabricaPage, worktreeId, firstServer.secureUrl)
      const secondSlot = browserSlot(fabricaPage, secondTab.id)
      await expect(
      ...
      const probeTab = await createBrowserTab(fabricaPage, worktreeId, siblingProbeServer.url)
      await expect
        .poll(() => readBrowserState(fabricaPage, probeTab.id, '__siblingTlsProbe'))
        .toEqual({ asset: 'blocked', webSocket: 'blocked' })
      expect(firstServer.assetRequestCount()).toBe(1)
      expect(firstServer.webSocketConnectionCount()).toBe(1)

      await switchToBrowserTab(fabricaPage, worktreeId, firstTab.id)
      await reloadBrowserGuest(fabricaPage, firstTab.id)
      ...
      const firstAddressBar = firstSlot.locator('[data-FABRICA-browser-address-bar="true"]')
