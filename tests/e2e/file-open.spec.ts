...
test.describe('File Open & Markdown Preview', () => {
   ...
   * - you can open files (from the right sidebar)
   */
  test('opening the right sidebar shows file explorer', async ({ fabricaPage }) => {
   ...
   * - you can open files (from the right sidebar)
   */
  test('clicking a file in the file explorer opens it in an editor tab', async ({ fabricaPage }) => {
    const worktreeId = (await getActiveWorktreeId(fabricaPage))!
    await openFileExplorer(fabricaPage)

    const filesBefore = await getOpenFiles(fabricaPage, worktreeId)

    // Click a known non-directory file
    const clickedFile = await clickFileInExplorer(fabricaPage, [
      'package.json',
      'tsconfig.json',
      '.gitignore',
      'README.md'
    ...
    await expect.poll(async () => getActiveTabType(fabricaPage), { timeout: 5_000 }).toBe('editor')

    // There should be a new open file
    await expect
      .poll(async () => (await getOpenFiles(fabricaPage, worktreeId)).length, { timeout: 5_000 })
   ...
   * - you can open .md files and they show up as preview (from the right sidebar)
   */
  test('opening a .md file shows markdown content', async ({ fabricaPage }) => {
    await openFileExplorer(fabricaPage)
    const clickedFile = await clickFileInExplorer(fabricaPage, ['README.md', 'CLAUDE.md'])
    expect(clickedFile).not.toBeNull()
   ...
   */
  test('editor tab retains state when switching to terminal and back', async ({ fabricaPage }) => {
    const worktreeId = (await getActiveWorktreeId(fabricaPage))!
    await openFileExplorer(fabricaPage)
    ...
    const openFilesBefore = await getOpenFiles(fabricaPage, worktreeId)
    expect(openFilesBefore.length).toBeGreaterThan(0)

    const editorFileId = openFilesBefore[0].id

    // Switch to a terminal tab
    await switchToTerminal(fabricaPage, worktreeId)
    await expect.poll(async () => getActiveTabType(fabricaPage), { timeout: 3_000 }).not.toBe('editor')

    // Switch back to the same editor tab
    await switchToEditor(fabricaPage, editorFileId)
