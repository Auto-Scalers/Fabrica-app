...
const RUN_LOCALHOST_SSH = process.env.FABRICA_E2E_SSH_LOCALHOST === '1'
const RUN_REMOTE_HOOKS =
  process.env.FABRICA_FEATURE_REMOTE_AGENT_HOOKS === undefined ||
  (process.env.FABRICA_FEATURE_REMOTE_AGENT_HOOKS.trim() !== '' &&
    process.env.FABRICA_FEATURE_REMOTE_AGENT_HOOKS.trim() !== '0')
...
function parsePort(value: string | undefined): number {
  ...
  throw new Error(`Invalid FABRICA_E2E_SSH_PORT: ${value}`)
}

function currentUsername(): string {
  return (
    process.env.FABRICA_E2E_SSH_USER ??
    process.env.USER ??
...
function readLocalhostSshTarget(): LocalhostSshTarget {
  const configHost = process.env.FABRICA_E2E_SSH_CONFIG_HOST?.trim()
  const host = process.env.FABRICA_E2E_SSH_HOST?.trim() ?? (configHost ? '' : '127.0.0.1')
  const identityFile = process.env.FABRICA_E2E_SSH_IDENTITY_FILE?.trim()

  return {
    label: `Localhost SSH E2E ${Date.now()}`,
    host,
    port: parsePort(process.env.FABRICA_E2E_SSH_PORT),
...
function marker(name: string): string {
  return `__FABRICA_${name}_${Date.now()}__`
}
...
async function postCodexHook(
  ...
  await execInTerminal(
    ...
    [
      ...
      '  echo __FABRICA_AGENT_HOOK_ENV_MISSING__',
      'else',
      `  hook_payload=${shellQuote(JSON.stringify(payload))}`,
      '  (',
      '    sleep 0.1',
      '    if curl -sS -X POST "http://127.0.0.1:${FABRICA_AGENT_HOOK_PORT}/hook/codex" \\',
...
test.describe('Localhost SSH', () => {
  test.skip(
    !RUN_LOCALHOST_SSH,
    'Set FABRICA_E2E_SSH_LOCALHOST=1 to run this local-machine-only SSH E2E test.'
  )
  test.skip(
    !RUN_REMOTE_HOOKS,
    'Unset FABRICA_FEATURE_REMOTE_AGENT_HOOKS or set it to 1 so remote PTYs keep pane identity and forward hook events.'
  )
  ...
  test('routes a terminal and agent-hook status over localhost SSH', async ({
    ...
    const remote = await fabricaPage.evaluate(
    ...
    await ensureTerminalVisible(fabricaPage, 30_000)
    await waitForActiveTerminalManager(fabricaPage, 45_000)
    const ptyId = await waitForActivePanePtyId(fabricaPage, 45_000)
    const paneKey = await fabricaPage.evaluate(() => {
      const store = window.__store
      if (!store) {
        throw new Error('Store unavailable')
      }
    ...
    await execInTerminal(fabricaPage, ptyId, emitMarkerCommand(terminalMarker))
    await waitForTerminalOutput(fabricaPage, terminalMarker, 20_000)
    ...
    await execInTerminal(
      ...
      ptyId,
      [
        `if [ "$FABRICA_PANE_KEY" = ${shellQuote(paneKey)} ] && [ -n "$FABRICA_AGENT_HOOK_PORT" ] && [ -n "$FABRICA_AGENT_HOOK_TOKEN" ] && /bin/sh -c 'test -n "$FABRICA_PANE_KEY" && test -n "$FABRICA_AGENT_HOOK_PORT" && test -n "$FABRICA_AGENT_HOOK_TOKEN"'; then`,
        `  ${emitMarkerCommand(envMarker)}`,
        'else',
        '  token_state=${FABRICA_AGENT_HOOK_TOKEN:+set}',
        `  printf '%s pane=%s port=%s token=%s endpoint=%s\\n' ${shellQuote(envFailedMarker)} "$FABRICA_PANE_KEY" "$FABRICA_AGENT_HOOK_PORT" "$token_state" "$FABRICA_AGENT_HOOK_ENDPOINT"`,
        'fi'
      ].join('\n')
    )
    await waitForTerminalOutput(fabricaPage, envMarker, 20_000)
    ...
    await execInTerminal(
      ...
      ptyId,
      [
        'opencode_status_file="$OPENCODE_CONFIG_DIR/plugins/FABRICA-opencode-status.js"',
        'pi_status_file="$HOME/.pi/agent/extensions/FABRICA-agent-status.ts"',
    ...
    await waitForTerminalOutput(fabricaPage, pluginOverlayMarker, 20_000)

    const prompt = `FABRICA ssh e2e prompt ${Date.now()}`
    await postCodexHook(
      fabricaPage,
      ptyId,
    ...
    const ctrlPrompt = `FABRICA ssh ctrl-c interrupt ${Date.now()}`
    await postCodexHook(
      fabricaPage,
      ptyId,
    ...
    await focusTerminal(fabricaPage)
    await fabricaPage.keyboard.press('Control+C')
    ...
    const escapePrompt = `FABRICA ssh escape interrupt ${Date.now()}`
    await postCodexHook(
      fabricaPage,
      ptyId,
