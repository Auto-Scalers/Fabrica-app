...
function createIsolatedProofRepo(): string {
  // Why realpathSync: macOS tmpdir symlinks through /private and FABRICA canonicalizes repo.path.
  const repoDir = realpathSync(mkdtempSync(path.join(os.tmpdir(), 'FABRICA-mouse-reattach-repo-')))
  ...
  writeFileSync(path.join(repoDir, 'README.md'), '# FABRICA mouse-mode reattach proof repo\n')
  git('add', '-A')
...
// Why: this suite quits and relaunches FABRICA against one userDataDir, and the
