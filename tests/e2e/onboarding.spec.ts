...
test.describe('Onboarding flow', () => {
  ...
  test.beforeEach(async ({ fabricaPage }) => {
    // Per-test userData is freshly minted by the fabricaPage fixture, so persisted
  ...
  test('renders on first launch with the agent step active', async ({ fabricaPage }) => {
    ...
    await expect(onboardingFooterButton(fabricaPage, /^Continue\b/)).toBeVisible()
    await expect(onboardingFooterButton(fabricaPage, SKIP_TO_PROJECT_SETUP_BUTTON)).toBeVisible()
  ...
  test('Continue advances steps, persists progress, and applies user-visible settings', async ({
    ...
    const codexVisible = await codexButton
      ...
      .catch(() => false)
    if (!codexVisible) {
      await fabricaPage.getByText(/Show \d+ more agents/).click()
    }
    await codexButton.click()

    await continueOnboarding(fabricaPage)
    await expect(fabricaPage.getByRole('heading', { name: /Make it feel like home/i })).toBeVisible()
    await expectOnboardingProgress(fabricaPage, /^2 of [345]$/)
    await expect
    ...
    await expect
      .poll(async () => (await getSettings(fabricaPage)).defaultTuiAgent, { timeout: 5_000 })
      .toBe(targetAgent)
    ...
    const startingTheme = await getDocumentThemeClass(fabricaPage)
    ...
    await fabricaPage.getByRole('button', { name: oppositeTileName }).click()
    await expect
      .poll(async () => getDocumentThemeClass(fabricaPage), { timeout: 5_000 })
      .toBe(oppositeTheme)
    ...
    await expect
      .poll(async () => (await getSettings(fabricaPage)).theme, { timeout: 5_000 })
      .toBe(oppositeTheme)
    await continueThroughOptionalTaskSourcesAndWindowsTerminal(fabricaPage)
    await expectOnboardingProgress(fabricaPage, /^[345] of [345]$/)
    await expect
      .poll(async () => [3, 4].includes((await getOnboardingState(fabricaPage)).lastCompletedStep), {
        timeout: 5_000,
    ...
    await expectOnboardingNotificationSound(fabricaPage, /System Default/i)
    await expect(fabricaPage.getByRole('button', { name: /Send Test Notification/i })).toBeVisible()
    await expectOnboardingCustomSoundOption(fabricaPage)

    await continueFromPostNotificationsToRepo(fabricaPage)
    ...
    await expect
      .poll(
        async () => {
          const s = await getSettings(fabricaPage)
          return {
    ...
    await expect
      .poll(
        async () => {
          const state = await getOnboardingState(fabricaPage)
          return {
  ...
  test('Cmd/Ctrl+Enter advances steps like Continue', async ({ fabricaPage }) => {
    await expect(fabricaPage.getByRole('heading', { name: /Pick your default agent/i })).toBeVisible({
      timeout: 15_000
    })
    ...
    const isMac = await fabricaPage.evaluate(() => navigator.userAgent.includes('Mac'))
    ...
    await fabricaPage.locator('footer').click({ position: { x: 1, y: 1 } })
    await fabricaPage.keyboard.press(accelerator)
    await expect(fabricaPage.getByRole('heading', { name: /Make it feel like home/i })).toBeVisible()
    await expect
      .poll(async () => (await getOnboardingState(fabricaPage)).lastCompletedStep, {
        timeout: 5_000
  ...
  test('Skip opens Add Project, saves the selected agent, and completes onboarding', async ({
    ...
    await onboardingFooterButton(fabricaPage, SKIP_TO_PROJECT_SETUP_BUTTON).click()

    await expectAddProjectDialog(fabricaPage)
  ...
  test('Skip from theme restores the entry theme choice', async ({ fabricaPage }) => {
    await expect(fabricaPage.getByRole('heading', { name: /Pick your default agent/i })).toBeVisible({
      timeout: 15_000
    })
    await continueOnboarding(fabricaPage)
    ...
    const entryTheme = (await getSettings(fabricaPage)).theme
    const startingTheme = await getDocumentThemeClass(fabricaPage)
    const oppositeTheme: 'dark' | 'light' = startingTheme === 'dark' ? 'light' : 'dark'
    const oppositeTileName = oppositeTheme === 'light' ? /Bright & crisp/ : /Easy on the eyes/
    await fabricaPage.getByRole('button', { name: oppositeTileName }).click()
  ...
  test('Skip preserves runtime server project setup UI', async ({ fabricaPage }) => {
    await expect(fabricaPage.getByRole('heading', { name: /Pick your default agent/i })).toBeVisible({
      timeout: 15_000
    })
    ...
    const environmentId = await fabricaPage.evaluate(async (code) => {
    ...
    await expect
      .poll(async () => (await getSettings(fabricaPage)).activeRuntimeEnvironmentId, {
        timeout: 5_000
      })
      .toBe(environmentId)
    ...
    await expect(fabricaPage.getByText('Existing Git repository or folder on this host')).toBeVisible()
    await expect(fabricaPage.getByRole('button', { name: /Browse folder/i })).toBeVisible()
    await expect(fabricaPage.getByRole('button', { name: /Clone from URL/i })).toBeVisible()
    await expect(fabricaPage.getByRole('button', { name: /Create new project/i })).toBeVisible()
    await expect(onboardingFooterButton(fabricaPage, SKIP_TO_PROJECT_SETUP_BUTTON)).toHaveCount(0)
    expect((await getOnboardingState(fabricaPage)).closedAt).not.toBeNull()
  })

  test('Skip from notifications does not request permission', async ({ fabricaPage }) => {
    await expect(fabricaPage.getByRole('heading', { name: /Pick your default agent/i })).toBeVisible({
      timeout: 15_000
    })
    await continueOnboarding(fabricaPage)
    await expect(fabricaPage.getByRole('heading', { name: /Make it feel like home/i })).toBeVisible()
    await continueFromThemeToNotifications(fabricaPage)

    await fabricaPage.evaluate(() => {
      localStorage.removeItem('FABRICA.e2e.notificationPermissionRequested')
      window.api.notifications.requestPermission = async () => {
        localStorage.setItem('FABRICA.e2e.notificationPermissionRequested', '1')
    ...
    await expect
      .poll(
        async () =>
          fabricaPage.evaluate(() => localStorage.getItem('FABRICA.e2e.notificationPermissionRequested')),
        { timeout: 5_000 }
      )
      .toBeNull()
  })

  test('selected agent button reports aria-pressed=true', async ({ fabricaPage }) => {
    await expect(fabricaPage.getByRole('heading', { name: /Pick your default agent/i })).toBeVisible({
      timeout: 15_000
    })
  ...
  test('notification sound choice persists on Continue', async ({ fabricaPage }) => {
    await expect(fabricaPage.getByRole('heading', { name: /Pick your default agent/i })).toBeVisible({
      timeout: 15_000
    })
    await continueOnboarding(fabricaPage)
    ...
    await chooseOnboardingNotificationSound(fabricaPage, /^Ding$/i)
  ...
  test('typing in the clone-url input does not hijack Enter as a global shortcut', async ({
    ...
    await continueOnboarding(fabricaPage)
    await continueFromPostNotificationsToRepo(fabricaPage)
    await fabricaPage.getByRole('button', { name: /Clone from URL/i }).click()
    ...
    const isMac = await fabricaPage.evaluate(() => navigator.userAgent.includes('Mac'))
    const accelerator = isMac ? 'Meta+Enter' : 'Control+Enter'
    const input = fabricaPage.getByPlaceholder('https://github.com/user/repo.git')
    await input.click()
    ...
    await expect(fabricaPage.getByRole('heading', { name: /Clone from URL/i })).toBeVisible()
    await expect(input).toBeVisible()
    expect((await getOnboardingState(fabricaPage)).closedAt).not.toBeNull()
  })

  test('Back returns to the previous step without losing progress', async ({ fabricaPage }) => {
    await expect(fabricaPage.getByRole('heading', { name: /Pick your default agent/i })).toBeVisible({
      timeout: 15_000
    })
  ...
  test('final notification step can be dismissed via Escape or click-off', async ({ fabricaPage }) => {
    await expect(fabricaPage.getByRole('heading', { name: /Pick your default agent/i })).toBeVisible({
      timeout: 15_000
    })
    ...
    await expect(fabricaPage.getByRole('heading', { name: /Set up notifications/i })).toBeVisible()
    await expect(onboardingFooterButton(fabricaPage, SKIP_TO_PROJECT_SETUP_BUTTON)).toHaveCount(0)
    ...
    await fabricaPage.getByRole('button', { name: /No, keep going/i }).click()
    await expectOnboardingSkipConfirmationClosed(fabricaPage)
    await expect(fabricaPage.getByRole('heading', { name: /Set up notifications/i })).toBeVisible()
