...
import {
  ...
  seedFABRICASshTargetMatchingAlias,
...
async function importPairThenDeleteAlias(
  ...
  await picker.getByRole('button', { name: 'Add all 2 to FABRICA' }).click()
  await expect(page.getByText('Added 2 hosts to FABRICA.')).toBeVisible({ timeout: 15_000 })
...
test.describe('SSH config host import (bulk + settings re-adopt)', () => {
  ...
  test('P5: already-in-FABRICA badge, disabled row, and Add all counts only new hosts', async ({
    ...
    await seedFABRICASshTargetMatchingAlias(fabricaPage, {
    ...
    await expect(alphaRow.getByText('In FABRICA', { exact: true })).toBeVisible()

    await expect(bravoRow).toBeVisible()
    await expect(bravoRow).toBeEnabled()
    await expect(bravoRow.getByText('In FABRICA', { exact: true })).toHaveCount(0)

    await expect(picker.getByRole('button', { name: 'Add all 1 to FABRICA' })).toBeEnabled()
  ...
  test('P6: Add all N to FABRICA imports new hosts; re-open shows all in FABRICA', async ({
    ...
    await picker.getByRole('button', { name: 'Add all 2 to FABRICA' }).click()
    await expect(fabricaPage.getByText('Added 2 hosts to FABRICA.')).toBeVisible({ timeout: 15_000 })
    await expect(fabricaPage.getByRole('dialog', { name: 'Choose from ~/.ssh/config' })).toBeHidden({
      timeout: 10_000
    })
    await expect(fabricaPage.getByRole('dialog', { name: 'Add SSH host' })).toBeHidden({
      timeout: 10_000
    })
    ...
    const reopened = await openSshConfigHostPicker(fabricaPage)
    await expect(configHostRow(reopened, hosts.alpha)).toBeDisabled()
    await expect(
      configHostRow(reopened, hosts.alpha).getByText('In FABRICA', { exact: true })
    ).toBeVisible()
    await expect(configHostRow(reopened, hosts.bravo)).toBeDisabled()
    await expect(
      configHostRow(reopened, hosts.bravo).getByText('In FABRICA', { exact: true })
    ).toBeVisible()
  ...
  test('P7: Add all does not re-adopt deleted config hosts (suppress tombstones)', async ({
    ...
    await expect(alphaRow.getByText('Removed from FABRICA', { exact: true })).toBeVisible()
    await expect(alphaRow.getByText('In FABRICA', { exact: true })).toHaveCount(0)
    await expect(configHostRow(picker, hosts.bravo)).toBeVisible()
    await expect(
      configHostRow(picker, hosts.bravo).getByText('In FABRICA', { exact: true })
    ).toBeVisible()
    await expect(picker.getByRole('button', { name: 'No new hosts to add' })).toBeDisabled()
    await expect(picker.getByRole('button', { name: /Add all \d+ to FABRICA/ })).toHaveCount(0)
  ...
  test('P9: Settings Import re-adopts deleted config hosts', async ({ electronApp, fabricaPage }) => {
    ...
    await expect(fabricaPage.getByText(/Synced \d+ servers?/i)).toBeVisible({ timeout: 15_000 })
