    ...
    async ({ tickCount, physicalMouseWheelDelta }) => {
      ...
      try {
        ...
        for (let i = 0; i < tickCount; i += 1) {
          ...
          // flicks into multi-row reports) does not engage â€” a notched mouse
...
test.describe('terminal TUI wheel reports', () => {
  test('notched mouse wheel ticks produce immediate mouse-reporting TUI scroll reports', async ({
    ...
    const samples = await probeSmallMouseWheelReports(fabricaPage, 4)
  ...
  test('fullscreen mouse-reporting TUI scroll distance follows wheel magnitude @headful', async ({
    ...
    await expect
      ...
      .toBe(true)
    await fabricaPage.waitForTimeout(1200)
    ...
    await execInTerminal(fabricaPage, ptyId, `node ${JSON.stringify(VISIBLE_TUI_FIXTURE_PATH)}`)

    await expect
      .poll(() => readVisibleTuiOffset(fabricaPage), {
        timeout: 10_000,
        message: 'visible fullscreen TUI did not render numbered rows'
      })
      .toBe(0)

    await dispatchTuiWheel(fabricaPage, {
      deltaY: 10,
    ...
    const cellHeight = await fabricaPage.evaluate(() => {
      const state = window.__store?.getState()
      const worktreeId = state?.activeWorktreeId
      const tabId =
        state?.activeTabType === 'terminal'
  ...
  test('TUI scroll setting scales notched mouse wheel reports', async ({ fabricaPage }) => {
    await waitForSessionReady(fabricaPage)
    await waitForActiveWorktree(fabricaPage)
    await ensureTerminalVisible(fabricaPage)
    await waitForActiveTerminalManager(fabricaPage, 30_000)
    ...
    const slow = await probeTimedSmallMouseWheelReports(fabricaPage, {
      drainWaitMs: 120,
      intervalMs: 220,
      ticks: 5
    })
    await fabricaPage.waitForTimeout(220)
    const paced = await probeTimedSmallMouseWheelReports(fabricaPage, {
      drainWaitMs: 220,
      intervalMs: 80,
      ticks: 5
    })
