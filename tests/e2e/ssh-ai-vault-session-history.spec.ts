...
test.describe('SSH Agent Session History', () => {
  ...
  test('shows remote session history only for the SSH host and resumes Codex on that worktree', async ({
    ...
    try {
      ...
      const scan = await fabricaPage.evaluate(
      ...
      expect(scan.remoteCommands.join('\n')).toContain(
        "CODEX_HOME='/root/.local/share/FABRICA/codex-runtime-home/home'"
      )

      const defaultSessionTitle = fabricaPage.getByText(defaultTitle, { exact: true })
      const runtimeSessionTitle = fabricaPage.getByText(runtimeTitle, { exact: true })

      await openAiVaultSidebar(fabricaPage)
      await expect(defaultSessionTitle.first()).toBeVisible({ timeout: 30_000 })

      const hostButton = fabricaPage.getByRole('button', { name: /Session History host:/ })
      await hostButton.click()
      await fabricaPage.getByRole('menuitemradio', { name: /Local/ }).click()
      await expect(defaultSessionTitle).toHaveCount(0, { timeout: 30_000 })

      await hostButton.click()
      await fabricaPage.getByRole('menuitemradio', { name: 'All hosts' }).click()
      ...
      await installStartupQueueProbe(fabricaPage)
      await defaultSessionTitle.first().click()
      await fabricaPage.getByText('Resume in Worktree', { exact: true }).click()

      await expect
        .poll(() => readLastQueuedStartupCommand(fabricaPage), { timeout: 30_000 })
        .toContain(`CODEX_HOME='/root/.codex' codex resume '${defaultSessionId}'`)
      const queuedWorktreeId = await readLastQueuedStartupWorktreeId(fabricaPage)
...
function seedRemoteAiVaultHistory(
  ...
  dockerExec(
    ...
    [
      ...
      'mkdir -p /root/.local/share/FABRICA/codex-runtime-home/home/sessions/2026/07/04',
      'mkdir -p /root/.claude/projects/FABRICA'
    ].join(' && ')
  )
  dockerWriteFile(
    target,
  ...
  dockerWriteFile(
    target,
    `/root/.local/share/FABRICA/codex-runtime-home/home/sessions/2026/07/04/${args.runtimeSessionId}.jsonl`,
    codexTranscript({
  ...
  dockerWriteFile(
    target,
    `/root/.claude/projects/FABRICA/${args.claudeSessionId}.jsonl`,
    claudeTranscript({
