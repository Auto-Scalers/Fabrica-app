...
async function seedCreatePREligibleBranch(
  ...
  return page.evaluate(async ({ createResult }) => {
    ...
    const pr = {
      ...
      state: 'open' as const,
      url: 'https://github.com/acme/FABRICA/pull/73',
...
test.describe('Source Control create pull request', () => {
  ...
  test('creates the pull request from the Source Control primary action', async ({ fabricaPage }) => {
    const { branch, worktreeId } = await seedCreatePREligibleBranch(fabricaPage)
    await openSourceControl(fabricaPage, worktreeId)
    await forceCreatePREligibleStatus(fabricaPage, worktreeId, branch)

    const titleInput = fabricaPage.getByRole('textbox', { name: 'Pull request title' })
    const descriptionInput = fabricaPage.getByRole('textbox', {
      name: 'Pull request description'
    })
    const createButton = getCreatePRComposerSubmitButton(fabricaPage)
    await expect(createButton).toBeVisible({ timeout: 10_000 })
    await expect(createButton).toBeEnabled()
    await expect(titleInput).toHaveValue('E2e secondary')
    await expect(fabricaPage.getByRole('textbox', { name: 'Pull request base branch' })).toHaveValue(
      'main'
    )
    ...
    const payloads = await fabricaPage.evaluate(
      () => (window as unknown as { __createPRPayloads: CreatePRPayload[] }).__createPRPayloads
    )
  ...
  test('surfaces create failures without clearing the pull request composer', async ({
    ...
    await expect(fabricaPage.getByText(failureMessage)).toBeVisible()
