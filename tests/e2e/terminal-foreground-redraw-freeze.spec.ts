...
test.describe('Terminal foreground redraw freeze repro', () => {
  test('Codex-style line rewrites request a visible row refresh', async ({
    ...
    const webglAttached = await forceActivePaneWebglRenderer(fabricaPage)
    ...
    await installActivePaneRefreshProbe(fabricaPage)
    try {
      const refreshBaseline = await readRefreshProbe(fabricaPage)
      await resetSchedulerDebug(fabricaPage)
      const measurement = await measureRendererDuringRewriteBurst(fabricaPage, paneKey)
      const scheduler = await readSchedulerDebug(fabricaPage)
      ...
      await expect
        .poll(
          async () => {
            const refresh = await readRefreshProbe(fabricaPage)
      ...
      await disposeActivePaneRefreshProbe(fabricaPage)
    }
  })
  ...
  test('active OpenTUI-style redraw bursts do not monopolize the renderer', async ({
    ...
    await waitForTerminalPtyDataInjector(fabricaPage, paneKey)
    await resetSchedulerDebug(fabricaPage)
    const measurement = await measureRendererDuringBurst(fabricaPage, paneKey)
    const scheduler = await readSchedulerDebug(fabricaPage)
  ...
  test('captured OpenCode/OpenTUI redraw bytes do not monopolize foreground writes', async ({
    ...
    await waitForTerminalPtyDataInjector(fabricaPage, paneKey)
    await resetSchedulerDebug(fabricaPage)
    const measurement = await measureRendererDuringFrames(fabricaPage, paneKey, frames)
    const scheduler = await readSchedulerDebug(fabricaPage)
    annotateMeasurement(testInfo, measurement, scheduler)
