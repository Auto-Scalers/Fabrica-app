...
test.describe('Windows terminal shell paste ownership', () => {
  ...
  test('PowerShell default terminal keyboard paste preserves exact content with one PTY owner', async ({
    ...
    await createWindowsDefaultShellTerminalTab(fabricaPage, 'powershell.exe')
    await waitForActiveTerminalManager(fabricaPage, 30_000)
    await installTerminalPtyWriteSpy(electronApp)

    const ptyId = await waitForActivePanePtyId(fabricaPage)
    const runId = randomUUID()
    const sentinel = `FABRICA_E2E_POWERSHELL_DONE_${runId}`
    const powershellEscape = '`'
    const payload = [
      `FABRICA_E2E_POWERSHELL_PASTE_${runId}`,
  ...
  test('cmd.exe default terminal keyboard paste preserves exact content with one PTY owner', async ({
    ...
    await waitForActiveWorktree(fabricaPage)
    await ensureTerminalVisible(fabricaPage)
    await createWindowsDefaultShellTerminalTab(fabricaPage, 'cmd.exe')
    await waitForActiveTerminalManager(fabricaPage, 30_000)
    await installTerminalPtyWriteSpy(electronApp)

    const ptyId = await waitForActivePanePtyId(fabricaPage)
    const runId = randomUUID()
    const sentinel = `FABRICA_E2E_CMD_DONE_${runId}`
    const payload = [
      `FABRICA_E2E_CMD_PASTE_${runId}`,
      'cmd metacharacters: %PATH% !PROMPT! ^ & | < >',
      'quoted Windows path: C:\\Program Files\\FABRICA Test\\file name.txt',
      'PowerShell metacharacters: ` $ " \' ; @ { } ( )',
      `mixed-newline-before\r\nlf-line\ncrlf-line\r\n${sentinel}`
    ].join('\n')
    const scriptPath = path.join(testRepoPath, `.FABRICA-paste-cmd-shell-${runId}.mjs`)
    writeFileSync(scriptPath, pasteCollectScript(runId, sentinel, payload))
    let scriptStarted = false
  ...
  test('Git Bash default terminal keyboard paste preserves POSIX-shaped content with one PTY owner', async ({
    ...
    await skipWhenGitBashUnavailable(fabricaPage)

    await waitForSessionReady(fabricaPage)
    await waitForActiveWorktree(fabricaPage)
    await ensureTerminalVisible(fabricaPage)
    await createWindowsDefaultShellTerminalTab(fabricaPage, WINDOWS_GIT_BASH_SHELL)
    await waitForActiveTerminalManager(fabricaPage, 30_000)
    await installTerminalPtyWriteSpy(electronApp)

    const ptyId = await waitForActivePanePtyId(fabricaPage)
    const runId = randomUUID()
    const sentinel = `FABRICA_E2E_GIT_BASH_DONE_${runId}`
    const payload = [
      `FABRICA_E2E_GIT_BASH_PASTE_${runId}`,
    ...
    const scriptPath = path.join(testRepoPath, `.FABRICA-paste-git-bash-shell-${runId}.mjs`)
    writeFileSync(scriptPath, pasteCollectScript(runId, sentinel, payload))
    let scriptStarted = false
  ...
  test('WSL terminal keyboard paste preserves Linux shell content with one PTY owner', async ({
    ...
    await waitForActiveWorktree(fabricaPage)
    await ensureTerminalVisible(fabricaPage)
    const wslDistro = await configureActiveProjectWslRuntime(fabricaPage)
    test.skip(!wslDistro, 'No WSL distro is available on this Windows host')
    await createWindowsProjectRuntimeTerminalTab(fabricaPage, 'wsl.exe')
    await waitForActiveTerminalManager(fabricaPage, 30_000)
    await installTerminalPtyWriteSpy(electronApp)

    const ptyId = await waitForActivePanePtyId(fabricaPage)
    const runId = randomUUID()
    const sentinel = `FABRICA_E2E_WSL_DONE_${runId}`
    const payload = [
      `FABRICA_E2E_WSL_PASTE_${runId}`,
      'POSIX shell metacharacters: $ ` " \' ; | & < > * ? [ ] ( )',
  ...
  test('existing WSL terminal keeps paste runtime after default shell changes', async ({
    ...
    const wslDistro = await configureActiveProjectWslRuntime(fabricaPage)
    test.skip(!wslDistro, 'No WSL distro is available on this Windows host')
    const tabId = await createWindowsProjectRuntimeTerminalTab(fabricaPage, 'wsl.exe')
    await updateWindowsDefaultShellSetting(fabricaPage, 'cmd.exe')
    await expect(
      fabricaPage.locator(`[data-testid="sortable-tab"][data-tab-id="${tabId}"] [data-shell-icon]`)
    ...
    const ptyId = await waitForActivePanePtyId(fabricaPage)
    const runId = randomUUID()
    const sentinel = `FABRICA_E2E_WSL_RETENTION_DONE_${runId}`
    const payload = [
      `FABRICA_E2E_WSL_RETENTION_PASTE_${runId}`,
    ...
    const scriptPath = path.join(testRepoPath, `.FABRICA-paste-wsl-retention-${runId}.mjs`)
    writeFileSync(scriptPath, pasteCollectScript(runId, sentinel, payload))
    let scriptStarted = false
