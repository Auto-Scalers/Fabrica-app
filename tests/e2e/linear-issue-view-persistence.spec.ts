...
const LINEAR_ISSUE_VIEW_STORAGE_KEY = 'FABRICA.linear.issue-view.v1'
...
test.describe('Linear issue view persistence', () => {
  test('preserves view mode, grouping, ordering, and filters across a tasks remount', async ({
    ...
    await openLinearTasks(fabricaPage)
    await waitForLinearIssuesChrome(fabricaPage, ISSUE_A.title)

    await setLinearViewPreferences(fabricaPage, {
      viewMode: 'Board',
      groupBy: 'Status',
      orderBy: 'Updated'
    })
    await applyStatusFilter(fabricaPage, STATE_A.name)

    await waitForLinearIssueViewPersisted(fabricaPage, (view) => {
      if (
        !view ||
        view.viewMode !== 'board' ||
        view.groupBy !== 'status' ||
    ...
    await expectRestoredLinearView(fabricaPage)
    const statusChip = fabricaPage.getByRole('button', { name: 'Remove Status filter' }).locator('..')
    await expect(statusChip).toContainText(STATE_A.name)

    await closeTasksPage(fabricaPage)
    await openLinearTasks(fabricaPage)
    await waitForLinearIssuesChrome(fabricaPage, ISSUE_A.title)

    await expectRestoredLinearView(fabricaPage)
    await expect(
      fabricaPage.getByRole('button', { name: 'Remove Status filter' }).locator('..')
    ).toContainText(STATE_A.name)
    // Board surface, not the flat list column header.
    await expect(fabricaPage.getByText(STATE_A.name, { exact: true }).first()).toBeVisible()
  })

  test('keeps attribute filters scoped per Linear workspace', async ({ electronApp, fabricaPage }) => {
    await waitForSessionReady(fabricaPage)
    await waitForActiveWorktree(fabricaPage)
    ...
    await applyPriorityFilter(fabricaPage, 'High')
    await expect(
      fabricaPage.getByRole('button', { name: 'Remove Priority filter' }).locator('..')
    ).toContainText('High')
    ...
    await switchLinearWorkspace(fabricaPage, WORKSPACE_A.organizationName)
    await waitForLinearIssuesChrome(fabricaPage, ISSUE_A.title)
    await expect(
      fabricaPage.getByRole('button', { name: 'Remove Priority filter' }).locator('..')
    ).toContainText('High')
