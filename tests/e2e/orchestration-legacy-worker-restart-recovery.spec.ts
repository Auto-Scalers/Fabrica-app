...
import { DEFAULT_LOCAL_FABRICA_PROFILE_ID } from '../../src/shared/FABRICA-profiles'
...
const fakeCliDir = mkdtempSync(path.join(os.tmpdir(), 'FABRICA-e2e-legacy-worker-'))
...
async function emitAuthorityHook() {
  const port = process.env.FABRICA_AGENT_HOOK_PORT
  const token = process.env.FABRICA_AGENT_HOOK_TOKEN
  const launchToken = process.env.FABRICA_AGENT_LAUNCH_TOKEN
  if (!port || !token || !launchToken || !process.env.FABRICA_PANE_KEY) return
  try {
    const response = await fetch('http://127.0.0.1:' + port + '/hook/codex', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'X-FABRICA-Agent-Hook-Token': token
      },
      body: JSON.stringify({
        paneKey: process.env.FABRICA_PANE_KEY,
        tabId: process.env.FABRICA_TAB_ID,
        worktreeId: process.env.FABRICA_WORKTREE_ID,
        env: process.env.FABRICA_AGENT_HOOK_ENV,
        version: process.env.FABRICA_AGENT_HOOK_VERSION,
        launchToken,
        payload: {
    ...
    appendLedger('FABRICA_E2E_AUTHORITY_LEDGER', { event: 'authority-hook', status: response.status })
  } catch (error) {
    appendLedger('FABRICA_E2E_AUTHORITY_LEDGER', {
...
appendLedger('FABRICA_E2E_SPAWN_LEDGER', { event: 'spawn', argv: process.argv.slice(2) })
...
process.stdin.on('data', (chunk) => {
  ...
  if (input.includes('\\x03')) {
    appendLedger('FABRICA_E2E_INTERRUPTION_LEDGER', { event: 'stdin-ctrl-c' })
  }
  if (!acknowledged && input.includes('\\r')) {
    acknowledged = true
    process.stdout.write('ACK\\n')
  }
  const legacyCompletion = input.match(/FABRICA_E2E_RUN_LEGACY_DONE:([A-Za-z0-9+/=]+)/)
  if (!lifecycleSent && legacyCompletion) {
    lifecycleSent = true
    const identity = JSON.parse(Buffer.from(legacyCompletion[1], 'base64').toString('utf8'))
    const cliEntry = process.env.FABRICA_E2E_CLI_ENTRY
    const args = [
      'orchestration',
      'send',
      '--to',
    ...
    const result = cliEntry
      ...
      : { status: 127, stdout: '', stderr: 'FABRICA_E2E_CLI_ENTRY missing' }
    appendLedger('FABRICA_E2E_LIFECYCLE_LEDGER', {
...
for (const signal of ['SIGINT', 'SIGHUP', 'SIGTERM']) {
  process.on(signal, () => {
    appendLedger('FABRICA_E2E_INTERRUPTION_LEDGER', { event: 'signal', signal })
    process.exit(0)
  })
}
...
function persistedDataPath(userDataDir: string): string {
  return path.join(userDataDir, 'profiles', DEFAULT_LOCAL_FABRICA_PROFILE_ID, 'FABRICA-data.json')
}
...
for (const contractVersion of [LEGACY_CONTRACT_VERSION, CURRENT_CONTRACT_VERSION]) {
  ...
  {}, testInfo) => {
    ...
    const session = createRestartSession(testInfo, {
      ...
      FABRICA_E2E_SPAWN_LEDGER: spawnLedgerPath,
      FABRICA_E2E_INTERRUPTION_LEDGER: interruptionLedgerPath,
      FABRICA_E2E_AUTHORITY_LEDGER: authorityLedgerPath,
      FABRICA_E2E_LIFECYCLE_LEDGER: lifecycleLedgerPath,
      FABRICA_E2E_CLI_ENTRY: path.join(process.cwd(), 'out', 'cli', 'index.js')
    })
    ...
    try {
      ...
      if (contractVersion === LEGACY_CONTRACT_VERSION) {
        ...
        await secondClient.call('terminal.send', {
          terminal: recovered!.handle,
          text: `FABRICA_E2E_RUN_LEGACY_DONE:${legacyCompletion}`,
          enter: true
        })
        await expect
