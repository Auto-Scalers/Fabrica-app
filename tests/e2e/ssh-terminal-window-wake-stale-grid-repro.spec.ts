...
async function startRemoteMonitor(page: Page, ptyId: string): Promise<void> {
  const marker = `FABRICA_SSH_WAKE_READY_${Date.now()}`
...
test.describe('SSH terminal window-wake stale PTY grid repro', () => {
  ...
  test('window focus heals a remote PTY whose applied grid drifted from xterm', async ({
    ...
    try {
      ...
      fabricaPage.on('pageerror', (error) => pageErrors.push(error.message))
      installIdleGridMonitor(target)
      await fabricaPage.setViewportSize(BASE_VIEWPORT)
      await waitForSessionReady(fabricaPage)
      await waitForActiveWorktree(fabricaPage)
      const identity = await fabricaPage.evaluate(() => window.api.app.getIdentity())
      expect(identity.isDev).toBe(true)
      expect(identity.devWorktreeName).toBe(path.basename(process.cwd()))

      await connectDockerSshRelayTarget(fabricaPage, target, { relayGracePeriodSeconds: 300 })
      await ensureTerminalVisible(fabricaPage, 60_000)
      await waitForActiveTerminalManager(fabricaPage, 60_000)
      const ptyId = await waitForActivePanePtyId(fabricaPage, 60_000)
      await startRemoteMonitor(fabricaPage, ptyId)
      await expect
        .poll(
          () => {
            try {
        ...
        .poll(
          async () =>
            actualGridMatchesXterm(
              ...
              await readRendererGrid(fabricaPage, ptyId)
            ),
          { timeout: 15_000, message: 'Remote PTY and xterm did not establish a matching baseline' }
        )
        .toBe(true)

      const baseline = await readRendererGrid(fabricaPage, ptyId)
      if (!baseline.xterm) {
      ...
      const drifted = await readRendererGrid(fabricaPage, ptyId)
      ...
      if (!actualGridMatchesXterm(wakeResult.last.remote, wakeResult.last.renderer)) {
        ...
        await fabricaPage.setViewportSize({
      ...
      await fabricaPage.screenshot({ path: healedScreenshot, fullPage: true })
