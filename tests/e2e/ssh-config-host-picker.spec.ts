...
test.describe('SSH config host picker', () => {
  ...
  test.afterEach(async ({ fabricaPage }) => {
    await closeOpenDialogs(fabricaPage).catch(() => undefined)
  ...
  test('P1: empty config shows empty state; Back returns to blank form', async ({ fabricaPage }) => {
    ...
    await expect(picker.getByRole('button', { name: 'Add all to FABRICA' })).toBeDisabled()

    await picker.getByRole('button', { name: 'Back' }).click()
    const form = fabricaPage.getByRole('dialog', { name: 'Add SSH host' })
  ...
  test('P3: select host prefills form; Save persists; N3 identity hint', async ({
    ...
    await expect(
      fabricaPage.getByText(new RegExp(`Filled from ${escapeRegExp(prod.alias)}`, 'i'))
    ).toBeVisible({ timeout: 5_000 })

    await form.getByRole('button', { name: 'Save' }).click()
    await expect(form).toBeHidden({ timeout: 10_000 })
    await expect(fabricaPage.getByText('SSH host added.')).toBeVisible({ timeout: 5_000 })
  ...
  test('P8: Back without select leaves form empty', async ({ electronApp, fabricaPage }) => {
    ...
    await expect(fields.username).toHaveValue('')
    await expect(fields.label).toHaveValue('')
    await expect(fabricaPage.getByText(/Filled from /i)).toHaveCount(0)
  })
})
