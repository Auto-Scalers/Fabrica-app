...
test.describe('Setup script import prompt', () => {
  ...
  test('imports Superset local overlays through the prompt UI', async ({ fabricaPage }, testInfo) => {
    const repoPath = createSupersetSetupRepo(testInfo.outputPath('superset-setup-repo'))
    const repoId = await addAndActivateRepo(fabricaPage, repoPath)
    ...
    await fabricaPage.getByRole('button', { name: 'Save local setup' }).click()

    await expect(
      fabricaPage.getByText('2 unsupported fields skipped. Saved the setup command.')
    ).toBeVisible()

    const localCommands = await openImportedSetupSettingsFromToast(fabricaPage, repoId)
  ...
  test('imports cmux setup commands through the prompt UI', async ({ fabricaPage }, testInfo) => {
    ...
    await expect(
      fabricaPage.getByRole('button', { name: "project's settings", exact: true })
    ).toBeVisible()

    const repoSettings = await openRepoSettings(fabricaPage, repoId)
