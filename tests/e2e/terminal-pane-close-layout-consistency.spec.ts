...
test.describe('terminal pane close vs hidden/park lifecycle keeps layout consistent', () => {
  test('control: close while visible', async ({ fabricaPage }) => {
    const { tabId } = await setUpSplitTab(fabricaPage)
    await closeLastPaneOnTab(fabricaPage, tabId)
    await expectLayoutConsistent(fabricaPage, tabId, 1, 'close-visible')
  })

  test('close and hide the tab in the same tick', async ({ fabricaPage }) => {
    const { worktreeId, tabId } = await setUpSplitTab(fabricaPage)
    ...
    await expectLayoutConsistent(fabricaPage, tabId, 1, 'close-then-hide-same-tick')
  })

  test('close while hidden but still mounted (hot-retain window)', async ({ fabricaPage }) => {
    const { worktreeId, tabId } = await setUpSplitTab(fabricaPage)
    await createActiveTerminalTab(fabricaPage, worktreeId)
    await closeLastPaneOnTab(fabricaPage, tabId)
    await parkHiddenTabBehindDecoy(fabricaPage, worktreeId, tabId, {
      parkDelayMs: PARKING_DELAY_MS
    })
    await activateTerminalTab(fabricaPage, tabId)
    await waitForTabRemounted(fabricaPage, tabId)
    await expectLayoutConsistent(fabricaPage, tabId, 1, 'close-while-hidden-mounted')
  })

  test('close immediately after reveal remount, before panes settle', async ({ fabricaPage }) => {
    const { worktreeId, tabId } = await setUpSplitTab(fabricaPage)
    await createActiveTerminalTab(fabricaPage, worktreeId)
    await parkHiddenTabBehindDecoy(fabricaPage, worktreeId, tabId, {
      parkDelayMs: PARKING_DELAY_MS
    ...
    await expectLayoutConsistent(fabricaPage, tabId, 1, 'close-mid-reveal')
  })

  test('clean visible close survives a later park/reveal cycle', async ({ fabricaPage }) => {
    const { worktreeId, tabId } = await setUpSplitTab(fabricaPage)
    await closeLastPaneOnTab(fabricaPage, tabId)
    await expectLayoutConsistent(fabricaPage, tabId, 1, 'pre-park close')
    await createActiveTerminalTab(fabricaPage, worktreeId)
    await parkHiddenTabBehindDecoy(fabricaPage, worktreeId, tabId, {
      parkDelayMs: PARKING_DELAY_MS
    })
    await activateTerminalTab(fabricaPage, tabId)
    await waitForTabRemounted(fabricaPage, tabId)
    await expectLayoutConsistent(fabricaPage, tabId, 1, 'post-park-reveal')
  })

  test('split pane shell exits while hidden but still mounted', async ({ fabricaPage }) => {
    const { worktreeId, tabId, splitPtyId } = await setUpSplitTab(fabricaPage)
    await createActiveTerminalTab(fabricaPage, worktreeId)
    // The setup-script analog: the split's shell ends on its own while the
    // tab is hidden-but-mounted.
    await sendToTerminal(fabricaPage, splitPtyId, 'exit\r')
    await fabricaPage.waitForTimeout(PARKING_DELAY_MS / 2)
    await activateTerminalTab(fabricaPage, tabId)
    await waitForTabRemounted(fabricaPage, tabId)
    await expectLayoutConsistent(fabricaPage, tabId, 1, 'shell-exit-while-hidden-mounted', splitPtyId)
  })

  test('split pane shell exits while the tab is parked', async ({ fabricaPage }) => {
    const { worktreeId, tabId, splitPtyId } = await setUpSplitTab(fabricaPage)
    await createActiveTerminalTab(fabricaPage, worktreeId)
    await parkHiddenTabBehindDecoy(fabricaPage, worktreeId, tabId, {
      parkDelayMs: PARKING_DELAY_MS
    ...
    await sendToTerminal(fabricaPage, splitPtyId, 'exit\r')
    await fabricaPage.waitForTimeout(PARKING_DELAY_MS)
    await activateTerminalTab(fabricaPage, tabId)
    await waitForTabRemounted(fabricaPage, tabId)
