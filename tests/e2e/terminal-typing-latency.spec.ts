...
test.describe('Terminal typing latency', () => {
  test('interactive prompt echoes typed keys without visible lag', async ({
    ...
    const scriptPath = path.join(testRepoPath, `.FABRICA-typing-benchmark-${runId}.mjs`)
    ...
    try {
      ...
      commandSent = true
      await waitForTerminalOutput(fabricaPage, `TYPING_READY_${runId}`, 10_000)
      ...
      for (const [index, char] of [...KEY_LATENCY_SAMPLES].entries()) {
        ...
        await waitForMarkerLatency(fabricaPage, marker, MAX_WORST_KEY_LATENCY_MS)
