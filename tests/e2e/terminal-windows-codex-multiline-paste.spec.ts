...
async function activateTestRepository(
  ...
  await page.evaluate(async (targetRepoPath) => {
    ...
    if (!store) {
      throw new Error('FABRICA store unavailable')
    }
...
test.describe('Windows Codex multiline paste', () => {
  ...
  test('multiline Ctrl+V keeps the existing Codex draft unsent @local-real-codex', async ({
    ...
    test.skip(
      process.env.FABRICA_E2E_REAL_CODEX !== '1',
      'Set FABRICA_E2E_REAL_CODEX=1 to exercise the locally installed Codex TUI'
    )
    test.slow()

    await waitForSessionReady(fabricaPage)
    await activateTestRepository(fabricaPage, testRepoPath)
    ...
    await sendToTerminal(fabricaPage, ptyId, 'codex -m FABRICA-e2e-invalid-model\r')
    await expect
      .poll(() => getTerminalContent(fabricaPage, 12_000), { timeout: 20_000 })
      .toMatch(/Do[\s\S]*you[\s\S]*trust[\s\S]*contents|OpenAI Codex/i)
    if (CODEX_TRUST_PROMPT_RE.test(await getTerminalContent(fabricaPage, 12_000))) {
      await sendToTerminal(fabricaPage, ptyId, '\r')
    }
    await waitForTerminalOutput(fabricaPage, 'OpenAI Codex', 20_000, 30_000)
    await waitForCodexComposerReady(fabricaPage)
    await enableTerminalAccessibilityDom(fabricaPage, ptyId)
    await focusActiveTerminalInput(fabricaPage)
    await fabricaPage.keyboard.type(DRAFT)
    const terminalDom = fabricaPage.locator(
      `[data-pty-id=${JSON.stringify(ptyId)}] .xterm-accessibility-tree`
    )
    await expect(terminalDom).toContainText(DRAFT, { timeout: 10_000 })
    await fabricaPage.evaluate((text) => window.api.ui.writeClipboardText(text), pastePayload())
  ...
  test('delivers a normalized large paste through native ConPTY', async ({
    ...
    const marker = `FABRICA_LARGE_PASTE_${randomUUID().replaceAll('-', '')}`
    ...
    try {
      ...
      await waitForTerminalOutput(fabricaPage, `${marker}_READY`, 10_000, 12_000)
      await enableTerminalAccessibilityDom(fabricaPage, ptyId)
      await focusActiveTerminalInput(fabricaPage)
