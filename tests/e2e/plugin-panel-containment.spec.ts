...
import type { ElectronApplication, FrameLocator, Page, TestInfo } from '@playwright/test'
...
async function materializeHostilePlugin(origin: string): Promise<string> {
  const tempRoot = await mkdtemp(join(tmpdir(), 'FABRICA-hostile-panel-e2e-'))
...
test('contains hostile panel network and navigation probes', async ({
  ...
  fabricaPage.on('console', (message) => {
    browserEvents.push(`console:${message.type()}:${message.text()}`)
  })
  fabricaPage.on('pageerror', (error) => {
    browserEvents.push(`pageerror:${error.message}`)
  })
  fabricaPage.on('framenavigated', (frame) => {
    browserEvents.push(`framenavigated:${frame.url()}`)
  })
  try {
    const panel = await installApprovedPanel(fabricaPage, pluginRoot)
    await openPanel(fabricaPage, panel)

    const iframe = fabricaPage.locator(`iframe[title="${panel.title}"]`)
    await expect(iframe).toHaveAttribute('sandbox', 'allow-scripts')
    const frame = fabricaPage.frameLocator(`iframe[title="${panel.title}"]`)
        ...
        new Promise<string>((resolve, reject) => {
          ...
          const onMessage = (event: MessageEvent): void => {
            ...
            if (
              ...
              !data ||
              data.type !== 'FABRICA-panel-action-result' ||
    ...
    expect(fabricaPage.url()).toBe(appUrl)
...
test('detects and suspends a busy-looping panel in an isolated renderer', async ({
  ...
  try {
    ...
    await iframe.evaluate((element) => {
      const panelWindow = (element as HTMLIFrameElement).contentWindow
      panelWindow?.postMessage({ type: 'FABRICA-hostile-busy-probe' }, '*')
    })

    await expect(
      fabricaPage.getByText('This plugin panel stopped responding and was suspended.')
    ).toBeVisible({ timeout: 20_000 })
    await expect(
      fabricaPage.getByRole('button', { name: new RegExp(`${panel.title}.*Error`) })
    ).toBeVisible()
    expect(fabricaPage.url()).toBe(appUrl)
    expect(server.requests).toEqual([])
  } finally {
