...
export function createTerminalImeByteReader(
  ...
  const readyMarker = `FABRICA_IME_READER_READY_${runId}`
  const resultPrefix = `FABRICA_IME_BYTES_${runId}`
  const scriptPath = path.join(testRepoPath, `.FABRICA-ime-byte-reader-${runId}.cjs`)
  const source = `
