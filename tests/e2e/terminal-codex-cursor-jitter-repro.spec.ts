    ...
    ({ tabId, ptyId }) => {
      ...
      const marker =
        cursorVisible && cursorRect
          ...
          : null
      const cursFABRICAnvas = pane.container.querySelector<HTMLCanvasElement>(
        '.xterm-cursor-layer canvas'
      )
      const canvasLayer = cursFABRICAnvas?.closest<HTMLElement>('.xterm-cursor-layer')
      ...
      const canvasMarker = (() => {
        ...
        const context = cursFABRICAnvas.getContext('2d', { willReadFrequently: true })
        if (!context) {
          return null
        }
        const image = context.getImageData(0, 0, cursFABRICAnvas.width, cursFABRICAnvas.height)
        let pixelCount = 0
        ...
        let sumX = 0
        let sumY = 0
        for (let y = 0; y < cursFABRICAnvas.height; y += 1) {
          for (let x = 0; x < cursFABRICAnvas.width; x += 1) {
            const offset = (y * cursFABRICAnvas.width + x) * 4
        ...
        const canvasCellWidth = cursFABRICAnvas.width / terminal.cols
        const canvasCellHeight = cursFABRICAnvas.height / terminal.rows
      ...
      return {
        ...
        cursorDebug: {
          ...
          cursFABRICAnvasCount: pane.container.querySelectorAll('.xterm-cursor-layer canvas').length,
...
test.describe('Codex terminal cursor jitter repro', () => {
  test('keeps queued-message cursor out of the Working status row in native Windows Codex @headful', async ({
    ...
    await installPtyWriteDiagnostics(fabricaPage)
    rmSync(ARTIFACT_DIR, { recursive: true, force: true })
    mkdirSync(ARTIFACT_DIR, { recursive: true })

    const shellCase = SHELL_CASES[0]!
    const { tabId, ptyId } = await prepareCodexTerminal(fabricaPage, shellCase)
    await installPtyOutputDiagnostics(fabricaPage)
    await fabricaPage.keyboard.type(CODEX_REPO_PROMPT)
    await fabricaPage.waitForTimeout(250)
    await fabricaPage.keyboard.press('Enter')
    await fabricaPage.waitForTimeout(1_000)
    if (!CODEX_WORKING_STATUS_RE.test(await getTerminalContentForTab(fabricaPage, tabId, 8_000))) {
      await fabricaPage.keyboard.press('Enter')
    }
    await fabricaPage.waitForTimeout(3_000)
    const submittedContent = await getTerminalContentForTab(fabricaPage, tabId, 8_000)
    writeFileSync(path.join(ARTIFACT_DIR, 'queued-message-after-submit.txt'), submittedContent)
    await expect
      .poll(
        async () =>
          CODEX_WORKING_STATUS_RE.test(await getTerminalContentForTab(fabricaPage, tabId, 8_000)),
        {
          timeout: 30_000,
      ...
      .toBe(true)
    await applyCursorProbeTheme(fabricaPage, tabId)
    ...
    await fabricaPage.keyboard.insertText('s')
    await expect
      .poll(
        async () =>
          (await readScreenLines(fabricaPage, tabId)).some((line) => isQueuedInputLine(line.text)),
        {
          timeout: 5_000,
    ...
    const snapshot = await readScreenSnapshot(fabricaPage, shellCase.label, tabId, ptyId)
    const rawChunks = await readPtyOutputDiagnostics(fabricaPage)
