...
async function closeRightSidebarAndFeatureTips(page: Page): Promise<void> {
  await page.evaluate(() => {
    ...
    store.getState().markFeatureTipsSeen(['FABRICA-cli', 'cmd-j-palette', 'voice-dictation'])
...
test.describe('Terminal column desync repro', () => {
  test('PTY columns stay in sync with xterm across a visible resize', async ({ fabricaPage }) => {
    test.setTimeout(120_000)
    await waitForSessionReady(fabricaPage)
    await waitForActiveWorktree(fabricaPage)
    await closeRightSidebarAndFeatureTips(fabricaPage)
    ...
    const expectColumnsInSync = async (label: string): Promise<void> => {
      await expect
        .poll(
          async () => {
            const snap = await readColumnSnapshot(fabricaPage, ptyId)
    ...
    await fabricaPage.setViewportSize({ width: 760, height: 800 })
    await expectColumnsInSync('after shrink')

    await fabricaPage.setViewportSize({ width: 1280, height: 800 })
  ...
  test('pty:getSize reports the size the PTY actually applied', async ({ fabricaPage }) => {
    test.setTimeout(120_000)
    await waitForSessionReady(fabricaPage)
    await waitForActiveWorktree(fabricaPage)
    await closeRightSidebarAndFeatureTips(fabricaPage)
    ...
    await expect
      .poll(
        async () => {
          const ptyCols = await readPtyCols(fabricaPage, ptyId)
          const reportedCols = await readReportedPtyCols(fabricaPage, ptyId)
  ...
  test('PTY columns re-sync after the terminal is resized while hidden', async ({ fabricaPage }) => {
    test.setTimeout(120_000)
    await waitForSessionReady(fabricaPage)
    const homeWorktreeId = await waitForActiveWorktree(fabricaPage)
    const otherWorktreeId = (await getAllWorktreeIds(fabricaPage)).find((id) => id !== homeWorktreeId)
    ...
    const ptyId = await settleTerminal(fabricaPage)
    await fabricaPage.setViewportSize({ width: 1280, height: 800 })
    await fabricaPage.waitForTimeout(400)

    const baseline = await readColumnSnapshot(fabricaPage, ptyId)
    ...
    await switchToWorktree(fabricaPage, otherWorktreeId)
    await waitForActiveTerminalManager(fabricaPage, 30_000)
    await fabricaPage.setViewportSize({ width: 720, height: 800 })
    ...
    const afterReturn = await readColumnSnapshot(fabricaPage, ptyId)
    expect(
      afterReturn.ptyCols,
  ...
  test('PTY columns re-sync after repeated background resizes', async ({ fabricaPage }) => {
    ...
    for (const [index, width] of widths.entries()) {
      await switchToWorktree(fabricaPage, otherWorktreeId)
      await waitForActiveTerminalManager(fabricaPage, 30_000)
      await fabricaPage.setViewportSize({ width, height: 800 })
      await fabricaPage.waitForTimeout(350)
      await switchToWorktree(fabricaPage, homeWorktreeId)
      await ensureTerminalVisible(fabricaPage)
      await waitForActiveTerminalManager(fabricaPage, 30_000)
      await fabricaPage.waitForTimeout(500)

      const snapshot = await readColumnSnapshot(fabricaPage, ptyId)
      expect(
        snapshot.ptyCols,
  ...
  test('both panes keep PTY columns synced after a vertical split reparent', async ({
    ...
    const firstPtyId = await settleTerminal(fabricaPage)

    const baseline = await readColumnSnapshot(fabricaPage, firstPtyId)
    expect(baseline.ptyCols).toBe(baseline.xtermCols)
    ...
    for (const pane of snapshot.panes) {
      ...
      const ptyCols = await readPtyCols(fabricaPage, ptyId)
      const xtermCols = await readRenderedColsForPty(fabricaPage, ptyId)
      expect(
        ptyCols,
  ...
  test('both panes stay PTY-synced when a tab MOUNTS with a split layout present', async ({
    ...
    await closeRightSidebarAndFeatureTips(fabricaPage)
    await ensureTerminalVisible(fabricaPage)
    await fabricaPage.setViewportSize({ width: 1440, height: 900 })
    await fabricaPage.waitForTimeout(300)
    await settleTerminal(fabricaPage)
    ...
    for (let attempt = 0; attempt < MOUNT_ATTEMPTS; attempt += 1) {
      ...
      await fabricaPage.waitForFunction(() => Boolean(window.__store), null, { timeout: 30_000 })
      await waitForSessionReady(fabricaPage)
      await waitForActiveWorktree(fabricaPage)
      await closeRightSidebarAndFeatureTips(fabricaPage)
      await ensureTerminalVisible(fabricaPage)
      ...
      await fabricaPage.setViewportSize({ width: 1180, height: 800 })
      await fabricaPage.waitForTimeout(300)
  ...
  test('PTY columns stay synced when the window is resized during initial mount', async ({
    ...
    for (let attempt = 0; attempt < MOUNT_ATTEMPTS; attempt += 1) {
      ...
      const ptyId = await settleTerminal(fabricaPage)
      await fabricaPage.waitForTimeout(700)
