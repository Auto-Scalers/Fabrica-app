...
test.describe('Worktree switch responsiveness', () => {
  ...
  test('updates the selected workspace in the same click task when changing back', async ({
    ...
    const [firstWorktreeId, secondWorktreeId] = await prepareSidebarForSwitchTest(fabricaPage)
    const firstRow = worktreeRow(fabricaPage, firstWorktreeId)
    const secondRow = worktreeRow(fabricaPage, secondWorktreeId)

    await expect(firstRow).toBeVisible()
    await expect(secondRow).toBeVisible()
    await expect(firstRow).toHaveAttribute('aria-current', 'page')
    await expect(fabricaPage.locator('[data-rendered-active-worktree-id]')).toHaveAttribute(
