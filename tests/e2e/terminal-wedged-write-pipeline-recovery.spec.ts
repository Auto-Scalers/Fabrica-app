...
test.describe('Wedged terminal write pipeline recovery', () => {
  test('pane recovers rendering and input after its write pipeline wedges', async ({
    ...
    await fabricaPage.keyboard.type(`echo ${beforeMarker}`, { delay: 20 })
    ...
    await focusActiveTerminalInput(fabricaPage)
    await fabricaPage.keyboard.type(`echo ${afterMarker}`, { delay: 20 })
    await fabricaPage.keyboard.press('Enter')

    await expect
      .poll(async () => (await getTerminalContent(fabricaPage)).includes(afterMarker), {
  ...
  test('pane recovers after its xterm is disposed under live bindings (zombie pane)', async ({
    ...
    await fabricaPage.keyboard.type(`echo ${typedMarker}`, { delay: 20 })
    await fabricaPage.keyboard.press('Enter')
    await expect
      .poll(async () => (await getTerminalContent(fabricaPage)).includes(typedMarker), {
        timeout: 15_000,
