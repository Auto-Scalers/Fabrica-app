...
const scratch = mkdtempSync(path.join(os.tmpdir(), 'FABRICA-paired-stalled-stream-'))
...
test('restarts one ACK-starved paired terminal stream without replacing its PTY @headful', async ({
  ...
  const worktree = await fabricaPage.evaluate(() => {
  ...
  const client = await launchPairedWebClient(electronApp, offer, {
    disableRemoteTerminalStallRecovery:
      process.env.FABRICA_E2E_DISABLE_REMOTE_TERMINAL_STALL_RECOVERY === '1'
  })
  ...
  try {
    await expect
      ...
      .toBe(true)
    const observerOffer = await createRuntimeDesktopPairingOffer(fabricaPage)
    ...
    await minimizeHeadedHost(electronApp, fabricaPage)
    ...
    await restoreHeadedHost(electronApp, fabricaPage)
    await fabricaPage.evaluate(
      (worktreeId) => window.__store?.getState().setActiveWorktree(worktreeId),
      worktree.id
    )
    const hostTab = fabricaPage.locator(
      `[data-testid="sortable-tab"][data-tab-id="${created.tab.parentTabId}"]`
    )
    await expect(hostTab).toBeVisible({ timeout: 30_000 })
    await hostTab.click()
    const hostPane = await findHostPaneWithMarker(fabricaPage, `LIVE:${liveMarker}`)
    expect(hostPane.tabId).toBe(created.tab.parentTabId)
    await fabricaPage.evaluate(({ paneId, tabId }) => {
      const manager = window.__paneManagers?.get(tabId)
      manager?.setActivePane?.(paneId, { focus: true })
    }, hostPane)
    await expect
      .poll(() => getTerminalContent(fabricaPage), { timeout: 30_000 })
      .toContain(`LIVE:${liveMarker}`)
    const restoredTerminalScreenshot = await fabricaPage
      .locator(
    ...
    await restoreHeadedHost(electronApp, fabricaPage)
    const restoredHostScreenshot = await fabricaPage.screenshot({ fullPage: true })
    expect(
