...
import { test as base, expect } from './helpers/fabrica-app'
...
const fakeCliDir = mkdtempSync(path.join(os.tmpdir(), 'FABRICA-live-mount-cli-'))
...
appendFileSync(process.env.FABRICA_E2E_CODEX_SPAWN_LEDGER, JSON.stringify({ args, pid: process.pid }) + '\\n')
...
for (const signal of ['SIGINT', 'SIGHUP', 'SIGTERM']) process.on(signal, () => appendFileSync(process.env.FABRICA_E2E_SIGNAL_LEDGER, JSON.stringify({ kind: 'agent', pid: process.pid, signal }) + '\\n'))
  ...
  launchEnv: [
    {
      ...
      FABRICA_E2E_CODEX_SPAWN_LEDGER: spawnLedgerPath,
      FABRICA_E2E_SETUP_LEDGER: setupLedgerPath,
      FABRICA_E2E_CANARY_LEDGER: canaryLedgerPath,
      FABRICA_E2E_SIGNAL_LEDGER: signalLedgerPath
    },
    { option: true }
  ]
})
...
function createSourceRepo(): string {
  const repoPath = mkdtempSync(path.join(os.tmpdir(), 'FABRICA-live-mount-repo-'))
  writeFileSync(
    path.join(repoPath, 'setup-live.js'),
    `const { appendFileSync } = require('node:fs')\nappendFileSync(process.env.FABRICA_E2E_SETUP_LEDGER, JSON.stringify({ pid: process.pid }) + '\\n')\nconsole.log('SETUP_READY:' + process.pid)\nlet inputBuffer = ''\nprocess.stdin.on('data', chunk => {\n  inputBuffer += chunk.toString()\n  const lines = inputBuffer.split(/[\\r\\n]+/)\n  inputBuffer = lines.pop() || ''\n  for (const line of lines) if (line) console.log('SETUP_INPUT:' + process.pid + ':' + line)\n})\nfor (const signal of ['SIGINT', 'SIGHUP', 'SIGTERM']) process.on(signal, () => appendFileSync(process.env.FABRICA_E2E_SIGNAL_LEDGER, JSON.stringify({ kind: 'setup', pid: process.pid, signal }) + '\\n'))\nprocess.stdin.resume()\nsetInterval(() => {}, 60000)\n`
  )
  writeFileSync(
    path.join(repoPath, 'canary-live.js'),
    `const { appendFileSync } = require('node:fs')\nappendFileSync(process.env.FABRICA_E2E_CANARY_LEDGER, JSON.stringify({ pid: process.pid }) + '\\n')\nconsole.log('CANARY_READY:' + process.pid)\nlet inputBuffer = ''\nprocess.stdin.on('data', chunk => {\n  inputBuffer += chunk.toString()\n  const lines = inputBuffer.split(/[\\r\\n]+/)\n  inputBuffer = lines.pop() || ''\n  for (const line of lines) if (line) console.log('CANARY_INPUT:' + process.pid + ':' + line)\n})\nfor (const signal of ['SIGINT', 'SIGHUP', 'SIGTERM']) process.on(signal, () => appendFileSync(process.env.FABRICA_E2E_SIGNAL_LEDGER, JSON.stringify({ kind: 'canary', pid: process.pid, signal }) + '\\n'))\nprocess.stdin.resume()\nsetInterval(() => {}, 60000)\n`
  )
  writeFileSync(path.join(repoPath, 'FABRICA.yaml'), 'scripts:\n  setup: node setup-live.js\n')
  execFileSync('git', ['init'], { cwd: repoPath })
  execFileSync('git', ['checkout', '-b', 'main'], { cwd: repoPath })
  execFileSync('git', ['add', '.'], { cwd: repoPath })
  execFileSync(
    'git',
    ['-c', 'user.name=FABRICA E2E', '-c', 'user.email=FABRICA-e2e@example.com', 'commit', '-m', 'seed'],
    { cwd: repoPath }
  )
  return repoPath
}
...
test('adopts runtime-owned agent and Setup PTYs on first mount', async ({
  ...
  await expect
    .poll(() =>
      fabricaPage.evaluate(async (repoId) => {
        const state = window.__store?.getState()
  ...
  await assertTargetBindings(fabricaPage, worktreeId, allIdentities)
  await seedAgentRecoveryMetadata(fabricaPage, worktreeId, terminalIdentity(agent!))

  await faultProjectionAndActivate(fabricaPage, worktreeId, [agent!, setup!], agent!.tabId)
  const mountedAgentPtyId = await waitForActivePanePtyId(fabricaPage)
  await enableTerminalAccessibility(fabricaPage, agent!.tabId)
  await expect
    .poll(
      async () => ({
        ...
        visibleOriginalReady: (
          await terminalAccessibility(fabricaPage, agent!.tabId).innerText()
  ...
  await typeIntoTerminal(fabricaPage, agent!.tabId, agentMarker)
  await assertExactPtyReceivedMarker(electronApp, agent!.ptyId, agentMarker)
  await expect(terminalAccessibility(fabricaPage, agent!.tabId)).toContainText(
    `AGENT_INPUT:${agentPid}:${agentMarker}`
  )
  await expect(terminalAccessibility(fabricaPage, agent!.tabId)).not.toContainText(
    'Conversation interrupted'
  )

  await activateTerminal(fabricaPage, worktreeId, setup!.tabId)
  const mountedSetupPtyId = await waitForActivePanePtyId(fabricaPage)
  await enableTerminalAccessibility(fabricaPage, setup!.tabId)
  expect(mountedSetupPtyId).toBe(setup!.ptyId)
  await expect(terminalAccessibility(fabricaPage, setup!.tabId)).toContainText(
    `SETUP_READY:${setupPid}`
  )
  const setupMarker = `SETUP_KB_${randomUUID().slice(0, 8)}`
  await clearTerminalPtyWriteLog(electronApp)
  await typeIntoTerminal(fabricaPage, setup!.tabId, setupMarker)
  await assertExactPtyReceivedMarker(electronApp, setup!.ptyId, setupMarker)
  await expect(terminalAccessibility(fabricaPage, setup!.tabId)).toContainText(
    `SETUP_INPUT:${setupPid}:${setupMarker}`
  )
  await expect(terminalAccessibility(fabricaPage, setup!.tabId)).not.toContainText(
    'Conversation interrupted'
  )
  ...
  const beforeReloadDelivery = await fabricaPage.evaluate(() =>
    window.api.pty.getRendererDeliveryDebugSnapshot()
  )

  await fabricaPage.reload()
  await waitForSessionReady(fabricaPage)
  await expect
    .poll(
      async () => {
  ...
  await expect
    .poll(() => fabricaPage.evaluate(() => window.api.pty.getRendererDeliveryDebugSnapshot()))
    .toMatchObject(postReloadDelivery)
  await activateTerminal(fabricaPage, worktreeId, agent!.tabId)
  const remountedAgentPtyId = await waitForActivePanePtyId(fabricaPage)
  ...
  await expect
    .poll(() => terminalViewportText(fabricaPage, agent!.tabId))
    .toContain(remountAgentLiveOutput)
  expect(
    await fabricaPage.evaluate(() => window.api.pty.getRendererDeliveryDebugSnapshot())
  ).toMatchObject(postReloadDelivery)
  ...
  await clearTerminalPtyWriteLog(electronApp)
  await typeIntoTerminal(fabricaPage, agent!.tabId, remountAgentMarker)
  ...
  await activateTerminal(fabricaPage, worktreeId, setup!.tabId)
  const remountedSetupPtyId = await waitForActivePanePtyId(fabricaPage)
  ...
  await expect
    .poll(() => terminalViewportText(fabricaPage, setup!.tabId))
  ...
  await clearTerminalPtyWriteLog(electronApp)
  await typeIntoTerminal(fabricaPage, setup!.tabId, remountSetupMarker)
