...
test('@headful keeps resizing after the divider loses pointer capture', async ({
  ...
  await setFullscreen(electronApp, fabricaPage)
  await addTestRepo(fabricaPage, testRepoPath)
  ...
  await waitForPaneCount(fabricaPage, 2, 30_000)
  ...
  const before = await readDividerGeometry(fabricaPage)
  const startX = box.x + box.width / 2
  const startY = box.y + box.height / 2
  await fabricaPage.mouse.move(startX, startY)
  await fabricaPage.mouse.down()
  await fabricaPage.mouse.move(startX + 140, startY, { steps: 10 })
  await expect
    .poll(async () =>
      Math.abs((await readDividerGeometry(fabricaPage)).first.width - before.first.width)
    )
    .toBeGreaterThan(80)
  ...
  await fabricaPage.mouse.move(startX + 260, startY, { steps: 10 })
  await fabricaPage.mouse.up()
  await expect.poll(async () => gridsMatch(await readDividerGeometry(fabricaPage))).toBe(true)
  const after = await readDividerGeometry(fabricaPage)
