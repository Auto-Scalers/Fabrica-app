...
const FIXTURE = {
  ...
  webUrl: 'https://gitlab.example.test/acme/FABRICA/-/jobs/987654',
  mrUrl: 'https://gitlab.example.test/acme/FABRICA/-/merge_requests/4242',
...
const SCREENSHOT_DIR =
  process.env.FABRICA_GITLAB_CHECKS_JOB_DETAILS_SCREENSHOT_DIR ??
...
test.describe('#7732 GitLab Checks panel job details', () => {
  test('expanding a failed pipeline job shows its log, not "No inline details"', async ({
    ...
    await fabricaPage.waitForTimeout(8_000)

    const jobRow = fabricaPage.getByText(`${FIXTURE.stage}: ${FIXTURE.jobName}`, { exact: true })
    for (let attempt = 0; attempt < 40 && (await jobRow.count()) === 0; attempt++) {
      await linkGitLabMRToWorktree(fabricaPage, worktreeId)
    ...
    await jobRow.click()
    const noDetails = fabricaPage.getByText('No inline details are available for this check.')
    const viewFullLogs = fabricaPage.getByRole('button', { name: 'View full logs' })
