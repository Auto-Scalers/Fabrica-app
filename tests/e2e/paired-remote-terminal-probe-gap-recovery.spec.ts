...
const scratch = mkdtempSync(path.join(os.tmpdir(), 'FABRICA-paired-probe-gap-'))
...
test('replaces a stale paired stream when the PTY snapshot advanced @headful', async ({
  ...
  try {
    ...
    const hostTerminals = await callRuntime<RuntimeTerminalListResult>(fabricaPage, 'terminal.list', {
      worktree: `id:${worktreeId}`,
    ...
    await client.page
      ...
      .catch(() => undefined)
    if (terminal) {
      await callRuntime(fabricaPage, 'terminal.closeTab', { terminal }).catch(() => undefined)
    }
