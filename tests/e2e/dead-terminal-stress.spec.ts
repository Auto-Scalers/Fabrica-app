...
test.describe('Dead Terminal Stress @headful', () => {
  ...
  test('@headful setup-split with forced WebGL context loss recovers', async ({ fabricaPage }) => {
    ...
    for (let i = 0; i < STRESS_ITERATIONS; i++) {
      const newId = await createAndActivateWorktreeWithSetup(fabricaPage, `ctxloss-${i}`, 'vertical')
      ...
      const lostCount = await fabricaPage.evaluate(() => {
      ...
      await waitForAllPanesToHaveContent(fabricaPage, `ctxloss-${i} after context loss`)
   ...
   */
  test('@headful rapid worktree switching during setup-split lifecycle', async ({ fabricaPage }) => {
    test.setTimeout(120_000)
    const homeWorktreeId = await waitForActiveWorktree(fabricaPage)
    await waitForActiveTerminalManager(fabricaPage, 30_000)

    for (let i = 0; i < 3; i++) {
      const newId = await createAndActivateWorktreeWithSetup(fabricaPage, `rapid-${i}`, 'vertical')
      createdWorktreeIds.push(newId)
      ...
      await waitForActiveTerminalManager(fabricaPage, 30_000)
      await waitForPaneCount(fabricaPage, 2, 15_000)
      await waitForAllPanesToHaveContent(fabricaPage, `rapid-${i} after return`)
