...
test.describe('Create Workspace', () => {
  ...
  test('creates a worktree through the composer UI and activates it', async ({ fabricaPage }) => {
    const worktreeIdBefore = await getActiveWorktreeId(fabricaPage)
    ...
    fabricaPage.on('console', onConsole)
    ...
    try {
      ...
      await fabricaPage.getByRole('button', { name: 'New workspace', exact: true }).click()

      const dialog = fabricaPage.getByRole('dialog', { name: /Create (Workspace|Worktree)/i })
      ...
      await expect
        .poll(async () => worktreeExists(fabricaPage, workspaceName), {
          timeout: 10_000,
      ...
      await expect
        .poll(
          async () => {
            const id = await getActiveWorktreeId(fabricaPage)
      ...
      fabricaPage.off('console', onConsole)
  ...
  test('creates an emoji-named worktree with a safe git branch', async ({ fabricaPage }) => {
    const workspaceName = '?????'
    ...
    try {
      ...
      await expect(dialog).toBeHidden({ timeout: 15_000 })
      await expect(fabricaPage.getByRole('option', { name: new RegExp(workspaceName) })).toBeVisible({
        timeout: 10_000
      })

      const branch = await fabricaPage.evaluate((displayName) => {
  ...
  test('enters the Korean flag with the flag_kr shortcode suggestion', async ({ fabricaPage }) => {
    try {
      await fabricaPage.getByRole('button', { name: 'New workspace', exact: true }).click()
      ...
      const emojiSuggestions = fabricaPage.locator('[data-workspace-emoji-suggestions="true"]')
      const sourceSuggestions = fabricaPage.locator('[data-workspace-source-suggestions="true"]')
      ...
      await expect(fabricaPage.getByRole('option', { name: /:flag_kr:/i })).toHaveCount(0)
  ...
  test('shows a failed workspace entry when worktree creation fails', async ({ fabricaPage }) => {
    ...
    try {
      ...
      const failedWorkspace = fabricaPage.getByRole('button', {
  ...
  test('reuses a resolved pasted GitHub URL when quick create submits', async ({
    ...
    const url = 'https://github.com/Auto-Scalers/Fabrica-app/pull/2049'
    ...
    try {
      ...
      await expect(dialog).toBeHidden({ timeout: 15_000 })
      await expect(fabricaPage.getByRole('option', { name: linkedWorkspacePattern })).toBeVisible({
        timeout: 10_000
      })
      await expect(fabricaPage.getByRole('option', { name: url })).toHaveCount(0)
  ...
  test('names the workspace after the PR title when the pasted URL suggestion is selected', async ({
    ...
    const url = 'https://github.com/Auto-Scalers/Fabrica-app/pull/2050'
    const linkedWorkspacePattern = new RegExp(title.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'))
    ...
    try {
      ...
      const suggestion = fabricaPage.getByRole('option', { name: linkedWorkspacePattern })
      ...
      await expect(fabricaPage.getByRole('option', { name: /https-github/i })).toHaveCount(0)
      await expect(fabricaPage.getByText('Linked PR #2050')).toBeVisible()
    } finally {
      await fabricaPage
        .evaluate(() => {
          window.__store?.getState().closeModal()
