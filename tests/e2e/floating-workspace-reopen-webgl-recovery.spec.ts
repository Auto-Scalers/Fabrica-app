...
test.describe('floating workspace reopen WebGL recovery @headful', () => {
  test('reopening the floating workspace recovers a corrupted glyph atlas', async ({
    ...
    const shots = await setUpCorruptedFloatingTerminal(fabricaPage, 'REOPEN')
    ...
    await settleRecoveryWindows(fabricaPage)

    const counters = await readRecoveryCounters(fabricaPage)
    const afterReopen = await screenshotFloatingTerminal(fabricaPage)
  ...
  test('window focus regain recovers the corrupted atlas (harness control)', async ({
    ...
    const afterFocus = await screenshotFloatingTerminal(fabricaPage)
