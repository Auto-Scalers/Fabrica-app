 ...
 * - PR #677 (`return to FABRICA landing screen after closing last terminal`)
...
test.describe('Tab Close Navigation', () => {
   ...
   */
  test('closing the active editor tab activates its visual neighbor', async ({ fabricaPage }) => {
    const worktreeId = await waitForActiveWorktree(fabricaPage)

    const fileIds = await openSeededEditorTabs(fabricaPage, worktreeId, [
      'package.json',
      'README.md',
      'tsconfig.json'
    ])
    ...
    await setActiveFile(fabricaPage, fileIds[1])
    await expect.poll(async () => getActiveFileId(fabricaPage), { timeout: 3_000 }).toBe(fileIds[1])

    await closeFile(fabricaPage, fileIds[1])

    const openFilesAfter = await getOpenFiles(fabricaPage, worktreeId)
    ...
    await expect
      .poll(async () => getActiveFileId(fabricaPage), {
        timeout: 5_000,
    ...
    await expect.poll(async () => getActiveTabType(fabricaPage), { timeout: 3_000 }).toBe('editor')
  })
   ...
   */
  test('closing the active diff tab activates a still-open neighbor', async ({ fabricaPage }) => {
    const worktreeId = await waitForActiveWorktree(fabricaPage)

    // Seed two editor tabs + one diff tab in the same worktree.
    const editorIds = await openSeededEditorTabs(fabricaPage, worktreeId, [
      'package.json',
      'README.md'
    ])
    expect(editorIds.length).toBe(2)

    const diffId = await fabricaPage.evaluate((wId) => {
      const store = window.__store
      if (!store) {
        return null
      }
    ...
    await expect.poll(async () => getActiveFileId(fabricaPage), { timeout: 3_000 }).toBe(diffId)

    await closeFile(fabricaPage, diffId!)
   ...
   */
  test('closing the last visible surface returns the app to Landing', async ({ fabricaPage }) => {
    const worktreeId = await waitForActiveWorktree(fabricaPage)
    ...
    await fabricaPage.evaluate((wId) => {
      const store = window.__store
      if (!store) {
        return
      }
    ...
    const editorIds = await openSeededEditorTabs(fabricaPage, worktreeId, ['package.json'])
    expect(editorIds.length).toBe(1)

    await setActiveFile(fabricaPage, editorIds[0])
    await expect.poll(async () => getActiveFileId(fabricaPage), { timeout: 3_000 }).toBe(editorIds[0])
    ...
    await closeFile(fabricaPage, editorIds[0])
