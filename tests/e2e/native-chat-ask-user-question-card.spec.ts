...
test.describe('Desktop chat AskUserQuestion card (#11761)', () => {
  test('renders the answerable question card from the transcript when live status carries no ask', async ({
    ...
    const descriptor = await waitForActivePaneHookDescriptor(fabricaPage)
    const [tabId] = descriptor.paneKey.split(':')
    const sessionId = `e2e-ask-card-${randomUUID()}`

    const scratchDir = mkdtempSync(path.join(os.tmpdir(), 'FABRICA-e2e-ask-card-'))
    ...
    try {
      ...
      await enableNativeChatSetting(fabricaPage)
      await seedStatusWithoutAskPayload(fabricaPage, {
        paneKey: descriptor.paneKey,
        worktreeId: descriptor.worktreeId,
        sessionId,
        transcriptPath
      })
      await toggleTerminalTabToChatView(fabricaPage, { tabId, worktreeId: descriptor.worktreeId })

      await expect(fabricaPage.locator('[data-native-chat-root="true"]')).toBeVisible({
        timeout: 15_000
      })
      await expect(fabricaPage.getByText(userText)).toBeVisible({ timeout: 30_000 })
      ...
      await expect(fabricaPage.getByText(QUESTION)).toBeVisible({ timeout: 10_000 })
      await expect(fabricaPage.getByRole('button', { name: /Spaces/ })).toBeVisible()
      await fabricaPage.screenshot({ path: path.join(screenshotDir, '01-question-card.png') })
      ...
      await fabricaPage.getByRole('button', { name: /Spaces/ }).click()
      await expect(fabricaPage.getByRole('button', { name: 'Submit' })).toBeVisible()
      await fabricaPage.screenshot({ path: path.join(screenshotDir, '02-option-selected.png') })

      await fabricaPage.getByRole('button', { name: 'Submit' }).click()
      ...
      await expect(fabricaPage.getByText(QUESTION)).toHaveCount(0, { timeout: 20_000 })
      await fabricaPage.screenshot({ path: path.join(screenshotDir, '03-answered.png') })
    } finally {
