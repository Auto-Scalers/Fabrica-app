...
test.describe('Setup guide sidebar entry', () => {
  ...
  test('does not flash while completed setup waits for capability readiness', async ({
    ...
    await waitForSessionReady(fabricaPage)
    await seedCompletedSetupExceptCapabilityReadiness(fabricaPage)

    await expect
      .poll(async () => getStoreState<boolean>(fabricaPage, 'setupGuideSidebarDismissed'), {
        timeout: 5_000
      })
      .toBe(false)
    await expect(fabricaPage.getByText(CHECKLIST_TEXT)).toHaveCount(0)

    await startSetupGuideFlashMonitor(fabricaPage)

    await setActiveViewForFlashProbe(fabricaPage, 'tasks')
    await expect
      .poll(async () => getStoreState<string>(fabricaPage, 'activeView'), { timeout: 5_000 })
      .toBe('tasks')
    await fabricaPage.waitForTimeout(500)

    await setActiveViewForFlashProbe(fabricaPage, 'automations')
    await expect
      .poll(async () => getStoreState<string>(fabricaPage, 'activeView'), { timeout: 5_000 })
      .toBe('automations')
    await fabricaPage.waitForTimeout(500)

    await setActiveViewForFlashProbe(fabricaPage, 'mobile')
    await expect
      .poll(async () => getStoreState<string>(fabricaPage, 'activeView'), { timeout: 5_000 })
      .toBe('mobile')
    await fabricaPage.waitForTimeout(500)

    const flashSamples = await stopSetupGuideFlashMonitor(fabricaPage)
    ...
    await fabricaPage.evaluate(() => {
      window.dispatchEvent(new CustomEvent('FABRICA:installed-agent-skills-changed'))
    })
  })
})
...
async function installBlockedCompletedCapabilityFakes(
  ...
  await evaluateInElectronMainWithNavigationRetry(electronApp, ({ ipcMain }) => {
    ...
    const makeSkill = (name: string, id: string): SkillDiscoveryResult['skills'][number] => ({
      ...
      rootPath: '/tmp/FABRICA-e2e-skills',
      directoryPath: `/tmp/FABRICA-e2e-skills/${name}`,
      skillFilePath: `/tmp/FABRICA-e2e-skills/${name}/SKILL.md`,
      installed: true,
      fileCount: 1,
      updatedAt: 1
    })
    ...
    ipcMain.handle('skills:discover', async (): Promise<SkillDiscoveryResult> => {
      ...
      return {
        skills: [
          makeSkill('FABRICA-cli', 'e2e-FABRICA-cli'),
