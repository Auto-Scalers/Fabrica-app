...
const missingGitPath = mkdtempSync(path.join(os.tmpdir(), 'FABRICA-preflight-path-'))
...
test('routes landing preflight across runtime switch and reconnect', async ({
  electronApp,
  fabricaPage
}, testInfo) => runRuntimePreflightJourney(electronApp, fabricaPage, testInfo, false))

test('routes landing preflight across runtime switch and reconnect @headful', async ({
  electronApp,
  fabricaPage
}, testInfo) => runRuntimePreflightJourney(electronApp, fabricaPage, testInfo, true))
