...
test.describe('terminal scroll intent keeps following output', () => {
  test('a sub-row wheel-up that never moves the viewport must not stop follow-output', async ({
    ...
    const ptyId = await startStreamingFixturePhase1(fabricaPage)
    ...
    await dispatchSubRowWheelUp(fabricaPage)
    await fabricaPage.waitForTimeout(INTENT_SETTLE_WAIT_MS)

    // Any byte releases the fixture's phase-2 stream.
    await sendToTerminal(fabricaPage, ptyId, 'g')
    await waitForMarkerAtBottom(fabricaPage, 'STREAM_PHASE2_DONE')
  })

  test('a plain Home keypress delivered to the app must not stop follow-output', async ({
    fabricaPage
  }) => {
    await startStreamingFixturePhase1(fabricaPage)
    ...
    await dispatchPlainHomeKeydown(fabricaPage)
    await waitForMarkerAtBottom(fabricaPage, 'STREAM_PHASE2_DONE')
  })

  test('a real wheel pin stays fixed while visible output streams', async ({ fabricaPage }) => {
    const ptyId = await startStreamingFixturePhase1(fabricaPage)

    await dispatchRealWheel(fabricaPage, -240)
    await expect
      .poll(async () => {
        const probe = await probeActiveViewport(fabricaPage, 'STREAM_PHASE1_DONE')
        return probe ? probe.baseY - probe.viewportY : 0
      })
      .toBeGreaterThan(1)
    const pinned = await probeActiveViewport(fabricaPage, 'STREAM_PHASE1_DONE')
    if (!pinned) {
    ...
    await expect
      .poll(
        async () => {
          const probe = await probeActiveViewport(fabricaPage, 'STREAM_PHASE2_DONE')
  ...
  test('typing after a pinned write is queued resumes follow-output', async ({ fabricaPage }) => {
    await startStreamingFixturePhase1(fabricaPage)
    const { paneKey } = await waitForActivePaneHookDescriptor(fabricaPage)
    await waitForTerminalPtyDataInjector(fabricaPage, paneKey)

    await dispatchRealWheel(fabricaPage, -320)
    await expect
      .poll(async () => {
        const probe = await probeActiveViewport(fabricaPage, 'STREAM_PHASE1_DONE')
        return probe ? probe.baseY - probe.viewportY : 0
    ...
    await injectQueuedWriteThenType(fabricaPage, paneKey)
    await expect
      .poll(
        async () => {
          const probe = await probeActiveViewport(fabricaPage, 'STREAM_PHASE1_DONE')
