...
const isDockerNestedRuntimeRun =
  process.env.FABRICA_E2E_NESTED_RUNTIME_SSH === '1' && process.env.FABRICA_E2E_WEB_CLIENT === '1'
...
const test = base.extend<{ proxyJumpFixture: NestedRuntimeProxyJumpFixture | null }>({
  ...
  FABRICAAppExtraEnv: async ({ proxyJumpFixture }, provideFixture) => {
    await provideFixture(
      proxyJumpFixture ? { FABRICA_SYSTEM_SSH_PATH: proxyJumpFixture.wrapperPath } : {}
    )
  }
})
...
test.skip(
  ...
  'Run with FABRICA_E2E_NESTED_RUNTIME_SSH=1 and FABRICA_E2E_WEB_CLIENT=1'
)
...
test('routes HUB desktop, web, and two paired desktops through HUB-owned SSH', async ({
  ...
  try {
    ...
    const remote = await connectDockerSshRelayTarget(fabricaPage, sshTarget)
    await installProxyJumpFixture(proxyJumpFixture, proxyJumpDestination, proxyJumpHost)
    const proxyJumpRemote = await connectDockerSshRelayTarget(fabricaPage, proxyJumpDestination, {
      viaProxyJump: true
    })
    const localRepoId = await fabricaPage.evaluate(() => {
    ...
    const webOffer = await createRuntimeDesktopPairingOffer(fabricaPage)
    ...
    const offerA = await createRuntimeDesktopPairingOffer(fabricaPage)
    ...
    const offerB = await createRuntimeDesktopPairingOffer(fabricaPage)
    ...
    await disconnectDockerSshRelayTarget(fabricaPage, remote.targetId)
    await assertRuntimeSshStatus(clientA, remote.targetId, 'disconnected')
    await reconnectDisconnectedDockerSshRelayTarget(fabricaPage, remote.targetId)
    ...
    const rePairOffer = await createRuntimeDesktopPairingOffer(fabricaPage)
