...
test.describe('Terminal pinned viewport with streaming agent across worktree switch', () => {
  test('returning to a pinned pane with an active stream does not land at the top', async ({
    ...
    const scriptPath = path.join(testRepoPath, `.FABRICA-streaming-switch-${runId}.mjs`)
    ...
    try {
      ...
      const pinned = await pinActiveTerminalNearBottom(fabricaPage)
      ...
      const settled = await readSettledViewport(fabricaPage, pinned.tabId)
