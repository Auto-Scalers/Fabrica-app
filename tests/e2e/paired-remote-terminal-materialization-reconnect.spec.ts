...
const scratch = mkdtempSync(path.join(os.tmpdir(), 'FABRICA-paired-materialize-'))
...
test('materializes a stopped terminal on reconnect from a headed paired host', async ({
  ...
  const worktreeId = await fabricaPage.evaluate(() => window.__store?.getState().activeWorktreeId)
  if (!worktreeId) {
...
// Why fixme: this journey's fault injection cannot be set up on a headless `FABRICA serve` host.
