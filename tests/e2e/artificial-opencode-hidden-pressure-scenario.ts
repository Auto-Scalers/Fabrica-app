...
>({
  ...
  const hiddenPanes = await deps.ensureActiveWorktreePaneLoad(fabricaPage, hiddenPaneCount)

  const runId = randomUUID()
  const typingScriptPath = path.join(
    testRepoPath,
    `.FABRICA-opencode-hidden-pressure-typing-${runId}.mjs`
  )
  const pressureScriptPath = path.join(
    testRepoPath,
    `.FABRICA-opencode-hidden-pressure-load-${runId}.mjs`
  )
  ...
  try {
    ...
    await switchToTypingWorkspace(fabricaPage, firstWorktreeId)
    ...
    await waitForMainHiddenDeliveryDrops(fabricaPage, deps, pressureOutputChars)
    ...
    const debug = await deps.readTerminalPtyOutputDebug(fabricaPage)
...
async function waitForMainHiddenDeliveryDrops<TMainPressure extends HiddenPressureMainSnapshot>(
  ...
  await expect
    .poll(
      async () => (await deps.readMainPtyPressureDebug(fabricaPage))?.hiddenDeliveryDroppedChars ?? 0,
...
async function switchToTypingWorkspace(fabricaPage: Page, worktreeId: string): Promise<void> {
  await switchToWorktree(fabricaPage, worktreeId)
  await expect.poll(() => getActiveWorktreeId(fabricaPage), { timeout: 10_000 }).toBe(worktreeId)
...
>({
  ...
  await deps.releaseTerminalAckGate(fabricaPage)
  await switchToWorktree(fabricaPage, firstWorktreeId).catch(() => undefined)
  await waitForActivePanePtyId(fabricaPage)
    .then((ptyId) => sendToTerminal(fabricaPage, ptyId, '\x03'))
    .catch(() => undefined)
  await switchToWorktree(fabricaPage, secondWorktreeId).catch(() => undefined)
  await Promise.all(
    hiddenPanes.map((pane) => sendToTerminal(fabricaPage, pane.ptyId, '\x03').catch(() => undefined))
  )
