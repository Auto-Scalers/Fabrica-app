...
test.describe('Existing-user golden core flow', () => {
  test('adds project, creates workspace, opens a terminal tab, and splits a pane', async ({
    ...
    const repoPath = await createGitRepo('FABRICA-e2e-golden-existing-', 'golden-existing-project')

    await addProjectFromSidebar(fabricaPage, electronApp, repoPath)
    const workspaceName = `golden-existing-${Date.now()}`
    await createWorkspace(fabricaPage, workspaceName)
    await expectActiveWorkspaceBelongsToRepo(fabricaPage, workspaceName, repoPath)
    await ensureTerminalVisible(fabricaPage)
    await expectTerminalSurface(fabricaPage)
    await waitForTerminalPaneManager(fabricaPage)

    await createTerminalTabThroughMenu(fabricaPage)
    await splitTerminalPaneAndAssertIdentity(fabricaPage)
  })
})
...
test.describe('New-user golden core flow', () => {
  ...
  test('completes onboarding, adds a project, and follows the workspace tour handoff', async ({
    ...
    await selectCodexAgent(fabricaPage)
    await continueOnboarding(fabricaPage)
    await expect(fabricaPage.getByRole('heading', { name: /Make it feel like home/i })).toBeVisible()
    await chooseOppositeTheme(fabricaPage)
    await continueOnboarding(fabricaPage)
    await continueThroughOptionalSetupToNotifications(fabricaPage)
    await expect(fabricaPage.getByRole('button', { name: /Send Test Notification/i })).toBeVisible()
    await chooseNotificationSound(fabricaPage)
    await continueFromNotificationsToAddProject(fabricaPage)

    const repoPath = await createGitRepo('FABRICA-e2e-golden-new-', 'golden-new-project')
    await chooseFolderInNativeDialog(electronApp, repoPath)
    await fabricaPage
      .getByRole('button', { name: /Browse for a folder|Open a folder|Browse folder/i })
      .click()
    await waitForRepoLoaded(fabricaPage, repoPath)
    await expectProjectVisible(fabricaPage, repoPath)
    await waitForActiveWorktree(fabricaPage)
    await ensureTerminalVisible(fabricaPage)
    await expectTerminalSurface(fabricaPage)
    await waitForTerminalPaneManager(fabricaPage)

    await requestAgentSessionsTour(fabricaPage)
    const paneCountBeforeTourSplit = await countVisibleTerminalPanes(fabricaPage)
    await fabricaPage.getByRole('button', { name: /^Split terminal$/ }).click()
    await waitForPaneCount(fabricaPage, paneCountBeforeTourSplit + 1)
    await waitForPaneIdentitySnapshot(fabricaPage, paneCountBeforeTourSplit + 1)

    await expect(
      fabricaPage.getByRole('dialog', { name: /Start another task in parallel/i })
    ).toBeVisible()
    const createControl = fabricaPage
    ...
    await completeWorkspaceCreationTour(fabricaPage, workspaceName)
    await expectActiveWorkspaceBelongsToRepo(fabricaPage, workspaceName, repoPath)
  })
})
