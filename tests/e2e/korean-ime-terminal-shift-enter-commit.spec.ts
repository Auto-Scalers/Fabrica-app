  ...
  for (const chord of COMMITTING_ENTER_CHORDS) {
    for (const redispatchAfterKeyup of [false, true]) {
      ...
      test(`${chord.name} sends once with ${order}`, async ({
        ...
        const scriptPath = path.join(testRepoPath, `.FABRICA-korean-ime-harness-${runId}.cjs`)
        ...
        try {
          ...
          await chord.assertOutcome(fabricaPage)
          await dispatchPlainEnter(session)
          await expect
            .poll(() => readReceived(fabricaPage), {
              timeout: 10_000,
              message: 'the next physical Enter must not be consumed by stale IME state'
            })
            .toBe(chord.expectedAfterPlainEnter.received)
          expect(await readSubmitted(fabricaPage)).toEqual(chord.expectedAfterPlainEnter.submitted)
          await attachEvidence(fabricaPage, testInfo, `korean-${chord.slug}-${order}-commit`)
        } finally {
          await attachEvidence(fabricaPage, testInfo, `korean-${chord.slug}-${order}-final`).catch(
            () => undefined
          )
