...
const isPairedBrowserRun = process.env.FABRICA_E2E_WEB_CLIENT === '1'
...
test('keeps two paired browser clients and the host on independent worktrees', async ({
  ...
  await expect
    .poll(() => loadTestWorktreeIds(fabricaPage, branchA, branchB), {
      timeout: 30_000,
  ...
  const ids = await loadTestWorktreeIds(fabricaPage, branchA, branchB)
  if (!ids) {
    throw new Error('Test worktrees disappeared after discovery')
  }

  await selectWorktree(fabricaPage, ids.host)

  let clientA: Page | null = null
  let clientB: Page | null = null
  try {
    const offerA = await createPairingOffer(fabricaPage)
    ...
    const offerB = await createPairingOffer(fabricaPage)
    ...
    await expectActiveWorktree(fabricaPage, ids.host)
