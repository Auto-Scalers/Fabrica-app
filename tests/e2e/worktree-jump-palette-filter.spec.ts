...
test.describe('Worktree jump-palette filters', () => {
  ...
  test('filters workspace results by host, intersects project selection, and resets on close', async ({
    ...
    const fixture = await seedPaletteFilterFixture(fabricaPage)
    await openPalette(fabricaPage)
    await searchFixtureWorkspaces(fabricaPage, fixture)

    // P1: keyboard focus reaches the control; its rendered selection narrows rows.
    await selectRemoteHost(fabricaPage, true)
    await expect(filterTrigger(fabricaPage)).toContainText('1')
    await expect(palette(fabricaPage).getByLabel(`Remove filter ${REMOTE_HOST}`)).toBeVisible()
    await expect(worktreeRow(fabricaPage, fixture.remoteWorktreeId)).toBeVisible()
    await expect(worktreeRow(fabricaPage, fixture.localWorktreeId)).toHaveCount(0)

    // P2: host and project fields intersect, with the filter-specific empty state.
    await palette(fabricaPage).getByPlaceholder(SEARCH_PLACEHOLDER).fill('')
    await filterTrigger(fabricaPage).click()
    await palette(fabricaPage).getByText('Projects', { exact: true }).click()
    const projects = palette(fabricaPage).getByRole('listbox', { name: 'Projects' })
    ...
    await filterTrigger(fabricaPage).click()
    await expect(palette(fabricaPage).getByText('No results match the active filter')).toBeVisible()
    await expect(
      palette(fabricaPage).getByText('Clear the filter above, or widen it to more hosts and projects.')
    ).toBeVisible()

    // P3: clear restores both rows; closing drops the ephemeral filter.
    await filterTrigger(fabricaPage).click()
    await palette(fabricaPage).getByRole('button', { name: 'Clear all' }).last().click()
    await filterTrigger(fabricaPage).click()
    await expect(filterTrigger(fabricaPage)).not.toContainText('1')
    await searchFixtureWorkspaces(fabricaPage, fixture)

    await selectRemoteHost(fabricaPage)
    await fabricaPage.evaluate(() => window.__store?.getState().closeModal())
    await expect(palette(fabricaPage)).toBeHidden()
    await openPalette(fabricaPage)
    await searchFixtureWorkspaces(fabricaPage, fixture)
    await expect(filterTrigger(fabricaPage)).not.toContainText('1')
  })
