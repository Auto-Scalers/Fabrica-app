...
test.describe('local Codex terminal typing latency', () => {
  test('keeps Codex prompt typing responsive @local-real-codex', async ({ fabricaPage }, testInfo) => {
    test.skip(
    ...
    try {
      await sendToTerminal(fabricaPage, ptyId, launchCommand)
      await dismissCodexPromptsIfPresent(fabricaPage)
      const composerMarker = await waitForCodexComposer(fabricaPage)
      ...
      await installCodexEchoLatencyProbe(fabricaPage, typed)
      for (const char of typed) {
        await fabricaPage.keyboard.type(char)
        ...
        await fabricaPage.waitForTimeout(KEYSTROKE_INTERVAL_MS)
      }
      // Why: the last keystroke's echo can still be in flight when typing ends.
      await fabricaPage.waitForTimeout(1_000)
      const report = await collectCodexEchoLatencyReport(fabricaPage)
