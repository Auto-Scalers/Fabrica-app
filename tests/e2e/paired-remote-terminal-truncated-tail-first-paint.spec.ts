...
const scratch = mkdtempSync(path.join(os.tmpdir(), 'FABRICA-paired-truncated-tail-'))
...
test.use({
  FABRICAAppExtraEnv: {
    FABRICA_E2E_TERMINAL_PARKING_DELAY_MS: String(RETENTION_PARK_DELAY_MS),
...
test('legacy paired hosts retain the lossy hidden-manager budget fallback @headful', async ({
  ...
  test.skip(
    process.env.FABRICA_E2E_DISABLE_PAIRED_TERMINAL_PARKING !== '1',
    'The legacy fallback requires a host without terminal.paired-parking.v1.'
  )
  test.setTimeout(120_000)
  const worktreeIds = await fabricaPage.evaluate(() =>
    window.__store
      ?.getState()
      .allWorktrees()
      .slice(0, 2)
