...
test.describe('Terminal tab switch visual restore', () => {
  ...
  test('keeps full-width geometry after switching away and back', async ({
    ...
    const { firstTabId, secondTabId } = await ensureTwoTerminalTabs(fabricaPage)
    await forceWebglOnActiveTab(fabricaPage)

    const runId = `${Date.now()}`
    const marker = `${TAB_SWITCH_MARKER_PREFIX}_${runId}`
    const firstPtyId = await waitForPanePtyIdOnTab(fabricaPage, firstTabId)
    await writeStaticTabContent(fabricaPage, firstTabId, marker, TAB_A_GLYPH_ROW)

    const baseline = await readTabTerminalGeometry(fabricaPage, firstTabId, runId)
    ...
    await resetTerminalOutputSchedulerDebug(fabricaPage)
    await startHiddenPtyOutputBurst(fabricaPage, firstPtyId, runId)

    for (let cycle = 0; cycle < 12; cycle += 1) {
      await activateTerminalTab(fabricaPage, secondTabId)
      await injectHiddenStreamingBurst(fabricaPage, firstTabId, runId)
      ...
      await fabricaPage.waitForTimeout(60)
      const settled = await readTabTerminalGeometry(fabricaPage, firstTabId, runId)
    ...
    const schedulerActivity = await waitForHiddenOutputSchedulerActivity(fabricaPage)
  ...
  test('keeps geometry after hidden alt-screen TUI redraws during tab switches', async ({
    ...
    const { firstTabId, secondTabId } = await ensureTwoTerminalTabs(fabricaPage)
    await forceWebglOnActiveTab(fabricaPage)
    await waitForPanePtyIdOnTab(fabricaPage, firstTabId)
    ...
    for (let cycle = 0; cycle < 6; cycle += 1) {
      ...
      await setHiddenSnapshotOverride(fabricaPage, ptyId, {
        data: buildAltScreenFrame(finalMarker, restoreFrame),
        cols,
        rows
      })
      await activateTerminalTab(fabricaPage, secondTabId)
      await writeToPaneTerminal(fabricaPage, firstTabId, redraw)
      await activateTerminalTab(fabricaPage, firstTabId)

      const geometry = await readTabTerminalGeometry(fabricaPage, firstTabId, `${runId}_ALT`)
      const renderedFrame = await readRenderedAltScreenFrame(fabricaPage, firstTabId, finalMarker)
      renderPaths.push(
  ...
  test('restores skipped hidden agent output on light tab resume', async ({ fabricaPage }) => {
    await waitForSessionReady(fabricaPage)
    await waitForActiveWorktree(fabricaPage)
    await ensureTerminalVisible(fabricaPage)
    await waitForActiveTerminalManager(fabricaPage, 30_000)

    const shellTabId = (await getActiveTabId(fabricaPage))!
    const agentTabId = await createCodexMarkedTerminalTab(fabricaPage)
    await waitForActiveTerminalManager(fabricaPage, 30_000)
    await waitForPanePtyIdOnTab(fabricaPage, agentTabId)
    const paneIdentity = await readPaneIdentityOnTab(fabricaPage, agentTabId)
    const paneKey = `${agentTabId}:${paneIdentity.leafId}`

    await activateTerminalTab(fabricaPage, shellTabId)
    const runId = `${Date.now()}`
    ...
    await resetHiddenOutputDebug(fabricaPage)
    await injectPaneData(fabricaPage, paneKey, hiddenFrame, {
      seq: hiddenFrame.length,
      rawLength: hiddenFrame.length
    })

    await expect
      .poll(async () => (await readHiddenOutputDebug(fabricaPage))?.hiddenRendererSkipCount ?? 0, {
        timeout: 5_000,
        message: 'Codex-marked hidden output did not take the skipped renderer path'
      })
      .toBeGreaterThan(0)
    await setHiddenSnapshotOverride(fabricaPage, paneIdentity.ptyId, {
    ...
    await activateTerminalTab(fabricaPage, agentTabId)

    await expect
      .poll(() => getTerminalContent(fabricaPage, 8_000), {
        timeout: 10_000,
  ...
  test('restores skipped hidden Grok output on light tab resume', async ({ fabricaPage }) => {
    await waitForSessionReady(fabricaPage)
    await waitForActiveWorktree(fabricaPage)
    await ensureTerminalVisible(fabricaPage)
    await waitForActiveTerminalManager(fabricaPage, 30_000)
    ...
    const shellTabId = (await getActiveTabId(fabricaPage))!
    const grokTabId = await createGrokMarkedTerminalTab(fabricaPage)
    await waitForActiveTerminalManager(fabricaPage, 30_000)
    await waitForPanePtyIdOnTab(fabricaPage, grokTabId)
    const paneIdentity = await readPaneIdentityOnTab(fabricaPage, grokTabId)
    ...
    await activateTerminalTab(fabricaPage, grokTabId)
  ...
  test('keeps returned tab glyphs intact across tab switches', async ({ fabricaPage }, testInfo) => {
    ...
    await forceWebglOnActiveTab(fabricaPage)
    await activateTerminalTab(fabricaPage, firstTabId)
    const firstWebgl = await waitForWebglOnTab(fabricaPage, firstTabId)
    await activateTerminalTab(fabricaPage, secondTabId)
    await fabricaPage.evaluate((id) => {
      window.__paneManagers?.get(id)?.setTerminalGpuAcceleration?.('on')
    }, secondTabId)
    const secondWebgl = await waitForWebglOnTab(fabricaPage, secondTabId)
    ...
    const firstPtyId = await waitForPanePtyIdOnTab(fabricaPage, firstTabId)
    const secondPtyId = await waitForPanePtyIdOnTab(fabricaPage, secondTabId)
    await sendToTerminal(fabricaPage, firstPtyId, SILENT_FOREGROUND_COMMAND)
    await sendToTerminal(fabricaPage, secondPtyId, SILENT_FOREGROUND_COMMAND)
    await fabricaPage.waitForTimeout(1_000)

    const runId = `${Date.now()}`
    const markerA = `${TAB_SWITCH_MARKER_PREFIX}_A_${runId}`
    const markerB = `${TAB_SWITCH_MARKER_PREFIX}_B_${runId}`
    await writeStaticTabContent(fabricaPage, firstTabId, markerA, TAB_A_GLYPH_ROW)
    await activateTerminalTab(fabricaPage, secondTabId)
    await writeStaticTabContent(fabricaPage, secondTabId, markerB, TAB_B_GLYPH_ROW)

    await activateTerminalTab(fabricaPage, firstTabId)
    await resetAtlasOnTab(fabricaPage, firstTabId)
    await fabricaPage.waitForTimeout(800)
    const baseline = await captureStableTabScreenshot(fabricaPage, firstTabId)
    ...
    for (let cycle = 0; cycle < 8; cycle += 1) {
      ...
      const afterReturn = await captureStableTabScreenshot(fabricaPage, firstTabId)
