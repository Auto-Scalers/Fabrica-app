...
const scratch = mkdtempSync(path.join(os.tmpdir(), 'FABRICA-headless-stalled-stream-'))
...
test('recovers an ACK-starved stream from an isolated headless FABRICA host @headful', async ({
  testRepoPath
}) => {
