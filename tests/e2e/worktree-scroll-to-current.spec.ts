...
test.describe('Reveal active workspace button', () => {
  test.beforeEach(async ({ fabricaPage }) => {
    ...
    await fabricaPage.emulateMedia({ reducedMotion: 'reduce' })
  ...
  test('clears sidebar filters before revealing a hidden current workspace', async ({
    ...
    await prepareSidebarForScrollTest(fabricaPage)

    const renderedOptions = fabricaPage.locator('[data-worktree-sidebar] [role="option"]')
    ...
    const targetRows = fabricaPage.locator(
      `[data-worktree-sidebar] [data-worktree-id=${JSON.stringify(targetId)}]`
    )
    const targetRow = targetRows.first()
    const revealButton = fabricaPage.getByRole('button', { name: 'Reveal active workspace' })

    await fabricaPage.evaluate((targetId) => {
      const store = window.__store
      if (!store) {
        throw new Error('window.__store is not available')
      }
  ...
  test('reveals the current workspace when it starts outside the virtualized window', async ({
    ...
    const targetId = await fabricaPage.evaluate(() => {
      const store = window.__store
      if (!store) {
        throw new Error('window.__store is not available')
      }
    ...
    const scroller = fabricaPage.locator('[data-worktree-sidebar]')
    await expect
      .poll(
        () =>
          fabricaPage.evaluate(() => {
  ...
  test('uses the active workspace key when the legacy active worktree id is not set', async ({
    ...
    const folderWorktreeId = await fabricaPage.evaluate(() => {
      const store = window.__store
      if (!store) {
        throw new Error('window.__store is not available')
      }
