...
test.describe('Terminal tab switch SIGWINCH restore', () => {
  test('keeps an alternate-screen Codex viewport after hidden snapshot replay', async ({
    ...
    await sendToTerminal(fabricaPage, paneIdentity.ptyId, 'ARM_SIGWINCH_PROBE\n')
    await expect
      .poll(() => getTerminalContent(fabricaPage, 8_000), {
        timeout: 10_000,
    ...
    const armedContentAfterSettle = await getTerminalContent(fabricaPage, 8_000)
    ...
    await expect
      .poll(
        async () => {
          const content = await getTerminalContent(fabricaPage, 8_000)
      ...
      .toBe('snapshot')
    await fabricaPage.waitForTimeout(1_200)
    const contentAfterSettle = await getTerminalContent(fabricaPage, 8_000)
