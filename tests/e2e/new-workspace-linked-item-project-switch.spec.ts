...
test.describe('New workspace composer linked item across project switches', () => {
  ...
  test.beforeEach(async ({ fabricaPage }) => {
    ...
    tempRoot = mkdtempSync(path.join(os.tmpdir(), 'FABRICA-e2e-linked-item-'))
    secondRepoPath = path.join(tempRoot, SECOND_PROJECT_NAME)
    createGitRepo(secondRepoPath)
    await addSecondProject(fabricaPage, secondRepoPath)
  })
  ...
  test('keeps a Jira issue linked when the project changes', async ({ fabricaPage }) => {
    const jiraSourceContext = await getJiraSourceContext(fabricaPage)
    ...
    const composer = fabricaPage.getByRole('dialog')
    ...
    await switchComposerProject(fabricaPage, SECOND_PROJECT_NAME)

    await expect(sourcePill).toContainText('RDG-344 Migrate homepage from NuxtJS to NextJS')
  })

  test('clears a repo-scoped GitHub issue when the project changes', async ({ fabricaPage }) => {
    await openComposerWithLinkedWorkItem(
      fabricaPage,
      {
        type: 'issue',
