...
test.describe('terminal streaming refocus viewport', () => {
  test('keeps follow-output at the bottom through a queued-write refocus wobble', async ({
    ...
    await execInTerminal(fabricaPage, ptyId, nodeTerminalCommand([STREAMING_FIXTURE_PATH]))
    await waitForPhaseOneAtBottom(fabricaPage, tabId)

    const framesPromise = sampleRevealFrames(fabricaPage, tabId)
    await injectQueuedWriteAndRefocus(fabricaPage, tabId, paneKey)
    ...
    await expect
      .poll(() => getTerminalContent(fabricaPage), { timeout: 15_000 })
      .toContain('REFOCUS_STREAM_DONE')
    const visibleScrollbar = fabricaPage.locator('.xterm-scrollbar.xterm-vertical:visible').first()
