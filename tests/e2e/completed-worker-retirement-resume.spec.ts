...
for (const closeMode of ['terminal-close-cli', 'worker-release'] as const) {
  test(`completed background worker ${closeMode} retires resume authority before first activation`, async ({
    ...
    const coordinatorWorktreeId = await waitForActiveWorktree(fabricaPage)
    ...
    const coordinatorPane = await waitForActivePaneHookDescriptor(fabricaPage)
    ...
    await expect
      .poll(
        async () => {
          ...
          const rendererWorktreeIds = await fabricaPage.evaluate(() =>
    ...
    await fabricaPage.evaluate(
      ({ tabId, worktreeId }) => {
        window.dispatchEvent(
          new CustomEvent('FABRICA-background-mount-terminal-worktree', {
    ...
    await expect
      .poll(() =>
        fabricaPage.evaluate((tabId) => Boolean(window.__paneManagers?.get(tabId)), workerBefore.tabId)
      )
      .toBe(true)
    expect(
      await fabricaPage.evaluate(
    ...
    await client.call('terminal.send', {
      ...
      text: 'FABRICA_E2E_EXIT_AFTER_DONE',
      enter: true
    })
    await expect
    ...
    await fabricaPage.evaluate(
      ({ paneKey, tabId, worktreeId }) => {
        ...
        const e2eWindow = window as typeof window & {
          __FABRICARetiredWorkerTransitions?: Transition[]
          __FABRICARetiredWorkerUnsubscribe?: () => void
        }
        ...
        e2eWindow.__FABRICARetiredWorkerTransitions = transitions
        e2eWindow.__FABRICARetiredWorkerUnsubscribe = store.subscribe((state) => {
          const next = {
            tabPresent: Boolean(state.tabsByWorktree[worktreeId]?.some((tab) => tab.id === tabId)),
            recoveryPresent: Boolean(state.sleepingAgentSessionsByPaneKey[paneKey])
          }
    ...
    if (closeMode === 'terminal-close-cli') {
      const closed = runBuiltFABRICACli(['terminal', 'close', '--terminal', workerHandle, '--json'], {
        userDataDir,
        cwd: process.cwd()
      })
    ...
    await expect
      .poll(() =>
        fabricaPage.evaluate(() => {
          type Transition = { tabPresent: boolean; recoveryPresent: boolean }
          return (window as typeof window & { __FABRICARetiredWorkerTransitions?: Transition[] })
            .__FABRICARetiredWorkerTransitions
        })
      )
      .toEqual(
    ...
    await fabricaPage.evaluate(() => {
      const e2eWindow = window as typeof window & { __FABRICARetiredWorkerUnsubscribe?: () => void }
      e2eWindow.__FABRICARetiredWorkerUnsubscribe?.()
      delete e2eWindow.__FABRICARetiredWorkerUnsubscribe
    })
    await expect
      .poll(() =>
        fabricaPage.evaluate(
    ...
    await fabricaPage.evaluate(() => window.dispatchEvent(new Event('beforeunload')))
    await expect
      .poll(() =>
        fabricaPage.evaluate(async (paneKey) => {
          const session = await window.api.session.get()
          return session.sleepingAgentSessionsByPaneKey?.[paneKey] ?? null
        }, workerPaneKey)
      )
      .toBeNull()
    await fabricaPage.evaluate(() => window.api.session.flush())
    expect(readPersistedWorkerRecoveryRecord(userDataDir, workerPaneKey)).toBeNull()

    await fabricaPage.reload()
    await waitForSessionReady(fabricaPage)

    const beforeActivation = await fabricaPage.evaluate((worktreeId) => {
      const state = window.__store?.getState()
      return {
    ...
    await waitForActiveTerminalManager(fabricaPage)
    await waitForActivePanePtyId(fabricaPage)
    const activatedPane = await waitForActivePaneHookDescriptor(fabricaPage)
    ...
    await expect(fabricaPage.locator('.session-restored-banner')).toHaveCount(0)

    const afterActivation = await fabricaPage.evaluate(
