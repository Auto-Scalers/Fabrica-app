...
>({
  ...
  const secondWorktreeId = (await getAllWorktreeIds(fabricaPage)).find((id) => id !== firstWorktreeId)
  ...
  await sendToTerminal(fabricaPage, typingPtyId, `printf '\\n${typingPtyReadyMarker}\\n'\r`)
  await waitForMarkerLatency(fabricaPage, typingPtyReadyMarker, 10_000)

  await switchToWorktree(fabricaPage, firstWorktreeId)
  await ensureTerminalVisible(fabricaPage)
  await waitForActiveTerminalManager(fabricaPage, 30_000)
  const panes = await deps.ensureActiveWorktreePaneLoad(fabricaPage, backgroundPaneCount + 1)
  const [revisitPane, ...loadPanes] = panes
  await deps.focusPane(fabricaPage, revisitPane.paneKey)

  const typingScriptPath = path.join(testRepoPath, `.FABRICA-revisit-typing-${runId}.mjs`)
  const pressureScriptPath = path.join(testRepoPath, `.FABRICA-revisit-pressure-${runId}.mjs`)
  ...
  await deps.resetTerminalPtyOutputDebug(fabricaPage)
  ...
  try {
    ...
    const pressureBeforeSwitch = await deps.waitForMainPtyPressureBacklog(fabricaPage)

    await switchToWorktree(fabricaPage, secondWorktreeId)
    await ensureTerminalVisible(fabricaPage)
    await waitForActiveTerminalManager(fabricaPage, 30_000)
    await waitForTerminalPtyVisible(fabricaPage, typingPtyId)
    ...
    const duringPressure = await deps.readMainPtyPressureDebug(fabricaPage)
    const ackGate = await deps.readTerminalAckGateDebug(fabricaPage)
    const scheduler = await deps.readTerminalOutputSchedulerDebug(fabricaPage)
    const hiddenDebug = await deps.readTerminalPtyOutputDebug(fabricaPage)
    ...
    await waitForTerminalPtyVisible(fabricaPage, revisitPane.ptyId)
    await deps.focusPane(fabricaPage, revisitPane.paneKey)
    await sendToTerminal(fabricaPage, revisitPane.ptyId, `printf '\\n${revisitMarker}\\n'\r`)
    const revisitLatencyMs = await waitForMarkerLatency(fabricaPage, revisitMarker, 10_000)
    ...
    await deps.releaseTerminalAckGate(fabricaPage)
    await deps.focusPane(fabricaPage, loadPanes[0]?.paneKey ?? revisitPane.paneKey)
    const pressureDrainLatencyMs = await waitForMarkerLatency(fabricaPage, pressureDoneMarker, 20_000)
    const finalScheduler = await deps.readTerminalOutputSchedulerDebug(fabricaPage)
    testInfo.annotations.push({
    ...
    await sendToTerminal(fabricaPage, revisitPane.ptyId, '\x03').catch(() => undefined)
    await Promise.all(
      loadPanes.map((pane) => sendToTerminal(fabricaPage, pane.ptyId, '\x03').catch(() => undefined))
    )
