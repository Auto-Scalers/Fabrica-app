...
test.describe('Large diff freeze repro', () => {
  ...
  test('opening a large single-file diff keeps the renderer responsive', async ({ fabricaPage }) => {
    await waitForSessionReady(fabricaPage)
    const fixture = createIsolatedLargeDiffRepo()
    const lineCount = Number(process.env.FABRICA_LARGE_DIFF_REPRO_LINES ?? '60000')
    if (!Number.isFinite(lineCount) || lineCount < 0) {
      throw new Error(
        `Invalid FABRICA_LARGE_DIFF_REPRO_LINES: ${process.env.FABRICA_LARGE_DIFF_REPRO_LINES}`
      )
    }
    ...
    try {
      ...
      const measurement = await fabricaPage.evaluate(
