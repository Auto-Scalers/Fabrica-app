...
const SETUP_SCRIPT_COMMAND = 'echo FABRICA-e2e-setup'
...
/** Repo whose shared FABRICA.yaml carries a real setup script, plus a second worktree. */
function createRepoWithSharedSetupScript(repoPath: string, featureWorktreePath: string): void {
  ...
  writeFileSync(path.join(repoPath, 'README.md'), '# Unreadable FABRICA.yaml E2E\n')
...
/**
 * Makes the main process report the failure the fix now surfaces: FABRICA.yaml could
 ...
 */
async function installUnreadableFABRICAYamlFault(electronApp: ElectronApplication): Promise<void> {
  await electronApp.evaluate(({ ipcMain }) => {
    type InvokeHandler = (event: unknown, ...args: unknown[]) => unknown
    const faultState = globalThis as typeof globalThis & { __FABRICAE2eFABRICAYamlUnreadable?: boolean }
    ...
    faultState.__FABRICAE2eFABRICAYamlUnreadable = true
    ipcMain.removeHandler('hooks:check')
    ipcMain.handle('hooks:check', async (event, ...args) => {
      if (faultState.__FABRICAE2eFABRICAYamlUnreadable) {
...
async function healFABRICAYamlRead(electronApp: ElectronApplication): Promise<void> {
  await electronApp.evaluate(() => {
    ;(
      globalThis as typeof globalThis & { __FABRICAE2eFABRICAYamlUnreadable?: boolean }
    ).__FABRICAE2eFABRICAYamlUnreadable = false
  })
}
...
test.describe('Setup script prompt', () => {
  ...
  test('recovers from an unreadable FABRICA.yaml instead of pinning the failed verdict', async ({
    electronApp,
    fabricaPage
  }, testInfo) => {
    const repoPath = testInfo.outputPath('unreadable-FABRICA-yaml-repo')
    const featureWorktreePath = testInfo.outputPath('unreadable-FABRICA-yaml-feature')
    createRepoWithSharedSetupScript(repoPath, featureWorktreePath)

    await installUnreadableFABRICAYamlFault(electronApp)
    ...
    const promptCard = fabricaPage.locator('[data-setup-script-prompt-layer]')
    const inspectionError = promptCard.getByText(INSPECTION_ERROR_TEXT)
    await expect(inspectionError).toBeVisible({ timeout: 20_000 })

    // FABRICA.yaml is readable again; the card is still pinned to the failed verdict.
    await healFABRICAYamlRead(electronApp)
    const healthyCheck = await fabricaPage.evaluate(
    ...
    await fabricaPage.waitForTimeout(RECORDING_DWELL_MS)

    // Activating another worktree in the same repo must re-inspect.
    const featureRow = worktreeRow(fabricaPage, featureWorktreeId)
    await expect(featureRow).toBeVisible()
    await worktreeRowSurface(fabricaPage, featureWorktreeId).click()
    await expect(featureRow).toHaveAttribute('aria-current', 'page')

    // The repo has a valid FABRICA.yaml scripts.setup, so no prompt may remain.
