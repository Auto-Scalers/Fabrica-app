...
test.describe('Diff note edit', () => {
  ...
  test('editing a saved inline note updates the open diff card', async ({ fabricaPage }) => {
    ...
    const addResult = await fabricaPage.evaluate(
      async ({ wId, rel, body }) => {
        const store = window.__store
        if (!store) {
    ...
    const card = fabricaPage.locator('.FABRICA-diff-comment-card').first()
    await expect(card, 'seeded inline note did not render').toBeVisible({ timeout: 15_000 })
    await expect(card.locator('.FABRICA-diff-comment-body')).toHaveText(seededBody)

    await card.getByTitle('Edit note').click()

    const textarea = card.locator('.FABRICA-diff-comment-popover-textarea')
    await expect(textarea).toBeVisible()
    await expect(textarea).toHaveValue(seededBody)

    const saveButton = card
      .locator('.FABRICA-diff-comment-popover-footer button')
      .filter({ hasText: 'Save' })
    ...
    await expect
      .poll(
        async () =>
          fabricaPage.evaluate((id: string) => {
            const store = window.__store
            if (!store) {
              return null
            }
    ...
    const updatedCard = fabricaPage
      .locator('.FABRICA-diff-comment-card')
      .filter({ has: fabricaPage.locator('.FABRICA-diff-comment-body', { hasText: editedBody }) })
      .first()
    await expect(updatedCard, 'inline card did not update in the open diff').toBeVisible()
    await expect(updatedCard.locator('.FABRICA-diff-comment-body')).toHaveText(editedBody)
    await expect(updatedCard.locator('.FABRICA-diff-comment-body')).not.toHaveText(seededBody)
