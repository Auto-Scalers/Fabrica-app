...
async function createBrowserFixture(
  ...
  const fixtureDir = mkdtempSync(path.join(os.tmpdir(), 'FABRICA-browser-recovery-'))
...
test('browser chrome recovers a live registered file guest after renderer loss', async ({
  ...
  await expect
    .poll(() => readBrowserGuestState(fabricaPage, browserTab.id), { timeout: 10_000 })
    .toMatchObject({
      chromePresent: true,
      marker: 'painted-file-guest',
      url: fixtureUrl
    })
  const before = await readBrowserGuestState(fabricaPage, browserTab.id)
  expect(before.webContentsId).not.toBeNull()
  const beforeProcessId = await readGuestProcessId(electronApp, before.webContentsId!)
  await expect
    .poll(
      async () =>
        (await listRegisteredBrowserPages(fabricaPage, worktreeId)).result?.tabs?.find(
  ...
  const recovered = await readBrowserGuestState(fabricaPage, browserTab.id)
  ...
  await expect
    .poll(() => listRegisteredBrowserPages(fabricaPage, worktreeId), { timeout: 10_000 })
    .toMatchObject({
      ok: true,
  ...
  await fabricaPage.evaluate((browserPageId) => {
    return window.api.browser.unregisterGuest({ browserPageId })
  }, browserTab.activePageId)
  await expect
    .poll(
      async () =>
        (await listRegisteredBrowserPages(fabricaPage, worktreeId)).result?.tabs?.some(
          (tab) => tab.browserPageId === browserTab.activePageId
        ) ?? false
    )
    .toBe(false)
  ...
  const resumeRecovered = await readBrowserGuestState(fabricaPage, browserTab.id)
  ...
  const backgroundTab = await fabricaPage.evaluate(
    ({ targetWorktreeId }) =>
  ...
  await expect
    .poll(() => readBrowserGuestState(fabricaPage, backgroundTab!.id), { timeout: 10_000 })
  ...
  await expect
    .poll(() => readBrowserGuestState(fabricaPage, browserTab.id), { timeout: 10_000 })
    .toMatchObject({ chromePresent: true, marker: 'painted-file-guest', url: fixtureUrl })
  const rendererReloaded = await readBrowserGuestState(fabricaPage, browserTab.id)
  expect(rendererReloaded.webContentsId).not.toBe(beforeRendererReloadId)

  await fabricaPage.evaluate((targetBrowserTabId) => {
    window.__store?.getState().setActiveBrowserTab(targetBrowserTabId)
  }, backgroundTab!.id)
  const hiddenBefore = await readBrowserGuestState(fabricaPage, browserTab.id)
...
test('dom-ready ID loss waits for validation without reloading the guest', async ({
  ...
  await expect
    .poll(() => readBrowserGuestState(fabricaPage, browserTab.id))
  ...
  await expect
    .poll(() => listRegisteredBrowserPages(fabricaPage, worktreeId))
    .toMatchObject({
      ok: true,
      result: { tabs: [{ browserPageId: browserTab.activePageId, url: fixtureUrl }] }
    })
...
test('explicit navigation repairs a recovery error without dom-ready churn', async ({
  ...
  await expect
    .poll(() => readBrowserPageRecoveryState(fabricaPage, browserTab.id, browserTab.activePageId))
  ...
  const addressBar = fabricaPage.locator(
    `[data-browser-overlay-tab-id="${browserTab.id}"] [data-FABRICA-browser-address-bar="true"]`
  )
  ...
  await expect(fabricaPage.getByText('Recovery fixture error', { exact: true })).toBeVisible()
  expect(
    await electronApp.evaluate(() => {
      const testState = globalThis as typeof globalThis & {
        browserRecoveryValidationCalls?: number
...
test('attachment keeps recovery error until document readiness', async ({
  ...
  await fabricaPage
    ...
    .nth(2)
    .click()
  const recoveryErrorCode = await fabricaPage.evaluate(
    ({ workspaceId, browserPageId }) =>
...
test('minimized browser guest stays painted and registered after restore @headful', async ({
  ...
  await expect
    .poll(() => readBrowserGuestState(fabricaPage, browserTab.id))
    .toMatchObject({ chromePresent: true, marker: 'painted-file-guest', url: fixtureUrl })
  const restored = await readBrowserGuestState(fabricaPage, browserTab.id)
