...
test.describe('Tab Rename (Inline)', () => {
  ...
  test('double-clicking a tab opens an inline rename input and Enter commits', async ({
    ...
    const originalTitle = await getActiveTabTitle(fabricaPage, worktreeId)
    expect(originalTitle.length).toBeGreaterThan(0)

    const tabLocator = tabLocatorByTitle(fabricaPage, originalTitle)
    await tabLocator.dblclick()

    const renameInput = fabricaPage.getByRole('textbox', {
    ...
    await expect
      .poll(async () => getActiveCustomTitle(fabricaPage, worktreeId), { timeout: 3_000 })
      .toBe('My Custom Title')
    await expect(renameInput).toBeHidden()
    await expect(tabLocatorByTitle(fabricaPage, 'My Custom Title')).toBeVisible()
  })
  ...
  test('context-menu Change Title opens a focused select-all rename input', async ({
    ...
    await tabLocatorByTitle(fabricaPage, originalTitle).click({ button: 'right' })
    await fabricaPage.getByRole('menuitem', { name: 'Change Title', exact: true }).click()
    ...
    await expect(tabLocatorByTitle(fabricaPage, 'Context Menu Title')).toBeVisible()
  })

  test('Escape during inline rename discards the edit', async ({ fabricaPage }) => {
    const worktreeId = (await getActiveWorktreeId(fabricaPage))!
    const originalTitle = await getActiveTabTitle(fabricaPage, worktreeId)
  ...
  test('renaming to an empty string resets the tab to its default title', async ({ fabricaPage }) => {
    const worktreeId = (await getActiveWorktreeId(fabricaPage))!
    ...
    const tabLocator = tabLocatorByTitle(fabricaPage, 'Seeded Custom')
    await tabLocator.dblclick()
    ...
    await expect(tabLocatorByTitle(fabricaPage, defaultTitle)).toBeVisible()
    await expect
      .poll(async () => getActiveCustomTitle(fabricaPage, worktreeId), { timeout: 3_000 })
      .toBe(null)
  })

  test('clicking away (blur) commits the rename', async ({ fabricaPage }) => {
    const worktreeId = (await getActiveWorktreeId(fabricaPage))!
    ...
    await expect
      .poll(async () => (await getWorktreeTabs(fabricaPage, worktreeId)).length, { timeout: 3_000 })
      .toBeGreaterThanOrEqual(2)

    const tabs = await getWorktreeTabs(fabricaPage, worktreeId)
    const activeId = await getActiveTabId(fabricaPage)
    const activeTab = tabs.find((t) => t.id === activeId)!
    const otherTab = tabs.find((t) => t.id !== activeId)!

    const tabLocator = tabLocatorByTitle(fabricaPage, activeTab.title!)
    await tabLocator.dblclick()
    ...
    await tabLocatorByTitle(fabricaPage, otherTab.title!).click()

    await expect(renameInput).toBeHidden()
    await expect(tabLocatorByTitle(fabricaPage, 'Committed By Blur')).toBeVisible()
    expect(
      await fabricaPage.evaluate(
  ...
  test('terminal title updates do not resize neighboring tabs', async ({ fabricaPage }) => {
    const worktreeId = (await getActiveWorktreeId(fabricaPage))!
    const tabIds = await fabricaPage.evaluate((targetWorktreeId) => {
    ...
    const tabs = tabIds.map((id) =>
      fabricaPage.locator(`[data-testid="sortable-tab"][data-tab-id="${id}"]`)
    )
  ...
  test('rename input stays at a usable width when many tabs are open', async ({ fabricaPage }) => {
    const worktreeId = (await getActiveWorktreeId(fabricaPage))!
    const targetTabId = await getActiveTabId(fabricaPage)
    ...
    const tabLocator = tabLocatorByTitle(fabricaPage, targetTitle)
  ...
  test('middle-clicking inside the rename input does not close the tab', async ({ fabricaPage }) => {
    const worktreeId = (await getActiveWorktreeId(fabricaPage))!
    const tabsBefore = (await getWorktreeTabs(fabricaPage, worktreeId)).length
    const originalTitle = await getActiveTabTitle(fabricaPage, worktreeId)
    ...
    await expect(renameInput).toBeVisible()
    await expect(tabLocatorByTitle(fabricaPage, originalTitle)).toBeVisible()
    expect((await getWorktreeTabs(fabricaPage, worktreeId)).length).toBe(tabsBefore)
  })
})
