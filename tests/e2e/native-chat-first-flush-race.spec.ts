...
test.describe('Native chat first-flush transcript race (#8401)', () => {
  test('stays in loading (never errors) until a not-yet-flushed transcript appears, then hydrates live', async ({
    ...
    try {
      ...
      await seedClaudeProviderSession(fabricaPage, {
      ...
      await expect(fabricaPage.getByText(LOADING_TITLE)).toBeVisible({ timeout: 10_000 })
      ...
      await expect(fabricaPage.getByText(assistantText)).toBeVisible({ timeout: 30_000 })
      await expect(fabricaPage.getByText(ERROR_TITLE)).toHaveCount(0)
      await fabricaPage.screenshot({
