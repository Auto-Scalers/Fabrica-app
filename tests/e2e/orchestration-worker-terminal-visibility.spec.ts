...
const fakeCliDir = mkdtempSync(path.join(os.tmpdir(), 'FABRICA-e2e-orchestration-worker-'))
...
appendLedger('FABRICA_E2E_SPAWN_LEDGER', { event: 'spawn', startedAt: Date.now() })
...
test('worker-start preserves one live inactive worker across workspace re-entry', async ({
  ...
  const coordinatorTabId = await getActiveTabId(fabricaPage)
  ...
  const workerTab = fabricaPage.locator(
  ...
  await expect(
    fabricaPage.locator(`[data-testid="sortable-tab"][data-tab-id="${coordinatorTabId}"]`)
  ...
  const otherWorktreeId = await switchToOtherWorktree(fabricaPage, worktreeId)
  ...
  await expect(workerTab).toBeVisible()
  await expect(
    fabricaPage.locator(`[data-testid="sortable-tab"][data-tab-id="${workerTerminal!.tabId}"]`)
  ).toHaveCount(1)
  await expect(
    fabricaPage.locator(`[data-testid="sortable-tab"][data-tab-title="${workerTabTitle}"]`)
  ).toHaveCount(1)
  ...
  await expect(fabricaPage.locator('body')).not.toContainText('Conversation interrupted')
})
