...
const scratch = mkdtempSync(path.join(os.tmpdir(), 'FABRICA-paired-host-restart-background-'))
...
{}, testInfo) => {
  ...
  const previousParkDelay = process.env.FABRICA_E2E_TERMINAL_PARKING_DELAY_MS
  process.env.FABRICA_E2E_TERMINAL_PARKING_DELAY_MS = String(PARK_DELAY_MS)
  const session = createRestartSession(testInfo, {
    FABRICA_DAEMON_STREAM_BACKLOG_FILE: backlogPath
  })
  ...
  try {
    ...
    if (previousParkDelay === undefined) {
      delete process.env.FABRICA_E2E_TERMINAL_PARKING_DELAY_MS
    } else {
      process.env.FABRICA_E2E_TERMINAL_PARKING_DELAY_MS = previousParkDelay
    }
  }
})
