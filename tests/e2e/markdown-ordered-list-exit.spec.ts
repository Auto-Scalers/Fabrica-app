...
test.describe('Markdown ordered-list exit regression', () => {
  ...
  for (const row of rows) {
    test(row.name, async ({ fabricaPage }, testInfo) => {
      ...
      try {
        ...
        const activeFile = await openMarkdownFixture(fabricaPage, context, filePath)
        const draftKey = activeFile.filePath

        await row.run(fabricaPage, row.sentinel)

        await expectSentinelParagraphOutsideOrderedList(fabricaPage, row.sentinel)
        await expectSerializedDraftOutsideOrderedList(fabricaPage, draftKey, row.sentinel)
      } finally {
