...
([#8512](https://github.com/Auto-Scalers/Fabrica-app/pull/8512)) merged; PR B not yet
implemented
...
Reduce the recurring engineering and human work required to localize Fabrica
...
Use gettext PO as Fabrica's canonical bilingual translation source, under a
constrained profile:

- `msgctxt` — the stable Fabrica message ID;
...
dependency. Fabrica must not require network access or a translation service to
...
Fabrica will support only the subset needed for its message model. PR B must
define and validate that subset rather than accepting arbitrary PO documents.

Each supported entry must carry:

- one stable Fabrica message ID (`msgctxt`), unique within the file;
...
Plurals use Fabrica's existing per-key convention: each i18next plural-suffixed
...
- XML is the format most likely to let a hand-edited community PR fail on a
  ...
  external editors produce whole-file diffs; Fabrica's translation contributions
  arrive overwhelmingly as direct catalog PRs.

If a translation provider requires XLIFF, Fabrica should add a deterministic
...
translator context, or review state. Meeting Fabrica's requirements would require
...
own platform but are not a suitable authority for Fabrica's macOS, Linux,
...
not become Fabrica's sole source of truth or a runtime dependency. Selecting one
...
to establish a format decision. Fabrica therefore chooses based on its own
