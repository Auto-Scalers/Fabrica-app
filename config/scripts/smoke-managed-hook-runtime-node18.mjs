...
const home = await mkdtemp(join(tmpdir(), 'fabrica-managed-hook-node18-'))
...
try {
  ...
  assert.match(codexHooks, /\.fabrica\/agent-hooks\/codex-hook\.sh/)
  assert.match(claudeSettings, /\.fabrica\/agent-hooks\/claude-hook\.sh/)
  await access(join(home, '.fabrica', 'agent-hooks', 'codex-hook.sh'), constants.X_OK)
  await access(join(home, '.fabrica', 'agent-hooks', 'claude-hook.sh'), constants.X_OK)
} finally {
