...
function readAppDirArg(argv) {
  ...
  if (process.platform === 'darwin') {
    return 'dist/mac-arm64/Fabrica.app'
  }
...
function getPackagedCliPath(appDir) {
  if (process.platform === 'darwin' || appDir.endsWith('.app')) {
    return join(appDir, 'Contents', 'Resources', 'bin', 'fabrica')
  }
  if (process.platform === 'win32') {
    return join(appDir, 'resources', 'bin', 'fabrica.exe')
  }
  return join(appDir, 'resources', 'bin', 'fabrica')
}

const appDir = resolve(readAppDirArg(process.argv.slice(2)))
const tempRoot = await mkdtemp(join(tmpdir(), 'fabrica-packaged-cli-smoke-'))
...
try {
  ...
  assert(list.topics.some((topic) => topic.name === 'fabrica-cli'))
  assert.match((await run(['skills', 'get', 'fabrica-cli'])).stdout, /name: fabrica-cli/)
  ...
  const update = JSON.parse(
    (await run(['skills', 'update', '--skill', 'fabrica-cli', '--dry-run', '--json'])).stdout
  )
...
// Why: on Windows the launcher above spawns the copied Fabrica.exe (and its crashpad/utility children)
