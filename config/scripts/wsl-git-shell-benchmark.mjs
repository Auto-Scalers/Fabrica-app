...
async function main() {
  ...
  const loginPath = /__FABRICA_PATH__(.*)/.exec(probeText)?.[1]?.trim()
  const gitPath = /__FABRICA_GIT__(.*)/.exec(probeText)?.[1]?.trim()
  const loginHome = /__FABRICA_HOME__(.*)/.exec(probeText)?.[1]?.trim()
  ...
  const outputMarker = `__FABRICA_GIT_OUTPUT_${process.pid}__\n`
  ...
  const benchmarkRepo = async (repo) => {
    const branch = (
      ...
      .toString('utf8')
      .trim()
    const fixtureName = `.fabrica-wsl-git-shell-benchmark-${process.pid}-${randomUUID()}.txt`
    ...
    const prepareStage = async () => {
      ...
      await run(
        ...
        wslArgs(distro, [
          ...
          '-c',
          'printf "benchmark\\n" > "$1"',
          'fabrica-wsl-git-shell-benchmark',
          fixturePath
        ])
      )
    }
  ...
  const result = {
    ...
    loginProbePreambleBytes: Buffer.byteLength(probeText.split('__FABRICA_PATH__', 1)[0]),
    guestProcessShape: {
