...
export function getRequiredReleaseAssetNames(tag) {
  ...
  return [
    ...
    `fabrica_${version}_amd64.deb`,
    `fabrica_${version}_arm64.deb`,
    `fabrica-${version}.x86_64.rpm`,
    `fabrica-${version}.aarch64.rpm`,
    'fabrica-windows-setup.exe',
    'fabrica-windows-setup.exe.blockmap',
    `Fabrica-${version}-mac.zip`,
    `Fabrica-${version}-mac.zip.blockmap`,
    `Fabrica-${version}-arm64-mac.zip`,
    `Fabrica-${version}-arm64-mac.zip.blockmap`,
    'fabrica-macos-x64.dmg',
    'fabrica-macos-x64.dmg.blockmap',
    'fabrica-macos-arm64.dmg',
    'fabrica-macos-arm64.dmg.blockmap'
  ]
}
...
async function main() {
  ...
  const repo = process.env.GITHUB_REPOSITORY || 'Auto-Scalers/Fabrica-app'
