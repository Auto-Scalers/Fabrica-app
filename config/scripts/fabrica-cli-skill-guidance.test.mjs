...
// Why: fabrica-cli now ships a hybrid discovery stub, so its version-sensitive command
...
const guidePath = join(projectDir, 'skill-guides', 'fabrica-cli.md')
...
describe('fabrica CLI skill guidance', () => {
  it('keeps independent worktree lineage separate from Git base selection', () => {
    const skill = readSkill()

    expect(skill).toContain('`--no-parent` only controls Fabrica lineage')
    expect(skill).toContain('omit `--base-branch` so Fabrica uses the repo default base')
  ...
  it('keeps browser injection guidance narrow and avoids literal secret examples', () => {
    ...
    expect(skill).toContain('`fabrica eval` expressions, or `fabrica exec` commands')
...
describe('fabrica CLI install stub', () => {
  ...
  it('does not mistake resolution or execution failures for an older binary', () => {
    ...
    // Falling through can silently pair a version-matched guide with the wrong Fabrica build.
  ...
  it('drops the changing command reference from the installable file', () => {
    ...
    expect(stub).not.toContain('FABRICA automations create')
