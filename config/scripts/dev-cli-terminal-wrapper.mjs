...
export function prepareDevCliTerminalWrappers({
  ...
  if (platform === 'win32') {
    const wrapperContent = `@echo off\r\nset "FABRICA_USER_DATA_PATH=${escapeWindowsBatchValue(userDataPath)}"\r\nset "FABRICA_DEV_CLI_INVOCATION=1"\r\nset "FABRICA_APP_EXECUTABLE=${escapeWindowsBatchValue(electronExecutable)}"\r\nset "FABRICA_APP_EXECUTABLE_NEEDS_APP_ROOT=1"\r\nnode "${escapeWindowsBatchValue(cliPath)}" %*\r\n`
    for (const targetDir of [binDir, userDataBinDir]) {
      for (const commandName of ['fabrica-dev.cmd', 'fabrica.cmd', 'fabrica-dev.cmd', 'fabrica.cmd']) {
        writeFileSync(path.join(targetDir, commandName), wrapperContent, 'utf8')
      }
    }
  } else {
    const wrapperContent = `#!/usr/bin/env bash\nexport FABRICA_USER_DATA_PATH=${JSON.stringify(userDataPath)}\nexport FABRICA_DEV_CLI_INVOCATION=1\nexport FABRICA_APP_EXECUTABLE=${JSON.stringify(electronExecutable)}\nexport FABRICA_APP_EXECUTABLE_NEEDS_APP_ROOT=1\nexec node ${JSON.stringify(cliPath)} "$@"\n`
    for (const targetDir of [binDir, userDataBinDir]) {
      for (const commandName of ['fabrica-dev', 'fabrica', 'fabrica-dev', 'fabrica']) {
