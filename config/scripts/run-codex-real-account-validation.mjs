...
const RESTRICTED_ENV_KEYS = [
  ...
  'CODEX_HOME',
  'FABRICA_CODEX_HOME',
  'FABRICA_E2E_HOME_DIR',
  'FABRICA_E2E_USER_DATA_DIR',
  'FABRICA_USER_DATA_PATH',
  'ZDOTDIR',
  'FABRICA_ORIG_ZDOTDIR',
  'BASH_ENV',
  'ENV',
...
export function createValidationEnv(inheritedEnv, layout) {
  ...
  return {
    ...
    FABRICA_E2E_HOME_DIR: layout.homeDir,
    FABRICA_E2E_USER_DATA_DIR: layout.userDataDir,
    FABRICA_USER_DATA_PATH: layout.userDataDir
  }
}
  ...
  if (samePath(tempParentReal, primaryHomeReal) || isWithin(tempParentReal, primaryHomeReal)) {
    throw new Error(
      `Refusing to place the disposable validation root inside the primary home (${primaryHome}). ` +
        'Pass --temp-parent <dir> or set FABRICA_CODEX_VALIDATION_TEMP_PARENT to a directory outside it.'
    )
  }
  const tempRoot = await mkdtemp(path.join(tempParent, 'fabrica-codex-real-'))
...
async function seedCompletedProfile(layout) {
  ...
  await writeFile(
    path.join(layout.userDataDir, 'fabrica-data.json'),
...
function validationCliCommand() {
  if (process.env.FABRICA_VALIDATION_CLI) {
    return process.env.FABRICA_VALIDATION_CLI
  }
  if (process.env.FABRICA_CLI_COMMAND) {
    return process.env.FABRICA_CLI_COMMAND
  }
  return 'fabrica'
}

async function probeTerminalEnvironment(terminalHandle, launchEnv) {
  const marker = `__FABRICA_CODEX_VALIDATION_${randomUUID()}__`
  const command = [
    'node -e',
    `"console.log('${marker}:' + JSON.stringify({home: require('node:os').homedir(), codexHome: process.env.CODEX_HOME || null, fabricaCodexHome: process.env.FABRICA_CODEX_HOME || null}))"`
  ].join(' ')
...
async function main() {
  ...
  const reportPath =
    options.reportPath ??
    path.join(os.tmpdir(), `fabrica-codex-real-account-${options.scenario}-${Date.now()}.json`)
