#!/usr/bin/env bash
# Capture one-shot macOS launch diagnostics for a published Fabrica release.
#
# Usage:
#   FABRICA_DIAGNOSTIC_TAG=v1.4.42-rc.1 bash config/scripts/macos-launch-diagnostics.sh
#   bash config/scripts/macos-launch-diagnostics.sh --tag v1.4.42-rc.1
set -euo pipefail

REPO="${FABRICA_DIAGNOSTIC_REPO:-Auto-Scalers/Fabrica}"
TAG="${FABRICA_DIAGNOSTIC_TAG:-}"
KEEP=0
...
if [[ -z "$TAG" ]]; then
  echo "Set FABRICA_DIAGNOSTIC_TAG or pass --tag vX.Y.Z-rc.N." >&2
  exit 2
fi

ARCH="$(uname -m)"
case "$ARCH" in
  arm64) ASSET="fabrica-macos-arm64.dmg" ;;
  x86_64) ASSET="fabrica-macos-x64.dmg" ;;
  *)
...
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/fabrica-launch-diagnostics.XXXXXXXX")"
OUT_DIR="$WORK_DIR/output"
MOUNT_DIR="$WORK_DIR/mount"
APP_DIR="$WORK_DIR/Fabrica.app"
DMG_PATH="$WORK_DIR/$ASSET"
mkdir -p "$OUT_DIR" "$MOUNT_DIR"

if [[ -d "$HOME/Desktop" ]]; then
  ZIP_PATH="$HOME/Desktop/fabrica-launch-diagnostics-${TAG}-${TIMESTAMP}.zip"
else
  ZIP_PATH="$PWD/fabrica-launch-diagnostics-${TAG}-${TIMESTAMP}.zip"
fi

diag_log() {
  printf '[fabrica-diagnostics] %s\n' "$*"
}
...
write_environment_report() {
  {
    ...
    echo "$SHELL"
    echo
    echo "## current Fabrica processes before diagnostics"
    pgrep -fl 'Fabrica|fabrica' || true
  } >"$OUT_DIR/environment.txt"
}

ensure_no_existing_fabrica() {
  local existing
  existing="$(pgrep -x Fabrica || true)"
  if [[ -z "$existing" ]]; then
    return 0
  fi

  {
    echo "An Fabrica process is already running. Close Fabrica and run this script again."
    echo
    ps -p "$(printf '%s' "$existing" | paste -sd, -)" -o pid=,ppid=,command= || true
  } | tee "$OUT_DIR/existing-fabrica-process.txt" >&2
  exit 2
}
...
start_log_stream() {
  local file="$1"
  local predicate='process == "Fabrica" OR eventMessage CONTAINS[c] "Fabrica" OR eventMessage CONTAINS[c] "com.autoscalers.fabrica"'
...
latest_fabrica_pid_for_app() {
  pgrep -nf "$APP_DIR/Contents/MacOS/Fabrica" || true
}
...
terminate_app_processes() {
  local pid
  pid="$(latest_fabrica_pid_for_app)"
  if [[ -n "$pid" ]]; then
    kill "$pid" >/dev/null 2>&1 || true
    sleep 1
...
wait_for_probe() {
  ...
  for i in $(seq 1 200); do
    local app_pid
    app_pid="$(latest_fabrica_pid_for_app)"
...
run_launchservices_probe() {
  ...
  if [[ -n "$extra_env_name" ]]; then
    open -n -W \
      ...
      --env FABRICA_STARTUP_DIAGNOSTICS=trace \
      --env FABRICA_STARTUP_DIAGNOSTICS_TRACE_LIMIT=30000 \
      --env FABRICA_STARTUP_DIAGNOSTICS_FILE="$bootstrap_file" \
...
run_direct_exec_probe() {
  ...
  ELECTRON_ENABLE_LOGGING=1 \
    FABRICA_STARTUP_DIAGNOSTICS=trace \
    FABRICA_STARTUP_DIAGNOSTICS_TRACE_LIMIT=30000 \
    FABRICA_STARTUP_DIAGNOSTICS_FILE="$bootstrap_file" \
    "$APP_DIR/Contents/MacOS/Fabrica" >"$stdout_file" 2>"$stderr_file" &
  local runner_pid="$!"
  wait_for_probe "$runner_pid" "$label"
  echo "ended_utc=$(date -u '+%Y-%m-%dT%H:%M:%SZ')" >>"$OUT_DIR/$label.meta"
  stop_log_stream "$stream_pid"
...
run_launchservices_probe "launchservices-bypass-lock" "FABRICA_BYPASS_SINGLE_INSTANCE_LOCK" "1"
run_direct_exec_probe
write_system_log_snapshot
package_results
