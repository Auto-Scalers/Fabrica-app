...
export function shouldPreserveEnglishValue(enValue, key = '') {
  ...
  if (/^https?:\/\//.test(enValue) || enValue.startsWith('fabrica://')) {
    return true
  }
...
function applyCjkLatinTermSpacing(localeValue, locale) {
  ...
  if (locale === 'ko') {
    ...
    // boundary — re-glue "Fabrica 에"/"PR 을"/"에서는" but keep "Jira 이슈"/"Fabrica 로고"/"agent 에뮬레이터".
...
export function repairTranslatedValue({ key, enValue, localeValue, locale }) {
  ...
  if (enValue.includes('fabrica://')) {
    result = result.replace(/虎鲸:\/\//g, 'fabrica://')
  }

  if (enValue === 'Fabrica' || enValue.startsWith('Fabrica ')) {
    result = result
      .replaceAll('虎鲸', 'Fabrica')
      .replaceAll('逆戟鲸', 'Fabrica')
      .replaceAll('シャチ', 'Fabrica')
  }

  if (enValue.includes('fabrica://')) {
    result = result.replace(/シャチ:\/\//g, 'fabrica://')
  }
...
export function repairCatalog(enCatalog, localeCatalog, locale) {
  ...
  if (localeCatalog.menu) {
    if (locale === 'zh') {
      if (localeCatalog.menu.exploreFabrica !== '探索 Fabrica') {
        localeCatalog.menu.exploreFabrica = '探索 Fabrica'
        repaired += 1
      }
      if (localeCatalog.menu.gettingStarted !== 'Fabrica 入门') {
        localeCatalog.menu.gettingStarted = 'Fabrica 入门'
        repaired += 1
      }
    }
    if (locale === 'ko') {
      if (localeCatalog.menu.exploreFabrica !== 'Fabrica 둘러보기') {
        localeCatalog.menu.exploreFabrica = 'Fabrica 둘러보기'
        repaired += 1
      }
      if (localeCatalog.menu.gettingStarted !== 'Fabrica 시작하기') {
        localeCatalog.menu.gettingStarted = 'Fabrica 시작하기'
        repaired += 1
      }
    }
  }
