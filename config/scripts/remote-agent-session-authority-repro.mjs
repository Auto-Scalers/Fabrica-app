...
const agentSessionToken = '--fabrica-repro-agent-session'
...
try {
  ...
  execFileSync(
    ...
    [
      ...
      projectPath,
      '-c',
      'user.name=Fabrica Repro',
      '-c',
      'user.email=fabrica-repro@example.invalid',
      'commit',
      '--allow-empty',
      '-m',
  ...
  writeFileSync(
    path.join(profilePath, 'fabrica-data.json'),
    JSON.stringify({
...
async function startServer(port) {
  ...
  const env = {
    ...
    FABRICA_DEV_USER_DATA_PATH: profilePath,
    FABRICA_USER_DATA_PATH: profilePath,
    FABRICA_REPRO_SPAWN_MARKER: spawnMarkerPath,
    FABRICA_REPRO_EXIT_TRIGGER: exitTriggerPath,
    FABRICA_REPRO_INPUT_MARKER: inputMarkerPath,
    FABRICA_REPRO_AGENT_SESSION_TOKEN: agentSessionToken,
  ...
  return await new Promise((resolve, reject) => {
    ...
    lines.on('line', (line) => {
      try {
        ...
        if (parsed.type === 'fabrica_server_ready' && parsed.pairing?.url) {
