...
const MUTABLE_BUILD_ENV = [
  ...
  'FABRICA_MAC_ADHOC',
  'FABRICA_MAC_RELEASE',
  'FABRICA_HOURLY_BUILD_VERSION',
  'FABRICA_DAILY_BUILD_VERSION',
  'FABRICA_ADHOC_BUILD_VERSION',
  'FABRICA_LOCAL_BUILD_VERSION'
]
...
const withHourlyEnv = (assert) => withEnv({ FABRICA_MAC_HOURLY: '1' }, assert)
const withDailyEnv = (assert) => withEnv({ FABRICA_MAC_DAILY: '1' }, assert)
const withAdhocEnv = (assert) => withEnv({ FABRICA_MAC_ADHOC: '1' }, assert)
...
describe('electron-builder mac channel config', () => {
  ...
  // (com.autoscalers.fabrica.local, ad-hoc) identity would be un-installable over a real
  // Fabrica — the whole point of the channel.
  it('builds hourly artifacts with the release signing identity', () => {
    withHourlyEnv((config) => {
      expect(config.mac.appId).toBeUndefined()
      expect(config.appId).toBe('com.autoscalers.fabrica')
  ...
  it('notarizes hourly builds like releases, and neither locally', () => {
    ...
    withEnv({ FABRICA_MAC_RELEASE: '1' }, (config) => {
      expect(config.mac.notarize).toBe(true)
    })
  ...
  it('publishes hourly builds to the separate hourly repo', () => {
    withHourlyEnv((config) => {
      expect(config.publish).toMatchObject({ repo: 'fabrica-hourly', releaseType: 'prerelease' })
    })
  ...
  it('stamps hourly packages with the hourly version', () => {
    withEnv(
      { FABRICA_MAC_HOURLY: '1', FABRICA_HOURLY_BUILD_VERSION: '1.4.160-hourly.202607281400' },
      (config) => {
  ...
  // real Fabrica through the same updater path, so the same signing and the same TCC
  ...
  it('builds adhoc artifacts with the release identity and its own repo', () => {
    withAdhocEnv((config) => {
      ...
      expect(config.mac.notarize).toBe(true)
      expect(config.forceCodeSigning).toBe(true)
      expect(config.publish).toMatchObject({ repo: 'fabrica-adhoc', releaseType: 'prerelease' })
    })
  })

  it('stamps adhoc packages with the adhoc version', () => {
    withEnv(
      { FABRICA_MAC_ADHOC: '1', FABRICA_ADHOC_BUILD_VERSION: '1.4.160-adhoc.20260728140533' },
      (config) => {
  ...
  it('builds daily artifacts with the release identity and its own repo', () => {
    withDailyEnv((config) => {
      ...
      expect(config.mac.notarize).toBe(true)
      expect(config.forceCodeSigning).toBe(true)
      expect(config.publish).toMatchObject({ repo: 'fabrica-daily', releaseType: 'prerelease' })
    })
  })

  it('stamps daily packages with the daily version', () => {
    withEnv(
      { FABRICA_MAC_DAILY: '1', FABRICA_DAILY_BUILD_VERSION: '1.4.160-daily.202607281300' },
      (config) => {
  ...
  // repos — a branch or daily build landing in fabrica-hourly would be offered to
