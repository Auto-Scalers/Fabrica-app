...
function withoutFabricaEnvironment(extra = {}) {
  return {
    ...Object.fromEntries(Object.entries(process.env).filter(([key]) => !key.startsWith('FABRICA_'))),
    ...extra
  }
}
...
async function verifyNoOpWrites(scripts, home, payload) {
  const commandCodeBin = mkdtempSync(join(tmpdir(), 'fabrica-hook-command-code-bin-'))
  ...
  try {
    ...
    const result = await runShell(
      ...
      withoutFabricaEnvironment({
        ...
        FABRICA_AGENT_HOOK_TOKEN: 'electron-verification-token',
        FABRICA_PANE_KEY: 'electron-verification-pane'
      })
    )
    ...
    for (const script of scripts) {
      ...
      const result = await runShell(
        ...
        withoutFabricaEnvironment({
          ...
          FABRICA_AGENT_HOOK_TOKEN: 'electron-verification-token',
          FABRICA_PANE_KEY: 'electron-verification-pane',
          FABRICA_TAB_ID: 'electron-verification-tab',
          FABRICA_WORKTREE_ID: 'electron-verification-worktree',
          FABRICA_AGENT_HOOK_ENV: 'test',
          FABRICA_AGENT_HOOK_VERSION: '1',
          FABRICA_ANTIGRAVITY_EVENT: 'PostInvocation',
          FABRICA_COPILOT_HOOK_EVENT: 'PostToolUse'
        })
      )
      ...
      if (request.headers['x-fabrica-agent-hook-token'] !== 'electron-verification-token') {
...
async function verifyInstalledLauncher(home, payload) {
  ...
  const scratch = mkdtempSync(join(tmpdir(), 'fabrica-hook-launcher-'))
  try {
    ...
    const missingResult = await runShell(
      ...
      payload,
      withoutFabricaEnvironment({ HOME: home })
    )
