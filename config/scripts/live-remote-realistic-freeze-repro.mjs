 ...
 *   restart-proxy                — idle, then fabrica open + status/list storm + open
 *                                  (does NOT kill the desktop; proxies restore work)
 *
 * Usage:
 *   FABRICA_FREEZE_ENV=paired-remote FABRICA_FREEZE_SCENARIO=idle-backlog-open \
...
import { createFabricaRpc } from './live-remote-freeze-rpc.mjs'
...
const envName = process.env.FABRICA_FREEZE_ENV || 'paired-remote'
const scenario = process.env.FABRICA_FREEZE_SCENARIO || 'idle-backlog-open'
const createCount = Math.max(0, readFreezeNumberEnv('FABRICA_FREEZE_CREATE', 0))
const openCount = Math.max(2, readFreezeNumberEnv('FABRICA_FREEZE_OPEN_COUNT', 20))
const idleMs = Math.max(0, readFreezeNumberEnv('FABRICA_FREEZE_IDLE_MS', 45_000))
const paceMs = Math.max(0, readFreezeNumberEnv('FABRICA_FREEZE_PACE_MS', 250))
const paceJitterMs = Math.max(0, readFreezeNumberEnv('FABRICA_FREEZE_PACE_JITTER_MS', 150))
const createWorktreeSpan = Math.max(1, readFreezeNumberEnv('FABRICA_FREEZE_CREATE_WT_SPAN', 12))
const softMs = readFreezeNumberEnv('FABRICA_FREEZE_SOFT_MS', DEFAULT_SOFT_MS)
const hardMs = readFreezeNumberEnv('FABRICA_FREEZE_HARD_MS', DEFAULT_HARD_MS)
/** Concurrent opens during lockup-storm (wake refresh overlaps fan-out). */
const stormParallel = Math.max(1, readFreezeNumberEnv('FABRICA_FREEZE_STORM_PARALLEL', 16))
/** Kill a switch if it exceeds this — counts toward permanent lockup. */
const opTimeoutMs = Math.max(10_000, readFreezeNumberEnv('FABRICA_FREEZE_OP_TIMEOUT_MS', 60_000))
const permanentTimeoutMs = Math.max(15_000, readFreezeNumberEnv('FABRICA_FREEZE_PERMANENT_MS', 60_000))
const foreverWindowMs = Math.max(
  10_000,
  readFreezeNumberEnv('FABRICA_FREEZE_FOREVER_WINDOW_MS', DEFAULT_FOREVER_WINDOW_MS)
)
const statusSlowMs = Math.max(
  5_000,
  readFreezeNumberEnv('FABRICA_FREEZE_STATUS_SLOW_MS', DEFAULT_STATUS_SLOW_MS)
)
const watchdogIntervalMs = Math.max(
  500,
  readFreezeNumberEnv('FABRICA_FREEZE_WATCHDOG_INTERVAL_MS', 1500)
)
const scratchDir = process.env.FABRICA_FREEZE_SCRATCH || ''

function sleep(ms) {
  return new Promise((resolve) => setTimeout(resolve, ms))
}

const rpc = createFabricaRpc({ envName })
const { fabricaJsonSync, fabricaJsonAsync, runReconnectRefreshStorm, runRestartProxy } = rpc
...
function sampleFabricaIfPossible() {
  if (process.platform !== 'darwin') {
    return null
  }
  try {
    const status = fabricaJsonSync(['status'], { local: true }).result
    const pid = status?.app?.pid
    if (!pid) {
      return null
    }
    const out = path.join(reportDir, `fabrica-sample-realistic-${Date.now()}.txt`)
...
function listLiveTerminalHandles() {
  const listed = fabricaJsonSync(['terminal', 'list'])
...
async function main() {
  if (!REALISTIC_SCENARIOS.includes(scenario)) {
    throw new Error(
      `Unknown FABRICA_FREEZE_SCENARIO=${scenario}. Expected one of: ${REALISTIC_SCENARIOS.join(', ')}`
    )
  }
  ...
  const local = fabricaJsonSync(['status'], { local: true })
  const remote = fabricaJsonSync(['status'])
  notes.push(
  ...
  const worktrees = fabricaJsonSync(['worktree', 'list']).result
  ...
  if (createCount > 0) {
    ...
    await mapPool(
      ...
      async (i) => {
        ...
        try {
          const createdTerm = await fabricaJsonAsync(
            [
              'terminal',
              'create',
              '--worktree',
  ...
  try {
    ...
    const parked = await fabricaJsonAsync(['terminal', 'switch', '--terminal', parkHandle], {
      timeoutMs: 60_000
    })
  ...
  if (scenario === 'idle-backlog-reconnect-open' || scenario === 'lockup-storm') {
    ...
    console.log('[realistic-freeze] restart proxy: fabrica open + refresh storm (no process kill)')
    ...
    for (let offset = 0; offset < openList.length; offset += stormParallel) {
      ...
      const batchResults = await Promise.all(
        batch.map(async (handle, batchIndex) => {
          ...
          try {
            const sw = await fabricaJsonAsync(['terminal', 'switch', '--terminal', handle], {
  ...
  try {
    const statusProbe = await fabricaJsonAsync(['status'], {
      local: true,
  ...
  try {
    const mem = await fabricaJsonAsync(['diagnostics', 'memory'], {
      local: true,
      timeoutMs: permanentTimeoutMs
    })
  ...
  if (signals.softFreeze || signals.hardFreeze || fullApp.foreverUiLockupObserved) {
    samplePath = sampleFabricaIfPossible()
    if (samplePath) {
