...
const ITERATIONS = Number(process.env.FABRICA_HYDRATE_BENCH_ITERATIONS ?? '60')
const WARMUP = Number(process.env.FABRICA_HYDRATE_BENCH_WARMUP ?? '10')
const ROUNDS = 6

for (const [name, value] of [
  ['FABRICA_HYDRATE_BENCH_ITERATIONS', ITERATIONS],
  ['FABRICA_HYDRATE_BENCH_WARMUP', WARMUP]
]) {
...
// Row shapes are synthetic, sized against a real fabrica-data.json on a heavy machine
