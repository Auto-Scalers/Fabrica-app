...
function run(entry, argv, env) {
  ...
  if (result.error) {
    if (result.error.code === 'ETIMEDOUT') {
      throw new Error(
        `fabrica ${argv.join(' ')} did not exit within ${RUN_TIMEOUT_MS} ms — it reached a blocking command`
      )
    }
    throw result.error
  }
...
function buildCases(isolatedUserData) {
  const isolated = { FABRICA_USER_DATA_PATH: isolatedUserData }
  const cases = [
    ...
    [['no-such-command'], {}],
    [['no-such-command'], { FABRICA_PAIRING_CODE: 'garbage' }],
    [['wrktree', 'list'], {}],
    [['agent-context'], {}],
    [['agent-context', '--json'], {}],
    ...
    [['status', '--pairing-code', 'legacy://pair?code=zzzz'], isolated],
    ...
    [['status', '--json'], { ...isolated, FABRICA_PAIRING_CODE: 'not-a-pairing-code' }],
    [['status', '--json'], { ...isolated, FABRICA_REMOTE_PAIRING: 'not-a-pairing-code' }],
    [['status', '--json'], { ...isolated, FABRICA_ENVIRONMENT: 'no-such-environment' }],
    ...
    // FABRICA_USER_DATA_PATH. They are deliberately NOT invoked here. The
    ...
    [['environment', 'list', '--json'], { ...isolated, FABRICA_ENVIRONMENT: 'no-such-environment' }],
    [['environment', 'list', '--json'], { ...isolated, FABRICA_PAIRING_CODE: 'not-a-pairing-code' }],
    [['agent-context', '--json'], { ...isolated, FABRICA_PAIRING_CODE: 'not-a-pairing-code' }],
...
//
//   1. FOREGROUND — `fabrica serve` runs Fabrica until Ctrl+C and `fabrica open` /
...
//   2. MUTATING — writes outside FABRICA_USER_DATA_PATH (`agent hooks off` parks
...
const UNSAFE_TOKENS = new Map([
  ...
  ...MUTATING_TOKENS.map((token) => [token, 'can write outside FABRICA_USER_DATA_PATH'])
])
