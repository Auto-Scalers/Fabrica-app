...
if (!appImageArg) {
  fail('Usage: run-headless-linux-pairing-docker.mjs --appimage /path/to/fabrica.AppImage')
}
...
const artifactVolume = `fabrica-headless-pairing-artifact-${suffix}`
const network = `fabrica-headless-pairing-${suffix}`
const containers = new Set()
const images = [
  {
    name: 'ubuntu-24.04',
    tag: 'fabrica-headless-pairing:ubuntu-24.04',
    base: 'ubuntu@sha256:4fbb8e6a8395de5a7550b33509421a2bafbc0aab6c06ba2cef9ebffbc7092d90',
    libasound: 'libasound2t64'
  },
  {
    name: 'debian-13',
    tag: 'fabrica-headless-pairing:debian-13',
    base: 'debian@sha256:020c0d20b9880058cbe785a9db107156c3c75c2ac944a6aa7ab59f2add76a7bd',
    libasound: 'libasound2t64'
  },
  {
    name: 'ubuntu-22.04-baseline',
    tag: 'fabrica-headless-pairing:ubuntu-22.04',
...
function extractAppImage(image) {
  ...
  docker([
    ...
    'bash',
    '-v',
    `${appImage}:/input/fabrica.AppImage:ro`,
    '-v',
    `${artifactVolume}:/artifacts`,
    image,
    '-lc',
    'cp /input/fabrica.AppImage /artifacts/fabrica.AppImage && chmod +x /artifacts/fabrica.AppImage && cd /artifacts && ./fabrica.AppImage --appimage-extract >/dev/null && chmod -R a+rX /artifacts/squashfs-root'
  ])
}
...
async function validateStartupMatrix() {
  ...
  const publicEntry = await startAndWait({
    ...
    mode: 'json',
    address: '127.0.0.1',
    appPath: '/artifacts/fabrica.AppImage',
...
async function validateAuthenticatedPairing() {
  const server = await startAndWait({
    ...
    launch: 'direct',
    mode: 'json',
    address: 'ws://fabrica-pairing-server:6768/runtime?route=runtime',
    port: '6768',
    networkAlias: 'fabrica-pairing-server'
  })
  ...
  assert(
    ...
    'paired server did not report its Fabrica app version'
  )
  assert(
...
async function validateUnreachableOffer() {
  const server = await startAndWait({
    ...
    launch: 'direct',
    mode: 'json',
    address: 'fabrica-pairing-server:6769',
    port: '6768',
    networkAlias: 'fabrica-pairing-server'
  })
  const payload = readyJson(server.stdout)
...
async function startAndWait({
  ...
  const name = `fabrica-pairing-${suffix}-${containers.size}`
  const args = [
    'run',
    '-d',
    '--init',
    ...
    '-e',
    'FABRICA_KEEP_RUNNING=1',
    '-e',
    'LIBGL_ALWAYS_SOFTWARE=1',
    '-e',
    `FABRICA_READY_JSON=${mode === 'json' ? '1' : '0'}`,
    '-e',
    `FABRICA_PAIRING_ADDRESS=${address}`,
    '-e',
    `FABRICA_SERVE_PORT=${port}`,
    '-e',
    `FABRICA_TEST_APPIMAGE=${appPath}`,
    ...(noPairing ? ['-e', 'FABRICA_NO_PAIRING=1'] : []),
    '-v',
...
function hasCompleteReadyContract(stdout) {
  if (
    stdout.includes('Fabrica server ready\n') &&
...
function validateReady(logs, mode, expectedHost, options = {}) {
  if (mode === 'human') {
    assert(
      (logs.match(/^Fabrica server ready$/gm) ?? []).length === 1,
...
function readyJsonObjects(logs) {
  const marker = '{"type":"fabrica_server_ready"'
  return logs
    .split(/\r?\n/)
    .map((line) => {
    ...
    .filter((value) => value?.type === 'fabrica_server_ready')
}
