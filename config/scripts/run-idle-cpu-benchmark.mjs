...
function createIdleRepo(worktreeCount) {
  const repoDir = mkdtempSync(path.join(os.tmpdir(), 'fabrica-idle-cpu-repo-'))
  ...
  writeFileSync(path.join(repoDir, 'README.md'), '# Fabrica idle CPU benchmark\n')
  writeFileSync(
  ...
  for (let i = 2; i <= worktreeCount; i += 1) {
    const worktreeDir = path.join(
      ...
      `fabrica-idle-cpu-worktree-${i}-${Date.now()}`
    )
...
async function main() {
  ...
  const userDataDir = mkdtempSync(path.join(os.tmpdir(), 'fabrica-idle-cpu-userdata-'))
  ...
  const {
    ...
    FABRICA_CODEX_HOME: _fabricaCodexHome,
    ...cleanEnv
  } = process.env
  void ELECTRON_RUN_AS_NODE
  void _codexHome
  void _fabricaCodexHome
  ...
  const app = await electron.launch({
    ...
    env: {
      ...
      FABRICA_E2E_USER_DATA_DIR: userDataDir,
      HOME: isolatedHome,
      USERPROFILE: isolatedHome,
      FABRICA_E2E_HOME_DIR: isolatedHome,
      ...(options.headful ? { FABRICA_E2E_HEADFUL: '1' } : { FABRICA_E2E_HEADLESS: '1' })
    }
  })
