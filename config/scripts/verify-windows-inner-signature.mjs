...
$signature = Get-AuthenticodeSignature -FilePath $env:FABRICA_WINDOWS_INNER_EXECUTABLE
...
export function parseExpectedSigners(value = process.env.FABRICA_WINDOWS_EXPECTED_SIGNERS) {
...
export function parseExpectedThumbprints(value = process.env.FABRICA_WINDOWS_EXPECTED_THUMBPRINTS) {
...
export function validateExecutablePath(executablePath) {
  if (typeof executablePath !== 'string' || executablePath.trim() === '') {
    throw new Error('Usage: node config/scripts/verify-windows-inner-signature.mjs <Fabrica.exe>')
  }
...
export function getPowerShellSignatureJson(executablePath, spawnSyncImpl = spawnSync) {
  ...
  const result = spawnSyncImpl(
    ...
    ],
    {
      ...
      env: {
        ...process.env,
        FABRICA_WINDOWS_INNER_EXECUTABLE: executablePath
      }
    }
  )
