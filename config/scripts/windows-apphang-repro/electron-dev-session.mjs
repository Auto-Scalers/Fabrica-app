...
export function createGpuUserDataDirectory(gpuMode) {
  const userDataDir = mkdtempSync(path.join(os.tmpdir(), `fabrica-apphang-${gpuMode}-userdata-`))
...
export async function installRendererProbe(page) {
  await page.evaluate(() => {
    if (globalThis.__fabricaApphangProbe) {
      return
    }
    const probe = {
      intervalMs: 50,
    ...
    globalThis.__fabricaApphangProbe = { probe, timer }
  })
}
      ...
      () =>
        page.evaluate(async () => {
          ...
          return {
            ...
            rendererProbe: globalThis.__fabricaApphangProbe?.probe ?? null,
