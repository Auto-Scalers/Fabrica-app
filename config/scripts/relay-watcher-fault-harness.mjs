...
async function main() {
  ...
  let relay
  try {
    tempRoot = await mkdtemp(join(tmpdir(), 'fabrica-relay-watcher-fault-'))
    ...
    const socketPath =
      process.platform === 'win32'
        ? `\\\\.\\pipe\\fabrica-relay-watcher-fault-${process.pid}-${Date.now()}`
    ...
    daemon = spawn(
      ...
      ],
      {
        ...
        env: { ...process.env, FABRICA_WATCHER_CHILD_PID_FILE: pidFile },
    ...
    const beforePtyMarker = `FABRICA_PTY_BEFORE_${Date.now()}`
    ...
    const afterPtyMarker = `FABRICA_PTY_AFTER_${Date.now()}`
    startIndex = relay.messageCount()
