...
describe('verify packaged plugin resources', () => {
  it('accepts exact launch bytes copied into a packaged resources directory', async () => {
    const resourcesDir = await mkdtemp(join(tmpdir(), 'fabrica-packaged-plugins-'))
    try {
      await cp(
  ...
  it('rejects mutated bytes in the packaged output', async () => {
    ...
    try {
      ...
      await writeFile(
        join(launchRoot, 'autoscalers.fabrica-navigation-shortcuts', 'extra.json'),
        '{"mutated":true}\n'
      )

      expect(() => verifyPackagedPluginResources(resourcesDir)).toThrow(
        'packaged bytes do not match autoscalers.fabrica-navigation-shortcuts'
      )
    } finally {
      await rm(resourcesDir, { recursive: true, force: true })
    }
  ...
  it('rejects a CRLF checkout of the launch tree', async () => {
    ...
    try {
      ...
      expect(() => verifyPackagedPluginResources(resourcesDir)).toThrow(
        /packaged bytes do not match autoscalers\./
      )
    } finally {
      await rm(resourcesDir, { recursive: true, force: true })
    }
