...
const fabricaDevScript = path.join(scriptDir, 'fabrica-dev.mjs')
const ensureNativeRuntimeScript = path.join(scriptDir, 'ensure-native-runtime.mjs')
const fixedProfileDir = process.env.FABRICA_HEADLESS_PAIRING_PROFILE_DIR
...
const profileDir =
  fixedProfileDir ?? mkdtempSync(path.join(tmpdir(), 'fabrica-headless-pairing-profile-'))
...
delete childEnv.FABRICA_CODEX_HOME
Object.assign(childEnv, {
  // Why: a fresh temporary Fabrica profile must not make the default Codex lane
  // read or mutate the developer profile during a pairing smoke test.
  FABRICA_DEV_USER_DATA_PATH: profileDir,
  HOME: isolatedHome,
...
console.error(`[headless-pairing] starting: fabrica-dev serve --json${formatForwardedArgs(serveArgs)}`)

child = spawn(process.execPath, [fabricaDevScript, 'serve', '--json', ...serveArgs], {
  cwd: repoRoot,
...
/**
 * Parses wrapper flags and forwards everything else to `fabrica serve`.
 */
 ...
 */
function printHelp() {
  console.log(`Usage: node config/scripts/serve-headless-fresh-profile-pairing.mjs [--keep] [fabrica serve flags]

Starts fabrica-dev serve --json with a fresh isolated userData profile, ensures Electron's dev runtime is usable, and prints the pairing URL.
...
Environment:
  FABRICA_HEADLESS_PAIRING_ADDRESS=<host|host:port|ws://...>  Override the auto pairing address.
  FABRICA_HEADLESS_PAIRING_PROFILE_DIR=/path/to/profile       Use a fixed profile directory.
`)
}
...
function resolveDefaultPairingAddress() {
  const configured = process.env.FABRICA_HEADLESS_PAIRING_ADDRESS?.trim()
  if (configured) {
    return configured
  }
...
function printReadyLine(line) {
  ...
  if (!payload || payload.type !== 'fabrica_server_ready') {
    return false
  }
  console.log(`Fabrica server ready: ${payload.boundEndpoint ?? 'websocket unavailable'}`)
...
function stopChild(signal) {
  ...
  if (process.platform === 'win32' && child.pid) {
    // Why: child.kill() only targets fabrica-dev on Windows; taskkill walks the
  ...
  if (process.platform !== 'win32' && child.pid) {
    // Why: fabrica-dev synchronously owns the CLI child, which owns Electron; kill
...
function cleanupProfile() {
  ...
  if (!existsSync(profileDir) || !profileDir.includes('fabrica-headless-pairing-profile-')) {
