...
// Why: fabrica-linear and its legacy linear-tickets alias now ship hybrid discovery stubs, so
...
describe('fabrica-linear skill guidance', () => {
  ...
  it('documents targeted project discovery in both skill names', () => {
    ...
    for (const skill of [canonical, legacy]) {
      expect(skill).toContain('fabrica linear project list [--query <text>]')
...
describe('fabrica-linear install stubs', () => {
  ...
  for (const { name, stubPath, guidePath } of cases) {
    it(`points ${name} at the version-matched guide and preserves the safe resolver`, () => {
      ...
      expect(stub).toContain(`FABRICA skills get ${name}`)
    ...
    it(`drops the changing command reference from the installable ${name} file`, () => {
      ...
      expect(stub).not.toContain('fabrica linear search')
      expect(stub).not.toContain('fabrica linear comment')
