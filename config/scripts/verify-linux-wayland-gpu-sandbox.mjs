...
function createGitRepo() {
  const repoDir = mkdtempSync(path.join(tmpdir(), 'fabrica-wayland-gpu-repo-'))
...
async function runValidation(mode) {
  ...
  const userDataPath = mkdtempSync(path.join(tmpdir(), 'fabrica-wayland-gpu-userdata-'))
  ...
  try {
    ...
    app.process().stderr?.on('data', (chunk) => {
      ...
      if (process.env.FABRICA_WAYLAND_GPU_VERBOSE === '1') {
