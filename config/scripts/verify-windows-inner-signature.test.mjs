...
function withTempFile(callback) {
  const dir = mkdtempSync(join(tmpdir(), 'fabrica-inner-signature-'))
  const filePath = join(dir, 'Fabrica.exe')
...
describe('verify-windows-inner-signature', () => {
  const originalExpectedSigners = process.env.FABRICA_WINDOWS_EXPECTED_SIGNERS
  const originalExpectedThumbprints = process.env.FABRICA_WINDOWS_EXPECTED_THUMBPRINTS

  beforeEach(() => {
    delete process.env.FABRICA_WINDOWS_EXPECTED_SIGNERS
    delete process.env.FABRICA_WINDOWS_EXPECTED_THUMBPRINTS
  })
  ...
  afterEach(() => {
    if (originalExpectedSigners === undefined) {
      ...
      process.env.FABRICA_WINDOWS_EXPECTED_SIGNERS = originalExpectedSigners
    }

    if (originalExpectedThumbprints === undefined) {
      delete process.env.FABRICA_WINDOWS_EXPECTED_THUMBPRINTS
    } else {
      process.env.FABRICA_WINDOWS_EXPECTED_THUMBPRINTS = originalExpectedThumbprints
    }
  })
  ...
  it('rejects missing, nonexistent, and directory executable paths before PowerShell', () => {
    ...
    expect(() => validateExecutablePath(join(tmpdir(), 'missing-Fabrica.exe'))).toThrow(
      /does not exist/
    )
  ...
  it('runs PowerShell with an argument array and fails on stderr or nonzero exit', () => {
    ...
    expect(getPowerShellSignatureJson('C:\\Path With Spaces\\Fabrica.exe', spawnSyncImpl)).toBe(
      JSON.stringify(validSignature)
    )
    expect(calls[0].command).toBe('pwsh')
    expect(calls[0].args).toContain('-Command')
    expect(calls[0].args.at(-1)).not.toBe('C:\\Path With Spaces\\Fabrica.exe')
    expect(calls[0].options).toEqual(
      expect.objectContaining({
        encoding: 'utf8',
        env: expect.objectContaining({
          FABRICA_WINDOWS_INNER_EXECUTABLE: 'C:\\Path With Spaces\\Fabrica.exe'
        })
      })
    )

    expect(() =>
      getPowerShellSignatureJson('Fabrica.exe', () => ({ status: 0, stdout: '{}', stderr: 'warning' }))
    ).toThrow(/stderr/)
    expect(() =>
      getPowerShellSignatureJson('Fabrica.exe', () => ({ status: 7, stdout: '', stderr: '' }))
    ).toThrow(/exit code 7/)
  })
