...
async function installElectronPackageBinary() {
  ...
  const tempDir = mkdtempSync(resolve(tmpdir(), 'fabrica-electron-'))
...
function getDownloadRetryDelays() {
  const configured = process.env.FABRICA_ELECTRON_PACKAGE_RETRY_DELAYS_MS
  if (!configured) {
    return [1_000, 3_000]
  }
  ...
  if (delays.some((delay) => !Number.isSafeInteger(delay) || delay < 0)) {
    throw new Error('FABRICA_ELECTRON_PACKAGE_RETRY_DELAYS_MS must contain non-negative integers')
  }
  return delays
}
...
function getExtractorCommand(zipPath, extractDir) {
  if (process.env.FABRICA_ELECTRON_PACKAGE_EXTRACTOR) {
    return {
      file: process.execPath,
      args: [process.env.FABRICA_ELECTRON_PACKAGE_EXTRACTOR, zipPath, extractDir],
      label: `node ${process.env.FABRICA_ELECTRON_PACKAGE_EXTRACTOR}`
    }
  }

  if (osPlatform() === 'win32') {
    return {
      file: process.env.FABRICA_POWERSHELL_BIN || 'powershell',
      args: [
        '-NoProfile',
        '-NonInteractive',
        '-ExecutionPolicy',
  ...
  return {
    file: process.env.FABRICA_UNZIP_BIN || 'unzip',
