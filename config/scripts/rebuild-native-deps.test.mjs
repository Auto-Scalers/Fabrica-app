...
describe('rebuild-native-deps Electron install fallback', () => {
  it('continues non-strict postinstall when Electron retry download fails', () => {
    ...
    try {
      ...
      const result = runRebuildScript(projectDir, {
        ...
        FABRICA_STRICT_ELECTRON_INSTALL: ''
      })
  ...
  it('fails strict postinstall when Electron retry download fails', () => {
    ...
    try {
      ...
      const result = runRebuildScript(projectDir, {
        npm_lifecycle_event: 'postinstall',
        FABRICA_STRICT_ELECTRON_INSTALL: '1'
      })
  ...
  it.skipIf(process.platform !== 'win32')(
    ...
    () => {
      ...
      try {
        ...
        writeFakeElectronRebuild(projectDir, { logPathEnv: 'FABRICA_REBUILD_TEST_LOG' })
        ...
        const result = runRebuildScript(projectDir, {
          FABRICA_REBUILD_TEST_LOG: rebuildLogPath,
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      ...
      try {
        ...
        expect(result.stdout).toContain("expected build/Release so Fabrica's node-pty patch is active")
...
function mkTempProject() {
  const projectDir = mkdtempSync(join(tmpdir(), 'fabrica-rebuild-native-deps-'))
...
function runRebuildScript(projectDir, extraEnv = {}, args = []) {
  ...
  for (const key of Object.keys(env)) {
    if (
      key.toLowerCase() === 'fabrica_strict_electron_install' ||
