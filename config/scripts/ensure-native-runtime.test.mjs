    ...
    try {
      ...
      const result = spawnSync(process.execPath, [scriptPath, '--runtime=node'], {
        ...
        env: envWithPrependedPath(binDir, {
          FABRICA_NATIVE_TEST_LOG: logPath,
          FABRICA_NATIVE_TEST_MARKER: markerPath
        })
      })
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      ...
      try {
        ...
        expect(result.status, result.stderr).toBe(0)
        expect(result.stderr).toContain("expected build/Release so Fabrica's node-pty patch is active")
        expect(readFileSync(logPath, 'utf8')).toContain('pnpm rebuild node-pty\n')
      } finally {
        rmSync(projectDir, { recursive: true, force: true })
      }
...
function mkTempProject() {
  const projectDir = mkdtempSync(join(tmpdir(), 'fabrica-native-runtime-'))
...
exports.loadNativeModule = function loadNativeModule(nativeName) {
  const markerExists = existsSync(process.env.FABRICA_NATIVE_TEST_MARKER)
  appendFileSync(
    process.env.FABRICA_NATIVE_TEST_LOG,
...
exports.loadNativeModule = function loadNativeModule(nativeName) {
  const rebuilt = existsSync(process.env.FABRICA_NATIVE_TEST_MARKER)
  const dir = ${JSON.stringify(nativeDir)} ??
    (rebuilt ? '../build/Release/' : '../prebuilds/' + process.platform + '-' + process.arch + '/')
  appendFileSync(process.env.FABRICA_NATIVE_TEST_LOG, \`node-pty load \${nativeName} dir=\${dir}\\n\`)
  return { dir, module: {} }
}
`
  )
...
appendFileSync(process.env.FABRICA_NATIVE_TEST_LOG, \`pnpm \${process.argv.slice(2).join(' ')}\\n\`)
writeFileSync(process.env.FABRICA_NATIVE_TEST_MARKER, 'rebuilt')
`
  )
