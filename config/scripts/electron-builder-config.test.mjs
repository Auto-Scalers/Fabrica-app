...
describe('electron-builder config', () => {
  ...
  it('keeps plugin authoring examples out of app.asar', () => {
    ...
    for (const authoringOnly of [
      ...
      'examples/plugins/hostile-panel/fabrica-plugin.json',
      'examples/plugins/hello-fabrica/main.mjs',
      'examples/plugins/hello-fabrica/fabrica-plugin.json'
    ]) {
  ...
  it('keeps runtime resources available through extraResources', () => {
    ...
    expect(electronBuilderConfig.mac.extraResources).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          from: 'native/computer-use-macos/.build/release/Fabrica Computer Use.app',
          to: 'Fabrica Computer Use.app'
        })
      ])
    )
    ...
    expect(electronBuilderConfig.win.extraResources).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({
          from: 'native/windows-cli-launcher/.build/fabrica.exe',
          to: 'bin/fabrica.exe'
        })
      ])
    )
  ...
  // resources/bin/fabrica.cmd (beside the native resources/bin/fabrica.exe). If the
  // source tree is also packed into app.asar it gets extracted by
  // asarUnpack:['resources/**'] to app.asar.unpacked/resources/win32/bin/fabrica.cmd,
  // a duplicate with no adjacent fabrica.exe that fails to launch (#7351).
  it('keeps the Windows CLI shim source tree out of app.asar', () => {
    ...
    expect(electronBuilderConfig.win.extraResources).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          from: 'resources/win32/bin/fabrica.cmd',
          to: 'bin/fabrica.cmd'
        })
      ])
    )
  ...
  it('ships the mac notification-status helper in Contents/MacOS, not Resources', () => {
    expect(electronBuilderConfig.mac.extraFiles).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          from: 'native/notification-status-macos/.build/release/fabrica-notification-status',
          to: 'MacOS/fabrica-notification-status'
        })
      ])
    )
    expect(electronBuilderConfig.mac.extraResources).not.toEqual(
      expect.arrayContaining([expect.objectContaining({ to: 'fabrica-notification-status' })])
    )
  })
  ...
  it('matches the Linux desktop entry to Electron window class', () => {
    expect(electronBuilderConfig.linux.desktop.entry.StartupWMClass).toBe('fabrica')
  })
  ...
  it('uses AppImage and deb as local Linux targets without changing existing artifact names', () => {
    ...
    expect(electronBuilderConfig.appImage.artifactName).toBe('fabrica-linux.${ext}')
    expect(electronBuilderConfig.deb.artifactName).toBe('fabrica_${version}_${arch}.${ext}')
    expect(electronBuilderConfig.rpm).toMatchObject({
      packageName: 'fabrica',
      artifactName: 'fabrica-${version}.${arch}.${ext}'
    })
  })

  it('uses a distinct AppImage name for Linux arm64 release uploads', () => {
    const configPath = require.resolve('../electron-builder.config.cjs')
    const original = process.env.FABRICA_LINUX_ARM64_RELEASE
    try {
      delete require.cache[configPath]
      process.env.FABRICA_LINUX_ARM64_RELEASE = '1'
      expect(require('../electron-builder.config.cjs').appImage.artifactName).toBe(
        'fabrica-linux-arm64.${ext}'
      )
    } finally {
      if (original === undefined) {
        delete process.env.FABRICA_LINUX_ARM64_RELEASE
      } else {
        process.env.FABRICA_LINUX_ARM64_RELEASE = original
      }
      delete require.cache[configPath]
  ...
  it('overrides packaged semver only for local macOS builds', () => {
    const configPath = require.resolve('../electron-builder.config.cjs')
    const original = process.env.FABRICA_LOCAL_BUILD_VERSION
    const originalMacRelease = process.env.FABRICA_MAC_RELEASE
    try {
      delete require.cache[configPath]
      delete process.env.FABRICA_MAC_RELEASE
      process.env.FABRICA_LOCAL_BUILD_VERSION = '1.4.159-rc.0.local.123.abc'
      ...
      if (originalMacRelease === undefined) {
        ...
        process.env.FABRICA_MAC_RELEASE = originalMacRelease
      }
      if (original === undefined) {
        delete process.env.FABRICA_LOCAL_BUILD_VERSION
      } else {
        process.env.FABRICA_LOCAL_BUILD_VERSION = original
      }
      delete require.cache[configPath]
      require('../electron-builder.config.cjs')
    }
  ...
  it('never applies local semver to release packaging', () => {
    const configPath = require.resolve('../electron-builder.config.cjs')
    const originalLocalVersion = process.env.FABRICA_LOCAL_BUILD_VERSION
    const originalMacRelease = process.env.FABRICA_MAC_RELEASE
    try {
      delete require.cache[configPath]
      process.env.FABRICA_LOCAL_BUILD_VERSION = '1.4.159-local.123.abc'
      process.env.FABRICA_MAC_RELEASE = '1'
      expect(require('../electron-builder.config.cjs').extraMetadata).toBeUndefined()
    } finally {
      if (originalLocalVersion === undefined) {
        delete process.env.FABRICA_LOCAL_BUILD_VERSION
      } else {
        process.env.FABRICA_LOCAL_BUILD_VERSION = originalLocalVersion
      }
      if (originalMacRelease === undefined) {
        delete process.env.FABRICA_MAC_RELEASE
      } else {
  ...
  it('uses Fabrica native rebuild hook instead of electron-builder default rebuild', () => {
  ...
  it('verifies packaged main runtime deps from Windows-style asar entries', async () => {
    const resourcesDir = await mkdtemp(join(tmpdir(), 'fabrica-runtime-deps-'))
    try {
  ...
  it('prunes non-target node-pty architecture outputs from packaged runtime resources', async () => {
    const resourcesDir = await mkdtemp(join(tmpdir(), 'fabrica-node-pty-prune-'))
    try {
  ...
  it('copies the Windows node-pty ConPTY runtime beside the rebuilt addon', async () => {
    for (const [arch, electronArch] of [
      ...
      const resourcesDir = await mkdtemp(join(tmpdir(), `fabrica-node-pty-conpty-${arch}-`))
      try {
        const nodePtyDir = join(resourcesDir, 'node_modules', 'node-pty')
  ...
  it('prunes non-target @parcel/watcher architecture subpackages', async () => {
    const resourcesDir = await mkdtemp(join(tmpdir(), 'fabrica-parcel-watcher-prune-'))
    try {
  ...
  it('leaves unrelated @parcel/* runtime deps untouched when pruning the watcher', async () => {
    const resourcesDir = await mkdtemp(join(tmpdir(), 'fabrica-parcel-watcher-prune-unrelated-'))
    try {
      const parcelDir = join(resourcesDir, 'node_modules', '@parcel')
      await mkdir(join(parcelDir, 'watcher'), { recursive: true })
      await mkdir(join(parcelDir, 'watcher-darwin-arm64'), { recursive: true })
  ...
  it('prunes type declaration artifacts from packaged runtime node_modules', async () => {
    const resourcesDir = await mkdtemp(join(tmpdir(), 'fabrica-runtime-type-prune-'))
    try {
  ...
  it('prunes duplicate darwin sherpa-onnx runtime dylib aliases', async () => {
    const resourcesDir = await mkdtemp(join(tmpdir(), 'fabrica-sherpa-prune-'))
    try {
  ...
  it('prunes zod TypeScript sources from packaged runtime resources', async () => {
    const resourcesDir = await mkdtemp(join(tmpdir(), 'fabrica-zod-prune-'))
    try {
  ...
  it('fails when the packaged resources directory is missing', async () => {
    const root = await mkdtemp(join(tmpdir(), 'fabrica-electron-builder-config-'))
    try {
      await expect(
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      try {
        ...
        const launcherPath = join(resourcesDir, 'bin', 'fabrica')
        ...
        await writeFile(
          ...
          [
            ...
            "if (args[1] === 'list') console.log(JSON.stringify({ topics: [{ name: 'fabrica-cli' }, { name: 'computer-use' }] }))",
