...
describe('orchestration skill guidance', () => {
  it('requires Fabrica runtime state before claiming a worker was orchestrated', () => {
    const skill = readSkill()
    const toolBoundary = getSection(skill, 'Tool Boundary')

    expect(toolBoundary).toContain('must create or bind a Run')
    expect(toolBoundary).toContain('create the Task with `fabrica orchestration task-create`')
    expect(toolBoundary).toContain('preferred `fabrica orchestration worker-start` composition')
    expect(toolBoundary).toContain('low-level `fabrica orchestration dispatch --inject` path')
    expect(toolBoundary).not.toContain('or `fabrica orchestration run`')
    expect(skill).toContain(
      '`coordinator-start`, `coordinator-stop`, `run`, and `run-stop` are retired scheduler commands'
    )
    expect(toolBoundary).toContain(
      'Do not substitute non-Fabrica subagent tools, generic agent-spawn APIs, or chat-only parallel worker features'
    )
    expect(toolBoundary).toContain('do not create Fabrica task/dispatch provenance')
    ...
    expect(toolBoundary).toContain('fabrica orchestration task-list --json')
    expect(toolBoundary).toContain('fabrica orchestration dispatch-show --task <task_id> --json')
    expect(toolBoundary).toContain(
  ...
  it('keeps full handoffs out of dispatch lifecycle and off the active branch base', () => {
    ...
    expect(skill).toContain(
      '`--no-parent` only controls Fabrica lineage; it does not choose the Git base.'
    )
    expect(skill).toContain(
      'never base it on the current feature branch unless the user explicitly asks'
    )
    expect(skill).toContain(
      'fabrica worktree create --name <task-name> --no-parent --agent codex --prompt'
    )
    expect(fullHandoffs).toContain(
      'Before creating a new worktree from an active feature branch, decide and state whether the desired Fabrica lineage is child or top-level'
    )
    expect(fullHandoffs).toContain(
  ...
  it('makes settled worker terminal release an explicit coordinator step', () => {
    ...
    expect(workerLoop).toContain(
      'fabrica orchestration worker-start --task <next_task_id> --terminal <handle> --json` so Fabrica ' +
  ...
  it('keeps agent-first launch, handle recovery, and inbox injection distinct', () => {
    ...
    expect(agentFirstExample).not.toContain('fabrica terminal list')
...
describe('orchestration install stub', () => {
  ...
  it('does not tell agents to mutate orchestration state before loading the guide', () => {
    ...
    expect(preGuide).not.toContain('fabrica orchestration task-create')
    expect(preGuide).not.toContain('fabrica orchestration dispatch')
  })
