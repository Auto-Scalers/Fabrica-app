...
describe('locale-translation-policy zh round 5', () => {
  it('fixes brand spacing, hosted review, and Fabrica Mobile regressions', () => {
    expect(
      repairTranslatedValue({
        key: 'auto.components.stats.ShareUsageCard.0eb31e79ee',
        enValue: 'Fabrica IDE',
        localeValue: 'Fabrica集成开发环境',
        locale: 'zh'
      })
    ).toBe('Fabrica IDE')
    expect(
      repairTranslatedValue({
        ...
        enValue: 'Fabrica first',
        localeValue: 'Fabrica第一',
        locale: 'zh'
      })
    ).toBe('Fabrica 优先')
    expect(
    ...
    expect(
      repairTranslatedValue({
        ...
        enValue: 'Show Fabrica Mobile Button',
        localeValue: '显示 Fabrica 移动按钮',
        locale: 'zh'
      })
    ).toBe('显示 Fabrica Mobile 按钮')
    expect(
      repairTranslatedValue({
        key: 'auto.components.settings.GitPane.e02ea23a32',
        enValue: 'Fabrica Attribution',
        localeValue: 'Fabrica归属',
        locale: 'zh'
      })
    ).toBe('Fabrica 归因')
    expect(
  ...
  it('keeps brand names English and generic workflow terms translated', () => {
    ...
    expect(
      repairTranslatedValue({
        ...
        enValue:
          'Git could not safely delete branch "{{value0}}"{{value1}}, so Fabrica kept it to avoid losing local commits.',
        localeValue:
          'Git 无法安全删除分支“{{value0}}”{{value1}}，因此 Fabrica 保留它以避免丢失本地提交。',
        locale: 'zh'
      })
    ).toBe('Git 无法安全删除分支“{{value0}}”{{value1}}，因此 Fabrica 保留它以避免丢失本地提交。')
  })
