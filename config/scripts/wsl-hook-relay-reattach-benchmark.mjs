...
function parseEndpoint(contents) {
  const port = Number(/FABRICA_AGENT_HOOK_PORT=['"]?(\d+)/.exec(contents)?.[1])
  const token = /FABRICA_AGENT_HOOK_TOKEN=['"]?([^'"\r\n]+)/.exec(contents)?.[1]
...
async function writeEndpoint(distro, path, endpoint) {
  const contents = [
    `FABRICA_AGENT_HOOK_PORT=${endpoint.port}`,
    `FABRICA_AGENT_HOOK_TOKEN=${endpoint.token}`,
    'FABRICA_AGENT_HOOK_ENV=benchmark',
...
async function invokeHook(distro, scriptPath, endpointPath) {
  ...
  const result = await run(
    ...
    wslArgs(distro, [
      ...
      `FABRICA_PANE_KEY=${PANE_KEY}`,
      'FABRICA_TAB_ID=wsl-relay-bench',
      'FABRICA_WORKTREE_ID=wsl-relay-bench',
      '/bin/sh',
      scriptPath
    ]),
...
async function main() {
  ...
  if (nodeVersion.status !== 0 || Number(nodeVersion.stdout.trim().split('.')[0]) < 18) {
    throw new Error(`WSL distro '${distro}' needs Node.js 18 or newer to run Fabrica's relay`)
  }
  ...
  let staller = null
  try {
    userDataDir = mkdtempSync(join(tmpdir(), 'fabrica-wsl-relay-bench-'))
    ...
    const benchmarkRoot = `${guestHome}/.fabrica-wsl/benchmarks/${instanceKey}`
    const scriptPath = `${benchmarkRoot}/.fabrica/agent-hooks/codex-hook.sh`
    const endpointPath = `${guestHome}/.fabrica-wsl/agent-hooks/instance-${instanceKey}/endpoint.env`
    cleanupPaths = [benchmarkRoot, `${guestHome}/.fabrica-wsl/agent-hooks/instance-${instanceKey}`]
    if (cleanupPaths.some((cleanupPath) => !cleanupPath.startsWith(`${guestHome}/.fabrica-wsl/`))) {
    ...
    const survivingSessionId = `fabrica-wsl-relay-bench-${instanceKey}`
    ...
    const createManager = async (token) => {
      ...
      return new WslHookRelayManager({
        ...
        hookCoordsEnv: () => ({
          FABRICA_AGENT_HOOK_PORT: String(preferredPort),
