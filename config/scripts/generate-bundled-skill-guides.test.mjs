...
async function createFixture() {
  const root = await mkdtemp(path.join(tmpdir(), 'fabrica-bundled-skill-guides-'))
...
describe('bundled skill guide generator', () => {
  ...
  it('keeps pre-guide fallback useful and read-only for every converted domain', async () => {
    ...
    for (const [name, commands] of Object.entries(expectedFallbackCommands)) {
      ...
      const fallback = stub.split('## If an older Fabrica does not recognize `skills get`')[1]

      expect(fallback, name).toBeDefined()
      for (const command of commands) {
        expect(fallback, name).toContain(command)
      }
      expect(fallback, name).not.toContain('FABRICA worktree ps --json')
    }
  })
  ...
  it('keeps CLI guide examples safe across shells and Linux command names', async () => {
    for (const name of ['fabrica-cli', 'computer-use', 'fabrica-emulator', 'fabrica-emulator-android']) {
      ...
      expect(source).toContain('fabrica-dev')
      expect(source).toContain('fabrica-ide')
      expect(source).toContain('PowerShell')
      expect(source).toContain('cmd.exe')
      expect(source).toMatch(/^FABRICA .+--json$/mu)
      // Why: bare command lines can launch GNOME Fabrica, while shell variables make
      // the same guide unusable from PowerShell and cmd.exe.
      expect(source).not.toMatch(/^fabrica /mu)
      expect(source).not.toMatch(/\$FABRICA(?:_|\b)/u)
    }
  })
