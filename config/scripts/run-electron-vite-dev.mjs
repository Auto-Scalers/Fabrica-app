...
const useStableElectronName =
  process.env.FABRICA_DEV_STABLE_NAME === '1' || rawForwardedArgs.includes(STABLE_NAME_FLAG)
const forwardedRaw = rawForwardedArgs.filter((arg) => arg !== STABLE_NAME_FLAG)
if (useStableElectronName) {
  process.env.FABRICA_DEV_STABLE_NAME = '1'
}
...
function createDockTitle(branch, label) {
  return `Fabrica: ${branch || label || 'dev'}`
}

function seedDevInstanceIdentityEnv() {
  const branch =
    process.env.FABRICA_DEV_BRANCH ||
    readGitValue(['symbolic-ref', '--quiet', '--short', 'HEAD']) ||
    readGitValue(['rev-parse', '--short', 'HEAD'])
  const worktreeName = process.env.FABRICA_DEV_WORKTREE_NAME || path.basename(repoRoot)
  const label = process.env.FABRICA_DEV_INSTANCE_LABEL || formatDevInstanceLabel(branch, worktreeName)
  const identitySeed = process.env.FABRICA_DEV_INSTANCE_KEY || repoRoot
  const dockTitle = process.env.FABRICA_DEV_DOCK_TITLE || createDockTitle(branch, label)

  process.env.FABRICA_DEV_REPO_ROOT ||= repoRoot
  process.env.FABRICA_DEV_INSTANCE_KEY ||= identitySeed
  if (branch) {
    process.env.FABRICA_DEV_BRANCH ||= branch
  }
  if (worktreeName) {
    process.env.FABRICA_DEV_WORKTREE_NAME ||= worktreeName
  }
  if (label) {
    ...
    process.env.FABRICA_DEV_INSTANCE_LABEL ||= label
  }
  process.env.FABRICA_DEV_DOCK_TITLE ||= dockTitle
}
...
function prepareMacDevElectronApp() {
  ...
  const title = process.env.FABRICA_DEV_DOCK_TITLE || 'Fabrica: dev'
  const identityKey = process.env.FABRICA_DEV_INSTANCE_KEY || repoRoot
  // v7: give the terminal daemon helper an Fabrica-specific TCC identity.
  ...
  const markerPath = path.join(distDir, 'fabrica-dev-electron-app.json')
  ...
  // piling up "Fabrica: <branch>" rows forever and breaking the notification
  ...
  try {
    execFileSync(
      ...
      [
        ...
        '--single-arch',
        '--output',
        path.join(appPath, 'Contents', 'MacOS', 'fabrica-notification-status')
      ],
      { stdio: 'inherit' }
    )
  } catch (error) {
    console.warn(
      `[fabrica-dev] notification-status helper build failed (permission card falls back to probes): ${error?.message ?? error}`
    )
  }
  ...
  try {
    ...
    console.warn(
      `[fabrica-dev] ad-hoc codesign failed (dev notifications will not deliver): ${error?.message ?? error}`
    )
  }
...
function getDevUserDataPath() {
  if (process.env.FABRICA_DEV_USER_DATA_PATH) {
    return process.env.FABRICA_DEV_USER_DATA_PATH
  }
  if (process.platform === 'darwin') {
    return path.join(process.env.HOME ?? '', 'Library', 'Application Support', 'fabrica-dev')
  }
...
function prepareDevCliWrapper() {
  ...
  console.log(`[fabrica-dev] Prepared wrapper in ${binDir}`)
}
...
if (process.env.FABRICA_SKIP_DEV_CLI_PREPARE !== '1') {
  prepareDevCliWrapper()
}

seedDevInstanceIdentityEnv()
if (!useStableElectronName && process.env.FABRICA_SKIP_DEV_ELECTRON_APP_PREPARE !== '1') {
...
function prepareDevWebClient() {
  if (process.env.FABRICA_SKIP_DEV_WEB_PREPARE === '1' || isHelpOrVersion) {
    return
  }
  ...
  if (!existsSync(getDevWebClientIndexPath()) && process.env.FABRICA_DEV_WEB_PREPARE !== '1') {
    console.error(
      '[fabrica-dev] Web client bundle missing; skipping pairing web build. Run `pnpm run build:web` or set FABRICA_DEV_WEB_PREPARE=1 when you need browser pairing.'
    )
    return
  }
  ...
  console.error('[fabrica-dev] Building web client for pairing...')
  execFileSync(
    process.execPath,
...
if (!isHelpOrVersion && process.env.FABRICA_DEV_INSTANCE_LABEL) {
  console.error(`[fabrica-dev] Instance: ${process.env.FABRICA_DEV_INSTANCE_LABEL}`)
}
let forwardedExtras = []
if (!userPassedPort && !isHelpOrVersion) {
  ...
  if (envPortRaw) {
    ...
    if (port === null) {
      console.error(
        `[fabrica-dev] Ignoring invalid REMOTE_DEBUGGING_PORT=${JSON.stringify(envPortRaw)}; falling back to probe.`
      )
    }
  }
  if (port === null) {
  ...
  if (port !== null) {
    ...
    console.error(`[fabrica-dev] Remote debugging on http://127.0.0.1:${port}`)
  } else {
    console.error(
      '[fabrica-dev] No free debug port found in sweep; starting without --remote-debugging-port.'
    )
  }
}
prepareDevWebClient()
