...
function makeSocketPath(userDataDir) {
  ...
  if (process.platform === 'win32') {
    return `\\\\.\\pipe\\fabrica-daemon-smoke-${process.pid}-${randomUUID()}`
  }
...
async function main() {
  const userDataDir = mkdtempSync(join(tmpdir(), 'fabrica-daemon-boot-smoke-'))
  ...
  const child = fork(
    ...
    ],
    {
      ...
      env: { ...process.env, FABRICA_USER_DATA_PATH: userDataDir }
    }
  )
