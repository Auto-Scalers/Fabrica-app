 ...
 * graceful-then-immediate kill pair emitted when Fabrica closes a workspace.
...
const iterations = Number(process.env.FABRICA_WINDOWS_DAEMON_CLOSE_ITERATIONS ?? 25)
...
async function main() {
  ...
  const scratch = mkdtempSync(join(tmpdir(), 'fabrica-windows-daemon-close-'))
  const socketPath = `\\\\.\\pipe\\fabrica-daemon-close-${process.pid}-${randomUUID()}`
  ...
  const child = fork(
    ...
    ['--socket', socketPath, '--token', tokenPath, '--log-file', daemonLogPath],
    {
      ...
      windowsHide: true,
      env: { ...process.env, FABRICA_USER_DATA_PATH: scratch }
    }
  )
