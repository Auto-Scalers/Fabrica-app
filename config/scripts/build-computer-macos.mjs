...
const binaryPath = path.join(packagePath, '.build', 'release', 'fabrica-computer-use-macos')
const appPath = path.join(packagePath, '.build', 'release', 'Fabrica Computer Use.app')
const appExecutablePath = path.join(appPath, 'Contents', 'MacOS', 'fabrica-computer-use-macos')
...
function buildUniversalBinary() {
  const builtBinaries = universalTriples.map((triple) => {
    ...
    return path.join(packagePath, '.build', triple, 'release', 'fabrica-computer-use-macos')
  })
...
function codesignArgs(identity, targetPath) {
  ...
  if (process.env.FABRICA_MAC_RELEASE === '1' && identity !== '-') {
...
function resolveSigningIdentity() {
  const explicitIdentity = process.env.FABRICA_COMPUTER_MACOS_SIGN_IDENTITY ?? process.env.CSC_NAME
  if (explicitIdentity) {
  ...
  if (process.env.FABRICA_MAC_RELEASE !== '1' && developmentMatch) {
...
<dict>
  ...
  <string>fabrica-computer-use-macos</string>
  ...
  <string>Fabrica Computer Use needs Accessibility permission to read and interact with app interfaces when you ask Fabrica to use apps.</string>
  <key>NSScreenCaptureUsageDescription</key>
  <string>Fabrica Computer Use needs Screen Recording permission to capture app windows when you ask Fabrica to inspect your screen.</string>
</dict>
</plist>
`
}
