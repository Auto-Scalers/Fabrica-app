#!/usr/bin/env node
/**
 * Terminal E2E helpers for agent-browser + CDP testing against a running Fabrica
 ...
 *
 * Usage:
 *   import { FabricaTerminal } from './terminal-e2e-helpers.mjs'
 *
 *   const term = new FabricaTerminal(9444)       // CDP port
 *   await term.connect()
...
export class FabricaTerminal {
  /** @param {number} cdpPort — the --remote-debugging-port used when launching Fabrica */
  constructor(cdpPort) {
    this.port = cdpPort
  }
   ...
   */
  probePtyIdWithScreenshot(maxId = 10, screenshotPath = tempScreenshotPath('fabrica-pty-probe.png')) {
    for (let i = 1; i <= maxId; i++) {
  ...
  /**
   * Take a screenshot of the Fabrica window.
   *
   * @param {string} path — output file path
   * @returns {string} the screenshot path
   */
  screenshot(path = tempScreenshotPath('fabrica-terminal.png')) {
    ab(this.port, ['screenshot', path])
    return path
  }

  /**
   * Open a new terminal tab in Fabrica.
   * @returns {void}
   */
  newTerminal() {
  ...
  readLang(ptyId) {
    ...
    return this.screenshot(tempScreenshotPath('fabrica-lang-check.png'))
  }
}
...
if (process.argv[1]?.endsWith('terminal-e2e-helpers.mjs')) {
  ...
  const term = new FabricaTerminal(port)
  console.log('Connecting to Fabrica on CDP port', port, '...')
  term.connect()
