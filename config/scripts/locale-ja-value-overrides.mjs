...
const JA_AUDIT_OVERRIDES = {
  ...
  'Split Up': '上に分割',
  'Split Down': '下に分割',
  "Install the Browser Use skill so agents can operate Fabrica's browser.":
    'Browser Use スキルをインストールすると、 Agent が Fabrica のブラウザを操作できるようになります。',
  'bold ·': '太字 ·',
  ...
  '{{host}} is no longer a saved SSH host, so this workspace is no longer connected to a live host. It can only be removed from Fabrica — files and branches on the remote are left untouched.':
    '{{host}} は保存済みの SSH ホストではなくなったため、このワークスペースは稼働中のホストに接続されていません。Fabrica から削除することのみ可能です。リモート上のファイルとブランチはそのまま残ります。',
  'The app shell could not finish rendering. Retry to remount it, or relaunch Fabrica if the error persists.':
    'アプリシェルのレンダリングを完了できませんでした。再マウントを試すか、エラーが解決しない場合は Fabrica を再起動してください。',
  'Workspaces are unavailable on a mobile-scope pairing. Reconnect using the browser access link from Settings → Runtime Environments → Share this Fabrica server.':
    'モバイル向けペアリングではワークスペースを使用できません。[設定] → [リモート Fabrica サーバー] → [この Fabrica サーバーを共有する] から、ブラウザ用リンクで再接続してください。',
  'Change this later in Settings → Browser.': '後で [設定] → [ブラウザ] から変更できます。',
  'Fabrica Cloud sign-in is not configured': 'Fabrica Cloud へのサインインが設定されていません',
  ...
  'only when Fabrica cannot derive the API base URL from the git remote.':
    'は、Fabrica が git リモートから API のベース URL を取得できない場合のみ設定します。',
  'only when Fabrica cannot derive the API URL from the remote.':
    'は、Fabrica がリモートから API URL を取得できない場合のみ設定します。',
  Set: '設定:',
  '. Set': '。設定:',
  ', or set': '、または',
  and: 'と',
  ...
  ' vs {{value0}}': ' vs {{value0}}',
  'When you create a workspace, Fabrica refreshes the remote base and safely fast-forwards your matching local branch, such as':
    'ワークスペースを作成すると、Fabrica はリモートベースを更新し、一致するローカルブランチを安全に fast-forward します。対象は',
  '. This keeps commands like': '。これにより、',
  'from comparing against stale history. Fabrica skips the update if that branch has uncommitted changes or local-only commits.':
    'などのコマンドが古い履歴と比較されるのを防ぎます。対象のブランチにコミットしていない変更やローカルのみのコミットがある場合、Fabrica は更新をスキップします。',
  ...
  'Jump to...': '移動先…',
  'Start work': '作業を開始',
  'Remove {{value0}} contained {{value1}} from Fabrica':
    '含まれている {{value1}} {{value0}} 件を Fabrica から削除',
  'People in {{orgName}} who can collaborate on Fabrica.':
    '{{orgName}} で Fabrica を共同利用できるメンバーです。',
  'People in your organization who can collaborate on Fabrica.':
    '組織内で Fabrica を共同利用できるメンバーです。',
  ...
  'This QR code grants limited (mobile) access. To use the full web app, open the browser access link from Settings → Runtime Environments → Share this Fabrica server → New Link.':
    'この QR コードでは、モバイル向けの制限付きアクセスのみ利用できます。完全版の Web アプリを使用するには、[設定] → [リモート Fabrica サーバー] → [この Fabrica サーバーを共有する] → [新規リンク] から、ブラウザ用アクセスリンクを開いてください。',
  'Loading skills...': 'スキルを読み込み中…',
  'Start agents on your tasks without leaving Fabrica': 'Fabrica から離れずに、タスクから Agent を開始',
  'Workspace created from {{value0}}, but Fabrica could not fast-forward local {{value1}} because {{value2}}':
    'ワークスペースは {{value0}} から作成されましたが、{{value2}} のため Fabrica はローカル {{value1}} を fast-forward できませんでした',
  ...
  'fabrica.yaml first, then your local commands.': 'まず fabrica.yaml、次にローカルのコマンドの順です。',
  'Rename Fabrica-created branches from the initial agent task.':
    'Fabrica が作成したブランチの名前を、最初の Agent タスクに基づいて変更します。',
  'Dim files matched by .gitignore in the file explorer.':
    'ファイルエクスプローラーで、.gitignore に一致するファイルを薄く表示します。',
  'Command line Fabrica runs when a text recipe uses Custom command.':
    'テキストレシピで「カスタムコマンド」を使用したときに Fabrica が実行するコマンドライン。',
  ...
  'Repository not in Fabrica': 'このリポジトリは Fabrica に登録されていません',
  ...
  'Clears them from Fabrica only. Remote files, worktrees, and branches are left untouched.':
    'Fabrica からのみ削除します。リモートのファイル、ワークツリー、ブランチはそのまま残ります。',
  ...
  'Install the Fabrica CLI before running agent skill setup.':
    'Agent スキルのセットアップを実行する前に、Fabrica CLI をインストールしてください。',
  'Run grok in a terminal on the computer running Fabrica and wait for it to start. If prompted, complete sign-in, then retry usage. You do not need to send a chat message.':
    'Fabrica を実行しているコンピュータのターミナルで grok を実行し、起動するまで待ってください。サインインを求められた場合は完了してから、使用状況を再取得してください。チャットメッセージを送る必要はありません。',
  ...
  "Fabrica works with every CLI agent. Choose the one you'll reach for most. Switch any time.":
    'Fabrica はすべての CLI Agent で動作します。最もよく使うものを選択してください。いつでも切り替えられます。',
  ...
  'Install and manage experimental Fabrica plugins.':
    '実験的機能の Fabrica プラグインをインストール・管理します。',
  "Resident memory held by Fabrica plus the processes under each worktree's terminals.":
    'Fabrica が保持する常駐メモリと、各ワークツリーのターミナル配下で実行中のプロセス。',
  ...
  'Optional. Fabrica can use your normal Codex login; add accounts only if you want quick switching in Fabrica.':
    '任意。Fabrica は通常の Codex ログインを使用できます。Fabrica 上ですばやく切り替えたい場合にのみ、アカウントを追加してください。',
  'Optional. Fabrica can use your normal Claude login; add accounts only if you want quick switching without moving chat sessions.':
    '任意。Fabrica は通常の Claude ログインを使用できます。チャットセッションを移さずにすばやく切り替えたい場合にのみ、アカウントを追加してください。',
  'Optional. Fabrica works with your existing provider logins; add accounts only if you want Fabrica to help switch between them.':
    '任意。Fabrica は既存のプロバイダーのログインと連携します。Fabrica に切り替えを任せたい場合にのみ、アカウントを追加してください。',
