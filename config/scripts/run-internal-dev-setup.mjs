...
// Why: under a Git Bash setup runner, Fabrica exports FABRICA_WORKTREE_PATH in MSYS form (/c/...), which
...
export function runInternalDevSetup({
  ...
  const setupPath = env.FABRICA_INTERNAL_DEV_SETUP?.trim()
  ...
  spawnOptionalSetup(spawn, setupPath, env.FABRICA_WORKTREE_PATH || cwd, platform, env)
