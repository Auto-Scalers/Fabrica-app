...
const ROUNDS = Number(process.env.FABRICA_CLI_DEFER_BENCH_ROUNDS ?? '30')
const WARMUP = Number(process.env.FABRICA_CLI_DEFER_BENCH_WARMUP ?? '3')

for (const [name, value] of [
  ['FABRICA_CLI_DEFER_BENCH_ROUNDS', ROUNDS],
  ['FABRICA_CLI_DEFER_BENCH_WARMUP', WARMUP]
]) {
...
if (ROUNDS % 2 !== 0) {
  ...
  throw new Error(`FABRICA_CLI_DEFER_BENCH_ROUNDS must be even, received ${ROUNDS}`)
}
...
try {
  ...
  const isolated = { FABRICA_USER_DATA_PATH: userDataPath }
  /** @type {Array<[string, string[], Record<string, string>]>} */
  const cases = [
    ['fabrica --help', ['--help'], {}],
    ['fabrica help worktree', ['help', 'worktree'], {}],
    ['fabrica (no args)', [], {}],
    ['unknown command', ['no-such-command'], {}],
    ['unknown flag', ['worktree', 'list', '--nope'], {}],
    ['fabrica agent-context --json', ['agent-context', '--json'], {}],
    ['fabrica status --json', ['status', '--json'], isolated],
    ['fabrica worktree list --json', ['worktree', 'list', '--json'], isolated]
  ]
