...
function hashPackagedPluginTree(root) {
  ...
  visit(root)
  const hash = createHash('sha256').update('fabrica-plugin-tree-v1\0')
...
function verifyPackagedPluginResources(resourcesDir) {
  ...
  readJsonFile(join(launchRoot, 'fabrica-marketplace.json'), 'marketplace index')
  ...
  for (const entry of index.plugins) {
    ...
    const manifest = readJsonFile(join(pluginRoot, 'fabrica-plugin.json'), 'plugin manifest')
