...
describe('Windows CLI launcher', () => {
  itCrossHost('fails closed when the Windows launcher cannot be compiled on this host', () => {
    const outputRoot = mkdtempSync(join(tmpdir(), 'fabrica cross-host launcher '))
    try {
      const result = spawnSync(
        process.execPath,
        ['config/scripts/build-windows-cli-launcher.mjs', '--output', join(outputRoot, 'fabrica.exe')],
  ...
  itCrossHost('never materializes the child environment block from ProcessStartInfo', () => {
    ...
    // dictionary that throws when the inherited block holds PATH and Path (Auto-Scalers/Fabrica#12046).
    const source = readFileSync(
      join(projectRoot, 'native', 'windows-cli-launcher', 'FabricaCliLauncher.cs'),
      'utf8'
    )
  ...
  itWindows('preserves a multiline argument from PowerShell through the native launcher', () => {
    const appRoot = mkdtempSync(join(tmpdir(), 'fabrica cli launcher '))
    try {
      const resourcesPath = join(appRoot, 'resources')
      const launcherPath = join(resourcesPath, 'bin', 'fabrica.exe')
      const cliPath = join(resourcesPath, 'app.asar.unpacked', 'out', 'cli', 'index.js')
      mkdirSync(join(resourcesPath, 'bin'), { recursive: true })
      mkdirSync(dirname(cliPath), { recursive: true })
      copyFileSync(process.execPath, join(appRoot, 'Fabrica.exe'))
      writeFileSync(
        cliPath,
  ...
  fabricaNodeOptions: process.env.FABRICA_NODE_OPTIONS ?? null
      ...
      const powershell = spawnSync(
        ...
        [
          ...
          '-NonInteractive',
          '-Command',
          '& $env:FABRICA_TEST_LAUNCHER orchestration send --body $env:FABRICA_TEST_BODY --json'
        ],
        {
          encoding: 'utf8',
          env: {
            ...
            FABRICA_TEST_BODY: body,
            FABRICA_TEST_LAUNCHER: launcherPath
          }
        }
      )
      ...
      expect(JSON.parse(powershell.stdout)).toEqual({
        ...
        electronRunAsNode: '1',
        nodeOptions: null,
        fabricaNodeOptions: '--no-warnings'
      })
    } finally {
  ...
  itWindows('survives an inherited environment block containing PATH and Path', () => {
    const appRoot = mkdtempSync(join(tmpdir(), 'fabrica duplicate path launcher '))
    try {
      const resourcesPath = join(appRoot, 'resources')
      const launcherPath = join(resourcesPath, 'bin', 'fabrica.exe')
      const cliPath = join(resourcesPath, 'app.asar.unpacked', 'out', 'cli', 'index.js')
      ...
      writeFileSync(
        cliPath,
        `require('node:fs').writeFileSync(process.env.FABRICA_TEST_OUTPUT, JSON.stringify({
  electronRunAsNode: process.env.ELECTRON_RUN_AS_NODE,
