#!/usr/bin/env bash
#
# Creates Auto-Scalers/fabrica-daily and grants the existing release App write access to
# it, so daily-mac-build.yml can publish there.
#
# Why a separate repo rather than reusing fabrica-hourly: the daily channel is a
...
if gh api "repos/$DAILY_REPO" --jq '.full_name' >/dev/null 2>&1; then
  ...
  gh repo create "$DAILY_REPO" \
    --public \
    --description "Daily macOS dev builds of Fabrica, cut from main each morning. Not a source repo." \
    --add-readme \
    --disable-issues \
    --disable-wiki ||
...
Could not do it from here${INSTALL_ID:+ (needs an Organization Owner)}. Do it in the browser:
  ...
  3. Repository access  ->  Only select repositories  ->  add $DAILY_REPO
     (keep fabrica-hourly and fabrica-adhoc selected; all dev channels use this one App)
  4. Save.
EOF
fi
