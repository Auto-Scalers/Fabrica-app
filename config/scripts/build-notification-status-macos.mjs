#!/usr/bin/env node
// Builds the fabrica-notification-status helper binary.
//
...
const workDir = path.join(tmpdir(), `fabrica-notification-status-${process.pid}`)
...
try {
  ...
  const builtBinaries = triples.map((triple) => {
    const output = path.join(workDir, `fabrica-notification-status-${triple}`)
    execFileSync(
      'swiftc',
      [
        '-O',
...
<dict>
  ...
  <string>fabrica-notification-status</string>
</dict>
</plist>
`
}
