#!/usr/bin/env bash
#
# Creates Auto-Scalers/fabrica-adhoc and grants the existing release App write access to
# it, so adhoc-mac-build.yml can publish there.
#
# Why a separate repo rather than reusing fabrica-hourly: an adhoc build is somebody's
...
ORG="Auto-Scalers"
ADHOC_REPO="$ORG/fabrica-adhoc"
MAIN_REPO="$ORG/fabrica"
APP_SLUG="fabrica-hourly-release"
...
if gh api "repos/$ADHOC_REPO" --jq '.full_name' >/dev/null 2>&1; then
  ...
  # as it does for fabrica-hourly. A private repo would 404 for every client.
  #
  ...
  gh repo create "$ADHOC_REPO" \
    --public \
    --description "Adhoc macOS dev builds of Fabrica, cut from unlanded branches. Not a source repo." \
    --add-readme \
    --disable-issues \
    --disable-wiki ||
...
Could not do it from here${INSTALL_ID:+ (needs an Organization Owner)}. Do it in the browser:
  ...
  3. Repository access  ->  Only select repositories  ->  add $ADHOC_REPO
     (keep fabrica-hourly/fabrica-daily selected; all dev channels use this one App)
  4. Save.
EOF
fi
