...
module.exports = {
  ...
  files: [
    ...
    '!examples{,/**/*}',
    // Why: pr-evidence/ is a local e2e screenshot output (FABRICA_CAPTURE_EVIDENCE);
    ...
    // Why: the Windows CLI shim ships via extraResources to resources/bin/fabrica.cmd
    // (beside the native resources/bin/fabrica.exe). Packing the source tree into
    // app.asar too lets asarUnpack:['resources/**'] extract a second copy at
    // app.asar.unpacked/resources/win32/bin/fabrica.cmd with no adjacent fabrica.exe,
  ...
  afterPack: async (context) => {
    ...
    if (context.electronPlatformName === 'darwin') {
      ...
      let commit = process.env.FABRICA_BUILD_COMMIT || process.env.GITHUB_SHA || 'unknown'
      if (commit === 'unknown') {
        try {
    ...
    if (context.electronPlatformName === 'darwin') {
      await signMacComputerUseHelper(join(resourcesDir, 'Fabrica Computer Use.app'), context.packager)
      await signMacNotificationStatusHelper(
        join(resourcesDir, '..', 'MacOS', 'fabrica-notification-status'),
        context.packager
      )
    }
  },
  win: {
    executableName: 'Fabrica',
  ...
  nsis: {
    artifactName: 'fabrica-windows-setup.${ext}',
  ...
  mac: {
    ...
    extendInfo: {
      NSAppleEventsUsageDescription:
        'Fabrica allows terminal-launched developer tools to automate local apps when you request it.',
      NSBluetoothAlwaysUsageDescription:
        'Fabrica allows terminal-launched developer tools to access Bluetooth devices when you request it.',
      NSBluetoothPeripheralUsageDescription:
        'Fabrica allows terminal-launched developer tools to access Bluetooth devices when you request it.',
      NSCameraUsageDescription: "Application requests access to the device's camera.",
      NSLocationUsageDescription:
        'Fabrica allows terminal-launched developer tools to access location when you request it.',
      NSLocalNetworkUsageDescription:
        'Fabrica allows terminal-launched developer tools to discover and connect to local development servers when you request it.',
      NSMicrophoneUsageDescription: "Application requests access to the device's microphone.",
      NSAudioCaptureUsageDescription:
        'Fabrica allows terminal-launched developer tools to capture desktop audio when you request it.',
    ...
    extraResources: [
      ...
      {
        from: 'resources/darwin/bin/fabrica',
        to: 'bin/fabrica'
      },
      {
  ...
  dmg: {
    artifactName: 'fabrica-macos-${arch}.${ext}'
  },
  linux: {
    // Why: Ubuntu desktop ships GNOME Fabrica as the `fabrica` package and /usr/bin/fabrica.
    ...
    desktop: {
      entry: {
        // Why: Electron reports WM_CLASS=fabrica for the visible Linux window;
        // GNOME docks need an exact match to group it with fabrica.desktop.
        StartupWMClass: 'fabrica'
      }
    },
    extraResources: [
      ...commonExtraResources,
      ...
      {
        from: 'resources/linux/bin/fabrica',
        to: 'bin/fabrica'
      },
      {
    ...
    target: ['AppImage', 'deb'],
    maintainer: 'Auto-Scalers',
    category: 'Utility'
  },
  appImage: {
    artifactName: isLinuxArm64Release ? 'fabrica-linux-arm64.${ext}' : 'fabrica-linux.${ext}'
  },
  deb: {
    packageName: 'fabrica',
    artifactName: 'fabrica_${version}_${arch}.${ext}',
    // Why: xvfb lets the bundled `fabrica serve` CLI run browser panes on a headless
    ...
    // Why: symlink the bundled CLI onto PATH at install time so `fabrica serve`
  ...
  // on Intel Macs. The beforeBuild hook performs Fabrica's targeted rebuild and
...
async function signMacComputerUseHelper(helperAppPath, packager) {
  if (!existsSync(helperAppPath)) {
    if (isMacRelease) {
      throw new Error(`Missing Fabrica Computer Use helper app at ${helperAppPath}`)
    }
    return
  }
  const codeSigningInfo =
    isMacRelease && process.env.CSC_LINK && packager?.codeSigningInfo?.value
      ...
      : null
  const identity =
    process.env.FABRICA_COMPUTER_MACOS_SIGN_IDENTITY ??
    process.env.CSC_NAME ??
    findInstalledMacSigningIdentity(codeSigningInfo?.keychainFile) ??
    (isMacRelease ? null : '-')
  if (!identity) {
    throw new Error('Missing signing identity for Fabrica Computer Use helper app')
  }
  // Why: TCC grants attach to this nested app's code identity. Sign it before
  // the outer Fabrica.app is sealed so production builds preserve that identity.
...
async function signMacNotificationStatusHelper(helperPath, packager) {
  if (!existsSync(helperPath)) {
    if (isMacRelease) {
      throw new Error(`Missing fabrica-notification-status helper at ${helperPath}`)
    }
    return
  }
  const codeSigningInfo =
  ...
  const identity =
    ...
    (isMacRelease ? null : '-')
  if (!identity) {
    throw new Error('Missing signing identity for fabrica-notification-status helper')
  }
  ...
  // before the outer Fabrica.app is sealed, like the computer-use helper.
