...
; (fabrica-terminal-daemon.exe) under %LOCALAPPDATA%\Fabrica\daemon-host so that app
...
!macro customUnInstall
  ${ifNot} ${isUpdated}
    nsExec::Exec 'taskkill /F /IM fabrica-terminal-daemon.exe'
    ; Give the OS a moment to release the image lock before removing the tree.
    Sleep 500
    RMDir /r "$LOCALAPPDATA\Fabrica\daemon-host"
  ${endIf}
!macroend
