...
appimage=${FABRICA_TEST_APPIMAGE:-/artifacts/squashfs-root/AppRun}
timeout_seconds=${FABRICA_STARTUP_TIMEOUT_SECONDS:-12}
pairing_address=${FABRICA_PAIRING_ADDRESS:-127.0.0.1}
port=${FABRICA_SERVE_PORT:-0}
state_dir="/tmp/fabrica-${case_name}"

if ((EUID == 0)); then
  mkdir -p "$state_dir/config" "$state_dir/cache"
  chown -R fabrica:fabrica "$state_dir"
  # Why: packaged serve should exercise the same unprivileged account required by production systemd guidance.
  exec runuser --user fabrica --preserve-environment -- "$0" "$@"
fi
...
app_args=("${launcher[@]}")
if [[ ${FABRICA_TEST_NO_SANDBOX:-1} == 1 ]]; then
  app_args+=(--no-sandbox)
fi
app_args+=(serve --port "$port" --pairing-address "$pairing_address")
if [[ ${FABRICA_READY_JSON:-0} == 1 ]]; then
  app_args+=(--json)
fi
if [[ ${FABRICA_NO_PAIRING:-0} == 1 ]]; then
  app_args+=(--no-pairing)
fi
...
if [[ $is_appimage == 0 ]]; then
  export APPDIR=${FABRICA_TEST_APPDIR:-"$(dirname "$appimage")"}
fi

if [[ ${FABRICA_KEEP_RUNNING:-0} == 1 ]]; then
  exec "${command[@]}"
fi
