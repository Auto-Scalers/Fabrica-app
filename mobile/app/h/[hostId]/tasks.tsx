...
type FabricaYamlTrustPrompt = WorkspaceCreateArgs & {
  repoId: string
  repoName: string
  scriptContent: string
  contentHash: string
...
type ProjectRepoNotInFabricaPrompt = {
  owner: string
  repo: string
  url: string | null
}
...
export default function MobileTasksScreen() {
  ...
  const [projectRepoNotInFabrica, setProjectRepoNotInFabrica] =
    useState<ProjectRepoNotInFabricaPrompt | null>(null)
  ...
  const persistSetupHookTrust = useCallback(
    async (repoId: string, contentHash: string, alwaysTrust: boolean): Promise<void> => {
      ...
      const response = await client.sendRequest('ui.set', { trustedFabricaHooks: next })
      if (!isSuccess(response)) {
        throw new Error(response.error.message)
      }
      setTrustedFabricaHooks(next)
    },
    [client, trustedFabricaHooks]
  )
  ...
  const resetWorkspaceCreateState = useCallback((): void => {
    ...
    setSetupPrompt(null)
    setFabricaYamlTrustPrompt(null)
  }, [])
  ...
  useEffect(() => {
    if (!client || connState !== 'connected') {
      ...
      setProjectRepoNotInFabrica(null)
    ...
    const hydrateTaskState = async (): Promise<void> => {
      ...
      if (!status.capabilities?.includes(MOBILE_TASKS_CAPABILITY)) {
        ...
        setMergeMethodProjectRow(null)
        resetWorkspaceCreateState()
        setError('Update Fabrica desktop to use Tasks on mobile.')
        setTaskStateHydrated(false)
        return
      }
      ...
      const uiState = isSuccess(uiResponse)
        ...
        : null
      setTrustedFabricaHooks(uiState?.trustedFabricaHooks ?? {})
    ...
    async (project: GitHubProjectRef, options: { viewNumber?: number } = {}): Promise<void> => {
      ...
      try {
        ...
        if (explicitView && explicitView.layout !== 'TABLE_LAYOUT') {
          throw new Error("Fabrica doesn't support this GitHub Project layout yet.")
        }
    ...
    async (
      ...
      try {
        ...
        if (
          ...
          // Why: desktop prompts before running repo-owned fabrica.yaml hooks. Mobile
          ...
          setFabricaYamlTrustPrompt({
            ...
            previouslyApproved: wasSetupHookPreviouslyApproved(trustedFabricaHooks, targetRepo.id)
          })
          return
        }
  ...
  const createWorkspaceFromProjectRow = useCallback(
    async (row: GitHubProjectRow): Promise<void> => {
      ...
      if (!kind || !row.content.number || !row.content.url) {
        setError('Add the project item repository to Fabrica before creating a workspace.')
        return
      }
      if (!repo) {
        const slug = splitRepositorySlug(row.content.repository)
        setProjectRepoNotInFabrica({
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      {!tasksSupported ? (
        tasksUnsupported ? (
          <View style={styles.centered}>
            <Text style={styles.emptyText}>Update Fabrica desktop</Text>
      ...
      <PickerModal
        ...
        onSelect={(viewId) => {
          ...
          if (view && view.layout !== 'TABLE_LAYOUT') {
            setGithubProjectError("Fabrica doesn't support this GitHub Project layout yet.")
            return
          }
      ...
      <BottomDrawer
        visible={taskUiReady && fabricaYamlTrustPrompt != null}
        onClose={() => setFabricaYamlTrustPrompt(null)}
        zIndex={TASK_SECONDARY_DRAWER_Z_INDEX + 1}
      >
        {fabricaYamlTrustPrompt ? (
          <View>
            <View style={styles.sheetHeader}>
              <Text style={styles.sheetTitle}>
                {fabricaYamlTrustPrompt.previouslyApproved
                  ? `${fabricaYamlTrustPrompt.repoName}'s setup script changed`
                  : `Run setup from ${fabricaYamlTrustPrompt.repoName}?`}
              </Text>
              <Text style={styles.sheetSubtitle}>
                This repository's fabrica.yaml runs on your machine before the workspace starts. Only
            ...
            <View style={styles.setupPromptBox}>
              <View style={styles.detailSectionHeader}>
                <Text style={styles.detailSectionTitle}>
                  {fabricaYamlTrustPrompt.previouslyApproved ? 'New setup script' : 'Setup script'}
                </Text>
              </View>
              <Text style={styles.setupPromptCommand}>{fabricaYamlTrustPrompt.scriptContent}</Text>
            </View>

            <View style={styles.actionGroup}>
              <Pressable
                style={styles.actionRow}
                disabled={creatingKey === fabricaYamlTrustPrompt.item.key}
                onPress={() =>
                  void (async () => {
                    try {
                      await persistSetupHookTrust(
                        fabricaYamlTrustPrompt.repoId,
                        fabricaYamlTrustPrompt.contentHash,
                        false
                      )
                      setFabricaYamlTrustPrompt(null)
                      await createWorkspace(
                        fabricaYamlTrustPrompt.item,
                        fabricaYamlTrustPrompt.repoIdOverride,
                        'run',
                        fabricaYamlTrustPrompt.agentOverride,
                        fabricaYamlTrustPrompt.workspaceNameOverride,
                        fabricaYamlTrustPrompt.noteOverride,
                        fabricaYamlTrustPrompt.baseBranchOverride,
                        fabricaYamlTrustPrompt.branchNameOverride,
                        fabricaYamlTrustPrompt.sparseCheckoutOverride,
                        fabricaYamlTrustPrompt.contentHash
                      )
                    } catch (err) {
              ...
              <Pressable
                ...
                disabled={creatingKey === fabricaYamlTrustPrompt.item.key}
                onPress={() => {
                  const prompt = fabricaYamlTrustPrompt
                  setFabricaYamlTrustPrompt(null)
                  void createWorkspace(
                    prompt.item,
      ...
      <BottomDrawer
        visible={taskUiReady && projectRepoNotInFabrica != null}
        onClose={() => {
          setProjectRepoNotInFabrica(null)
        }}
      >
        {projectRepoNotInFabrica ? (
          <View>
            <View style={styles.sheetHeader}>
              <Text style={styles.sheetTitle}>Repository not in Fabrica</Text>
              <Text style={styles.sheetSubtitle}>
                {projectRepoNotInFabrica.owner}/{projectRepoNotInFabrica.repo} is not added to Fabrica. Add
                this repository from the desktop app, then refresh mobile Tasks.
              </Text>
            </View>

            <View style={styles.actionGroup}>
              {projectRepoNotInFabrica.url ? (
                <Pressable
                  style={styles.actionRow}
                  onPress={() => {
                    if (projectRepoNotInFabrica.url) {
                      void Linking.openURL(projectRepoNotInFabrica.url)
                    }
                  }}
                >
              ...
              {projectRepoNotInFabrica.url ? <View style={styles.actionSeparator} /> : null}
              <Pressable
                style={styles.actionRow}
                onPress={() =>
                  void copyTextToClipboard(
                    `project-repo:${projectRepoNotInFabrica.owner}/${projectRepoNotInFabrica.repo}`,
                    `${projectRepoNotInFabrica.owner}/${projectRepoNotInFabrica.repo}`
                  )
                }
              >
            ...
            <View style={styles.actionGroup}>
              ...
              projectRowItem.content.state !== 'MERGED' ? (
                <>
                  ...
                  {!projectRowHostedRepo ? (
                    <Text style={styles.emptyInlineText}>
                      Merge requires this repository in Fabrica.
                    </Text>
                  ) : null}
                </>
              ) : null}
