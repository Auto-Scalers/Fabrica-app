...
describe('readRepoIdentity', () => {
  it('parses a valid owner/repo identity', () => {
    expect(readRepoIdentity({ owner: 'octo', repo: 'fabrica' })).toEqual({
      owner: 'octo',
      repo: 'fabrica'
    })
  })
  ...
  it('preserves an Enterprise host', () => {
    expect(readRepoIdentity({ owner: 'octo', repo: 'fabrica', host: 'github.acme.test' })).toEqual({
      owner: 'octo',
      repo: 'fabrica',
  ...
  it('drops a non-record value', () => {
    ...
    expect(readRepoIdentity('octo/FABRICA')).toBeUndefined()
  })

  it('drops a missing owner or repo', () => {
    expect(readRepoIdentity({ repo: 'fabrica' })).toBeUndefined()
    expect(readRepoIdentity({ owner: 'octo' })).toBeUndefined()
  })

  it('drops an empty owner or repo as malformed', () => {
    expect(readRepoIdentity({ owner: '', repo: 'fabrica' })).toBeUndefined()
