...
const repos = [
  { id: 'repo-1', path: '/Users/me/FABRICA', displayName: 'FABRICA' },
...
describe('GitHub project repo matching', () => {
  ...
  it('matches project rows by resolved repo slug before path/display heuristics', () => {
    expect(
      findRepoForGitHubProjectRepository('Auto-Scalers/FABRICA', repos, {
        'repo-1': {
          path: '/Users/me/FABRICA',
          repository: { owner: 'Auto-Scalers', repo: 'fabrica' }
        }
      })
    ).toBe(repos[0])
  })
  ...
  it('falls back to exact display/path slug matching when slug resolution is unavailable', () => {
    expect(
      findRepoForGitHubProjectRepository('Auto-Scalers/FABRICA', [
        { id: 'repo-1', path: '/Users/me/Auto-Scalers/FABRICA', displayName: 'FABRICA' }
      ])
    ).toEqual({ id: 'repo-1', path: '/Users/me/Auto-Scalers/FABRICA', displayName: 'FABRICA' })
  })

  it('normalizes Windows paths before path slug fallback matching', () => {
    expect(
      findRepoForGitHubProjectRepository('Auto-Scalers/FABRICA', [
        { id: 'repo-1', path: 'C:\\Users\\me\\Auto-Scalers\\FABRICA', displayName: 'FABRICA' }
      ])
    ).toEqual({ id: 'repo-1', path: 'C:\\Users\\me\\Auto-Scalers\\FABRICA', displayName: 'FABRICA' })
  })
  ...
  it('does not path-match a repo whose resolved slug points somewhere else', () => {
    expect(
      findRepoForGitHubProjectRepository(
        ...
        {
          'repo-1': {
            path: '/Users/me/Auto-Scalers/FABRICA',
            repository: { owner: 'fork', repo: 'fabrica' }
          }
        }
      )
    ).toBeNull()
  ...
  it('filters project rows to rows backed by open repositories', () => {
    const rows = [
      { id: 'row-1', content: { repository: 'Auto-Scalers/FABRICA' } },
    ...
    expect(
      findRepoForGitHubProjectRepository(
        ...
        repos,
        {
          'repo-1': {
            path: '/Users/me/FABRICA',
            repository: { owner: 'Auto-Scalers', repo: 'fabrica', host: 'github.com' }
          },
          'repo-2': {
            path: '/Users/me/other',
            repository: {
  ...
  it('leaves a failed slug lookup matchable by the path fallback', () => {
    expect(
      findRepoForGitHubProjectRepository(
        'Auto-Scalers/FABRICA',
        [{ id: 'repo-1', path: '/Users/me/Auto-Scalers/FABRICA', displayName: 'FABRICA' }],
        { 'repo-1': { path: '/Users/me/Auto-Scalers/FABRICA', repository: null, failed: true } }
      )
    ).toEqual({ id: 'repo-1', path: '/Users/me/Auto-Scalers/FABRICA', displayName: 'FABRICA' })
  })
})
...
describe('dropFailedGitHubRepoSlugEntries', () => {
  it('drops only the entries a retry could still resolve', () => {
    expect(
      dropFailedGitHubRepoSlugEntries({
        'repo-1': { path: '/a', repository: { owner: 'Auto-Scalers', repo: 'fabrica' } },
