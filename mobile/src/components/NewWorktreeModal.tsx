...
import type { PersistedTrustedFABRICAHooks, TuiAgent } from '../../../src/shared/types'
...
function NewWorktreeModalContent({
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (uiResult) {
        const ui = (uiResult.result as { ui?: { trustedFABRICAHooks?: PersistedTrustedFABRICAHooks } }).ui
        setTrustedFABRICAHooks(ui?.trustedFABRICAHooks ?? {})
      }
  ...
  async function handleCreate(options: CreateOptions = {}) {
    ...
    try {
      ...
      if (
        ...
        !isSetupHookTrusted(trustedFABRICAHooks, selectedRepo.id, setupTrust.contentHash)
      ) {
        // Why: desktop prompts before running repo-owned FABRICA.yaml setup hooks.
        ...
        setSetupTrustPrompt({
          ...
          previouslyApproved: wasSetupHookPreviouslyApproved(trustedFABRICAHooks, selectedRepo.id)
        })
  ...
  async function approveSetupTrust(alwaysTrust: boolean): Promise<void> {
    ...
    try {
      ...
      setTrustedFABRICAHooks(nextTrust)
                      ...
                      {setupSource && (
                        <View style={styles.sourceBadge}>
                          <Text style={styles.sourceBadgeText}>
                            {setupSource === 'FABRICA.yaml' ? 'FABRICA.YAML' : 'HOOKS'}
                          </Text>
                        </View>
                      )}
                    </View>
