...
// Same-named repos on different hosts (a local "FABRICA" and a runtime "FABRICA")
...
describe('every worktree stays visible regardless of repo.list contents', () => {
  it('shows worktrees whose repoId is missing from repo.list', () => {
    ...
    const worktrees = [
      worktree('FABRICA-local', 'a'),
      worktree('FABRICA-local', 'b'),
      worktree('FABRICA-runtime', 'c')
    ]
    const repoIdsByName = new Map([['FABRICA-runtime', 'FABRICA-runtime']])
  ...
  it('shows every worktree when the same repo name maps to two host-specific ids', () => {
    // The exact trigger: a local "FABRICA" and a runtime "FABRICA". repo.list returns
    // both under the same displayName; a name-keyed map keeps only the last id.
    const worktrees = [
      worktree('FABRICA-local', 'local-1', 'FABRICA'),
      worktree('FABRICA-local', 'local-2', 'FABRICA'),
      worktree('FABRICA-runtime', 'runtime-1', 'FABRICA')
    ]
    // Name-keyed map collapses "FABRICA" to a single id, as the host screen builds it.
    const repoIdsByName = new Map([['FABRICA', 'FABRICA-runtime']])
