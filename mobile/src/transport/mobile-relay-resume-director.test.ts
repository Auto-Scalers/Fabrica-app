...
const relay = {
  ...
  cellUrl: 'https://relay-old.autoscalers.workers.dev',
  assignmentEpoch: 7,
...
describe('mobile relay resume director', () => {
  it('uses a bounded POST body and never puts the bearer in the URL', async () => {
    ...
    expect(url).toBe('https://relay.autoscalers.workers.dev/v1/resolve')
    ...
    const badTarget = vi.fn(
      async () =>
        new Response(
          JSON.stringify({
            v: 1,
            cellUrl: 'http://relay-c2.autoscalers.workers.dev',
            assignmentEpoch: 8,
            leaseExpiresAt: 1
          })
        )
