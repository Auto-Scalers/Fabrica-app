      ...
      expect.objectContaining({
        ...
        endpoints: [
          { id: 'direct-primary', kind: 'lan', url: directOffer.endpoint },
          {
            id: 'relay-primary',
            kind: 'relay',
            url: `wss://relay-c1.autoscalers.workers.dev/v1/connect/${relayOffer.relay!.relayHostId}`
          }
        ]
      })
    )
  ...
  it('streams the relay path and the winning path into the pairing log', async () => {
    ...
    deps.connectRelay.mockImplementation((connectArgs) => {
      connectArgs.onLog?.({
        ...
        detail: 'relay-c1.autoscalers.workers.dev'
      })
      return relay
    })
    ...
    expect(entries[0]!.detail).toBe('relay-c1.autoscalers.workers.dev')
