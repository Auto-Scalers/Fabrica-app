...
describe('resolveHostEndpointEdit', () => {
  it.each([
    ...
    ['secure path-routed proxy', 'wss://desk.example.com/FABRICA'],
    ['secure query-routed proxy', 'wss://desk.example.com/FABRICA?route=runtime'],
  ...
  it('discriminates invalid input without a candidate endpoint', () => {
    expect(
      resolveHostEndpointEdit('wss://desk.example.com/FABRICA', 'https://desk.example.com')
    ).toEqual({
      kind: 'invalid',
  ...
  it('stays stable through repeated name-only edits', () => {
    const stored = 'wss://desk.example.com/FABRICA?route=runtime'
