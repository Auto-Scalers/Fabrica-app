...
export async function startHeadlessPairingRuntime({
  ...
  const runDir = mkdtempSync(path.join(os.tmpdir(), 'fabrica-mobile-run.'))
  ...
  const child = spawn(
    ...
    ['serve', '--mobile-pairing', '--pairing-address', pairingAddress, '--json'],
    {
      ...
      env: {
        ...process.env,
        FABRICA_E2E_USER_DATA_DIR: userData,
        FABRICA_E2E_HOME_DIR: homeDir,
        HOME: homeDir,
        USERPROFILE: homeDir
      },
...
export async function registerWorktreeForPairingRuntime(runtime, worktree, tools) {
  ...
  await tools.fabrica(['repo', 'add', '--path', worktree, '--json'], {
    cwd: worktree,
    env: runtime.env,
    timeout: 60000
  })
...
async function waitForPairingRuntime({ child, userData, pairingAddress, logSuccess }) {
  ...
  const runtimeResult = (pairingUrl) => ({
    ...
    env: {
      ...process.env,
      FABRICA_USER_DATA_PATH: userData,
      // Why: `fabrica-dev` derives its own profile and ignores FABRICA_USER_DATA_PATH, so
      // without this an FABRICA_CLI=fabrica-dev run would address the dev profile instead
      // of this disposable runtime. Plain `fabrica` ignores it.
      FABRICA_DEV_USER_DATA_PATH: userData
    },
    stop
  })
