#!/usr/bin/env node
/**
 * Start Fabrica Mobile server and load it in the iOS emulator.
...
const FABRICA_CLI = process.env.FABRICA_CLI || 'fabrica'
...
// Execute fabrica CLI command
async function fabrica(args, options = {}) {
  const { stdout, stderr } = await execFileAsync(FABRICA_CLI, args, {
...
async function getWorktree() {
  ...
  try {
    const { stdout } = await fabrica(['worktree', 'current', '--json'])
...
async function attachEmulator(worktree, device, runtime) {
  ...
  try {
    await fabrica(['emulator', 'attach', device.udid, '--worktree', worktree, '--focus', '--json'], {
      cwd: worktree,
...
function devClientUrlForMetroUrl(url) {
  return `exp+fabrica-mobile://expo-development-client/?url=${encodeURIComponent(url)}`
}
...
async function startMetro(worktree) {
  ...
  return new Promise((resolve, reject) => {
    ...
    rl.on('line', (line) => {
      ...
      const urlMatch = line.match(/exp\+fabrica-mobile:\/\/expo-development-client\/\?url=([^\s]+)/)
...
async function openPairingUrlInSimulator(pairingUrl, deviceUdid, runtime, worktree) {
  ...
  await fabrica(['emulator', 'tap', '0.5', '0.56', '--worktree', worktree, '--json'], {
    cwd: worktree,
    env: runtime?.env || process.env,
    timeout: 30000
  })
...
async function takeScreenshot(
  deviceUdid,
  outputPath = path.join(os.tmpdir(), 'fabrica-mobile-ios.png')
) {
...
// Main function
async function main() {
  log(colors.bright + 'Starting Fabrica Mobile in Emulator\n' + colors.reset)
  ...
  try {
    ...
    // Why: emulator helpers are worktree-scoped in Fabrica; attach is idempotent
