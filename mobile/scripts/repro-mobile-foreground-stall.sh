...
#
# Everything is driven with xcrun simctl rather than `fabrica emulator`, because the
...
#     > /tmp/fabrica-emulator-boot.log 2>&1 &
#
...
LOG="${3:-/tmp/fabrica-emulator-boot.log}"
BUNDLE_ID=com.autoscalers.fabrica.mobile
# Long enough for the tiered backoff to reach its 30s/60s tail.
ESCALATE_SECONDS=200

APP=$(pgrep -f "CoreSimulator.*Fabrica.app/Fabrica" | head -1)
...
T0=$(date +%s)
step "[5] user returns to Fabrica  <-- t0, desktop still down"
