    ...
    private func buildSnapshot(
        ...
        guard accessibilityTrustedSettled() else {
            ...
            throw ProviderError.coded(
                "permission_denied",
                "Accessibility permission is required for Fabrica Computer Use. Run `fabrica computer permissions` or open Settings > Computer Use, grant Accessibility to Fabrica Computer Use, then retry."
            )
        }
        ...
        let screenshotStatus: ScreenshotStatus = if screenshot != nil {
            ...
            .failed("Screen Recording permission is required for Fabrica Computer Use; grant permission or pass --no-screenshot to inspect accessibility state only.")
...
private func focusedWindow(appElement: AXUIElement, app: AppDescriptor, visibleWindowCount: Int, allowRecovery: Bool) throws -> AXUIElement {
    ...
    if visibleWindowCount > 0 {
        ...
        throw ProviderError.coded("permission_denied", "app '\(app.name)' has visible windows but no accessibility window (AX reads stayed blocked for \(outcome.waitedMs)ms after retries). macOS Accessibility may need Fabrica Computer Use toggled off and on again in System Settings.")
    }
...
private struct WindowCapture {
    ...
    private static func captureImage(windowId: CGWindowID, bounds: CGRect) -> CapturedImage? {
        if ProcessInfo.processInfo.environment["FABRICA_COMPUTER_USE_SCK_SCREENSHOTS"] == "1",
...
private enum PermissionKind: CaseIterable {
    ...
    var dragInstruction: String {
        ...
        case .screenshots:
            "Drag Fabrica Computer Use into the list above to allow Screenshots."
        }
    }
...
private final class PermissionView: NSView {
    ...
    private func build() {
        ...
        let titleText = checking
            ...
            : (ready ? "Computer Use is Ready" : "Enable Fabrica Computer Use")
        ...
        let subtitle = label(
            checking
                ...
                : (ready
                    ...
                    : "Grant permissions so Fabrica can use apps when you ask."),
            size: 12,
            weight: .regular
        )
    ...
    private func permissionRow(permission: PermissionKind, action: @escaping () -> Void) -> NSView {
        ...
        objc_setAssociatedObject(button, "fabrica-action", target, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
...
private final class PermissionDragAssistantController: NSWindowController {
    ...
    convenience init(
        ...
        window.title = "Drag Fabrica Computer Use"
...
private final class PermissionDragAssistantView: NSView {
    ...
    private func build() {
        ...
        objc_setAssociatedObject(closeButton, "fabrica-action", target, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
...
private final class DraggableAppTile: NSView, NSDraggingSource {
    ...
    private func build() {
        ...
        let title = NSTextField(labelWithString: "Fabrica Computer Use")
...
private func isAuthorizedAgentPeer(_ pid: pid_t) -> Bool {
    ...
    if isTrustedFabricaApplication(pid) {
        return true
    }
    guard let parentPid = parentProcessId(pid) else { return false }
    return isTrustedFabricaApplication(parentPid)
}

private func isTrustedFabricaApplication(_ pid: pid_t) -> Bool {
    guard let app = NSRunningApplication(processIdentifier: pid),
...
private func runStdio() {
    fputs("Fabrica Computer Use provider must be launched by Fabrica in app-agent mode.\n", stderr)
    exit(13)
}
...
if arguments.first == "--agent" {
    guard arguments.count >= 2 else {
        fputs("usage: fabrica-computer-use-macos --agent <socket-path> --token-file <token-path>\n", stderr)
        exit(2)
    }
    ...
    guard let token, !token.isEmpty else {
        fputs("fabrica-computer-use-macos --agent requires a non-empty --token-file\n", stderr)
        exit(2)
    }
    ...
    guard arguments.count >= 2 else {
        fputs("usage: fabrica-computer-use-macos --permission-status-file <path>\n", stderr)
        exit(2)
    }
