...
public sealed class FabricaRenderTestCounter {
    public int FindAll;
}

public sealed class FabricaRenderTestBounds {
...
public sealed class FabricaRenderTestControlType {
    public string ProgrammaticName = "";
}

public sealed class FabricaRenderTestCurrent {
    public string AutomationId = "";
    public FabricaRenderTestBounds BoundingRectangle = new FabricaRenderTestBounds();
    public string ClassName = "fake";
    public FabricaRenderTestControlType ControlType = new FabricaRenderTestControlType();
...
public sealed class FabricaRenderTestPatternCurrent {
    public bool IsSelected;
    public string Value = "";
}

public sealed class FabricaRenderTestPattern {
    public FabricaRenderTestPatternCurrent Current = new FabricaRenderTestPatternCurrent();
}

public sealed class FabricaRenderTestCollection : IEnumerable {
    private readonly FabricaRenderTestElement[] values;

    public FabricaRenderTestCollection(FabricaRenderTestElement[] values) {
        this.values = values;
    }
    ...
    public FabricaRenderTestElement Item(int index) {
        return values[index];
    }
...
public sealed class FabricaRenderTestElement {
    public readonly List<FabricaRenderTestElement> Children =
        new List<FabricaRenderTestElement>();
    public FabricaRenderTestCounter Counter = new FabricaRenderTestCounter();
    public readonly FabricaRenderTestCurrent Current = new FabricaRenderTestCurrent();
    public bool FailFindAll;
    public int RuntimeIdValue;
    public string ValueText = "";

    public FabricaRenderTestCollection FindAll(object scope, object condition) {
        Counter.FindAll++;
        if (FailFindAll) {
            throw new System.InvalidOperationException("defunct node");
        }
        return new FabricaRenderTestCollection(Children.ToArray());
    }

    public FabricaRenderTestPattern GetCurrentPattern(object pattern) {
        FabricaRenderTestPattern result = new FabricaRenderTestPattern();
...
function New-TestCounter {
    New-Object -TypeName FabricaRenderTestCounter
}
...
function New-TestElement {
    ...
    $element = New-Object -TypeName FabricaRenderTestElement
...
$operationPath = Join-Path ([IO.Path]::GetTempPath()) ("fabrica-runtime-render-test-" + [guid]::NewGuid() + ".json")
try {
    ...
    $tree = Render-FabricaTree $root $null
