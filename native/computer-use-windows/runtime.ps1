...
public static class FabricaDesktopWin32 {
...
function Write-FabricaJson($Payload) {
    $Payload | ConvertTo-Json -Depth 100 -Compress
}

function New-FabricaFrame([double]$X, [double]$Y, [double]$Width, [double]$Height) {
    if ($Width -le 0 -or $Height -le 0) { return $null }
    [pscustomobject]@{ x = $X; y = $Y; width = $Width; height = $Height }
}

function Read-FabricaOperation([string]$Path) {
    Get-Content -Raw -Encoding UTF8 -Path $Path | ConvertFrom-Json
}

function ConvertTo-FabricaLParam([int]$X, [int]$Y) {
    [IntPtr]((($Y -band 0xffff) -shl 16) -bor ($X -band 0xffff))
}

function ConvertTo-FabricaWheelParam([int]$Delta) {
    [IntPtr](($Delta -band 0xffff) -shl 16)
}

function Get-FabricaWindowProcesses {
    @(Get-Process | Where-Object { $_.MainWindowHandle -ne 0 } | Sort-Object ProcessName, Id)
}

function Find-FabricaProcess([string]$Query) {
    $needle = ""
...
function Assert-FabricaProcessAllowed($Process) {
...
function Test-FabricaBrowserProcess($Process) {
...
function Get-FabricaRootElement($Process) {
...
function Get-FabricaWindowFrame($Process, $RootElement) {
    $rect = New-Object FabricaDesktopWin32+RECT
    if ([FabricaDesktopWin32]::GetWindowRect([IntPtr]$Process.MainWindowHandle, [ref]$rect)) {
        return New-FabricaFrame $rect.Left $rect.Top ($rect.Right - $rect.Left) ($rect.Bottom - $rect.Top)
    }

    try {
        $bounds = $RootElement.Current.BoundingRectangle
        if (-not $bounds.IsEmpty) {
            return New-FabricaFrame $bounds.X $bounds.Y $bounds.Width $bounds.Height
        }
    } catch {}
    $null
}

function Get-FabricaWindowId($Process) {
    [int64]$Process.MainWindowHandle
}

function Get-FabricaAppName($Process) {
...
function New-FabricaAppRecord($Process) {
    [pscustomobject]@{
        name = Get-FabricaAppName $Process
...
function Assert-FabricaWindowTarget($Process, $WindowId, $WindowIndex) {
    if ($null -ne $WindowIndex -and [int]$WindowIndex -ne 0) {
        throw "windowNotFound(`"$WindowIndex`")"
    }
    if ($null -ne $WindowId -and [int64]$WindowId -ne (Get-FabricaWindowId $Process)) {
        throw "windowNotFound(`"$WindowId`")"
    }
}

function Restore-FabricaWindow($Process) {
    if ($Process.MainWindowHandle -eq 0) { return }
    [void][FabricaDesktopWin32]::ShowWindow([IntPtr]$Process.MainWindowHandle, 9)
    [void][FabricaDesktopWin32]::SetForegroundWindow([IntPtr]$Process.MainWindowHandle)
}

function Test-FabricaWindowFocused([IntPtr]$WindowHandle) {
    [FabricaDesktopWin32]::GetForegroundWindow() -eq $WindowHandle
}

function Wait-FabricaWindowFocused([IntPtr]$WindowHandle, [int]$TimeoutMilliseconds) {
    $stopwatch = [Diagnostics.Stopwatch]::StartNew()
    while ($stopwatch.ElapsedMilliseconds -lt $TimeoutMilliseconds) {
        if (Test-FabricaWindowFocused $WindowHandle) { return $true }
        Start-Sleep -Milliseconds 50
    }
    Test-FabricaWindowFocused $WindowHandle
}

function Assert-FabricaKeyboardFocus([IntPtr]$WindowHandle, $Operation) {
    if (Test-FabricaWindowFocused $WindowHandle) { return }
    if ([bool]$Operation.restoreWindow) {
        if (Wait-FabricaWindowFocused $WindowHandle 500) { return }
...
function Get-FabricaElementFrame($Element, $WindowFrame) {
    try {
        ...
        New-FabricaFrame ($bounds.X - $WindowFrame.x) ($bounds.Y - $WindowFrame.y) $bounds.Width $bounds.Height
    } catch {
        $null
    }
}

function Get-FabricaProperty($Element, [string]$Name) {
    try { [string]$Element.Current.$Name } catch { "" }
}

function Get-FabricaRuntimeId($Element) {
    try { @($Element.GetRuntimeId()) } catch { @() }
}

function Test-FabricaSensitiveElement($Element) {
    try {
        if ($Element.Current.IsPassword) { return $true }
    } catch {}
    $controlType = try { [string]$Element.Current.ControlType.ProgrammaticName } catch { "" }
    $parts = @(
        (Get-FabricaProperty $Element "LocalizedControlType"),
        $controlType,
        (Get-FabricaProperty $Element "Name"),
        (Get-FabricaProperty $Element "AutomationId"),
        (Get-FabricaProperty $Element "ClassName")
    )
...
function Get-FabricaValueText($Element) {
    try {
        if (Test-FabricaSensitiveElement $Element) { return "[redacted]" }
...
function Get-FabricaActions($Element) {
...
function Get-FabricaMeaningfulActions($Actions) {
    $noisy = @("Invoke", "ScrollToVisible", "ShowMenu")
    @($Actions | Where-Object { $noisy -notcontains $_ })
}

function Format-FabricaSnapshotText([string]$Text) {
    if ([string]::IsNullOrWhiteSpace($Text)) { return "" }
    (($Text -replace "\s+", " ").Trim())
}

function Format-FabricaValueSegment([string]$RoleKey, [string]$Title, [string]$Value) {
    $clean = Format-FabricaSnapshotText $Value
...
function Test-FabricaSuppressChildren([string]$RoleKey, [string]$Title, [string]$Value, [string]$Summary) {
    $hasCompactLabel = -not [string]::IsNullOrWhiteSpace($Title) -or -not [string]::IsNullOrWhiteSpace((Format-FabricaSnapshotText $Value)) -or -not [string]::IsNullOrWhiteSpace((Format-FabricaSnapshotText $Summary))
...
function Get-FabricaTextSnippets($Element, [int]$Limit = 6, [int]$MaxDepth = 3) {
    $values = New-Object System.Collections.Generic.List[string]
    $seen = New-Object System.Collections.Generic.HashSet[string]

    function Visit-FabricaText($Node, [int]$Depth) {
        ...
        if ($role -match "text|link|label") {
            foreach ($raw in @((Get-FabricaProperty $Node "Name"), (Get-FabricaValueText $Node))) {
        ...
        try {
            ...
            for ($i = 0; $i -lt $children.Count; $i++) {
                Visit-FabricaText $children.Item($i) ($Depth + 1)
                if ($values.Count -ge $Limit) { return }
            }
        } catch {}
    }

    Visit-FabricaText $Element 0
    @($values.ToArray())
}

function Test-FabricaPlainTextSubtree($Element, [int]$MaxDepth = 4) {
    $script:sawFabricaText = $false
    $allowed = @("pane", "group", "custom", "unknown", "text", "link", "image")

    function Visit-FabricaPlainText($Node, [int]$Depth) {
        if ($Depth -gt $MaxDepth) { return $false }
        $role = try { [string]$Node.Current.LocalizedControlType } catch { "" }
        $roleKey = $role.ToLowerInvariant()
        if ($allowed -notcontains $roleKey) { return $false }
        if ($roleKey -match "text|link") { $script:sawFabricaText = $true }
        if (@(Get-FabricaMeaningfulActions @(Get-FabricaActions $Node)).Count -gt 0) { return $false }
        try {
            $children = $Node.FindAll([Windows.Automation.TreeScope]::Children, [Windows.Automation.Condition]::TrueCondition)
            for ($i = 0; $i -lt $children.Count; $i++) {
                if (-not (Visit-FabricaPlainText $children.Item($i) ($Depth + 1))) { return $false }
            }
        } catch {}
        return $true
    }

    (Visit-FabricaPlainText $Element 0) -and $script:sawFabricaText
}

function New-FabricaElementRecord($Element, [int]$Index, $WindowFrame) {
    $controlType = try { [string]$Element.Current.ControlType.ProgrammaticName } catch { "" }
    $nativeWindowHandle = try { [int64]$Element.Current.NativeWindowHandle } catch { 0 }
    [pscustomobject]@{
        index = $Index
        runtimeId = @(Get-FabricaRuntimeId $Element)
        automationId = Get-FabricaProperty $Element "AutomationId"
        name = Get-FabricaProperty $Element "Name"
        controlType = $controlType
        localizedControlType = Get-FabricaProperty $Element "LocalizedControlType"
        className = Get-FabricaProperty $Element "ClassName"
        value = Get-FabricaValueText $Element
        isSelected = Test-FabricaElementSelected $Element
        nativeWindowHandle = $nativeWindowHandle
        frame = Get-FabricaElementFrame $Element $WindowFrame
        actions = @(Get-FabricaActions $Element)
    }
}

function Test-FabricaElementSelected($Element) {
    try {
...
function Render-FabricaTree($RootElement, $WindowFrame, [bool]$CompactBrowserTabs = $false) {
    ...
    function Visit-FabricaNode($Node, [int]$Depth) {
        ...
        $record = New-FabricaElementRecord $Node $records.Count $WindowFrame
        $children = @()
        try {
            $children = @($Node.FindAll([Windows.Automation.TreeScope]::Children, [Windows.Automation.Condition]::TrueCondition))
        } catch {}
        $meaningfulActions = @(Get-FabricaMeaningfulActions $record.actions)
        ...
        if (($roleKey -in @("pane", "group", "custom", "unknown")) -and [string]::IsNullOrWhiteSpace($title) -and [string]::IsNullOrWhiteSpace($record.value)) {
            $snippets = @(Get-FabricaTextSnippets $Node 8 4)
            if ($snippets.Count -ge 2 -and (Test-FabricaPlainTextSubtree $Node)) {
        ...
        if (($roleKey -in @("pane", "group", "custom", "unknown")) -and [string]::IsNullOrWhiteSpace($title) -and [string]::IsNullOrWhiteSpace($record.value) -and $meaningfulActions.Count -eq 0 -and $null -eq $genericSummary -and $children.Count -le 1) {
            for ($i = 0; $i -lt $children.Count; $i++) {
                Visit-FabricaNode $children.Item($i) $Depth
            }
            return
        }
        ...
        $line = "$($record.index) $role $(Format-FabricaSnapshotText $title)".TrimEnd()
        $line += Format-FabricaValueSegment $roleKey $title $record.value
        if (-not [string]::IsNullOrWhiteSpace($genericSummary) -and $genericSummary -ne $title) {
            $line += ", Text: " + (Format-FabricaSnapshotText $genericSummary)
        } elseif ($roleKey -in @("row", "data item", "list item")) {
            $rowSummary = @((Get-FabricaTextSnippets $Node 6 3)) -join " "
            if (-not [string]::IsNullOrWhiteSpace($rowSummary) -and $rowSummary -ne $title) {
                $line += ", Text: " + (Format-FabricaSnapshotText $rowSummary)
            }
        }
        ...
        if (-not [string]::IsNullOrWhiteSpace($genericSummary) -or (Test-FabricaSuppressChildren $roleKey $title $record.value $genericSummary)) { return }
        $childLineStart = $lines.Count
        for ($i = 0; $i -lt $children.Count; $i++) {
            Visit-FabricaNode $children.Item($i) ($Depth + 1)
        }
        if ($CompactBrowserTabs) {
            Compress-FabricaRenderedBrowserTabs $records $lines $childLineStart ($Depth + 1)
        }
    }

    Visit-FabricaNode $RootElement 0
    [pscustomobject]@{ elements = @($records.ToArray()); lines = @($lines.ToArray()); truncation = $truncation }
}

function Compress-FabricaRenderedBrowserTabs($Records, $Lines, [int]$StartLine, [int]$Depth) {
    $tabLineIndexes = New-Object System.Collections.Generic.List[int]
    for ($lineIndex = $StartLine; $lineIndex -lt $Lines.Count; $lineIndex++) {
        if (Test-FabricaDirectRenderedBrowserTabLine ([string]$Lines[$lineIndex]) $Depth) {
    ...
    foreach ($lineIndex in $tabLineIndexes) {
        if (Test-FabricaActiveRenderedBrowserTabLine ([string]$Lines[$lineIndex]) $Depth $recordsByIndex) {
    ...
    for ($i = $tabLineIndexes.Count - 1; $i -ge 0; $i--) {
        ...
        $recordIndex = Get-FabricaRenderedElementIndex ([string]$Lines[$lineIndex]) $Depth
...
function Test-FabricaDirectRenderedBrowserTabLine([string]$Line, [int]$Depth) {
    $indent = "`t" * $Depth
...
function Test-FabricaActiveRenderedBrowserTabLine([string]$Line, [int]$Depth, $RecordsByIndex) {
    if ($Line.Contains("(selected")) { return $true }
    $recordIndex = Get-FabricaRenderedElementIndex $Line $Depth
    ...
    [bool]$record.isSelected -or (Format-FabricaSnapshotText $record.value) -eq "1"
}

function Get-FabricaRenderedElementIndex([string]$Line, [int]$Depth) {
    $text = $Line.Substring(("`t" * $Depth).Length)
    if ($text -match "^(\d+)") { return [int]$Matches[1] }
    $null
}

function ConvertTo-FabricaPngBytes([System.Drawing.Image]$Image) {
    $stream = $null
    try {
...
function New-FabricaScreenshotPayload([byte[]]$Bytes, [int]$Width, [int]$Height, [double]$Scale) {
    [pscustomobject]@{
...
function Resize-FabricaBitmap([System.Drawing.Bitmap]$Source, [int]$Width, [int]$Height) {
    $resized = $null
    $graphics = $null
    try {
...
function Get-FabricaBoundedScreenshotPayload([System.Drawing.Bitmap]$Bitmap) {
    $originalWidth = [int][Math]::Max(1, $Bitmap.Width)
    $originalHeight = [int][Math]::Max(1, $Bitmap.Height)
    $pngBytes = ConvertTo-FabricaPngBytes $Bitmap
    if ($pngBytes.Length -le $MaxScreenshotPngBytes) {
        return New-FabricaScreenshotPayload $pngBytes $originalWidth $originalHeight 1.0
    }
    ...
    while ($scale -ge $MinScreenshotScale) {
        ...
        $resized = $null
        try {
            $resized = Resize-FabricaBitmap $Bitmap $width $height
            $candidateBytes = ConvertTo-FabricaPngBytes $resized
            if ($candidateBytes.Length -le $MaxScreenshotPngBytes) {
                return New-FabricaScreenshotPayload $candidateBytes $width $height ($width / [double]$originalWidth)
            }
        } finally {
            if ($null -ne $resized) { $resized.Dispose() }
        }
...
function Get-FabricaScreenshot([bool]$IncludeScreenshot, $WindowFrame) {
    ...
    try {
        ...
        Get-FabricaBoundedScreenshotPayload $bitmap
    } catch {
        $null
    } finally {
        if ($null -ne $graphics) { $graphics.Dispose() }
...
function New-FabricaSnapshot([string]$Query, [bool]$IncludeScreenshot, $WindowId = $null, $WindowIndex = $null, [bool]$RestoreWindow = $false) {
    $process = Find-FabricaProcess $Query
    if ($RestoreWindow) { Restore-FabricaWindow $process }
    Assert-FabricaWindowTarget $process $WindowId $WindowIndex
    $root = Get-FabricaRootElement $process
    $windowFrame = Get-FabricaWindowFrame $process $root
    $tree = Render-FabricaTree $root $windowFrame (Test-FabricaBrowserProcess $process)
    $screenshot = Get-FabricaScreenshot $IncludeScreenshot $windowFrame

    [pscustomobject]@{
        snapshotId = [guid]::NewGuid().ToString()
        app = New-FabricaAppRecord $process
        windowTitle = $process.MainWindowTitle
        windowId = Get-FabricaWindowId $process
...
function Get-FabricaAppList {
    @(Get-FabricaWindowProcesses | ForEach-Object {
        New-FabricaAppRecord $_
    })
}

function Get-FabricaWindowList([string]$Query) {
    $process = Find-FabricaProcess $Query
    $root = Get-FabricaRootElement $process
    $windowFrame = Get-FabricaWindowFrame $process $root
    $x = $null
    ...
    [pscustomobject]@{
        ...
        windows = @([pscustomobject]@{
            index = 0
            app = $app
            id = Get-FabricaWindowId $process
            ...
            isOffscreen = $false
            screenIndex = $null
            platform = [pscustomobject]@{ backend = "uia"; nativeWindowHandle = Get-FabricaWindowId $process }
        })
    }
}

function Get-FabricaHandshake {
    [pscustomobject]@{
        platform = "win32"
...
function Test-FabricaSameRuntimeId($Left, $Right) {
...
function Find-FabricaElement($RootElement, $Record) {
    ...
    try {
        ...
        for ($i = 0; $i -lt $descendants.Count; $i++) {
            ...
            if (Test-FabricaSameRuntimeId @($candidate.GetRuntimeId()) @($Record.runtimeId)) {
                return $candidate
            }
        }
    } catch {}
...
function Invoke-FabricaPrimaryAction($Element) {
    foreach ($pattern in @(
...
function Invoke-FabricaNamedAction($Element, [string]$Action) {
    $wanted = ""
...
function Set-FabricaElementValue($Element, [string]$Value) {
    try {
        $pattern = $Element.GetCurrentPattern([Windows.Automation.ValuePattern]::Pattern)
...
function Get-FabricaRequiredNumber($Value, [string]$Name) {
...
function Get-FabricaPositiveInteger($Value, [string]$Name) {
...
function Get-FabricaPositiveNumber($Value, [string]$Name) {
    if ($null -eq $Value) { $Value = 1 }
    $number = Get-FabricaRequiredNumber $Value $Name
    if ($number -le 0) { throw "$Name must be a positive number" }
    $number
}

function Get-FabricaRequiredString($Value, [string]$Name) {
    if ($null -eq $Value) { throw "$Name is required" }
    $text = [string]$Value
...
function Get-FabricaScreenPoint($Operation, $WindowFrame) {
    if ($null -ne $Operation.element) {
        throw "stale element frame; run get-app-state again and use a fresh element index"
    }
    $x = Get-FabricaRequiredNumber $Operation.x "x"
    $y = Get-FabricaRequiredNumber $Operation.y "y"
    @{
...
function Get-FabricaElementScreenPoint($Element) {
...
function Send-FabricaMouseClick([IntPtr]$WindowHandle, [int]$ScreenX, [int]$ScreenY, [string]$Button, [int]$Count, [string]$Modifiers) {
    [void][FabricaDesktopWin32]::SetForegroundWindow($WindowHandle)
    [void][FabricaDesktopWin32]::SetCursorPos($ScreenX, $ScreenY)
    ...
    $modifierKeys = @(Get-FabricaClickModifierVirtualKeys $Modifiers)
    $clickCount = Get-FabricaPositiveInteger $Count "click_count"
    if ($modifierKeys.Count -eq 0) {
        for ($i = 0; $i -lt $clickCount; $i++) {
            [FabricaDesktopWin32]::mouse_event($down, 0, 0, 0, [UIntPtr]::Zero)
            Start-Sleep -Milliseconds 35
            [FabricaDesktopWin32]::mouse_event($up, 0, 0, 0, [UIntPtr]::Zero)
        }
        return
    }
    for ($i = 0; $i -lt $clickCount; $i++) {
        [FabricaDesktopWin32]::SendModifiedClick(
            [byte[]]$modifierKeys,
            [uint32]$down,
            [uint32]$up
        )
...
function Send-FabricaDrag([IntPtr]$WindowHandle, $From, $To) {
    [void][FabricaDesktopWin32]::SetForegroundWindow($WindowHandle)
    $startX = [int]$From.x
    $startY = [int]$From.y
    $endX = [int]$To.x
    $endY = [int]$To.y
    [void][FabricaDesktopWin32]::SetCursorPos($startX, $startY)
    [FabricaDesktopWin32]::mouse_event($MouseEvents.LeftDown, 0, 0, 0, [UIntPtr]::Zero)
    for ($step = 1; $step -le 12; $step++) {
        $x = [int][Math]::Round($startX + (($endX - $startX) * $step / 12))
        $y = [int][Math]::Round($startY + (($endY - $startY) * $step / 12))
        [void][FabricaDesktopWin32]::SetCursorPos($x, $y)
        Start-Sleep -Milliseconds 20
    }
    [FabricaDesktopWin32]::mouse_event($MouseEvents.LeftUp, 0, 0, 0, [UIntPtr]::Zero)
}

function Send-FabricaText([IntPtr]$WindowHandle, [string]$Text) {
    [void][FabricaDesktopWin32]::SetForegroundWindow($WindowHandle)
    $hasNonAscii = $false
    ...
    if ($hasNonAscii) {
        foreach ($character in $Text.ToCharArray()) {
            [void][FabricaDesktopWin32]::PostMessage($WindowHandle, $WindowsMessages.Char, [IntPtr][int][char]$character, [IntPtr]::Zero)
            Start-Sleep -Milliseconds 8
        }
        return
    }
    [System.Windows.Forms.SendKeys]::SendWait((ConvertTo-FabricaSendKeysText $Text))
}

function Get-FabricaVirtualKey([string]$Key) {
...
function Send-FabricaKey([IntPtr]$WindowHandle, [string]$Key) {
    [void][FabricaDesktopWin32]::SetForegroundWindow($WindowHandle)
    [System.Windows.Forms.SendKeys]::SendWait((ConvertTo-FabricaSendKeysKey $Key))
}

function Get-FabricaModifierVirtualKey([string]$Modifier) {
...
function Get-FabricaClickModifierVirtualKeys([string]$Modifiers) {
    ...
    @($parts | ForEach-Object { Get-FabricaModifierVirtualKey $_ })
}

function Send-FabricaHotkey([IntPtr]$WindowHandle, [string]$KeySpec) {
    ...
    if ($parts.Count -gt 1) {
        foreach ($modifier in $parts[0..($parts.Count - 2)]) {
            $prefix += ConvertTo-FabricaSendKeysModifier $modifier
        }
    }
    [void][FabricaDesktopWin32]::SetForegroundWindow($WindowHandle)
    [System.Windows.Forms.SendKeys]::SendWait($prefix + (ConvertTo-FabricaSendKeysKey $key))
}

function ConvertTo-FabricaSendKeysText([string]$Text) {
...
function ConvertTo-FabricaSendKeysKey([string]$Key) {
    switch ($Key.ToLowerInvariant()) {
        ...
        default {
            if ($Key.Length -eq 1) { return (ConvertTo-FabricaSendKeysText $Key) }
            throw "Unsupported key: $Key"
        }
    }
}

function ConvertTo-FabricaSendKeysModifier([string]$Modifier) {
    switch ($Modifier.ToLowerInvariant()) {
...
function Send-FabricaPasteText([IntPtr]$WindowHandle, [string]$Text) {
    $previous = $null
    $hadPrevious = $false
    ...
    try {
        ...
        Send-FabricaHotkey $WindowHandle "Ctrl+v"
    } finally {
        if ($hadPrevious) {
...
function Invoke-FabricaOperation($Operation) {
    $includeScreenshot = -not [bool]$Operation.noScreenshot
    if ($Operation.tool -eq "handshake") {
        return [pscustomobject]@{ ok = $true; capabilities = Get-FabricaHandshake }
    }
    if ($Operation.tool -eq "list_apps") {
        return [pscustomobject]@{ ok = $true; apps = @(Get-FabricaAppList) }
    }
    if ($Operation.tool -eq "list_windows") {
        $list = Get-FabricaWindowList $Operation.app
        return [pscustomobject]@{ ok = $true; app = $list.app; windows = @($list.windows) }
    }
    if ($Operation.tool -eq "get_app_state") {
        return [pscustomobject]@{ ok = $true; snapshot = New-FabricaSnapshot $Operation.app $includeScreenshot $Operation.windowId $Operation.windowIndex ([bool]$Operation.restoreWindow) }
    }

    $process = Find-FabricaProcess $Operation.app
    if ([bool]$Operation.restoreWindow) { Restore-FabricaWindow $process }
    Assert-FabricaWindowTarget $process $Operation.windowId $Operation.windowIndex
    $root = Get-FabricaRootElement $process
    $windowFrame = if ($null -ne $Operation.windowBounds) { $Operation.windowBounds } else { Get-FabricaWindowFrame $process $root }
    $element = Find-FabricaElement $root $Operation.element
    $fromElement = Find-FabricaElement $root $Operation.fromElement
    $toElement = Find-FabricaElement $root $Operation.toElement
    $handle = [IntPtr]$process.MainWindowHandle
    if ($Operation.tool -in @("type_text", "press_key", "hotkey", "paste_text")) {
        Assert-FabricaKeyboardFocus $handle $Operation
    }
    $action = $null
    ...
    switch ($Operation.tool) {
        "click" {
            ...
            Restore-FabricaWindow $process
            $handledByPattern = $false
            $clickCount = Get-FabricaPositiveInteger $Operation.click_count "click_count"
            ...
            if (-not $hasModifiers -and $null -ne $element -and $Operation.mouse_button -ne "right" -and $Operation.mouse_button -ne "middle" -and $clickCount -le 1) {
                $handledByPattern = Invoke-FabricaPrimaryAction $element
            }
            if (-not $handledByPattern) {
                $point = Get-FabricaElementScreenPoint $element
                if ($null -eq $point) { $point = Get-FabricaScreenPoint $Operation $windowFrame }
                Send-FabricaMouseClick $handle $point.x $point.y $Operation.mouse_button $clickCount $Operation.modifiers
        ...
        "perform_secondary_action" {
            ...
            if (-not (Invoke-FabricaNamedAction $element $Operation.action)) {
        ...
        "scroll" {
            $delta = 120 * [int][Math]::Ceiling((Get-FabricaPositiveNumber $Operation.pages "pages"))
            ...
            $point = Get-FabricaElementScreenPoint $element
            if ($null -eq $point) { $point = Get-FabricaScreenPoint $Operation $windowFrame }
            [void][FabricaDesktopWin32]::SetForegroundWindow($handle)
            [void][FabricaDesktopWin32]::SetCursorPos([int]$point.x, [int]$point.y)
            [FabricaDesktopWin32]::mouse_event($MouseEvents.Wheel, 0, 0, $delta, [UIntPtr]::Zero)
            $action = [pscustomobject]@{ path = "synthetic"; actionName = "scroll"; fallbackReason = $null }
        }
        "drag" {
            $from = Get-FabricaElementScreenPoint $fromElement
            ...
            if ($null -eq $from) {
                $from = @{
                    x = $windowFrame.x + (Get-FabricaRequiredNumber $Operation.from_x "from_x")
                    y = $windowFrame.y + (Get-FabricaRequiredNumber $Operation.from_y "from_y")
                }
            }
            $to = Get-FabricaElementScreenPoint $toElement
            ...
            if ($null -eq $to) {
                $to = @{
                    x = $windowFrame.x + (Get-FabricaRequiredNumber $Operation.to_x "to_x")
                    y = $windowFrame.y + (Get-FabricaRequiredNumber $Operation.to_y "to_y")
                }
            }
            Send-FabricaDrag $handle $from $to
            $action = [pscustomobject]@{ path = "synthetic"; actionName = "drag"; fallbackReason = $null }
        }
        "type_text" {
            Send-FabricaText $handle (Get-FabricaRequiredString $Operation.text "text")
        ...
        "press_key" {
            Send-FabricaKey $handle (Get-FabricaRequiredString $Operation.key "key")
        ...
        "hotkey" {
            Send-FabricaHotkey $handle (Get-FabricaRequiredString $Operation.key "key")
        ...
        "paste_text" {
            Send-FabricaPasteText $handle (Get-FabricaRequiredString $Operation.text "text")
        ...
        "set_value" {
            if ($null -eq $element -or -not (Set-FabricaElementValue $element ([string]$Operation.value))) {
    ...
    try {
        $snapshot = New-FabricaSnapshot $Operation.app $includeScreenshot $Operation.windowId $Operation.windowIndex
    } catch {
        ...
        $snapshot = New-FabricaSnapshot $Operation.app $includeScreenshot $null $null
    }
    [pscustomobject]@{ ok = $true; action = $action; snapshot = $snapshot }
}

try {
    $operation = Read-FabricaOperation $OperationPath
    Write-FabricaJson (Invoke-FabricaOperation $operation)
} catch {
    Write-FabricaJson ([pscustomobject]@{ ok = $false; error = [string]$_.Exception.Message })
}
