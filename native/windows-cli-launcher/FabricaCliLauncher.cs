...
internal static class FabricaCliLauncher
{
    private static int Main(string[] args)
    {
        try
        {
            string launcherDirectory = Path.GetDirectoryName(typeof(FabricaCliLauncher).Assembly.Location);
            ...
            {
                Console.Error.WriteLine("Unable to locate Fabrica.exe next to \"{0}\"", resourcesDirectory);
                return 1;
            }

            if (!File.Exists(cliPath))
            {
                Console.Error.WriteLine("Unable to locate the Fabrica CLI entrypoint at \"{0}\"", cliPath);
                return 1;
            }
        ...
        {
            Console.Error.WriteLine("Unable to start the Fabrica CLI: {0}", error.Message);
            return 1;
        }
    }
