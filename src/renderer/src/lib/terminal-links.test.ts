...
describe('terminal path helpers', () => {
  ...
  describe('extractTerminalFileLinks bare-filename tokens', () => {
    it('does not treat regular URL hosts as local file paths', () => {
      expect(
        extractTerminalFileLinks(
          'PR opened: https://github.com/Auto-Scalers/Fabrica-marketing-website/pull/82'
        )
      ).toEqual([])
    })
  ...
  describe('plain-text file:// URIs', () => {
    it('extracts a printed file:// URI as a file link resolving to its path', () => {
      const line = 'Report: file:///Users/dev/FABRICA/report.html'
      const link = extractTerminalFileLinks(line).find(
        (candidate) => candidate.displayText === 'file:///Users/dev/FABRICA/report.html'
      )
      expect(link).toMatchObject({ pathText: '/Users/dev/FABRICA/report.html' })
      expect(resolveTerminalFileLink(link!, '/Users/dev/FABRICA')).toEqual({
        absolutePath: '/Users/dev/FABRICA/report.html',
        line: null,
        column: null
      })
    })
    ...
    it('does not also emit a bare-path link for the URI body', () => {
      const links = extractTerminalFileLinks('file:///Users/dev/FABRICA/report.html')
      expect(links.map((link) => link.displayText)).toEqual(['file:///Users/dev/FABRICA/report.html'])
    })
