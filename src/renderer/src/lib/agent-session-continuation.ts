...
export function buildAgentSessionContinuationPrompt(
  ...
  const sourceLines = [
    ...
    source.sourceLabel ? `Fabrica pane: ${source.sourceLabel}` : null,
  ...
  return [
    'Continue work from the prior Fabrica session using the context below.',
    ...
    ...(statusHints.length > 0 ? ['', 'Latest Fabrica status hints:', ...statusHints] : []),
    '',
