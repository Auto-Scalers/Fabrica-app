...
import type { FABRICAHooks } from '../../../shared/types'
import { resolveHookCommandSourcePolicy } from '../../../shared/hook-command-source-policy'
import { hashFABRICAHookScript, type FABRICAHookScriptKind } from './fabrica-hook-trust'
import {
  checkRuntimeHooks,
...
export type HookScriptKind = FABRICAHookScriptKind
...
function getSetupTrustContent(yamlHooks: FABRICAHooks | null): string {
...
function getVmRecipeTrustContent(yamlHooks: FABRICAHooks | null): string {
...
function canUseRepoWideTrust(state: AppState, repoId: string): boolean {
  ...
  return Boolean(state.trustedFABRICAHooks[repoId]?.all) && !hasDuplicateRepoId
}
...
async function confirmScriptContent(
  ...
  const contentHash = await hashFABRICAHookScript(scriptContent)
  if (isCancelled()) {
    return 'skip'
  }
  const existingHash = state.trustedFABRICAHooks[repoId]?.[scriptKind]?.contentHash
  ...
  return new Promise<'run' | 'skip'>((resolve) => {
    state.openModal('confirm-FABRICA-yaml-hooks', {
      repoId,
      repoName,
      scriptKind,
      scriptContent,
  ...
  return enqueueTrustPrompt(async () => {
    ...
    try {
      if (scriptKind === 'issueCommand') {
        // Local overrides are user-owned; only shared FABRICA.yaml commands need repo trust.
        ...
        const yamlHooks = (result.hooks as FABRICAHooks | null) ?? null
        scriptContent =
          scriptKind === 'setup'
