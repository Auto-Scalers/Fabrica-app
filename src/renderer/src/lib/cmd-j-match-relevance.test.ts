...
describe('scorePaletteRelevance', () => {
  ...
  it('takes the best field when several matched', () => {
    expect(
      scorePaletteRelevance([
        { text: 'FABRICA', range: { start: 0, end: 4 }, tier: 2 },
        { text: 'FABRICA-main', range: { start: 0, end: 4 }, tier: 0 }
      ])
    ).toBe(scorePaletteRelevance([{ text: 'FABRICA-main', range: { start: 0, end: 4 }, tier: 0 }]))
  })
})
