...
export type LinearAgentSkillUpdateTarget = {
  skillName: typeof FABRICA_LINEAR_SKILL_NAME | typeof LINEAR_TICKETS_SKILL_NAME
  command: string
}
...
export function getLinearAgentSkillUpdateTarget(
  ...
  const canonicalSkillInstalled = hasInstalledAgentSkill(skills, FABRICA_LINEAR_SKILL_NAME, {
  ...
  return !installed || canonicalSkillInstalled || !legacySkillInstalled
    ? { skillName: FABRICA_LINEAR_SKILL_NAME, command: FABRICA_LINEAR_SKILL_UPDATE_COMMAND }
