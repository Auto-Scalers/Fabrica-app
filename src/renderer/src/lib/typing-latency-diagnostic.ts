 ...
 *
 *   window.__FABRICATypingDiagnostic.start()   // then type normally for ~20s
 *   window.__FABRICATypingDiagnostic.report()  // logs + returns a JSON-safe object
 *   window.__FABRICATypingDiagnostic.stop()
 *
...
function startProbe(): string {
  if (active) {
    return 'Typing diagnostic already running. Type for ~20s, then run __FABRICATypingDiagnostic.report().'
  }
  cacheAppVersion()
  ...
  active = state
  lastState = state
  return `Typing diagnostic started on ${state.panes.length} pane(s). Click into the agent terminal, type normally for ~20 seconds, then run __FABRICATypingDiagnostic.report().`
}
...
function stopProbe(): string {
  ...
  return 'Typing diagnostic stopped. Run __FABRICATypingDiagnostic.report() to read the last samples.'
}

function reportProbe(): TypingLatencyReport {
  const report = buildReport(active ?? lastState, active !== null)
  console.log('[FABRICA] typing latency diagnostic', report)
  return report
}
...
type TypingDiagnosticWindow = Window & { __FABRICATypingDiagnostic?: TypingDiagnosticBridge }
...
export function installTypingLatencyDiagnostic(): void {
  ...
  if (target.__FABRICATypingDiagnostic) {
    return
  }
  target.__FABRICATypingDiagnostic = { start: startProbe, stop: stopProbe, report: reportProbe }
}
