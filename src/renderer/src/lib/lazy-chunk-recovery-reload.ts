...
function waitForRefusedNavigation(win: Window): RefusedNavigationWait {
  ...
  const cancel = (): void => {
    ...
    win.removeEventListener(FABRICA_RENDERER_UNLOAD_PREVENTED_EVENT, onUnloadPrevented)
  }
  const outcome = new Promise<'unload-vetoed' | 'never-landed'>((resolve) => {
    ...
    win.addEventListener(FABRICA_RENDERER_UNLOAD_PREVENTED_EVENT, onUnloadPrevented)
...
export async function requestLazyChunkRecoveryReload(
  ...
  try {
    await prepareRendererForAppRestart(win, {
      startedEventName: FABRICA_APP_RESTART_STARTED_EVENT,
      abortedEventName: FABRICA_APP_RESTART_ABORTED_EVENT,
      awaitCheckpoint
    })
  } catch {
  ...
  try {
    ...
    win.dispatchEvent(new Event(FABRICA_APP_RESTART_ABORTED_EVENT))
  }
}
