...
describe('launchAgentInNewTab', () => {
  ...
  it('surfaces a toast when host agent launch fails in paired web clients', async () => {
    ...
    mockCreateWebRuntimeSessionTerminal.mockResolvedValue({
      status: 'failed',
      message: 'Upgrade the remote Fabrica host before starting or resuming agent sessions.'
    })
    store.settings = {
      agentCmdOverrides: {},
      agentDefaultArgs: {},
    ...
    expect(mockToastError).toHaveBeenCalledWith(
      'Upgrade the remote Fabrica host before starting or resuming agent sessions.'
    )
