...
describe('project-host workspace target resolution', () => {
  it('falls back to a local setup for a local-only repo', () => {
    ...
    expect(resolution).toMatchObject({
      status: 'ready',
      target: {
        projectId: 'repo:FABRICA',
        hostId: 'local',
        projectHostSetupId: 'FABRICA',
        repoId: 'FABRICA'
      }
    })
  })
  ...
  it('matches duplicate repo ids to the setup execution host', () => {
    const localRepo = makeRepo('FABRICA', { path: '/local/FABRICA' })
    const sshRepo = makeRepo('FABRICA', {
      path: '/remote/FABRICA',
      connectionId: 'builder'
    })
    const projects = [makeProject('github:Auto-Scalers/Fabrica-app', ['FABRICA'])]
    const projectHostSetups = [
      makeSetup('local-setup', 'github:Auto-Scalers/Fabrica-app', 'local', 'FABRICA'),
      makeSetup('ssh-setup', 'github:Auto-Scalers/Fabrica-app', 'ssh:builder', 'FABRICA')
    ]
    ...
    expect(resolution).toMatchObject({
      ...
      target: {
        hostId: 'ssh:builder',
        repo: { path: '/remote/FABRICA', connectionId: 'builder' }
      }
    })
  })
  ...
  it('keeps a focused duplicate repo id on its selected host', () => {
    const localRepo = makeRepo('FABRICA', { path: '/local/FABRICA' })
    const sshRepo = makeRepo('FABRICA', { path: '/remote/FABRICA', connectionId: 'builder' })
    const projects = [makeProject('github:Auto-Scalers/Fabrica-app', ['FABRICA'])]
    const projectHostSetups = [
      makeSetup('local-setup', 'github:Auto-Scalers/Fabrica-app', 'local', 'FABRICA'),
      makeSetup('ssh-setup', 'github:Auto-Scalers/Fabrica-app', 'ssh:builder', 'FABRICA')
  ...
  it('resolves an explicit project and host to the matching setup', () => {
    ...
    expect(resolution).toMatchObject({
      ...
      target: {
        projectId: 'github:Auto-Scalers/Fabrica-app',
        hostId: 'runtime:gpu-1',
        projectHostSetupId: 'FABRICA-runtime',
        repoId: 'FABRICA-runtime'
      }
    })
  })
  ...
  it('canonicalizes a stale same-host setup id to the setup the picker shows', () => {
    ...
    const resolution = resolveWorkspaceCreationTarget({
      ...
      projects,
      projectHostSetups,
      projectHostSetupId: 'FABRICA-worktree'
    })

    expect(resolution).toMatchObject({
      status: 'ready',
      target: { projectHostSetupId: 'FABRICA-main', repoId: 'FABRICA-main', hostId: 'local' }
    })
  })
  ...
  it('keeps an explicit setup id that is the only one on its host', () => {
    ...
    expect(
      resolveWorkspaceCreationTarget({
        ...
        projects,
        projectHostSetups,
        projectHostSetupId: 'FABRICA-ssh'
      })
    ).toMatchObject({
      status: 'ready',
      target: { projectHostSetupId: 'FABRICA-ssh', repoId: 'FABRICA-ssh', hostId: 'ssh:builder' }
    })
  })

  it('does not merge same-name repos without shared project identity', () => {
    const repos = [
      makeRepo('personal-FABRICA', { displayName: 'FABRICA' }),
      makeRepo('work-FABRICA', { displayName: 'FABRICA', connectionId: 'work-linux' })
    ]

    expect(
      resolveWorkspaceCreationRepoId({
        eligibleRepos: repos,
        projectId: 'repo:personal-FABRICA',
        focusedHostScope: 'ssh:work-linux'
      })
    ).toBe('personal-FABRICA')
  })
  ...
  it('reports setup-not-ready when the selected host has pending setup metadata', () => {
    ...
    const projectHostSetups = [
      makeSetup('FABRICA', 'github:Auto-Scalers/Fabrica-app', 'local', 'FABRICA'),
      makeSetup('gpu-pending', 'github:Auto-Scalers/Fabrica-app', 'runtime:gpu', '', {
        path: '',
  ...
  it('does not silently switch an explicit setup id to an actionable sibling host', () => {
    ...
    const projects = [makeProject('repo:FABRICA', ['remote-repo', 'local-repo'])]
    const projectHostSetups = [
      makeSetup('removed-setup', 'repo:FABRICA', 'ssh:removed', 'remote-repo'),
      makeSetup('local-setup', 'repo:FABRICA', 'local', 'local-repo')
    ]
