...
describe('new-workspace-composer-repo', () => {
  ...
  describe('resolveComposerActiveRepoId', () => {
    const localFABRICA = makeRepo('local-FABRICA', { upstream: { owner: 'Auto-Scalers', repo: 'fabrica' } })
    const runtimeFABRICA = makeRepo('runtime-FABRICA', {
      connectionId: 'runtime-ssh-FABRICA-1',
      upstream: { owner: 'Auto-Scalers', repo: 'fabrica' }
    })
    const otherProject = makeRepo('noqa', { upstream: { owner: 'Auto-Scalers', repo: 'noqa' } })
    const repos = [otherProject, localFABRICA, runtimeFABRICA]
    ...
    it('maps an active runtime-owned SSH repo to its local same-project sibling', () => {
      expect(resolveComposerActiveRepoId(repos, eligibleRepos, 'runtime-FABRICA')).toBe('local-FABRICA')
    })

    it('leaves a normal active repo unchanged', () => {
      expect(resolveComposerActiveRepoId(repos, eligibleRepos, 'local-FABRICA')).toBe('local-FABRICA')
    })

    it('keeps the runtime repo id when no same-project sibling is eligible', () => {
      const onlyRuntime = [runtimeFABRICA]
      expect(
        resolveComposerActiveRepoId(
          onlyRuntime,
