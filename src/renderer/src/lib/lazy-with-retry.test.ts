...
import { FABRICA_APP_RESTART_ABORTED_EVENT } from '../../../shared/updater-renderer-events'
import {
  FABRICA_EDITOR_PREPARE_HOT_EXIT_EVENT,
  type EditorPrepareHotExitDetail
} from '../../../shared/editor-save-events'

const RELOAD_GUARD_KEY = 'FABRICA:lazy-chunk-reload-attempted'
...
describe('loadLazyWithRetry recovery reload vs the dirty-editor-tab unload veto', () => {
  ...
  function installDirtyEditorTab(options: { hotExitBackupFails?: boolean } = {}): Harness {
    ...
    window.addEventListener(FABRICA_EDITOR_PREPARE_HOT_EXIT_EVENT, hotExitBackup)
    vi.spyOn(window.location, 'reload').mockImplementation(() => {
      ...
      if (!accepted) {
        window.dispatchEvent(new Event(FABRICA_RENDERER_UNLOAD_PREVENTED_EVENT))
      }
    })

    cleanupHarness = () => {
      window.removeEventListener('beforeunload', dirtyTabGuard)
      window.removeEventListener(FABRICA_EDITOR_PREPARE_HOT_EXIT_EVENT, hotExitBackup)
      cleanupBypass()
    }
    return harness
  }
  ...
  it('refuses to reload when unsaved buffers cannot be backed up', async () => {
    ...
    window.addEventListener(FABRICA_APP_RESTART_ABORTED_EVENT, restartAborted)
    const error = chunkParseError()
    ...
    window.removeEventListener(FABRICA_APP_RESTART_ABORTED_EVENT, restartAborted)
  })
