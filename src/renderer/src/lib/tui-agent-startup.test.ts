...
describe('buildAgentDraftLaunchPlan', () => {
  ...
  it('uses FABRICA_PI_PREFILL env var for pi (no CLI flag exists)', () => {
    // Why: pi has no `--prefill` flag, and bracketed-paste-after-ready races
    // against pi's lengthy startup output. The FABRICA overlay installs an
    // `FABRICA-prefill` extension that reads FABRICA_PI_PREFILL on session_start
    ...
    expect(
      ...
      agent: 'pi',
      launchCommand: 'pi; unset FABRICA_PI_PREFILL',
      expectedProcess: 'pi',
      env: { FABRICA_PI_PREFILL: 'https://github.com/acme/repo/issues/42' },
