...
describe('openHttpLink', () => {
  it('routes into Fabrica when openLinksInApp is on and a worktree is known', () => {
  ...
  it('routes floating workspace links into Fabrica without changing the active repo worktree', () => {
    storeState.settings = { openLinksInApp: true }
  ...
  it('forceInApp opens a local link in Fabrica when the setting is off', () => {
    storeState.settings = { openLinksInApp: false }
  ...
  it('does not force a remote link into the Fabrica browser', () => {
    storeState.settings = { openLinksInApp: false }
  ...
  it('keeps a runtime-owned link out of Fabrica when no runtime is globally active', () => {
  ...
  it('labels explicit local links from the local scan instead of a merged remote port', async () => {
    ...
    registerLocalhostLabelMock.mockResolvedValue({ url: 'http://wt-local.FABRICA.localhost:60016/' })
  ...
  it('labels localhost links from terminal output before opening the system browser', async () => {
    ...
    registerLocalhostLabelMock.mockResolvedValue({
      url: 'http://analytics.FABRICA.localhost:60016/episodes'
    })
    ...
    expect(openUrlMock).toHaveBeenCalledWith('http://analytics.FABRICA.localhost:60016/episodes')
  })
  ...
  it('resolves display URLs for labeled localhost links without opening them', async () => {
    ...
    registerLocalhostLabelMock.mockResolvedValue({
      url: 'http://snapstudio-main.FABRICA.localhost:60016/'
    })

    await expect(resolveLocalhostHttpLinkDisplayUrl('http://localhost:5180/')).resolves.toBe(
      'http://snapstudio-main.FABRICA.localhost:60016/'
    )
    expect(openUrlMock).not.toHaveBeenCalled()
    expect(createBrowserTabMock).not.toHaveBeenCalled()
  })
  ...
  it('labels a local-owned localhost link from the local scan', async () => {
    ...
    ).resolves.toBe('http://snapstudio-main.FABRICA.localhost:60016/')
  })
})

describe('openHttpLink modifier routing', () => {
  it('forces the system browser when inverting is off and links open in Fabrica', () => {
  ...
  it('opens in Fabrica when inverting is on and links open externally', () => {
  ...
  it('opens in the system browser when inverting is on and links open in Fabrica', () => {
  ...
  // Why: remote-owned links must never land in an FABRICA tab that cannot reach them.
  it('never routes a remote source into Fabrica even when inverting', () => {
    storeState.settings = { openLinksInApp: false, openLinksInAppModifierInverts: true }
