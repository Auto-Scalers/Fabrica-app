...
function createTestState(overrides?: Partial<AppState>): {
  ...
  const trust: PersistedTrustedFABRICAHooks = {}
  const state = {
    trustedFABRICAHooks: trust,
...
describe('ensureHooksConfirmed', () => {
  ...
  it('short-circuits to run when the persisted content hash matches the current script', async () => {
    ...
    const hash = await hashFABRICAHookScript(script)
    state.trustedFABRICAHooks['repo-1'] = {
  ...
  it('re-prompts when the script content differs from the persisted hash', async () => {
    const { state, pending } = createTestState()
    const staleHash = await hashFABRICAHookScript('old script')
    state.trustedFABRICAHooks['repo-1'] = {
    ...
    // FABRICA.yaml changed, not because they've never approved this hook.
  ...
  it('includes default tab commands in the setup trust prompt', async () => {
    ...
    expect(pending[0].data.contentHash).toBe(await hashFABRICAHookScript(expectedContent))
  ...
  it('inspects the requested host when duplicate repo ids exist', async () => {
    const { state } = createTestState({
      settings: { activeRuntimeEnvironmentId: 'env-1' },
      trustedFABRICAHooks: { 'repo-1': { all: { approvedAt: 1 } } },
      repos: [
  ...
  it('does not prompt for FABRICA.yaml when the repo uses local commands only', async () => {
    const { state, pending } = createTestState({
      repos: [
        {
          id: 'repo-1',
  ...
  it('does not prompt for FABRICA.yaml when local commands are the implicit default', async () => {
    const { state, pending } = createTestState({
      repos: [
        {
          id: 'repo-1',
  ...
  it('opens a modal with the computed content hash and resolves with the user decision', async () => {
    ...
    expect(pending[0].data).toMatchObject({
      ...
      contentHash: await hashFABRICAHookScript('pnpm install'),
