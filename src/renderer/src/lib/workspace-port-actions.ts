...
export function shouldOpenWorkspacePortInFABRICABrowser(
...
export function resolvePortOpenInFABRICABrowser({
  settings,
  event,
  isMac
}: {
  ...
  return shouldOpenWorkspacePortInFABRICABrowser(settings)
}
...
export async function openWorkspacePortInBrowser(args: {
  ...
  if (args.openInFABRICABrowser === false && args.runtimeTarget.kind === 'local') {
    try {
