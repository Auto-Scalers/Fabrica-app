...
describe('getProjectHostCloneUrl', () => {
  it('builds a GitHub HTTPS clone URL from provider identity', () => {
    ...
    ).toBe('https://github.com/Auto-Scalers/Fabrica-app.git')
  })
  ...
  it('preserves an authenticated Enterprise host and port', () => {
    ...
    ).toBe('https://github.acme-corp.com:8443/enterprise%20owner/FABRICA%20repo.git')
  })
  ...
  it('rejects malformed or path-bearing Enterprise hosts', () => {
    for (const host of [
      ...
      expect(
        getProjectHostCloneUrl(
          createProject({
            providerIdentity: { provider: 'github', owner: 'acme', repo: 'fabrica', host }
          })
        )
      ).toBeNull()
    }
