...
import {
  findFABRICADispatchTaskMarkerIndex,
  FABRICA_DISPATCH_STATUS_PREAMBLE_PREFIX,
  FABRICA_DISPATCH_STATUS_TASK_MARKER
} from '../../../shared/fabrica-dispatch-status-prompt'

export const FABRICA_DISPATCH_PREAMBLE_PREFIX = FABRICA_DISPATCH_STATUS_PREAMBLE_PREFIX
const FABRICA_DISPATCH_TASK_MARKER = FABRICA_DISPATCH_STATUS_TASK_MARKER
const FABRICA_DISPATCH_TASK_ID_MARKER = 'Your task ID is:'
...
const FABRICA_DISPATCH_TASK_PREVIEW_SCAN_LIMIT = 512
// Why: task id lives near the top of the preamble; keep that scan tight.
const FABRICA_DISPATCH_TASK_ID_SCAN_LIMIT = 1024
...
const FABRICA_DISPATCH_TASK_MARKER_SCAN_LIMIT = 32_768

/** True when the live prompt is still an FABRICA dispatch turn (not sticky metadata alone). */
export function isFABRICADispatchPrompt(prompt: string): boolean {
  return prompt.trimStart().startsWith(FABRICA_DISPATCH_PREAMBLE_PREFIX)
}
...
export function orchestrationLabelsMatchLiveDispatch(
  ...
  if (!isFABRICADispatchPrompt(entry.prompt)) {
    return false
  }
  ...
  const liveTaskId = getFABRICADispatchTaskId(entry.prompt)
  if (!liveTaskId) {
    return true
  }
...
export function getAgentRowPrimaryText(
  ...
  // first characters are boilerplate ("You are working inside FABRICA…").
  if (orchestrationLabelsMatchLiveDispatch(entry)) {
    return (
      entry.orchestration?.displayName?.trim() ||
      entry.orchestration?.taskTitle?.trim() ||
      getFABRICADispatchTaskPreview(entry.prompt)
    )
  }
  if (isFABRICADispatchPrompt(entry.prompt)) {
    return getFABRICADispatchTaskPreview(entry.prompt)
  }
...
export function getFABRICADispatchTaskId(prompt: string): string | null {
  if (!isFABRICADispatchPrompt(prompt)) {
    return null
  }
  const scan = prompt.trimStart().slice(0, FABRICA_DISPATCH_TASK_ID_SCAN_LIMIT)
  const markerIndex = scan.indexOf(FABRICA_DISPATCH_TASK_ID_MARKER)
  if (markerIndex === -1) {
    return null
  }
  ...
  const afterMarker = scan.slice(markerIndex + FABRICA_DISPATCH_TASK_ID_MARKER.length).trimStart()
  const idEnd = afterMarker.search(/\s/)
  const idLine = idEnd === -1 ? afterMarker : afterMarker.slice(0, idEnd)
  return idLine || null
}

function getFABRICADispatchTaskPreview(prompt: string): string {
  ...
  const scan = prompt
    .trimStart()
    .slice(0, FABRICA_DISPATCH_TASK_MARKER_SCAN_LIMIT + FABRICA_DISPATCH_TASK_PREVIEW_SCAN_LIMIT)
  ...
  const taskMarkerIndex = findFABRICADispatchTaskMarkerIndex(scan)
  if (taskMarkerIndex === -1) {
    return ''
  }
  const taskBodyStart = taskMarkerIndex + FABRICA_DISPATCH_TASK_MARKER.length
  const taskBody = scan.slice(taskBodyStart, taskBodyStart + FABRICA_DISPATCH_TASK_PREVIEW_SCAN_LIMIT)
