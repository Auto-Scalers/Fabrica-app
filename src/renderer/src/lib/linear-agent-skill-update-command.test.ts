...
describe('getLinearAgentSkillUpdateTarget', () => {
  it('targets the canonical skill for missing, canonical, and dual-name installs', () => {
    expect(getLinearAgentSkillUpdateTarget([], false)).toEqual({
      skillName: 'fabrica-linear',
      command: FABRICA_LINEAR_SKILL_UPDATE_COMMAND
    })
    expect(getLinearAgentSkillUpdateTarget([skill('fabrica-linear')], true)).toEqual({
      skillName: 'fabrica-linear',
      command: FABRICA_LINEAR_SKILL_UPDATE_COMMAND
    })
    expect(
      getLinearAgentSkillUpdateTarget([skill('fabrica-linear'), skill('linear-tickets')], true)
    ).toEqual({
      skillName: 'fabrica-linear',
      command: FABRICA_LINEAR_SKILL_UPDATE_COMMAND
    })
