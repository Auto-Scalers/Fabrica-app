 ...
 *
 * The incident chain was: Cmd+Q leaves a packaged `FABRICA serve` owning the
...
/** Argv macOS delivers for `open -n -a FABRICA` / Finder / Dock relaunch. */
const DESKTOP_RELAUNCH_ARGV = ['/Applications/Fabrica.app/Contents/MacOS/Fabrica'] as const
...
describe('#8457 headless serve promotion preserves surviving agent sessions', () => {
  ...
  it('ignores a duplicate `FABRICA serve` launch instead of promoting the headless owner', () => {
    const serve = bootHeadlessServeOwner()
