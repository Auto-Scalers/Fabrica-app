...
export type OpenHttpLinkOptions = {
  ...
  /** Unconditional for local sources: open inside FABRICA regardless of settings. */
  forceInApp?: boolean
...
): { wantsFABRICA: boolean; wantsSystemBrowser: boolean } {
  if (!modifierHeld) {
    return { wantsFABRICA: false, wantsSystemBrowser: false }
  }
  if (!modifierInverts) {
    return { wantsFABRICA: false, wantsSystemBrowser: true }
  }
  return { wantsFABRICA: !openLinksInApp, wantsSystemBrowser: openLinksInApp }
}
...
export function openHttpLink(url: string, opts: OpenHttpLinkOptions = {}): void {
  ...
  const routeToFABRICA =
    sourceIsLocal &&
    !forceSystemBrowser &&
    !modifier.wantsSystemBrowser &&
    Boolean(worktreeId) &&
    (forceInApp || openLinksInApp || modifier.wantsFABRICA)

  if (routeToFABRICA && worktreeId && state) {
