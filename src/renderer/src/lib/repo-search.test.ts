...
function makeRepo(overrides: Partial<Repo> = {}): Repo {
  return {
    id: 'repo-1',
    path: '/Users/test/src/FABRICA',
    displayName: 'Auto-Scalers/Fabrica-app',
    badgeColor: '#22c55e',
    addedAt: 0,
    ...overrides
  }
...
describe('repo-search', () => {
  ...
  it('matches display names case-insensitively', () => {
    const repos = [
      makeRepo({ id: '1', displayName: 'Auto-Scalers/Fabrica-app', path: '/repos/FABRICA' }),
      makeRepo({ id: '2', displayName: 'Auto-Scalers/noqa', path: '/repos/noqa' })
    ]

    expect(searchRepos(repos, 'FABRICA').map((repo) => repo.id)).toEqual(['1'])
  })

  it('falls back to matching repo paths', () => {
    const repos = [
      makeRepo({ id: '1', displayName: 'frontend', path: '/src/team-a/FABRICA' }),
  ...
  it('keeps display-name matches ahead of path-only matches', () => {
    const repos = [
      makeRepo({ id: '1', displayName: 'misc', path: '/src/FABRICA-tools/misc' }),
      makeRepo({ id: '2', displayName: 'FABRICA', path: '/src/team-a/project' })
    ]

    expect(searchRepos(repos, 'FABRICA').map((repo) => repo.id)).toEqual(['2', '1'])
  })
