...
export function useIpcEvents(): void {
  useEffect(() => {
        ...
        toast.warning(
          ...
          ),
          {
            ...
            description: translate(
              ...
              'If this was your phone or another Fabrica client, re-pair it from Settings → Mobile.'
            ),
    ...
    // Why: `FABRICA tab switch --focus` must NOT call setActiveWorktree — a global focus from one agent's parallel-worktree switch would steal the user's view.
    ...
    unsubs.push(
      window.api.browser.onOpenLinkInFABRICATab(({ browserPageId, url }) => {
        const store = useAppStore.getState()
        ...
        // Why: only the renderer owns FABRICA's tab model, so main delegates link-open here.
    ...
    const unsubscribeEmulatorAutoAttach = window.api.emulator?.onAutoAttach(
      ({ worktreeId, info }) => {
        ...
        window.setTimeout(() => {
          window.dispatchEvent(
            new CustomEvent('FABRICA:emulator-auto-attach', {
