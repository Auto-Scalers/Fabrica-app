...
describe('settings navigation metadata', () => {
  ...
  it('places the Fabrica account in Set Up on desktop only', () => {
    ...
    const account = desktopSections.find((section) => section.id === 'FABRICA-account')

    expect(account?.group).toBe('setup')
    expect(account?.searchEntries[0]?.title).toBe('Fabrica account')
    expect(ids({ isWebClient: true })).not.toContain('FABRICA-account')
  })
