...
export type ComposerCardProps = {
  ...
  ephemeralVmRecipes: NonNullable<FABRICAHooks['environmentRecipes']>
...
export function getInitialAutoManagedWorkspaceName({
  ...
  // Why: a prefilled name counts as user input unless it exactly matches the linked-item seed FABRICA generated.
...
export function useComposerState(options: UseComposerStateOptions): UseComposerStateResult {
  ...
  // Why: SSH remotes deploy the CLI shim as plain `fabrica`, so the `fabrica` rename must not apply to remote launch commands.
  ...
  const [yamlHooks, setYamlHooks] = useState<FABRICAHooks | null>(null)
  ...
  const commitHookCheckIfCurrent = useCallback(
    (targetContextKey: string, hooks: FABRICAHooks | null): boolean => {
  ...
  const uploadComposerPaths = useCallback(
    async (
      ...
      const destinationDir = joinPath(targetRepoPath, '.fabrica/drops')
