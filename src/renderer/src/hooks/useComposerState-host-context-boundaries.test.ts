...
describe('useComposerState host-context boundaries', () => {
    ...
    expect(
      getInitialGitHubPrStartPointSelection({
        ...
        linkedWorkItem: {
          ...
          number: 43,
          title: item.title,
          url: 'https://github.com/Auto-Scalers/Fabrica-app/pull/43'
        },
        repoId: 'repo-1'
      })
    ).toBeNull()
  ...
  it('treats typed workspace names as user-authored, not auto-managed', () => {
    ...
    expect(
      isExplicitWorkspaceNameInput({
        name: 'https://github.com/Auto-Scalers/Fabrica-app/pull/1234',
        lastAutoName: ''
      })
    ).toBe(false)
  })
  ...
  it('keeps smart GitHub names for auto-managed PR start-point submissions', () => {
    expect(
      resolveSmartGitHubCreateNames({
        ...
        smartWorkspaceName: 'title-derived-name',
        smartDisplayName: 'Title derived name',
        fallbackWorkspaceName: 'https://github.com/Auto-Scalers/Fabrica-app/pull/6772',
    ...
    expect(
      getInitialAutoManagedWorkspaceName({
        ...
        initialLinkedWorkItem: {
          ...
          url: 'https://github.com/Auto-Scalers/Fabrica-app/issues/1234'
        }
      })
  ...
  it('restores Jira draft context only when its site and issue identity agree', () => {
    const item = {
      ...
      url: 'https://company.atlassian.net/jira/browse/FABRICA-123',
    ...
    expect(
      getMatchingLinkedTaskSourceContext({ ...item, jiraIdentifier: 'FABRICA-999' }, context)
    ).toBeNull()
  })
  ...
  it('keeps sentinel-based Jira and Linear starts out of issue-command templates', () => {
    expect(HOOK_SOURCE).not.toContain('isFABRICACliAvailableForLaunch')
