...
describe('hasInstalledAgentSkill', () => {
  it('matches installed skills by summarized name', () => {
    expect(hasInstalledAgentSkill([skill({ name: 'FABRICA-cli' })], 'FABRICA-cli')).toBe(true)
  })
    ...
    expect(
      hasInstalledAgentSkill(
        [
          skill({
            name: 'Fabrica CLI',
            directoryPath: 'C:\\Users\\test\\.agents\\skills\\FABRICA-cli'
          })
        ],
        'FABRICA-cli'
      )
  ...
  it('ignores non-installed discovery entries', () => {
    expect(
      hasInstalledAgentSkill([skill({ name: 'FABRICA-cli', installed: false })], 'FABRICA-cli')
    ).toBe(false)
  })
      ...
      hasInstalledAgentSkill(
        [
          skill({
            ...
            directoryPath: '/repo/.agents/skills/FABRICA-cli',
            skillFilePath: '/repo/.agents/skills/FABRICA-cli/SKILL.md'
          }),
          skill({
            id: 'skill-2',
            name: 'FABRICA-cli',
            ...
            directoryPath: '/Users/test/.codex/plugins/cache/vendor/FABRICA-cli',
            skillFilePath: '/Users/test/.codex/plugins/cache/vendor/FABRICA-cli/SKILL.md'
          })
        ],
        'FABRICA-cli',
  ...
  it('counts home skills when matching global installs', () => {
    expect(
      hasInstalledAgentSkill([skill({ name: 'FABRICA-cli' })], 'FABRICA-cli', {
        sourceKinds: GLOBAL_AGENT_SKILL_SOURCE_KINDS
      })
    ).toBe(true)
  })
    ...
    expect(
      hasInstalledAgentSkillNamed(
        [
          skill({
            name: 'Linear Tickets',
            directoryPath: 'C:\\Users\\test\\.agents\\skills\\fabrica-linear'
          })
        ],
        ['fabrica-linear', 'linear-tickets']
      )
  ...
  it('keeps aliases opt-in for unrelated single-name checks', () => {
    expect(hasInstalledAgentSkill([skill({ name: 'linear-tickets' })], 'fabrica-linear')).toBe(false)
  })
})
...
describe('discoverInstalledAgentSkills', () => {
  ...
  it('starts a fresh scan when a forced refresh arrives during a background scan', async () => {
    ...
    const freshResult = discoveryResult([skill({ name: 'FABRICA-cli' })])
  ...
  it('lets completed-scan broadcasts reuse the cached result', async () => {
    const discover = vi
      ...
      .mockResolvedValue(discoveryResult([skill({ name: 'fabrica-linear' })]))
    vi.stubGlobal('window', {
      api: { skills: { discover } },
      dispatchEvent: vi.fn(),
      CustomEvent: class {}
    ...
    for (const pending of fromSubscribers) {
      await expect(pending).resolves.toMatchObject({
        skills: [expect.objectContaining({ name: 'fabrica-linear' })]
      })
    }
    expect(discover).toHaveBeenCalledTimes(1)
  })
