...
describe('composer drop upload result', () => {
  it('separates imported files and folders while counting skipped or failed paths', () => {
    const results: ComposerDropUploadImportResult[] = [
      { status: 'imported', kind: 'file', destPath: '/repo/.FABRICA/drops/file.txt' },
      { status: 'imported', kind: 'directory', destPath: '/repo/.FABRICA/drops/folder' },
      { status: 'skipped' },
      { status: 'failed' }
    ]

    expect(collectComposerDropUploadResult(results)).toEqual({
      filePaths: ['/repo/.FABRICA/drops/file.txt'],
      folderPaths: ['/repo/.FABRICA/drops/folder'],
      skippedOrFailed: 2
    })
  })
