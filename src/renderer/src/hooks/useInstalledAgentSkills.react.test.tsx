...
const LINEAR_AGENT_SKILL_NAMES = ['fabrica-linear', 'linear-tickets'] as const
...
describe('useInstalledAgentSkill', () => {
  ...
  it('stays settled through a focus rescan so status surfaces do not flash', async () => {
    ...
    firstScan.resolve(discoveryResult([skill({ name: 'fabrica-linear' })]))
    await act(async () => {
    ...
    focusScan.resolve(discoveryResult([skill({ name: 'fabrica-linear' })]))
    await act(async () => {
  ...
  it('reports unsettled again when the discovery target changes', async () => {
    ...
    await renderProbe()
    hostScan.resolve(discoveryResult([skill({ name: 'fabrica-linear' })]))
    await act(async () => {
      await hostScan.promise
    })
    expect(latestState?.settled).toBe(true)
