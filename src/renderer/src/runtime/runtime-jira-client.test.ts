...
describe('runtime Jira client search bounds', () => {
  it('routes isolated status and summary reads through local and paired-runtime owners', async () => {
    ...
    jiraLookupIssueSummaryLocal.mockResolvedValue({ key: 'FABRICA-1' })
    runtimeCall.mockImplementation(async (args: { method: string }) => {
      ...
      return {
        ...
        ok: true,
        result:
          args.method === 'jira.readStatus' ? { connected: true, viewer: null } : { key: 'FABRICA-1' },
        _meta: { runtimeId: 'remote-runtime' }
      }
    })

    await expect(jiraReadStatus(localContext)).resolves.toMatchObject({ connected: true })
    await expect(jiraLookupIssueSummary(localContext, 'FABRICA-1', 'site-1')).resolves.toMatchObject({
      key: 'FABRICA-1'
    })
    await expect(jiraReadStatus(runtimeContext)).resolves.toMatchObject({ connected: true })
    await expect(jiraLookupIssueSummary(runtimeContext, 'FABRICA-1', 'site-1')).resolves.toMatchObject(
      { key: 'FABRICA-1' }
    )
    ...
    expect(runtimeCall).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        params: { key: 'FABRICA-1', siteId: 'site-1' },
        selector: 'env-1'
      })
    )
  })
  ...
  it('cancels a superseded local Jira summary read', async () => {
    ...
    const lookup = jiraLookupIssueSummary(context, 'FABRICA-1', 'site-1', controller.signal)
    controller.abort()
  ...
  it('streams image-bearing issue and comment payloads from remote runtimes', async () => {
      ...
      async (args: RuntimeSubscribeArgs, callbacks: RuntimeSubscribeCallbacks) => {
        const payload =
          args.method === 'jira.getIssueStream'
            ? { key: 'FABRICA-1', description: '![shot](data:image/png;base64,abc)' }
    ...
    await expect(
      jiraGetIssue({ activeRuntimeEnvironmentId: 'env-1' }, 'FABRICA-1', 'site-1')
    ).resolves.toMatchObject({ key: 'FABRICA-1' })
    await expect(
      jiraIssueComments({ activeRuntimeEnvironmentId: 'env-1' }, 'FABRICA-1', 'site-1')
