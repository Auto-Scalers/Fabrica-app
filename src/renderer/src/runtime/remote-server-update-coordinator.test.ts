...
const INSTALL_FAILURE =
  "Could not start the update installer. Fabrica remains open. (Command failed: pkexec dpkg -i '/home/u/.cache/FABRICA-updater/pending/fabrica_1.5.0_amd64.deb' pkexec must be setuid root)"
...
describe('remote server update execution', () => {
  ...
  it('fails when no offered update reaches the requested version', async () => {
    ...
    expect(result).toMatchObject({
      phase: 'failed',
      error: 'The server updater did not offer the requested Fabrica version.'
    })
  })
