...
function writeStoredRuntimeEnvironment(storage: Storage, environmentId = 'web-env-1'): void {
  storage.setItem(
    'FABRICA.web.runtimeEnvironment.v1',
    JSON.stringify({
      id: environmentId,
      name: 'Test runtime',
      createdAt: 1,
...
describe('web before-unload persistence', () => {
  ...
  it('persists final UI and host-partitioned sessions synchronously', async () => {
    ...
    expect(JSON.parse(storage.getItem('FABRICA.web.workspaceSession.v1') ?? '{}')).toMatchObject({
      activeWorktreeId: 'local-worktree'
    })
    expect(
      JSON.parse(storage.getItem('FABRICA.web.workspaceSession.v1.runtime:web-env-1') ?? '{}')
    ).toMatchObject({ activeWorktreeId: 'remote-worktree' })
    expect(JSON.parse(storage.getItem('FABRICA.web.ui.v1') ?? '{}')).toMatchObject({
      activeView: 'settings'
    })
  })
})
...
describe('web runtime environment identity', () => {
  ...
  it('does not resolve an old server selector through a differently keyed server', async () => {
    ...
    ).rejects.toThrow('Unknown Fabrica runtime environment: web-server-a')
  })
  ...
  it('stores paired device identity from a web access link', async () => {
    ...
    expect(
      JSON.parse(globals.storage.getItem('FABRICA.web.runtimeEnvironment.v1') ?? '{}')
  ...
  it('persists an explicit Active Server choice across unrelated web settings writes', async () => {
    ...
    expect(JSON.parse(globals.storage.getItem('FABRICA.web.settings.v1') ?? '{}')).toMatchObject({
  ...
  it('rejects an unknown explicit Active Server choice without corrupting the preference', async () => {
    ...
    ).rejects.toThrow('Unknown Fabrica runtime environment: unknown-server')
    expect(JSON.parse(globals.storage.getItem('FABRICA.web.settings.v1') ?? '{}')).toMatchObject({
      activeRuntimeEnvironmentId: paired.environment.id
    })
  })
  ...
  it('ignores malformed persisted compatibility ids when resolving selectors', async () => {
    ...
    const stored = JSON.parse(
      globals.storage.getItem('FABRICA.web.runtimeEnvironment.v1') ?? '{}'
    ) as Record<string, unknown>
    stored.compatibleEnvironmentIds = { old: 'web-server-old' }
    globals.storage.setItem('FABRICA.web.runtimeEnvironment.v1', JSON.stringify(stored))
    const { installWebPreloadApi } = await import('./web-preload-api')
    installWebPreloadApi()

    await expect(
      globals.window.api.runtimeEnvironments.resolve({ selector: 'web-server-old' })
    ).rejects.toThrow('Unknown Fabrica runtime environment: web-server-old')
  })
  ...
  it('keeps the current host when verification rejects an incompatible replacement', async () => {
    ...
    const globals = installBrowserGlobals('Linux')
    writeStoredRuntimeEnvironment(globals.storage, 'web-server-a')
    const previousStored = globals.storage.getItem('FABRICA.web.runtimeEnvironment.v1')
    const { installWebPreloadApi } = await import('./web-preload-api')
    installWebPreloadApi()
    ...
    expect(globals.storage.getItem('FABRICA.web.runtimeEnvironment.v1')).toBe(previousStored)
    await expect(globals.window.api.runtimeEnvironments.list()).resolves.toMatchObject([
      { id: 'web-server-a' }
    ])
  })
...
describe('web keybindings preload API', () => {
  ...
  it('returns snapshots and persists customized bindings in browser storage', async () => {
    ...
    expect(storage.getItem('FABRICA.web.keybindings.v1')).toContain('worktree.palette')
...
describe('web settings preload API', () => {
  ...
  it('migrates first-work branch auto-rename on for stored legacy web settings once', async () => {
    ...
    const stored = JSON.parse(globals.storage.getItem('FABRICA.web.settings.v1') ?? '{}') as {
  ...
  it('normalizes terminal cursor style before web settings writes return or persist', async () => {
    ...
    const invalidStored = JSON.parse(storage.getItem('FABRICA.web.settings.v1') ?? '{}') as {
      terminalCursorStyle?: string
      terminalCursorStyleDefaultedToBlock?: boolean
    }
    ...
    expect(JSON.parse(storage.getItem('FABRICA.web.settings.v1') ?? '{}').terminalCursorStyle).toBe(
      'bar'
    )
  })
  ...
  it('arms the OSC 52 notice in the web UI store when the flip overrides a persisted off', async () => {
    ...
    const storedUi = JSON.parse(globals.storage.getItem('FABRICA.web.ui.v1') ?? '{}') as {
  ...
  it('does not arm the OSC 52 notice for a web profile with no persisted value', async () => {
    const globals = installBrowserGlobals('Linux')
    globals.storage.setItem('FABRICA.web.settings.v1', JSON.stringify({ terminalFontSize: 15 }))
    const { installWebPreloadApi } = await import('./web-preload-api')
    installWebPreloadApi()
  ...
  it('stamps the first-work branch auto-rename guard for web setting updates', async () => {
    ...
    const stored = JSON.parse(storage.getItem('FABRICA.web.settings.v1') ?? '{}') as {
      autoRenameBranchFromWork?: boolean
      autoRenameBranchFromWorkDefaultedOn?: boolean
    }
...
describe('web UI preload API', () => {
  ...
  it('saves browser clipboard images through bounded upload chunks', async () => {
      ...
      WebRuntimeClient: class {
        call(method: string, params?: unknown): Promise<RuntimeRpcResponse<unknown>> {
          ...
          return Promise.resolve({
            id: `call-${runtimeCalls.length}`,
            ok: true,
            result: 'C:\\Users\\alice\\AppData\\Local\\Temp\\FABRICA-paste-image.png',
            _meta: { runtimeId: 'runtime-1' }
          })
        }
    ...
    ).resolves.toBe('C:\\Users\\alice\\AppData\\Local\\Temp\\FABRICA-paste-image.png')
    expect(runtimeCalls).toEqual([
      {
  ...
  it('falls back to one-shot clipboard save for small payloads when the host lacks upload RPCs', async () => {
      ...
      WebRuntimeClient: class {
        call(method: string, params?: unknown): Promise<RuntimeRpcResponse<unknown>> {
          ...
          return Promise.resolve({
            id: `call-${runtimeCalls.length}`,
            ok: true,
            result: '/tmp/FABRICA-paste-image.png',
            _meta: { runtimeId: 'runtime-1' }
          })
        }
    ...
    ).resolves.toBe('/tmp/FABRICA-paste-image.png')
    expect(runtimeCalls).toEqual([
      {
        method: 'clipboard.startImageUpload',
        params: {
  ...
  it('keeps explicit local right sidebar visibility over the legacy default', async () => {
    const { api, storage } = await installApi('Linux')
    storage.setItem('FABRICA.web.ui.v1', JSON.stringify({ rightSidebarOpen: true }))
  ...
  it('keeps newer feature interaction counts when runtime responses resolve out of order', async () => {
    ...
    const stored = JSON.parse(globals.storage.getItem('FABRICA.web.ui.v1') ?? '{}') as {
  ...
  it('keeps the workspace origin filter browser-local across host UI responses', async () => {
    ...
    expect(JSON.parse(globals.storage.getItem('FABRICA.web.ui.v1') ?? '{}')).toMatchObject({
      hideWorkspacesFromOtherDevices: true
    })
  })
  ...
  it('does not keep a local shadow copy of main-owned feature telemetry markers', async () => {
    ...
    const ui = await globals.window.api.ui.get()
    const stored = JSON.parse(globals.storage.getItem('FABRICA.web.ui.v1') ?? '{}') as Record<
      string,
      unknown
    >
        ...
        call(method: string, params: unknown): Promise<RuntimeRpcResponse<unknown>> {
          ...
          if (method === 'computer.permissionsStatus') {
            return Promise.resolve({
              ...
              result: {
                platform: 'darwin',
                helperAppPath: '/Applications/Fabrica Computer Use.app',
...
describe('web repos preload API', () => {
  ...
  it.each([
    ['/home/alice', '/home/alice/FABRICA/projects'],
    ['/', '/FABRICA/projects'],
    ['C:\\', 'C:\\FABRICA\\projects']
  ])(
...
describe('web worktree preload API', () => {
  ...
  it('does not let a stale listAll response repopulate the next server cache', async () => {
    ...
    await expect(serverAList).rejects.toThrow(
      'The paired Fabrica server changed while the request was in progress.'
    )
    ...
    await globals.window.api.worktrees.create({
      ...
      startup: {
        ...
        launchConfig: {
          ...
          agentEnv: { FABRICA_AGENT_MODE: 'direct' }
        },
    ...
    expect(runtimeCalls).toEqual([
      {
        ...
        params: expect.objectContaining({
          ...
          startupEnv: { FABRICA_AGENT_MODE: 'direct' },
          startupLaunchConfig: {
            agentCommand: 'codex',
            agentArgs: '--model gpt-5',
            agentEnv: { FABRICA_AGENT_MODE: 'direct' }
  ...
  it('rejects SSH clone requests in paired web clients', async () => {
    ...
    await expect(
      api.repos.cloneRemote({
        connectionId: 'ssh-1',
        url: 'https://github.com/Auto-Scalers/Fabrica-app.git',
  ...
  it('routes every runtime-backed GitHub method through the expected RPC method', async () => {
    ...
    const routeCases: {
      ...
      },
      {
        ...
        args: { repoPath, owner: 'acme', repo: 'fabrica', number: 7, type: 'pr' },
      ...
      },
      {
        ...
        args: { owner: 'acme', repo: 'fabrica', number: 7, type: 'issue' },
        expectedMethod: 'github.project.workItemDetailsBySlug',
        expectedParams: { owner: 'acme', repo: 'fabrica', number: 7, type: 'issue' }
      },
      {
      ...
      },
      {
        ...
        args: { owner: 'acme', repo: 'fabrica', number: 7, updates: { title: 'New' } },
        expectedMethod: 'github.project.updateIssueBySlug',
        expectedParams: { owner: 'acme', repo: 'fabrica', number: 7, updates: { title: 'New' } }
      },
      {
      ...
      },
      {
        ...
        args: { owner: 'acme', repo: 'fabrica', number: 7, body: 'Fixed' },
        expectedMethod: 'github.project.addIssueCommentBySlug',
        expectedParams: { owner: 'acme', repo: 'fabrica', number: 7, body: 'Fixed' }
      },
      {
        key: 'updateIssueCommentBySlug',
        args: { owner: 'acme', repo: 'fabrica', commentId: 9, body: 'Edited' },
        expectedMethod: 'github.project.updateIssueCommentBySlug',
        expectedParams: { owner: 'acme', repo: 'fabrica', commentId: 9, body: 'Edited' }
      },
      {
        key: 'deleteIssueCommentBySlug',
        args: { owner: 'acme', repo: 'fabrica', commentId: 9 },
        expectedMethod: 'github.project.deleteIssueCommentBySlug',
        expectedParams: { owner: 'acme', repo: 'fabrica', commentId: 9 }
      },
      {
        key: 'listLabelsBySlug',
        args: { owner: 'acme', repo: 'fabrica' },
        expectedMethod: 'github.project.listLabelsBySlug',
        expectedParams: { owner: 'acme', repo: 'fabrica' }
      },
      {
        key: 'listAssignableUsersBySlug',
        args: { owner: 'acme', repo: 'fabrica', seedLogins: ['alice'] },
        expectedMethod: 'github.project.listAssignableUsersBySlug',
        expectedParams: { owner: 'acme', repo: 'fabrica', seedLogins: ['alice'] }
      },
      {
      ...
      },
      {
        ...
        args: { owner: 'acme', repo: 'fabrica', number: 7, issueTypeId: 'it-1' },
        expectedMethod: 'github.project.updateIssueTypeBySlug',
        expectedParams: { owner: 'acme', repo: 'fabrica', number: 7, issueTypeId: 'it-1' }
      }
    ]
