...
describe('web pairing input', () => {
  ...
  it('parses query-form pairing URLs', () => {
    expect(parseWebPairingInput(`FABRICA://pair?code=${encodeOffer()}`)).toEqual(offer)
  })

  it('still parses legacy hash-form pairing URLs', () => {
    expect(parseWebPairingInput(`FABRICA://pair#${encodeOffer()}`)).toEqual(offer)
  })

  it('preserves optional device scope metadata', () => {
    expect(parseWebPairingInput(`FABRICA://pair?code=${encodeOffer({ scope: 'mobile' })}`)).toEqual({
      ...offer,
      scope: 'mobile'
    })
  })
  ...
  it('preserves optional paired device identity', () => {
    expect(
      parseWebPairingInput(`FABRICA://pair?code=${encodeOffer({ pairedDeviceId: 'paired-device-a' })}`)
    ).toEqual({
      ...offer,
  ...
  it.each([
    ['wss://proxy.example:443/FABRICA/runtime', 'wss://proxy.example:443/FABRICA/runtime'],
    ['https://proxy.example/FABRICA/runtime', 'wss://proxy.example/FABRICA/runtime'],
    ['http://proxy.example:8080/FABRICA/runtime', 'ws://proxy.example:8080/FABRICA/runtime']
  ...
  it('treats invalid device scope metadata as unknown', () => {
    expect(parseWebPairingInput(`FABRICA://pair?code=${encodeOffer({ scope: 'admin' })}`)).toEqual(
      offer
    )
  })

  it('rejects FABRICA URLs outside the exact pairing route', () => {
    expect(parseWebPairingInput(`FABRICA://pairing?code=${encodeOffer()}`)).toBeNull()
    expect(parseWebPairingInput(`FABRICA://pair-extra?code=${encodeOffer()}`)).toBeNull()
  })

  it('auto-saves scoped runtime offers during web startup', () => {
    const input = `FABRICA://pair?code=${encodeOffer({ scope: 'runtime' })}`
    expect(
  ...
  it('shows the connect screen for mobile-scope and legacy unknown-scope offers', () => {
    const mobileInput = `FABRICA://pair?code=${encodeOffer({ scope: 'mobile' })}`
    const legacyInput = `FABRICA://pair?code=${encodeOffer()}`
