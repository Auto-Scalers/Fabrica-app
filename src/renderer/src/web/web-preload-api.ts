...
const SETTINGS_STORAGE_KEY = 'FABRICA.web.settings.v1'
const UI_STORAGE_KEY = 'FABRICA.web.ui.v1'
const SESSION_STORAGE_KEY = 'FABRICA.web.workspaceSession.v1'
const ONBOARDING_STORAGE_KEY = 'FABRICA.web.onboarding.v1'
const GITHUB_CACHE_STORAGE_KEY = 'FABRICA.web.githubCache.v1'
const KEYBINDINGS_STORAGE_KEY = 'FABRICA.web.keybindings.v1'
...
const webE2EConfig = createE2EConfig({
  ...
  terminalParkingDelayMs: Number(webE2EQuery?.get('FABRICAE2ETerminalParkingDelayMs')) || null,
  terminalRetentionLimit: Number(webE2EQuery?.get('FABRICAE2ETerminalRetentionLimit')) || null
})
...
export function installWebPreloadApi(): void {
  ...
  const webWindow = window as unknown as { __FABRICA_WEB_CLIENT__?: boolean }
  webWindow.__FABRICA_WEB_CLIENT__ = true
...
function createWebPreloadApi(): Partial<PreloadApi> {
  const webFABRICAProfileAuthStatus = () =>
    Promise.resolve({
      activeProfileId: DEFAULT_LOCAL_FABRICA_PROFILE_ID,
      configured: false,
      state: 'unconfigured' as const,
      persistence: 'none' as const,
      setupMessage: 'Fabrica Cloud sign-in is not available in the browser fallback.'
    })
  ...
  return {
    ...
    starNag: {
      ...
      starFABRICA: () => Promise.resolve(false),
    ...
    FABRICAProfiles: {
      list: () =>
        Promise.resolve({
          activeProfileId: DEFAULT_LOCAL_FABRICA_PROFILE_ID,
          profiles: [createDefaultLocalFABRICAProfile(0)],
          multiProfileUi: false
        }),
      authStatus: webFABRICAProfileAuthStatus,
      createLocal: () =>
        Promise.resolve({
          activeProfileId: DEFAULT_LOCAL_FABRICA_PROFILE_ID,
          profiles: [createDefaultLocalFABRICAProfile(0)],
          profile: createDefaultLocalFABRICAProfile(0)
        }),
      createCloudLinked: async () => ({
        status: 'unconfigured',
        auth: await webFABRICAProfileAuthStatus()
      }),
...
function createRuntimeEnvironmentsApi(): NonNullable<Partial<PreloadApi>['runtimeEnvironments']> {
  return {
    ...
    addFromPairingCode: async ({ name, pairingCode }) => {
      ...
      if (!offer) {
        throw new Error('Invalid Fabrica pairing code.')
      }
      ...
      try {
        ...
        return {
          ...
          message: translate(
            ...
            'Cannot reach Fabrica at {{endpoint}}.',
...
function createBrowserApi(): NonNullable<Partial<PreloadApi>['browser']> {
  return {
    ...
    onOpenLinkInFABRICATab: () => noopUnsubscribe,
...
function createGitHubApi(): WebGitHubApi {
  ...
  const githubApi = {
    ...
    checkFABRICAStarred: () => Promise.resolve(null),
    starFABRICA: () => Promise.resolve(false),
    rateLimit: (args) =>
...
function createCliApi(): NonNullable<Partial<PreloadApi>['cli']> {
  const status = {
    platform: getBrowserPlatform(),
    commandName: getBrowserPlatform() === 'linux' ? 'fabrica' : 'fabrica',
    commandPath: null,
    pathDirectory: null,
    ...
    detail: 'CLI registration is managed on the Fabrica server, not in the web browser.'
  } as const
  return {
...
function createAgentHooksApi(): NonNullable<Partial<PreloadApi>['agentHooks']> {
  const status = (
    ...
    Promise.resolve({
      ...
      detail: 'Agent hook status is only available on the Fabrica server.'
    } as const)
  return {
  ...
  return {
    ...
    openSetup: (args) =>
      callRuntimeResult<ComputerUsePermissionSetupResult>(
        ...
        nextStep: 'Computer-use permissions are managed on the Fabrica server.'
      })),
    reset: () =>
      Promise.resolve({
        platform: getBrowserPlatform(),
...
function resolveEnvironment(selector: string): StoredWebRuntimeEnvironment {
  ...
  throw new Error(`Unknown Fabrica runtime environment: ${selector}`)
}
...
function requireActiveEnvironment(): StoredWebRuntimeEnvironment {
  ...
  if (!activeEnvironment) {
    throw new Error('Pair this web client with an Fabrica server first.')
  }
...
function assertActiveEnvironment(environmentId: string): void {
  if (requireActiveEnvironment().id !== environmentId) {
    throw new Error('The paired Fabrica server changed while the request was in progress.')
  }
}
...
function getStoredOnboarding(): OnboardingState {
  ...
  // Why: paired clients already have an FABRICA server; skip desktop first-run onboarding that would probe browser-local tools.
  writeJson(ONBOARDING_STORAGE_KEY, closed)
  return closed
}
