...
describe('removeWorktree cascade', () => {
  ...
  it('offers force delete for orphaned Fabrica worktree directories', async () => {
    const store = createTestStore()
    const worktreeId = 'repo1::/path/wt1'
...
// Why: CLI-spawned terminals stamp FABRICA_PANE_KEY at spawn; renderer must adopt the tab under that id so hook events route correctly.
