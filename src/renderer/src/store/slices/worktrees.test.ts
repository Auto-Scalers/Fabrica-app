...
function makeDetectedResult(
  ...
  return {
    ...
    worktrees: worktrees.map((worktree) => ({
      ...worktree,
      ownership: 'FABRICA-managed' as const,
...
describe('fetchWorktrees', () => {
  ...
  it('merges one exact direct provider result once without a second scan', async () => {
    ...
    const worktree = makeWorktree({
      id: 'repo-ssh::/home/FABRICA/feature',
      repoId: 'repo-ssh',
      path: '/home/FABRICA/feature'
    })
    store.setState({
      repos: [
        {
  ...
  it('rejects a late duplicate exact-host owner with zero mutation publications', async () => {
    const store = createTestStore()
    const existing = makeWorktree({
      id: 'repo-ssh::/home/FABRICA/existing',
      repoId: 'repo-ssh',
      path: '/home/FABRICA/existing',
  ...
  it('shows persisted secondary worktrees while SSH is connecting', async () => {
    ...
    const queued = makeWorktree({
      id: 'repo-ssh::/home/FABRICA/queued',
      repoId: 'repo-ssh',
      path: '/home/FABRICA/queued',
      displayName: 'queued'
    })
  ...
  it('inserts metadata rows inside the SSH block instead of past sibling hosts', async () => {
    ...
    const sshExisting = makeWorktree({
      id: 'repo-shared::/home/FABRICA/existing',
      repoId: sshRepo.id,
      path: '/home/FABRICA/existing',
      hostId: 'ssh:ssh-1'
    })
    ...
    const queued = makeWorktree({
      id: 'repo-shared::/home/FABRICA/queued',
      repoId: sshRepo.id,
      path: '/home/FABRICA/queued'
    })
    listKnownForExecutionHostMock.mockResolvedValueOnce({
  ...
  it('drops metadata rows when SSH authority lands during the read', async () => {
    ...
    const live = makeWorktree({
      id: 'repo-ssh::/home/FABRICA/live',
      repoId: 'repo-ssh',
      path: '/home/FABRICA/live',
      hostId: 'ssh:ssh-1'
    })
    ...
    const purged = makeWorktree({
      id: 'repo-ssh::/home/FABRICA/purged',
      repoId: 'repo-ssh',
      path: '/home/FABRICA/purged'
    })
  ...
  it('replaces metadata rows once the authoritative SSH scan lands', async () => {
    ...
    const stale = makeWorktree({
      id: 'repo-ssh::/home/FABRICA/stale',
      repoId: 'repo-ssh',
      path: '/home/FABRICA/stale'
    })
    listKnownForExecutionHostMock.mockResolvedValueOnce({
      status: 'complete',
      repoId: sshRepo.id,
  ...
  it('does not resurrect worktrees an authoritative SSH scan already removed', async () => {
    ...
    const deletedOnRemote = makeWorktree({
      id: 'repo-ssh::/home/FABRICA/deleted',
      repoId: 'repo-ssh',
      path: '/home/FABRICA/deleted'
    })
  ...
  it('stamps runtime worktrees with the owning project host setup', async () => {
    const store = createTestStore()
    const remote = makeWorktree({
      id: 'repo-remote::/vercel/sandbox/FABRICA',
      repoId: 'repo-remote',
  ...
  it('falls back to legacy remote worktree.list when detectedList is unavailable', async () => {
    ...
    expect(store.getState().detectedWorktreesByRepo.repo1).toMatchObject({
      ...
      worktrees: [{ id: remote.id, ownership: 'FABRICA-managed', visible: true }]
    })
    expect(runtimeEnvironmentCall).toHaveBeenCalledWith({
      selector: 'env-1',
      method: 'worktree.detectedList',
...
describe('removeWorktree state cleanup', () => {
  ...
  it('purges the orphaned project that pointed at a destroyed runtime-owned SSH target', async () => {
    ...
    const orphanedSetup = {
      ...
      hostId: 'ssh:runtime-ssh-FABRICA-1'
    ...
    mockApi.ephemeralVm.listRuntimes.mockResolvedValueOnce([
      {
        ...
        sshTargetId: 'runtime-ssh-FABRICA-1'
      }
    ])
  ...
  it('records the sidebar scroll anchor in the same tick it removes the worktree', async () => {
    ...
    sidebar.addEventListener('FABRICA-record-virtualized-scroll-anchor', () => {
...
describe('worktree remote runtime mutations', () => {
  ...
  it('persists Jira item and source context through paired-runtime create', async () => {
    ...
    const linkedWorkItem = {
      ...
      type: 'issue' as const,
      number: 0,
      title: 'FABRICA-123 Link Jira',
      url: 'https://company.atlassian.net/browse/FABRICA-123',
      jiraIdentifier: 'FABRICA-123'
    }
  ...
  it('passes task startup drafts only to the owning remote runtime', async () => {
    ...
    args[10] = 'codex'
    args[25] = { startupDraft: 'https://github.com/Auto-Scalers/Fabrica-app/issues/12' }
    ...
    expect(runtimeEnvironmentCall).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        params: expect.objectContaining({
          createdWithAgent: 'codex',
          startupDraft: 'https://github.com/Auto-Scalers/Fabrica-app/issues/12'
        })
      })
    )
    expect(mockApi.worktrees.create).not.toHaveBeenCalled()
  ...
  it('does not forget a row that becomes ambiguous while remote removal is in flight', async () => {
    ...
    store.setState({
      settings: { activeRuntimeEnvironmentId: 'env-1' } as never,
      trustedFABRICAHooks: { 'repo-shared': { all: { approvedAt: 1 } } },
  ...
  it('hydrates an SSH-owned linked GitHub PR push target through local IPC when a runtime is focused', async () => {
    ...
    const wt = makeWorktree({
      id: 'repo-ssh::/home/FABRICA/runtime-wt',
      repoId: 'repo-ssh',
      path: '/home/FABRICA/runtime-wt',
      linkedPR: 5571
    })
    mockApi.worktrees.resolvePrBase.mockResolvedValueOnce({
      baseBranch: 'fork-head',
  ...
  it('waits for branch confirmation before linking a terminal PR URL for a known push target', async () => {
    ...
    const wt = makeWorktree({
      id: 'repo1::/path/wt1',
      repoId: 'repo1',
      path: '/worktrees/FABRICA',
      branch: 'refs/heads/feature/pr-link',
      pushTarget: {
        remoteName: 'origin',
        branchName: 'feature/pr-link',
        remoteUrl: 'https://github.com/acme/FABRICA.git'
      }
    })
    store.setState({
      repos: [
    ...
    expect(mockApi.worktrees.resolvePrBase).not.toHaveBeenCalled()
    expect(mockApi.worktrees.updateMeta).not.toHaveBeenCalled()
    expect(fetchPRForBranch).toHaveBeenCalledWith('/repos/FABRICA', 'feature/pr-link', {
      force: true,
      repoId: 'repo1',
      worktreeId: wt.id,
      linkedPRNumber: null,
  ...
  it('does not link an arbitrary same-repo terminal PR URL for a known push target when lookup misses', async () => {
    ...
    store.getState().observeTerminalGitHubPullRequestLink(wt.id, {
      url: 'https://github.com/acme/FABRICA/pull/1',
      slug: { owner: 'acme', repo: 'fabrica' },
      number: 1
    })
...
describe('fetchAllWorktrees hydration-time purge (design §4.4)', () => {
  ...
  it('preserves floating workspace state while purging a real stale worktree', async () => {
    ...
    store.setState({
      ...
      browserTabsByWorktree: {
        [FLOATING_TERMINAL_WORKTREE_ID]: [{ id: 'floating-browser', url: 'https://FABRICA.test' }]
      },
      activeBrowserTabIdByWorktree: {
    ...
    expect(store.getState().browserTabsByWorktree[FLOATING_TERMINAL_WORKTREE_ID]).toEqual([
      { id: 'floating-browser', url: 'https://FABRICA.test' }
    ])
