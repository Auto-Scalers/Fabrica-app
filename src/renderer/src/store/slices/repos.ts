...
async function warnIfProjectKnownInAnotherProfile(
  ...
  const findProjectProfiles = window.api.FABRICAProfiles?.findProjectProfiles
  ...
  if (!findProjectProfiles || !activeFABRICAProfileId) {
    return
  }
  try {
    const result = await findProjectProfiles({
      ...
      excludeProfileId: activeFABRICAProfileId
    })
...
async function assertProjectHostSetupRuntimeCapability(
  ...
  await assertRuntimeEnvironmentCapability(
    ...
    PROJECT_HOST_SETUP_RUNTIME_CAPABILITY,
    'The selected Fabrica server does not support project host setup yet. Update Fabrica on the server and try again.',
    15_000
  )
}
...
async function assertProjectHostSetupMutationRuntimeCapabilities(
  ...
  await assertRuntimeEnvironmentCapability(
    target.environmentId,
    WORKSPACE_RUN_CONTEXT_RUNTIME_CAPABILITY,
    'The selected Fabrica server does not support explicit workspace run hosts yet. Update Fabrica on the server and try again.',
    15_000
  )
}
...
function mergeFetchedProjectCompatibilityForHost({
  ...
  const setupBelongsToFetchedCatalog = (setup: ProjectHostSetup): boolean => {
    ...
    // Why: desktop persistence owns local and direct-SSH setups; runtime setups stay authoritative on their remote FABRICA server.
...
function filterTrustedFABRICAHooksToValidRepos(
  trust: AppState['trustedFABRICAHooks'],
  validRepoIds: Set<string>
): AppState['trustedFABRICAHooks'] {
  const next: AppState['trustedFABRICAHooks'] = {}
...
async function fetchRuntimeAddProjectPathStatus(args: {
  ...
  await assertRuntimeEnvironmentCapability(
    ...
    translate(
      ...
      'Update Fabrica server to open non-Git folders on this runtime.'
    ),
    15_000
  )
  try {
...
export const createRepoSlice: StateCreator<AppState, [], [], RepoSlice> = (set, get) => ({
    ...
    const validateRepoScopedUi = (): void => {
      set((s) => {
        ...
        return {
          ...
          trustedFABRICAHooks: filterTrustedFABRICAHooksToValidRepos(s.trustedFABRICAHooks, validRepoIds)
        }
      })
    }
    ...
    for (const projectId of targets.projectIds) {
      ...
      if (stillExists) {
        failedProjectRemovals.push({
          projectId,
          reason: 'Project remained in Fabrica after removeProject completed.'
        })
      } else {
  ...
  addRepoPath: async (path, kind = 'git', options) => {
    try {
      ...
      if (alreadyAdded) {
        get().clearFABRICAHookTrustForRepo(repo.id)
      }
      set((s) => {
      ...
      if (alreadyAdded) {
        ...
        await warnIfProjectKnownInAnotherProfile(repo, get().activeFABRICAProfileId)
      }
      return repo
    } catch (err) {
  ...
  removeProject: async (projectId, options) => {
    try {
      ...
      get().clearFABRICAHookTrustForRepo(projectId)
