...
function sanitizeTrustedFABRICAHooks(trust: unknown): PersistedTrustedFABRICAHooks {
  if (!isPlainPersistedRecord(trust)) {
    return {}
  }
  const next: PersistedTrustedFABRICAHooks = {}
  for (const [repoId, entry] of Object.entries(trust)) {
    if (!isSafePersistedRecordKey(repoId) || !isPlainPersistedRecord(entry)) {
      continue
    }
    next[repoId] = entry as PersistedTrustedFABRICAHooks[string]
  }
  return next
}

function filterTrustedFABRICAHooksToValidRepos(
  trust: unknown,
  validRepoIds: Set<string>
): PersistedTrustedFABRICAHooks {
  const sanitized = sanitizeTrustedFABRICAHooks(trust)
  const next: PersistedTrustedFABRICAHooks = {}
...
function hydrateTrustedFABRICAHooks(
  trust: unknown,
  validRepoIds: Set<string>
): PersistedTrustedFABRICAHooks {
  const sanitized = sanitizeTrustedFABRICAHooks(trust)
  ...
  return filterTrustedFABRICAHooksToValidRepos(sanitized, validRepoIds)
}
...
export type UISlice = {
  ...
  markFABRICAHookScriptConfirmed: (
    repoId: string,
    kind: FABRICAHookScriptKind,
    contentHash: string
  ) => void
  markFABRICAHookRepoAlwaysTrusted: (repoId: string) => void
  clearFABRICAHookTrustForRepo: (repoId: string) => void
...
export const createUISlice: StateCreator<AppState, [], [], UISlice> = (set, get) => ({
  ...
  trustedFABRICAHooks: {},
  markFABRICAHookScriptConfirmed: (repoId, kind, contentHash) =>
    set((s) => {
      const existing = s.trustedFABRICAHooks[repoId]
      ...
      const next = { ...s.trustedFABRICAHooks, [repoId]: nextRepo }
      window.api.ui.set({ trustedFABRICAHooks: next }).catch(console.error)
      return { trustedFABRICAHooks: next }
    }),
  ...
  clearFABRICAHookTrustForRepo: (repoId) =>
    set((s) => {
      if (!(repoId in s.trustedFABRICAHooks)) {
        return s
      }
      const next = { ...s.trustedFABRICAHooks }
      delete next[repoId]
      window.api.ui.set({ trustedFABRICAHooks: next }).catch(console.error)
      return { trustedFABRICAHooks: next }
    }),
      ...
      const hydrated = {
        ...
        contextualToursAutoEligible:
          typeof ui.contextualToursAutoEligible === 'boolean'
            ...
            : null,
        trustedFABRICAHooks: hydrateTrustedFABRICAHooks(ui.trustedFABRICAHooks, validRepoIds),
        setupScriptPromptDismissedRepoIds:
