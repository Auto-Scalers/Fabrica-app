...
describe('createBrowserSlice runtime guard', () => {
        ...
        profiles: [
          {
            id: 'default',
            scope: 'default',
            partition: 'persist:FABRICA-default',
            label: 'Default',
            source: null
          }
        ]
        ...
        profiles: [
          {
            id: 'remote-default',
            scope: 'default',
            partition: 'persist:FABRICA-remote',
    ...
    mockApi.browser.sessionListProfiles.mockResolvedValueOnce([
      {
        id: 'local-default',
        scope: 'default',
        partition: 'persist:FABRICA-local',
        label: 'Local Default',
        source: null
      }
    ])
  ...
  it('stores a runtime-resolved browser partition without a renderer profile mirror', () => {
    ...
    expect(tab.sessionPartition).toBe('persist:FABRICA-browser-session-profile-isolated')
    expect(store.getState().browserTabsByWorktree['wt-1']?.[0]?.sessionPartition).toBe(
      'persist:FABRICA-browser-session-profile-isolated'
    )
  })
  ...
  it('stores a runtime-resolved partition when switching browser tab profiles', () => {
    ...
    const tab = store.getState().createBrowserTab('wt-1', 'https://example.com', {
      ...
      sessionPartition: 'persist:FABRICA-browser'
    })
