...
export type AgentStatusSlice = {
  ...
  /** Ephemeral launch snapshots keyed by pane; hook payloads lack FABRICA launch settings, so the renderer supplies them from startup. */
  ...
  return {
    ...
    setAgentStatus: (paneKey, payload, terminalTitle, timing, routing, metadata) => {
      ...
      set((s) => {
        ...
        // Why: launch tokens can outlive an FABRICA-started TUI in the shell; once the session is done they must no longer authorize config reuse.
      ...
      if (entryForGeneratedTitle) {
        ...
        const liveIsDispatchPrompt = isFABRICADispatchPrompt(entryForGeneratedTitle.prompt)
        const liveDispatchTaskId = liveIsDispatchPrompt
          ? getFABRICADispatchTaskId(entryForGeneratedTitle.prompt)
          : null
      ...
      if (completionRefreshWorktreeId) {
        ...
        // Why: agents can create a PR via `gh pr create`, bypassing FABRICA's flow and leaving a stale "no PR" cache entry in place.
