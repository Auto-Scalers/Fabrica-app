...
export const createGitHubSlice: StateCreator<AppState, [], [], GitHubSlice> = (set, get) => ({
  ...
  patchProjectIssueOrPr: async (cacheKey, rowId, updates) => {
    ...
    // Why: slug-only Project rows have no registered FABRICA repo, so fall back to the view source in the cache key, not the focused host.
  ...
  fetchWorkItems: async (repoId, repoPath, limit, query, options): Promise<GitHubWorkItem[]> => {
    ...
    const request = (async () => {
      ...
      try {
        ...
        // Why: stamp repoId at the fetch boundary so downstream consumers can rely on it — main doesn't know FABRICA's Repo.id.
        ...
        const errorFFABRICAche: WorkItemsCacheError | undefined =
        ...
        set((s) => ({
          workItemsCache: withBoundedCacheEntry(s.workItemsCache, key, {
            ...
            ...(errorFFABRICAche ? { error: errorFFABRICAche } : {}),
