...
// Mirrors the real report: one project name ("FABRICA") set up on the local Mac and on a
// remote FABRICA server, where only the local repo row carries the user's chosen color.
const SHARED_PROJECT_ID = 'github:Auto-Scalers/Fabrica-app'
...
function runtimeResult(method: string): unknown {
  ...
  if (method === 'projectHostSetup.list') {
    return { setups: [setup(SHARED_PROJECT_ID, 'remote-repo', '/srv/FABRICA')] }
  }
  return {}
}
...
beforeEach(() => {
  ...
  listHostSetups.mockResolvedValue([setup(SHARED_PROJECT_ID, 'local-repo', '/local/FABRICA')])
