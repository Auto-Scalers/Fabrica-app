...
export type OpenFile = {
  ...
  /** Why: routes an FABRICA-owned move's destination-watcher echo into content verification. On the tab so it survives the atomic rekey; operationId supersedes a stale verification on re-move. Not persisted. */
...
export type EditorSlice = {
  ...
  /** Atomically retargets open editor sessions across an FABRICA-owned move — one commit-only update migrating every path-derived id + all id-keyed state, no close/reopen. Returns collision/stale without mutating. */
...
/** One tab's migration in an FABRICA-owned move; precomputed by the move coordinator. */
  ...
  activateMarkdownLink: async (rawHref, ctx) => {
    ...
    if (target.kind === 'file') {
      ...
      if (target.relativePath === undefined) {
        ...
        // Why: markdown file:// links need the same user-gesture authorization terminal links get, so external paths (e.g. /tmp screenshots) can open in FABRICA.
