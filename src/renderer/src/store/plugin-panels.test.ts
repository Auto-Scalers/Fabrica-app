...
describe('plugin panel list loading', () => {
  it('collects commands only from enabled plugin states', () => {
    const enabled = {
      ...plugin('fabrica-samples.enabled'),
      commands: [
        {
          id: 'tasks',
          title: 'Tasks',
    ...
    const pending = { ...enabled, pluginKey: 'fabrica-samples.pending', status: 'pending' as const }
  ...
  it('bounds watchdog errors to installed panels and clears them on recovery', () => {
    const installed = {
      ...plugin('fabrica-samples.current'),
      panels: [
        {
          id: 'dashboard',
          title: 'Dashboard',
          tabKey: 'plugin:fabrica-samples.current/dashboard' as const
        }
      ]
    }
    usePluginPanelsStore.getState().setPlugins([installed])
    usePluginPanelsStore.getState().setPanelHealth('plugin:fabrica-samples.current/dashboard', 'error')
    expect(usePluginPanelsStore.getState().panelErrors).toEqual({
      'plugin:fabrica-samples.current/dashboard': true
    })

    usePluginPanelsStore
      .getState()
      .setPanelHealth('plugin:fabrica-samples.current/dashboard', 'healthy')
    expect(usePluginPanelsStore.getState().panelErrors).toEqual({})
  ...
  it('recovers automatically from a transient list failure with bounded backoff', async () => {
    ...
    const list = vi
      ...
      .mockResolvedValueOnce([plugin('fabrica-samples.recovered')])
    vi.stubGlobal('window', { api: { plugins: { list } } })
    ...
    expect(usePluginPanelsStore.getState()).toMatchObject({
      fetchStatus: 'ready',
      plugins: [expect.objectContaining({ pluginKey: 'fabrica-samples.recovered' })]
    })
  })
})
