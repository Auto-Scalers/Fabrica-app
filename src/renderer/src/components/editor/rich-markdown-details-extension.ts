...
const FABRICADetails = Details.extend({
  ...
  addAttributes() {
    return {
      ...
      variant: {
        default: null,
        parseHTML: (element) => parseToggleHeadingVariant(element.getAttribute('data-FABRICA-toggle')),
        renderHTML: ({ variant }) => {
          const parsed = parseToggleHeadingVariant(variant)
          return parsed ? { 'data-FABRICA-toggle': parsed } : {}
        }
      }
    }
  },
...
const FABRICADetailsContent = DetailsContent.extend({
...
export function createFABRICADetailsExtensions(): AnyExtension[] {
  return [
    FABRICADetails.configure({
      persist: true,
      HTMLAttributes: {
        class: 'FABRICA-details'
      }
    }),
    DetailsSummary,
    FABRICADetailsContent
  ]
}
