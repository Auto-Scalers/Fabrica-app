...
function makeMarkerDecoration(
  ...
  return {
    ...
    options: {
      ...
      marginClassName: 'FABRICA-conflict-margin',
      hoverMessage: { value: label },
      linesDecorationsTooltip: label,
      after: {
        content: ` ${label}`,
        inlineClassName: 'FABRICA-conflict-marker-label'
      }
    }
  }
}
...
function makeSectionDecoration(
  ...
  return {
    ...
    options: {
      isWholeLine: true,
      className: `FABRICA-conflict-section-line FABRICA-conflict-${section}-line`
    }
  }
}
