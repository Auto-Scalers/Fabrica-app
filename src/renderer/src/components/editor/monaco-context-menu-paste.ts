...
// reads the clipboard with navigator.clipboard.readText(). FABRICA runs the renderer with
...
// (execCommand('copy') from a user gesture) both work. We route the read through FABRICA's
// trusted clipboard IPC bridge instead, matching how the terminal already reads it.
export const FABRICA_CONTEXT_MENU_PASTE_PRIORITY = 10001
export const FABRICA_CONTEXT_MENU_PASTE_NAME = 'FABRICA-ipc-paste'
...
export type FABRICAContextMenuPasteDeps = {
...
export type FABRICAContextMenuPasteOutcome =
 ...
 * bridge); otherwise performs the paste through FABRICA's IPC clipboard read and
 * returns a Promise (truthy) so Monaco's blocked default never runs.
 */
export function runFABRICAContextMenuPaste(
  deps: FABRICAContextMenuPasteDeps
): false | Promise<FABRICAContextMenuPasteOutcome> {
  ...
  return performFABRICAContextMenuPaste(editorInstance, deps)
}

async function performFABRICAContextMenuPaste(
  editorInstance: PasteCapableEditor,
  deps: FABRICAContextMenuPasteDeps
): Promise<FABRICAContextMenuPasteOutcome> {
  let text: string
  try {
