...
export function parseDetailsAttributes(rawAttributes: string): Record<string, unknown> {
  ...
  const variantMatch = rawAttributes.match(
    /\sdata-FABRICA-toggle\s*=\s*(?:"(heading-[1-5])"|'(heading-[1-5])'|(heading-[1-5]))(?:\s|$)/i
  )
  return {
...
export function renderDetailsAttributes(attrs: Record<string, unknown> | undefined): string {
  const attributes = ['class="FABRICA-details"']

  const variant = parseToggleHeadingVariant(attrs?.variant)
  if (variant) {
    attributes.push(`data-FABRICA-toggle="${variant}"`)
  }
...
function hasOnlySupportedDetailsAttributes(rawAttributes: string): boolean {
  return (
    rawAttributes
      ...
      .replace(/\s+class\s*=\s*(?:"FABRICA-details"|'FABRICA-details'|FABRICA-details)(?=\s|$)/giu, '')
      .replace(
        /\s+data-FABRICA-toggle\s*=\s*(?:"heading-[1-5]"|'heading-[1-5]'|heading-[1-5])(?=\s|$)/giu,
        ''
      )
      .trim() === ''
  )
