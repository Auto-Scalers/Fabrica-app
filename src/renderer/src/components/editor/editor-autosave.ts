...
export const FABRICA_EDITOR_QUIESCE_FILE_SAVES_EVENT = 'FABRICA:editor-quiesce-file-saves'
export const FABRICA_EDITOR_EXTERNAL_FILE_CHANGE_EVENT = 'FABRICA:editor-external-file-change'
export const FABRICA_EDITOR_SAVE_FILE_EVENT = 'FABRICA:editor-save-file'
export const FABRICA_EDITOR_SAVE_AND_CLOSE_EVENT = 'FABRICA:save-and-close'
export const FABRICA_EDITOR_FILE_SAVED_EVENT = 'FABRICA:editor-file-saved'
export const FABRICA_EDITOR_REQUEST_CMD_SAVE_EVENT = 'FABRICA:editor-request-cmd-save'
export const FABRICA_EDITOR_REQUEST_FILE_CLOSE_EVENT = 'FABRICA:editor-request-file-close'
...
export async function requestEditorSaveQuiesce(target: EditorSaveQuiesceTarget): Promise<void> {
  await new Promise<void>((resolve) => {
    ...
    window.dispatchEvent(
      new CustomEvent<EditorSaveQuiesceDetail>(FABRICA_EDITOR_QUIESCE_FILE_SAVES_EVENT, {
        detail: {
          ...target,
          claim: () => {
            claimed = true
...
export async function requestEditorFileSave(target: EditorSaveFileTarget): Promise<void> {
  await new Promise<void>((resolve, reject) => {
    let claimed = false
    window.dispatchEvent(
      new CustomEvent<EditorSaveFileDetail>(FABRICA_EDITOR_SAVE_FILE_EVENT, {
        detail: {
          ...target,
          claim: () => {
            claimed = true
...
export function requestEditorFileClose(fileId: string): void {
  window.dispatchEvent(
    new CustomEvent<EditorRequestFileCloseDetail>(FABRICA_EDITOR_REQUEST_FILE_CLOSE_EVENT, {
      detail: { fileId }
    })
  )
}
...
export function notifyEditorExternalFileChange(target: EditorPathMutationTarget): void {
  window.dispatchEvent(
    new CustomEvent<EditorPathMutationTarget>(FABRICA_EDITOR_EXTERNAL_FILE_CHANGE_EVENT, {
      detail: target
    })
  )
}
