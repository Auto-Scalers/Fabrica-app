...
import { runFABRICAContextMenuPaste, type FABRICAContextMenuPasteDeps } from './monaco-context-menu-paste'
...
function makeDeps(
  overrides: Partial<FABRICAContextMenuPasteDeps> & { editor?: ReturnType<typeof makeEditor> }
): FABRICAContextMenuPasteDeps {
...
describe('runFABRICAContextMenuPaste', () => {
  ...
  it('reads via IPC and dispatches a native paste for writable focused editors', async () => {
    ...
    const outcome = runFABRICAContextMenuPaste(deps)
  ...
  it('applies in-app copy metadata (empty-selection + multicursor) when enabled', async () => {
    ...
    await runFABRICAContextMenuPaste(deps)
    expect(handle.trigger).toHaveBeenCalledWith('keyboard', 'paste', {
      text: 'line text',
      pasteOnNewLine: true,
      multicursorText: ['a', 'b'],
  ...
  it('does nothing on an empty clipboard', async () => {
    ...
    await expect(runFABRICAContextMenuPaste(deps)).resolves.toEqual({
      status: 'noop',
      reason: 'empty'
    })
  ...
  it('routes oversized pastes through the chunked inserter instead of a native trigger', async () => {
    ...
    const outcome = await runFABRICAContextMenuPaste(deps)
