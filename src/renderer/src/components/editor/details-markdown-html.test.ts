...
describe('details markdown html', () => {
  ...
  it('accepts heading-5 toggle variants and rejects unsupported levels', () => {
    ...
    expect(parseDetailsAttributes(' data-FABRICA-toggle="heading-5"')).toMatchObject({
      variant: 'heading-5'
    })
    expect(parseDetailsAttributes(' data-FABRICA-toggle="heading-6"')).toMatchObject({
      variant: null
    })

    const editableHeading5: DetailsHtmlBlock = {
      raw: '',
      openingAttributes: ' data-FABRICA-toggle="heading-5"',
      inner: '<summary>Toggle</summary><p>Body</p>',
      hasNestedDetails: false
    }
    const unsupportedHeading6: DetailsHtmlBlock = {
      raw: '',
      openingAttributes: ' data-FABRICA-toggle="heading-6"',
      inner: '<summary>Toggle</summary><p>Body</p>',
      hasNestedDetails: false
    }
