...
export function useEditorPanelExternalContentEvents({
  ...
  useEffect(() => {
    ...
    window.addEventListener(FABRICA_EDITOR_FILE_SAVED_EVENT, handler as EventListener)
    return () => window.removeEventListener(FABRICA_EDITOR_FILE_SAVED_EVENT, handler as EventListener)
