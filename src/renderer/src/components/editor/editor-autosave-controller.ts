...
import {
  ...
  FABRICA_EDITOR_FILE_SAVED_EVENT,
  FABRICA_EDITOR_QUIESCE_FILE_SAVES_EVENT,
  FABRICA_EDITOR_SAVE_AND_CLOSE_EVENT,
  FABRICA_EDITOR_SAVE_FILE_EVENT,
...
import {
  FABRICA_EDITOR_PREPARE_HOT_EXIT_EVENT,
  FABRICA_EDITOR_SAVE_DIRTY_FILES_EVENT,
...
export function attachEditorAutosaveController(store: AppStoreApi): () => void {
    ...
    const queuedSave = previousSave
      ...
      .then(async () => {
        ...
        window.dispatchEvent(
          new CustomEvent<EditorFileSavedDetail>(FABRICA_EDITOR_FILE_SAVED_EVENT, {
  ...
  const handleExternalFileChange = (event: Event): void => {
    ...
    for (const file of matchingFiles) {
      if (file.isDirty) {
        // Why: skip FABRICA's own-save echo, which routes here bypassing the watch hook's echo verification.
  ...
  window.addEventListener(FABRICA_EDITOR_SAVE_DIRTY_FILES_EVENT, handleSaveDirtyFiles as EventListener)
  window.addEventListener(FABRICA_EDITOR_PREPARE_HOT_EXIT_EVENT, handlePrepareHotExit as EventListener)
  window.addEventListener(FABRICA_EDITOR_SAVE_AND_CLOSE_EVENT, handleSaveAndClose as EventListener)
  window.addEventListener(FABRICA_EDITOR_SAVE_FILE_EVENT, handleSaveFile as EventListener)
  window.addEventListener(FABRICA_EDITOR_QUIESCE_FILE_SAVES_EVENT, handleQuiesce as EventListener)
  ...
  return () => {
    ...
    window.removeEventListener(FABRICA_EDITOR_SAVE_FILE_EVENT, handleSaveFile as EventListener)
    window.removeEventListener(FABRICA_EDITOR_QUIESCE_FILE_SAVES_EVENT, handleQuiesce as EventListener)
    window.removeEventListener(
      FABRICA_EDITOR_EXTERNAL_FILE_CHANGE_EVENT,
      handleExternalFileChange as EventListener
    )
