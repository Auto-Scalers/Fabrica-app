...
function GitHubStarButton({ hasRepos }: { hasRepos: boolean }): React.JSX.Element | null {
  ...
  const handleClick = async (): Promise<void> => {
    ...
    const ok = await window.api.gh.starFABRICA('landing')
    if (!ok) {
      ...
      <div className="w-full max-w-lg px-6">
        <div className="flex flex-col items-center gap-4 py-8">
          ...
          >
            <img
              src={logo}
              alt={translate('auto.components.Landing.520304a067', 'Fabrica logo')}
              className="size-12"
            />
          </div>
          <h1 className="text-4xl font-bold text-foreground tracking-tight">
            {translate('auto.components.Landing.6ca6ff404e', 'FABRICA')}
          </h1>
