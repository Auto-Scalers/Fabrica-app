...
export const STARTUP_CWD_FALLBACK_NOTICE =
  '\r\n[Fabrica opened this terminal at the workspace root because its saved start folder no longer exists.]\r\n'
...
const HIDDEN_OUTPUT_RESTORE_UNAVAILABLE_WARNING =
  '\x18\x1b[0m\r\n[Fabrica skipped hidden terminal output because main recovery was unavailable.]\r\n'
...
export function connectPanePty(
  ...
  const registerEffectiveLaunchConfig = (
    ...
    if (!effectiveLaunchConfig) {
      if (metadata?.launchAgent) {
        // Why: daemon launch identity can outlive the process while FABRICA is
  ...
  const scheduleReattachIdleAgentCursorReset = (): void => {
    ...
    reattachIdleAgentCursorResetTimer = setTimeout(() => {
      ...
      // fire because the agent was already idle before FABRICA restarted.
      queueAgentIdleTerminalModeReset()
  ...
  const dropCommandFinishedStatusIfSameTurn = (
    ...
    const state = useAppStore.getState()
    if (!entry) {
      // Why: an FABRICA-started agent can exit before its first hook status. The
  ...
  const onTerminalKeyDown = (event: KeyboardEvent): void => {
    ...
    // Why: user shell frameworks (bash-preexec/iTerm2) can replace FABRICA's
  ...
  const onBell = (): void => {
    ...
    // standalone BEL some time after daemon snapshot reattach, even when FABRICA
  ...
  // Why: inject FABRICA_PANE_KEY so global Claude/Codex hooks can attribute their
  // callbacks to the correct FABRICA pane without resolving worktrees from cwd.
  // The key matches the `${tabId}:${leafId}` composite used for cacheTimerByKey
  // and agentStatusByPaneKey. Treat it as opaque outside FABRICA.
  const state = useAppStore.getState()
  ...
  const folderWorkspace =
    parsedWorkspaceKey?.type === 'folder'
      ...
      : null
  const workspaceEnv: Record<string, string> = { FABRICA_WORKSPACE_ID: deps.worktreeId }
  if (folderWorkspace) {
    workspaceEnv.FABRICA_PROJECT_GROUP_ID = folderWorkspace.projectGroupId
    workspaceEnv.FABRICA_WORKSPACE_ROOT = folderWorkspace.folderPath
  }
  const paneIdentityEnv = {
    ...workspaceEnv,
    FABRICA_PANE_KEY: cacheKey,
    FABRICA_TAB_ID: deps.tabId,
    FABRICA_WORKTREE_ID: deps.worktreeId,
    ...(launchToken ? { FABRICA_AGENT_LAUNCH_TOKEN: launchToken } : {})
  }
  const paneEnv = {
    ...paneStartup?.env,
    ...paneIdentityEnv
  ...
  const transportOptions = {
    ...
    // Why: local IPC terminals are now model-owned in main: FABRICARuntimeService
  ...
  const runDeferredConnect = (): void => {
    ...
    const buildColdRestoreAgentResumeStartup = (): ColdRestoreAgentResumeStartup | null => {
      ...
      return {
        ...
        env: {
          ...startupPlan.env,
          FABRICA_AGENT_LAUNCH_TOKEN: coldRestoreLaunchToken
        },
    ...
    const mergeStartupEnvWithPaneIdentity = (
      ...
      env
        ? {
            ...env,
            ...paneIdentityEnv,
            ...(env.FABRICA_AGENT_LAUNCH_TOKEN
              ? { FABRICA_AGENT_LAUNCH_TOKEN: env.FABRICA_AGENT_LAUNCH_TOKEN }
              : {})
          }
        : undefined
    ...
    const schedulePendingStartupCommandDelivery = (): void => {
      ...
      if (!sshStartupShellReady) {
        if (sshShellReadyFallbackTimer === null) {
          // Why: some SSH shells cannot emit FABRICA's ready marker. Prefer the
    ...
    const reattachReplayResetSequence = (
      ...
      // Why: an alt-screen pane is a live TUI FABRICA just does not recognise as an agent, and the
