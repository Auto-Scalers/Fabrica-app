...
export function getTerminalFABRICAFileOpenHint(showActions = true): string {
  const prefix = showActions ? 'Click for actions or ' : ''
  return isMacPlatform()
    ? `${prefix}⌘+click to open in Fabrica`
    : `${prefix}Ctrl+click to open in Fabrica`
}
...
// Why: openHttpLink only routes to FABRICA when the source is local, so a remote pane
// pins every link to the system browser and inverting cannot reach FABRICA there. The
...
export function getTerminalUrlOpenHint(options: TerminalUrlOpenHintOptions = {}): string {
  const invertsToFABRICA = options.modifierInverts === true && options.openLinksInApp !== true
  const prefix = terminalLinkActionHintPrefix(options.showActions !== false)
  if (invertsToFABRICA) {
    return isMacPlatform()
      ? `${prefix}⌘+click to open, or ⇧⌘+click to open in Fabrica`
      : `${prefix}Ctrl+click to open, or Shift+Ctrl+click to open in Fabrica`
  }
  return isMacPlatform()
...
export function getTerminalUrlFABRICABrowserHint(): string {
  return isMacPlatform() ? '⇧⌘+click to open in Fabrica' : 'Shift+Ctrl+click to open in Fabrica'
}
