...
describe('connectPanePty', () => {
  ...
  beforeEach(() => {
    ...
    mockStoreState = {
      ...
      repos: [{ id: 'repo1', connectionId: null, displayName: 'FABRICA' }],
      projects: [],
  ...
  it.each(['rejects', 'resolves empty'] as const)(
    ...
    async (outcome) => {
      ...
      mockStoreState = {
        ...
        ptyIdsByTabId: { 'tab-1': [] },
        repos: [{ id: 'repo1', connectionId: 'target-a', displayName: 'FABRICA' }],
        sshConnectionStates: new Map([
          [
            'target-a',
            {
  ...
  it('waits for shell-ready before unhinted SSH startup commands', async () => {
    ...
    try {
      ...
      capturedDataCallback.current?.('\x1b]777;FABRICA-shell-ready\x07user@remote $ ')
  ...
  it('orders a startup draft behind existing user input when Codex renders its composer', async () => {
    ...
    const deps = createDeps({
      startup: {
        ...
        draftPrompt: 'https://github.com/Auto-Scalers/Fabrica-app/issues/42'
      }
    })
    ...
    expect(transport.sendInputAccepted).toHaveBeenCalledWith(
      '\x1b[200~https://github.com/Auto-Scalers/Fabrica-app/issues/42\x1b[201~'
    )
  ...
  it('clears pre-hook launch config when an Fabrica-started command exits', async () => {
    vi.useFakeTimers({ toFake: ['setTimeout'] })
    const { connectPanePty } = await import('./pty-connection')
  ...
  it('confirms an Fabrica-launched Droid fresh spawn in a no-OSC shell (Git Bash)', async () => {
      ...
      expect(transport.connect).toHaveBeenNthCalledWith(
        ...
        expect.objectContaining({
          ...
          env: expect.objectContaining({
            ...
            FABRICA_WORKSPACE_ID: 'wt-1',
            FABRICA_AGENT_LAUNCH_TOKEN: expect.stringMatching(new RegExp(`^${UUID_RE}$`))
          })
        })
      )
  ...
  it('uses sleeping-record launch config for pane cold restore after settings change', async () => {
    ...
    const launchConfig = {
      ...
      agentEnv: {
        ...
        FABRICA_PANE_KEY: 'wrong-pane',
        FABRICA_TAB_ID: 'wrong-tab',
        FABRICA_WORKTREE_ID: 'wrong-worktree',
        FABRICA_WORKSPACE_ID: 'wrong-workspace'
      }
    }
    mockStoreState = {
      ...mockStoreState,
  ...
  it('does not schedule hidden atlas recovery for ordinary rich text or metadata output', async () => {
    ...
    vi.useFakeTimers()
    try {
      capturedDataCallback.current?.('plain hidden emoji ?? and CJK ????\r\n')
  ...
  describe('mode 2031 replies are decided per raw PTY chunk', () => {
    ...
    const FISH_PROMPT_HANDOFF = '\x1b[?2031h\x1b[0m~/FABRICA \x1b[32m?\x1b[0m \x1b[?2031l'
  ...
  it('pauses capable paired output while hidden and restores exactly on reveal', async () => {
    ...
    mockStoreState.repos = [
      { id: 'repo1', connectionId: null, displayName: 'FABRICA', executionHostId: 'runtime:env-1' }
    ]
  ...
  it('retries null remote snapshots for overflowed hidden runtime output', async () => {
    ...
    expect(pane.terminal.write).not.toHaveBeenCalledWith(
      expect.stringContaining('Fabrica skipped hidden terminal output'),
      expect.any(Function)
    )
    expect(pane.terminal.write).not.toHaveBeenCalledWith(
  ...
  it('does not switch renderers for Arabic output', async () => {
    ...
    capturedDataCallback.current?.('Arabic: ?????? ?????\r\n')
  ...
  it('does not schedule WebGL atlas recovery for ordinary foreground shell rewrites', async () => {
    ...
    try {
      ...
      capturedDataCallback.current?.('\r\x1b[KFABRICA % npm test')
  ...
  it('runs an inline setup terminal locally instead of failing its host closed', async () => {
    ...
    const setupWorktreeId =
      'ephemeral-setup-terminal:settings-mobile-emulator-FABRICA-cli-skill-terminal'
    mockStoreState = {
      ...mockStoreState,
  ...
  it('withholds a local spawn while a repo-backed worktree has no hydrated host', async () => {
    ...
    const deps = createDeps({
      ...
      cwd: '/tmp/FABRICA-docker-relay-perf-repo'
    })
    connectPanePty(createPane(1) as never, createManager(1) as never, deps as never)
