...
export type TerminalHttpLinkDestination = 'FABRICA' | 'system'
...
export function handleTerminalHttpLink(
  ...
  const labelForDestination = (destination: TerminalHttpLinkDestination): string =>
    destination === 'FABRICA'
      ? translate(
          'auto.components.terminal.pane.TerminalLinkActionPopover.FABRICABrowser',
          'Fabrica Browser'
        )
      : translate(
...
export function openTerminalHttpLink(url: string, deps: UrlLinkHitTestDeps): void {
  // Why: FABRICA browser tabs are local-only, so a link clicked in a runtime-hosted
  ...
  if (deps.forceDestination) {
    openHttpLink(url, {
      worktreeId: deps.worktreeId,
      forceInApp: deps.forceDestination === 'FABRICA',
  ...
  void Promise.resolve(preferenceDecision)
    .then((openInFABRICA) => {
      openHttpLink(url, {
        worktreeId: deps.worktreeId,
        forceSystemBrowser: !openInFABRICA,
        sourceOwner
      })
    })
    .catch(() => {
