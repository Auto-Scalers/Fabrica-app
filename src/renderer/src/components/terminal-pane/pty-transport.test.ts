...
describe('createIpcPtyTransport', () => {
  ...
  it('forwards requested environment deletions to the PTY spawn', async () => {
    ...
    expect(spawn).toHaveBeenCalledWith(
      expect.objectContaining({ envToDelete: ['CODEX_HOME', 'FABRICA_CODEX_HOME'] })
    )
  })
  ...
  it('suppresses the SSH-not-active toast for a runtime-owned (per-workspace-env) target', async () => {
    ...
    const spawnMock = vi
      .fn()
      .mockRejectedValue(new Error('No PTY provider for connection runtime-ssh-FABRICA-1'))
    ;(globalThis as { window: typeof window }).window = {
      ...originalWindow,
      api: {
        ...originalWindow?.api,
    ...
    const onError = vi.fn()
    await createIpcPtyTransport({ connectionId: 'runtime-ssh-FABRICA-1' }).connect({
      url: '',
      callbacks: { onError }
    })
