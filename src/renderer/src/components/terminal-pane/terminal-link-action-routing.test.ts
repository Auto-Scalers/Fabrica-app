...
describe('terminal link action routing', () => {
  ...
  it('offers system browser first and Fabrica second when system browser is the default', () => {
    const request = vi.fn()
    ...
    expect(
      handleTerminalHttpLink('https://example.com/path', event, {
        ...
        actionDestinations: { primary: 'system', alternate: 'FABRICA' }
      })
    ).toBe(true)
    ...
    expect(request.mock.calls[0][0].alternate.label).toBe('Fabrica Browser')
  ...
  it('offers Fabrica first and system browser second when Fabrica is the default', () => {
    const request = vi.fn()

    handleTerminalHttpLink('https://example.com/path', plainEvent(), {
      worktreeId: 'wt-1',
      linkActionContext: actionContext(request),
      actionDestinations: { primary: 'FABRICA', alternate: 'system' }
    })

    expect(request.mock.calls[0][0].primary.label).toBe('Fabrica Browser')
