...
const INDENT = ''
const FULL_URL = [
  'http://127.0.0.1:8765/FABRICA-double-open-repro-wrapped/',
...
describe('hard-wrapped terminal HTTP clicks', () => {
  ...
  it('does not glue the next logical line onto a URL that ends mid-row (#8832)', () => {
    const { terminal, registrations } = makeTerminal({
      cols: 80,
      urlRows: ['Repo: https://github.com/Auto-Scalers/Fabrica-app/', 'Description: 123'],
      softWrapped: false
    })
    const disposable = installHttpLinkClickFallback(terminal, { worktreeId: 'wt-1' })
    ...
    expect(openUrlMock).toHaveBeenCalledOnce()
    expect(openUrlMock).toHaveBeenCalledWith('https://github.com/Auto-Scalers/Fabrica-app/')
    disposable.dispose()
  })
