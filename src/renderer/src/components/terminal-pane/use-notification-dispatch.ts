...
import {
  isFABRICAWindowForegroundFocused,
...
export function dispatchTerminalNotification(
  ...
  if (event.source === 'agent-task-complete') {
    ...
    const shouldMarkUnread = event.paneKey
      ...
      : state.activeWorktreeId !== worktreeId || !isFABRICAWindowForegroundFocused()
    if (shouldMarkUnread) {
      // Why: activeWorktreeId is only in-app selection. If FABRICA is backgrounded,
