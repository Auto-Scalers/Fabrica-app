...
function TerminalPane(
  ...
  const requestOpenLinksInAppPreference = useCallback(
    (url: string): Promise<boolean> | null => {
      ...
      const preferencePromise = (async () => {
        const openInFABRICA = await requestLinkRoutingPreference({
          openLinksInAppDefault: settingsRef.current?.openLinksInApp === true,
          url
        })
        await updateSettings({
          openLinksInApp: openInFABRICA,
          openLinksInAppPreferencePrompted: true
        })
        return openInFABRICA
      })()
  ...
  useEffect(() => {
    ...
    if (
      !isHostAuthoritativeLayout({
        isWebClient: !!(globalThis as { __FABRICA_WEB_CLIENT__?: boolean }).__FABRICA_WEB_CLIENT__,
  ...
  useTerminalKeyboardShortcuts({
    ...
    paneKittyKeyboardModesRef,
    keybindings,
    terminalShortcutPolicy: settings?.terminalShortcutPolicy ?? 'FABRICA-first'
  })
  ...
  useEffect(() => {
    if (
      !(globalThis as { __FABRICA_WEB_CLIENT__?: boolean }).__FABRICA_WEB_CLIENT__ ||
      !isVisible ||
      !isActive
    ) {
      return
  ...
  const terminalContainerStyle: CSSProperties = {
    ...
    overflow: 'hidden',
    ...hiddenStartupStyle,
    ['--fabrica-terminal-divider-color' as string]:
      effectiveAppearance?.dividerColor ?? DEFAULT_TERMINAL_DIVIDER_DARK,
    ['--fabrica-terminal-divider-color-strong' as string]: normalizeColor(
