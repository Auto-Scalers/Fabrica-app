...
const OPENED_COMPUTER_USE_SETUP: ComputerUsePermissionSetupResult = {
  platform: 'darwin',
  helperAppPath: '/Applications/Fabrica.app',
  openedSettings: true,
  launchedHelper: true
}
...
describe('onboarding feature setup runner', () => {
  ...
  it('builds one skill command for selected onboarding feature setup skills', () => {
    ...
    expect(text).toBe(
      'npx skills add https://github.com/Auto-Scalers/Fabrica-app --skill FABRICA-cli --skill computer-use --skill orchestration --skill fabrica-linear --global'
    )
  })
  ...
  it('skips openSetup and warns when the macOS Computer Use helper app is unavailable', async () => {
    ...
    const unavailableStatus: ComputerUsePermissionStatusResult = {
      platform: 'darwin',
      helperAppPath: null,
      helperUnavailableReason: 'Fabrica Computer Use.app was not found',
      permissions: [
        { id: 'accessibility', status: 'not-granted' },
    ...
    expect(result.warnings).toContainEqual({
      ...
      message: 'Fabrica Computer Use.app was not found'
    })
  })
  ...
  it('shows CLI registration context before installing a missing CLI during onboarding', async () => {
    const staleStatus: CliInstallStatus = {
      ...
      state: 'stale',
      currentTarget: '/tmp/other-FABRICA',
      detail: '/usr/local/bin/FABRICA points to a different launcher.'
    }
