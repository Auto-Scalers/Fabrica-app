...
import { showFABRICACliRegistrationPromptToast } from '@/lib/agent-skill-cli-prerequisite'
...
export function createOnboardingFeatureSetupDeps(
  ...
  // Register `FABRICA` on the same PATH used by the skill install (#12103).
  ...
  return {
    ...
    showCliRegistrationPrompt: showFABRICACliRegistrationPromptToast,
    installCli: () =>
...
export async function runOnboardingFeatureSetup(
  ...
  try {
    ...
    if (!status.supported) {
      warnings.push({
        featureId: 'cli',
        message: status.detail ?? 'Fabrica CLI registration is not available on this platform.'
      })
    } else if (status.pathConfigured === null) {
      // Why: an unknown registry read cannot safely drive a PATH read-modify-write.
      warnings.push({
        featureId: 'cli',
        message: status.detail ?? 'Fabrica could not check your Windows user PATH.'
      })
      ...
      if (next.state !== 'installed') {
        warnings.push({
          featureId: 'cli',
          message: next.detail ?? 'Fabrica CLI registration needs attention.'
        })
