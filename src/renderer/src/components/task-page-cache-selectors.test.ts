...
describe('task page cache selectors', () => {
  ...
  it('scopes repo source rows by source cache scope for retry ownership', () => {
    const localRepo = {
      id: 'repo-1',
      path: '/same/path',
      sourceCacheScope: 'source:local:github:Auto-Scalers/Fabrica-app'
    }
    const sshRepo = {
      id: 'repo-1',
      path: '/same/path',
      sourceCacheScope: 'source:ssh:devbox:github:Auto-Scalers/Fabrica-app'
    }

    expect(buildTaskPageRepoSourceState([localRepo, sshRepo], [])).toMatchObject([
      {
        sourceKey: 'repo-1::source:local:github:Auto-Scalers/Fabrica-app'
      },
      {
        sourceKey: 'repo-1::source:ssh:devbox:github:Auto-Scalers/Fabrica-app'
      }
    ])
  })
