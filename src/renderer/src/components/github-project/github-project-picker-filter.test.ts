...
describe('github-project-picker-filter', () => {
  it('excludes pinned and recent projects from browse results', () => {
    const projects = [
      project('autoscalers', 1, 'Pinned Project'),
      project('autoscalers', 2, 'Recent Project'),
      project('autoscalers', 3, 'Browse Project')
    ]

    expect(
      filterGitHubProjectPickerProjects({
        projects,
        pinned: [{ owner: 'Auto-Scalers', ownerType: 'organization', number: 1 }],
        recent: [
          {
            owner: 'Auto-Scalers',
  ...
  it('matches project title, owner, and number case-insensitively', () => {
    const projects = [
      project('autoscalers', 42, 'Roadmap'),
  ...
  it('enforces the query budget by UTF-8 byte length', () => {
    ...
    expect(
      filterGitHubProjectPickerProjects({
        projects: [project('autoscalers', 1, 'Roadmap')],
        pinned: [],
        recent: [],
        query
      })
