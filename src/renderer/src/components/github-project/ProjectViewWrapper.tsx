...
const FABRICA_FEATURE_REQUEST_URL = 'https://github.com/Auto-Scalers/Fabrica-app/issues/new'
...
export default function ProjectViewWrapper({ selectedRepoIds }: Props): React.JSX.Element {
  ...
  // Why: matched-repo rows open `GitHubItemDialog`, unmatched the slug dialog; `repoNotInFABRICA` drives the `repo-not-in-FABRICA` modal.
  ...
  // Why: slug dialog only serves unregistered-repo rows; the parent (not this dialog) owns the repo-not-in-FABRICA "Start work" flow.
  const [slugDialog, setSlugDialog] = useState<{
    origin: GitHubItemDialogProjectOrigin
  } | null>(null)
  const [repoNotInFABRICA, setRepoNotInFABRICA] = useState<{
    owner: string
    repo: string
    host?: string
    url: string | null
  ...
  if (resolvedDialogRepoItem !== dialogRepoItem) {
    // Why: clear the repo-backed dialog when its repo leaves FABRICA, before the modal tree gets stale repo ids.
  ...
  if (resolvedMissingRepoDialogs.repoNotInFABRICA !== repoNotInFABRICA) {
    setRepoNotInFABRICA(resolvedMissingRepoDialogs.repoNotInFABRICA)
  }
  ...
  return (
    <div className="flex min-h-0 min-w-0 flex-1 flex-col">
      ...
      {/* Slug-only dialog for unadded-repo rows; Start-work lives in the parent's `repoNotInFABRICA` modal, not here (avoids a confusing duplicate button). */}
      ...
      {/* repo-not-in-FABRICA prompt: see design doc Interaction States. */}
      <Dialog
        open={resolvedMissingRepoDialogs.repoNotInFABRICA !== null}
        onOpenChange={(open) => !open && setRepoNotInFABRICA(null)}
      >
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>
              {translate(
                ...
                'Repository not in Fabrica'
              )}
            </DialogTitle>
            <DialogDescription>
              {resolvedMissingRepoDialogs.repoNotInFABRICA
                ? translate(
                    'auto.components.github.project.ProjectViewWrapper.1850fceac8',
                    "{{value0}}/{{value1}} isn't added to Fabrica. Add it to start work, or open in GitHub.",
                    {
                      value0: resolvedMissingRepoDialogs.repoNotInFABRICA.owner,
                      value1: resolvedMissingRepoDialogs.repoNotInFABRICA.repo
                    }
                  )
                : null}
            </DialogDescription>
          ...
          <DialogFooter className="gap-2 sm:justify-end">
            <Button variant="ghost" onClick={() => setRepoNotInFABRICA(null)}>
              {translate('auto.components.github.project.ProjectViewWrapper.dffa899f36', 'Cancel')}
            </Button>
            {resolvedMissingRepoDialogs.repoNotInFABRICA?.url ? (
              <Button
                variant="outline"
                onClick={() => {
                  if (resolvedMissingRepoDialogs.repoNotInFABRICA?.url) {
                    void window.api.shell.openUrl(resolvedMissingRepoDialogs.repoNotInFABRICA.url)
                  }
                  setRepoNotInFABRICA(null)
                }}
              >
                {translate(
  ...
  return (
    <div className="project-view-tab-strip flex min-h-[41px] min-w-0 flex-none items-end gap-1 overflow-x-auto overflow-y-hidden border-b border-border/50 bg-muted/20 px-3 pt-3">
      {views.map((v) => {
        ...
        const unsupportedMessage = `Fabrica doesn't support ${layoutLabel} project views yet.`
        return (
              ...
              <div className="space-y-2">
                <p className="text-xs leading-5 text-muted-foreground">
                  ...
                  {translate(
                    ...
                    'Switch to a Table view to work with this project in Fabrica.'
                  )}
                </p>
                <Button
                  type="button"
                  size="xs"
                  variant="outline"
                  onClick={() => void window.api.shell.openUrl(FABRICA_FEATURE_REQUEST_URL)}
                >
                  {translate(
