...
const repos = [repo('r1', 'Fabrica'), repo('r2', 'Fabrica (builder)', 'ssh-1'), repo('r3', 'Solo')]
const projects = [project('p1', 'Fabrica')]
...
describe('buildPaletteFilterModel', () => {
  it('collapses the repos of one project into a single row', () => {
    ...
    expect(model.projects.map((option) => [option.id, option.label, option.count])).toEqual([
      ['project:p1', 'Fabrica', 2],
      ['repo:r3', 'Solo', 1]
    ])
    expect(model.projects[0]?.searchText).toBe('FABRICA')
  })
  ...
  it('sorts project rows by workspace count then label', () => {
    ...
    // FABRICA has 2 workspaces, Solo has 1 — popularity beats alpha.
    expect(model.projects.map((option) => option.label)).toEqual(['Fabrica', 'Solo'])
  })
