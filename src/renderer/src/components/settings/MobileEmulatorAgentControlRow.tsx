...
export function MobileEmulatorAgentControlRow(): React.JSX.Element {
  ...
  const cliSkillInstallCommand = buildSkillCommandForRuntime(FABRICA_CLI_SKILL_INSTALL_COMMAND)
  const cliSkillUpdateCommand = buildSkillCommandForRuntime(FABRICA_CLI_SKILL_UPDATE_COMMAND)
        ...
        <div className="space-y-0.5">
          ...
          <p className="text-xs text-muted-foreground">
            {translate(
              ...
              'Let coding agents control the active mobile emulator with Fabrica CLI commands.'
            )}
          </p>
        </div>
        <span
          ...
          <div className="min-w-0 flex-1 space-y-1">
            ...
            <p className="text-xs text-muted-foreground">
              {translate(
                ...
                'Registers the Fabrica CLI command so agents can control the active emulator from their shell.'
              )}
            </p>
        ...
        <div className={cn('py-3', setup.step2Blocked && 'opacity-60')}>
          <AgentSkillSetupPanel
            ...
            description={translate(
              ...
              'Enables agents to use Fabrica CLI commands, including mobile emulator control.'
            )}
            command={cliSkillInstallCommand}
            installedCommand={cliSkillUpdateCommand}
            terminalTitle="Fabrica CLI skill setup"
            terminalAriaLabel="Fabrica CLI skill install terminal"
            terminalWorktreeId="settings-mobile-emulator-FABRICA-cli-skill-terminal"
            ...
            openingHint={translate(
              ...
              'Checking Fabrica CLI before opening skill setup.'
            )}
            onBeforeOpenTerminal={async () => {
              await ensureFABRICACliAvailableForAgentSkillTerminal()
            }}
            onRecheck={setup.refreshCliSkill}
            freshnessSkillName={
              activeSkillRuntime.canUseLocalSkillFreshness ? FABRICA_CLI_SKILL_NAME : undefined
            }
          />
        </div>
