...
const updateCapableCallers = new Map<string, readonly string[]>([
  ...
  [
    ...
    ['FABRICA_CLI_SKILL_UPDATE_COMMAND', 'installedCommand={cliSkillUpdateCommand}']
  ],
  [
    'src/renderer/src/components/settings/BrowserUsePane.tsx',
    ['FABRICA_CLI_SKILL_UPDATE_COMMAND', 'installedCommand={browserUseUpdateCommand}']
  ],
  [
  ...
  [
    ...
    ['FABRICA_CLI_SKILL_UPDATE_COMMAND', 'installedCommand={updateCommand}']
  ],
  [
  ...
  [
    ...
    [
      ...
      'buildSkillCommandForRuntime(FABRICA_CLI_SKILL_INSTALL_COMMAND)',
      'buildSkillCommandForRuntime(FABRICA_CLI_SKILL_UPDATE_COMMAND)'
    ]
  ]
])
...
describe('AgentSkillSetupPanel installed-command call sites', () => {
  ...
  it('routes the combined feature-tip install through runtime command setup', () => {
    ...
    expect(source).not.toContain('command={FABRICA_CLI_ORCHESTRATION_SKILL_INSTALL_COMMAND}')
