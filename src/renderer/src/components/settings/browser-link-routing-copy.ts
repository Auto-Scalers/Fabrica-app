...
export function getBrowserLinkRoutingDescription(
  ...
  if (modifierInverts) {
    return translate(
      ...
      "Open http(s) links in Fabrica's built-in browser — from the terminal, markdown, and the editor."
    )
  }
  return translate(
    'auto.components.settings.BrowserLinkRoutingSetting.description',
    "Open http(s) links in Fabrica's built-in browser — from the terminal, markdown, and the editor. {{shortcut}} always uses your system browser.",
...
export function getLinkRoutingModifierTitle(openLinksInApp: boolean): string {
  return openLinksInApp
    ...
    : translate(
        'auto.components.settings.BrowserLinkRoutingModifierSetting.titleFABRICA',
        'Hold Shift to open in Fabrica'
      )
}

// Why: the FABRICA branch is enabled-state copy — with the toggle off the chord
// still lands on the system browser, so it must not promise FABRICA in present tense.
export function getLinkRoutingModifierDescription({
  ...
  return openLinksInApp
    ? translate(
        ...
        'Links open in Fabrica, so {{chord}}+click sends one to your system browser instead.',
        { chord }
      )
    : translate(
        'auto.components.settings.BrowserLinkRoutingModifierSetting.descriptionFABRICA',
        "Links open in your system browser. When enabled, {{chord}}+click opens one in Fabrica's built-in browser instead.",
        { chord }
      )
}
