...
describe('CliSection project runtime defaults', () => {
  it('exposes freshness only for a resolved local host runtime', () => {
    ...
    expect(capturedPanel.props?.freshnessSkillName).toBe('FABRICA-cli')
  ...
  it('renders an inline unknown PATH state without offering a mutation', async () => {
    const getInstallStatus = vi.fn().mockResolvedValue({
      platform: 'win32',
      commandName: 'FABRICA',
      commandPath: 'C:\\Program Files\\Fabrica\\resources\\bin\\FABRICA.exe',
      pathDirectory: 'C:\\Program Files\\Fabrica\\resources\\bin',
      pathConfigured: null,
      launcherPath: 'C:\\Program Files\\Fabrica\\resources\\bin\\FABRICA.exe',
      installMethod: 'wrapper',
      supported: true,
      state: 'installed',
      currentTarget: 'C:\\Program Files\\Fabrica\\resources\\bin\\FABRICA.exe',
