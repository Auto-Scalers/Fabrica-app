...
import {
  FABRICA_LINEAR_SKILL_INSTALL_COMMAND,
...
const UPDATE_COMMAND = 'npx skills update fabrica-linear --global'

const mocks = vi.hoisted(() => ({
  panelProps: [] as Record<string, unknown>[],
  runtime: 'native' as 'native' | 'wsl',
  skillInstalled: true,
  updateSkillName: 'fabrica-linear',
...
describe('LinearAgentSkillPane', () => {
  afterEach(async () => {
    ...
    mocks.updateSkillName = 'fabrica-linear'
  ...
  it('renders checklist with inlined skill panel and usage examples', () => {
    ...
    for (const example of examples) {
      ...
      expect(example.prompt).toContain('/fabrica-linear')
  ...
  it('passes the fabrica-linear install/update commands and freshness on a local runtime', async () => {
    await renderPane()

    expect(mocks.panelProps.at(-1)).toEqual(
      expect.objectContaining({
        command: FABRICA_LINEAR_SKILL_INSTALL_COMMAND,
        installedCommand: UPDATE_COMMAND,
        freshnessSkillName: FABRICA_LINEAR_SKILL_NAME
      })
    )
  })
