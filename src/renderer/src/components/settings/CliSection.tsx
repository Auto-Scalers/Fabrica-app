...
function getInstallDescription(platform: string): string {
  if (platform === 'darwin') {
    return 'Register `fabrica` in /usr/local/bin.'
  }
  if (platform === 'linux') {
    return 'Register `fabrica` in ~/.local/bin.'
  }
  if (platform === 'win32') {
    return 'Register `fabrica` in your user PATH.'
  }
  return 'CLI registration is not yet available on this platform.'
}

function getFallbackCommandName(platform: string): string {
  return platform === 'linux' ? 'fabrica' : 'fabrica'
}
...
export function CliSection({
  ...
  } = useInstalledAgentSkill(FABRICA_CLI_SKILL_NAME, {
    ...
    <section className="space-y-4" data-settings-section="cli">
      <div className="space-y-1">
        <h2 className="text-sm font-semibold">
          {translate('auto.components.settings.CliSection.c5c0f2641d', 'Fabrica CLI')}
        </h2>
        <p className="text-xs text-muted-foreground">
          {translate(
            'auto.components.settings.CliSection.6930feda9e',
            'Use Fabrica from your terminal to open the app, manage worktrees, and interact with Fabrica terminals.'
          )}
        </p>
      </div>
          ...
          <div className="border-t border-border/60 pt-3">
            <div className="space-y-0.5">
              ...
              <p className="text-xs text-muted-foreground">
                {translate(
                  ...
                  'Give agents Fabrica-aware workspace, terminal, and progress workflows.'
                )}
              </p>
            </div>
            ...
            <AgentSkillSetupPanel
              ...
              description={translate(
                ...
                'Enables agents to use Fabrica workspace, terminal, and progress commands.'
              )}
              ...
              isPrerequisiteAvailable={isFABRICACliAvailableOnPath}
