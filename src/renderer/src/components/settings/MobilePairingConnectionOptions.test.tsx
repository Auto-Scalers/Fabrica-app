...
type MobileRelayStoreState = {
  ...
  connectCurrentFABRICAProfile: () => Promise<null>
...
describe('MobilePairingConnectionOptions', () => {
  ...
  beforeEach(() => {
    ...
    mocks.state = {
      ...
      connectCurrentFABRICAProfile: connect,
      fetchFABRICAProfileAuthStatus: fetchAuthStatus
    }
  })

  afterEach(() => cleanup())

  it('shows Sign in directly under Fabrica Relay, above LAN', async () => {
    const user = userEvent.setup()
    const onChange = vi.fn()
    render(<MobilePairingConnectionOptions value="automatic" onChange={onChange} />)

    const relay = screen.getByRole('radio', { name: /Fabrica Relay/i })
  ...
  it('moves selection with the arrow keys as a radiogroup', async () => {
    ...
    screen.getByRole('radio', { name: /Fabrica Relay/i }).focus()
  ...
  it('selects a path from the compact list', async () => {
    ...
    await user.click(screen.getByRole('radio', { name: /Fabrica Relay/i }))
    expect(onChange).toHaveBeenCalledWith('automatic')
  })
  ...
  it('shows relay status when signed in on Fabrica Relay', async () => {
    mocks.state = {
      FABRICAProfileAuthStatus: {
        activeProfileId: 'profile-1',
        configured: true,
