...
describe('CliSkillRuntimeSetup runtime helpers', () => {
  ...
  it('wraps WSL skill installs as a directly runnable selected-distro command', () => {
    ...
    expect(decodeWslLoginShellScript(command)).toContain(
      'exec "$_FABRICA_wsl_shell" -ilc \'npx skills add orchestration --global\''
    )
  })
  ...
  it('wraps WSL skill updates as a directly runnable selected-distro command', () => {
    ...
    expect(decodeWslLoginShellScript(command)).toContain(
      'exec "$_FABRICA_wsl_shell" -ilc \'npx skills update orchestration --global\''
    )
  })
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      const root = mkdtempSync(join(tmpdir(), 'FABRICA-wsl-skill-command-'))
      ...
      writeFileSync(
        loginShell,
        '#!/bin/sh\nexport PATH="$FABRICA_TEST_NPX_BIN:/usr/bin:/bin"\nexec /bin/sh -c "$2"\n'
      )
      writeFileSync(
        join(npxBin, 'npx'),
        '#!/bin/sh\nread -r input\nprintf \'%s:%s\' "$*" "$input"\n'
  ...
  it('preflights npx before Windows-host skill installs', () => {
    const installCommand = buildAgentFeatureSkillInstallCommand(['FABRICA-cli', 'orchestration'])
  ...
  it('treats missing runtime as a preflighted Windows host fallback for skill updates', () => {
    const installCommand = buildAgentFeatureSkillInstallCommand(['FABRICA-cli'])

    expect(
      buildSkillCommandForRuntime('npx skills update FABRICA-cli --global', undefined, 'win32')
    ).toBe(`${windowsNpxPreflightPrefix}${windowsNpxGuidance}) else (${installCommand})"`)
  })
  ...
  it('keeps the npx preflight in the PowerShell-forced setup terminal', () => {
    ...
    try {
      ...
      // FABRICA forces its own setup terminal to powershell.exe, where cmd.exe works.
