...
const KEEP_LOCAL_MAIN_UP_TO_DATE_DESCRIPTION =
  'When you create a workspace, Fabrica refreshes the remote base and safely fast-forwards your matching local branch, such as main or master. This keeps commands like git diff main...HEAD from comparing against stale history. Fabrica skips the update if that branch has uncommitted changes or local-only commits.'
...
export function GitPane({
  ...
  const visibleSections = [
    ...
    matchesSettingsSearch(searchQuery, {
      title: translate('auto.components.settings.GitPane.e02ea23a32', 'Fabrica Attribution'),
      description: translate(
        'auto.components.settings.GitPane.d2eede4c54',
        'Add Fabrica attribution to commits, PRs, and issues.'
      ),
      keywords: [
        ...
        translate('auto.components.settings.GitPane.e71ce09c42', 'FABRICA')
      ]
    }) ? (
      <SearchableSetting
        ...
        keywords={['github', 'gh', 'pr', 'issue', 'co-author', 'coauthored', 'attribution', 'FABRICA']}
        className="flex items-center justify-between gap-4 py-2"
      >
        <div className="space-y-0.5">
          <Label>
            {translate('auto.components.settings.GitPane.e02ea23a32', 'Fabrica Attribution')}
          </Label>
          <p className="text-xs text-muted-foreground">
            {translate(
              'auto.components.settings.GitPane.d2eede4c54',
        ...
        <Switch
          aria-label={translate('auto.components.settings.GitPane.e02ea23a32', 'Fabrica Attribution')}
