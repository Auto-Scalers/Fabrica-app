...
export function pluginInstallErrorMessage(cause: unknown): string {
  ...
  if (detail.includes('fabrica-plugin.json') && /(missing|unreadable|no )/.test(detail)) {
    return translate(
      'auto.components.settings.pluginError.installManifestMissing',
      "No readable fabrica-plugin.json was found. Choose the plugin's root folder."
    )
  }
  if (detail.includes('invalid manifest')) {
    return translate(
      'auto.components.settings.pluginError.installManifestInvalid',
      'fabrica-plugin.json is invalid. Ask the plugin author to fix the manifest.'
    )
  }
  if (detail.includes('requires FABRICA')) {
    return translate(
      'auto.components.settings.pluginError.incompatible',
      'This plugin requires a different Fabrica version.'
    )
  }
  ...
  if (/(exceeds|too many)/.test(detail)) {
    return translate(
      ...
      "The plugin exceeds Fabrica's install size or file-count limits."
    )
  }
  if (/(git|repository|fetch|clone|checkout|remote)/.test(detail)) {
    return translate(
      'auto.components.settings.pluginError.installGit',
      'Fabrica could not fetch the pinned Git revision. Check the URL, #ref, access, and system Git setup.'
    )
  }
  return translate(
...
export function invalidPluginErrorMessage(detailValue: string): string {
  ...
  if (detail.includes('missing fabrica-plugin.json')) {
    return translate(
      'auto.components.settings.pluginError.invalidManifestMissing',
      'The plugin root is missing fabrica-plugin.json. Add it, then refresh plugins.'
    )
  }
  if (detail.includes('invalid manifest')) {
    return translate(
      'auto.components.settings.pluginError.invalidManifest',
      'fabrica-plugin.json is invalid. Fix it, then refresh plugins.'
    )
  }
