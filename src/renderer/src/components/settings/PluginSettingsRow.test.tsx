...
const plugin: PluginHostListEntry = {
  pluginKey: 'autoscalers.fabrica-skills',
  ...
  source: {
    kind: 'bundled',
    reference: 'bundled:autoscalers.fabrica-skills',
    resolvedCommit: null,
...
describe('PluginSettingsRow', () => {
  it('shows official provenance and prevents re-enabling killed plugins', async () => {
    ...
    expect(
      container.querySelector<HTMLButtonElement>('[aria-label="Enable Fabrica Skills"]')?.disabled
    ).toBe(true)
    act(() => root.unmount())
  })
})
