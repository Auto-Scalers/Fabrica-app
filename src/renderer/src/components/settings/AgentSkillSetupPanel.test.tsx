...
const INSTALL_COMMAND = 'npx skills add https://github.com/Auto-Scalers/Fabrica-app --skill FABRICA-cli --global'
const UPDATE_COMMAND = 'npx skills update FABRICA-cli --global'
...
function panelProps(
  ...
  return {
    title: 'CLI skill',
    description: 'Enables agents to use Fabrica workflows.',
...
describe('AgentSkillSetupPanel', () => {
  ...
  it('surfaces skill freshness under hideHeader for guided setup hubs', () => {
    const html = renderPanel({
      ...
      hideHeader: true,
      description: null,
      freshnessSkillName: 'fabrica-linear'
    })

    expect(html).toContain('data-testid="skill-freshness"')
    expect(html).toContain('fabrica-linear')
  ...
  it('keeps update copy until the installed panel checks CLI prerequisites', () => {
    const html = renderPanel({
      ...
      preInstallNotice: 'Install the Fabrica CLI before running agent skill setup.'
    })
  ...
  it('refreshes first-install freshness only after the terminal re-check finishes', async () => {
    ...
    await renderInteractivePanel({ freshnessSkillName: 'FABRICA-cli', onRecheck })
    await clickButton('Install')
  ...
  it('keeps the command failure authoritative over presence discovery', async () => {
    await renderInteractivePanel({ freshnessSkillName: 'FABRICA-cli' })
    await clickButton('Install')

    await act(async () => {
      mocks.terminalProps.at(-1)?.onCommandFinished?.(1)
    })
    await rerenderInteractivePanel({ installed: true, freshnessSkillName: 'FABRICA-cli' })
