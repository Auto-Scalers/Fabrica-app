...
describe('ArtifactsSettingsPane', () => {
  beforeEach(() => {
    ...
    mocks.state.FABRICAProfileAuthStatus = { configured: true, state: 'connected' }
  ...
  it('explains the complete sharing workflow', () => {
    ...
    expect(screen.getByText('Manage it in Fabrica')).toBeInTheDocument()
    expect(
    ...
    expect(screen.queryByText('Fabrica account')).not.toBeInTheDocument()
    expect(screen.queryByRole('button', { name: 'Sign in to Fabrica' })).not.toBeInTheDocument()
  })
  ...
  it('shows reconnect and connecting states', () => {
    mocks.state.FABRICAProfileAuthStatus = { configured: true, state: 'reconnect-required' }
    ...
    mocks.state.FABRICAProfileConnecting = true
    rerender(
      <ArtifactsSettingsPane settings={getDefaultSettings('/tmp')} updateSettings={vi.fn()} />
    )
  ...
  it('controls only sidebar visibility and always allows opening Artifacts', async () => {
    ...
    expect(screen.queryByText(/FABRICA artifacts share/)).not.toBeInTheDocument()
  ...
  it('points web clients at the desktop app for the opt-in step', () => {
    ...
    expect(
      screen.getByText(/Open Settings → Artifacts in the Fabrica desktop app on the host device/)
    ).toBeInTheDocument()
  })
})
