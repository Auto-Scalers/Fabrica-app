...
// Dev-only preview of the first-party FABRICA Cloud sign-in. The sidebar/titlebar
...
function FABRICACloudDevSubsection(): React.JSX.Element {
  const authStatus = useAppStore((s) => s.FABRICAProfileAuthStatus)
  const connecting = useAppStore((s) => s.FABRICAProfileConnecting)
  const connect = useAppStore((s) => s.connectCurrentFABRICAProfile)
  const signOut = useAppStore((s) => s.signOutCurrentFABRICAProfile)
  const refresh = useAppStore((s) => s.fetchFABRICAProfileAuthStatus)
    ...
    <section className="space-y-3">
      <div className="flex items-start justify-between gap-3">
        <SettingsSubsectionHeader
          title={translate('auto.components.settings.DevToolsPane.FABRICACloud', 'Fabrica Cloud')}
          description={translate(
            'auto.components.settings.DevToolsPane.FABRICACloudDescription',
            'Dev-only preview of first-party cloud sign-in. Hidden in production; in dev it also appears in the sidebar account switcher once FABRICA_CLOUD_API_URL and FABRICA_CLOUD_CLIENT_ID are set.'
          )}
        />
      ...
      {configured ? (
        <div className="space-y-2">
          <p className="text-xs text-muted-foreground">
            {translate('auto.components.settings.DevToolsPane.FABRICACloudStatus', 'Status')}:{' '}
          ...
          <div className="flex flex-wrap gap-2">
            {connected ? (
              ...
              >
                {translate('auto.components.settings.DevToolsPane.FABRICACloudSignOut', 'Sign out')}
              </Button>
            ) : (
              <Button
                type="button"
              ...
              >
                {translate(
                  'auto.components.settings.DevToolsPane.FABRICACloudConnect',
                  'Connect profile'
                )}
              </Button>
            )}
            <Button type="button" variant="ghost" size="sm" onClick={() => void refresh()}>
              {translate(
                'auto.components.settings.DevToolsPane.FABRICACloudRefresh',
                'Refresh status'
              )}
            </Button>
          </div>
        ...
        <p className="text-xs text-muted-foreground">
          {authStatus?.setupMessage ??
            translate(
              'auto.components.settings.DevToolsPane.FABRICACloudNotConfigured',
              'Set FABRICA_CLOUD_API_URL and FABRICA_CLOUD_CLIENT_ID to preview Fabrica Cloud sign-in in this dev build.'
            )}
        </p>
      )}
    </section>
