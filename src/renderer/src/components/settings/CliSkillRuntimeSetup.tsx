...
import {
  ...
  showFABRICACliRegistrationPromptToast
 ...
 * or the setup terminal FABRICA spawns itself.
 */
type SkillCommandTarget = 'copied-command' | 'FABRICA-setup-terminal'

/**
 * Adapts a copied skill command for FABRICA's inline setup terminal auto-paste.
...
export async function ensureWslCliAvailableForAgentSkillTerminal(
  ...
  try {
    ...
    if (status.state !== 'installed' || status.pathConfigured === false) {
      await showFABRICACliRegistrationPromptToast()
      const next = await window.api.cli.installWsl(args)
      if (!isFABRICACliAvailableOnPath(next)) {
        toast.warning(
          translate(
