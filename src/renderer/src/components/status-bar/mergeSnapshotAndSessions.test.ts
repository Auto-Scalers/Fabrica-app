...
describe('mergeSnapshotAndSessions', () => {
  ...
  it('includes browser-only workspaces in their repo', () => {
    const worktree = {
      id: 'FABRICA::/Users/me/browser-only',
      repoId: 'FABRICA',
    ...
    const out = mergeSnapshotAndSessions(
      ...
      [],
      baseCtx({
        repoDisplayNameById: new Map([['FABRICA', 'FABRICA']]),
  ...
  it('passes through snapshot worktrees with numeric metrics and hasLocalSamples', () => {
    const wt: WorktreeMemory = {
      worktreeId: 'FABRICA::/Users/me/Triton',
      worktreeName: 'Triton',
      repoId: 'FABRICA',
      repoName: 'FABRICA',
      cpu: 1.5,
  ...
  it('binds a deferred SSH row so its single-row kill cannot skip confirmation', () => {
    ...
    const sessionId = 'FABRICA::/remote/Stingray@@deferred1'
    const out = mergeSnapshotAndSessions(
      null,
      [{ id: sessionId, cwd: '', title: 'FABRICA/Stingray', agentOwnership: 'absent' as const }],
      baseCtx({
        tabsByWorktree: { 'FABRICA::/remote/Stingray': [makeTab('tab-ssh')] },
        deferredSshSessionIdsByTabId: { 'tab-ssh': sessionId },
        repoConnectionIdById: new Map([['FABRICA', 'ssh-conn-1']])
      })
    )
  ...
  it('@@ parse: an SSH-style session id resolves to its worktree group', () => {
    const ds: DaemonSession[] = [
      {
        id: 'FABRICA::/remote/Stingray@@abcd1234',
        cwd: '',
        title: 'FABRICA/Stingray',
    ...
    expect(out[0].worktrees[0]).toMatchObject({
      worktreeId: 'FABRICA::/remote/Stingray',
    ...
    expect(out[0].worktrees[0].sessions[0]).toMatchObject({
      sessionId: 'FABRICA::/remote/Stingray@@abcd1234',
      hasLocalSamples: false,
      cpu: null,
      memory: null,
      bound: false
  ...
  it('warm-reattach local PTY: chip stays off when repo has no connectionId', () => {
    ...
    const ds: DaemonSession[] = [
      {
        id: 'FABRICA::/local/Triton@@deadbeef',
        cwd: '/local/Triton',
        title: 'FABRICA/Triton',
        agentOwnership: 'absent' as const
      }
    ]
    const ctx = baseCtx({
      repoConnectionIdById: new Map([['FABRICA', null]])
    })
    const out = mergeSnapshotAndSessions(null, ds, ctx)
    expect(out[0]).toMatchObject({
      repoId: 'FABRICA',
  ...
  it('tab walk wins over @@ parse when they disagree', () => {
    ...
    const ds: DaemonSession[] = [
      {
        id: 'FABRICA::/wrong/path@@feedface',
        cwd: '',
        title: 'FABRICA',
        agentOwnership: 'absent' as const
      }
    ...
    const ctx = baseCtx({
      tabsByWorktree: {
        'FABRICA::/correct/path': [makeTab(tabId, 'My Tab')]
      },
      ptyIdsByTabId: { [tabId]: ['FABRICA::/wrong/path@@feedface'] }
    })
    const out = mergeSnapshotAndSessions(null, ds, ctx)
    expect(out[0].worktrees[0].worktreeId).toBe('FABRICA::/correct/path')
  ...
  it('treats startup deferred reattach tab ptyId wake hints as bound sessions', () => {
    ...
    const sessionId = 'FABRICA::/Users/me/Triton@@deferred'
    const ds: DaemonSession[] = [
      {
        id: sessionId,
        cwd: '/Users/me/Triton',
    ...
    const ctx = baseCtx({
      tabsByWorktree: {
        'FABRICA::/Users/me/Triton': [restoredTab]
      },
      ptyIdsByTabId: { [tabId]: [] }
    })
  ...
  it('local-bound interaction state: numeric metrics + bound=true + tabId set', () => {
    ...
    const ctx = baseCtx({
      tabsByWorktree: { 'FABRICA::/Users/me/Triton': [makeTab(tabId)] },
