...
if (typeof window !== 'undefined') {
  const guardedWindow = window as Window & {
    __FABRICAContextualTourGlobalKeyGuardInstalled?: boolean
  }
  if (!guardedWindow.__FABRICAContextualTourGlobalKeyGuardInstalled) {
    guardedWindow.__FABRICAContextualTourGlobalKeyGuardInstalled = true
    window.addEventListener('keydown', handleContextualTourGlobalKeyDown, true)
  }
}

const PANEL_BASE_CLASSES =
  'FABRICA-contextual-tour-panel rounded-lg border border-border text-popover-foreground backdrop-blur-[2px]'
  ...
  return (
    ...
    >
      {showTargetRings ? (
        <div
          aria-hidden="true"
          className="FABRICA-contextual-tour-target-rings fixed z-[75]"
