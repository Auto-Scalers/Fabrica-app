...
export const EMULATOR_LOCAL_SHUTDOWN_EVENT = 'FABRICA:emulator-shutdown'
...
export function useEmulatorPaneSessionEvents({
  ...
  useEffect(() => {
    ...
    window.addEventListener('FABRICA:emulator-auto-attach', onAuto)
    return () => window.removeEventListener('FABRICA:emulator-auto-attach', onAuto)
