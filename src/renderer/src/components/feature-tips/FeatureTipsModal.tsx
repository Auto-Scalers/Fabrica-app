  ...
  const handlePrimaryAction = async (): Promise<void> => {
    ...
    switch (currentTip.action) {
      case 'learn-cmd-j-palette': {
        ...
        trackCmdJPaletteFeatureTipAcknowledged(
          getFABRICACliFeatureTipTelemetrySource(modalData.source)
        )
        closeModal()
        break
      }
      ...
      case 'setup-cli': {
        ...
        const canApplySetupResult = (): boolean =>
          ...
          setupRequestIdRef.current === setupRequestId
        const telemetrySource = getFABRICACliFeatureTipTelemetrySource(modalData.source)
        trackFABRICACliFeatureTipSetupClicked(telemetrySource)
        setPrimaryBusy(true)
        try {
          const result = await installCliFromFeatureTip(() => window.api.cli.install())
          if (result.kind === 'installed') {
            trackFABRICACliFeatureTipSetupResult(telemetrySource, 'installed')
          ...
          trackFABRICACliFeatureTipSetupResult(telemetrySource, 'needs_attention')
          if (!canApplySetupResult()) {
            return
          }
          toast.warning(
          ...
          const message = error instanceof Error ? error.message : 'Failed to install Fabrica CLI.'
          if (
            import.meta.env.DEV &&
            message.includes('Development mode uses a generated launcher for validation only')
          ) {
            trackFABRICACliFeatureTipSetupResult(telemetrySource, 'dev_preview')
            if (!canApplySetupResult()) {
              return
            }
            enableOrchestrationSkillSetup()
          ...
          trackFABRICACliFeatureTipSetupResult(telemetrySource, 'failed')
          if (canApplySetupResult()) {
            toast.error(message)
          }
        } finally {
