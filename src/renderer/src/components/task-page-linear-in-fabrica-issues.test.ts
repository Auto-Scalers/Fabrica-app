...
} from './task-page-linear-in-fabrica-issues'
...
describe('filterLinearIssuesForInFABRICAWorkspace', () => {
  it('keeps issues without workspace metadata when a workspace is selected', () => {
    ...
    expect(
      filterLinearIssuesForInFABRICAWorkspace(issues, 'ws-a').map((item) => item.identifier)
    ).toEqual(['ENG-1', 'ENG-3'])
  })
})
