...
describe('groupSkillFreshness', () => {
  it('marks an eligible outdated skill as update-available with one location', () => {
    const groups = groupSkillFreshness([placement('FABRICA-cli')], ['FABRICA-cli'])
    expect(groups).toHaveLength(1)
    expect(groups[0]).toMatchObject({ name: 'FABRICA-cli', status: 'update-available' })
    expect(groups[0]?.locations).toEqual([
      {
        id: expect.any(String),
        path: '/home/.agents/skills/FABRICA-cli',
        chip: null,
  ...
  it('hides skills whose every copy is current', () => {
    const groups = groupSkillFreshness(
      [
        placement('FABRICA-cli', { status: 'current' }),
        placement('FABRICA-cli', { status: 'current', topology: 'provider-alias' })
      ],
      []
    )
  ...
  it('raises no group when every finding is project-owned', () => {
    // FABRICA's updater only passes --global, so a project copy has no remedy; a row here
    // would claim FABRICA considered an update it could never perform.
    expect(
      groupSkillFreshness(
