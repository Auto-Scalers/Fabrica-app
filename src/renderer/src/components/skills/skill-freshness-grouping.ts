 ...
 * wrong in a way the update cannot fix — an edited copy, one FABRICA could not read.
 *
...
export function groupSkillFreshness(
  ...
  for (const [name, entries] of byName) {
    ...
    // row would assert FABRICA considered an update it never could. Locations below are still
