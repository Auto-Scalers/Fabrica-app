...
function eligibleInventory(): SkillFreshnessInventory {
  return {
    schemaVersion: 1,
    installations: [placement('FABRICA-cli')],
    eligibleUpdateNames: ['FABRICA-cli'],
    scanIssues: [],
    scannedAt: 1
  }
}
...
describe('SkillFreshnessUpdateDialog', () => {
  ...
  it('starts a background run with the eligible names', async () => {
    ...
    expect(skillsApi.startUpdateRun).toHaveBeenCalledWith(['FABRICA-cli'])
  })
  ...
  it('keeps the same row elements across the whole run instead of swapping layouts', async () => {
    await renderDialog()
    await openViaRequest()
    const before = container?.querySelector('[data-skill-row="FABRICA-cli"]')
    expect(before?.getAttribute('data-state-label')).toBe('available')

    await emitRun({ state: 'running', names: ['FABRICA-cli'], startedAt: 1, output: '' })
    const during = container?.querySelector('[data-skill-row="FABRICA-cli"]')
    ...
    mocks.inventory = {
      schemaVersion: 1,
      installations: [placement('FABRICA-cli', { status: 'current', installedReleaseRevision: 2 })],
      eligibleUpdateNames: [],
      scanIssues: [],
      scannedAt: 5
    }
    await emitRun({ state: 'success', names: ['FABRICA-cli'], finishedAt: 2, output: 'done' })
    await rerender()
    const after = container?.querySelector('[data-skill-row="FABRICA-cli"]')
  ...
  it('shows the captured log verbatim without parsing it', async () => {
    ...
    await emitRun({
      ...
      names: ['FABRICA-cli'],
      finishedAt: 2,
      output: 'Checking skills from source: Auto-Scalers/Fabrica\n  ? Updated FABRICA-cli'
    })

    expect(container?.querySelector('pre')?.textContent).toContain(
      'Checking skills from source: Auto-Scalers/Fabrica-app'
    )
  })
  ...
  it('raises no row for a skill whose only finding is a project-owned copy', async () => {
    ...
    // directory. FABRICA only ever updates global skills, so a "Skipped" row here asserts
  ...
  it('keeps the stale-record remedy when a project copy is listed beside it', async () => {
    ...
    // runnable command with advice about a copy the user never asked FABRICA to update.
    mocks.inventory = {
      schemaVersion: 1,
      installations: [
  ...
  it('surfaces a scan error instead of a stale summary', async () => {
    ...
    expect(container?.querySelector('[data-skill-row="FABRICA-cli"]')).toBeNull()
    expect(findButton('Update 1 skill')).toBeUndefined()
  })
  ...
  it('shows incomplete plugin coverage without presenting a fabricated skill copy', async () => {
    mocks.inventory = {
      schemaVersion: 1,
      installations: [
        placement('FABRICA-cli', { status: 'current', observedPackageDigest: 'current' })
      ],
      eligibleUpdateNames: [],
      scanIssues: [
        {
    ...
    expect(container?.textContent).toContain(
      'Fabrica could not finish checking plugin-managed skills.'
    )
