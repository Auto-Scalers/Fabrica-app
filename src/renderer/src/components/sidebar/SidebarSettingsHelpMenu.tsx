...
const FABRICA_WEBSITE_URL = 'https://fabrica-ai.vercel.app/'
const DOCS_URL = 'https://fabrica-ai.vercel.app/docs'
const CHANGELOG_URL = 'https://fabrica-ai.vercel.app/changelog'
const GITHUB_URL = 'https://github.com/Auto-Scalers/Fabrica-app'
const DISCORD_URL = 'https://discord.gg/fzjDKHxv8Q'
const X_URL = 'https://x.com/FABRICA_build'
...
export function SidebarSettingsHelpMenu(): React.JSX.Element {
  ...
  const [isRestartingFABRICA, setIsRestartingFABRICA] = useState(false)
  ...
  const handleRestartFABRICA = (): void => {
    if (isRestartingFABRICA) {
      return
    }
    setIsRestartingFABRICA(true)
    toast.info(
      translate('auto.components.sidebar.SidebarSettingsHelpMenu.5161eef55d', 'Restarting Fabrica�')
    )
    void window.api.app.restart().catch((error) => {
      if (mountedRef.current) {
        setIsRestartingFABRICA(false)
        toast.error(
          translate(
            'auto.components.sidebar.SidebarSettingsHelpMenu.4e8f5710d3',
            "Couldn't restart Fabrica."
          ),
          {
      ...
      <div className="flex items-center gap-1">
        ...
        <DropdownMenu modal={false} open={menuOpen} onOpenChange={handleMenuOpenChange}>
          ...
          <DropdownMenuContent side="top" align="start" sideOffset={8} className="w-52">
            ...
            <ExternalMenuItem
              label={translate(
                'auto.components.sidebar.SidebarSettingsHelpMenu.website',
                'Website'
              )}
              url={FABRICA_WEBSITE_URL}
            ...
            <DropdownMenuSeparator />
            <DropdownMenuItem onSelect={handleRestartFABRICA} disabled={isRestartingFABRICA}>
