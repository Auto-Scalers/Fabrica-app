...
describe('getRenderedSetupScriptPromptState', () => {
  ...
  it('does not reuse a matching repo id from a different host', () => {
    expect(
      getRenderedSetupScriptPromptState({
        promptState: prompt('repo-FABRICA', 'local'),
        activeRepoId: 'repo-FABRICA',
        activeRepoHostIdentity: repoIdentity('repo-FABRICA', 'runtime:windows'),
...
describe('markSetupScriptPromptSaved', () => {
  it('does not apply a completed save to the same repo id on another host', () => {
    const remote = prompt('repo-FABRICA', 'runtime:windows')

    expect(markSetupScriptPromptSaved(remote, repoIdentity('repo-FABRICA', 'local'))).toBe(remote)
  })

  it('marks the prompt for the saved host effective', () => {
    expect(
      markSetupScriptPromptSaved(
        prompt('repo-FABRICA', 'runtime:windows'),
        repoIdentity('repo-FABRICA', 'runtime:windows')
      )
