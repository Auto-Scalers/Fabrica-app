    ...
    if (
      ...
      toast.error(
        translate(
          ...
          'That SSH host is already in Fabrica.'
        )
      )
      return 'validation-failed'
    }
  ...
  if (typeof ssh.resolveConfigHost !== 'function') {
    throw new Error(
      translate(
        ...
        'Restart Fabrica to finish applying the SSH config picker update.'
      )
    )
  }
...
/** Bulk-load ~/.ssh/config hosts into FABRICA’s host list (sidebar targets). */
export async function addAllSshConfigHostsToFABRICA({
  ssh,
  recordSshRepoReadoptions,
  setSshTargetsMetadata,
  recordFeatureInteraction
  ...
  try {
    ...
    toast.success(
      translate(
        ...
        'Added {{value0}} host{{value1}} to Fabrica.',
    ...
    return {
      ...
      newHostCount: value.filter(
        (host): host is SshConfigHostSummary =>
          typeof host === 'object' && host !== null && host.alreadyInFABRICA === false
      ).length,
