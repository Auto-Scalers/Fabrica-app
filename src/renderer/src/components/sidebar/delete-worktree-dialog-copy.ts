  ...
  return {
    ...
    descriptionSuffix: args.isBatchDelete
      ? allFolderWorkspaceDeletes
        ? 'from Fabrica. Project folders on disk will not be deleted.'
        : mixedFolderWorkspaceDeletes
          ? 'from Fabrica. Git worktrees will also be removed from git and disk; folder workspaces will only remove the Fabrica workspace entry.'
          : 'from git and delete their workspace folders.'
      : args.isFolderWorkspaceDelete
        ? 'from Fabrica. The project folder on disk will not be deleted.'
