      ...
      <DialogHeader className="text-left">
        ...
        <DialogDescription>
          {translate(
            ...
            'Pick a host to fill the form, or add every new host to Fabrica’s host list.'
          )}
        </DialogDescription>
      </DialogHeader>
          ...
          >
            {hosts.map((host) => (
              <li key={host.alias}>
                <button
                  type="button"
                  disabled={picksDisabled || host.alreadyInFABRICA}
                  className={cn(
                ...
                >
                  ...
                  ) : host.alreadyInFABRICA ? (
                    <Badge
                      variant="outline"
                      className="mt-0.5 shrink-0 border-emerald-500/40 text-[10.5px] text-emerald-400"
                    >
                      {translate(
                        'auto.components.sidebar.AddRemoteHostDialog.sshConfigPickerInFABRICA',
                        'In Fabrica'
                      )}
                    </Badge>
          ...
          {isBulkImporting
            ...
            : newHostCount > 0
              ? translate(
                  ...
                  'Add all {{value0}} to Fabrica',
                  { value0: newHostCount }
                )
              : // Why: the remainder can be already-in-Fabrica or merely tombstoned, so the
