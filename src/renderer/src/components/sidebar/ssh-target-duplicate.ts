...
/** True when an existing FABRICA host already owns this config alias / label. */
export function isDuplicateSshTargetAlias({
  ...
  // Why: the config picker's `alreadyInFABRICA` flag compares lowercased aliases; match it or the
 ...
 *  otherwise an alias it greys out as "In FABRICA" is still savable as a second target. */
