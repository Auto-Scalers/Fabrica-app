...
describe('shouldDeferRemoteFileBrowserPasteResolve', () => {
  it('keeps small path and filter pastes on the immediate resolver path', () => {
    expect(shouldDeferRemoteFileBrowserPasteResolve('/workspaces/FABRICA/src')).toBe(false)
...
describe('parsePathInput', () => {
  ...
  it('`Documents/FABRICA` commits `Documents` and filters by `FABRICA`', () => {
    expect(parsePathInput('Documents/FABRICA')).toEqual({
      mode: 'path',
      base: 'cwd',
      committedSegments: ['Documents'],
      trailingFilter: 'FABRICA'
    })
  })
...
describe('resolveSegmentStep', () => {
  const listing: DirEntry[] = [
    ...
    { name: 'FABRICA-internal', isDirectory: true },
  ...
  it('unique prefix descends', () => {
    expect(resolveSegmentStep('FABRICA', '/home/neil', listing)).toEqual({
      type: 'descend',
      name: 'FABRICA-internal'
    })
  })
