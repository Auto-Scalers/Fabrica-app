...
/**
 * Re-runs the setup-script prompt inspection when a shared `FABRICA.yaml` setup hook
...
export function useSetupScriptPromptRevalidation(input: {
  ...
  // Why: FABRICA.yaml is edited on disk or the hook runs in a terminal outside React
  // state. Re-inspect on window focus so returning to FABRICA detects it (mirrors
  ...
  useEffect(() => {
    ...
    // Why: a runtime reconnect can turn an unreadable FABRICA.yaml into a readable one,
