...
describe('NewWorkspaceComposerCard folder task source mode', () => {
  ...
  it('opens the add-host submenu on hover without a click', () => {
    ...
    expect(findRunTargetItem('Add Remote Fabrica Server')).toBeTruthy()
  })

  it('opens the remote Fabrica server add dialog over the composer without leaving for Settings', () => {
    current = renderCard({
      projectHostSetupOptions: [localReadyHostOption, devboxNeedsSetupHostOption],
      selectedProjectHostSetupId: 'setup-local'
    })
    ...
    openRunTargetPicker(current.container)
    act(() => findRunTargetItem('Add host')?.click())
    act(() => findRunTargetItem('Add Remote Fabrica Server')?.click())
      ...
      ephemeralVmRecipes: [
        {
          id: 'vercel',
          name: 'Vercel Sandbox',
          create: './scripts/FABRICA-vm/vercel.start.sh',
          destroy: './scripts/FABRICA-vm/vercel.cleanup.sh',
