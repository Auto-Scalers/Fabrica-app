...
function getAutomationSetupSource(
  ...
  yamlHooks: FABRICAHooks | null | undefined
...
export function getVisibleAutomationSetupDecision(args: {
  ...
  if (args.createTarget !== 'FABRICA' || args.workspaceMode !== 'new_per_run') {
    return undefined
  }
...
export function resolveAutomationSetupDecisionForSave(args: {
  ...
  if (args.yamlHooks === undefined) {
    // Why: automations cannot pause later for an FABRICA.yaml trust prompt; when
