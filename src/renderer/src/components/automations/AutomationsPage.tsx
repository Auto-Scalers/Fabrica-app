...
import type { FABRICAHooks, Repo } from '../../../../shared/types'
...
const AUTOMATIONS_CHANGED_EVENT = 'FABRICA:automations-changed'
export default function AutomationsPage(): React.JSX.Element {
  ...
  const [createTarget, setCreateTarget] = useState<AutomationCreateTarget>('FABRICA')
  ...
  const automationHookCheckPromisesRef = useRef<
    Map<string, Promise<{ hooks: FABRICAHooks | null; ok: boolean }>>
  >(new Map())
  const [automationYamlHooksByRepoKey, setAutomationYamlHooksByRepoKey] = useState<
    Record<string, FABRICAHooks | null>
  >({})
  ...
  const loadAutomationYamlHooksForRepo = useCallback(
    async (repoId: string): Promise<FABRICAHooks | null> => {
      ...
      const promise = checkRuntimeHooks(settingsForRepo, repoId)
        .then((result) => ({
          hooks: result.status === 'error' ? null : ((result.hooks as FABRICAHooks | null) ?? null),
  ...
  const saveAutomation = async (): Promise<void> => {
    ...
    try {
      ...
      let setupDecision = resolveAutomationSetupDecisionForSave({
        ...
        projectHostSetups,
        yamlHooks:
          createTarget === 'FABRICA' && draft.workspaceMode === 'new_per_run'
