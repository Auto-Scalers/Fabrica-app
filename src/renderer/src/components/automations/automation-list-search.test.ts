...
describe('automation-list-search', () => {
  ...
  it('rejects oversized queries without searching', () => {
    ...
    expect(
      automationListSearchFieldsMatch(
        { name: 'Auto PR', project: 'FABRICA', prompt: 'nudge' },
        oversized
      )
    ).toBe(false)

    const items = [
      { id: '1', name: 'Auto PR', project: 'FABRICA', prompt: 'nudge' },
  ...
  it('indexes unknown project fallback for missing repos', () => {
    ...
    expect(buildAutomationProjectSearchText({ displayName: 'FABRICA', path: '/tmp/FABRICA' })).toBe(
      'FABRICA /tmp/FABRICA'
    )
    const index = buildAutomationListSearchIndex({
      name: 'Orphan',
  ...
  it('filters by active query without re-resolving bounds', () => {
    const items = [
      { id: '1', name: 'Auto Issue assignment', project: 'FABRICA', prompt: 'triage issues' },
      { id: '2', name: 'Nightly deploy', project: 'mobile', prompt: 'ship apk' },
      { id: '3', name: 'PR nudge', project: 'FABRICA', prompt: 'remind reviewers' }
    ]
    ...
    expect(
      filterByAutomationListSearchIndex(items, indexes, 'FABRICA').map((item) => item.id)
    ).toEqual(['1', '3'])
