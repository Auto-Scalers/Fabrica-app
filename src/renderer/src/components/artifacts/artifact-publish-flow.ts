    ...
    for (let attempt = 0; attempt < 2; attempt += 1) {
      ...
      toast.error(
        translate(
          ...
          'Sign in to Fabrica and try again.'
        )
      )
      return null
    }
...
async function ensureArtifactAccountConnected(): Promise<boolean> {
  ...
  if (state.FABRICAProfileAuthStatus?.state === 'connected') {
    return true
  }
  return (await state.connectCurrentFABRICAProfile())?.status === 'connected'
}

async function reconnectArtifactAccount(): Promise<boolean> {
  return (await useAppStore.getState().connectCurrentFABRICAProfile())?.status === 'connected'
}
  ...
  switch (code) {
    ...
    case 'too-large':
      return translate(
        ...
        'Artifacts shared from Fabrica must be smaller than 800 KB.'
      )
    case 'unreadable':
      return translate(
        'auto.components.artifacts.artifact-publish-flow.e2ed5acd8c',
        "Fabrica couldn't read this file. Open it from a workspace and try again."
      )
    case 'unsupported':
      return translate(
