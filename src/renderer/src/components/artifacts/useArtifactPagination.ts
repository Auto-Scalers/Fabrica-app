...
export function artifactAccountIdentity(authStatus: FABRICAProfileAuthStatus | null): string | null {
...
function artifactRequestIsCurrent(
  ...
  return (
    ...
    artifactAccountIdentity(useAppStore.getState().FABRICAProfileAuthStatus) === identity
  )
}

export function useArtifactPagination(
  authStatus: FABRICAProfileAuthStatus | null,
      ...
      if (result.status === 'ok') {
        ...
        setError(
          translate(
            ...
            'Sign in to Fabrica again to load artifacts.'
          )
        )
      }
    } catch (loadError) {
