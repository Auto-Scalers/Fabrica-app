...
const mocks = vi.hoisted(() => ({
  ...
  toastSuccess: vi.fn(),
  state: {
    FABRICAProfileAuthStatus: { state: 'connected' } as { state: string } | null,
    connectCurrentFABRICAProfile: vi.fn()
  }
}))
...
beforeEach(() => {
  vi.clearAllMocks()
  mocks.state.FABRICAProfileAuthStatus = { state: 'connected' }
  mocks.state.connectCurrentFABRICAProfile = mocks.connect
})

describe('artifact publish flow', () => {
  it('signs in before preparing and publishing the request', async () => {
    mocks.state.FABRICAProfileAuthStatus = { state: 'local' }
  ...
  it('rejects an oversized request before RPC', async () => {
    ...
    expect(mocks.toastError).toHaveBeenCalledWith('Could not share artifact', {
      description: 'Artifacts shared from Fabrica must be smaller than 800 KB.'
    })
  })
