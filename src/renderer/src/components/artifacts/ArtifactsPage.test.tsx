...
function storeState(): Record<string, unknown> {
  return {
    ...
    FABRICAProfileAuthStatus: mocks.authStatus,
    FABRICAProfileConnecting: false,
    refreshCurrentFABRICAProfileAuth: mocks.refreshAuth,
...
describe('ArtifactsPage', () => {
  beforeEach(() => {
    ...
    mocks.resolvePartition.mockReset().mockResolvedValue('persist:FABRICA-default')
  ...
  it('does not surface an initial auth error after the account changes during refresh', async () => {
    ...
    await waitFor(() =>
      expect(screen.queryByText('Sign in to Fabrica again to load artifacts.')).not.toBeInTheDocument()
    )
  })
  ...
  it('treats an organization switch as an account identity change', () => {
    const status = {
      ...
      state: 'connected'
    } satisfies FABRICAProfileAuthStatus
