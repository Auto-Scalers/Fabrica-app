...
export default function ArtifactsPage(): React.JSX.Element {
  ...
  const refreshAuth = useAppStore((state) => state.refreshCurrentFABRICAProfileAuth)
  ...
  const openAccountSettings = (): void => {
    openSettingsTarget({ pane: 'FABRICA-account', repoId: null })
    openSettingsPage()
  }
  const {
    accountIdentity,
  ...
  const deleteArtifact = async (item: ArtifactListItem): Promise<void> => {
    ...
    const requestedAccountIsCurrent = (): boolean =>
      artifactAccountIdentity(useAppStore.getState().FABRICAProfileAuthStatus) === requestedIdentity
              ...
              <p className="max-w-sm text-xs leading-5 text-muted-foreground">
                {needsReconnect
                  ...
                  : translate(
                      ...
                      'Use your Fabrica account to upload artifacts and manage their public links.'
                    )}
              </p>
            </div>
              ...
              <div className="flex flex-col items-center gap-2">
                <p className="max-w-sm text-xs leading-5 text-muted-foreground">
                  {translate(
                    ...
                    'Fabrica account sign-in is not configured on this machine yet.'
                  )}
                </p>
