...
import {
  canOpenAiVaultSessionLogInFABRICA,
...
function AiVaultVirtualRow({
  ...
  // Why: in-FABRICA View Log additionally withholds synthetic (SQLite/OpenCode)
  // identities that have no single file to open, while Reveal/CWD stay on the
  // existing local-path gate.
  const canOpenLogInFABRICA = row.type === 'session' && canOpenAiVaultSessionLogInFABRICA(row.session)
    ...
    >
      {row.type === 'group' ? (
        ...
        <VaultSessionRow
          ...
          onOpenLog={canOpenLogInFABRICA ? () => onOpenLog(row.session) : undefined}
