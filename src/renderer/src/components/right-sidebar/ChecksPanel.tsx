...
import {
  getTerminalUrlFABRICABrowserHint,
...
export function ChecksPanelReviewHeader({
  ...
  const modifierHint =
    modifierHintDestination === 'system-browser'
      ? getTerminalUrlSystemBrowserHint()
      : modifierHintDestination === 'FABRICA'
        ? getTerminalUrlFABRICABrowserHint()
        : null
...
export default function ChecksPanel(): React.JSX.Element {
        ...
        description:
          githubTarget && activeReview.provider === 'github'
            ? translate(
                ...
                'Review the prompt before starting an agent. After the prompt is delivered, Fabrica replies to the selected comments and resolves host threads when possible.'
              )
            : translate(
                'auto.components.right.sidebar.ChecksPanel.abf59262fb',
                'Review and edit the full command input before starting an agent.'
  ...
  /** Prompt reached the agent: only now may FABRICA write to the host. */
