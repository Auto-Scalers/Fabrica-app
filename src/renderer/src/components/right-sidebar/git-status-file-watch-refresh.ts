...
export function useGitStatusFileWatchRefresh({
  ...
  useEffect(() => {
    ...
    window.addEventListener(FABRICA_WORKTREE_FILE_CHANGE_EVENT, handleFsChanged as EventListener)

    return () => {
      if (refreshTimer) {
        clearTimeout(refreshTimer)
      }
      window.removeEventListener(FABRICA_WORKTREE_FILE_CHANGE_EVENT, handleFsChanged as EventListener)
    }
