...
export type ChecksPanelHostedReviewModifierDestination = 'system-browser' | 'FABRICA' | null

// Why: mirrors openHttpLink's routing inputs — with inverting on and Link Routing off the
// modifier now reaches FABRICA here, so gating the hint on openLinksInApp alone hides a live gesture.
export function resolveChecksPanelHostedReviewModifierDestination(
  ...
  // runtime id while the click still routes to FABRICA.
  ...
  return settings?.openLinksInAppModifierInverts === true ? 'FABRICA' : null
}
