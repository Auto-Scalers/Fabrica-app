...
describe('resolveAiVaultSessionWorktreeInfo', () => {
  ...
  it('uses prior worktree paths to identify renamed active worktrees', () => {
    const worktree = makeWorktree({
      id: 'repo-1::/repo/FABRICA-renamed',
      path: '/repo/FABRICA-renamed',
      priorWorktreeIds: ['repo-1::/repo/FABRICA']
    })
  ...
  it('falls back to unavailable when no known worktree owns the transcript cwd', () => {
    expect(
      ...
      status: 'unavailable',
      label: 'FABRICA/src',
      path: '/repo/FABRICA/src'
    })
  })

  it('matches WSL UNC worktree paths to Linux transcript cwd values', () => {
    const worktree = makeWorktree({
      path: '\\\\wsl.localhost\\Ubuntu\\home\\ada\\FABRICA'
    })

    expect(
      resolveAiVaultSessionWorktreeInfo({
        session: { ...baseSession, cwd: '/home/ada/FABRICA/src' },
        worktrees: [worktree],
        activeWorktreeId: null
      })
    ).toMatchObject({
  ...
  it('uses the session host when multiple worktrees share the same path', () => {
    ...
    expect(
      resolveAiVaultSessionWorktreeInfo({
        session: { ...baseSession, cwd: '/srv/FABRICA/src', executionHostId: 'ssh:target-1' },
...
describe('extractWorktreePathFromSessionTitle', () => {
  it('reads worktree paths embedded in session titles', () => {
    expect(
      extractWorktreePathFromSessionTitle(
        'Inspect PR #6229 - Worktree: /Users/ada/projects/FABRICA/fix-tabs'
      )
    ).toBe('/Users/ada/projects/FABRICA/fix-tabs')
    expect(extractWorktreePathFromSessionTitle('Worktree: /tmp/FABRICA-worker')).toBe(
      '/tmp/FABRICA-worker'
    )
  })
})
    ...
    expect(
      resolveAiVaultSessionWorktreeDisplay({
        session: {
          ...
          cwd: null,
          branch: null,
          title: 'Fix tabs - Worktree: /Users/ada/projects/FABRICA/fix-tabs'
        },
...
describe('aiVaultWorktreeCompactPath', () => {
  it('keeps the last two path segments for dense detail rows', () => {
    expect(aiVaultWorktreeCompactPath('/Users/ada/projects/FABRICA/improve-agent-session')).toBe(
      'FABRICA/improve-agent-session'
    )
  })
})
...
function makeWorktreeInfo(
  ...
  return {
    ...
    label: 'FABRICA',
    path: '/repo/FABRICA',
    ...(status === 'unavailable' ? {} : { worktreeId: 'repo-1::/repo/FABRICA' })
  }
}
