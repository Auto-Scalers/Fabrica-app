...
import { openAiVaultSessionLogInFABRICA } from './ai-vault-session-log-open'
...
describe('openAiVaultSessionLogInFABRICA', () => {
  it('authorizes the exact path and opens a permanent read-only local tab', async () => {
    const state = makeState()
    getStateMock.mockReturnValue(state)

    await openAiVaultSessionLogInFABRICA({ filePath: LOG_PATH, executionHostId: 'local' })
  ...
  it('withholds blank, remote, and synthetic paths without authorizing', async () => {
    ...
    await openAiVaultSessionLogInFABRICA({ filePath: '   ', executionHostId: 'local' })
    await openAiVaultSessionLogInFABRICA({ filePath: LOG_PATH, executionHostId: 'ssh:dev-box' })
    await openAiVaultSessionLogInFABRICA({
  ...
  it('shares one in-flight open for concurrent clicks of the same path', async () => {
    ...
    const first = openAiVaultSessionLogInFABRICA({ filePath: LOG_PATH, executionHostId: 'local' })
    const second = openAiVaultSessionLogInFABRICA({ filePath: LOG_PATH, executionHostId: 'local' })
    resolveAuth?.()
