...
const baseSession: AiVaultSession = {
  ...
  resumeCommand: "cd '/Users/ada/FABRICA' && claude --resume 'session-1'",
  subagent: null
}
...
describe('buildAiVaultProjectContext', () => {
  it('uses durable worktree project ids before repo fallback', () => {
    const repo = makeRepo({ id: 'repo-1', displayName: 'Legacy Repo', path: '/Users/ada/FABRICA' })
    const project = makeProject({ id: 'project-1', displayName: 'Canonical Fabrica' })
    const worktree = makeWorktree({
      id: 'wt-1',
      repoId: repo.id,
      projectId: project.id,
      path: '/Users/ada/FABRICA'
    })
    ...
    expect(context.sessionProjectById.get(baseSession.id)).toMatchObject({
      ...
      label: 'Canonical Fabrica'
    })
  })

  it('normalizes compatibility project ids to repo keys', () => {
    const repo = makeRepo({ id: 'repo-1', displayName: 'Fabrica', path: '/Users/ada/FABRICA' })
    const worktree = makeWorktree({
      id: 'wt-1',
      repoId: repo.id,
    ...
    const context = buildAiVaultProjectContext({
      ...
      worktrees: [worktree],
      projectHostSetupProjection: makeProjection({
        projects: [makeProject({ id: 'repo:repo-1', displayName: 'Compatibility Fabrica' })],
    ...
    expect(context.projectLabelByKey.get('repo:repo-1')).toBe('Fabrica')
  })
  ...
  it('inherits setup project ids for legacy worktrees without project metadata', () => {
    const repo = makeRepo({ id: 'repo-1', displayName: 'Fabrica Repo', path: '/repo/FABRICA' })
    const worktree = makeWorktree({
      id: 'wt-legacy',
      repoId: repo.id,
      path: '/repo/FABRICA'
    })
    const session = makeSession({ id: 'claude:legacy-worktree', cwd: '/repo/FABRICA/src' })
  ...
  it('uses active worktree setup project ids when active repo is unavailable', () => {
    ...
    const session = makeSession({ id: 'claude:restored', cwd: '/repo/FABRICA/src' })
  ...
  it('inherits setup host ids for legacy worktrees without host metadata', () => {
    const repo = makeRepo({ id: 'repo-1', displayName: 'Runtime Repo', path: '/runtime/FABRICA' })
    const worktree = makeWorktree({
      id: 'wt-runtime',
      repoId: repo.id,
      path: '/runtime/FABRICA'
    })
    const session = makeSession({
      id: 'claude:runtime-worktree',
      cwd: '/runtime/FABRICA/src',
  ...
  it('uses the session host when matching overlapping local and SSH project paths', () => {
    const localRepo = makeRepo({ id: 'local', displayName: 'Local', path: '/srv/FABRICA' })
  ...
  it('falls back to folder when a legacy hostless session matches multiple host buckets', () => {
    const localRepo = makeRepo({ id: 'local', displayName: 'Local', path: '/srv/FABRICA' })
    const runtimeRepo = makeRepo({ id: 'runtime', displayName: 'Runtime', path: '/srv/FABRICA' })
    const session = makeSession({
  ...
  it('maps null cwd sessions to unknown', () => {
    const repo = makeRepo({ id: 'repo-1', displayName: 'Fabrica', path: '/repo' })
