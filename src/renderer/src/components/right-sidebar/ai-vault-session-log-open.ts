...
import { canOpenAiVaultSessionLogInFABRICA } from './ai-vault-session-path-actions'
...
/**
 * Open a local AI Vault session log inside FABRICA as a permanent, read-only editor
 * tab (or activate an existing tab without reducing its authority). Reuses
 * FABRICA's external-file authorize + `openFile` pipeline; it never grants write
 * capability by itself and never redirects the open to a remote host.
 */
export async function openAiVaultSessionLogInFABRICA(session: AiVaultLogSession): Promise<void> {
  ...
  if (!filePath || !canOpenAiVaultSessionLogInFABRICA(session)) {
    return
  }
  ...
  try {
    ...
    try {
      ...
      // trust gesture. Reuses FABRICA's existing external R/W open grant.
