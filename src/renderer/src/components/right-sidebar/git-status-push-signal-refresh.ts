...
import {
  FABRICA_TERMINAL_COMMAND_FINISHED_EVENT,
...
export function useGitStatusPushSignalRefresh({
  ...
  useEffect(() => {
    ...
    window.addEventListener(FABRICA_TERMINAL_COMMAND_FINISHED_EVENT, handleCommandFinished)
    return () => {
      window.removeEventListener(FABRICA_TERMINAL_COMMAND_FINISHED_EVENT, handleCommandFinished)
    }
