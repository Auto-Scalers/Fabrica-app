...
describe('deriveAiVaultWorkspaceScopePaths', () => {
  it('returns the active workspace path', () => {
    ...
    expect(deriveAiVaultWorkspaceScopePaths(active, [active])).toEqual(['/repo/FABRICA'])
  })
  ...
  it('drops a prior path another live workspace now owns', () => {
    ...
    const claimant = makeWorktree({ id: 'repo-1::/repo/FABRICA', path: '/repo/FABRICA' })
    const active = makeWorktree({
      id: 'repo-1::/repo/FABRICA-renamed',
      path: '/repo/FABRICA-renamed',
      priorWorktreeIds: ['repo-1::/repo/FABRICA']
  ...
  it('keeps a prior path the active workspace itself still owns', () => {
    const active = makeWorktree({ priorWorktreeIds: ['repo-1::/repo/FABRICA'] })
    expect(deriveAiVaultWorkspaceScopePaths(active, [active])).toEqual(['/repo/FABRICA'])
  })
  ...
  it('drops a claimed prior path regardless of where the claimant sits in the list', () => {
    ...
    const activeAtPriorPath = makeWorktree({ id: active.id, path: '/repo/FABRICA' })
  ...
  it('derives workspace scope paths at scale', () => {
    ...
    const prefix = '/Users/dev/FABRICA/workspaces/FABRICA-monorepo/feature-'
...
describe('deriveAiVaultScopeSessionPaths', () => {
  ...
  it('deduplicates paths that differ only by separators or trailing slash', () => {
    ...
    const active = makeWorktree()
    const trailing = makeWorktree({ id: 'repo-1::a', path: '/repo/FABRICA/' })
    const doubled = makeWorktree({ id: 'repo-1::b', path: '/repo//FABRICA' })
