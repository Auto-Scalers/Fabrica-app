...
export function resolveHostedReviewAuthInstruction(provider: HostedReviewProvider): string {
  ...
  if (provider === 'azure-devops') {
    return 'Set FABRICA_AZURE_DEVOPS_TOKEN'
  }
  if (provider === 'gitea') {
    return 'Set FABRICA_GITEA_TOKEN'
  }
...
export function resolveUnavailableCreateReviewLookupNoticeMessage(
  ...
  return `Create ${copy.shortLabel} failed: Fabrica could not confirm whether this branch already has a ${copy.reviewLabel}. Retry once the ${copy.providerName} lookup succeeds.`
}
