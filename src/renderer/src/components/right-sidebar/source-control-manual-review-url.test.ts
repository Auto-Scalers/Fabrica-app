...
describe('buildSourceControlManualReviewUrl', () => {
  it('builds a GitHub compare URL for the current remote branch', () => {
    expect(
      ...
      'https://github.com/Auto-Scalers/Fabrica-app/compare/main...native-chat-does-not-auto-open?expand=1'
    )
  })
  ...
  it('qualifies GitHub fork heads when the push target remote differs from the base repo', () => {
    expect(
      buildSourceControlManualReviewUrl({
        ...
        repoRemoteUrl: 'https://github.com/Auto-Scalers/Fabrica-app.git',
        pushTarget: {
          remoteName: 'fork',
          branchName: 'feature/fork-head',
  ...
  it('keeps slashes literal in a GitHub compare URL for a slash-containing branch name', () => {
    expect(
      ...
      'https://github.com/Auto-Scalers/Fabrica-app/compare/main...slashdevcorpse/identifying-pwsh.exe-error?expand=1'
    )
  })
  ...
  it('builds a GitLab merge request URL for a self-hosted GitLab remote', () => {
    expect(
      buildSourceControlManualReviewUrl({
        ...
        repoRemoteName: 'origin',
        repoRemoteUrl: 'git@gitlab.company.test:group/sub/FABRICA.git',
        provider: 'gitlab',
        upstreamName: 'origin/feature/gitlab'
      })
    ).toBe(
      'https://gitlab.company.test/group/sub/FABRICA/-/merge_requests/new?merge_request%5Bsource_branch%5D=feature%2Fgitlab&merge_request%5Btarget_branch%5D=release%2Fnext'
    )
  })
  ...
  it('builds a Gitea compare URL when the provider is known', () => {
    expect(
      buildSourceControlManualReviewUrl({
        ...
        repoRemoteName: 'origin',
        repoRemoteUrl: 'ssh://git@gitea.company.test/team/FABRICA.git',
        provider: 'gitea',
        upstreamName: 'origin/feature/gitea'
      })
    ).toBe('https://gitea.company.test/team/FABRICA/compare/main...feature/gitea')
  })
  ...
  it('suppresses the link when the branch tracks a fork remote with no resolvable push URL', () => {
    ...
    expect(
      buildSourceControlManualReviewUrl({
        ...
        repoRemoteName: 'origin',
        repoRemoteUrl: 'https://github.com/Auto-Scalers/Fabrica-app.git',
        upstreamName: 'pr-devajmeireles-FABRICA/quick-commands'
      })
    ).toBeNull()
  })
  ...
  it('still qualifies the fork head when Fabrica resolved the fork push URL', () => {
    expect(
      buildSourceControlManualReviewUrl({
        ...
        upstreamName: 'pr-devajmeireles-FABRICA/quick-commands',
        pushTarget: {
          remoteName: 'pr-devajmeireles-FABRICA',
          branchName: 'quick-commands',
  ...
  it('does not guess a provider for unknown hosts without a provider hint', () => {
    expect(
      buildSourceControlManualReviewUrl({
        ...
        repoRemoteName: 'origin',
        repoRemoteUrl: 'git@git.company.test:team/FABRICA.git',
