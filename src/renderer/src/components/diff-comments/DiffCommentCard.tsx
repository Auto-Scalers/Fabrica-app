...
export function DiffCommentCard({
  ...
  return (
    <div ref={cardRef} className="FABRICA-diff-comment-card">
      <div className="FABRICA-diff-comment-content-col">
        {/* Header Row */}
        <div className="FABRICA-diff-comment-header">
          <div className="FABRICA-diff-comment-meta-group">{metaText}</div>

          {/* Action buttons pill (only shown if not editing) */}
          {!editing && (
            <div
              className="FABRICA-diff-comment-actions-pill"
              onMouseDown={(ev) => ev.stopPropagation()}
            >
              {headerActions}
              {headerActions && (url || onSubmitEdit || onDelete) && (
                <span className="FABRICA-diff-comment-pill-divider" />
              )}
              {url && (
                <>
                  <button
                    type="button"
                    className="FABRICA-diff-comment-pill-btn"
                    title={translate(
              ...
              {onSubmitEdit && (
                <>
                  ...
                  {onDelete && <span className="FABRICA-diff-comment-pill-divider" />}
                </>
              )}
              {onDelete && (
                <button
                  type="button"
                  className="FABRICA-diff-comment-pill-btn FABRICA-diff-comment-pill-btn-danger"
                  title={translate(
        ...
        {/* Quote Block */}
        {quote ? (
          <div className="FABRICA-diff-comment-quote">
            <div className="FABRICA-diff-comment-quote-text">{quote}</div>
          </div>
        ) : null}
        ...
        {editing ? (
          <div className="flex flex-col gap-2 mt-1">
            <textarea
              ref={textareaRef}
              className="FABRICA-diff-comment-popover-textarea"
              value={draft}
              onChange={(e) => {
              ...
              rows={3}
            />
            <div className="FABRICA-diff-comment-popover-footer">
          ...
          <div className="FABRICA-diff-comment-body">{body}</div>
        )}
      </div>
    </div>
  )
