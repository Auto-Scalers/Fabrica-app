...
type StarNagApi = {
  ...
  starFABRICA: ReturnType<typeof vi.fn>
}
...
describe('StarNagToastHost', () => {
  ...
  beforeEach(() => {
    ...
    starNag = {
      ...
      starFABRICA: vi.fn().mockResolvedValue(true)
    }
    shell = {
  ...
  it('opens GitHub fallback without calling direct star success path', async () => {
    ...
    expect(shell.openUrl).toHaveBeenCalledWith('https://github.com/Auto-Scalers/Fabrica-app')
    expect(starNag.openWeb).toHaveBeenCalledTimes(1)
    expect(starNag.starFABRICA).not.toHaveBeenCalled()
  ...
  it('switches to the explicit GitHub fallback when direct star fails', async () => {
    starNag.starFABRICA.mockResolvedValueOnce(false)
    ;({ root, container } = renderHost())
  ...
  it('does not allow unresolved close while the primary action is busy', async () => {
    ...
    starNag.starFABRICA.mockReturnValueOnce(pendingStar.promise)
    ;({ root, container } = renderHost())
