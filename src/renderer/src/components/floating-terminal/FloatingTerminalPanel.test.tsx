...
function makeFile(overrides: Partial<OpenFile> = {}): OpenFile {
  ...
  return {
    id,
    filePath: overrides.filePath ?? `/tmp/FABRICA/${id}.md`,
...
describe('FloatingTerminalPanel close behavior', () => {
  beforeEach(async () => {
    ...
    mocks.getFloatingMarkdownDirectory.mockResolvedValue('/tmp/FABRICA/floating-notes')
    mocks.getFloatingTerminalCwd.mockResolvedValue('/tmp/FABRICA')
      ...
      [FLOATING_TERMINAL_WORKTREE_ID]: [
        {
          ...
          sessionPartition: 'persist:FABRICA-browser-session-profile-1',
          createdAt: 1
        }
      ]
    }
  ...
  it('creates floating markdown files in local filesystem mode', async () => {
    ...
    vi.mocked(createUntitledMarkdownFileWithTemplateSelection).mockResolvedValue({
      filePath: '/tmp/FABRICA/floating-notes/untitled.md',
    ...
    expect(createUntitledMarkdownFileWithTemplateSelection).toHaveBeenCalledWith(
      '/tmp/FABRICA/floating-notes',
      FLOATING_TERMINAL_WORKTREE_ID,
      undefined,
      { activeRuntimeEnvironmentId: null }
    )
    expect(mocks.openFile).toHaveBeenCalledWith(
      expect.objectContaining({ filePath: '/tmp/FABRICA/floating-notes/untitled.md' }),
