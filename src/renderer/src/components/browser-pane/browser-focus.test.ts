...
describe('browser-focus', () => {
  ...
  it('requestBrowserFocus queues and dispatches the focus event', () => {
    ...
    window.addEventListener(FABRICA_BROWSER_FOCUS_REQUEST_EVENT, onFocusRequest)
    requestBrowserFocus(detail)
    window.removeEventListener(FABRICA_BROWSER_FOCUS_REQUEST_EVENT, onFocusRequest)
