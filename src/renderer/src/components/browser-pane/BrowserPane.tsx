...
import { FABRICA_BROWSER_BLANK_URL, FABRICA_BROWSER_PARTITION } from '../../../../shared/constants'
import { BROWSER_CERTIFICATE_TRUST_RUNTIME_CAPABILITY } from '../../../../shared/protocol-version'
import { getFABRICAProfileBrowserDefaultPartition } from '../../../../shared/fabrica-profiles'
import type {
...
import {
  ...
  FABRICA_BROWSER_FOCUS_REQUEST_EVENT,
...
function toDisplayUrl(url: string): string {
  return url === FABRICA_BROWSER_BLANK_URL ? 'about:blank' : redactKagiSessionToken(url)
}

function getBrowserDisplayTitle(title: string | null | undefined, url: string): string {
  if (
    url === 'about:blank' ||
    url === FABRICA_BROWSER_BLANK_URL ||
    title === 'about:blank' ||
    title === FABRICA_BROWSER_BLANK_URL ||
    !title
  ) {
    return 'New Tab'
  }
...
function RemoteBrowserPagePane({
  ...
  useEffect(() => {
    ...
    window.addEventListener(FABRICA_BROWSER_FOCUS_REQUEST_EVENT, handleBrowserFocusRequest)
    return () =>
      window.removeEventListener(FABRICA_BROWSER_FOCUS_REQUEST_EVENT, handleBrowserFocusRequest)
  ...
  const showRemoteFailureOverlay =
    ...
    remoteFailureUrl !== FABRICA_BROWSER_BLANK_URL
...
function BrowserPagePane({
  ...
  const activeFABRICAProfileId = useAppStore((s) => s.activeFABRICAProfileId)
  const fallbackBrowserPartition = activeFABRICAProfileId
    ? getFABRICAProfileBrowserDefaultPartition(activeFABRICAProfileId)
    : null
  ...
  useEffect(() => {
    ...
    const handleDidStopLoading = (): void => {
      ...
      if (keepAddressBarFocusRef.current && currentUrl === FABRICA_BROWSER_BLANK_URL) {
        focusAddressBarNow()
      } else {
        keepAddressBarFocusRef.current = false
      }
    ...
    if (needsInitialNavigation) {
      ...
      const initialUrl =
        normalizeBrowserNavigationUrl(initialBrowserUrlRef.current) ?? FABRICA_BROWSER_BLANK_URL
      trackNextLoadingEventRef.current = initialUrl !== FABRICA_BROWSER_BLANK_URL
  ...
  useEffect(() => {
    ...
    if (
      ...
      // Why: browserTab.url changes are FABRICA-driven navigations; gate did-start-loading so only real navigations show loading UI.
      trackNextLoadingEventRef.current = normalizedUrl !== FABRICA_BROWSER_BLANK_URL
      lastKnownWebviewUrlRef.current = normalizedUrl
      webview.src = normalizedUrl
      if (normalizedUrl !== FABRICA_BROWSER_BLANK_URL) {
        keepAddressBarFocusRef.current = false
        if (document.activeElement === addressBarInputRef.current) {
          focusWebviewNow()
        }
  ...
  const navigateToUrl = useCallback(
    (url: string): void => {
      const navigateBrowserUrl = (targetUrl: string): void => {
        ...
        trackNextLoadingEventRef.current = targetUrl !== FABRICA_BROWSER_BLANK_URL
        lastKnownWebviewUrlRef.current = normalizedBrowserModelUrl
        ...
        if (targetUrl !== FABRICA_BROWSER_BLANK_URL) {
          focusWebviewNow()
        }
      }
  ...
  const isBlankTab = browserTab.url === 'about:blank' || browserTab.url === FABRICA_BROWSER_BLANK_URL
