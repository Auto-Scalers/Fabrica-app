...
describe('browser notice formatting', () => {
  it('formats denied permissions with safe copy', () => {
    ...
    ).toBe('https://example.com asked for camera or microphone access, and Fabrica denied it.')
  })
  ...
  it('formats popup outcomes', () => {
    ...
    ).toBe('https://example.com opened a new page in Fabrica.')
    ...
    ).toBe('A site tried to open a popup Fabrica does not support here.')
  })
  ...
  it('formats certificate failures without local-server recovery advice', () => {
    ...
    expect(formatLoadFailureDescription(loadError(-202), meta)).toBe(
      "Fabrica doesn't trust the authority that issued the certificate for localhost:3443."
    )
    expect(formatLoadFailureDescription(loadError(-208), meta)).toBe(
      "Fabrica couldn't verify the certificate for localhost:3443."
    )
