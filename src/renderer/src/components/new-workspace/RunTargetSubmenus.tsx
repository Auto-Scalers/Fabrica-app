        ...
        <div role="listbox" aria-label={addHostLabel} onMouseLeave={() => setHoveredKey(null)}>
          ...
          {onAddRemoteServer ? (
            <RunTargetRow
              icon={<Cloud className="size-3.5 shrink-0 text-muted-foreground" />}
              label={translate(
                'auto.components.NewWorkspaceComposerCard.addRemoteFABRICAServer',
                'Add Remote Fabrica Server'
              )}
              detail={translate(
                'auto.components.NewWorkspaceComposerCard.addRemoteFABRICAServerHint',
                'Pair another Fabrica runtime'
              )}
