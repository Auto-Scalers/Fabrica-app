...
const FABRICA = project('FABRICA', 'FABRICA', 'Auto-Scalers/Fabrica-app')
const relay = project('relay', 'FABRICA-relay', 'Auto-Scalers/Fabrica-relay')
...
describe('rankProjectOptions', () => {
  it('ranks a name-prefix match above a mid-name match', () => {
    const ranked = rankProjectOptions([relay, FABRICA], 'FABRICA', [])
    expect(ranked[0]?.option.id).toBe('FABRICA')
  })
  ...
  it('orders an unfiltered list by recency', () => {
    const ranked = rankProjectOptions([FABRICA, relay, gateway], '', ['gateway', 'relay'])
    expect(ranked.map((r) => r.option.id)).toEqual(['gateway', 'relay', 'FABRICA'])
  })

  it('returns nothing for an oversized query rather than scanning it', () => {
    expect(rankProjectOptions([FABRICA], 'x'.repeat(4096), [])).toEqual([])
  })
})
...
describe('sectionProjectOptions', () => {
  ...
  it('keeps a short list unsectioned', () => {
    const matches = rankProjectOptions([FABRICA, relay], '', ['relay'])
...
describe('getAmbiguousProjectOptionIds', () => {
  it('flags only ids whose display name repeats', () => {
    ...
    const ids = getAmbiguousProjectOptionIds([a, b, FABRICA])
...
describe('splitDetailForElision', () => {
  ...
  it('leaves short or shallow details alone', () => {
    expect(splitDetailForElision('Auto-Scalers/Fabrica-app')).toBeNull()
