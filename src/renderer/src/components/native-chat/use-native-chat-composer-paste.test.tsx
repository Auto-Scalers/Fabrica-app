...
describe('useNativeChatComposerPaste', () => {
  ...
  it('saves on the SSH host and attaches the returned remote path', async () => {
    mocks.saveClipboardImageAsTempFile.mockResolvedValue('/remote/tmp/FABRICA-paste-1.png')
    const attachResolvedPaths = vi.fn()
    const probe = await renderProbe({
      resolveAttachmentOwner: () => sshOwner,
      attachResolvedPaths
    ...
    expect(attachResolvedPaths).toHaveBeenCalledWith(['/remote/tmp/FABRICA-paste-1.png'])
  })
