...
// 'severed': macOS can no longer attribute daemon terminals to FABRICA, so Accessibility/
...
export type AppApi = {
  ...
  /** Restarts FABRICA through the normal quit pipeline so daemon-backed terminal
  ...
  /** Resolves FABRICA's app-owned directory for auto-created Floating Workspace
   *  markdown notes. */
...
export type PreloadApi = {
  app: AppApi
  FABRICAProfiles: {
    list: () => Promise<FABRICAProfileListResult>
    authStatus: () => Promise<FABRICAProfileAuthStatus>
    createLocal: (args?: CreateLocalFABRICAProfileArgs) => Promise<CreateLocalFABRICAProfileResult>
    createCloudLinked: (
      args?: CreateCloudLinkedFABRICAProfileArgs
    ) => Promise<CreateCloudLinkedFABRICAProfileResult>
    switchProfile: (args: SwitchFABRICAProfileArgs) => Promise<SwitchFABRICAProfileResult>
    transferProject: (
    ...
    ) => Promise<FindFABRICAProfileProjectsByPathResult>
    connectCurrent: () => Promise<ConnectCurrentFABRICAProfileResult>
    refreshAuth: () => Promise<RefreshCurrentFABRICAProfileAuthResult>
    signOutCurrent: () => Promise<SignOutCurrentFABRICAProfileResult>
    selectOrg: (args: SelectFABRICAProfileOrgArgs) => Promise<SelectFABRICAProfileOrgResult>
    orgMembersList: (
      args: FABRICAProfileOrgMembersListArgs
  ...
  worktrees: {
    ...
    // Forget a workspace from FABRICA only (no remote Git/FS work) — for workspaces pinned to a removed/disconnected SSH host.
    forgetLocal: (args: {
      worktreeId: string
      hostId?: ExecutionHostId
    }) => Promise<RemoveWorktreeResult>
  ...
  macosTccPrompts: {
    /** Fires once macOS has raised its Nth consent dialog naming FABRICA (#9756). */
  ...
  ephemeralVm: {
    listRecipes: (args: { repoId: string }) => Promise<{
      ...
      recipes: FABRICAHooks['environmentRecipes']
      diagnostics: NonNullable<FABRICAHooks['environmentRecipeDiagnostics']>
      message?: string
    }>
    listRecipeCatalog: () => Promise<
      {
        ...
        repoName: string
        repoPath: string
        recipes: NonNullable<FABRICAHooks['environmentRecipes']>
        diagnostics: NonNullable<FABRICAHooks['environmentRecipeDiagnostics']>
      }[]
    >
