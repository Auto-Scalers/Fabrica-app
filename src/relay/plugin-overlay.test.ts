...
describe('PluginOverlayManager', () => {
  ...
  it('materializes OpenCode plugin into <overlay>/plugins/<file>', () => {
    ...
    const expected = join(dir!, 'plugins', 'FABRICA-opencode-status.js')
    expect(existsSync(expected)).toBe(true)
    expect(readFileSync(expected, 'utf8')).toBe('export const X = 1')
  })

  it('mirrors a preexisting remote OpenCode config dir before adding FABRICA plugin', () => {
    ...
    writeFileSync(join(userConfigDir, 'plugins', 'FABRICA-opencode-status.js'), 'user same-name')

    manager.setSources({ opencodePluginSource: 'FABRICA plugin' })
    ...
    expect(readFileSync(join(dir!, 'plugins', 'FABRICA-opencode-status.js'), 'utf8')).toBe(
      'FABRICA plugin'
    )
  ...
  it('installs Pi extension into the real agent extensions dir', () => {
    ...
    const file = join(result!.sourceAgentDir!, 'extensions', 'FABRICA-agent-status.ts')
    expect(result?.statusExtensionPath).toBe(file)
    expect(existsSync(file)).toBe(true)
    expect(readFileSync(file, 'utf8')).toContain('@FABRICA-managed-pi-extension')
  })

  it("does not overwrite a user's same-named remote Pi extension file", () => {
    const piAgentDir = join(homeDir, '.pi', 'agent')
    const extensionFile = join(piAgentDir, 'extensions', 'FABRICA-agent-status.ts')
  ...
  it('uses the kind-specific Pi-compatible extension source when available', () => {
    ...
    expect(
      readFileSync(join(piResult!.sourceAgentDir!, 'extensions', 'FABRICA-agent-status.ts'), 'utf8')
    ).toContain('// pi extension')
    expect(
      readFileSync(join(ompResult!.sourceAgentDir!, 'extensions', 'FABRICA-agent-status.ts'), 'utf8')
  ...
  it('installs FABRICA status extension into the remote default Pi agent dir', () => {
    const piAgentDir = join(homeDir, '.pi', 'agent')
  ...
  it('mirrors a preexisting remote Pi agent dir instead of the default', () => {
    ...
    expect(readFileSync(join(dir!, 'extensions', 'FABRICA-agent-status.ts'), 'utf8')).toContain(
      '// pi extension'
    )
  })
  ...
  it('leaves lazy OMP agent.db in the real remote home on the relay', () => {
    ...
    expect(existsSync(join(homeDir, '.FABRICA-relay', 'omp-overlays'))).toBe(false)
  ...
  describe('per-agent default source dir (no cross-agent fallback)', () => {
    ...
    it('launching pi with both ~/.pi/agent and ~/.omp/agent present installs into ~/.pi/agent', () => {
      ...
      expect(extensions).toContain('FABRICA-agent-status.ts')
    ...
    it('launching omp when only ~/.pi/agent exists does NOT mirror Pi state', () => {
      ...
      const extensions = readdirSync(join(dir!, 'extensions')).sort()
      expect(extensions).toEqual(['FABRICA-agent-status.ts'])
    })
    ...
    it('bare-shell prep does not create missing remote agent homes (#10196)', () => {
      ...
      // Why: bare OMP keeps status via ~/.FABRICA-relay/… without SOURCE_AGENT_DIR or ~/.omp.
      expect(bareOmp?.sourceAgentDir).toBeUndefined()
      expect(bareOmp?.statusExtensionPath).toEqual(
        expect.stringContaining(join('.FABRICA-relay', 'omp-managed-status-extension'))
      )
  ...
  it('produces stable overlay dirs for a given id (idempotent re-materialization)', () => {
    ...
    expect(readFileSync(join(dirA, 'plugins', 'FABRICA-opencode-status.js'), 'utf8')).toBe('second')
  })
  ...
  it('hashes unsafe pane ids into portable overlay directory names', () => {
    ...
    expect(existsSync(join(dir!, 'plugins', 'FABRICA-opencode-status.js'))).toBe(true)
  })
})
...
describe('resolvePiSourceAgentDir', () => {
  it('uses only the selected kind source shadow when resolving inherited overlays', () => {
    const env = {
      ...
      FABRICA_PI_SOURCE_AGENT_DIR: '/user/.pi/agent'
    }
    try {
  ...
  it('keeps explicit PI_CODING_AGENT_DIR values when they are not FABRICA overlays', () => {
    const env = {
      HOME: mkdtempSync(join(tmpdir(), 'plugin-overlay-env-')),
