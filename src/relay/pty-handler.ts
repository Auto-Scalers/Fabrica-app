...
type ManagedPty = {
  ...
  /** Renderer-supplied paneKey (FABRICA_PANE_KEY); captured so exit observers can evict per-pane cache state. */
  paneKey?: string
  tabId?: string
...
export class PtyHandler {
   ...
   *  Used by the relay-hook server to inject FABRICA_AGENT_HOOK_* coords. See docs/design/agent-status-over-ssh.md §3. */
    ...
    const baseEnv = mergeGitConfigEnvProtocol(
      {
        ...
        TERM_PROGRAM_VERSION:
          rendererEnv?.FABRICA_APP_VERSION || process.env.FABRICA_APP_VERSION || '0.0.0-dev',
        FORCE_HYPERLINK: '1'
      },
      rendererEnv
  ...
  private async spawnOnce(
    ...
    const worktreeId = env?.FABRICA_WORKTREE_ID
  ...
  private async spawnAfterAdmission(
    ...
    const paneKey = typeof env?.FABRICA_PANE_KEY === 'string' ? env.FABRICA_PANE_KEY : undefined
    // Why: kept so a restarted runtime can re-adopt this PTY under its original handle (survives revive).
    const terminalHandle =
      typeof env?.FABRICA_TERMINAL_HANDLE === 'string' ? env.FABRICA_TERMINAL_HANDLE : undefined
    ...
    try {
      term = pty.spawn(shell, shellLaunch.args, {
        ...
        rows,
        cwd,
        // Why: relay shells inherit process.env; don't let an ambient FABRICA marker enable shell-ready unless requested.
        env: { ...spawnEnv, FABRICA_SHELL_READY_MARKER: '0', ...shellLaunch.env }
      })
    } catch (error) {
    ...
    const tabId = typeof env?.FABRICA_TAB_ID === 'string' ? env.FABRICA_TAB_ID : undefined
    ...
    const worktreeId = typeof env?.FABRICA_WORKTREE_ID === 'string' ? env.FABRICA_WORKTREE_ID : undefined
      ...
      ...(shouldProviderDeliverCommand
        ? {
            startupCommand: {
              command,
              delivered: false,
              waitForShellReady: shellLaunch.env.FABRICA_SHELL_READY_MARKER === '1',
              scanState:
                shellLaunch.env.FABRICA_SHELL_READY_MARKER === '1'
                  ? createShellReadyScanState()
                  : null,
              timer: null
            }
  ...
  private async reviveEntry(entry: SerializedPtyEntry): Promise<void> {
    ...
    // Why: pane identity comes from the serialized entry (not env) since hook scripts exit without FABRICA_PANE_KEY.
    const revivedEnv: Record<string, string> = {}
    if (entry.paneKey) {
      revivedEnv.FABRICA_PANE_KEY = entry.paneKey
    }
    if (entry.tabId) {
      revivedEnv.FABRICA_TAB_ID = entry.tabId
    }
    if (entry.worktreeId) {
      revivedEnv.FABRICA_WORKTREE_ID = entry.worktreeId
    }
    if (entry.terminalHandle) {
      revivedEnv.FABRICA_TERMINAL_HANDLE = entry.terminalHandle
    }
    const explicitTerm =
