// Wire-level handshake helpers for the FABRICA relay.
...
export function setupDaemonHandshake(sock: Socket, cb: DaemonHandshakeCallbacks): void {
  ...
  ;(sock as Socket & { __FABRICAOnHandshake?: typeof onHandshakeData }).__FABRICAOnHandshake =
    onHandshakeData
}

export function detachHandshakeListener(sock: Socket): void {
  const tagged = sock as Socket & { __FABRICAOnHandshake?: (chunk: Buffer) => void }
  if (tagged.__FABRICAOnHandshake) {
    sock.removeListener('data', tagged.__FABRICAOnHandshake)
    delete tagged.__FABRICAOnHandshake
  }
}
