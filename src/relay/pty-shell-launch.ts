...
const RELAY_SHELL_READY_DIR = '.FABRICA-relay/shell-ready'
...
function hasOverlayRestoreEnv(env: Record<string, string>): boolean {
  return Boolean(
    ...
    env.FABRICA_REMOTE_CLI_BIN_DIR ||
...
function resolveOriginalZdotdir(env: Record<string, string>): string {
  return (
    ...
    normalizeOriginalZdotdirCandidate(env.FABRICA_ORIG_ZDOTDIR) ||
    env.HOME ||
    process.env.HOME ||
    ''
  )
...
function ensureOverlayRestoreWrappers(root: string): void {
  ...
  const zshEnv = `# FABRICA relay zsh overlay wrapper
export FABRICA_ORIG_ZDOTDIR="\${FABRICA_ORIG_ZDOTDIR:-$HOME}"
case "\${FABRICA_ORIG_ZDOTDIR%/}" in
  */shell-ready/zsh) export FABRICA_ORIG_ZDOTDIR="$HOME" ;;
esac
[[ -f "$FABRICA_ORIG_ZDOTDIR/.zshenv" ]] && source "$FABRICA_ORIG_ZDOTDIR/.zshenv"
export FABRICA_USER_ZDOTDIR="\${ZDOTDIR:-\${FABRICA_ORIG_ZDOTDIR:-$HOME}}"
case "\${FABRICA_USER_ZDOTDIR%/}" in
  */shell-ready/zsh) export FABRICA_USER_ZDOTDIR="$HOME" ;;
esac
export ZDOTDIR=${quotePosixSingle(zshDir)}
`
  const zshProfile = `# FABRICA relay zsh overlay wrapper
${getZshStartupFileSourceBlock({
  fileName: '.zprofile',
  homeExpression: '"${FABRICA_USER_ZDOTDIR:-${FABRICA_ORIG_ZDOTDIR:-$HOME}}"'
})}
`
  const zshRc = `# FABRICA relay zsh overlay wrapper
${getZshStartupFileSourceBlock({
  fileName: '.zshrc',
  homeExpression: '"${FABRICA_USER_ZDOTDIR:-${FABRICA_ORIG_ZDOTDIR:-$HOME}}"',
...
if [[ ! -o login ]]; then
  ...
  [[ -n "\${FABRICA_REMOTE_CLI_BIN_DIR:-}" ]] && case ":$PATH:" in *:"\${FABRICA_REMOTE_CLI_BIN_DIR}":*) ;; *) export PATH="\${FABRICA_REMOTE_CLI_BIN_DIR}:$PATH" ;; esac
  ${getPosixOmpShellWrapper()}
fi
if [[ ! -o login ]]; then
${getZshFinalZdotdirRestoreBlock('"${FABRICA_USER_ZDOTDIR:-${FABRICA_ORIG_ZDOTDIR:-$HOME}}"')}
fi
`
  const zshLogin = `# FABRICA relay zsh overlay wrapper
${getZshStartupFileSourceBlock({
  fileName: '.zlogin',
  homeExpression: '"${FABRICA_USER_ZDOTDIR:-${FABRICA_ORIG_ZDOTDIR:-$HOME}}"',
  interactiveOnly: true
...
`
  const bashRc = `# FABRICA relay bash overlay wrapper
...
__FABRICA_osc133_preexec() {
  ...
  case "$BASH_COMMAND" in
    *__FABRICA_osc133_precmd*|*__FABRICA_osc133_prompt_done*) return ;;
  esac
...
__FABRICA_normalize_prompt_command() {
  ...
  # a ";"/whitespace separator; trim it so FABRICA's prepend/append never form ";;".
...
`
  ...
  for (const [path, content] of files) {
    ...
    // Why: relay wrapper files persist under ~/.FABRICA-relay across app
...
export function getRelayShellLaunchConfig(
  ...
  if (shellName === 'zsh') {
    return {
      ...
      env: {
        FABRICA_ORIG_ZDOTDIR: resolveOriginalZdotdir(env),
        ZDOTDIR: join(root, 'zsh'),
        ...(emitReadyMarker ? { FABRICA_SHELL_READY_MARKER: '1' } : {})
      }
    }
  }
  ...
  return {
    ...
    env: emitReadyMarker ? { FABRICA_SHELL_READY_MARKER: '1' } : {}
  }
}
