// Why: relay-side equivalent of FABRICA's local agent integration installers.
// OpenCode still needs a config overlay, while Pi/OMP now get FABRICA-managed
...
// relay is versioned independently from FABRICA and the plugin source changes
...
// directly: those modules import `electron` and ride on FABRICA's userData
// path. The relay's electron-free constraint forces a thin parallel
// implementation rooted at $HOME/.FABRICA-relay/ for OpenCode and at the remote
...
const RELAY_HOOKS_DIR = '.FABRICA-relay'
const OPENCODE_OVERLAY_SUBDIR = 'opencode-overlays'
const PI_OVERLAY_SUBDIR_BY_KIND: Record<LegacyOverlayAgentKind, string> = {
  pi: 'pi-overlays',
  omp: 'omp-overlays'
}
const OPENCODE_PLUGIN_FILE = 'FABRICA-opencode-status.js'
const PI_EXTENSION_FILE = 'FABRICA-agent-status.ts'
const PI_AGENT_SUBDIR = 'agent'
// Why: bare-shell OMP still needs FABRICA_OMP_STATUS_EXTENSION without mkdir ~/.omp.
...
const FABRICA_MANAGED_EXTENSION_MARKER = '@FABRICA-managed-pi-extension'

function withFABRICAManagedPiExtensionMarker(source: string): string {
  return source.includes(FABRICA_MANAGED_EXTENSION_MARKER)
    ? source
    : `// ${FABRICA_MANAGED_EXTENSION_MARKER}\n${source}`
}
...
function safeDirName(input: string): string {
  ...
  // filesystem-unsafe characters in some FABRICA builds. Hash to a fixed-width
...
export type PluginSources = {
  /** Source body of `FABRICA-opencode-status.js` to drop into <overlay>/plugins/. */
  opencodePluginSource?: string
  /** Source body of Pi's `FABRICA-agent-status.ts` to drop into <overlay>/extensions/. */
  piExtensionSource?: string
  /** Source body of OMP's `FABRICA-agent-status.ts` to drop into <overlay>/extensions/. */
  ompExtensionSource?: string
  /** Source body of Prime Agent's `FABRICA-agent-status.ts` to install in its real agent dir. */
...
export type MaterializePiResult = {
  ...
  /** Absolute path to FABRICA-agent-status.ts (real home or relay-managed fallback). */
...
export class PluginOverlayManager {
  ...
  /** Replace the cached source bodies. Called from relay.ts when FABRICA sends
   *  `agent_hook.installPlugins`. The first install enables the augmenter
   *  output; subsequent installs (e.g. FABRICA version upgrade in flight) refresh
  ...
  setSources(sources: PluginSources): void {
    ...
    if (typeof sources.piExtensionSource === 'string') {
      this.piExtensionSources.pi = withFABRICAManagedPiExtensionMarker(sources.piExtensionSource)
    }
    if (typeof sources.ompExtensionSource === 'string') {
      this.piExtensionSources.omp = withFABRICAManagedPiExtensionMarker(sources.ompExtensionSource)
    }
    if (typeof sources.primeAgentExtensionSource === 'string') {
      this.piExtensionSources['prime-agent'] = withFABRICAManagedPiExtensionMarker(
  ...
  materializeOpenCode(id: string, existingConfigDir?: string): string | null {
    ...
    try {
      ...
      if (existingConfigDir) {
        ...
        // remote root into the overlay before adding FABRICA's plugin so status
  ...
  private canOverwritePiExtension(path: string): boolean {
    try {
      return readFileSync(path, 'utf8').includes(FABRICA_MANAGED_EXTENSION_MARKER)
    } catch {
      return true
    }
  }
   ...
   *  export FABRICA_OMP_STATUS_EXTENSION without FABRICA_OMP_SOURCE_AGENT_DIR. */
  materializePi(
    id: string,
