...
// FABRICA Relay — lightweight daemon deployed to remote hosts over SCP and launched via an SSH exec channel.
...
const EMPTY_DETACHED_STARTUP_GRACE_MS = parseNonNegativeIntEnv(
  'FABRICA_RELAY_EMPTY_STARTUP_GRACE_MS',
  60_000
)
...
const IDLE_RELAY_GRACE_MS = parseNonNegativeIntEnv('FABRICA_RELAY_IDLE_GRACE_MS', 15 * 60_000)
...
async function runFABRICACliMode(
  sockPath: string,
  argv: string[],
  ...
  const stdin =
    ...
    (shouldReadRemoteCliStdin(argv) ? await readFABRICACliStdin() : undefined)
  ...
  const connectTimeout = setTimeout(() => {
    process.stderr.write(`[FABRICA-cli] Relay connection timed out after ${CONNECT_TIMEOUT_MS}ms\n`)
    sock.destroy()
    process.exit(1)
  }, CONNECT_TIMEOUT_MS)
  ...
  sock.on('error', (err) => {
    clearTimeout(connectTimeout)
    process.stderr.write(`[FABRICA-cli] Relay socket error: ${err.message}\n`)
    process.exit(1)
  })
}

async function readFABRICACliStdin(): Promise<string | undefined> {
...
async function main(): Promise<void> {
  ...
  if (cliMode) {
    const marker = process.argv.indexOf('--fabrica-cli')
    await runFABRICACliMode(
      sockPath,
  ...
  dispatcher.onRequest('FABRICA.cli', async (params, context) => {
    return await dispatcher.requestAnyClient('FABRICA.cli', params, {
      excludeClientId: context.clientId,
      timeoutMs: remoteCliRequestTimeoutMs(params)
    })
  })
  dispatcher.onRequest('FABRICA.cli.postOutput', async (params, context) => {
    return await dispatcher.requestAnyClient('FABRICA.cli.postOutput', params, {
      excludeClientId: context.clientId,
      timeoutMs: remoteCliRequestTimeoutMs(params)
    })
  })
  ...
  // ── Agent-hook server ─────────────────────────────────────────────
  // Why: loopback HTTP receiver so remote-PTY agent CLIs post hook events locally, forwarded to FABRICA as agent.hook notifications. See docs/design/agent-status-over-ssh.md §2-§5.
  ...
  // Why: await the bind before announcing readiness so the first PTY spawn already sees FABRICA_AGENT_HOOK_* env; bind failure is soft (log and continue).
  try {
  ...
  // Why: read the augmenter on every spawn so a late (or restarted) hook-server bind still lands in the next PTY's FABRICA_AGENT_HOOK_* env.
  ...
  ptyHandler.addEnvAugmenter((ctx) => {
    ...
    if (pluginOverlay.hasOpenCodeSource()) {
      ...
      if (dir) {
        ...
        env.FABRICA_OPENCODE_CONFIG_DIR = dir
        if (sourceDir) {
          env.FABRICA_OPENCODE_SOURCE_CONFIG_DIR = sourceDir
        }
      }
    }
    if (pluginOverlay.hasPiSource()) {
      // Why: install FABRICA's guarded extension into the launched agent's (Pi vs OMP) real remote dir without redirecting PI_CODING_AGENT_DIR.
      ...
      if (kind === 'pi') {
        ...
        if (result?.sourceAgentDir) {
          env.FABRICA_PI_SOURCE_AGENT_DIR = result.sourceAgentDir
        }
      }
      if (shouldPrepareOmpShadow) {
        ...
        const sourceDir =
          kind === 'omp'
            ...
            : ctx.env.FABRICA_OMP_SOURCE_AGENT_DIR
        ...
        // shells without ~/.omp — still export FABRICA_OMP_STATUS_EXTENSION (#10196).
        if (result?.statusExtensionPath) {
          env.FABRICA_OMP_STATUS_EXTENSION = result.statusExtensionPath
        }
        if (result?.sourceAgentDir) {
          env.FABRICA_OMP_SOURCE_AGENT_DIR = result.sourceAgentDir
        }
      }
      if (kind === 'prime-agent') {
        ...
        if (result?.sourceAgentDir) {
          env.FABRICA_PRIME_AGENT_SOURCE_AGENT_DIR = result.sourceAgentDir
        }
      }
    }
    return env
  ...
  // Why: plugin sources ship over the wire so an FABRICA update doesn't force a relay redeploy; cache them per spawn. See docs/design/agent-status-over-ssh.md §4.
  // Why: bound per-source size so a buggy/hostile FABRICA can't OOM the relay by pushing a giant string.
