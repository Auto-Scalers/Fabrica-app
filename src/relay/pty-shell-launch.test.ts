...
describe('getRelayShellLaunchConfig', () => {
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      ...
      const zshRoot = join(homeDir, '.FABRICA-relay', 'shell-ready', 'zsh')
      ...
      expect(readFileSync(join(zshRoot, '.zprofile'), 'utf8')).toContain(
        '_FABRICA_home="${FABRICA_USER_ZDOTDIR:-${FABRICA_ORIG_ZDOTDIR:-$HOME}}"'
      )
      ...
      expect(zshrc).toContain('_FABRICA_home="${FABRICA_USER_ZDOTDIR:-${FABRICA_ORIG_ZDOTDIR:-$HOME}}"')
      expect(zlogin).toContain('_FABRICA_home="${FABRICA_USER_ZDOTDIR:-${FABRICA_ORIG_ZDOTDIR:-$HOME}}"')
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      ...
      expect(config.args).toEqual(['-l'])
      expect(config.env.ZDOTDIR).toBe(zshRoot)
      expect(config.env.FABRICA_SHELL_READY_MARKER).toBe('1')
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      ...
      expect(config.env.FABRICA_SHELL_READY_MARKER).toBe('1')
      expect(bashRc).toContain('__FABRICA_append_prompt_command "__FABRICA_prompt_mark"')
      expect(bashRc).toContain('printf "\\033]777;FABRICA-shell-ready\\007"')
    }
  )
  ...
  // FABRICA inherits ends in a ";"+whitespace separator. Prepend/append must not
