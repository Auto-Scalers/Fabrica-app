...
describe.skipIf(process.platform === 'win32')('createInstallPluginsHandler (guest side)', () => {
  ...
  it('writes FABRICA-opencode-status.js into the overlay and returns that dir', () => {
    withHome((home) => {
      const install = createInstallPluginsHandler(new PluginOverlayManager({ homeDir: home }), {
        HOME: home,
        FABRICA_WSL_HOOK_INSTANCE: 'inst1'
      } as NodeJS.ProcessEnv)
      const source = '// FABRICA opencode status plugin\nexport const Plugin = () => ({})\n'
      ...
      const pluginPath = join(dir as string, 'plugins', 'FABRICA-opencode-status.js')
  ...
  it('rebuilds if the resolved source dir ever changes (defensive)', () => {
    withHome((home) => {
      ...
      const env = { HOME: home, FABRICA_WSL_HOOK_INSTANCE: 'inst1' } as NodeJS.ProcessEnv
      ...
      env.FABRICA_OPENCODE_SOURCE_CONFIG_DIR = userConfig
  ...
  it('rebuilds when the cached overlay lost its plugin file', () => {
    withHome((home) => {
      ...
      rmSync(join(dir, 'plugins', 'FABRICA-opencode-status.js'))
  ...
  it('re-materializes when the shipped source changes', () => {
    withHome((home) => {
      ...
      // Why: a mid-session FABRICA upgrade ships new plugin source; future spawns must see it.
      const dir = install({ opencodePluginSource: '// v2\n' }).overlayDirs.opencode as string
      expect(readFileSync(join(dir, 'plugins', 'FABRICA-opencode-status.js'), 'utf8')).toBe('// v2\n')
    })
  })
  ...
  it('mirrors an explicitly-set config root so overriding the var does not drop it', () => {
    withHome((home) => {
      ...
      const install = createInstallPluginsHandler(new PluginOverlayManager({ homeDir: home }), {
        HOME: home,
        FABRICA_OPENCODE_SOURCE_CONFIG_DIR: userConfig,
        FABRICA_WSL_HOOK_INSTANCE: 'inst1'
      } as NodeJS.ProcessEnv)
