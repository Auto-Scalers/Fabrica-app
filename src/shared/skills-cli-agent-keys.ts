...
/**
 * The community `skills` CLI's own `--agent` key for each agent FABRICA detects.
 *
 ...
 * rather than guessed. FABRICA ids and skills keys agree less often than they look
 * (`claude` is `claude-code`, `rovo` is `rovodev`, `aug` is `augment`), and some
 * near-matches are different products — FABRICA's `aider` CLI is not the CLI's
 * `aider-desk`, and FABRICA's `openclaude` is its `openclaw` in name only, so it
 * follows FABRICA's own rule that OpenClaude reads Claude-owned roots.
 */
export const SKILLS_CLI_AGENT_KEY_BY_TUI_AGENT = {
  claude: 'claude-code',
  'claude-agent-teams': 'claude-code',
  // Why: FABRICA states OpenClaude reads Claude-owned roots (native-chat-agent-profiles).
  ...
  devin: 'devin',
  ante: null,
  // Why: FABRICA detects trae by `traecli`, an alias only TRAE CN ships.
  trae: 'trae-cn'
} satisfies Record<TuiAgent, string | null>

/**
 * The shared `.agents/skills` target every universal agent reads. Always included
 * so agents FABRICA cannot map still receive the skill.
 */
...
/** Map detected FABRICA agents onto `skills --agent` keys, plus the universal target. */
