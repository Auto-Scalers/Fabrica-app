...
import {
  ...
  isFABRICADispatchStatusPrompt
} from './fabrica-dispatch-status-prompt'
...
/** Normalize the agent prompt field, compacting FABRICA dispatch preambles. */
export function normalizePromptField(value: unknown): string {
  if (typeof value !== 'string') {
    return ''
  }
  if (isFABRICADispatchStatusPrompt(value)) {
