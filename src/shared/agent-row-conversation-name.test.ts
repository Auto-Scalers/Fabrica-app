...
describe('getAgentRowConversationName', () => {
  ...
  it('rejects spinner+cwd titles instead of surfacing paths as names', () => {
    expect(
      getAgentRowConversationName(makeTab({ title: '⠋ ~/FABRICA/workspaces' }), 'codex', false)
    ).toBeNull()
    expect(
      getAgentRowConversationName(makeTab({ title: '/Users/dev/repo' }), 'codex', false)
    ).toBeNull()
    expect(
      getAgentRowConversationName(makeTab({ title: 'C:\\repos\\FABRICA' }), 'codex', false)
    ).toBeNull()
    expect(
      getAgentRowConversationName(makeTab({ title: 'FABRICA/workspaces' }), 'codex', false)
    ).toBeNull()
    expect(
      getAgentRowConversationName(
        makeTab({ title: '\\\\wsl.localhost\\Ubuntu\\home\\dev\\FABRICA' }),
        'codex',
        false
      )
    ).toBeNull()
    expect(
      getAgentRowConversationName(makeTab({ title: 'repos\\FABRICA' }), 'codex', false)
    ).toBeNull()
  })
  ...
  it('rejects identity-echo, management, and placeholder titles', () => {
    ...
    expect(
      getAgentRowConversationName(makeTab({ title: '◇ Ready (FABRICA)' }), 'gemini', false)
    ).toBeNull()
    expect(
