...
describe('pairing offer', () => {
  ...
  it('encode then decode round-trips correctly', () => {
    ...
    expect(url).toMatch(/^FABRICA:\/\/pair\?code=/)
  ...
  it('rejects FABRICA URLs outside the exact pairing route', () => {
    ...
    expect(parsePairingCode(`FABRICA://pairing?code=${code}`)).toBeNull()
    expect(parsePairingCode(`FABRICA://pair-extra?code=${code}`)).toBeNull()
    expect(() => decodePairingOffer(`FABRICA://pairing?code=${code}`)).toThrow('Invalid pairing URL')
  })

  it('rejects URLs without a pairing code', () => {
    expect(() => decodePairingOffer('FABRICA://pair')).toThrow('Invalid pairing URL')
  })

  it('decodes legacy hash URLs', () => {
    const url = encodePairingOffer(offer)
    const code = new URLSearchParams(url.slice(url.indexOf('?') + 1)).get('code')!
    expect(decodePairingOffer(`FABRICA://pair#${code}`)).toEqual(offer)
  })
  ...
  it('rejects payloads with missing fields', () => {
    ...
    expect(() => decodePairingOffer(`FABRICA://pair#${base64}`)).toThrow()
  })
...
describe('parsePairingCode', () => {
  ...
  it('parses a full FABRICA://pair# URL', () => {
    const url = encodePairingOffer(offer)
