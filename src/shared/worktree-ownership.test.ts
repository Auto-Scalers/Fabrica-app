...
function makeSettings(overrides: Partial<GlobalSettings> = {}): GlobalSettings {
  return {
    workspaceDir: '/FABRICA/workspaces',
    nestWorkspaces: true,
    workspaceDirHistory: [],
...
describe('worktree ownership classification', () => {
  it('treats explicit FABRICA metadata as managed even outside the workspace root', () => {
    const repo = makeRepo()
    ...
    expect(
      classifyWorktreeOwnership({
        ...
        meta: makeMeta({ FABRICACreatedAt: 1 }),
        knownFABRICALayouts: buildKnownFABRICAWorkspaceLayouts(settings, repo)
      })
    ).toBe('FABRICA-managed')
  })

  it('treats nested FABRICA workspace paths without metadata as external', () => {
    const repo = makeRepo()
    const settings = makeSettings()
    const layouts = buildKnownFABRICAWorkspaceLayouts(settings, repo)
    expect(
      classifyWorktreeOwnership({
        repo,
        settings,
        worktree: makeWorktree({ path: '/FABRICA/workspaces/app/feature' }),
        knownFABRICALayouts: layouts
      })
    ).toBe('external')
    expect(
      classifyWorktreeOwnership({
        repo,
        settings,
        worktree: makeWorktree({ path: '/FABRICA/workspaces/other/feature' }),
        knownFABRICALayouts: layouts
      })
    ).toBe('external')
  })

  it('treats explicit FABRICA creation layout metadata as managed', () => {
    const repo = makeRepo()
    const settings = makeSettings()
    expect(
      classifyWorktreeOwnership({
        ...
        worktree: makeWorktree({ path: '/FABRICA/workspaces/app/feature' }),
        meta: makeMeta({
          FABRICACreationWorkspaceLayout: { path: '/FABRICA/workspaces', nestWorkspaces: true }
        }),
        knownFABRICALayouts: buildKnownFABRICAWorkspaceLayouts(settings, repo)
      })
    ).toBe('FABRICA-managed')
  ...
  it('does not treat metadata-free nested workspace paths as FABRICA-managed for new repos', () => {
    ...
    const detected = toDetectedWorktree({
      ...
      worktree: makeWorktree({
        path: '/FABRICA/workspaces/app/manual-git-worktree',
  ...
  it('does not treat generic discovery metadata on nested workspace paths as FABRICA-managed', () => {
    const repo = makeRepo({ externalWorktreeVisibility: 'hide' })
    const settings = makeSettings()
    const detected = toDetectedWorktree({
      repo,
  ...
  it('treats flat workspace-root descendants as unknown legacy without strong metadata', () => {
    ...
    expect(
      classifyWorktreeOwnership({
        repo,
        settings,
        worktree: makeWorktree({ path: '/FABRICA/workspaces/feature' }),
        knownFABRICALayouts: buildKnownFABRICAWorkspaceLayouts(settings, repo)
      })
    ).toBe('unknown-legacy')
  })
  ...
  it('keeps flat-layout history weak after switching the same root to nested mode', () => {
    ...
    const settings = makeSettings({
      nestWorkspaces: true,
      workspaceDirHistory: [{ path: '/FABRICA/workspaces', nestWorkspaces: false }]
    })
    expect(
      classifyWorktreeOwnership({
        repo,
  ...
  it('handles Windows drive casing and separators', () => {
    ...
    const settings = makeSettings({ workspaceDir: 'C:\\FABRICA\\Workspaces' })
    expect(
      classifyWorktreeOwnership({
        repo,
        settings,
        worktree: makeWorktree({
          id: 'repo-1::C:\\FABRICA\\WORKSPACES\\App\\Feature',
          path: 'C:\\FABRICA\\WORKSPACES\\App\\Feature',
          isMainWorktree: false
        }),
        knownFABRICALayouts: buildKnownFABRICAWorkspaceLayouts(settings, repo)
      })
...
describe('agent scratch worktrees', () => {
  ...
  it('classifies scratch worktrees created inside another linked checkout', () => {
    const repo = makeRepo()
    const settings = makeSettings()
    const linkedCheckoutPath = '/FABRICA/workspaces/app/feature-x'
    expect(
      classifyWorktreeOwnership({
        repo,
        settings,
  ...
  it('keeps strong FABRICA metadata authoritative over the scratch path match', () => {
    const repo = makeRepo()
    const settings = makeSettings()
    expect(
      classifyWorktreeOwnership({
  ...
  it('hides agent scratch even when the repo shows non-FABRICA worktrees', () => {
    for (const repo of [
