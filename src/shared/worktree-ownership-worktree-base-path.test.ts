...
import { buildKnownFABRICAWorkspaceLayouts, classifyWorktreeOwnership } from './worktree-ownership'
...
describe('repo-specific worktree ownership layouts', () => {
  it('resolves the same relative base path from each repo root', () => {
    ...
    expect(buildKnownFABRICAWorkspaceLayouts(settings, repoA)[0]).toEqual({
      path: '/projects/a/worktrees',
      nestWorkspaces: true
    })
    expect(buildKnownFABRICAWorkspaceLayouts(settings, repoB)[0]).toEqual({
    ...
    expect(
      classifyWorktreeOwnership({
        ...
        knownFABRICALayouts: buildKnownFABRICAWorkspaceLayouts(settings, repoA)
      })
    ).toBe('external')
    expect(
      classifyWorktreeOwnership({
        ...
        settings,
        worktree: makeWorktree('/projects/a/worktrees/repo/feature'),
        knownFABRICALayouts: buildKnownFABRICAWorkspaceLayouts(settings, repoB)
      })
    ).toBe('external')
  })
  ...
  it('includes relative global layouts for SSH repos without applying absolute desktop paths', () => {
    ...
    expect(buildKnownFABRICAWorkspaceLayouts(relativeSettings, repo)[0]).toEqual({
      path: '/remote/worktrees',
      nestWorkspaces: true
    })
    expect(
      buildKnownFABRICAWorkspaceLayouts(absoluteSettings, repo).some(
