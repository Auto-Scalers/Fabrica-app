...
export function parseAgentHookEndpointFile(contents: string): AgentHookEndpoint {
  ...
  if (
    !values.FABRICA_AGENT_HOOK_PORT ||
    !values.FABRICA_AGENT_HOOK_TOKEN ||
    !values.FABRICA_AGENT_HOOK_ENV ||
    !values.FABRICA_AGENT_HOOK_VERSION
  ) {
    throw new Error('Agent hook endpoint file is missing required fields')
  }
  return {
    port: values.FABRICA_AGENT_HOOK_PORT,
    token: values.FABRICA_AGENT_HOOK_TOKEN,
    env: values.FABRICA_AGENT_HOOK_ENV,
    version: values.FABRICA_AGENT_HOOK_VERSION
  }
}
