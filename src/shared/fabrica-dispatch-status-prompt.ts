// Why: full FABRICA dispatch preambles are multi-KB (CLI instructions before
...
export const FABRICA_DISPATCH_STATUS_PREAMBLE_PREFIX =
  'You are working inside FABRICA, a multi-agent IDE.'
export const FABRICA_DISPATCH_STATUS_TASK_MARKER = '=== TASK ==='
const FABRICA_DISPATCH_STATUS_TASK_ID_MARKER = 'Your task ID is:'
...
const FABRICA_DISPATCH_STATUS_SOURCE_SCAN_LIMIT = 24_576

export function isFABRICADispatchStatusPrompt(value: string): boolean {
  ...
  const scanEnd = Math.min(value.length, FABRICA_DISPATCH_STATUS_SOURCE_SCAN_LIMIT)
  let start = 0
  while (start < scanEnd && isEcmaTrimWhitespace(value.charCodeAt(start))) {
    start++
  }
  return (
    start + FABRICA_DISPATCH_STATUS_PREAMBLE_PREFIX.length <= scanEnd &&
    value.startsWith(FABRICA_DISPATCH_STATUS_PREAMBLE_PREFIX, start)
  )
}
...
export function compactDispatchPromptForStatus(
  ...
  let taskId = ''
  const idMarkerIndex = scan.indexOf(FABRICA_DISPATCH_STATUS_TASK_ID_MARKER)
  if (idMarkerIndex !== -1) {
    const afterId = scan.slice(idMarkerIndex + FABRICA_DISPATCH_STATUS_TASK_ID_MARKER.length)
    let idStart = 0
  ...
  if (taskMarkerIndex !== -1) {
    const body = scan.slice(taskMarkerIndex + FABRICA_DISPATCH_STATUS_TASK_MARKER.length)
  ...
  // Why: keep the dispatch prefix (isFABRICADispatchPrompt) + task id (label match)
  // + task body (fallback preview) so UI helpers still work on the 200-char field.
  let compact = FABRICA_DISPATCH_STATUS_PREAMBLE_PREFIX
  if (taskId) {
    compact += ` ${FABRICA_DISPATCH_STATUS_TASK_ID_MARKER} ${taskId}`
  }
  if (taskBody) {
    compact += ` ${FABRICA_DISPATCH_STATUS_TASK_MARKER} ${taskBody}`
  }
  return normalizeSingleLine(compact, maxLength)
}

/**
 * Locate the FABRICA task separator in a dispatch prompt scan window.
 * Why: base-drift commit subjects are repository-controlled and may mention
 * `=== TASK ===`. Raw multi-line preambles must use the standalone line FABRICA
 ...
 */
export function findFABRICADispatchTaskMarkerIndex(value: string): number {
  let searchFrom = 0
  while (searchFrom < value.length) {
    const markerIndex = value.indexOf(FABRICA_DISPATCH_STATUS_TASK_MARKER, searchFrom)
    if (markerIndex === -1) {
      break
    }
    const markerEnd = markerIndex + FABRICA_DISPATCH_STATUS_TASK_MARKER.length
  ...
  return value.includes('\n') || value.includes('\r')
    ? -1
    : value.indexOf(FABRICA_DISPATCH_STATUS_TASK_MARKER)
}
