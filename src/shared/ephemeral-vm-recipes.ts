...
const EphemeralVmRecipeFABRICAServerConnectionSchema = z
  .object({
    type: z.literal('FABRICA-server'),
...
export const EphemeralVmRecipeConnectionSchema = z.discriminatedUnion('type', [
  EphemeralVmRecipeFABRICAServerConnectionSchema,
...
export function parseEphemeralVmRecipeResult(stdout: string): EphemeralVmRecipeResultParseResult {
  ...
  if (connection.type === 'FABRICA-server' && !parsePairingCode(connection.pairingCode)) {
    return { ok: false, error: 'Recipe result pairingCode is not a valid FABRICA pairing code.' }
  }
...
export function getEphemeralVmRecipeResultPairingCode(
  ...
  return connection.type === 'FABRICA-server' ? connection.pairingCode : null
}
