...
describe('keybindings', () => {
  it('accepts bounded plugin command action IDs and rejects malformed variants', () => {
    expect(isKeybindingActionId('plugin:fabrica-samples.tasks/open')).toBe(true)
    expect(isKeybindingActionId('plugin:fabrica-samples.tasks/task.open-latest')).toBe(true)
    expect(isKeybindingActionId('plugin:tasks/open')).toBe(false)
    expect(isKeybindingActionId('plugin:fabrica-samples.tasks/../open')).toBe(false)
    expect(isKeybindingActionId(`plugin:fabrica-samples.tasks/${'a'.repeat(401)}`)).toBe(false)
  })
  ...
  it('binds close-all editor tabs to Mod+Alt+W beside tab.close', () => {
    ...
    expect(
      keybindingMatchesAction('tab.closeAll', linuxCloseAll, 'linux', undefined, {
        context: 'terminal',
        terminalShortcutPolicy: 'FABRICA-first'
      })
    ).toBe(true)
  ...
  it('keeps FABRICA-first terminal context backward compatible', () => {
    const ctrlP = {
      key: 'p',
      code: 'KeyP',
      control: true,
