...
const FABRICA_UNICODE_VERSION = 'FABRICA-11-zwj'
...
class FABRICAUnicodeProvider implements IUnicodeVersionProvider {
  public readonly version = FABRICA_UNICODE_VERSION
...
export function activateFABRICATerminalUnicodeProvider(terminal: XtermTerminalWithUnicodeCore): void {
  const { unicode } = terminal
  if (unicode.activeVersion === FABRICA_UNICODE_VERSION) {
    return
  }
  ...
  if (!unicode.versions.includes(FABRICA_UNICODE_VERSION)) {
    unicode.register(new FABRICAUnicodeProvider(baseProvider))
  }
  unicode.activeVersion = FABRICA_UNICODE_VERSION
}
