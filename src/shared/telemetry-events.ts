...
// Small set: only failures FABRICA's PTY-typed-command launch can observe (`binary_not_found` = shell ENOENT, `paste_readiness_timeout`, `unknown`).
// Provider-side errors (auth/rate-limit/network) happen inside the agent CLI subprocess and are invisible to FABRICA. See telemetry-plan.md §Defer per-incident error fields.
...
// Whitelist of settings emittable on `settings_changed`. `FABRICA_channel` (build-time, not user-togglable) is absent.
...
const appStarredFABRICASchema = z
  .object({
...
// instead of FABRICA's self-computed trusted_hash. `fallback`/`verify_failed`
...
const FABRICACliFeatureTipSourceSchema = z.enum(['app_open', 'manual'])
const FABRICACliFeatureTipShownSchema = z
  .object({
    source: FABRICACliFeatureTipSourceSchema,
    nth_repo_added: nthRepoAddedSchema
  })
  .strict()
const FABRICACliFeatureTipSetupClickedSchema = z
  .object({
    source: FABRICACliFeatureTipSourceSchema,
    nth_repo_added: nthRepoAddedSchema
  })
  .strict()
const FABRICACliFeatureTipSetupResultSchema = z
  .object({
    source: FABRICACliFeatureTipSourceSchema,
...
export const eventSchemas = {
  ...
  app_starred_FABRICA: appStarredFABRICASchema,
  ...
  FABRICA_cli_feature_tip_shown: FABRICACliFeatureTipShownSchema,
  FABRICA_cli_feature_tip_setup_clicked: FABRICACliFeatureTipSetupClickedSchema,
  FABRICA_cli_feature_tip_setup_result: FABRICACliFeatureTipSetupResultSchema,
...
type _CohortExtendedRoster =
  ...
  | 'agent_prompt_sent'
  | 'agent_error'
  | 'FABRICA_cli_feature_tip_shown'
  | 'FABRICA_cli_feature_tip_setup_clicked'
  | 'FABRICA_cli_feature_tip_setup_result'
...
export const commonPropsSchema = z
  .object({
    ...
    FABRICA_channel: z.enum(['stable', 'rc'])
  })
  .strict()
