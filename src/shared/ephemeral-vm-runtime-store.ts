...
const EPHEMERAL_VM_RUNTIMES_FILE = 'FABRICA-ephemeral-vm-runtimes.json'
...
function readEphemeralVmRuntimeStore(userDataPath: string): EphemeralVmRuntimeStore {
  ...
  try {
    ...
    throw new EphemeralVmRuntimeStoreError(
      'runtime_error',
      `Could not read FABRICA ephemeral VM runtimes at ${path}; the file is invalid.`
    )
  }
}
...
function writeEphemeralVmRuntimeStore(userDataPath: string, store: EphemeralVmRuntimeStore): void {
  ...
  try {
    ...
    if (error instanceof JsonStringifyByteLimitError) {
      throw new EphemeralVmRuntimeStoreError(
        'runtime_error',
        `Could not write FABRICA ephemeral VM runtimes at ${path}; the store exceeds its durable capacity.`
      )
    }
    throw error
  }
