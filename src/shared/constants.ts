...
function getDefaultWorkspaceDir(homeDir: string): string {
  ...
  return [trimmedHomeDir, 'Fabrica', 'workspaces'].join(separator)
}
...
export function getDefaultSettings(homedir: string): GlobalSettings {
  return {
    ...
    // Why: FABRICA-first keeps core shortcuts working from a focused terminal; TUI-ownership users opt in.
