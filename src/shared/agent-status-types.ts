...
export type AgentStateHistoryEntry = {
   ...
   *  or FABRICA's guarded fallback). Always falsy for non-`done` states so retention logic can preserve it. */
  interrupted?: boolean
}
...
export type AgentStatusEntry = {
   ...
   *  (agent-reported or FABRICA's guarded fallback). Undefined otherwise. */
  interrupted?: boolean
   ...
   *  Used only for exact CLI resume; FABRICA terminal ids are not agent-session ids. */
