/* eslint-disable max-lines -- Why: canonical transport-agnostic listener; parser, normalizer, per-CLI extractors, and endpoint writer share invariants that must not drift between FABRICA's main process and the relay. */
...
import { FABRICA_HOOK_PROTOCOL_VERSION } from './agent-hook-types'
...
/** Per-listener-instance caches needing per-PTY teardown; FABRICA's main process and the relay each get their own, never shared. */
...
export function warnOnHookEnvOrVersionMismatch(
  ...
  if (
    version &&
    version !== FABRICA_HOOK_PROTOCOL_VERSION &&
    !state.warnedVersions.has(version) &&
    state.warnedVersions.size < MAX_WARNED_KEYS
  ) {
    state.warnedVersions.add(version)
    console.warn(
      `[agent-hooks] received hook v${version}; server expects v${FABRICA_HOOK_PROTOCOL_VERSION}. ` +
    ...
    if (!state.warnedEnvs.has(key) && state.warnedEnvs.size < MAX_WARNED_KEYS) {
      ...
      console.warn(
        `[agent-hooks] received ${env} hook on ${expectedEnv} server. ` +
          'Likely a stale terminal from another FABRICA install.'
      )
    }
  }
}
...
/** Rebuild a pane's working roster from a persisted snapshot; live activity confirms a seed, a complete task inventory may reap an unconfirmed one whose finish hook arrived while FABRICA was offline. */
export function seedClaudeSubagentRosterFromSnapshots(
  ...
  for (const snapshot of snapshots) {
    ...
    roster.set(snapshot.id, {
      ...
      // Why: the seed can be a phantom (child finished while FABRICA was down, SubagentStop lost); let a PRESENT background_tasks list omitting the id remove it, not gate the pane 'working' forever.
 ...
 *  the pane any more (its PTY died while FABRICA was down, so no finish hook could
...
function buildClaudeCachedLeadStatusPayload(
  ...
  // Why: default 'working' — a spawn proves activity even before the lead's first state-bearing event (e.g. FABRICA restarted mid-session).
  const lead = state.claudeLeadStateByPaneKey.get(paneKey)
...
export function writeEndpointFile(
  ...
  const valuesToWrite: [string, string][] = [
    ['FABRICA_AGENT_HOOK_PORT', String(fields.port)],
    ['FABRICA_AGENT_HOOK_TOKEN', fields.token],
    ['FABRICA_AGENT_HOOK_ENV', fields.env],
    ['FABRICA_AGENT_HOOK_VERSION', fields.version]
  ]
