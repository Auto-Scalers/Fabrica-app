...
import { activateFABRICATerminalUnicodeProvider } from './terminal-unicode-provider'
 ...
 *  (pane-dom-creation.ts) and the FABRICA ZWJ provider (pane-lifecycle.ts).
...
export function createRendererParityTerminal(dims: { cols: number; rows: number }): ParityTerminal {
  ...
  activateFABRICATerminalUnicodeProvider(terminal)
 ...
 *    while FABRICA's WebGL glyph renderer skips decorations on null cells.
 ...
 *  FABRICARuntime.serializeHeadlessTerminalBuffer (normal buffer separated from
