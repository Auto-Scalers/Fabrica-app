...
describe('getGitCloneFailureMessage', () => {
  it('turns an existing destination into an actionable message after progress output', () => {
    expect(
      getGitCloneFailureMessage(
        [
          'Cloning into \u001b[32mFABRICA\u001b[0m...\r',
          "fatal: destination path 'FABRICA' already exists and is not an empty directory.\n"
        ].join(''),
        { clonePath: '/work/FABRICA' }
      )
    ).toBe(
      'Destination already exists and is not empty: /work/FABRICA. Choose a different parent folder, delete the existing folder, or add the existing repository instead.'
    )
  })
  ...
  it('prefers the last fatal line over a trailing fragment', () => {
    expect(
      ...
      'Destination already exists and is not empty: FABRICA. Choose a different parent folder, delete the existing folder, or add the existing repository instead.'
    )
  })
  ...
  it('uses the known clone path for relay destination fragments', () => {
    expect(
      getGitCloneFailureMessage('Clone failed: and the repository exists.', {
        clonePath: '/srv/FABRICA'
      })
    ).toBe(
      'Destination already exists and is not empty: /srv/FABRICA. Choose a different parent folder, delete the existing folder, or add the existing repository instead.'
    )
  })
