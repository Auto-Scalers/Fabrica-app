...
describe('wsl login shell command helpers', () => {
  ...
  it('runs commands through the distro user login shell', () => {
    ...
    expect(command).toContain('exec "$_FABRICA_wsl_shell" -ilc')
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      const root = mkdtempSync(join(tmpdir(), 'FABRICA-wsl-login-codex-'))
      ...
      writeFileSync(
        join(tools, 'getent'),
        `#!/bin/sh\nprintf '%s\\n' "user:x:1000:1000::/home/user:$FABRICA_TEST_LOGIN_SHELL"\n`
      )
      writeFileSync(
        loginShell,
        '#!/bin/sh\nexport PATH="$FABRICA_TEST_CODEX_BIN:/usr/bin:/bin"\nexec /bin/sh -c "$2"\n'
      )
      ...
      const run = (codexBin: string): string =>
        execFileSync('/bin/sh', ['-c', command], {
          ...
          env: {
            ...
            FABRICA_TEST_LOGIN_SHELL: loginShell,
            FABRICA_TEST_CODEX_BIN: codexBin
          }
        })
  ...
  it('preserves command-scoped environment variables through the outer WSL shell', () => {
    const command = buildWslLoginShellCommand('HISTFILE=/tmp/FABRICA-history printf "$HISTFILE"')
    const escaped = escapeWslShCommandForWindows(command)

    expect(command).toContain('\'HISTFILE=/tmp/FABRICA-history printf "$HISTFILE"\'')
    expect(escaped).toContain('\\$_FABRICA_wsl_shell')
  ...
  it('preserves user command variables across the Windows-to-WSL argv boundary', () => {
    ...
    const command = buildWslLoginShellCommand('FABRICA_value=ok; printf "<%s>" "$FABRICA_value"')
    const escaped = escapeWslShCommandForWindows(command)
  ...
  it('starts an interactive login shell without assuming bash', () => {
    ...
    expect(command).toContain('getent passwd')
    expect(command).toContain('if [ -z "$_FABRICA_wsl_shell" ] || [ ! -x "$_FABRICA_wsl_shell" ]; then')
    expect(command).toContain('_FABRICA_shell_ready_root=""')
    expect(command).toContain('if [ -n "${FABRICA_USER_DATA_PATH:-}" ]; then')
    expect(command).toContain('_FABRICA_wsl_shell_name=$(basename "$_FABRICA_wsl_shell"')
    expect(command).toContain('bash)')
    expect(command).toContain('--rcfile "${_FABRICA_shell_ready_root}/bash/rcfile"')
    expect(command).toContain('zsh)')
    expect(command).toContain('export ZDOTDIR="${_FABRICA_shell_ready_root}/zsh"')
    expect(command).toContain('exec "$_FABRICA_wsl_shell" -l')
    expectValidShSyntax(command)
  })
})
