...
const UNREACHABLE = 'Could not connect to the remote FABRICA runtime.'
...
describe('isTailscaleEndpoint', () => {
  ...
  it('rejects non-Tailscale hosts and the surrounding 100.x space', () => {
    ...
    expect(isTailscaleEndpoint('wss://FABRICA.example.com')).toBe(false)
...
describe('withRemoteRuntimeTailscaleHint', () => {
  ...
  it('leaves non-connectivity errors untouched', () => {
    const auth = 'Remote FABRICA runtime rejected the pairing token.'
