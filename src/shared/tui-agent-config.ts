...
import { getFABRICACliCommandNameForPlatform } from './fabrica-cli-command-name'
...
export const TUI_AGENT_CONFIG: Record<TuiAgent, TuiAgentConfig> = {
  claude: {
    ...
    // Why: `claude --prefill <text>` seeds the input without submitting, avoiding the paste-after-ready race (PR https://github.com/Auto-Scalers/Fabrica-app/pull/926).
    draftPromptFlag: '--prefill'
  },
  'claude-agent-teams': {
    // Why: an FABRICA-provided launch mode, not a separate binary; detection follows the FABRICA CLI.
    detectCmd: 'fabrica',
    detectCmdAliases: ['FABRICA', 'FABRICA-dev', 'FABRICA-ide'],
    // Why: require Claude too so fresh installs (FABRICA shim always present) don't report Agent Teams without an agent CLI.
    detectRequiredCommands: ['claude'],
    // Why: Windows/WSL use Claude's in-process Agent Teams fallback, not this FABRICA native-pane/tmux-shim wrapper.
    detectUnsupportedRuntimes: ['win32', 'wsl'],
    launchCmd: 'fabrica claude-teams',
    launchCmdByPlatform: {
      linux: `${getFABRICACliCommandNameForPlatform('linux')} claude-teams`,
      win32: `${getFABRICACliCommandNameForPlatform('win32')} claude-teams`
    },
    expectedProcess: 'claude',
  ...
  pi: {
    ...
    expectedProcess: 'pi',
    promptInjectionMode: 'argv',
    // Why: pi has no `--prefill` and paste-after-ready races its long startup; the FABRICA-prefill extension seeds this env var instead.
    draftPromptEnvVar: 'FABRICA_PI_PREFILL',
  ...
  omp: {
    ...
    expectedProcess: 'omp',
    promptInjectionMode: 'argv',
    draftPromptEnvVar: 'FABRICA_OMP_PREFILL'
  },
  'prime-agent': {
  ...
  hermes: {
    detectCmd: 'hermes',
    // Why: bare `hermes` opens the classic REPL; `--tui` starts the full-screen agent UI FABRICA hosts.
