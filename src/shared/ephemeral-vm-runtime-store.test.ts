...
describe('ephemeral VM runtime store', () => {
  ...
  function makeUserDataPath(): string {
    const userDataPath = mkdtempSync(join(tmpdir(), 'FABRICA-ephemeral-vm-store-'))
  ...
  it('updates lifecycle and cleanup state', () => {
    ...
    const failed = updateEphemeralVmRuntimeStatus(userDataPath, 'FABRICA-instance-1', {
    ...
    const recovered = updateEphemeralVmRuntimeStatus(userDataPath, 'FABRICA-instance-1', {
      status: 'cleaned',
