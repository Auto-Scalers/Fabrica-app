...
export const WSL_HOOK_RELAY_DIR = '.FABRICA-wsl/hook-relay'
 ...
 *  namespaces the guest install dir, so concurrent FABRICA instances with
 *  different bundle versions (dev + prod) never reinstall over each other. */
export const WSL_HOOK_RELAY_VERSION_ENV = 'FABRICA_WSL_HOOK_RELAY_VERSION'
 ...
 *  after an FABRICA restart — the exact re-coordination this exists to serve. */
export const WSL_HOOK_RELAY_INSTANCE_ENV = 'FABRICA_WSL_HOOK_INSTANCE'
...
export function wslHookRelayEndpointDir(guestHome: string, instanceKey: string): string {
  ...
  return `${home}/.FABRICA-wsl/agent-hooks/instance-${instanceKey}`
}
