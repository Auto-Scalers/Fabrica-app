...
export function buildAgentStartupPlan(args: {
  ...
  /** Why: SSH remotes deploy the CLI shim as plain `fabrica`, so the local
   * `fabrica` rename must be skipped for remote launches. */
  isRemote?: boolean
  ...
  if (config.promptInjectionMode === 'hermes-query') {
    ...
    return {
      agent,
      // Why: Hermes owns readiness and submission for `chat --query`; FABRICA
...
export function buildAgentResumeStartupPlan(args: {
  ...
  sessionOptions?: Record<string, SessionOptionValue>
  /** Why: see buildAgentStartupPlan — remote launches use the plain `FABRICA` shim. */
  isRemote?: boolean
}): AgentStartupPlan | null {
