...
export function buildKnownFABRICAWorkspaceLayouts(
  ...
  const layouts: FABRICAWorkspaceLayout[] = []
...
function appendWorkspaceLayouts(
  target: FABRICAWorkspaceLayout[],
  source: readonly FABRICAWorkspaceLayout[]
): void {
...
function buildWslWorkspaceLayouts(
  ...
  const root = `//wsl.localhost/${parsed.distro}${linuxHome}/FABRICA/workspaces`
...
export function classifyWorktreeOwnership(args: {
  ...
  meta?: WorktreeMeta
  settings: Pick<GlobalSettings, 'workspaceDir' | 'nestWorkspaces' | 'workspaceDirHistory'>
  knownFABRICALayouts: FABRICAWorkspaceLayout[]
  agentScratchWorktreePathMatcher?: AgentScratchWorktreePathMatcher
}): WorktreeOwnership {
  if (hasStrongFABRICAMetadata(args.meta)) {
    return 'FABRICA-managed'
  }
  ...
  if (isUnderFlatOrUntrustedFABRICARoot(args.worktree.path, args.knownFABRICALayouts)) {
    return 'unknown-legacy'
  }

  if (canClassifyAsExternal(args.worktree.path, args.knownFABRICALayouts)) {
    // Why: a plain `git worktree add` can target FABRICA's nested workspace
    // folder. Only metadata proves FABRICA created it.
    return 'external'
  }
...
export function shouldShowWorktree(args: {
  ...
  if (args.ownership === 'FABRICA-managed') {
    return true
  }
  if (
  ...
  // Why: agent scratch stays hidden even when the repo shows non-FABRICA
...
export function applyMetadataFallbackVisibility(detected: DetectedWorktree): DetectedWorktree {
  ...
  return {
    ...detected,
    visible: true,
    ownership: detected.ownership === 'FABRICA-managed' ? 'FABRICA-managed' : 'unknown-legacy'
  }
}
...
function hasStrongFABRICAMetadata(meta: WorktreeMeta | undefined): boolean {
  return Boolean(
    meta?.FABRICACreatedAt ||
    meta?.FABRICACreationWorkspaceLayout ||
    meta?.createdAt ||
...
function isUnderFlatOrUntrustedFABRICARoot(
  worktreePath: string,
  knownFABRICALayouts: FABRICAWorkspaceLayout[]
): boolean {
  for (const layout of knownFABRICALayouts) {
...
function canClassifyAsExternal(
  ...
  if (knownFABRICALayouts.length === 0) {
    return false
  }
  for (const layout of knownFABRICALayouts) {
    const relative = relativePathInsideRoot(layout.path, worktreePath)
