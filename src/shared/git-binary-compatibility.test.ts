...
const image = process.env.FABRICA_GIT_COMPAT_IMAGE
const binary = process.env.FABRICA_GIT_COMPAT_BINARY
const expectedVersion = process.env.FABRICA_GIT_COMPAT_VERSION
...
describeBinaryCompatibility('real Git binary compatibility', () => {
  ...
  beforeAll(async () => {
    repoPath = await mkdtemp(join(tmpdir(), 'FABRICA-git-binary-compat-'))
  ...
  it('recognizes worktree-list and rev-parse compatibility boundaries', async () => {
    ...
    // FABRICA's path-existence fallback (issue #8389).
  ...
  it('deregisters a worktree whose directory was renamed away', async () => {
    // FABRICA renames the checkout into a trash directory and then clears the registration, so every
