...
function createRunner(overrides: {
  ...
  const runGit = vi.fn(async (args: string[]) => {
    ...
    if (args[0] === 'remote' && args[1] === 'get-url') {
      return { stdout: overrides.upstreamUrl ?? 'git@github.com:Auto-Scalers/Fabrica-app.git\n' }
    }
    if (args[0] === 'remote') {
...
describe('syncForkDefaultBranch', () => {
  ...
  it('blocks when the upstream remote no longer matches the expected fork metadata', async () => {
    const { runGit, calls } = createRunner({
      upstreamUrl: 'git@github.com:someone-else/FABRICA.git\n'
    })

    await expect(
      syncForkDefaultBranch(runGit, {
        expectedUpstream: { owner: 'Auto-Scalers', repo: 'fabrica' }
      })
  ...
  it('blocks when a non-GitHub upstream remote has the expected owner and repo suffix', async () => {
    const { runGit, calls } = createRunner({
      upstreamUrl: 'ssh://evil.example.com/Auto-Scalers/Fabrica-app.git\n'
    })
