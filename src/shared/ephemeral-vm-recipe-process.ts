...
function buildRecipeEnv(
  ...
  return {
    ...process.env,
    ...env,
    FABRICA_VM_MODE: mode,
    FABRICA_VM_INSTANCE_ID: context.instanceId ?? '',
    FABRICA_RECIPE_ID: context.recipeId,
    FABRICA_PROJECT_ID: context.projectId ?? '',
    FABRICA_WORKSPACE_ID: context.workspaceId ?? '',
    FABRICA_WORKSPACE_NAME: context.workspaceName ?? '',
    FABRICA_REPO_PATH: context.repoPath,
    FABRICA_REPO_URL: context.repoUrl ?? '',
    FABRICA_REPO_BRANCH: context.branch ?? '',
    FABRICA_REPO_REF: context.ref ?? '',
    FABRICA_VERSION: context.FABRICAVersion ?? ''
  }
}
