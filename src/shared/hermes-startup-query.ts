...
const QUERY_PLACEHOLDER = '__FABRICA_HERMES_STARTUP_QUERY__'
const QUERY_ARG_PLACEHOLDER = `--query=${QUERY_PLACEHOLDER}`
const POSIX_QUERY_VARIABLE = '__FABRICA_hermes_startup_query'
const POWERSHELL_QUERY_VARIABLE = 'FABRICAHermesStartupQuery'
const POWERSHELL_NATIVE_QUERY_VARIABLE = 'FABRICAHermesNativeQuery'

export const FABRICA_HERMES_STARTUP_QUERY_ENV = 'FABRICA_HERMES_STARTUP_QUERY'
...
function stripFABRICAOwnedHermesArgs(args: readonly string[]): string[] {
...
function normalizeHermesArgv(
  ...
  return [
    ...
    ...stripFABRICAOwnedHermesArgs(baseArgsWithoutChat),
    ...stripFABRICAOwnedHermesArgs(configuredArgsWithoutChat),
    '--tui'
  ]
}
...
function buildQueryCommand(argv: string[], shell: AgentStartupShell): string {
  if (shell !== 'posix') {
    ...
    const script = `$${POWERSHELL_QUERY_VARIABLE} = $env:${FABRICA_HERMES_STARTUP_QUERY_ENV}; $${POWERSHELL_NATIVE_QUERY_VARIABLE} = $${POWERSHELL_QUERY_VARIABLE} -replace '(\\\\*)"', '$1$1\\"'; Remove-Item Env:${FABRICA_HERMES_STARTUP_QUERY_ENV} -ErrorAction SilentlyContinue; ${invocation}`
  ...
  const script = `${POSIX_QUERY_VARIABLE}="\${${FABRICA_HERMES_STARTUP_QUERY_ENV}}"; unset ${FABRICA_HERMES_STARTUP_QUERY_ENV}; eval "$(printf %b "${encodedInvocation}")"`
...
export function planHermesStartupQuery(args: {
  ...
  const env = { ...args.agentEnv, [FABRICA_HERMES_STARTUP_QUERY_ENV]: args.prompt }
