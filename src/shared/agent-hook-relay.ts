...
// the relay sends to FABRICA. Consumed by `src/relay/agent-hook-server.ts` (which
...
// between FABRICA's main process and the remote relay.
//
// Per the design doc:
// - The relay normalizes; FABRICA routes. The envelope's `payload` field has
//   already been through `normalizeHookPayload` (which calls
//   `parseAgentStatusPayload` → `normalizeAgentStatusObject`) on the relay
//   side. FABRICA's `ingestRemote` re-runs the canonical payload normalizer at
...
// - The wire `connectionId` is **always `null`**: a `connectionId` is FABRICA's
//   local handle on an `ssh2` connection, not a wire identity. FABRICA stamps the
...
//   POST body so FABRICA's warn-once protocol diagnostics still fire. The relay
...
// with the same value so FABRICA can attribute the event back to the right CLI.
...
/** Wire envelope for a single hook event flowing relay → FABRICA. */
export type AgentHookRelayEnvelope = {
  source: AgentHookSource
  paneKey: string
  /** Ephemeral FABRICA launch identity stamped into the PTY env for this process. */
  launchToken?: string
  tabId?: string
  worktreeId?: string
  /** Always `null` on the wire — relay does not know FABRICA's local connectionId. */
  connectionId: null
  /** Preserved from the relay-side normalized hook event so FABRICA can
  ...
  /** True when the relay is replaying its cache after FABRICA reconnects. */
  isReplay?: boolean
  ...
  env?: string
  /** Forwarded verbatim from the agent CLI POST body. Lets FABRICA's warn-once
   ...
   *  FABRICA's `ingestRemote` validates it again at the SSH trust boundary. */
...
/**
 * Re-attaches shed fields from FABRICA's cached payload for this pane, so a transport-level
...
/** JSON-RPC request method FABRICA issues after `--connect` reattach to ask the
...
/** JSON-RPC request method FABRICA issues at session-ready to ship the
...
export type AgentHookInstallManagedHooksParams = {
  /** SHA-256 fingerprint of the server key negotiated by FABRICA's SSH transport. */
  hostKeyFingerprint?: string
  /** Positively detected and enabled agents allowed to mutate remote config. */
  agents: readonly AgentHookTarget[]
}

/** Feature-flag env var. Read once at process start by FABRICA and the relay.
 *  Remote agent hooks ship as the default SSH behavior; set "0" to opt out. */
export const FABRICA_FEATURE_REMOTE_AGENT_HOOKS_ENV = 'FABRICA_FEATURE_REMOTE_AGENT_HOOKS' as const

export function isRemoteAgentHooksEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  const raw = env[FABRICA_FEATURE_REMOTE_AGENT_HOOKS_ENV]
  if (raw === undefined) {
    return true
  }
