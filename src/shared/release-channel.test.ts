...
describe('release channel', () => {
  ...
  it('keeps dev builds out of the main release repo, and apart from each other', () => {
    expect(getReleaseRepoForChannel('hourly')).toBe('Auto-Scalers/Fabrica-hourly')
    expect(getReleaseRepoForChannel('daily')).toBe('Auto-Scalers/Fabrica-daily')
    ...
    expect(getReleaseRepoForChannel('adhoc')).toBe('Auto-Scalers/Fabrica-adhoc')
  ...
  it('builds release-notes links against the repo that published the version', () => {
    ...
    expect(getReleaseNotesUrlForVersion('1.4.160-daily.202607281300')).toBe(
      'https://github.com/Auto-Scalers/Fabrica-daily/releases/tag/v1.4.160-daily.202607281300'
    )
    ...
    expect(getReleaseNotesUrlForVersion('1.4.160-adhoc.20260728140533')).toBe(
      'https://github.com/Auto-Scalers/Fabrica-adhoc/releases/tag/v1.4.160-adhoc.20260728140533'
    )
    expect(getReleaseNotesUrlForVersion(null)).toBe('https://github.com/Auto-Scalers/Fabrica-app/releases')
  })
  ...
  it('sorts consecutive hourly builds newest first', () => {
    const build = (version: string): ReleaseBuild => ({
      ...
      name: null,
      publishedAt: null,
      releaseUrl: `https://github.com/Auto-Scalers/Fabrica-hourly/releases/tag/v${version}`
    })
  ...
  it('sorts consecutive adhoc builds newest first', () => {
    const build = (version: string): ReleaseBuild => ({
      ...
      name: null,
      publishedAt: null,
      releaseUrl: `https://github.com/Auto-Scalers/Fabrica-adhoc/releases/tag/v${version}`
    })
    const sorted = sortReleaseBuildsNewestFirst([
