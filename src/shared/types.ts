...
export type Repo = {
  ...
  /** Controls FABRICA's fork-default-branch sync offer for repos with upstream metadata. */
  forkSyncMode?: ForkSyncMode
  /** Canonical identity for the repo remote FABRICA should use for provider-level grouping. */
  gitRemoteIdentity?: GitRemoteIdentity | null
  /** Controls whether worktrees FABRICA did not create appear in the sidebar. */
...
export type Worktree = {
  ...
  manualOrder?: number
  lastActivityAt: number
  /** Set once when FABRICA creates the worktree. Absent for worktrees discovered
  ...
  createdAt?: number
  /** Agent selected when FABRICA originally created the worktree. Used only to
  ...
  baseRef?: string
  /** Remote/branch FABRICA should publish review commits to when it created this worktree. */
...
/** Provenance for workspaces created through `FABRICA worktree create`. Absent on
...
export type CliWorkspaceProvenance = {
  kind: 'created-by-cli'
  createdAt: number
  /** FABRICA terminal the CLI ran inside, when the caller had one — distinguishes
...
export type GitPushTarget = {
  ...
  branchName: string
  remoteUrl?: string
  /** True when FABRICA added this remote while preparing a fork-PR worktree. */
...
export type WorktreeMeta = {
  ...
  manualOrder?: number
  lastActivityAt: number
  /** See {@link Worktree.createdAt}. Persisted to FABRICA-data.json. */
  createdAt?: number
  /** See {@link Worktree.createdWithAgent}. Persisted to FABRICA-data.json. */
  createdWithAgent?: TuiAgent
  ...
  /** Intended create base for stale-base probes. Persisted metadata, not UI drift state. */
  baseRef?: string
  /** True when FABRICA checked out a pre-existing local branch that delete must not prune. */
  preserveBranchOnDelete?: boolean
  /** See {@link Worktree.pushTarget}. Persisted so refreshed worktree lists keep the target. */
  pushTarget?: GitPushTarget
  /** Explicit marker stamped when FABRICA creates the worktree. */
  FABRICACreatedAt?: number
  FABRICACreationSource?: 'desktop' | 'runtime' | 'cli' | 'ssh'
  /** Workspace layout active when FABRICA created the worktree. */
  FABRICACreationWorkspaceLayout?: FABRICAWorkspaceLayout
  ...
  automationProvenance?: AutomationWorkspaceProvenance
  /** System-owned provenance for workspaces created via `FABRICA worktree create`. */
  cliProvenance?: CliWorkspaceProvenance
}

export type WorktreeOwnership = 'FABRICA-managed' | 'external' | 'unknown-legacy' | 'agent-scratch'
...
// existing persistence layer writes them to FABRICA-data.json automatically.
...
export type TerminalTab = {
   ...
   *  Why: agent CLIs overwrite the live title via OSC updates, but FABRICA still
  ...
  startupCwd?: string
  /** Why: the coding-harness agent FABRICA launched in this tab. Lets the tab bar
...
export type BrowserWorkspace = {
  id: string
  worktreeId: string
  /** Stable display label for the outer FABRICA tab ("Browser 1", "Browser 2", …).
  ...
  // FABRICA tab strip still renders one workspace entry. Mirror the active page's
...
export type GitHubWorkItemDetails = {
  // Why: main-process doesn't know FABRICA's Repo.id, so this inner item omits
...
export type FABRICAHooks = {
  ...
  defaultTabs?: FABRICADefaultTabTemplate[] // Terminal tabs to create once for a new worktree
  environmentRecipes?: FABRICAVmRecipe[] // Project-scoped per-workspace environment recipes
  environmentRecipeDiagnostics?: FABRICAVmRecipeDiagnostic[] // Non-fatal validation issues from environmentRecipes
  worktree?: FABRICAWorktreeDefaults // Project-scoped defaults applied when a worktree is created
}

export type FABRICAWorktreeDefaults = {
...
export type FABRICADefaultTabTemplate = {
  title?: string
  color?: string
  command?: string
}

export type FABRICAVmRecipe = {
  id: string
  name: string
  create: string
  description?: string
...
export type FABRICAVmRecipeDiagnostic = {
  index: number
  field?: string
  message: string
}
...
export type WorktreeDefaultTabsLaunch = {
  tabs: FABRICADefaultTabTemplate[]
  runCommands: boolean
}
 ...
 *  system-default (activeAccountId:null) Codex account. FABRICA reads this to
...
/** All AI coding agents FABRICA knows how to launch. Used for the agent picker in the new-workspace
 *  flow and for the default-agent setting. Extend this union as new agents are added. */
export type TuiAgent =
  | 'claude' // Claude Code
  | 'claude-agent-teams' // Claude Code Agent Teams via FABRICA native panes
...
export type GlobalSettings = {
  ...
  workspaceDirHistory?: FABRICAWorkspaceLayout[]
  ...
  /** When enabled, FABRICA renames a workspace's auto-generated creature branch to
  ...
  /** macOS: toggles the additive menu-bar entry (FABRICA survives last-window close); doesn't change Dock behavior. */
  ...
  /** Whether to show the FABRICA app name in the titlebar. */
  ...
  /** Only toggles the sidebar shortcut; FABRICA Mobile stays reachable from Settings. */
  ...
  /** FABRICA-first keeps app shortcuts from TUIs; terminal-first is opt-in to let shell/TUI bindings win. */
  ...
  /** Legacy keyboard-shortcut overrides; new writes go to ~/.FABRICA/keybindings.json, migrated once when present. */
  ...
  /** Why: durable main-owned pref so FABRICA can prepare shared ~/.codex before the renderer hydrates. */
   ...
   *  History-only: does not change which account/config/hooks FABRICA uses. */
  ...
  /** When true, FABRICA requests local awake assertions while hook-reported agents are working. */
...
export type FABRICAWorkspaceLayout = {
  path: string
  nestWorkspaces: boolean
}
...
export type WorktreeCardProperty =
  ...
  | 'pr'
  | 'automation'
  // Badge marking workspaces created through `FABRICA worktree create`.
  | 'cli'
  | 'comment'
  | 'ports'
...
export type TaskResumeState = {
  ...
  linearMode?: 'issues' | 'projects' | 'views' | 'in-FABRICA'
...
export type PersistedUIState = {
  ...
  /** Hide workspaces created through `FABRICA worktree create`. */
  ...
  /** Whether FABRICA already tried triggering the macOS notification permission dialog; prevents re-firing every launch. */
  ...
  /** Why: Windows-only. Set once on first hide to tray so the "FABRICA is still running" notice shows only once. */
  ...
  /** Once the user has starred FABRICA (any entry point), permanently suppress the nag. */
...
export type PersistedTrustedFABRICAHookEntry = {
  contentHash: string
  approvedAt: number
}

export type PersistedTrustedFABRICAHookRepo = {
  all?: {
    approvedAt: number
  }
  setup?: PersistedTrustedFABRICAHookEntry
  archive?: PersistedTrustedFABRICAHookEntry
  issueCommand?: PersistedTrustedFABRICAHookEntry
  vmRecipe?: PersistedTrustedFABRICAHookEntry
}

export type PersistedTrustedFABRICAHooks = Record<string, PersistedTrustedFABRICAHookRepo>
...
export type AppMemory = UsageValues & {
  ...
  other: UsageValues
  /** Oldest-first memory samples (bytes) for the whole FABRICA app; empty before the first snapshot. */
  history: number[]
}
