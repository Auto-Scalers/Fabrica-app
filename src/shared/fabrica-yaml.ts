...
import type {
  FABRICADefaultTabTemplate,
  FABRICAHooks,
  FABRICAVmRecipe,
  FABRICAVmRecipeDiagnostic
} from './types'
import {
  isFABRICAYamlFieldWithinLimit,
  isFABRICAYamlTextWithinLimit,
  MAX_FABRICA_YAML_ALIAS_COUNT,
  MAX_FABRICA_YAML_COLLECTION_ENTRIES
} from './fabrica-yaml-file-limit'
...
function asTrimmedString(value: unknown): string | undefined {
  if (typeof value !== 'string' || !isFABRICAYamlFieldWithinLimit(value)) {
    return undefined
  }
...
export const FABRICA_VM_RECIPE_ID_PATTERN = /^[a-z0-9][a-z0-9._-]{0,63}$/
export const FABRICA_VM_RECIPE_ID_RULE =
...
function normalizeDefaultTabs(value: unknown): FABRICADefaultTabTemplate[] {
  if (!Array.isArray(value) || value.length > MAX_FABRICA_YAML_COLLECTION_ENTRIES) {
    return []
  }
  ...
  return value
    ...
    .filter((entry): entry is FABRICADefaultTabTemplate => entry !== null)
}

type VmRecipeParseResult = {
  recipes: FABRICAVmRecipe[]
  diagnostics: FABRICAVmRecipeDiagnostic[]
}

function normalizeVmRecipes(value: unknown): VmRecipeParseResult {
  const diagnostics: FABRICAVmRecipeDiagnostic[] = []
  if (!Array.isArray(value)) {
    return { recipes: [], diagnostics }
  }
  if (value.length > MAX_FABRICA_YAML_COLLECTION_ENTRIES) {
    return {
      recipes: [],
      diagnostics: [
        {
          index: MAX_FABRICA_YAML_COLLECTION_ENTRIES,
          message: `At most ${MAX_FABRICA_YAML_COLLECTION_ENTRIES} environment recipes are supported.`
        }
      ]
    }
  }
  ...
  const recipes = value
    .map((entry, index) => {
      ...
      if (!FABRICA_VM_RECIPE_ID_PATTERN.test(id)) {
        diagnostics.push({
          index,
          field: 'id',
          message: `Invalid recipe id "${id}". ${FABRICA_VM_RECIPE_ID_RULE}`
        })
        return null
      }
      if (seenIds.has(id)) {
    ...
    .filter((entry): entry is FABRICAVmRecipe => entry !== null)
  return { recipes, diagnostics }
}

/**
 * Parse the supported project defaults from `FABRICA.yaml`.
 */
export function parseFABRICAYaml(content: string): FABRICAHooks | null {
  if (!isFABRICAYamlTextWithinLimit(content)) {
    return null
  }
  ...
  try {
    ...
    root = document.toJS({ maxAliasCount: MAX_FABRICA_YAML_ALIAS_COUNT })
  } catch {
    return null
  }
