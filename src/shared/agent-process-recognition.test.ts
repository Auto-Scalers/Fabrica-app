...
describe('agent process recognition', () => {
  ...
  it('does not recognize Trae headless one-shot commands as interactive agents', () => {
    ...
    // Why: past `--` nothing is a flag, so this is the interactive pane FABRICA itself launches.
  ...
  it('does not recognize Prime Agent non-interactive --mode runs as interactive agents', () => {
    ...
    // Why: `text` is the interactive TUI mode FABRICA hosts.
  ...
  it('recognizes only the agent subcommand of the generic FABRICA CLI', () => {
    expect(recognizeAgentProcessFromCommandLine('FABRICA claude-teams')).toEqual({
      agent: 'claude-agent-teams',
      processName: 'fabrica'
    })
    expect(recognizeAgentProcessFromCommandLine('FABRICA status')).toBeNull()
    expect(recognizeAgentProcessFromCommandLine('FABRICA-dev terminal list')).toBeNull()
    expect(recognizeAgentProcessFromCommandLine('node /usr/local/bin/FABRICA claude-teams')).toEqual({
      agent: 'claude-agent-teams',
      processName: 'fabrica'
    })
    expect(recognizeAgentProcessFromCommandLine('node /usr/local/bin/FABRICA status')).toBeNull()
  })
  ...
  it('does not classify prompt text as a wrapped agent command', () => {
    expect(
      recognizeAgentProcessFromCommandLine(
        'node /tmp/not-an-agent.js "compare opencode vs FABRICA in Gemini CLI"'
      )
    ).toBeNull()
