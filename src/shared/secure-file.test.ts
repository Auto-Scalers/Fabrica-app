...
describe('hardenSecurePath', () => {
  ...
  it('rewrites Windows ACLs through the system PowerShell path', () => {
    hardenSecurePath('C:\\Users\\me\\.FABRICA\\secret.json', {
      isDirectory: false,
      platform: 'win32'
    })
    ...
    expect(powershellArgs).toEqual(
      expect.arrayContaining([
        ...
        '-ExecutionPolicy',
        'Bypass',
        'C:\\Users\\me\\.FABRICA\\secret.json',
        'S-1-5-21-1000',
        '0'
      ])
    )
  ...
  it('adds inheritable rules when hardening a Windows directory', () => {
    hardenSecurePath('C:\\Users\\me\\.FABRICA', { isDirectory: true, platform: 'win32' })
  ...
  it('caches successful existing-file hardening within a process', () => {
    ...
    const userDataPath = mkdtempSync(join(tmpdir(), 'FABRICA-secure-file-'))
  ...
  it('LRU-evicts Windows directory hardening entries instead of retaining every path', () => {
    ...
    const root = mkdtempSync(join(tmpdir(), 'FABRICA-secure-file-'))
    tempDirs.push(root)
  ...
  it('does not re-harden the parent directory when its mtime changes between reads', async () => {
    ...
    // another part of FABRICA writes to the same directory (changing its mtime).
    hardenExistingSecureFile(targetPath)
    await waitForFileTimestampTick()
