...
describe('fingerprintPluginConsent', () => {
  it('drops malformed persisted consent identities and oversized fingerprints', () => {
    expect(
      normalizePluginConsents({
        ...
        'fabrica-samples.demo': 'sha256-valid',
        'fabrica-samples.large': 'x'.repeat(257)
      })
    ).toEqual({ 'fabrica-samples.demo': 'sha256-valid' })
  })
  ...
  it('changes when a panel-only plugin gains a trusted Node worker', () => {
    ...
    const lists = {
      pluginConsents: { 'fabrica-samples.demo': panelOnly },
      disabledPlugins: []
    }
    expect(getPluginActivationState('fabrica-samples.demo', withWorker, lists)).toBe('pending')
    expect(needsReconsent('fabrica-samples.demo', withWorker, lists)).toBe(true)
  })
  ...
  it('requires re-consent when instructional content bytes change', () => {
    ...
    expect(
      getPluginActivationState('fabrica-samples.recipes', second, {
        pluginConsents: { 'fabrica-samples.recipes': first },
        disabledPlugins: []
      })
    ).toBe('pending')
  })
...
describe('plugin install lockfile consent fingerprints', () => {
  ...
  it('reads the legacy capabilityHash field as a consent fingerprint', () => {
    const parsed = parsePluginLockfile({
      version: 1,
      plugins: { 'fabrica-samples.demo': persistedEntry }
    })

    expect(parsed.plugins['fabrica-samples.demo']?.consentFingerprint).toBe('sha256-legacy-name')
  })
  ...
  it('drops a lockfile whose record key disagrees with its embedded identity', () => {
    const parsed = parsePluginLockfile({
      version: 1,
      plugins: {
        'fabrica-samples.other': persistedEntry
      }
    })
  ...
  it('accepts SHA-256 Git object ids in lockfile provenance', () => {
    const parsed = parsePluginLockfile({
      version: 1,
      plugins: {
        'fabrica-samples.demo': { ...persistedEntry, resolvedCommit: 'a'.repeat(64) }
      }
    })

    expect(parsed.plugins['fabrica-samples.demo']?.resolvedCommit).toBe('a'.repeat(64))
  })
})
