...
describe('hello FABRICA plugin fixture', () => {
  it('uses an ESM entry that remains loadable outside a type-module package', async () => {
    const root = join(process.cwd(), 'examples', 'plugins', 'hello-FABRICA')
    const manifest = pluginManifestSchema.parse(
      JSON.parse(await readFile(join(root, 'fabrica-plugin.json'), 'utf8'))
    )
