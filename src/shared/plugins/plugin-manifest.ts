...
/**
 * Plugin manifest v1 (`fabrica-plugin.json` at the plugin root). The
 * `contributes` key names deliberately mirror common Electron-ecosystem
 * manifest conventions so future adapters stay cheap.
 *
 * Lives in `shared` so the desktop app, the headless `FABRICA serve` runtime,
...
const FABRICAEngineRangeSchema = z
  .string()
  .max(64)
...
export const pluginManifestSchema = z
  .object({
    ...
    engines: z.object({ fabrica: FABRICAEngineRangeSchema }),
...
export const PLUGIN_MANIFEST_FILENAME = 'fabrica-plugin.json'
...
export function satisfiesFabricaEngineRange(hostVersion: string, range: string): boolean {
