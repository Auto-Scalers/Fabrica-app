...
describe('Pi-compatible title detection', () => {
  ...
  it.each([
    ...
    ['\u280b π - tmp', 'omp', '\u280b OMP'],
    ['\u280b OMP', 'pi', '\u280b Pi'],
    ['lucky-echidna | \u283c π - Diagnose FABRICA terminal title flicker - test', 'omp', '\u280b OMP'],
...
describe('Cursor agent title identity', () => {
  // Why: the accepted vocabulary is the set of labels FABRICA actually synthesizes for Cursor.
  ...
  it('accepts every label FABRICA synthesizes for Cursor', () => {
  ...
  ])('accepts the native or FABRICA-synthesized Cursor title %j', (title) => {
  ...
  it.each([
    ...
    'Terminal Cursor and FABRICA slows down',
    'cursor-agent',
    'cursor.exe',
    '~/cursor-rules',
    '',
