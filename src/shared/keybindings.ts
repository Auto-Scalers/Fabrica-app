...
export type TerminalShortcutPolicy = 'FABRICA-first' | 'terminal-first'
...
export const KEYBINDING_DEFINITIONS: readonly KeybindingDefinition[] = [
  ...
  },
  {
    ...
    // Why: macOS uses Shift+Space as an input-source shortcut; FABRICA otherwise rejects Shift-only bindings to avoid stealing typed text.
...
export function normalizeTerminalShortcutPolicy(
  ...
  return policy === 'terminal-first' ? 'terminal-first' : 'FABRICA-first'
}
...
export function keybindingIsActiveInContext(
  ...
  // Why: FABRICA-first keeps app shortcuts inside terminals; terminal-first is the escape hatch for shells and TUIs.
  if (normalizeTerminalShortcutPolicy(options.terminalShortcutPolicy) === 'FABRICA-first') {
    return true
  }
