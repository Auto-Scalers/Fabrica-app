...
describe('reviewHeadRemoteRefComponent', () => {
  ...
  it('builds provider refs under the FABRICA namespace', () => {
    const component = reviewHeadRemoteRefComponent('origin', 'git@github.com:org/repo.git')
    expect(githubPullRequestHeadLocalRef(component, 42)).toBe(`refs/FABRICA/pull/${component}/42`)
    expect(gitlabMergeRequestHeadLocalRef(component, 77)).toBe(
      `refs/FABRICA/merge-requests/${component}/77`
    )
  })
})
