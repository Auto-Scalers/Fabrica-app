...
const HERMES_PLUGIN_NAME = 'FABRICA-status'
const HERMES_PLUGIN_MARKER = 'Managed by FABRICA. Do not edit; changes may be overwritten.'
...
function enablePlugin(config: HermesConfig): HermesConfig {
  ...
  if (disabled === null) {
    ...
    // so FABRICA's install status matches what the real Hermes loader will do.
    plugins.disabled = []
...
function getPluginFilesState(pluginDir = getPluginDir()): {
  ...
  try {
    ...
    return {
      present: true,
      managed,
      detail: managed ? null : 'Hermes FABRICA-status plugin exists but is not FABRICA-managed'
    }
  } catch (error) {
    return {
      present: true,
...
function buildStatus(configPath: string, config: HermesConfig): AgentHookInstallStatus {
  ...
  const details = [
    pluginFiles.detail,
    enablement.detail,
    !enablement.enabled ? 'FABRICA-status is not enabled in Hermes config.yaml' : null,
    enablement.disabled ? 'FABRICA-status is disabled in Hermes config.yaml' : null
...
function getPluginManifest(): string {
  return [
    ...
    'version: 1.0.0',
    'description: "Reports Hermes Agent lifecycle events to Fabrica."',
    'author: "FABRICA"',
    'kind: standalone',
    'provides_hooks:',
...
def _endpoint_env() -> dict[str, str]:
    env = dict(os.environ)
    endpoint = env.get("FABRICA_AGENT_HOOK_ENDPOINT", "")
...
def _post_to_FABRICA(payload: dict[str, Any]) -> None:
    env = _endpoint_env()
    port = env.get("FABRICA_AGENT_HOOK_PORT", "")
    token = env.get("FABRICA_AGENT_HOOK_TOKEN", "")
    pane_key = env.get("FABRICA_PANE_KEY", "")
    if not port or not token or not pane_key:
        return
    body = {
        "paneKey": pane_key,
        "launchToken": env.get("FABRICA_AGENT_LAUNCH_TOKEN", ""),
        "tabId": env.get("FABRICA_TAB_ID", ""),
        "worktreeId": env.get("FABRICA_WORKTREE_ID", ""),
        "env": env.get("FABRICA_AGENT_HOOK_ENV", ""),
        "version": env.get("FABRICA_AGENT_HOOK_VERSION", ""),
        "payload": payload,
    }
...
def _make_hook(event_name: str) -> Callable[..., None]:
    def _hook(**kwargs: Any) -> None:
        _post_to_FABRICA(_payload_for_event(event_name, kwargs))
