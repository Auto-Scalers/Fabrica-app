...
const DISTRO = process.env.FABRICA_WSL_TEST_DISTRO ?? 'Ubuntu-24.04'
const runRealWsl = process.platform === 'win32' && process.env.FABRICA_REAL_WSL_DELETE_TEST === '1'
...
async function wsl(command: string, ...args: string[]): Promise<string> {
  const result = await execFileAsync(
    'wsl.exe',
    ['-d', DISTRO, '--exec', 'sh', '-c', command, 'FABRICA-wsl-test', ...args],
...
describe.skipIf(!runRealWsl)('WSL approved-root traversal race', () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await wsl("mktemp -d -p /tmp 'FABRICA-wsl-root-race.XXXXXX'")
    const statHook = String.raw`#!/bin/sh
for argument do last=$argument; done
if [ "$PWD" = "$FABRICA_RACE_PARENT" ] && [ "$(/usr/bin/basename "$last")" = vault ]; then
  inspected=$(/usr/bin/stat "$@") || exit $?
  /usr/bin/mv -- "$FABRICA_RACE_PARENT/vault" "$FABRICA_RACE_PARENT/vault-original" || exit $?
  /usr/bin/ln -s -- "$FABRICA_RACE_OUTSIDE" "$FABRICA_RACE_PARENT/vault" || exit $?
  printf '%s\n' "$inspected"
  exit 0
fi
exec /usr/bin/stat "$@"
`
  ...
  afterAll(async () => {
    if (/^\/tmp\/FABRICA-wsl-root-race\.[A-Za-z0-9]+$/u.test(fixtureRoot)) {
    ...
    await expect(
      execFileAsync(
        ...
        [
          ...
          `FABRICA_RACE_PARENT=${fixtureRoot}/race`,
          `FABRICA_RACE_OUTSIDE=${fixtureRoot}/outside/root-target`,
          ...(command ?? [])
        ],
        { encoding: 'utf-8', timeout: 30000 }
      )
    ).rejects.toMatchObject({ stderr: expect.stringContaining('FABRICA_WSL_DELETE_REJECT:race') })
