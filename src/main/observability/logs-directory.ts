...
// macOS convention is `~/Library/Application Support/Fabrica/logs/`; Windows and
...
function getUserDataDir(): string {
  try {
    ...
    const home = homedir()
    if (platform() === 'darwin') {
      return join(home, 'Library', 'Application Support', 'Fabrica')
    }
    if (platform() === 'win32') {
      return join(process.env.APPDATA ?? home, 'Fabrica')
    }
    return join(home, '.config', 'Fabrica')
  }
}
