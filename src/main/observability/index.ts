...
//   FABRICA_TELEMETRY_DISABLED=1 → identical to DO_NOT_TRACK for this lane.
//   FABRICA_DIAGNOSTICS_DISABLED=1 → ALSO disable local file writes. The escape
...
export function resolveObservabilityConsent(): ObservabilityConsent {
  ...
  const FABRICADisabled = envOn('FABRICA_TELEMETRY_DISABLED')
  const diagnosticsDisabled = envOn('FABRICA_DIAGNOSTICS_DISABLED')
  const ci = inCI()
  ...
  if (diagnosticsDisabled) {
    return {
      localFileEnabled: false,
      bundleEnabled: false,
      disabledReason: 'FABRICA_diagnostics_disabled'
    }
  }
  if (dnt || FABRICADisabled) {
    // Local file remains active — DNT is a *network* signal, and the local
    // file never leaves the machine.
    return {
      localFileEnabled: true,
      bundleEnabled: false,
      disabledReason: dnt ? 'do_not_track' : 'FABRICA_telemetry_disabled'
    }
  }
...
export function initObservability(): ObservabilityConsent {
  ...
  if (!c.localFileEnabled) {
    // Disabled at the CI / FABRICA_DIAGNOSTICS_DISABLED level — leave the
 ...
 *  `platform` / `arch` / `osRelease` / `FABRICAChannel` inputs come from main
...
export function collectDiagnosticBundle(
  meta: Pick<
    ...
    'appVersion' | 'platform' | 'arch' | 'osRelease' | 'FABRICAChannel' | 'lookbackMinutes'
  >
): CollectedBundle {
