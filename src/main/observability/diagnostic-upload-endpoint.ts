...
export function resolveDiagnosticBuildTokenEndpoint(): string | null {
  const endpoint =
    typeof FABRICA_DIAGNOSTICS_TOKEN_URL !== 'undefined'
      ? FABRICA_DIAGNOSTICS_TOKEN_URL
      : ((globalThis as { FABRICA_DIAGNOSTICS_TOKEN_URL?: string | null })
          .FABRICA_DIAGNOSTICS_TOKEN_URL ?? null)
...
export function resolveDiagnosticTokenEndpoint(): string | null {
  ...
  // cannot redirect uploads that the UI labels as going to FABRICA support.
  if (resolveDiagnosticBuildIdentity()) {
    return buildEndpoint
  }
  const fromEnv = process.env.FABRICA_DIAGNOSTICS_TOKEN_URL
...
export function resolveDiagnosticFABRICAChannel(): 'stable' | 'rc' | 'dev' {
