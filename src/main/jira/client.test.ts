...
function tokenPathForSite(siteId: string): string {
  return join(tempHome, '.FABRICA', 'jira-tokens', `${Buffer.from(siteId).toString('base64url')}.enc`)
}

function writeJiraFiles(siteId: string, token: string | Buffer): void {
  const FABRICADir = join(tempHome, '.FABRICA')
  mkdirSync(join(FABRICADir, 'jira-tokens'), { recursive: true })
  writeFileSync(
    join(FABRICADir, 'jira-sites.json'),
    JSON.stringify(
      {
        version: 1,
        activeSiteId: siteId,
...
beforeEach(() => {
  tempHome = mkdtempLike('FABRICA-jira-client-')
...
describe('Jira client credential storage', () => {
  ...
  it('keeps distinct self-hosted PAT accounts on one host as separate sites', async () => {
    ...
    const stored = JSON.parse(
      readFileSync(join(tempHome, '.FABRICA', 'jira-sites.json'), 'utf-8')
    ) as {
  ...
  it('bridges proxy environment settings before Jira connect requests', async () => {
    ...
    expect(netFetchMock).toHaveBeenCalledTimes(1)
    const headers = netFetchMock.mock.calls[0]?.[1]?.headers as Headers
    expect(headers.get('User-Agent')).toBe('FABRICA')
    expect(fetchMock).not.toHaveBeenCalled()
  })
})
