...
async function getComputerUsePermissionStatusAsync(): Promise<ComputerUsePermissionStatusResult> {
  ...
  if (!helperAppPath) {
    return createUnavailablePermissionStatus('Fabrica Computer Use.app was not found', null)
  }

  const executablePath = resolveMacOSComputerUseExecutablePath()
  if (!executablePath) {
    return createUnavailablePermissionStatus(
      `${helperAppPath}/Contents/MacOS/FABRICA-computer-use-macos was not found`,
      helperAppPath
    )
  }
...
async function readPermissionStatusFromHelperApp(
  ...
  const tempDir = await mkdtemp(join(tmpdir(), 'FABRICA-computer-use-permissions-'))
