...
const permissionStatusTempDir = '/tmp/FABRICA-computer-use-permissions-test'
const helperAppPath = '/Applications/Fabrica Computer Use.app'
...
describe('openComputerUsePermissions', () => {
  ...
  beforeEach(() => {
    ...
    resolveHelperExecutablePathMock.mockReturnValue(
      '/Applications/Fabrica Computer Use.app/Contents/MacOS/FABRICA-computer-use-macos'
    )
  ...
  it('does not launch the setup helper when all permissions are granted', async () => {
    resolveHelperAppPathMock.mockReturnValue('/Applications/Fabrica Computer Use.app')
    ...
    expect(spawn).not.toHaveBeenCalledWith(
      '/usr/bin/open',
      ['-n', '/Applications/Fabrica Computer Use.app', '--args', '--permissions'],
  ...
  it('launches the helper app in permissions mode', async () => {
    ...
    await expect(openComputerUsePermissions()).resolves.toEqual({
      ...
      nextStep: 'Grant Screen Recording to Fabrica Computer Use, then retry get-app-state.'
    })
    expect(spawnSync).toHaveBeenCalledWith(
      '/usr/bin/pkill',
      ['-f', 'FABRICA-computer-use-macos[[:space:]]+--permission([[:space:]]|$)'],
      { stdio: 'ignore' }
    )
    expect(spawnSync).toHaveBeenCalledWith(
      '/usr/bin/pkill',
      ['-f', 'FABRICA-computer-use-macos[[:space:]]+--permissions([[:space:]]|$)'],
      { stdio: 'ignore' }
    )
  ...
  it('launches a targeted permission helper flow', async () => {
    ...
    await expect(openComputerUsePermissions('accessibility')).resolves.toEqual({
      ...
      nextStep: 'Grant Accessibility to Fabrica Computer Use, then retry get-app-state.'
    })
    expect(spawn).toHaveBeenCalledWith(
      '/usr/bin/open',
      ['-n', '/Applications/Fabrica Computer Use.app', '--args', '--permission', 'accessibility'],
      { detached: true, stdio: 'ignore' }
    )
  })
  ...
  it('throws when the helper app is missing on macOS', async () => {
    ...
    await expect(openComputerUsePermissions()).rejects.toThrow(
      'Fabrica Computer Use.app was not found'
    )
  })
  ...
  it('throws when the helper executable is missing during setup', async () => {
    ...
    await expect(openComputerUsePermissions('accessibility')).rejects.toThrow(
      '/Applications/Fabrica Computer Use.app/Contents/MacOS/FABRICA-computer-use-macos was not found'
    )
  })
  ...
  it('resets stale macOS TCC grants for the helper bundle id', async () => {
    ...
    vi.mocked(execFileSync).mockReturnValueOnce('com.example.FABRICA.computer-use\n')
    ...
    await expect(resetComputerUsePermissions()).resolves.toEqual({
      ...
      bundleId: 'com.example.FABRICA.computer-use',
      permissions: [
        { id: 'accessibility', status: 'not-granted' },
        { id: 'screenshots', status: 'not-granted' }
      ]
    ...
    expect(spawnSync).toHaveBeenCalledWith(
      '/usr/bin/tccutil',
      ['reset', 'Accessibility', 'com.example.FABRICA.computer-use'],
      { encoding: 'utf8', stdio: ['ignore', 'pipe', 'pipe'] }
    )
    expect(spawnSync).toHaveBeenCalledWith(
      '/usr/bin/tccutil',
      ['reset', 'ScreenCapture', 'com.example.FABRICA.computer-use'],
      { encoding: 'utf8', stdio: ['ignore', 'pipe', 'pipe'] }
    )
  })
})
