...
const WSL_RATE_LIMIT_PTY_CWD_DIR = 'FABRICA-rate-limit-pty-cwd'
...
function resolveUserDataRoot(userDataPath?: string | null): string {
  const root = userDataPath?.trim() || process.env.FABRICA_USER_DATA_PATH?.trim()
  if (root && !isRootLikePath(root)) {
    return root
  }
  return join(tmpdir(), 'FABRICA-rate-limit-pty')
}
...
export function getHiddenRateLimitWslCwdSetupCommands(): string[] {
  return [
    `FABRICA_rate_limit_cwd="\${TMPDIR:-/tmp}/${WSL_RATE_LIMIT_PTY_CWD_DIR}"`,
    'mkdir -p "$FABRICA_rate_limit_cwd"',
    'cd "$FABRICA_rate_limit_cwd"'
  ]
}
