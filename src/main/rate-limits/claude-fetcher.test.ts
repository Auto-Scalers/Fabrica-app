...
describe('fetchClaudeRateLimits', () => {
  ...
  beforeEach(() => {
    ...
    appGetPathMock.mockReturnValue('/tmp/FABRICA-claude-fetcher-test')
  ...
  it('does not read inactive managed credentials from unowned auth paths', async () => {
    setPlatform('linux')
    tempDir = mkdtempSync(join(tmpdir(), 'FABRICA-claude-fetcher-'))
    ...
    writeFileSync(join(unownedAuthPath, '.FABRICA-managed-claude-auth'), 'account-1\n', 'utf-8')
    writeFileSync(
  ...
  it('supplements inactive managed account OAuth usage with Fable from its usage panel', async () => {
    ...
    writeFileSync(join(ownedAuthPath, '.FABRICA-managed-claude-auth'), 'account-1\n', 'utf-8')
