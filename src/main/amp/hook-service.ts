...
const AMP_PLUGIN_FILE = 'FABRICA-agent-status.ts'
const AMP_PLUGIN_MARKER = 'Managed by FABRICA. Do not edit; changes may be overwritten.'
  ...
  switch (state.kind) {
    ...
    case 'unmanaged':
      return {
        ...
        configPath: pluginPath,
        managedHooksPresent: false,
        detail: 'Amp Fabrica status plugin exists but is not Fabrica-managed'
      }
    case 'error':
      return {
        agent: 'amp',
...
function getAmpPluginSource(): string {
  return [
    ...
    '  const endpointPath = process.env.FABRICA_AGENT_HOOK_ENDPOINT',
    ...
    "      if (match[1] === 'FABRICA_AGENT_HOOK_PORT') out.port = value",
    "      if (match[1] === 'FABRICA_AGENT_HOOK_TOKEN') out.token = value",
    "      if (match[1] === 'FABRICA_AGENT_HOOK_ENV') out.env = value",
    "      if (match[1] === 'FABRICA_AGENT_HOOK_VERSION') out.version = value",
    '    }',
    ...
    "      console.warn('[FABRICA-hook] failed to parse Amp endpoint file:', (error as Error).message)",
    '    }',
    '    return null',
    '  }',
    '}',
    ...
    '  // Why: Amp sessions can outlive an FABRICA restart; the endpoint file is',
    '  // rewritten on each start, so read it per event before falling back to env.',
    '  const fileEnv = readEndpointFile() ?? {}',
    '  return {',
    '    port: fileEnv.port || process.env.FABRICA_AGENT_HOOK_PORT,',
    '    token: fileEnv.token || process.env.FABRICA_AGENT_HOOK_TOKEN,',
    '    env: fileEnv.env || process.env.FABRICA_AGENT_HOOK_ENV || "",',
    '    version: fileEnv.version || process.env.FABRICA_AGENT_HOOK_VERSION || ""',
    '  }',
    '}',
    '',
    ...
    '        "X-FABRICA-Agent-Hook-Token": coords.token',
    '      },',
    '      body: JSON.stringify({',
    '        paneKey,',
    '        launchToken: process.env.FABRICA_AGENT_LAUNCH_TOKEN || "",',
    ...
    '    })',
    '  } catch {',
    '    // Why: FABRICA status reporting must never affect the Amp run.',
    '  } finally {',
    ...
    '  // payload closures when FABRICA is down and each POST waits for timeout.',
