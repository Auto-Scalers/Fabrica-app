...
import { runWithFreshFABRICACloudSession } from '../fabrica-profiles/profile-cloud-session-refresh'
import {
...
import type { ActiveFABRICAProfileState } from '../fabrica-profiles/profile-index-store'
...
function authContext(
  ...
  return {
    ...
    scope,
    assertCurrent: () => {
      const current = ensureActiveFABRICAProfile(userDataPath)
      const cloudCurrent =
        !expectedCloud ||
      ...
      if (
        ...
        throw new Error(
          'The signed-in Fabrica account changed while the artifact request was running.'
        )
      }
    }
  }
...
function storedSessionAuthContext(
  ...
  if (!active.profile.cloud) {
    throw new Error('The active Fabrica profile is not linked to a cloud account.')
  }
  return authContext(
    active,
    {
...
export class ArtifactCloudService {
  ...
  async update(request: ArtifactWriteRequest): Promise<ArtifactCloudOperation<ArtifactListItem>> {
    ...
    return this.withAuth(request, (token, apiUrl, auth) =>
      this.publisher.runForSource(request.sourceKey, auth, async () => {
        ...
        if (!record) {
          throw new Error('This file has not been shared from the active Fabrica profile.')
        }
  ...
  private async withAuth<T>(
    ...
    const active = ensureActiveFABRICAProfile(this.userDataPath)
    ...
    const config = getFABRICACloudAuthConfig()
    if (!config.configured) {
      return { status: 'unconfigured', message: config.setupMessage }
    }
    const result = await runWithFreshFABRICACloudSession(
      config.config,
      active,
      this.userDataPath,
      async (session) => {
