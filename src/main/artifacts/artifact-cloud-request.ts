...
import { FABRICACloudRequestError } from '../fabrica-profiles/profile-cloud-client'
...
export async function artifactRequest<T>(
  ...
  const response = await fetch(`${apiUrl}/v1/artifacts${path}`, {
    ...
    headers: {
      ...
      ...(options.editToken ? { 'x-FABRICA-edit-token': options.editToken } : {}),
  ...
  if (!response.ok) {
    ...
    throw new FABRICACloudRequestError(response.status, body?.code)
  }
