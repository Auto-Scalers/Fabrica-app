...
import type { FABRICAProfileCloudSummary } from '../../shared/fabrica-profiles'
...
} from '../fabrica-profiles/profile-cloud-index'
import {
  cloudSessionIdentity,
  recordSuccessfulCloudSessionLogin,
  tombstoneCloudSession
} from '../fabrica-profiles/profile-cloud-session-mutation'
import { saveFABRICACloudSession } from '../fabrica-profiles/profile-cloud-session-store'
import {
...
const cloudA: FABRICAProfileCloudSummary = {
  cloudProfileId: 'cloud-a',
  userId: 'user-a',
  email: 'a@example.com',
  linkedAt: 1
}
const cloudB: FABRICAProfileCloudSummary = {
...
async function setup(sharingEnabled: { value: boolean } = { value: true }): Promise<{
  ...
  const userDataPath = await mkdtemp(join(tmpdir(), 'FABRICA-artifact-service-'))
  createdPaths.push(userDataPath)
  const active = ensureActiveFABRICAProfile(userDataPath)
  linkFABRICAProfileToCloud(active.profile.id, cloudA, userDataPath)
...
describe('ArtifactCloudService record authorization', () => {
  ...
  it('keeps the idempotency key stable across an auth-refresh retry', async () => {
    ...
    vi.stubEnv('FABRICA_CLOUD_API_URL', 'http://localhost:4100')
    vi.stubEnv('FABRICA_CLOUD_CLIENT_ID', 'desktop-client')
    saveFABRICACloudSession(profileId, userDataPath, {
  ...
  it('refuses account B update and unshare after account A signs out', async () => {
    ...
    unlinkFABRICAProfileFromCloud(profileId, userDataPath)
    linkFABRICAProfileToCloud(profileId, cloudB, userDataPath)
