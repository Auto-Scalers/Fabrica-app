...
const { workerState, appMock } = vi.hoisted(() => ({
  ...
  appMock: {
    isPackaged: true,
    getAppPath: vi.fn(() => '/apps/FABRICA/app.asar'),
    on: vi.fn()
  }
}))
...
describe('installMainThreadHangWatchdog', () => {
  beforeEach(() => {
    ...
    delete process.env.FABRICA_HANG_WATCHDOG_FORCE
    delete process.env.FABRICA_HANG_WATCHDOG_TIMEOUT_MS
    delete process.env.FABRICA_HANG_WATCHDOG_CHECK_INTERVAL_MS
  })
  ...
  it('is a no-op in unpackaged builds unless forced', () => {
    ...
    process.env.FABRICA_HANG_WATCHDOG_FORCE = '1'
  ...
  it('starts a worker with pid, marker, and timing config', () => {
    ...
    expect(workerPath).toBe(
      join('/apps/FABRICA/app.asar', 'out', 'main', 'main-thread-hang-watchdog-entry.js')
    )
    expect(options.name).toBe('FABRICA-main-thread-hang-watchdog')
  ...
  it('passes test timing overrides to the worker', () => {
    process.env.FABRICA_HANG_WATCHDOG_TIMEOUT_MS = '900'
    process.env.FABRICA_HANG_WATCHDOG_CHECK_INTERVAL_MS = '100'
