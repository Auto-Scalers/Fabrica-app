...
function getManagedScript(
  ...
  if (target === 'local' && process.platform === 'win32') {
    return [
      '@echo off',
      'setlocal',
      // Why: refresh endpoint coordinates for PTYs surviving an FABRICA restart.
      ...
      // and outside an FABRICA pane the caller can abandon stdin, so more.com never returns.
      ...buildWindowsHookEnvironmentGuardLines(),
      ...(options.skipWhenDevinImportsClaude
        ? [
            // Why: Devin imports .claude hooks by default; skip FABRICA's managed hook there so status posts stay attributed to Devin.
  ...
  return [
    ...
    'printf \'%s\' "$payload" | curl -sS -X POST "http://127.0.0.1:${FABRICA_AGENT_HOOK_PORT}/hook/claude" \\',
...
export class ClaudeHookService {
  ...
  remove(): AgentHookInstallStatus {
    ...
    if (this.options.agent === 'claude') {
      try {
        // Why: an FABRICA-level uninstall resets the opt-out memory so a later re-enable installs the statusline again.
