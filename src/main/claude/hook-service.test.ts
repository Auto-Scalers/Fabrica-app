...
describe('ClaudeHookService.install', () => {
  it('installs managed hooks into Claude settings and preserves user Bedrock settings', () => {
    const tmpHome = mkdtempSync(join(tmpdir(), 'FABRICA-claude-hooks-'))
    ...
    try {
            ...
            Stop: [
              ...
              },
              {
                hooks: [
                  {
                    type: 'command',
                    command: '/Users/old/.FABRICA/agent-hooks/claude-hook.sh'
                  }
                ]
              }
            ]
      ...
      expect(
        legacyCommands.some((command: string) =>
          command.includes('/Users/old/.FABRICA/agent-hooks/claude-hook.sh')
        )
      ).toBe(false)
      expect(isClaudeManagedCommand(legacy.hooks.StopFailure[0].hooks[0].command)).toBe(true)
      expect(
        readFileSync(join(tmpHome, '.FABRICA', 'agent-hooks', CLAUDE_SCRIPT_FILE_NAME), 'utf-8')
  ...
  it('installs the managed statusLine command and forwards rate_limits posts', () => {
    const tmpHome = mkdtempSync(join(tmpdir(), 'FABRICA-claude-statusline-'))
    vi.stubEnv('HOME', tmpHome)
    vi.stubEnv('USERPROFILE', tmpHome)
    try {
      ...
      const script = readFileSync(
        join(tmpHome, '.FABRICA', 'agent-hooks', STATUSLINE_SCRIPT_FILE_NAME),
        'utf-8'
      )
      ...
      if (process.platform === 'win32') {
        ...
        expect(script).toContain('--data-urlencode "payload@%FABRICA_STATUSLINE_PAYLOAD_FILE%"')
      } else {
  ...
  it('never overwrites a user-owned statusLine command', () => {
    const tmpHome = mkdtempSync(join(tmpdir(), 'FABRICA-claude-user-statusline-'))
    vi.stubEnv('HOME', tmpHome)
    vi.stubEnv('USERPROFILE', tmpHome)
    try {
  ...
  it('removes the managed statusLine on remove()', () => {
    const tmpHome = mkdtempSync(join(tmpdir(), 'FABRICA-claude-statusline-remove-'))
    vi.stubEnv('HOME', tmpHome)
    vi.stubEnv('USERPROFILE', tmpHome)
    try {
  ...
  it('does not re-install a managed statusLine the user deleted, until remove() resets the opt-out', () => {
    const tmpHome = mkdtempSync(join(tmpdir(), 'FABRICA-claude-statusline-optout-'))
    vi.stubEnv('HOME', tmpHome)
    vi.stubEnv('USERPROFILE', tmpHome)
    try {
      const settingsPath = join(tmpHome, '.claude', 'settings.json')
      ...
      // An FABRICA-level remove() resets the opt-out memory, so a fresh install re-adds it.
      new ClaudeHookService().remove()
      new ClaudeHookService().install()
      expect(JSON.parse(readFileSync(settingsPath, 'utf-8')).statusLine).toBeTruthy()
    } finally {
  ...
  it('keeps refreshing a still-managed statusLine across installs', () => {
    const tmpHome = mkdtempSync(join(tmpdir(), 'FABRICA-claude-statusline-refresh-'))
    vi.stubEnv('HOME', tmpHome)
    vi.stubEnv('USERPROFILE', tmpHome)
    try {
      const settingsPath = join(tmpHome, '.claude', 'settings.json')
  ...
  it.skipIf(process.platform !== 'win32')(
    ...
    () => {
      const tmpHome = mkdtempSync(join(tmpdir(), 'FABRICA claude home with spaces '))
      vi.stubEnv('HOME', tmpHome)
      vi.stubEnv('USERPROFILE', tmpHome)
      try {
        expect(new ClaudeHookService().install().state).toBe('installed')
  ...
  it.skipIf(process.platform !== 'win32')(
    ...
    () => {
      const tmpHome = mkdtempSync(join(tmpdir(), 'FABRICA-claude-curl-'))
      vi.stubEnv('HOME', tmpHome)
      vi.stubEnv('USERPROFILE', tmpHome)
      try {
        expect(new ClaudeHookService().install().state).toBe('installed')
        const script = readFileSync(
          join(tmpHome, '.FABRICA', 'agent-hooks', CLAUDE_SCRIPT_FILE_NAME),
          'utf-8'
        )
...
describe('ClaudeHookService.installRemote', () => {
  it('writes Claude settings + managed script under the remote $HOME', async () => {
    ...
    for (const event of [
      ...
      expect(cmd).toContain('/home/dev/.FABRICA/agent-hooks/claude-hook.sh')
      expect(cmd).toMatch(/^if \[ -f /)
    }
    // Managed script body
    const script = fs.files.get('/home/dev/.FABRICA/agent-hooks/claude-hook.sh')
    ...
    expect(fs.modes.get('/home/dev/.FABRICA/agent-hooks/claude-hook.sh')).toBe(0o755)
    ...
    expect(fs.files.get('/home/dev/.FABRICA/agent-hooks/claude-statusline.sh')).toBeUndefined()
  })
  ...
  it('preserves user-authored hook entries while sweeping old managed entries', async () => {
              ...
              hooks: [
                {
                  type: 'command',
                  command:
                    'if [ -x /home/dev/.FABRICA/agent-hooks/claude-hook.sh ]; then /bin/sh /home/dev/.FABRICA/agent-hooks/claude-hook.sh; fi'
                }
              ]
            }
          ]
    ...
    // Original user-authored entry survives, while stale FABRICA entries are
...
describe('OpenClaudeHookService-compatible install', () => {
  ...
  it('installs managed hooks into OpenClaude settings without touching Claude settings', () => {
    const tmpHome = mkdtempSync(join(tmpdir(), 'FABRICA-openclaude-hooks-'))
    vi.stubEnv('HOME', tmpHome)
    vi.stubEnv('USERPROFILE', tmpHome)
    try {
      ...
      expect(
        readFileSync(join(tmpHome, '.FABRICA', 'agent-hooks', OPENCLAUDE_SCRIPT_FILE_NAME), 'utf-8')
  ...
  it('writes remote OpenClaude settings under .openclaude', async () => {
    ...
    expect(command).toContain('/home/dev/.FABRICA/agent-hooks/openclaude-hook.sh')
    expect(fs.files.get('/home/dev/.FABRICA/agent-hooks/openclaude-hook.sh')).toContain('/hook/claude')
  })
})
