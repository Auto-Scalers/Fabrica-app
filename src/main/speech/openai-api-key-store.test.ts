...
beforeEach(() => {
  tempHome = mkdtempLike('FABRICA-openai-key-store-')
...
function writeStoredOpenAiKey(value: string): void {
  ...
  writeFileSync(join(FABRICADir, 'openai-speech-token.enc'), value)
}
...
describe('OpenAI speech API key store', () => {
  ...
  it('reports missing status without creating storage files', async () => {
    ...
    expect(existsSync(join(tempHome, '.FABRICA'))).toBe(false)
    expect(safeStorageMock.decryptString).not.toHaveBeenCalled()
  })
})
