...
describe('OpenCodeUsageStore', () => {
  ...
  beforeEach(() => {
    tempUserData = mkdtempSync(join(tmpdir(), 'FABRICA-opencode-usage-store-'))
  ...
  it('adapts OpenCode scans to pretty-printed cache persistence', async () => {
    ...
    const persistedJson = readFileSync(join(tempUserData, 'FABRICA-opencode-usage.json'), 'utf-8')
    expect(scanOpenCodeUsageDatabases).toHaveBeenCalledWith([], [])
    expect(persistedJson).toContain('\n')
  })

  it('reports no data for FABRICA scope when only non-FABRICA OpenCode usage exists', async () => {
    const store = createStoreWithState({
      sessions: [
        makeSession({
  ...
  it('uses recorded OpenCode costs and token totals without model pricing inference', async () => {
    ...
    const summary = await store.getSummary('FABRICA', '30d')
    const daily = await store.getDaily('FABRICA', '30d')
  ...
  it('returns recent sessions with OpenCode event and token fields', async () => {
    ...
    const sessions = await store.getRecentSessions('FABRICA', '30d', 5)
