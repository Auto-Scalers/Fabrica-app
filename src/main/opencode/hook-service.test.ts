...
describe('OpenCode hook plugin source', () => {
  ...
  it('still accepts an optional opaque plugin context instead of destructuring', () => {
    ...
    expect(source).toContain('export const FABRICAOpenCodeStatusPlugin = async (_ctx) => {')
  ...
  it('resolves hook coords from the endpoint file before falling back to process.env', () => {
    // Why: a forked session freezes the prior FABRICA's PORT/TOKEN in env; prefer the on-disk endpoint file or it posts to a dead port after restart.
    const source = _internals.getOpenCodePluginSource()

    expect(source).toContain('function readEndpointFile()')
    expect(source).toContain('process.env.FABRICA_AGENT_HOOK_ENDPOINT')
    ...
    expect(source).toContain(
      'port: fileEnv.FABRICA_AGENT_HOOK_PORT || process.env.FABRICA_AGENT_HOOK_PORT'
    )
    expect(source).toContain(
      'token: fileEnv.FABRICA_AGENT_HOOK_TOKEN || process.env.FABRICA_AGENT_HOOK_TOKEN'
    )
    ...
    expect(source).toContain('"X-FABRICA-Agent-Hook-Token": coords.token')
  })
...
describe('OpenCodeHookService buildPtyEnv / clearPty round-trip', () => {
  ...
  beforeAll(() => {
    userDataDir = mkdtempSync(join(tmpdir(), 'FABRICA-opencode-hooks-'))
  ...
  it('writes a shared OPENCODE_CONFIG_DIR and installs the plugin file', () => {
    ...
    const pluginPath = join(env.OPENCODE_CONFIG_DIR!, 'plugins', 'FABRICA-opencode-status.js')
    expect(existsSync(pluginPath)).toBe(true)
    // Sanity-check the file has plugin source, not a stray write.
    const pluginSource = readFileSync(pluginPath, 'utf8')
    expect(pluginSource).toContain('FABRICAOpenCodeStatusPlugin')
  ...
  it('buildPtyEnv preserves a user-set OPENCODE_CONFIG_DIR when the id is unusable', () => {
    ...
    const service = new OpenCodeHookService()
    const userDir = mkdtempSync(join(tmpdir(), 'FABRICA-opencode-userdir-'))
    try {
  ...
  it('works end-to-end for a plain UUID id (non-daemon path)', () => {
    ...
    expect(env.OPENCODE_CONFIG_DIR).toBe(join(userDataDir, 'opencode-hooks', 'shared'))
    expect(existsSync(join(env.OPENCODE_CONFIG_DIR!, 'plugins', 'FABRICA-opencode-status.js'))).toBe(
      true
    )
...
describe('OpenCodeHookService overlay mode (user OPENCODE_CONFIG_DIR set)', () => {
  ...
  beforeAll(() => {
    userDataDir = mkdtempSync(join(tmpdir(), 'FABRICA-opencode-overlay-userdata-'))
    getPathMock.mockImplementation((name: string) => {
      if (name === 'userData') {
        return userDataDir
      }
  ...
  beforeEach(() => {
    userConfigDir = mkdtempSync(join(tmpdir(), 'FABRICA-opencode-overlay-userconfig-'))
  ...
  it('builds an overlay under userData and exposes user config + FABRICA plugin together', () => {
    const service = new OpenCodeHookService()
    ...
    // FABRICA's status plugin is a sibling, not a replacement.
    const FABRICAPluginPath = join(env.OPENCODE_CONFIG_DIR!, 'plugins', 'FABRICA-opencode-status.js')
    expect(existsSync(FABRICAPluginPath)).toBe(true)
    expect(readFileSync(FABRICAPluginPath, 'utf8')).toContain('FABRICAOpenCodeStatusPlugin')
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      // Why: only plugins/ needs per-entry mirroring (FABRICA drops a sibling); other entries are single symlinks so user edits propagate live.
      const service = new OpenCodeHookService()
      const env = service.buildPtyEnv(ptyId, userConfigDir)
      ...
      // plugins/ must be a real dir in the overlay so FABRICA can drop its sibling plugin.
  ...
  it("does not overwrite a user plugin file with the same filename as FABRICA's plugin", () => {
    // Why: a user plugin named FABRICA-opencode-status.js must not be symlinked into the overlay, or writeFileSync would clobber it.
    const userFABRICASentinel = 'USER OWNED FABRICA-NAMED PLUGIN — DO NOT CLOBBER'
    writeFileSync(join(userConfigDir, 'plugins', 'FABRICA-opencode-status.js'), userFABRICASentinel)

    const service = new OpenCodeHookService()
    const env = service.buildPtyEnv(ptyId, userConfigDir)

    // User's source file must be untouched.
    expect(readFileSync(join(userConfigDir, 'plugins', 'FABRICA-opencode-status.js'), 'utf8')).toBe(
      userFABRICASentinel
    )

    // Overlay copy is FABRICA's real plugin source, not the user's file.
    const overlayPlugin = readFileSync(
      join(env.OPENCODE_CONFIG_DIR!, 'plugins', 'FABRICA-opencode-status.js'),
      'utf8'
    )
    expect(overlayPlugin).toContain('FABRICAOpenCodeStatusPlugin')
    expect(overlayPlugin).not.toBe(userFABRICASentinel)
    expectUserConfigIntact()
  })
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      // Why: writing FABRICA's plugin through a symlinked plugins/ would leak into the user's fs (docs/opencode-config-dir-collision.md).
      const realPluginsDir = mkdtempSync(join(tmpdir(), 'FABRICA-real-plugins-'))
      try {
        ...
        // The user's real filesystem must NOT receive FABRICA's status plugin.
        expect(existsSync(join(realPluginsDir, 'FABRICA-opencode-status.js'))).toBe(false)
        ...
        // FABRICA's status plugin lands in the overlay only.
        expect(
          existsSync(join(env.OPENCODE_CONFIG_DIR!, 'plugins', 'FABRICA-opencode-status.js'))
        ).toBe(true)
  ...
  it("preserves the user's OPENCODE_CONFIG_DIR when the path does not exist", () => {
    // Why: leave a nonexistent user path alone so OpenCode surfaces the typo instead of FABRICA silently hiding it.
    const service = new OpenCodeHookService()
    const missingPath = join(tmpdir(), `FABRICA-opencode-nope-${Date.now()}`)
  ...
  it('rebuilding the overlay for the same ptyId does not corrupt the user dir', () => {
    ...
    expect(
      readFileSync(join(env.OPENCODE_CONFIG_DIR!, 'plugins', 'FABRICA-opencode-status.js'), 'utf8')
    ).toContain('FABRICAOpenCodeStatusPlugin')
    expectUserConfigIntact()
  })
