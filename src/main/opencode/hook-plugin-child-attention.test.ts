...
describe('OpenCode plugin child attention', () => {
  ...
  beforeEach(() => {
    tempDir = mkdtempSync(join(tmpdir(), 'FABRICA-opencode-child-attention-'))
    posts = []
    savedEnv = {}
  ...
  async function createHooks(
    ...
    if (!pluginFactory) {
      ...
      const module = (await import(pathToFileURL(pluginPath).href)) as {
        FABRICAOpenCodeStatusPlugin: PluginFactory
      }
      pluginFactory = module.FABRICAOpenCodeStatusPlugin
    }
