 ...
 * coalesced and capped before POSTing to FABRICA's agent-hook server. The
 * un-throttled plugin re-posted the full accumulated reply per streamed
 * append — O(n²) bytes per turn — which saturated FABRICA's main + renderer
...
const ENV_KEYS = ['FABRICA_PANE_KEY', 'FABRICA_AGENT_HOOK_PORT', 'FABRICA_AGENT_HOOK_TOKEN'] as const
...
describe('OpenCode plugin MessagePart throttling', () => {
  ...
  beforeEach(() => {
    tempDir = mkdtempSync(join(tmpdir(), 'FABRICA-opencode-plugin-test-'))
    posts = []
    savedEnv = {}
  ...
  async function loadPluginEventHandler(): Promise<PluginEventHandler> {
    ...
    const module = (await import(pathToFileURL(pluginPath).href)) as {
      FABRICAOpenCodeStatusPlugin: (ctx: unknown) => Promise<{ event: PluginEventHandler }>
    }
    const client = {
      session: {
    ...
    const hooks = await module.FABRICAOpenCodeStatusPlugin({ client })
    return hooks.event
  }
