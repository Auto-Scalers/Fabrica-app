...
const FABRICA_OPENCODE_PLUGIN_FILE = 'FABRICA-opencode-status.js'
...
const OPENCODE_OVERLAY_MANIFEST_FILE = '.fabrica-opencode-overlay-manifest.json'
...
export function getOpenCodeFamilyPluginSource(hookPathname: string): string {
  ...
  return [
    ...
    "// OpenCode's Node process (not FABRICA's) and has no access to server.ts's",
    ...
    '// which reads the endpoint file. The file only changes on FABRICA restart',
    ...
    '  const path = process.env.FABRICA_AGENT_HOOK_ENDPOINT;',
    '  if (!path) return null;',
    '  try {',
    '    const fs = require("fs");',
    '    try {',
    ...
    '      // writeEndpointFile on the FABRICA side) allocates a fresh inode on',
    ...
    '        // digits in the key for forward-compat with future FABRICA_AGENT_HOOK_*',
    ...
    '      console.warn("[FABRICA-hook] failed to parse endpoint file:", err.message);',
    '    }',
    '    return null;',
    '  }',
    '}',
    ...
    '  // frozen when OpenCode was fork()ed — stale after an FABRICA restart. The',
    '  // file is rewritten on every FABRICA start(), so sourcing it per post lets',
    '  // a long-running OpenCode session reach the current server. Falls back',
    '  // to process.env when the file is absent (first-run / pre-endpoint-file / FABRICA',
    ...
    '    env: fileEnv.FABRICA_AGENT_HOOK_ENV || process.env.FABRICA_AGENT_HOOK_ENV || "",',
    '    version: fileEnv.FABRICA_AGENT_HOOK_VERSION || process.env.FABRICA_AGENT_HOOK_VERSION || "",',
    '  };',
    '}',
    '',
    ...
    '// per turn through FABRICA (loopback HTTP -> main JSON parse -> status compare',
    ...
    '// flip the root FABRICA pane to done or overwrite the parent turn preview.',
    ...
    '  // rewritten by a newer FABRICA since the last call. Pane/tab/worktree IDs',
    '  // stay on process.env because they are per-PTY (stable for the life of',
    '  // the OpenCode process), not per-FABRICA-instance.',
    '  const coords = resolveHookCoords();',
    ...
    '    // because FABRICA is unavailable or the local loopback request failed.',
    '    return false;',
    '  } finally {',
    '    clearTimeout(timeout);',
    '  }',
    ...
    'export const FABRICAOpenCodeStatusPlugin = async (_ctx) => {',
    ...
    '          // publish a final idle so FABRICA does not retain a dead owner.',
...
export class OpenCodeHookService {
  ...
  buildPtyEnv(ptyId: string, existingConfigDir?: string | undefined): Record<string, string> {
    if (!isUsableId(ptyId)) {
      // Why: on a bad id, still preserve a user-set OPENCODE_CONFIG_DIR; only the FABRICA status plugin is forfeited.
  ...
  private clearManifestEntries(overlayDir: string, manifest: OpenCodeOverlayManifest): void {
    ...
    for (const entryName of manifest.pluginEntries) {
      if (entryName === FABRICA_OPENCODE_PLUGIN_FILE) {
        continue
      }
  ...
  // Why: mirror user config entries as symlinks so edits propagate live; only plugins/ becomes a real overlay dir so FABRICA can drop a sibling plugin file.
  private mirrorUserConfig(sourceDir: string, overlayDir: string): void {
    ...
    // Why: overlays persist across terminals; remove only FABRICA-mirrored paths so stale user config clears but OpenCode runtime dirs (node_modules) survive.
      ...
      if (entry.name === 'plugins') {
        ...
        if ((!isSymlink && entry.isDirectory()) || isLinkPointingToDir) {
          ...
          for (const pluginEntry of readdirSync(resolvedSource, { withFileTypes: true })) {
            // Why: skip a user plugin sharing FABRICA's filename; mirroring it would let writePluginIntoOverlay clobber the user's file.
            if (pluginEntry.name === FABRICA_OPENCODE_PLUGIN_FILE) {
              continue
            }
            mirrorEntry(
  ...
  private writePluginIntoOverlay(overlayDir: string): void {
    ...
    const pluginPath = join(pluginsDir, FABRICA_OPENCODE_PLUGIN_FILE)
    try {
  ...
  private writeSharedPluginConfig(): string | null {
    ...
    try {
      mkdirSync(pluginsDir, { recursive: true })
      writeFileSync(join(pluginsDir, FABRICA_OPENCODE_PLUGIN_FILE), getOpenCodePluginSource())
    } catch {
