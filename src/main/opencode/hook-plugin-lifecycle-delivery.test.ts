 ...
 * lives inside OpenCode's process, not in FABRICA's TypeScript runtime.
 */
...
describe('OpenCode plugin lifecycle delivery', () => {
  ...
  beforeEach(() => {
    tempDir = mkdtempSync(join(tmpdir(), 'FABRICA-opencode-lifecycle-plugin-'))
    posts = []
    savedEnv = {}
    for (const key of ENV_KEYS) {
      savedEnv[key] = process.env[key]
    }
    process.env.FABRICA_PANE_KEY = 'tab-1:leaf-1'
    process.env.FABRICA_AGENT_HOOK_PORT = '45678'
    process.env.FABRICA_AGENT_HOOK_TOKEN = 'test-token'
  ...
  async function loadHooksWithSession(session: object): Promise<PluginHooks> {
    const pluginPath = join(tempDir, 'FABRICA-opencode-status.mjs')
    writeFileSync(pluginPath, _internals.getOpenCodePluginSource())
    const module = (await import(pathToFileURL(pluginPath).href)) as {
      FABRICAOpenCodeStatusPlugin: (ctx: unknown) => Promise<PluginHooks>
    }
    return module.FABRICAOpenCodeStatusPlugin({ client: { session } })
  }
  ...
  it('reasserts Busy through the refreshed endpoint after a live text delta', async () => {
    ...
    globalThis.fetch = vi.fn(async (url: RequestInfo | URL, init?: RequestInit) => {
      ...
      deliveries.push({
        url: String(url),
        token: new Headers(init?.headers).get('X-FABRICA-Agent-Hook-Token')
      })
      return new Response(null, { status: 204 })
    }) as typeof globalThis.fetch
    const handler = await loadHandler()
    ...
    await handler({ event: status('busy') })
    process.env.FABRICA_AGENT_HOOK_PORT = '56789'
    process.env.FABRICA_AGENT_HOOK_TOKEN = 'refreshed-token'
