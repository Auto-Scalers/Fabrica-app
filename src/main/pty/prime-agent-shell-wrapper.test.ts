...
function makeFixture(): {
  ...
  const root = mkdtempSync(join(tmpdir(), 'FABRICA-prime-wrapper-'))
  tempDirs.push(root)
  const binDir = join(root, 'bin')
  const primeHome = join(root, 'guest-prime-home')
  const extensionPath = join(root, 'FABRICA-agent-status.ts')
  ...
  writeFileSync(endpointPath, 'FABRICA_AGENT_HOOK_PORT=4567\nFABRICA_AGENT_HOOK_TOKEN=current-token\n')
...
function runPrime(
  ...
  const result = spawnSync('bash', ['--noprofile', '--rcfile', fixture.rcfile, '-ic', command], {
    ...
    env: {
      ...
      FABRICA_PRIME_AGENT_STATUS_EXTENSION: extensionPath,
      FABRICA_WSL_HOOK_INSTANCE: 'testinstance',
      FABRICA_AGENT_HOOK_ENDPOINT: '/mnt/c/current/endpoint.cmd',
      FABRICA_AGENT_HOOK_PORT: '4567',
      FABRICA_AGENT_HOOK_TOKEN: 'current-token',
      FABRICA_CAPTURE_FILE: fixture.capturePath
    },
    encoding: 'utf8'
  })
...
describePosix('Prime Agent shell wrapper', () => {
  ...
  itWithBash('keeps the stable guest endpoint path while its relay refreshes stale data', () => {
    const fixture = makeFixture()
    writeFileSync(fixture.endpointPath, 'FABRICA_AGENT_HOOK_PORT=4567\nFABRICA_AGENT_HOOK_TOKEN=stale\n')
  ...
  itWithBash('does not define wrapper helpers when the bridge env is absent', () => {
    ...
    const result = spawnSync(
      'bash',
      ['--noprofile', '--rcfile', fixture.rcfile, '-ic', 'type -t __FABRICA_prime_agent'],
      {
        cwd: fixture.root,
        env: { ...process.env, FABRICA_PRIME_AGENT_STATUS_EXTENSION: '' },
        encoding: 'utf8'
      }
    )
