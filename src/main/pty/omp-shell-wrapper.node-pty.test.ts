...
function makeTempDir(): string {
  const dir = mkdtempSync(join(tmpdir(), 'FABRICA-omp-node-pty-'))
  tempDirs.push(dir)
  return dir
}
...
function writeFakeOmp(binDir: string): void {
  ...
  writeFileSync(
    ompPath,
    `#!/bin/sh
agent_dir="\${PI_CODING_AGENT_DIR:-\${FABRICA_FAKE_OMP_DEFAULT_DIR:-}}"
...
describePosix('OMP shell wrapper node-pty reproduction', () => {
  ...
  itWithBash('reproduces why restored shells miss OMP status without the wrapper', async () => {
    ...
    const statusExtension = join(extensionDir, 'FABRICA-agent-status.ts')
    ...
    const makeEnv = (captureFile: string, afterPiFile: string): Record<string, string> => ({
      ...
      FABRICA_OMP_STATUS_EXTENSION: statusExtension,
      FABRICA_FAKE_OMP_DEFAULT_DIR: ompDir,
      FABRICA_CAPTURE_FILE: captureFile,
      FABRICA_AFTER_PI_FILE: afterPiFile,
    ...
    await runInteractiveBashPty({
      ...
      input: `omp ask
printf '%s' "$PI_CODING_AGENT_DIR" > "$FABRICA_AFTER_PI_FILE"
exit 0
`
    })
  ...
  itWithBash('runs OMP config subcommands without redirecting the home', async () => {
    ...
    await runInteractiveBashPty({
      ...
      env: {
        ...
        FABRICA_PI_CODING_AGENT_DIR: '',
        FABRICA_OMP_CODING_AGENT_DIR: '',
        FABRICA_OMP_SOURCE_AGENT_DIR: sourceDir,
        FABRICA_OMP_STATUS_EXTENSION: statusExtension,
        FABRICA_FAKE_OMP_DEFAULT_DIR: sourceDir,
        FABRICA_CAPTURE_FILE: captureFile,
        TERM: process.env.TERM || 'xterm-256color'
      },
      input: `omp config
    ...
    async () => {
      ...
      await runInteractiveBashPty({
        ...
        env: {
          ...
          FABRICA_OMP_CODING_AGENT_DIR: '',
          FABRICA_OMP_STATUS_EXTENSION: statusExtension,
          FABRICA_FAKE_OMP_DEFAULT_DIR: defaultOmpDir,
          FABRICA_CAPTURE_FILE: captureFile,
          TERM: process.env.TERM || 'xterm-256color'
        },
        input: `omp config
