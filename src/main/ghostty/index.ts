...
// Why: mapGhosttyToFABRICA creates new object instances for nested values like
...
export async function previewGhosttyImport(store: Store): Promise<GhosttyImportPreview> {
  ...
  const { diff: rawDiff, unsupportedKeys: mappedUnsupportedKeys } = mapGhosttyToFABRICA(
    parsed,
    platform() === 'darwin'
  )
