...
import { mapGhosttyToFABRICA } from './mapper'

describe('mapGhosttyToFABRICA — split-divider-color', () => {
  it('maps valid hex to both dark and light divider colors', () => {
    const result = mapGhosttyToFABRICA({ 'split-divider-color': '#ff5500' })
  ...
  it('rejects invalid split-divider-color', () => {
    const result = mapGhosttyToFABRICA({ 'split-divider-color': 'blue' })
    expect(result.diff).toEqual({})
    expect(result.unsupportedKeys).toEqual(['split-divider-color'])
  })
})

describe('mapGhosttyToFABRICA — unfocused-split-opacity', () => {
  it('maps valid float to terminalInactivePaneOpacity', () => {
    const result = mapGhosttyToFABRICA({ 'unfocused-split-opacity': '0.5' })
    expect(result.diff).toEqual({ terminalInactivePaneOpacity: 0.5 })
    expect(result.unsupportedKeys).toEqual([])
  })

  it('rejects out-of-range unfocused-split-opacity', () => {
    const result = mapGhosttyToFABRICA({ 'unfocused-split-opacity': '1.2' })
    expect(result.diff).toEqual({})
    expect(result.unsupportedKeys).toEqual(['unfocused-split-opacity'])
  })

  it('rejects negative unfocused-split-opacity', () => {
    const result = mapGhosttyToFABRICA({ 'unfocused-split-opacity': '-0.1' })
    expect(result.diff).toEqual({})
    expect(result.unsupportedKeys).toEqual(['unfocused-split-opacity'])
  })
})

describe('mapGhosttyToFABRICA — scrollback-limit', () => {
...
describe('mapGhosttyToFABRICA — adjust-cell-height', () => {
  ...
  it('rounds higher half-boundary decimal percentages to the nearest hundredth', () => {
    const result = mapGhosttyToFABRICA({ 'adjust-cell-height': '1.5%' })
...
describe('mapGhosttyToFABRICA — cursor-text', () => {
  ...
  it('rejects invalid cursor-text', () => {
    const result = mapGhosttyToFABRICA({ 'cursor-text': 'white' })
    expect(result.diff).toEqual({})
    expect(result.unsupportedKeys).toEqual(['cursor-text'])
  })
})

describe('mapGhosttyToFABRICA — bold-color', () => {
