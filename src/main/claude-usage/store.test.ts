...
describe('ClaudeUsageStore', () => {
  ...
  beforeEach(() => {
    tempUserData = mkdtempSync(join(tmpdir(), 'FABRICA-claude-usage-store-'))
  ...
  it('defaults a null legacy opt-in while invalidating the cache', () => {
    writeFileSync(
      join(tempUserData, 'FABRICA-claude-usage.json'),
  ...
  it('reports no data for FABRICA scope when only non-FABRICA usage exists', async () => {
  ...
  it('filters sessions by local calendar day instead of raw UTC date prefixes', async () => {
    ...
    const recentSessions = await store.getRecentSessions('FABRICA', '7d', 10)
  ...
  it('adapts Claude scans to pretty-printed cache persistence', async () => {
    ...
    expect(readFileSync(join(tempUserData, 'FABRICA-claude-usage.json'), 'utf-8')).toContain('\n')
  })
})
