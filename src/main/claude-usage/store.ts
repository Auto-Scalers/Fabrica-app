...
export function initClaudeUsagePath(): void {
  _claudeUsageFile = join(app.getPath('userData'), 'FABRICA-claude-usage.json')
}
...
> {
  ...
  private buildSummary(scope: ClaudeUsageScope, range: ClaudeUsageRange): ClaudeUsageSummary {
    ...
    return {
      ...
      // data here makes the FABRICA-only view render empty charts instead of the
      // intended "no usage for this scope" message when only off-FABRICA logs exist.
  ...
  async getAutomationRunUsage(input: AutomationUsageLookupInput): Promise<AutomationRunUsage> {
    ...
    return {
      ...
      // Why: FABRICA terminal tab ids and Claude usage session ids are different
  ...
  private getFilteredDaily(scope: ClaudeUsageScope, range: ClaudeUsageRange) {
    ...
    return this.state.dailyAggregates.filter((entry) => {
      ...
      if (scope === 'FABRICA' && entry.worktreeId === null) {
        return false
      }
      return true
    })
