...
function syncSplit(runtime: FABRICARuntimeService, snapshot = makeSplitSnapshot()): void {
...
describe('FABRICARuntimeService terminal surface retirement', () => {
  ...
  it('rejects one stale shared-PTY surface without removing its live sibling', () => {
    ...
    const runtime = new FABRICARuntimeService(runtimeStore({ getWorkspaceSession: () => session }))
    runtime.attachWindow(1)
