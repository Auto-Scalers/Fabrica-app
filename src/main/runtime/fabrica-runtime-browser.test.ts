...
import type { RuntimeBrowserCommandHost } from './fabrica-runtime-browser'
    ...
    profiles: new Map([
      ...
      [
        'profile-isolated',
        {
          id: 'profile-isolated',
          scope: 'isolated',
          partition: 'persist:FABRICA-browser-session-profile-isolated',
          label: 'Isolated',
          source: null
        }
      ]
...
describe('RuntimeBrowserCommands browser screencast', () => {
  beforeEach(() => {
    ...
    browserSessionRegistryMock.resolveKnownPartition.mockImplementation(
      (profileId: string | null | undefined) => {
        if (!profileId) {
          return 'persist:FABRICA-browser'
        }
  ...
  it('creates profiles with the requested user-agent mode', async () => {
    const { RuntimeBrowserCommands } = await import('./fabrica-runtime-browser')
    const profile = {
      id: 'profile-google',
      scope: 'isolated',
      partition: 'persist:FABRICA-browser-session-profile-google',
      label: 'Google',
      source: null,
  ...
  it('sends the resolved isolated profile partition when creating a renderer tab', async () => {
    ...
    expect(send).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        sessionPartition: 'persist:FABRICA-browser-session-profile-isolated'
      })
    )
  })
