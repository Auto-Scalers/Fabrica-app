// #11761: on a headless `FABRICA serve` host the HTTP agent hook is the only carrier of
...
): Promise<FABRICARuntimeService> {
  const runtime = new FABRICARuntimeService(null, undefined, {
...
async function projectAgentStatus(
  rows: AgentStatusIpcPayload[],
  preparePane?: (runtime: FABRICARuntimeService) => void
...
function observePaneTitle(runtime: FABRICARuntimeService, title: string): void {
  runtime.onPtyData(PTY_ID, `\x1b]0;${title}\x07`, Date.now())
}

function lastOscTitleEpochMs(runtime: FABRICARuntimeService): number {
  const pty = (
