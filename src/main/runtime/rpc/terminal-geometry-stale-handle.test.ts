...
function stubStaleHandleRuntime(overrides: Partial<FABRICARuntimeService> = {}): FABRICARuntimeService {
  return {
...
async function expectStale(method: string, params: unknown, mutators: string[]): Promise<void> {
  ...
  const runtime = stubStaleHandleRuntime(spies as Partial<FABRICARuntimeService>)
