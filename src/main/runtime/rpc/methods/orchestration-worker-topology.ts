...
export function requireWorkerAuthority(runtime: FABRICARuntimeService, terminalHandle: string) {
...
export async function createWorkerWorktree(args: {
  ...
  coordinatorWorktree: Awaited<ReturnType<FABRICARuntimeService['showManagedWorktree']>>
  params: {
    repo?: string
    name?: string
    baseBranch?: string
  ...
  worktree: Awaited<ReturnType<FABRICARuntimeService['showManagedWorktree']>>
  ...
  return {
    worktree: created.worktree as Awaited<ReturnType<FABRICARuntimeService['showManagedWorktree']>>,
    terminalHandle,
    setupReceipt
  }
}
