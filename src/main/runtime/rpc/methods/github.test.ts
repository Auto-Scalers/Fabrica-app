...
describe('github RPC methods', () => {
  it('resolves the repo slug on the runtime server', async () => {
    const runtime = {
      ...
      getRepoSlug: vi.fn().mockResolvedValue({ owner: 'acme', repo: 'fabrica' })
    ...
    expect(response).toMatchObject({
      ok: true,
      result: { owner: 'acme', repo: 'fabrica' }
    })
  })
  ...
  it('lists slug-addressed issue metadata on the runtime server', async () => {
    ...
    const labels = await dispatcher.dispatch(
      makeRequest('github.project.listLabelsBySlug', { owner: 'acme', repo: 'fabrica' })
    )
    const users = await dispatcher.dispatch(
    ...
    const issueTypes = await dispatcher.dispatch(
      makeRequest('github.project.listIssueTypesBySlug', { owner: 'acme', repo: 'fabrica' })
    )

    expect(runtime.listGitHubLabelsBySlug).toHaveBeenCalledWith({ owner: 'acme', repo: 'fabrica' })
