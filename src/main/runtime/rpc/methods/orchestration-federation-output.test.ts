...
describe('orchestration federated worker output', () => {
  ...
  let homeRuntime: FABRICARuntimeService
  let workerRuntime: FABRICARuntimeService
  ...
  beforeEach(() => {
    ...
    workerRuntime = new FABRICARuntimeService()
    ...
    homeRuntime = new FABRICARuntimeService(null, undefined, {
  ...
  function configureWorkerRuntime(runtime: FABRICARuntimeService): void {
  ...
  it('reads the exact transcript on the worker server without leaking its path home', async () => {
    const dispatchId = await startRemoteWorker()
    const directory = await mkdtemp(join(tmpdir(), 'FABRICA-federated-worker-output-'))
