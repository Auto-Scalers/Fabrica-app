...
export async function inspectWorkerTerminal(
  ...
  terminal: Awaited<ReturnType<FABRICARuntimeService['showTerminal']>> | null
  exact: boolean
...
export function resolvePinnedFederatedServer(
  ...
  if (server.peerFingerprint !== federated.peer_fingerprint) {
    throw new OrchestrationError(
      'peer_changed',
      `Saved environment ${federated.environment_name} now identifies a different Fabrica server.`
    )
  }
  return server
}
