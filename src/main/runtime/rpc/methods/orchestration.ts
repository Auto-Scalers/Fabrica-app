...
const CheckParams = z
  .object({
    ...
    compatibilityCliCommand: z.enum(['fabrica', 'FABRICA', 'FABRICA-ide', 'FABRICA-dev']).optional(),
    run: OptionalString,
...
const AskParams = z
  .object({
    ...
    run: OptionalString,
    compatibilityCliCommand: z.enum(['fabrica', 'FABRICA', 'FABRICA-ide', 'FABRICA-dev']).optional(),
    compatibilityWindowsCommand: z.enum(['fabrica', 'FABRICA', 'FABRICA-ide']).optional()
  })
  .superRefine((params, ctx) => {
        ...
        if (federatedTarget && federatedDispatchId) {
          ...
          if (
            ...
            throw new OrchestrationError(
              ...
              `Federated Dispatch ${dispatchId} does not support coordinator control mail; start a fresh worker after updating its Fabrica server.`
            )
          }
