...
describe('aiVault.listSessions handler + shared cache', () => {
  ...
  it('forwards codex-home through the real FABRICARuntimeService construction path', async () => {
    // Why: the dispatcher test above seeds the cache module directly, so it would
    // still pass if FABRICARuntimeService stopped forwarding the codex-home source.
