...
describe('orchestration new-worktree workers', () => {
  type CreateWorktreeResult = Awaited<ReturnType<FABRICARuntimeService['createManagedWorktree']>>
  ...
  it('persists pre-effect, post-effect, and post-input stages in order', async () => {
    mockCreatedWorktree({ hookFound: false })
    let finishWait:
      | ((value: Awaited<ReturnType<FABRICARuntimeService['waitForTerminal']>>) => void)
      | undefined
    let finishPrompt:
      | ((value: Awaited<ReturnType<FABRICARuntimeService['sendTerminalAgentPrompt']>>) => void)
      | undefined
