...
describe('orchestration compatibility runtime authority', () => {
  ...
  it('uses the hydrated hook commitment for a restored exact PTY', () => {
    ...
    const uncommitted = new FABRICARuntimeService()
  ...
  it('attests current coordinators immediately without upgrading restored legacy callers', () => {
    ...
    const createIntegratedRuntime = (launchTokenHash: string | null): FABRICARuntimeService => {
      const runtime = new FABRICARuntimeService(null, undefined, {
    ...
    const evidenceFor = (runtime: FABRICARuntimeService, launchToken = TOKEN) => ({
      terminalHandle: 'term-1',
      paneKey: PANE_KEY,
      launchToken,
