...
> = {
  permission_denied: {
    ...
    fallback:
      "Fabrica couldn't write its runtime file. Check permissions on Fabrica's data folder, then restart."
  },
  storage_unavailable: {
    key: 'runtimeRpc.startupFailure.guidance.storageUnavailable',
    fallback: 'Your disk may be full or read-only. Free up space, then restart Fabrica.'
  },
  invalid_path: {
    key: 'runtimeRpc.startupFailure.guidance.invalidPath',
    fallback:
      "Fabrica's data folder may be missing, moved, or at a path that is too long. Restore it or use a shorter path, then restart Fabrica."
  },
  address_in_use: {
    key: 'runtimeRpc.startupFailure.guidance.addressInUse',
    fallback: 'Another process may be holding the port. Restart Fabrica to try again.'
  },
  unknown: {
    key: 'runtimeRpc.startupFailure.guidance.unknown',
    fallback: 'Restart Fabrica to try again.'
  }
}
...
function createRuntimeRpcStartupFailureDialogOptions(error: unknown): MessageBoxOptions {
  ...
  return {
    ...
    cancelId: 0,
    noLink: true,
    title: translateMain('runtimeRpc.startupFailure.title', 'Fabrica CLI unavailable'),
    message: translateMain(
      'runtimeRpc.startupFailure.message',
      "Fabrica couldn't start its local command transport."
    ),
    detail: translateMain(
      'runtimeRpc.startupFailure.detail',
      'Fabrica will continue to work, but commands such as FABRICA status, FABRICA terminal, and orchestration are unavailable for this session.\n\n{{guidance}}\n\nCause: {{cause}}',
