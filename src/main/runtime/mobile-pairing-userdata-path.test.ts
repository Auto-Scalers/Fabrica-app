...
// app.setName('FABRICA') changing how app.getPath('userData') resolves (e.g. from
// lowercase 'FABRICA' to uppercase 'FABRICA' on a case-sensitive filesystem) — the
...
describe('mobile pairing userData path stability', () => {
  ...
  beforeEach(() => {
    root = mkdtempSync(join(tmpdir(), 'FABRICA-pairing-path-'))
  ...
  it('keeps returning the path captured before app.setName changes resolution', async () => {
    ...
    // app.setName('FABRICA') happens later in startup, changing late resolution.
  ...
  it('writes DeviceRegistry + E2EE keypair under the canonical path, not the late one', async () => {
    ...
    appState.userData = lateDir // app.setName('FABRICA') has run by the time the runtime starts

    const { DeviceRegistry } = await import('./device-registry')
    const { loadOrCreateE2EEKeypair } = await import('./e2ee-keypair')

    // Mirrors FABRICARuntimeRpcServer.start(): both read from the same userDataPath.
    ...
    // Pairing credentials land beside FABRICA-data.json so they survive restarts/updates.
