...
function environment(
  ...
  return {
    platform: 'win32',
    isPackaged: true,
    executablePath: "C:\\Users\\O'Brien\\FABRICA\\FABRICA.exe",
...
describe('windows mobile firewall', () => {
  it('inspects the exact executable, port, and selected interface profile', async () => {
    ...
    expect(script).toContain(
      "Get-NetFirewallApplicationFilter -PolicyStore ActiveStore -Program 'C:\\Users\\O''Brien\\FABRICA\\FABRICA.exe'"
    )
    ...
    expect(script).toContain("C:\\Users\\O''Brien\\FABRICA\\FABRICA.exe")
  ...
  it('repairs only FABRICA mobile pairing on private networks after elevation', async () => {
    ...
    expect(repairScript).toContain("-Name 'Fabrica.MobilePairing'")
    ...
    expect(repairScript).toContain("-Program 'C:\\Users\\O''Brien\\FABRICA\\FABRICA.exe'")
