...
export type PreambleParams = {
  ...
  workerHandle: string
  devMode?: boolean
  // Why: packaged WSL panes install the scoped launcher as `fabrica`;
  // other execution hosts keep their existing bare `fabrica` bridge.
  ...
  // shells have no agent prompt for FABRICA to reuse.
...
// Why: the dispatch preamble teaches agents about FABRICA's CLI commands for
...
export function buildDispatchPreamble(params: PreambleParams): string {
  // Why: in dev mode, agents must use FABRICA-dev to connect to the dev runtime's
  // socket. Without this, agents inside the dev Electron app would call the
  // production CLI and talk to the wrong FABRICA instance (Section 6.4).
  const cli = params.devMode ? 'FABRICA-dev' : (params.cliCommand ?? 'FABRICA')
  ...
  const header = `You are working inside FABRICA, a multi-agent IDE. You are a dispatched worker.
...
prompt for Fabrica to reuse; if the coordinator has more for you it will
