...
describe('buildDispatchPreamble', () => {
  ...
  it('uses FABRICA CLI by default when devMode is not set', () => {
    const result = buildDispatchPreamble(baseParams())
  ...
  it('uses FABRICA-dev CLI when devMode is true', () => {
    ...
    const fragments = result.split('FABRICA-dev')
    for (const fragment of fragments) {
      expect(fragment).not.toMatch(/FABRICA orchestration/)
    }
  })

  it('uses FABRICA CLI when devMode is false', () => {
