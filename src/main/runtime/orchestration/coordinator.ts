...
export class Coordinator {
  ...
  private async dispatchTask(
    ...
    // Why: dispatched agents use FABRICA-dev in dev mode to reach the dev runtime's socket, not production (Section 6.4).
    const preamble = buildDispatchPreamble({
      ...
      devMode: process.env.FABRICA_USER_DATA_PATH?.includes('FABRICA-dev'),
