...
function addLifecycleRejectionMarker(payload: string | null, code: string, reason: string): string {
  ...
  return JSON.stringify({
    ...parsed,
    _FABRICALifecycleRejection: { code, reason }
  })
}
...
function hasLifecycleRejectionMarker(payload: string | null): boolean {
  try {
    ...
    const marker = (value as Record<string, unknown>)._FABRICALifecycleRejection
    return Boolean(
      marker &&
...
function hardenOrchestrationDatabaseFiles(dbPath: string | ':memory:'): void {
  if (dbPath === ':memory:' || process.platform === 'win32') {
    // Why: Windows protects these files through FABRICA's current-user-only userData DACL; POSIX mode bits are inert there.
    return
  }
...
export class OrchestrationDb {
    ...
    try {
      ...
      if (
        ...
        throw new OrchestrationError(
          ...
          {
            effectsApplied: false,
            recoveryCommand: `FABRICA orchestration run-use --id ${params.runId} --takeover-legacy`
          }
        )
      }
  ...
  convertLifecycleMessageToRejection(
    ...
    const body = `Fabrica rejected this ${message.type}: ${reason}${originalBody}`
