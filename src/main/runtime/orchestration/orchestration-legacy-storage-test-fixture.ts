...
export function createLegacyStorageCutoverFixture(): {
  ...
  const tempDir = mkdtempSync(join(tmpdir(), 'FABRICA-legacy-storage-'))
  ...
  const rejection = first.insertMessage({
    ...
    type: 'heartbeat',
    payload: JSON.stringify({ _FABRICALifecycleRejection: { code: 'migration', reason: 'cutover' } })
  })
  const lookalike = first.insertMessage({
    ...
    payload: JSON.stringify({
      userData: { _FABRICALifecycleRejection: { code: 'not-a-top-level-audit-marker' } }
    })
  })
  const malformedRejections = [
    first.insertMessage({
      ...
      payload: '{"_FABRICALifecycleRejection":'
    }),
    first.insertMessage({
      from: 'term_legacy_worker',
      to: 'term_legacy_coord',
      subject: 'Array marker',
      payload: JSON.stringify({ _FABRICALifecycleRejection: [] })
    }),
    first.insertMessage({
      from: 'term_legacy_worker',
      to: 'term_legacy_coord',
      subject: 'String marker',
      payload: JSON.stringify({ _FABRICALifecycleRejection: 'migration' })
    }),
    first.insertMessage({
      from: 'term_legacy_worker',
      to: 'term_legacy_coord',
      subject: 'Incomplete marker',
      payload: JSON.stringify({ _FABRICALifecycleRejection: { code: 'migration' } })
    }),
    first.insertMessage({
      from: 'term_legacy_worker',
      to: 'term_legacy_coord',
      subject: 'Non-string marker fields',
      payload: JSON.stringify({ _FABRICALifecycleRejection: { code: 19, reason: false } })
    }),
    first.insertMessage({
      from: 'term_legacy_worker',
      to: 'term_legacy_coord',
      subject: 'Array root',
      payload: JSON.stringify([
        { _FABRICALifecycleRejection: { code: 'migration', reason: 'nested' } }
      ])
    }),
    first.insertMessage({
      from: 'term_legacy_worker',
