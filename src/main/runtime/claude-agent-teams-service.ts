...
export class ClaudeAgentTeamsService {
  ...
  createLaunchEnv(args: {
    ...
    const tmuxValue = `/tmp/FABRICA-claude-agent-teams/${teamId},0,1`
    const env: Record<string, string> = {
      ...
      FABRICA_AGENT_TEAMS_TEAM_ID: teamId,
      FABRICA_AGENT_TEAMS_TOKEN: token,
      FABRICA_AGENT_TEAMS_LEADER_PANE: leaderPane,
      FABRICA_AGENT_TEAMS_SHIM_DIR: args.shimDir,
      FABRICA_AGENT_TEAMS_SHIM_BIN: args.shimBin
    }
    if (args.baseEnv.FABRICA_PAIRING_CODE) {
      env.FABRICA_PAIRING_CODE = args.baseEnv.FABRICA_PAIRING_CODE
    }
    if (args.baseEnv.FABRICA_ENVIRONMENT) {
      env.FABRICA_ENVIRONMENT = args.baseEnv.FABRICA_ENVIRONMENT
    }
