...
// real in-process runtime via FABRICA_USER_DATA_PATH, and asserts:
...
import { FABRICARuntimeRpcServer } from './runtime-rpc'
...
async function runBuiltCli(
  ...
  const child = spawn(process.execPath, [CLI_PATH, ...args], {
    env: {
      ...process.env,
      FABRICA_USER_DATA_PATH: userDataPath,
      FABRICA_TERMINAL_HANDLE: 'term_cli',
      ...extraEnv
    },
...
describeIfBuilt('FABRICA orchestration check --wait subprocess (§3.4)', () => {
  it('emits newline-flushed JSON keepalives to stderr while waiting', async () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'FABRICA-cli-sub-'))
    ...
    try {
      // Why: use the FABRICA_KEEPALIVE_INTERVAL_MS escape hatch to shrink the
      ...
      const child = spawn(
        ...
        ],
        {
          env: {
            ...process.env,
            FABRICA_USER_DATA_PATH: userDataPath,
            FABRICA_TERMINAL_HANDLE: 'term_nobody',
            FABRICA_KEEPALIVE_INTERVAL_MS: String(keepaliveMs)
          },
...
describeIfBuilt('FABRICA orchestration reset subprocess', () => {
  it('validates reset scopes against an isolated runtime through the built CLI', async () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'FABRICA-cli-reset-'))
    const runtime = new FABRICARuntimeService()
    const db = new OrchestrationDb(':memory:')
    runtime.setOrchestrationDb(db)
