...
import { FABRICARuntimeService } from './fabrica-runtime'
...
import { createRuntimeTransportMetadata, FABRICARuntimeRpcServer } from './runtime-rpc'
...
describe('FABRICARuntimeRpcServer', () => {
  ...
  it('writes runtime metadata with transport details when started', async () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'FABRICA-runtime-rpc-'))
    const runtime = new FABRICARuntimeService()
    const server = new FABRICARuntimeRpcServer({ runtime, userDataPath })
  ...
  it('reclaims runtime metadata clobbered by a second instance that has since died', async () => {
    ...
    // FABRICA-runtime.json with its own pid, so the CLI reports stale_bootstrap
  ...
  it('leaves runtime metadata owned by a live sibling runtime untouched', async () => {
    ...
    const server = new FABRICARuntimeRpcServer({
      runtime: new FABRICARuntimeService(),
      userDataPath,
      pid: 4242
    })
    await server.start()
  ...
  it('stops reclaiming runtime metadata after the server is stopped', async () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'FABRICA-runtime-rpc-'))
    const server = new FABRICARuntimeRpcServer({ runtime: new FABRICARuntimeService(), userDataPath })
    await server.start()
  ...
  it('flushes a lastSeen refresh scheduled while transports stop', async () => {
    const server = new FABRICARuntimeRpcServer({
      runtime: new FABRICARuntimeService(),
      userDataPath: mkdtempSync(join(tmpdir(), 'FABRICA-runtime-rpc-')),
  ...
  it('preserves proxy path prefixes in web client URLs', async () => {
    ...
    try {
      const offer = server.createPairingOffer({
        address: 'wss://runtime.example.com/FABRICA',
        name: 'Proxy test'
      })
      expect(offer.available).toBe(true)
      if (offer.available) {
        expect(offer.webClientUrl).toContain('https://runtime.example.com/FABRICA/web-index.html')
      }
    } finally {
      await server.stop()
    }
  ...
  it('formats pairing-address overrides for IPv6 and host-port tunnel endpoints', async () => {
    ...
    try {
      ...
      if (fullUrl.available) {
        expect(fullUrl.endpoint).toBe('wss://runtime.example.com/FABRICA')
      }
    } finally {
      await server.stop()
    }
    ...
    try {
      ...
      expect(offer).toMatchObject({
        ...
        relayFailure: {
          ...
          message: 'FABRICA Relay is not available on this desktop'
        }
      })
      expect(server.getDeviceRegistry()?.getPendingDevice('mobile')).toBeNull()
    } finally {
  ...
  it('shares one socket close listener across concurrent WebSocket dispatches', async () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'FABRICA-runtime-rpc-'))
    const runtime = { getRuntimeId: () => 'test-runtime' } as unknown as FABRICARuntimeService
    const server = new FABRICARuntimeRpcServer({ runtime, userDataPath, enableWebSocket: false })
    server['deviceRegistry'] = new DeviceRegistry(userDataPath)
    const entry = server['deviceRegistry']!.addDevice('runtime-test', 'runtime')
    const ws = new FakeWebSocket()
    server['mobileSocketWiring'] = {
  ...
  it('limits mobile-scoped WebSocket tokens to the mobile RPC surface', async () => {
    ...
    } as unknown as FABRICARuntimeService
    const server = new FABRICARuntimeRpcServer({ runtime, userDataPath, enableWebSocket: false })
    server['deviceRegistry'] = new DeviceRegistry(userDataPath)
    ...
    await server['handleWebSocketMessage'](
      JSON.stringify({
        ...
        deviceToken: mobile.token,
        params: { owner: 'Auto-Scalers', repo: 'fabrica' }
      }),
      (response) => replies.push(JSON.parse(response) as Record<string, unknown>),
      () => {}
    )
    ...
    await server['handleWebSocketMessage'](
      JSON.stringify({
        ...
        deviceToken: mobile.token,
        params: { owner: 'Auto-Scalers', repo: 'fabrica', seedLogins: ['alex'] }
      }),
      (response) => replies.push(JSON.parse(response) as Record<string, unknown>),
      () => {}
    )
  ...
  it('serves terminal.list and terminal.show for live runtime terminals', async () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'FABRICA-runtime-rpc-'))
    const runtime = new FABRICARuntimeService(makeStore() as never)
    const writes: string[] = []
  ...
  it('authorizes a mobile artifact tap after first-connect backfill even once the raw window scrolls', async () => {
    ...
    const artifactPath = join(tmpdir(), `FABRICA-artifact-${process.pid}-${Date.now()}.json`)
  ...
  it('serves worktree.ps from the runtime summary builder', async () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'FABRICA-runtime-rpc-'))
    const runtime = new FABRICARuntimeService(makeStore({ isUnread: true }) as never)
    const server = new FABRICARuntimeRpcServer({ runtime, userDataPath })
  ...
  describe('orchestration DB migration (§3.2)', () => {
    it('is idempotent when delivered_at already exists', () => {
      ...
      const tmpPath = join(mkdtempSync(join(tmpdir(), 'FABRICA-orch-mig-')), 'orch.sqlite')
...
describe('FABRICARuntimeRpcServer WebSocket bind host (STA-2370)', () => {
  const wsTransportOf = (server: FABRICARuntimeRpcServer): WebSocketTransport | undefined =>
  ...
  it('binds all interfaces at startup when exposeNetworkByDefault is set (FABRICA serve)', async () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'FABRICA-runtime-rpc-'))
    const server = new FABRICARuntimeRpcServer({
      runtime: new FABRICARuntimeService(),
      userDataPath,
  ...
  it('upgrades a reused pending grant to network reach so its link survives a relaunch', async () => {
    ...
    const relaunched = new FABRICARuntimeRpcServer({
      runtime: new FABRICARuntimeService(),
      userDataPath,
      enableWebSocket: true,
      wsPort: 0
