...
// `FABRICARuntimeService.fetchRemoteWithCache` — in particular that a rejected
...
// without disturbing the large FABRICA-runtime.test.ts mock surface.
...
// Why: FABRICA-runtime.ts imports heavy modules (hooks, ipc/*, etc.) at top
...
describe('FABRICARuntimeService.fetchRemoteWithCache', () => {
  beforeEach(() => {
  ...
  it('evicts the in-flight Map entry on rejection so the next caller re-fetches', async () => {
    ...
    const runtime = new FABRICARuntimeService(null)
