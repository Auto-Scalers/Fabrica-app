...
function seedSessionTabs(runtime: FABRICARuntimeService): void {
...
describe('paired runtime navigation isolation', () => {
  const servers: FABRICARuntimeRpcServer[] = []
  ...
  async function startHarness() {
    ...
    const server = new FABRICARuntimeRpcServer({
      runtime,
      userDataPath: mkdtempSync(join(tmpdir(), 'FABRICA-navigation-isolation-')),
  ...
  it('restores a device tab selection after a runtime restart', async () => {
    ...
    const first = new FABRICARuntimeService(makeStoreWithSelections() as never)
    ...
    const restarted = new FABRICARuntimeService(makeStoreWithSelections() as never)
