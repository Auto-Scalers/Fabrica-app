...
import { getFABRICACliCommandNameForPlatform } from '../../shared/fabrica-cli-command-name'
...
export function resolveClaudeAgentTeamsShimBin(
  ...
  if (env.FABRICA_AGENT_TEAMS_SHIM_BIN) {
    return env.FABRICA_AGENT_TEAMS_SHIM_BIN
  }
  ...
  return (
    findExecutableOnPath(process.platform === 'win32' ? 'FABRICA-dev.cmd' : 'FABRICA-dev', env.PATH) ??
    findExecutableOnPath(getFABRICACliCommandNameForPlatform(process.platform), env.PATH) ??
    getFABRICACliCommandNameForPlatform(process.platform)
  )
}

function defaultShimRoot(): string {
  return join(homedir(), '.fabrica', 'claude-agent-teams-bin')
}
...
function bundledLauncherPath(): string | null {
  ...
  if (process.platform === 'darwin') {
    return join(process.resourcesPath, 'bin', 'fabrica')
  }
  if (process.platform === 'linux') {
    return join(process.resourcesPath, 'bin', 'fabrica')
  }
  if (process.platform === 'win32') {
    return join(process.resourcesPath, 'bin', 'fabrica.exe')
  }
  return null
}
...
function unixShimScript(): string {
  return [
    '#!/usr/bin/env sh',
    'set -eu',
    `exec "\${FABRICA_AGENT_TEAMS_SHIM_BIN:-${getFABRICACliCommandNameForPlatform(process.platform)}}" agent-teams-tmux "$@"`,
    ''
  ].join('\n')
}
...
function windowsShimScript(): string {
  return [
    '@echo off',
    'setlocal',
    'if "%FABRICA_AGENT_TEAMS_SHIM_BIN%"=="" (',
    `  set "FABRICA_AGENT_TEAMS_SHIM_BIN=${getFABRICACliCommandNameForPlatform(process.platform)}"`,
    ')',
    '"%FABRICA_AGENT_TEAMS_SHIM_BIN%" agent-teams-tmux %*',
    ''
  ].join('\r\n')
}
