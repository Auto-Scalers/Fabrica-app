...
function requirePairing(server: FABRICARuntimeRpcServer, name: string) {
...
describe('remote agent-session host authority integration', () => {
  ...
  it(
    ...
    async () => {
      const userDataPath = mkdtempSync(join(tmpdir(), 'FABRICA-agent-authority-repro-'))
      ...
      const runtime = new FABRICARuntimeService(store as never)
