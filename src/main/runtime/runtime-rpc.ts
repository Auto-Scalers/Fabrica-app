...
// reachable from the LAN; it widens to all interfaces only on explicit pairing (or `FABRICA serve`).
const WS_BIND_HOST_LOOPBACK = '127.0.0.1'
const WS_BIND_HOST_ALL_INTERFACES = '0.0.0.0'

type FABRICARuntimeRpcServerOptions = {
  ...
  wsPort?: number
  // Why: true when the caller pinned a port (`FABRICA serve --port`) so bind order prefers it over a stale STA-1511 fallback (#8535).
  ...
  // Only `FABRICA serve` (explicit remote opt-in) and E2E set this; the desktop app widens lazily on pairing.
...
const DEVICE_REGISTRY_UNAVAILABLE_GUIDANCE =
  'The pairing registry is unavailable. Verify that the Fabrica data directory is writable.'
const E2EE_KEY_UNAVAILABLE_GUIDANCE =
  'The E2EE identity is unavailable. Verify that the Fabrica data directory is writable.'
...
export class FABRICARuntimeRpcServer {
  private readonly runtime: FABRICARuntimeService
  ...
  }: FABRICARuntimeRpcServerOptions) {
  ...
  private async createMobilePairingOfferSerial(
    ...
    const refuseAutomaticWithoutRelay = (
      ...
      return {
        ...
        guidance:
          'Fabrica Relay could not create a pairing invite. Use LAN (Tailscale or same Wi‑Fi) or retry Relay.',
        relayFailure
      }
    }
  ...
  async start(): Promise<void> {
    ...
    this.metadataOwnershipWatch = watchRuntimeMetadataOwnership({
      ...
      onReclaim: (previous) => {
        console.warn(
          `[runtime] Reclaimed FABRICA-runtime.json from a dead runtime (pid ${previous?.pid ?? 'none'}); republished pid ${this.pid}.`
        )
      }
    })
  }
  ...
  // explicitly pairs; `FABRICA serve`/E2E (exposeNetworkByDefault) and a reconnecting paired device bind wide.
...
export function createRuntimeTransportMetadata(
  ...
  if (platform === 'win32') {
    return {
      ...
      endpoint: `\\\\.\\pipe\\FABRICA-${pid}-${endpointSuffix}`
    }
  }
  return {
    kind: 'unix',
