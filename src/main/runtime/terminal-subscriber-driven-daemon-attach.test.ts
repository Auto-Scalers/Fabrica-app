 ...
 *
 * Harness: real FABRICARuntimeService + real TERMINAL_METHODS multiplex handler,
 ...
 * assertion also holds for headless `FABRICA serve`.
 */
...
function internals(runtime: FABRICARuntimeService): RuntimeInternals {
...
function createDaemonProviderModel(opts: { snapshotCapable: boolean }) {
  ...
  return {
    ...
    bind(target: FABRICARuntimeService) {
      runtime = target
    },
...
function startMultiplex(runtime: FABRICARuntimeService, connectionId = 'conn-desktop') {
