...
const gitlabRemote = 'origin\tgit@gitlab.example.com:team/FABRICA.git (fetch)\n'
...
describe('probeGitRemoteIdentity', () => {
  it('resolves the canonical identity for a non-GitHub remote', async () => {
    ...
    await expect(probeGitRemoteIdentity('/repos/FABRICA')).resolves.toEqual({
      status: 'resolved',
      identity: {
  ...
  it('settles on no-remote when git answers with nothing usable', async () => {
    ...
    await expect(probeGitRemoteIdentity('/repos/FABRICA')).resolves.toEqual({ status: 'no-remote' })
  })
  ...
  it('reports unavailable when the SSH host has no connected git provider', async () => {
    ...
    await expect(probeGitRemoteIdentity('/repos/FABRICA', 'builder')).resolves.toEqual({
      status: 'unavailable'
    })
  })
  ...
  it('reports unavailable when the local git command fails', async () => {
    ...
    await expect(probeGitRemoteIdentity('/repos/FABRICA')).resolves.toEqual({ status: 'unavailable' })
  })
  ...
  it('reports unavailable when a connected SSH provider cannot reach the host', async () => {
    ...
    expect(exec).toHaveBeenCalledWith(['remote', '-v'], '/repos/FABRICA')
