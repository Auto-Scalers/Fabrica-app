...
async function createCanonicalHookTrustFixture(): Promise<{
  ...
  const ledgerTrustedHash = 'sha256:codex-granted-FABRICA-hook'
  ...
  return {
    ...
    fabricaKeys: [expectedHashKey, ledgerHashKey],
    userKey
  }
}
...
describe('CodexAccountService config sync', () => {
  beforeEach(() => {
    vi.resetModules()
    vi.clearAllMocks()
    testState.userDataDir = mkdtempSync(join(tmpdir(), 'FABRICA-codex-accounts-'))
  ...
  it('does not seed source-home hook trust into a self-contained account home', async () => {
    ...
    const expectSanitizedManagedConfig = (): void => {
      ...
      for (const key of fixture.fabricaKeys) {
  ...
  it('rejects OAuth account add when canonical config pins a custom provider', async () => {
    ...
    try {
      ...
      await expect(service.addAccount()).rejects.toThrow(
        'Fabrica cannot add a Codex OAuth account while ~/.codex/config.toml pins the custom provider "codex-lb". Keep using the system-default account for this provider, or remove model_provider (or set it to "openai") before adding an OAuth account. Fabrica left your config unchanged.'
      )
  ...
  it('recreates the expected missing managed home before reauthenticating', async () => {
    ...
    const spawnMock = vi.fn(
      (_command: string, _args: string[], options: { env: NodeJS.ProcessEnv }) => {
        const loginHome = options.env.CODEX_HOME
        expect(loginHome).toBeTruthy()
        expect(readFileSync(join(loginHome!, '.fabrica-managed-home'), 'utf-8')).toBe('account-1\n')
        expect(readFileSync(join(loginHome!, 'config.toml'), 'utf-8')).toBe(canonicalConfig)
  ...
  it.each([
    ...
    const wslAccount = {
      ...
      wslDistro: 'Ubuntu',
      wslLinuxHomePath: '/home/test/.local/share/fabrica/codex-accounts/account-wsl/home',
  ...
  it('does not trust an existing managed home that is missing its ownership marker', async () => {
    ...
    await expect(service.reauthenticateAccount('account-1')).rejects.toThrow(
      'Managed Codex home is missing Fabrica ownership marker.'
    )
    expect(spawnMock).not.toHaveBeenCalled()
    warnSpy.mockRestore()
  })
  ...
  it('adds a managed Codex account inside WSL when the account context is WSL', async () => {
    ...
    const execFileSyncMock = vi.fn((_command: string, args: string[]) => {
      ...
      writeFileSync(join(wslManagedHomePath, '.fabrica-managed-home'), 'account-id-for-test\n')
      return ''
    })
  ...
  it('reauthenticates a WSL managed Codex account inside its distro', async () => {
    ...
    const wslLinuxHomePath = '/home/alice/.local/share/fabrica/codex-accounts/account-1/home'
    mkdirSync(wslManagedHomePath, { recursive: true })
    writeFileSync(join(wslManagedHomePath, '.fabrica-managed-home'), 'account-1\n', 'utf-8')
    writeFileSync(
      join(wslManagedHomePath, 'auth.json'),
      JSON.stringify({
        tokens: {
  ...
  it('recreates the expected missing WSL managed home before reauthenticating', async () => {
    ...
    const spawnMock = vi.fn((command: string, args: string[]) => {
      expect(command).toBe('wsl.exe')
      expect(args).toEqual(buildWslCodexLoginArgs('Ubuntu', wslLinuxHomePath))
      expect(readFileSync(join(wslManagedHomePath, '.fabrica-managed-home'), 'utf-8')).toBe(
        'account-1\n'
      )
      const child = new EventEmitter() as EventEmitter & {
        stdout: PassThrough
    ...
    vi.doMock('node:child_process', () => ({
      execFileSync: vi.fn((_command: string, args: string[]) => {
        ...
        if (script.includes('readlink -f')) {
          ...
          expect(script).toContain(
            'test "$(cat "$candidate_real/.fabrica-managed-home")" = "$expected_marker"'
          )
          return `${wslLinuxHomePath}\n`
        }
        return ''
  ...
  it('keeps Windows and WSL active Codex account selections separate', async () => {
    ...
    const wslManagedHomePath =
      '\\\\wsl.localhost\\Ubuntu\\home\\alice\\.local\\share\\FABRICA\\codex-accounts\\wsl-account\\home'
    const settings = createSettings({
      codexManagedAccounts: [
        {
          id: 'host-account',
  ...
  // Why: quota probes against a cold per-account CODEX_HOME can take 10â€“25s
  // (RPC + PTY fallback) and queue behind an in-flight global usage refresh;
  // account mutations must never block on â€” or fail because of â€” that probe.
  describe('quota refresh decoupling', () => {
    ...
    it('keeps the new account and its managed home when the post-login quota refresh rejects', async () => {
      ...
      // The durable mutation must survive a failed usage probe â€” previously the
...
describe('CodexAccountService.addAccountFromHome', () => {
  ...
  it('registers a managed Codex account by importing an authenticated CODEX_HOME', async () => {
    vi.doMock('../codex-cli/command', () => ({ resolveCodexCommand: () => 'codex' }))
    const sourceHome = mkdtempSync(join(tmpdir(), 'FABRICA-codex-source-'))
    writeFileSync(
  ...
  it('restores settings and runtime selection when post-write activation fails', async () => {
    vi.doMock('../codex-cli/command', () => ({ resolveCodexCommand: () => 'codex' }))
    const sourceHome = mkdtempSync(join(tmpdir(), 'FABRICA-codex-source-rollback-'))
    writeFileSync(
      join(sourceHome, 'auth.json'),
      createCodexAuthJson('new@example.com', 'provider-account-1', 'refresh-token'),
      'utf-8'
  ...
  it('rejects when the source home has no auth.json', async () => {
    vi.doMock('../codex-cli/command', () => ({ resolveCodexCommand: () => 'codex' }))
    const sourceHome = mkdtempSync(join(tmpdir(), 'FABRICA-codex-source-empty-'))
