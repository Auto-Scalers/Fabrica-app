...
/**
 * Proves a host managed home is an FABRICA-owned account directory and cannot
...
export function assertOwnedHostCodexManagedHomePath({
  ...
  const markerPath = join(canonicalCandidate, '.fabrica-managed-home')
  let markerIsRegularFile: boolean
  try {
    markerIsRegularFile = lstatSync(markerPath).isFile()
  } catch (error) {
    throw new Error('Managed Codex home is missing Fabrica ownership marker.', { cause: error })
  }
