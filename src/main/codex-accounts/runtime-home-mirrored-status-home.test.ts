...
beforeEach(() => {
  vi.resetModules()
  testState.userData = mkdtempSync(join(tmpdir(), 'FABRICA-codex-status-home-ud-'))
  testState.home = mkdtempSync(join(tmpdir(), 'FABRICA-codex-status-home-'))
  ...
  for (const key of ['FABRICA_USER_DATA_PATH', 'CODEX_HOME', 'FABRICA_CODEX_HOME']) {
    previousEnv[key] = process.env[key]
    delete process.env[key]
  }
  process.env.FABRICA_USER_DATA_PATH = testState.userData
...
function createManagedAccount(id: string): CodexManagedAccount {
  ...
  writeFileSync(join(home, '.FABRICA-managed-home'), `${id}\n`, 'utf-8')
...
describe('CodexRuntimeHomeService.getMirroredHostHomePathForStatus', () => {
  ...
  it('returns the shared runtime home when a custom CODEX_HOME keeps the mirror lane', async () => {
    ...
    const { CodexRuntimeHomeService } = await import('./runtime-home-service')
    const { getFABRICAManagedCodexHomePath } = await import('../codex/codex-home-paths')
    const service = new CodexRuntimeHomeService(createStore([], null) as never)
    ...
    expect(service.getMirroredHostHomePathForStatus()).toBe(getFABRICAManagedCodexHomePath())
  })
})
