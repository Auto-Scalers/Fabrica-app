...
export class CodexAccountService {
   ...
   * instead of driving `codex login` here. Lets the `FABRICA account add --agent codex`
  ...
  private async persistCapturedCodexAccount(
    ...
    if (!identity.email) {
      throw new Error('Codex login completed, but Fabrica could not resolve the account email.')
    }
  ...
  private createManagedHome(
    ...
    // Why: marker lets future cleanup prove the path belongs to FABRICA before deleting anything.
  ...
  private tryCreateWslManagedHome(
    ...
    const wslLinuxHomePath = `${home.replace(/\/$/, '')}/.local/share/fabrica/codex-accounts/${accountId}/home`
    const markerPath = `${wslLinuxHomePath}/.fabrica-managed-home`
    execFileSync(
      'wsl.exe',
      [
        '-d',
  ...
  private syncCanonicalConfigIntoManagedHome(
    ...
    // Why: FABRICA account switching is meant to swap Codex credentials and quota
    ...
    // Why: source-home FABRICA trust is foreign to each managed home's hooks.json.
  ...
  private readCanonicalConfigForManagedHome(managedHomePath: string): CanonicalCodexConfig | null {
    ...
    const managedRootMarker = '/.local/share/fabrica/codex-accounts/'
  ...
  private assertOAuthAccountAddAllowed(canonicalConfig: CanonicalCodexConfig | null): void {
    ...
    throw new Error(
      `Fabrica cannot add a Codex OAuth account while ~/.codex/config.toml pins the custom provider ${JSON.stringify(modelProvider)}. Keep using the system-default account for this provider, or remove model_provider (or set it to "openai") before adding an OAuth account. Fabrica left your config unchanged.`
    )
  }
  ...
  private recreateExpectedHostManagedHomeForReauthentication(
    ...
    // Why: re-auth may recreate a lost empty home, but only at the exact FABRICA-owned path persisted for this account.
    mkdirSync(expectedManagedHomePath, { recursive: true })
    writeFileSync(join(expectedManagedHomePath, '.fabrica-managed-home'), `${account.id}\n`, 'utf-8')
  ...
  private ensureExpectedWslManagedHomeForReauthentication(
    ...
    if (
      ...
      !wslInfo.linuxPath.endsWith(`/.local/share/fabrica/codex-accounts/${account.id}/home`)
    ) {
      return
    }
      ...
      [
        ...
        buildEncodedWslBashCommand(
          [
            ...
            'marker="$candidate/.fabrica-managed-home"',
  ...
  private assertManagedHomePath(candidatePath: string, expectedAccountId?: string): string {
    ...
    if (wslInfo) {
      if (
        !wslInfo.linuxPath.includes('/.local/share/fabrica/codex-accounts/') ||
        !wslInfo.linuxPath.endsWith('/home')
      ) {
        throw new Error('Managed WSL Codex home is outside Fabrica account storage.')
      }
      if (
        expectedAccountId !== undefined &&
        !wslInfo.linuxPath.endsWith(`/.local/share/fabrica/codex-accounts/${expectedAccountId}/home`)
      ) {
            ...
            [
              ...
              buildEncodedWslBashCommand(
                [
                  'set -euo pipefail',
                  `candidate=${shellQuote(wslInfo.linuxPath)}`,
                  'managed_root="${HOME%/}/.local/share/fabrica/codex-accounts"',
                  'candidate_real=$(readlink -f -- "$candidate")',
                  'managed_root_real=$(readlink -f -- "$managed_root")',
                  'test -f "$candidate_real/.fabrica-managed-home"',
                  ...(expectedAccountId === undefined
                    ? [
      ...
      if (!existsSync(join(candidatePath, '.fabrica-managed-home'))) {
        throw new Error('Managed Codex home is missing Fabrica ownership marker.')
      }
      if (
        expectedAccountId !== undefined &&
        readFileSync(join(candidatePath, '.fabrica-managed-home'), 'utf-8').trim() !==
          expectedAccountId
      ) {
