...
beforeEach(() => {
  vi.resetModules()
  testState.userData = mkdtempSync(join(tmpdir(), 'FABRICA-codex-e-migration-'))
  testState.home = mkdtempSync(join(tmpdir(), 'FABRICA-codex-e-home-'))
  for (const key of [
    'FABRICA_USER_DATA_PATH',
    'FABRICA_DISABLE_CODEX_TRUST_RPC',
    'CODEX_HOME',
    'FABRICA_CODEX_HOME'
  ]) {
...
describe('CodexRuntimeHomeService per-account takeover composition', () => {
  ...
  it('does not expose an untrusted persisted home through rollout discovery', async () => {
    ...
    writeFileSync(join(outsideHome, '.fabrica-managed-home'), 'account-1\n', 'utf-8')
    writeFileSync(
