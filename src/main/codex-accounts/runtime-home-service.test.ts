...
const testState = {
  ...
  previousFabricaCodexHome: undefined as string | undefined,
...
function createSettings(overrides: TestSettingsOverrides = {}): GlobalSettings {
  ...
  return {
    ...
    terminalThemeDark: 'FABRICA-dark',
    terminalDividerColorDark: '#000000',
    terminalUseSeparateLightTheme: false,
    terminalThemeLight: 'FABRICA-light',
...
function createManagedAuth(rootDir: string, accountId: string, auth: string): string {
  ...
  writeFileSync(join(managedHomePath, '.fabrica-managed-home'), `${accountId}\n`, 'utf-8')
...
describe('CodexRuntimeHomeService', () => {
  beforeEach(() => {
    ...
    testState.userDataDir = mkdtempSync(join(tmpdir(), 'FABRICA-runtime-home-'))
    testState.fakeHomeDir = mkdtempSync(join(tmpdir(), 'FABRICA-codex-home-'))
    testState.previousUserDataPath = process.env.FABRICA_USER_DATA_PATH
    testState.previousCodexHome = process.env.CODEX_HOME
    testState.previousFabricaCodexHome = process.env.FABRICA_CODEX_HOME
    delete process.env.CODEX_HOME
    delete process.env.FABRICA_CODEX_HOME
    process.env.FABRICA_USER_DATA_PATH = testState.userDataDir
  ...
  afterEach(() => {
    ...
    if (testState.previousUserDataPath === undefined) {
      ...
      process.env.FABRICA_USER_DATA_PATH = testState.previousUserDataPath
    }
    ...
    if (testState.previousFabricaCodexHome === undefined) {
      delete process.env.FABRICA_CODEX_HOME
    } else {
      process.env.FABRICA_CODEX_HOME = testState.previousFabricaCodexHome
    }
  })
  ...
  it('uses the canonical Electron userData for legacy active host migration', async () => {
    const staleUserDataDir = mkdtempSync(join(tmpdir(), 'FABRICA-stale-runtime-home-'))
    const staleRuntimeHomePath = join(staleUserDataDir, 'codex-runtime-home', 'home')
    try {
      mkdirSync(staleRuntimeHomePath, { recursive: true })
      process.env.FABRICA_USER_DATA_PATH = staleUserDataDir
      const legacyLaunchHomePath = join(
        testState.userDataDir,
        'codex-runtime-home',
        'launch',
      ...
      const { configureFABRICAUserDataPathEnv } = await import('../startup/configure-process')
      configureFABRICAUserDataPathEnv()
      const { CodexRuntimeHomeService } = await import('./runtime-home-service')
      new CodexRuntimeHomeService(store as never)

      expect(process.env.FABRICA_USER_DATA_PATH).toBe(testState.userDataDir)
      expect(normalizeLinkTarget(readlinkSync(legacyActiveHomePath))).toBe(
        normalizeLinkTarget(getRuntimeCodexHomePath())
      )
  ...
  it('builds a valid WSL legacy active-home migration shell command', async () => {
    ...
    try {
      ...
      service.migrateLegacyWslActiveHomePointer(
        'Ubuntu',
        '\\\\wsl.localhost\\Ubuntu\\home\\alice\\.local\\share\\FABRICA\\codex-runtime-home\\home'
      )
      ...
      expect(shellCommand).toContain(
        "if [ ! -e '/home/alice/.local/share/FABRICA/codex-runtime-home/active/wsl/home' ] && [ ! -L '/home/alice/.local/share/FABRICA/codex-runtime-home/active/wsl/home' ]; then :"
      )
      expect(shellCommand).toContain(
        "elif [ -e '/home/alice/.local/share/FABRICA/codex-runtime-home/active/wsl/home' ] && [ ! -L '/home/alice/.local/share/FABRICA/codex-runtime-home/active/wsl/home' ]; then :"
      )
      expect(shellCommand).toContain(
        "mkdir -p '/home/alice/.local/share/FABRICA/codex-runtime-home/active/wsl'"
      )
      expect(shellCommand).toContain(
        "ln -s -- '/home/alice/.local/share/FABRICA/codex-runtime-home/home' '/home/alice/.local/share/FABRICA/codex-runtime-home/active/wsl/home.next-"
      )
      expect(shellCommand).toContain(
        "mv -Tf -- '/home/alice/.local/share/FABRICA/codex-runtime-home/active/wsl/home.next-"
      )
      expect(shellCommand).toContain(
        "' '/home/alice/.local/share/FABRICA/codex-runtime-home/active/wsl/home'"
      )
  ...
  it('promotes WSL in-Codex setting changes on the next Codex launch', async () => {
    ...
    try {
      ...
      const baselinePath = join(wslRuntimeHomePath, '.fabrica-config-settings-baseline.json')
      expect(existsSync(baselinePath)).toBe(true)

      // A direct WSL Codex edit wins and is mirrored into FABRICA's runtime before
      // the baseline advances, so later in-FABRICA changes remain promotable.
      ...
      // Codex now persists a /model change inside FABRICA's reconciled runtime.
      writeFileSync(
        runtimeConfigPath,
    ...
    vi.doMock('../../shared/wsl-paths', () => ({
      parseWslUncPath: (candidate: string) =>
        candidate === wslRuntimeHomePath
          ? {
              distro: 'Debian',
              linuxPath: '/home/alice/.local/share/FABRICA/codex-runtime-home/home'
            }
          : null
    }))
        ...
        codexManagedAccounts: [
          {
            ...
            wslDistro: 'Debian',
            wslLinuxHomePath: '/home/alice/.local/share/FABRICA/codex-accounts/debian/home',
  ...
  it('returns the Fabrica-managed runtime home for Codex launch and rate-limit preparation', async () => {
  ...
  it('routes host system default to the real home', async () => {
    ...
    process.env.FABRICA_CODEX_HOME = getRuntimeCodexHomePath()
    try {
      // Background fetchers prefer ambient CODEX_HOME when passed null, so an
      // explicit path proves nested FABRICA launches cannot poll the managed home.
  ...
  it('resolves only FABRICA-owned homes used by live retained host shells', async () => {
    ...
    writeFileSync(join(unownedHome, '.fabrica-managed-home'), 'account-2\n', 'utf-8')
    writePaneRegistry({
  ...
  it('recovers runtime auth quarantined by an interrupted guarded update', async () => {
    ...
    new CodexRuntimeHomeService(store as never)
    const runtimeAuthPath = getRuntimeCodexAuthPath()
    const heldAuthPath = `${runtimeAuthPath}.FABRICA-guarded`
  ...
  it('preserves a managed selection whose auth.json is temporarily missing', async () => {
    ...
    writeFileSync(join(brokenHome, '.fabrica-managed-home'), 'account-1\n', 'utf-8')
    const settings = createSettings({
      shellStartupEnvProbeSupported: true,
      codexManagedAccounts: [
        {
    ...
    const settings = createSettings({
      ...
      codexManagedAccounts: [
        ...
        },
        {
          ...
          managedHomeRuntime: 'wsl',
          wslDistro: 'Ubuntu',
          wslLinuxHomePath: '/home/alice/.local/share/FABRICA/codex-accounts/wsl-account/home',
          createdAt: 2,
          updatedAt: 2,
          lastAuthenticatedAt: 2
        }
      ...
      codexManagedAccounts: [
        {
          ...
          managedHomeRuntime: 'wsl',
          wslDistro: 'Ubuntu',
          wslLinuxHomePath: '/home/alice/.local/share/FABRICA/codex-accounts/account-1/home',
          providerAccountId: null,
          workspaceLabel: null,
          workspaceAccountId: null,
          createdAt: 1,
      ...
      createSettings({
        codexManagedAccounts: [
          ...
          },
          {
            ...
            managedHomeRuntime: 'wsl',
            wslDistro: 'Ubuntu',
            wslLinuxHomePath: '/home/alice/.local/share/FABRICA/codex-accounts/account-2/home',
        ...
        codexManagedAccounts: [
          {
            ...
            managedHomeRuntime: 'wsl',
            wslDistro: 'Ubuntu',
            wslLinuxHomePath: '/home/alice/.local/share/FABRICA/codex-accounts/ubuntu/home',
  ...
  it('persists runtime auth refreshes after returning to system default', async () => {
    ...
    // Codex used to refresh tokens directly in ~/.codex. With an FABRICA-owned
  ...
  it('restores system default when unverified runtime auth appears before deselect', async () => {
    ...
    // A stale or external process overwrites runtime with auth FABRICA cannot
  ...
  it('preserves conflicting legacy session files under deterministic names', async () => {
    ...
    expect(
      readFileSync(join(runtimeSessionsDir, 'session.FABRICA-legacy-account-1.json'), 'utf-8')
    ).toBe('{"turns":[1,2]}')
    expect(
      readFileSync(
        join(runtimeSessionsDir, 'nested', 'session.FABRICA-legacy-account-1.json'),
        'utf-8'
      )
    ).toBe('{"turns":[2,3]}')
