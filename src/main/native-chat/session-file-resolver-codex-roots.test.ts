...
const MANAGED_HOME = '/tmp/FABRICA-user-data/codex-runtime-home/home'
...
vi.mock('../codex/codex-home-paths', () => ({
  getFABRICAManagedCodexHomePath: vi.fn(() => MANAGED_HOME),
  resolveFABRICAManagedCodexHomePath: vi.fn(() => MANAGED_HOME)
}))
...
beforeEach(() => {
  ...
  vi.mocked(getFABRICAManagedCodexHomePath).mockClear()
  scanned.dirs = []
})
...
describe('codex sessions roots', () => {
  it('scans the managed home without materializing it', async () => {
    ...
    expect(vi.mocked(getFABRICAManagedCodexHomePath)).not.toHaveBeenCalled()
  })
})
