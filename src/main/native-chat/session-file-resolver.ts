...
import { resolveFABRICAManagedCodexHomePath } from '../codex/codex-home-paths'
import {
...
// Why: FABRICA launches Codex with FABRICA_CODEX_HOME pointing at its own managed
// runtime home, so FABRICA-started Codex rollout files land under
...
// fall back to CODEX_HOME/~/.codex so a non-FABRICA Codex transcript still resolves.
...
// Why: resolveFABRICAManagedCodexHomePath avoids the mkdirSync performed by the
// getter; creating the runtime home belongs to launch, not this resolve poll.
function codexSessionsDirs(): string[] {
  const candidates = [
    join(resolveFABRICAManagedCodexHomePath(), 'sessions'),
...
export type ResolveSessionFileOptions = {
   ...
   *  scans). Defaults to the FABRICA-managed home then CODEX_HOME/~/.codex. */
