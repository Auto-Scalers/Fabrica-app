...
function buildAtomFeed(tags: string[]): string {
  return `<?xml version="1.0" encoding="UTF-8"?><feed>${tags
    .map(
      (tag) =>
        `<entry><link rel="alternate" type="text/html" href="https://github.com/Auto-Scalers/Fabrica-app/releases/tag/${tag}"/><title>${tag}</title></entry>`
    )
    .join('')}</feed>`
}
...
function buildManifest(tag: string): string {
  ...
  return [
    `version: ${version}`,
    'files:',
    `  - url: FABRICA-${version}-arm64-mac.zip`,
    '    sha512: test',
    `path: FABRICA-${version}-arm64-mac.zip`
  ].join('\n')
}
...
function respondWithAtom(
  ...
  netFetchMock.mockImplementation((url: string, init?: { method?: string }) => {
    if (url === 'https://github.com/Auto-Scalers/Fabrica-app/releases.atom') {
...
describe('fetchNewerReleaseTagsWithReadiness', () => {
  ...
  it('requires every asset referenced by the manifest files list to be reachable', async () => {
    netFetchMock.mockImplementation((url: string, init?: { method?: string }) => {
          ...
          text: () =>
            Promise.resolve(
              [
                `version: ${version}`,
                'files:',
                '  - url: fabrica-windows-setup.exe',
                '    sha512: test',
                `  - url: FABRICA-${version}-mac.zip`,
                '    sha512: test',
                `path: FABRICA-${version}-mac.zip`
              ].join('\n')
            )
        })
      }
      ...
      if (init?.method === 'HEAD') {
        ...
        const unavailable = latest && url.endsWith('/FABRICA-1.4.28-mac.zip')
        const missing = latest && url.endsWith('/fabrica-windows-setup.exe')
        return Promise.resolve({
  ...
  it('treats an explicit asset 404 as not-ready when another asset is unavailable', async () => {
    netFetchMock.mockImplementation((url: string, init?: { method?: string }) => {
      ...
      if (init?.method === 'HEAD') {
        const isWindowsAsset = url.endsWith('/fabrica-windows-setup.exe')
        return Promise.resolve({
          ok: false,
  ...
  it('accepts absolute manifest asset URLs without rewriting them to release asset paths', async () => {
          ...
          text: () =>
            Promise.resolve(
              [
                'version: 1.4.27',
                'files:',
                '  - url: https://downloads.example.com/FABRICA-1.4.27-arm64-mac.zip',
                '    sha512: test'
              ].join('\n')
            )
        })
    ...
    expect(assetUrls).toEqual(['https://downloads.example.com/FABRICA-1.4.27-arm64-mac.zip'])
  })
