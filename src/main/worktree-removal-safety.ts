...
const FABRICA_CREATION_SOURCES = new Set<NonNullable<WorktreeMeta['FABRICACreationSource']>>([
  'desktop',
  'runtime',
  'cli',
  'ssh'
])
const FABRICA_OWNED_PROVENANCE_META_KEYS = [
  'FABRICACreatedAt',
  'FABRICACreationSource',
  'FABRICACreationWorkspaceLayout',
  'automationProvenance',
  'cliProvenance',
  'creatorProvenance'
] as const
type UnregisteredFABRICACleanupMeta = Pick<
  WorktreeMeta,
  | 'FABRICACreatedAt'
  | 'FABRICACreationSource'
  | 'createdAt'
...
export function canCleanupUnregisteredFABRICAWorktreeDirectory(args: {
  meta: UnregisteredFABRICACleanupMeta | null | undefined
}): boolean {
  if (hasCurrentFABRICACreationProvenance(args.meta)) {
    return true
  }

  if (hasLegacyFABRICACreationEvidence(args.meta)) {
    return true
  }

  // Why: path shape alone is not authority; users can create plain Git
  // worktrees inside FABRICA's workspace directory too.
  return false
}

export async function canCleanupUnregisteredFABRICALeftoverDirectory(args: {
  meta: UnregisteredFABRICACleanupMeta | null | undefined
  worktreePath: string
  ...
  // ownership for recursive deletion; require persisted FABRICA-created evidence.
  if (!hasCurrentFABRICACreationProvenance(args.meta) && !hasLegacyFABRICACreationEvidence(args.meta)) {
    return false
  }
...
function hasCurrentFABRICACreationProvenance(
  meta: Pick<WorktreeMeta, 'FABRICACreatedAt' | 'FABRICACreationSource'> | null | undefined
): boolean {
  return (
    typeof meta?.FABRICACreatedAt === 'number' &&
    !!meta.FABRICACreationSource &&
    FABRICA_CREATION_SOURCES.has(meta.FABRICACreationSource)
  )
}

function hasLegacyFABRICACreationEvidence(
  meta: UnregisteredFABRICACleanupMeta | null | undefined
): boolean {
  return Boolean(
    meta?.createdAt ||
...
export function stripFABRICAProvenanceMetaUpdates(
  updates: Partial<WorktreeMeta> | null | undefined
): Partial<WorktreeMeta> {
  const sanitized = { ...updates }
  for (const key of FABRICA_OWNED_PROVENANCE_META_KEYS) {
