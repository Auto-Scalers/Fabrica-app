...
const {
  ...
  checkFABRICAStarredMock: vi.fn(),
  starFABRICAMock: vi.fn(),
  trackMock: vi.fn(),
...
vi.mock('../github/client', () => ({
  checkFABRICAStarred: checkFABRICAStarredMock,
  starFABRICA: starFABRICAMock
}))
...
describe('StarNagService', () => {
  ...
  beforeEach(() => {
    ...
    checkFABRICAStarredMock.mockReset()
    checkFABRICAStarredMock.mockResolvedValue(false)
    starFABRICAMock.mockReset()
    starFABRICAMock.mockResolvedValue(true)
    trackMock.mockReset()
  ...
  it('shows the browser fallback when checkFABRICAStarred cannot determine star state', async () => {
    const window = createWindow()
    browserWindowMock.getAllWindows.mockReturnValue([window])
    checkFABRICAStarredMock.mockResolvedValue(null)
    const { service, emitAgentStarted } = createHarness()
  ...
  it('does not log a threshold exposure when checkFABRICAStarred returns true', async () => {
    const window = createWindow()
    browserWindowMock.getAllWindows.mockReturnValue([window])
    checkFABRICAStarredMock.mockResolvedValue(true)
    const { service, emitAgentStarted } = createHarness()
  ...
  it('hides a superseded visible card when onboarding completion detects an existing star', async () => {
    const window = createWindow()
    browserWindowMock.getAllWindows.mockReturnValue([window])
    checkFABRICAStarredMock.mockResolvedValueOnce(true)
    const { service, ui } = createHarness()
  ...
  it('queues onboarding completed while a threshold star check is in flight', async () => {
    ...
    checkFABRICAStarredMock.mockReturnValueOnce(deferredStarCheck.promise).mockResolvedValueOnce(null)
    const { service, emitAgentStarted, ui } = createHarness()
  ...
  it('keeps threshold source when force_show is requested during a successful threshold evaluation', async () => {
    ...
    browserWindowMock.getAllWindows.mockReturnValue([window])
    const deferredStarCheck = createDeferred<boolean | null>()
    checkFABRICAStarredMock.mockReturnValue(deferredStarCheck.promise)
    const { service, emitAgentStarted } = createHarness()
  ...
  it('does not replay a stale queued force_show after threshold delivery wins', async () => {
    ...
    checkFABRICAStarredMock.mockReturnValueOnce(firstStarCheck.promise).mockResolvedValue(null)
    const { service, emitAgentStarted } = createHarness()
  ...
  it('emits direct-star attempted and succeeded outcomes plus app_starred_FABRICA', async () => {
    const window = createWindow()
    browserWindowMock.getAllWindows.mockReturnValue([window])
    const { service, ui } = createHarness()
    ...
    service.registerIpcHandlers()
    getIpcHandler('star-nag:forceShow')()
    const ok = await getIpcHandler('star-nag:starFABRICA')()
    ...
    expect(trackMock).toHaveBeenCalledWith('app_starred_FABRICA', {
      source: 'star_nag',
      nth_repo_added: 3
    })
  })
  ...
  it('uses the source moment for confirmed direct-star success telemetry', async () => {
    ...
    service.registerIpcHandlers()
    await getIpcHandler('star-nag:onboardingCompleted')()
    await getIpcHandler('star-nag:starFABRICA')()
  ...
  it('uses fresh cohort context for canonical app_starred_FABRICA success telemetry', async () => {
    const window = createWindow()
    browserWindowMock.getAllWindows.mockReturnValue([window])
    getCohortAtEmitMock
  ...
  it('records success and completion when direct star resolves after dismissal cleared the visible session', async () => {
    ...
    starFABRICAMock.mockReturnValue(deferredStar.promise)
    const { service, ui } = createHarness()

    service.registerIpcHandlers()
    getIpcHandler('star-nag:forceShow')()
    const starPromise = getIpcHandler('star-nag:starFABRICA')()
    getIpcHandler('star-nag:dismiss')()
  ...
  it('clears the in-flight direct-star guard after thrown attempts so the user can retry', async () => {
    const window = createWindow()
    browserWindowMock.getAllWindows.mockReturnValue([window])
    starFABRICAMock.mockRejectedValueOnce(new Error('gh failed')).mockResolvedValueOnce(true)
    const { service, ui } = createHarness()

    service.registerIpcHandlers()
    getIpcHandler('star-nag:forceShow')()
    const starFromNag = getIpcHandler('star-nag:starFABRICA')

    await expect(starFromNag()).rejects.toThrow('gh failed')
    await expect(starFromNag()).resolves.toBe(true)

    expect(starFABRICAMock).toHaveBeenCalledTimes(2)
    expect(ui.starNagCompleted).toBe(true)
  })
