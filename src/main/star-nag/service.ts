...
import { checkFABRICAStarred } from '../github/client'
...
export class StarNagService {
  ...
  registerIpcHandlers(): void {
    ...
    ipcMain.handle('star-nag:starFABRICA', () => this.starFABRICAFromNag())
  ...
  private async maybeShow(
    ...
    try {
      // Why: checkFABRICAStarred lets us skip users who already starred outside
      ...
      // button.
      const starred = await checkFABRICAStarred()
  ...
  private async starFABRICAFromNag(): Promise<boolean> {
    const session = this.promptSession
    if (!session) {
      return false
    }
    ...
    const attempt = this.runStarFABRICAAttempt(session)
  ...
  private async runStarFABRICAAttempt(session: StarNagPromptSession): Promise<boolean> {
