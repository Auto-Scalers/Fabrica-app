...
describe('SshGitProvider', () => {
  ...
  it('reports an actionable reconnect message when the relay lacks submodule status', async () => {
    ...
    await expect(provider.getSubmoduleStatus('/home/user/repo', 'vendor/lib')).rejects.toThrow(
      'SSH submodule diff support is unavailable on this relay. Reconnect the SSH target to update FABRICA on the host, then try again.'
    )
  })
  ...
  it('reports an actionable reconnect message when the relay does not support cloning', async () => {
    ...
    await expect(
      ...
      'SSH clone support is unavailable on this relay. Reconnect the SSH target to update FABRICA on the host, then try again.'
    )
  })
  ...
  it('execNonInteractive forwards environment variables to the relay', async () => {
    ...
    await provider.execNonInteractive(
      '/bin/bash',
      ['-lc', 'echo "$FABRICA_WORKTREE_PATH"'],
      '/home/user/repo',
      120_000,
      undefined,
      {
        FABRICA_ROOT_PATH: '/home/user/repo',
        FABRICA_WORKTREE_PATH: '/home/user/repo-feature'
      }
    )
    ...
    expect(mux.request).toHaveBeenCalledWith(
      ...
      {
        binary: '/bin/bash',
        args: ['-lc', 'echo "$FABRICA_WORKTREE_PATH"'],
        cwd: '/home/user/repo',
        stdin: null,
        timeoutMs: 120_000,
        env: {
  ...
  it('syncForkDefaultBranch sends git.forkSync request', async () => {
    ...
    const expectedUpstream = { owner: 'Auto-Scalers', repo: 'fabrica' }
  ...
  it('fetchGitLabMergeRequestHead sends the durable-ref git.fetchGitLabMergeRequestHeadRef request', async () => {
    mux.request.mockResolvedValueOnce({
      localRef: 'refs/FABRICA/merge-requests/origin-abc/42'
    })
    ...
    expect(localRef).toBe('refs/FABRICA/merge-requests/origin-abc/42')
  })
  ...
  it('fetchGitLabMergeRequestHead maps old relays to the reconnect message', async () => {
    ...
    await expect(
      ...
      'This SSH host is running an older FABRICA relay that cannot fetch merge request heads. Reconnect to deploy the latest relay, then try again.'
    )
  })
  ...
  it('fetchGitHubPullRequestHead sends git.fetchGitHubPullRequestHead request', async () => {
    mux.request.mockResolvedValueOnce({ localRef: 'refs/FABRICA/pull/origin-abc/42' })
    ...
    expect(localRef).toBe('refs/FABRICA/pull/origin-abc/42')
  })
  ...
  it('fetchGitHubPullRequestHead maps old relays to the reconnect message', async () => {
    ...
    await expect(
      ...
      'This SSH host is running an older FABRICA relay that cannot fetch pull request heads. Reconnect to deploy the latest relay, then try again.'
    )
  })
  ...
  it('forceDeletePreservedBranch maps old relays to the reconnect message', async () => {
    ...
    await expect(
      ...
      'This SSH host is running an older FABRICA relay that cannot delete preserved branches. Reconnect to deploy the latest relay, then try again.'
    )
  })
