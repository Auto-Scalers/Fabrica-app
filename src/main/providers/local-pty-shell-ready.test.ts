...
// Why: can't import electron (bundled into the plain-node daemon-entry fork), so tests set the wrapper root via FABRICA_USER_DATA_PATH instead of mocking app.
function setTestUserDataPath(path: string): void {
  process.env.FABRICA_USER_DATA_PATH = path
}

const ORIGINAL_FABRICA_USER_DATA_PATH = process.env.FABRICA_USER_DATA_PATH

afterEach(() => {
  if (ORIGINAL_FABRICA_USER_DATA_PATH === undefined) {
    delete process.env.FABRICA_USER_DATA_PATH
  } else {
    process.env.FABRICA_USER_DATA_PATH = ORIGINAL_FABRICA_USER_DATA_PATH
  }
})
...
describe('scanForShellReady', () => {
  it('flushes marker-like output when the full marker is not BEL-terminated', () => {
    ...
    expect(scanForShellReady(state, 'before \x1b]777;FABRICA-shell-readyx')).toEqual({
      output: 'before \x1b]777;FABRICA-shell-readyx',
      matched: false,
  ...
  it('reports post-marker bytes only when bytes follow the BEL terminator in the matching call', () => {
    ...
    expect(scanForShellReady(state, 'before \x1b]777;FABRICA-shell-ready\x07')).toEqual({
      output: 'before ',
      matched: true,
      postMarkerBytesObserved: false
    })
    ...
    state = createShellReadyScanState()
    expect(scanForShellReady(state, 'before \x1b]777;FABRICA-shell-ready')).toEqual({
      output: 'before ',
      matched: false,
      postMarkerBytesObserved: false
    })
    ...
    state = createShellReadyScanState()
    expect(scanForShellReady(state, '\x1b]777;FABRICA-shell-ready')).toEqual({
      output: '',
      matched: false,
      postMarkerBytesObserved: false
    })
...
describe('shell-ready wrapper root resolution', () => {
  // Why: daemon-entry fork is plain Node (no electron), so the wrapper root resolves from FABRICA_USER_DATA_PATH, not app.getPath.
  it('resolves the wrapper root from FABRICA_USER_DATA_PATH', async () => {
    const root = mkdtempSync(join(tmpdir(), 'FABRICA-userdata-env-'))
    try {
...
function expectZdotdirSourceContext(content: string, fileName: '.zprofile' | '.zshrc' | '.zlogin') {
  expect(content).toContain('export ZDOTDIR="$_FABRICA_home"')
  expect(content).toContain(`source "$_FABRICA_home/${fileName}"`)
  expect(content).toContain('export ZDOTDIR="$_FABRICA_wrapper_zdotdir"')
}

function expectFinalZdotdirRestoreContext(content: string) {
  expect(content).toContain("after FABRICA's last wrapper file has loaded")
  expect(content).toContain('export ZDOTDIR="$_FABRICA_home"')
}

describePosix('local PTY shell-ready launch config', () => {
  let userDataPath: string
  let previousFABRICAOrigZdotdir: string | undefined

  beforeEach(() => {
    previousFABRICAOrigZdotdir = process.env.FABRICA_ORIG_ZDOTDIR
  ...
  afterEach(() => {
    if (previousFABRICAOrigZdotdir === undefined) {
      delete process.env.FABRICA_ORIG_ZDOTDIR
    } else {
      process.env.FABRICA_ORIG_ZDOTDIR = previousFABRICAOrigZdotdir
    }
  ...
  it('wraps fish launches with a fish_prompt shell-ready marker init command', async () => {
    ...
    expect(config.env).toEqual({ FABRICA_SHELL_READY_MARKER: '1' })
    ...
    expect(init).toContain('builtin printf "\\033]777;FABRICA-shell-ready\\007"')
    expect(init).toContain('functions -e __FABRICA_shell_ready_marker')
  })
  ...
  it('falls back to HOME for FABRICA_ORIG_ZDOTDIR when inherited ZDOTDIR points at a wrapper dir', async () => {
    // Why: mirrors the daemon path — guards the same zsh recursion loop for renderer/local PTYs spawned inside an FABRICA terminal.
    const previousZdotdir = process.env.ZDOTDIR
    const previousHome = process.env.HOME
    process.env.ZDOTDIR = '/some/other/FABRICA/shell-ready/zsh'
    process.env.HOME = '/Users/alice'
    try {
      const { getShellReadyLaunchConfig } = await importFreshLocalPtyShellReady()
      const config = getShellReadyLaunchConfig('/bin/zsh')
      expect(config.env.FABRICA_ORIG_ZDOTDIR).toBe('/Users/alice')
      expect(config.env.FABRICA_ZSHENV_SOURCE_DIR).toBe('/Users/alice')
    } finally {
  ...
  it('uses inherited FABRICA_ORIG_ZDOTDIR when ZDOTDIR is an FABRICA wrapper dir', async () => {
    const previousZdotdir = process.env.ZDOTDIR
    const previousOrigZdotdir = process.env.FABRICA_ORIG_ZDOTDIR
    const previousHome = process.env.HOME
    process.env.ZDOTDIR = '/some/other/FABRICA/shell-ready/zsh'
    process.env.FABRICA_ORIG_ZDOTDIR = '/Users/alice/.config/zsh'
    process.env.HOME = '/Users/alice'
    try {
      const { getShellReadyLaunchConfig } = await importFreshLocalPtyShellReady()
      const config = getShellReadyLaunchConfig('/bin/zsh')
      expect(config.env.FABRICA_ORIG_ZDOTDIR).toBe('/Users/alice/.config/zsh')
      expect(config.env.FABRICA_ZSHENV_SOURCE_DIR).toBe('/Users/alice')
    } finally {
      if (previousZdotdir === undefined) {
        delete process.env.ZDOTDIR
      ...
      if (previousOrigZdotdir === undefined) {
        ...
        process.env.FABRICA_ORIG_ZDOTDIR = previousOrigZdotdir
      }
      if (previousHome === undefined) {
        delete process.env.HOME
      } else {
  ...
  it('falls back to HOME when inherited FABRICA_ORIG_ZDOTDIR points at a wrapper dir', async () => {
    const previousZdotdir = process.env.ZDOTDIR
    const previousOrigZdotdir = process.env.FABRICA_ORIG_ZDOTDIR
    const previousHome = process.env.HOME
    delete process.env.ZDOTDIR
    process.env.FABRICA_ORIG_ZDOTDIR = '/some/other/FABRICA/shell-ready/zsh'
    process.env.HOME = '/Users/alice'
    try {
      const { getShellReadyLaunchConfig } = await importFreshLocalPtyShellReady()
      const config = getShellReadyLaunchConfig('/bin/zsh')
  ...
  it('writes zsh wrappers that guard against FABRICA_ORIG_ZDOTDIR self-loops', async () => {
    const { getShellReadyLaunchConfig } = await importFreshLocalPtyShellReady()
    ...
    expect(zshenv).toContain('_FABRICA_user_zdotdir="${_FABRICA_spawn_orig_zdotdir:-$HOME}"')
    expect(zshenv).toContain('*/shell-ready/zsh) _FABRICA_user_zdotdir="$HOME" ;;')
    expect(zshenv).toContain('""|*/shell-ready/zsh) export FABRICA_ORIG_ZDOTDIR="$HOME" ;;')
  ...
  it('owns zle-line-init for the shell-ready marker instead of an azhw hook', async () => {
    ...
    const zlogin = readFileSync(join(userDataPath, 'shell-ready', 'zsh', '.zlogin'), 'utf8')
    expect(zlogin).toContain('zle -N zle-line-init __FABRICA_prompt_mark')
    expect(zlogin).toContain('__FABRICA_prev_line_init_fn="${widgets[zle-line-init]#user:}"')
    expect(zlogin).toContain('printf "\\033]777;FABRICA-shell-ready\\007"')
    ...
    expect(zlogin).toContain('== "user:__FABRICA_prompt_mark"')
  })
  ...
  it('writes wrappers without restoring Pi/OMP homes after user startup files', async () => {
    ...
    const agentTeamsPathRestoreLine = '[[ -n "${FABRICA_AGENT_TEAMS_SHIM_DIR:-}" ]] || return 0'
    const ompWrapperLine = 'command omp --extension "${FABRICA_OMP_STATUS_EXTENSION}" "$@"'
    ...
    expect(zshrc).not.toContain('FABRICA_PI_CODING_AGENT_DIR')
    expect(zlogin).not.toContain('FABRICA_PI_CODING_AGENT_DIR')
    expect(bashRc).not.toContain('FABRICA_PI_CODING_AGENT_DIR')
    ...
    expect(zshrc).not.toContain('FABRICA_OMP_CODING_AGENT_DIR')
    expect(zlogin).not.toContain('FABRICA_OMP_CODING_AGENT_DIR')
    expect(bashRc).not.toContain('FABRICA_OMP_CODING_AGENT_DIR')
  ...
  it('emits OSC 133 C/D markers in the bash wrapper so agent exit cleanup fires', async () => {
    ...
    expect(bashRc.indexOf("trap '__FABRICA_osc133_preexec' DEBUG")).toBeGreaterThan(
      bashRc.indexOf('if [[ "${FABRICA_SHELL_READY_MARKER:-0}" == "1" ]]; then')
    )
  ...
  it('preserves a real inherited ZDOTDIR as FABRICA_ORIG_ZDOTDIR', async () => {
    const previousZdotdir = process.env.ZDOTDIR
    ...
    try {
      ...
      const config = getShellReadyLaunchConfig('/bin/zsh')
      expect(config.env.FABRICA_ORIG_ZDOTDIR).toBe('/Users/alice/.config/zsh')
      expect(config.env.FABRICA_ZSHENV_SOURCE_DIR).toBe('/Users/alice/.config/zsh')
    } finally {
      if (previousZdotdir === undefined) {
        delete process.env.ZDOTDIR
      } else {
  ...
  it('preserves ZDOTDIR that contains /shell-ready/zsh as a substring but does not end with it', async () => {
    ...
    try {
      const { getShellReadyLaunchConfig } = await importFreshLocalPtyShellReady()
      const config = getShellReadyLaunchConfig('/bin/zsh')
      expect(config.env.FABRICA_ORIG_ZDOTDIR).toBe('/Users/alice/shell-ready/zsh-custom')
    } finally {
      if (previousZdotdir === undefined) {
        delete process.env.ZDOTDIR
      } else {
  ...
  it('sources user .zshenv at wrapper top level before repinning ZDOTDIR', async () => {
    ...
    expect(zshenv).toContain('_FABRICA_zshenv_source_dir="${FABRICA_ZSHENV_SOURCE_DIR:-$HOME}"')
    expect(zshenv).toContain('source "${_FABRICA_zshenv_path}"')
    expect(zshenv).toContain('_FABRICA_discovered_zdotdir="${ZDOTDIR:-}"')
  ...
  it('preserves spawn-env FABRICA_ORIG_ZDOTDIR as fallback when discovery yields nothing', async () => {
    // Why: if user .zshenv returns early or doesn't set ZDOTDIR, fall back to spawn-env FABRICA_ORIG_ZDOTDIR, then HOME.
    const { getShellReadyLaunchConfig } = await importFreshLocalPtyShellReady()
    ...
    expect(zshenv).toContain('_FABRICA_spawn_orig_zdotdir="${FABRICA_ORIG_ZDOTDIR:-}"')

    // Fallback chain: discovered → normalized spawn-env path → HOME
    expect(zshenv).toContain('${_FABRICA_discovered_zdotdir:-${_FABRICA_user_zdotdir:-$HOME}}')
  })
  ...
  it('restores wrapper ZDOTDIR from the runtime sourced path, not the baked literal', async () => {
    ...
    expect(zshenv).toContain('_FABRICA_wrapper_zdotdir_self="${${(%):-%x}:h}"')
    ...
    expect(zshenv.indexOf('_FABRICA_wrapper_zdotdir_self="${${(%):-%x}:h}"')).toBeLessThan(
...
describePosix('live zsh subprocess tests', () => {
  ...
  describeIfZsh('ZDOTDIR discovery with real zsh', () => {
    ...
    beforeEach(async () => {
      testHome = mkdtempSync(join(tmpdir(), 'FABRICA-zsh-test-home-'))
      userDataPath = mkdtempSync(join(tmpdir(), 'FABRICA-zsh-test-userdata-'))
      setTestUserDataPath(userDataPath)
    })
...
`
      ...
      delete cleanEnv.FABRICA_ORIG_ZDOTDIR
      // Why: this test isolates zsh top-level path scoping, not attribution shim ordering.
      delete cleanEnv.FABRICA_ATTRIBUTION_SHIM_DIR
      cleanEnv.ZDOTDIR = config.env.ZDOTDIR // Point to FABRICA wrapper dir
      ...
      const result = spawnSync(
        ...
        [
          '-i',
          '-c',
          'echo "FABRICA_ORIG_ZDOTDIR=${FABRICA_ORIG_ZDOTDIR}" && echo "PATH_HAS_CUSTOM=${PATH%%:*}"'
        ],
        {
      ...
      expect(output).toContain(`FABRICA_ORIG_ZDOTDIR=${xdgZshDir}`)
...
FABRICA_zshenv_func() { echo "from-zshenv-function"; }
export ZDOTDIR="$HOME/.config/zsh"
`
      )
      ...
      const result = spawnSync(
        ...
        [
          '-c',
          'echo "PATH_HEAD=${PATH%%:*}" && echo "MY_VAR=${MY_VAR:-unset}" && FABRICA_zshenv_func'
        ],
        {
          env: cleanEnv as NodeJS.ProcessEnv,
          encoding: 'utf8'
...
`
      ...
      // Build clean env: use wrapper ZDOTDIR but let wrapper discover FABRICA_ORIG_ZDOTDIR at runtime
      const cleanEnv: Record<string, string | undefined> = { ...process.env, HOME: testHome }
      delete cleanEnv.ZDOTDIR
      delete cleanEnv.FABRICA_ORIG_ZDOTDIR
      cleanEnv.ZDOTDIR = config.env.ZDOTDIR // Point to FABRICA wrapper dir
      ...
      const result = spawnSync(
        'zsh',
        ['-c', 'echo "survived" && echo "FABRICA_ORIG_ZDOTDIR=${FABRICA_ORIG_ZDOTDIR}"'],
        {
          env: cleanEnv as NodeJS.ProcessEnv,
          encoding: 'utf8'
        }
      ...
      expect(result.stdout).toContain(`FABRICA_ORIG_ZDOTDIR=${testHome}`)
    })
...
`
      ...
      const result = spawnSync('zsh', ['-c', 'echo "FABRICA_ORIG_ZDOTDIR=${FABRICA_ORIG_ZDOTDIR}"'], {
        env: cleanEnv as NodeJS.ProcessEnv,
        encoding: 'utf8'
      })
  ...
  describeIfZsh('high-priority edge cases', () => {
    ...
    beforeEach(async () => {
      testHome = mkdtempSync(join(tmpdir(), 'FABRICA-zsh-edge-'))
      userDataPath = mkdtempSync(join(tmpdir(), 'FABRICA-zsh-userdata-'))
      setTestUserDataPath(userDataPath)
    })
    ...
    it('discovers ZDOTDIR when .zshenv sources another file that sets it', async () => {
      ...
      expect(result.status).toBe(0)
      expect(result.stdout).toContain(`FABRICA_ORIG_ZDOTDIR=${xdgZshDir}`)
    })
    ...
    it('preserves ZDOTDIR with spaces in path', async () => {
      ...
      expect(result.status).toBe(0)
      expect(result.stdout).toContain(`FABRICA_ORIG_ZDOTDIR=${spacePath}`)
    })
    ...
    it('captures last ZDOTDIR value when set multiple times', async () => {
      ...
      expect(result.status).toBe(0)
      expect(result.stdout).toContain(`FABRICA_ORIG_ZDOTDIR=${lastPath}`)
    })
    ...
    it('handles conditional ZDOTDIR based on environment', async () => {
      ...
      let result = spawnSync('zsh', ['-c', 'echo "FABRICA_ORIG_ZDOTDIR=${FABRICA_ORIG_ZDOTDIR}"'], {
        env: cleanEnv as NodeJS.ProcessEnv,
        encoding: 'utf8'
      })

      expect(result.status).toBe(0)
      expect(result.stdout).toContain(`FABRICA_ORIG_ZDOTDIR=${localPath}`)
      ...
      result = spawnSync('zsh', ['-c', 'echo "FABRICA_ORIG_ZDOTDIR=${FABRICA_ORIG_ZDOTDIR}"'], {
        env: cleanEnv as NodeJS.ProcessEnv,
        encoding: 'utf8'
      })

      expect(result.status).toBe(0)
      expect(result.stdout).toContain(`FABRICA_ORIG_ZDOTDIR=${remotePath}`)
    })
    ...
    it('does not source /.zshenv when HOME is empty', async () => {
      ...
      expect(zshenv).toContain('if [[ -n "${_FABRICA_zshenv_source_dir:-}"')
    })
    ...
    it('handles ZDOTDIR with single quote in path', async () => {
      ...
      expect(result.status).toBe(0)
      expect(result.stdout).toContain(`FABRICA_ORIG_ZDOTDIR=${quotePath}`)
    })
    ...
    it('does not evaluate command substitution in ZDOTDIR', async () => {
      ...
      expect(result.stdout).toContain(`FABRICA_ORIG_ZDOTDIR=${safePath}`)
    })
    ...
    it('handles ZDOTDIR with multiple trailing slashes', async () => {
      ...
      expect(result.stdout).toContain(`FABRICA_ORIG_ZDOTDIR=${cleanPath}`)
    })
  })
  ...
  describeIfZsh('terminal emulator edge cases', () => {
    ...
    beforeEach(async () => {
      testHome = mkdtempSync(join(tmpdir(), 'FABRICA-term-'))
      userDataPath = mkdtempSync(join(tmpdir(), 'FABRICA-term-userdata-'))
      setTestUserDataPath(userDataPath)
    })
    ...
    it('handles sudo -E where HOME and ZDOTDIR mismatch', async () => {
      ...
      try {
        ...
        expect(config.env.FABRICA_ORIG_ZDOTDIR).toBe(userZdotdir)
      } finally {
        if (previousZdotdir === undefined) {
          delete process.env.ZDOTDIR
        } else {
    ...
    it('re-discovers ZDOTDIR despite stale FABRICA_ORIG_ZDOTDIR from previous session', async () => {
      ...
      const previousFABRICAZdotdir = process.env.FABRICA_ORIG_ZDOTDIR
      process.env.FABRICA_ORIG_ZDOTDIR = '/opt/FABRICA-old/shell-ready/zsh' // stale wrapper path
      ...
      try {
        ...
        const cleanEnv: Record<string, string | undefined> = {
          ...process.env,
          HOME: testHome,
          FABRICA_ORIG_ZDOTDIR: '/opt/FABRICA-old/shell-ready/zsh'
        }
        delete cleanEnv.ZDOTDIR
        cleanEnv.ZDOTDIR = config.env.ZDOTDIR
        ...
        expect(result.stdout).toContain(`FABRICA_ORIG_ZDOTDIR=${currentZdotdir}`)
      } finally {
        if (previousFABRICAZdotdir === undefined) {
          delete process.env.FABRICA_ORIG_ZDOTDIR
        } else {
          process.env.FABRICA_ORIG_ZDOTDIR = previousFABRICAZdotdir
        }
      }
    })

    it('prioritizes fresh discovery over inherited FABRICA_ORIG_ZDOTDIR', async () => {
      ...
      process.env.FABRICA_ORIG_ZDOTDIR = oldZdotdir
      ...
      try {
        ...
        const cleanEnv: Record<string, string | undefined> = {
          ...process.env,
          HOME: testHome,
          FABRICA_ORIG_ZDOTDIR: oldZdotdir
        }
        delete cleanEnv.ZDOTDIR
        cleanEnv.ZDOTDIR = config.env.ZDOTDIR
        ...
        expect(result.stdout).toContain(`FABRICA_ORIG_ZDOTDIR=${freshZdotdir}`)
      } finally {
        if (previousFABRICAZdotdir === undefined) {
          delete process.env.FABRICA_ORIG_ZDOTDIR
        } else {
    ...
    it('sources launch-time ZDOTDIR .zshenv when it is explicitly inherited', async () => {
      ...
      try {
        const { getShellReadyLaunchConfig } = await importFreshLocalPtyShellReady()
        const config = getShellReadyLaunchConfig('/bin/zsh')
        expect(config.env.FABRICA_ZSHENV_SOURCE_DIR).toBe(inheritedZdotdir)
        ...
        const result = spawnSync(
          ...
          [
            '-c',
            'echo "FABRICA_ORIG_ZDOTDIR=${FABRICA_ORIG_ZDOTDIR}" && echo "SOURCE_MARKER=${SOURCE_MARKER:-unset}"'
          ],
          {
            env: cleanEnv as NodeJS.ProcessEnv,
            encoding: 'utf8'
        ...
        expect(result.status).toBe(0)
        expect(result.stdout).toContain(`FABRICA_ORIG_ZDOTDIR=${inheritedZdotdir}`)
  ...
  describeIfZsh('automation and edge cases', () => {
    ...
    beforeEach(async () => {
      testHome = mkdtempSync(join(tmpdir(), 'FABRICA-auto-'))
      userDataPath = mkdtempSync(join(tmpdir(), 'FABRICA-auto-userdata-'))
      setTestUserDataPath(userDataPath)
    })
    ...
    it('handles empty HOME gracefully', async () => {
      ...
      expect(result.status).toBe(0)
      // Empty HOME falls back to empty FABRICA_ORIG_ZDOTDIR
      expect(result.stdout).toContain('FABRICA_ORIG_ZDOTDIR=\n')
    })
    ...
    it('handles unset HOME gracefully', async () => {
      ...
      expect(result.stdout).toMatch(/FABRICA_ORIG_ZDOTDIR=.+/)
    })
