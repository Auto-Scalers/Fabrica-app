...
vi.mock('electron', () => ({
  app: {
    getPath: vi.fn(() => '/tmp/FABRICA-user-data')
  }
}))
...
describe('LocalPtyProvider', () => {
  ...
  describe('spawn', () => {
    ...
    it('does not inherit NODE_ENV from the FABRICA process env', async () => {
    ...
    it('uses fallback shell readiness when startup-command shell spawn falls back', async () => {
      ...
      try {
        ...
        dataCallback('\x1b]777;FABRICA-shell-ready\x07user@host % ')
        await Promise.resolve()
    ...
    it('honors explicit terminal env overrides after deleting requested defaults', async () => {
      provider.configure({
        buildSpawnEnv: (_id, env) => {
          env.TERM_PROGRAM = 'FABRICA'
          env.FABRICA_ATTRIBUTION_SHIM_DIR = '/tmp/FABRICA-attribution'
          env.PATH = `/tmp/FABRICA-attribution:${env.PATH ?? ''}`
          return env
        }
      })
      ...
      expect(spawnCall[2].env.PATH.split(':')[0]).toBe('/tmp/FABRICA-agent-teams-bin')
      expect(spawnCall[2].env.TERM_PROGRAM).toBeUndefined()
      expect(spawnCall[2].env.FABRICA_ATTRIBUTION_SHIM_DIR).toBeUndefined()
    })
    ...
    it('does not inherit AppImage runtime env into Linux PTY shells', async () => {
      ...
      process.env.LD_LIBRARY_PATH = ['/tmp/.mount_FABRICA123/usr/lib', '/opt/audio/lib'].join(
        delimiter
      )
    ...
    it('uses shell wrapper when MiMo home must survive shell startup', async () => {
      provider.configure({
        buildSpawnEnv: (_id, env) => {
          env.MIMOCODE_HOME = '/tmp/FABRICA-mimocode-overlay'
          env.FABRICA_MIMOCODE_HOME = '/tmp/FABRICA-mimocode-overlay'
          return env
        }
      })
      ...
      expect(spawnCall[2].env.FABRICA_SHELL_READY_MARKER).toBe('0')
    })
    ...
    it('promotes the agent-teams shim onto the Windows `Path` spelling', async () => {
      ...
      expect(spawnEnv.Path.split(':')[0]).toBe('/tmp/FABRICA-agent-teams-bin')
    })
    ...
    it('does not pass a Windows Codex home into WSL terminals', async () => {
      ...
      provider.configure({
        buildSpawnEnv: (_id, env) => {
          ...
          env.FABRICA_CODEX_HOME = 'C:\\Users\\jin\\.codex'
          return env
        }
      })
      ...
      expect(spawnCall[2].env.FABRICA_CODEX_HOME).toBeUndefined()
    })
    ...
    it('translates a WSL managed Codex home before launching a WSL terminal', async () => {
      ...
      const spawnCall = spawnMock.mock.calls.at(-1)!
      expect(spawnCall[0]).toBe('wsl.exe')
      expect(spawnCall[2].env.CODEX_HOME).toBe('/home/jin/.local/share/FABRICA/codex-accounts/a/home')
      expect(spawnCall[2].env.FABRICA_CODEX_HOME).toBe(
        '/home/jin/.local/share/FABRICA/codex-accounts/a/home'
      )
      expect(spawnCall[2].env.WSLENV).toContain('CODEX_HOME')
      expect(spawnCall[2].env.WSLENV).toContain('FABRICA_CODEX_HOME')
    })
    ...
    it('uses the preferred WSL distro for Windows cwd WSL terminals', async () => {
      ...
      expect(spawnCall[1][5]).toContain('exec "\\$_FABRICA_wsl_shell" -l')
    ...
    it('marks FABRICA terminal handle for WSL import when buildSpawnEnv opts in', async () => {
      Object.defineProperty(process, 'platform', { configurable: true, value: 'win32' })
      ...
      provider.configure({
        buildSpawnEnv: (_id, env, ctx) => {
          env.FABRICA_TERMINAL_HANDLE = 'term_wsl'
          if (ctx?.isWsl) {
            env.WSLENV = 'FABRICA_TERMINAL_HANDLE/u'
          }
          return env
        }
      })
      ...
      try {
        await provider.spawn({
          ...
          rows: 24,
          cwd: '\\\\wsl.localhost\\Ubuntu\\home\\jin\\repo',
          env: { FABRICA_HERMES_STARTUP_QUERY: 'line one\nline two' }
        })
      } finally {
    ...
    it('does not inherit parent FABRICA pane identity when caller omits pane env', async () => {
      const saved = {
      ...
      const spawnCall = spawnMock.mock.calls.at(-1)!
      expect(spawnCall[2].env.FABRICA_PANE_KEY).toBeUndefined()
      expect(spawnCall[2].env.FABRICA_TAB_ID).toBeUndefined()
      expect(spawnCall[2].env.FABRICA_WORKTREE_ID).toBeUndefined()
    })

    it('preserves explicit child FABRICA pane identity over parent env', async () => {
      const saved = {
        FABRICA_PANE_KEY: process.env.FABRICA_PANE_KEY,
        FABRICA_TAB_ID: process.env.FABRICA_TAB_ID,
        FABRICA_WORKTREE_ID: process.env.FABRICA_WORKTREE_ID
      ...
      const spawnCall = spawnMock.mock.calls.at(-1)!
      expect(spawnCall[2].env.FABRICA_PANE_KEY).toBe('child-tab:child-leaf')
      expect(spawnCall[2].env.FABRICA_TAB_ID).toBe('child-tab')
      expect(spawnCall[2].env.FABRICA_WORKTREE_ID).toBe('child-worktree')
    })
