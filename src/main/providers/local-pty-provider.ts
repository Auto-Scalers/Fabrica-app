...
export class LocalPtyProvider implements IPtyProvider {
  ...
  async spawn(args: PtySpawnOptions): Promise<PtySpawnResult> {
    ...
    const spawnEnv: Record<string, string> = {
      ...
      // Why: supports-hyperlinks rejects TERM_PROGRAM=FABRICA, so tools drop OSC 8 links; force it since xterm.js parses them.
      FORCE_HYPERLINK: '1'
    } as Record<string, string>
    // Why: FABRICA can be launched from an FABRICA terminal; pane identity belongs to the child PTY, not the parent shell.
      ...
      if (pathWin32.basename(shellPath).toLowerCase() === 'wsl.exe') {
        if (codexHomeWslInfo) {
          if (launchWslDistro && launchWslDistro !== codexHomeWslInfo.distro) {
            ...
            delete finalEnv.FABRICA_CODEX_HOME
          } else {
            finalEnv.CODEX_HOME = codexHomeWslInfo.linuxPath
            finalEnv.FABRICA_CODEX_HOME = codexHomeWslInfo.linuxPath
            // Why: wsl.exe only imports non-default env vars named in WSLENV.
            addWslEnvKeys(finalEnv, ['CODEX_HOME', 'FABRICA_CODEX_HOME'])
            if (!launchWslDistro) {
              const resolved = resolveWindowsShellLaunchArgs(shellPath, cwd, defaultCwd, {
          ...
          // Why: FABRICA's Codex home is host-local; WSL Codex must use its Linux-side ~/.codex, not a Windows path.
          delete finalEnv.CODEX_HOME
          delete finalEnv.FABRICA_CODEX_HOME
        ...
        if (finalEnv[FABRICA_HERMES_STARTUP_QUERY_ENV] !== undefined) {
          // Why: wsl.exe drops custom Windows env vars; the startup wrapper needs this imported inside WSL.
          addWslEnvKeys(finalEnv, [FABRICA_HERMES_STARTUP_QUERY_ENV])
        }
      } else if (codexHomeWslInfo || isWslCodexHomeForHost(finalEnv.CODEX_HOME)) {
        // Why: WSL Codex homes are Linux paths Windows can't use; also drop FABRICA_CODEX_HOME (shell-ready restores CODEX_HOME from it).
        delete finalEnv.CODEX_HOME
        delete finalEnv.FABRICA_CODEX_HOME
      }
    }
    ...
    if (!wslInfo && process.platform !== 'win32') {
      ...
      const needsNoMarkerWrapper =
        finalEnv.FABRICA_ATTRIBUTION_SHIM_DIR ||
        finalEnv.FABRICA_OPENCODE_CONFIG_DIR ||
        finalEnv.FABRICA_MIMOCODE_HOME ||
        finalEnv.FABRICA_OMP_STATUS_EXTENSION ||
        finalEnv.FABRICA_CODEX_HOME ||
        finalEnv.FABRICA_AGENT_TEAMS_SHIM_DIR
    ...
    if (finalEnv.FABRICA_TERMINAL_HANDLE) {
      ptyTerminalHandle.set(id, finalEnv.FABRICA_TERMINAL_HANDLE)
    }
    if (args.worktreeId) {
