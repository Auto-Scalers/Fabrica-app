...
describe('resolveWindowsShellLaunchArgs', () => {
  ...
  it('returns PowerShell args that install OSC 133 bootstrap after normal profile loading', () => {
    ...
    const opencodeRestoreIndex = command.indexOf(
      '$env:OPENCODE_CONFIG_DIR = $env:FABRICA_OPENCODE_CONFIG_DIR'
    )
    const mimocodeRestoreIndex = command.indexOf('$env:MIMOCODE_HOME = $env:FABRICA_MIMOCODE_HOME')
    const duplicateStateGuardIndex = command.indexOf('Test-Path variable:global:__FABRICAOsc133State')
    ...
    const ompExtensionIndex = command.indexOf('--extension $env:FABRICA_OMP_STATUS_EXTENSION')
    const codexRestoreIndex = command.indexOf('$env:CODEX_HOME = $env:FABRICA_CODEX_HOME')
    ...
    expect(command).not.toContain('FABRICA_PI_CODING_AGENT_DIR')
    expect(command).not.toContain('FABRICA_OMP_CODING_AGENT_DIR')
    expect(command).not.toContain('$env:PI_CODING_AGENT_DIR = $env:FABRICA_OMP_SOURCE_AGENT_DIR')
  ...
  it('preserves complex PowerShell startup command text through EncodedCommand', () => {
    const startupCommand =
      '& "C:\\Program Files\\FABRICA CLI\\FABRICA.exe" "--label" "quoted value"; $env:FABRICA_VALUE = "nested"'
    const result = resolveWindowsShellLaunchArgs(
      'powershell.exe',
      'C:\\Users\\alice',
      'C:\\Users\\alice',
  ...
  it('materializes shell-ready wrappers before building WSL shell args', () => {
    ...
    for (const wrapperFile of [bashRcfile, zshLogin]) {
      expect(wrapperFile).toContain('command omp --extension "${FABRICA_OMP_STATUS_EXTENSION}" "$@"')
      expect(wrapperFile).toContain('omp() { __FABRICA_omp "$@"; }')
      expect(wrapperFile).toContain(
        'command prime-agent --extension "${FABRICA_PRIME_AGENT_STATUS_EXTENSION}" "$@"'
      )
      expect(wrapperFile).toContain('prime-agent() { __FABRICA_prime_agent "$@"; }')
    }
  })
  ...
  it('escapes single quotes when translating a WSL cwd', () => {
    ...
    expect(result.shellArgs[3]).toContain('exec "\\$_FABRICA_wsl_shell" -l')
  })
...
describe('issue #7236: PowerShell setup-runner command delivery', () => {
  ...
  const runnerPath = 'C:/Users/alice/repo/.git/FABRICA/setup-runner.cmd'
