...
import {
  FABRICA_PI_AGENT_STATUS_EXTENSION_FILE,
  getPiAgentStatusExtensionSource
} from './agent-status-extension-source'
import {
  FABRICA_PI_PREFILL_EXTENSION_FILE,
  getPiPrefillExtensionSource
} from './prefill-extension-source'
import { FABRICA_PI_EXTENSION_FILE, getPiTitlebarExtensionSource } from './titlebar-extension-source'
import {
...
// Why: old FABRICA versions used per-kind overlay roots. Keep the names so
...
// shadowed one in FABRICA's per-launch agent picker.
...
function withFABRICAManagedExtensionMarker(source: string): string {
...
export class PiTitlebarExtensionService {
  ...
  private getPtyOverlayDir(ptyId: string, kind: LegacyOverlayAgentKind): string {
    // Why: old FABRICA versions used PTY-scoped hashed overlays. Keep resolving
  ...
  private writeFallbackStatusExtension(
    ...
    const fallbackPath = join(fallbackDir, FABRICA_PI_AGENT_STATUS_EXTENSION_FILE)
  ...
  private installManagedExtensions(
    ...
    if (kind !== 'prime-agent') {
      this.writeManagedExtension(
        join(extensionsDir, FABRICA_PI_EXTENSION_FILE),
        withFABRICAManagedExtensionMarker(getPiTitlebarExtensionSource())
      )
      this.writeManagedExtension(
        join(extensionsDir, FABRICA_PI_PREFILL_EXTENSION_FILE),
        withFABRICAManagedExtensionMarker(getPiPrefillExtensionSource(kind))
      )
    }
    const statusExtensionPath = join(extensionsDir, FABRICA_PI_AGENT_STATUS_EXTENSION_FILE)
    const statusSource = withFABRICAManagedExtensionMarker(getPiAgentStatusExtensionSource(kind))
  ...
  buildPtyEnv(
    ...
    if (!existsSync(sourceAgentDir) && !materializeDefaultHome) {
      if (kind === 'omp') {
        ...
        return statusExtensionPath ? { FABRICA_OMP_STATUS_EXTENSION: statusExtensionPath } : {}
      }
      return {}
    }
    ...
    if (kind === 'omp') {
      env.FABRICA_OMP_SOURCE_AGENT_DIR = installed.sourceAgentDir
      if (installed.statusExtensionPath) {
        env.FABRICA_OMP_STATUS_EXTENSION = installed.statusExtensionPath
      }
    } else if (kind === 'prime-agent') {
      env.FABRICA_PRIME_AGENT_SOURCE_AGENT_DIR = installed.sourceAgentDir
    } else {
      env.FABRICA_PI_SOURCE_AGENT_DIR = installed.sourceAgentDir
    }
    return env
  }

  buildStatusOnlyPtyEnv(kind: 'prime-agent'): Record<string, string> {
    const source = withFABRICAManagedExtensionMarker(getPiAgentStatusExtensionSource(kind))
    const statusExtensionPath = this.writeFallbackStatusExtension(source, kind)
    return statusExtensionPath ? { FABRICA_PRIME_AGENT_STATUS_EXTENSION: statusExtensionPath } : {}
  }
