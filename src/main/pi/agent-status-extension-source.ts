...
// /hook/<kind> using the same FABRICA_AGENT_HOOK_* + FABRICA_PANE_KEY env that every
...
// any FABRICA dep into the pi runtime.
...
export const FABRICA_PI_AGENT_STATUS_EXTENSION_FILE = 'FABRICA-agent-status.ts'
...
export function getPiAgentStatusExtensionSource(kind: PiAgentKind = 'pi'): string {
  ...
  // so it cannot import from FABRICA's main bundle. fs/http coords come from
  // the same endpoint file the OpenCode plugin reads (process.env is frozen
  // at PTY spawn, so on FABRICA restart we have to re-read it from disk).
  return [
    ...
    '// FABRICA receiver from building an unbounded queue of obsolete snapshots.',
    ...
    "      console.warn('[FABRICA-pi-status] failed to parse endpoint file:', (err as Error).message)",
    '    }',
    '    return null',
    '  }',
    '}',
    ...
    '    timeout = setTimeout(() => {',
    '      controller?.abort()',
    "      reject(new Error('FABRICA hook delivery timed out'))",
    ...
    '    ])',
    '  } catch {',
    '    // Why: status reporting must never fail the pi run just because FABRICA',
    '    // is unavailable or the loopback request failed (e.g. FABRICA restart).',
