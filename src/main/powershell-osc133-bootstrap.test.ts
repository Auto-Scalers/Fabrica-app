...
describe('PowerShell OSC 133 bootstrap', () => {
  it('wraps prompt/readline without bypassing profiles or execution policy', () => {
    ...
    expect(script).toContain('FABRICA_OPENCODE_CONFIG_DIR')
    expect(script).toContain('FABRICA_MIMOCODE_HOME')
    expect(script).not.toContain('FABRICA_PI_CODING_AGENT_DIR')
    expect(script).not.toContain('FABRICA_OMP_CODING_AGENT_DIR')
    expect(script).toContain('FABRICA_OMP_STATUS_EXTENSION')
    expect(script).toContain('function Global:omp')
    expect(script).toContain('--extension $env:FABRICA_OMP_STATUS_EXTENSION')
    expect(script).toContain('FABRICA_CODEX_HOME')
    ...
    const codexHomeRestore = script.indexOf('if ($env:FABRICA_CODEX_HOME)')
