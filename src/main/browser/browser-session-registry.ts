...
class BrowserSessionRegistry {
  ...
  private activeFABRICAProfileId = DEFAULT_LOCAL_FABRICA_PROFILE_ID
  private metadataPathOverride: string | null = null
  private defaultPartition = FABRICA_BROWSER_PARTITION

  constructor() {
    this.resetDefaultProfile()
  }

  configureForFABRICAProfile(options: BrowserSessionRegistryProfileOptions): void {
    this.activeFABRICAProfileId = options.FABRICAProfileId
    this.metadataPathOverride = join(options.profileDirectory, BROWSER_SESSION_META_FILE_NAME)
    this.defaultPartition = getFABRICAProfileBrowserDefaultPartition(options.FABRICAProfileId)
    this.profiles.clear()
    this.resetDefaultProfile()
  }
  ...
  resolveKnownPartition(profileId: string | null | undefined): string | null {
    if (!profileId) {
      // Why: use the active FABRICA profile's default partition, not the legacy constant, or profiles resolve local-default's cookie jar.
      return this.defaultPartition
    }
  ...
  createProfile(
    ...
    const partition = getFABRICAProfileBrowserSessionPartition(this.activeFABRICAProfileId, id)
  ...
  private isProfileOwnedSessionPartition(profileId: string, partition: string): boolean {
    return (
      ...
      partition === getFABRICAProfileBrowserSessionPartition(this.activeFABRICAProfileId, profileId)
    )
  }
