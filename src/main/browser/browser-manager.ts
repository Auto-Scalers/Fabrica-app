...
import { FABRICA_BROWSER_BLANK_URL } from '../../shared/constants'
import {
...
function releaseAutomationVisibilityToken(renderer: Electron.WebContents, token: string): void {
  ...
  renderer
    .executeJavaScript(
      `(function() {
        var bridge = window.__FABRICABrowserAutomationVisibility;
...
export class BrowserManager {
        ...
        .executeJavaScript(
          `(function() {
            ...
            if (
              ...
              // Why: FABRICA remembers the last browser workspace/page even when
  ...
  attachGuestPolicies(
    ...
    // Why: only the primary embedded browser converts new-tab clicks to FABRICA tabs; OAuth child windows keep native link behavior.
    const clickedLinkFrameName = inheritedOwnerContext
      ? null
      : `__FABRICA_clicked_link_foreground_${randomUUID()}`
    ...
    const installIframeClickedLinkRouting = (frame: Electron.WebFrameMain): void => {
      ...
      const name = `__FABRICA_clicked_link_iframe_foreground_${randomUUID()}`
    ...
    guest.setWindowOpenHandler(({ url, frameName }) => {
      ...
      if (isClickedLink) {
        if (browserTabId && browserUrl && this.openLinkInFABRICATab(browserTabId, browserUrl)) {
          this.forwardOrQueuePopupEvent(guest.id, {
            origin: safeOrigin(browserUrl),
            action: 'opened-in-FABRICA'
          })
        }
      ...
      const canOpenAsChild = Boolean(externalUrl || browserUrl === FABRICA_BROWSER_BLANK_URL)
      if (browserTabId && canOpenAsChild) {
        ...
        return {
          ...
          // Why: default child windows lack an address bar; host in an FABRICA origin-bar window so the destination is verifiable.
  ...
  // Why: headless FABRICA serve has no <webview> window; back pages with offscreen WebContents and skip the webview-only setup.
  ...
  unregisterAll(): void {
    ...
    for (const downloadId of this.downloadsById.keys()) {
      this.cancelDownloadInternal(downloadId, 'Fabrica is shutting down.')
    }
  ...
  // Why: guests are isolated from FABRICA's preload bridge, so main owns the devtools escape hatch after a tab→guest lookup.
  ...
  private openLinkInFABRICATab(browserTabId: string, rawUrl: string): boolean {
    const renderer = this.resolveRendererForBrowserTab(browserTabId)
    if (!renderer) {
      return false
    }
    const normalizedUrl = normalizeBrowserNavigationUrl(rawUrl)
    if (!normalizedUrl || normalizedUrl === FABRICA_BROWSER_BLANK_URL) {
      return false
    }
    // Why: only the renderer owns FABRICA's worktree/tab model; main forwards a validated URL, never letting guest content mutate it.
    renderer.send('browser:open-link-in-FABRICA-tab', {
      browserPageId: browserTabId,
      url: normalizedUrl
    })
    return true
