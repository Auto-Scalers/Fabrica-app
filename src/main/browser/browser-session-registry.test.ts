...
import { FABRICA_BROWSER_PARTITION } from '../../shared/constants'
import {
  DEFAULT_LOCAL_FABRICA_PROFILE_ID,
  getFABRICAProfileBrowserDefaultPartition,
  getFABRICAProfileBrowserSessionPartition
...
describe('BrowserSessionRegistry', () => {
  ...
  it('has a default profile on construction', () => {
    ...
    expect(defaultProfile.partition).toBe(FABRICA_BROWSER_PARTITION)
  })

  it('allows the default partition', () => {
    expect(browserSessionRegistry.isAllowedPartition(FABRICA_BROWSER_PARTITION)).toBe(true)
  })
  ...
  it('creates an isolated profile with a unique partition', () => {
    ...
    expect(profile!.partition).toMatch(/^persist:FABRICA-browser-session-/)
    expect(profile!.partition).not.toBe(FABRICA_BROWSER_PARTITION)
  ...
  it('resolves default partition for null/undefined profileId', () => {
    expect(browserSessionRegistry.resolvePartition(null)).toBe(FABRICA_BROWSER_PARTITION)
    expect(browserSessionRegistry.resolvePartition(undefined)).toBe(FABRICA_BROWSER_PARTITION)
  })

  it('resolves default partition for unknown profileId', () => {
    expect(browserSessionRegistry.resolvePartition('nonexistent')).toBe(FABRICA_BROWSER_PARTITION)
  })
  ...
  it('strictly resolves known profile partitions without downgrading unknown profiles', () => {
    ...
    expect(browserSessionRegistry.resolveKnownPartition(null)).toBe(FABRICA_BROWSER_PARTITION)
    expect(browserSessionRegistry.resolveKnownPartition(undefined)).toBe(FABRICA_BROWSER_PARTITION)
    expect(browserSessionRegistry.resolveKnownPartition('default')).toBe(FABRICA_BROWSER_PARTITION)
  ...
  it('hydrates profiles from persisted data', () => {
    const fakeProfile = {
      ...
      partition: 'persist:FABRICA-browser-session-00000000-0000-0000-0000-000000000001',
      label: 'Hydrated',
  ...
  it('rejects a persisted profile whose partition belongs to a different profile id', () => {
    ...
    const claimedPartition = 'persist:FABRICA-browser-session-00000000-0000-4000-8000-000000000022'
  ...
  it('routes media permission requests through macOS TCC for isolated partitions', async () => {
    ...
    // after the user grants Camera/Microphone to FABRICA.
  ...
  it('uses profile-owned partitions for non-default FABRICA profiles', () => {
    const FABRICAProfileId = 'local-work'
    ...
    expect(browserSessionRegistry.getDefaultProfile().partition).toBe(
      getFABRICAProfileBrowserDefaultPartition(FABRICAProfileId)
    )
    expect(browserSessionRegistry.isAllowedPartition(FABRICA_BROWSER_PARTITION)).toBe(false)

    const profile = browserSessionRegistry.createProfile('isolated', 'Work Browser')
    expect(profile).not.toBeNull()
    expect(profile!.partition).toBe(
      getFABRICAProfileBrowserSessionPartition(FABRICAProfileId, profile!.id)
    )

    browserSessionRegistry.configureForFABRICAProfile({
      FABRICAProfileId: DEFAULT_LOCAL_FABRICA_PROFILE_ID,
