...
const HIDE_BROWSER_GRAB_OVERLAY_SCRIPT = `(function(){
  var g = window.__FABRICAGrab;
  if (g && g.host) g.host.style.display = 'none';
  document.querySelectorAll('[data-FABRICA-browser-annotation-overlay]').forEach(function(el) {
    el.setAttribute('data-FABRICA-previous-display', el.style.display || '');
...
const RESTORE_BROWSER_GRAB_OVERLAY_SCRIPT = `(function(){
  ...
  document.querySelectorAll('[data-FABRICA-browser-annotation-overlay]').forEach(function(el) {
    el.style.display = el.getAttribute('data-FABRICA-previous-display') || '';
    el.removeAttribute('data-FABRICA-previous-display');
  });
})()`
