...
export class AgentBrowserBridge {
  ...
  async onTabClosed(webContentsId: number): Promise<void> {
    ...
    if (browserPageId) {
      const sessionName = `FABRICA-tab-${browserPageId}`
      ...
      async (_sessionName, target) => {
        ...
        try {
          ...
          if (!this.getWebContents(target.webContentsId)) {
            throw this.createPageUnavailableError(`FABRICA-tab-${target.browserPageId}`)
          }
  ...
  async setViewport(
    ...
    return this.enqueueTargetedCommand(worktreeId, browserPageId, async (_sessionName, target) => {
      ...
      // Why: agent-browser's `set viewport` has no `mobile` flag, so apply the emulation directly via CDP to honor FABRICA's --mobile.
  ...
  async exec(command: string, worktreeId?: string, browserPageId?: string): Promise<unknown> {
    return this.enqueueTargetedCommand(worktreeId, browserPageId, async (sessionName) => {
      // Why: strip target/session flags from passthrough so a caller can't override FABRICA's selected page or CDP proxy.
  ...
  private async enqueueTargetedCommand<T>(
    ...
    const sessionName = `FABRICA-tab-${target.browserPageId}`
  ...
  private async execAgentBrowser(
    ...
    // Why: a restarted named daemon auto-launches Chrome unless every invocation reasserts FABRICA's CDP owner.
