...
function getDiagLogPath(): string {
  if (!_diagLog) {
    try {
      ...
      _diagLog = join(tmpdir(), 'FABRICA-cookie-import-diag.log')
    }
  }
  return _diagLog
}
...
async function importCookiesFromFirefox(
  ...
  const tmpDir = mkdtempSync(join(tmpdir(), 'FABRICA-cookie-import-'))
  ...
  try {
    ...
    if (isPermError) {
      return {
        ok: false,
        reason:
          'macOS denied access to Safari cookies. Grant Full Disk Access to Fabrica in System Settings → Privacy & Security → Full Disk Access.'
      }
    }
...
export async function importCookiesFromBrowser(
  ...
  try {
    ...
    if (stagingAvailable) {
      // Why: the staged file is FABRICA's own partition DB, also named "Cookies", so the same
