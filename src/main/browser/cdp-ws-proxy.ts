...
export class CdpWsProxy {
  ...
  private handleHttpRequest(req: IncomingMessage, res: ServerResponse): void {
    ...
    if (url === '/json/version' || url === '/json/version/') {
      ...
      // "FABRICA/CdpWsProxy" leaks that this is an embedded automation surface, which
  ...
  private handleClientMessage(client: WebSocket, raw: string): void {
    ...
    if (msg.method === 'Browser.getVersion') {
      // Why: returning "FABRICA/Electron" identifies this as an embedded automation
    ...
    // Use the same direct webContents reload path as FABRICA's own browser.reload.
  ...
  private nextSyntheticPageSessionId(): string {
    ...
    return this.nextClientSessionOrdinal === 1
      ? 'FABRICA-proxy-session'
      : `FABRICA-proxy-session-${this.nextClientSessionOrdinal}`
  }
  ...
  private nextSyntheticBrowserSessionId(): string {
    ...
    return this.nextClientBrowserSessionOrdinal === 1
      ? 'FABRICA-proxy-browser-session'
      : `FABRICA-proxy-browser-session-${this.nextClientBrowserSessionOrdinal}`
  }
  ...
  private async reloadWithLifecycle(
    ...
    if (unsupportedParam) {
      this.sendError(
        clientId,
        `Page.reload parameter "${unsupportedParam}" is not supported for Fabrica tab reloads`,
        client
      )
      return
    }
