...
// arm: install the overlay + hover tracking; state lives on window.__FABRICAGrab so finalize/teardown can reach it.
const ARM_SCRIPT = `(function() {
  'use strict';

  // Why: always tear down any pre-existing state before arming. A malicious
  // guest page could predefine window.__FABRICAGrab with a fake extractPayload
  // function. By tearing down unconditionally we ensure our freshly installed
  // extraction logic is the only code that runs.
  if (window.__FABRICAGrab) {
    try {
      if (typeof window.__FABRICAGrab.cleanup === 'function') {
        window.__FABRICAGrab.cleanup();
      }
    } catch(e) {}
    delete window.__FABRICAGrab;
  }
...
const AWAIT_CLICK_SCRIPT = `(async function() {
  ...
  return await new Promise(function(resolve, reject) {
    ...
    function onContext(e) {
      ...
      if (!payload) return;
      grab.freezeHighlight();
      resolve({ __FABRICAContextMenu: true, payload: payload });
    }
    ...
    grab.cancelAwait = function() {
      ...
      resolve({ __FABRICACancelled: true });
    };
  });
})()`
...
const TEARDOWN_SCRIPT = `(function() {
  ...
  // it with the __FABRICACancelled marker so the executeJavaScript call in main
