...
function installModuleMocks(
  ...
  const sessionFromPartitionMock = vi.fn((partition: string) => ({
    partition,
    setUserAgent: vi.fn(),
    getUserAgent: vi.fn(() => 'Mozilla/5.0 Electron/31 FABRICA'),
...
describe('BrowserSessionRegistry persistence', () => {
  ...
  it('migrates and consumes legacy pendingCookieDbPath into default partition replay', async () => {
    ...
    expect(fsState.present.has('/user-data/Partitions/FABRICA-browser/Cookies')).toBe(true)
  })

  it('replays pending cookies into an existing Network database', async () => {
    const stagedPath = '/staged/network-import'
    const networkPath = '/user-data/Partitions/FABRICA-browser/Network/Cookies'
    const legacyPath = '/user-data/Partitions/FABRICA-browser/Cookies'
    const fsState = createFsState()
    seedMeta(fsState, {
      defaultSource: null,
      userAgent: null,
  ...
  it('persists new browser session profiles under the active FABRICA profile directory', async () => {
    const fsState = createFsState()
    ...
    browserSessionRegistry.configureForFABRICAProfile({
  ...
  it('keeps UA cleaning as the fallback for profiles without an override', async () => {
    ...
    expect(profileSession.setUserAgent).toHaveBeenCalledWith('Mozilla/5.0 FABRICA')
    expect(setupClientHintsOverrideMock).toHaveBeenCalledWith(profileSession, 'Mozilla/5.0 FABRICA')
  })
  ...
  it('merges partition-keyed pending entries without clobbering unrelated entries', async () => {
    ...
    browserSessionRegistry.setPendingCookieImport('persist:FABRICA-browser', '/staged/default')
    browserSessionRegistry.setPendingCookieImport(
      'persist:FABRICA-browser-session-aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa',
      '/staged/imported'
    )
    ...
    expect(written.pendingCookieImports).toEqual({
      'persist:FABRICA-browser': '/staged/default',
      'persist:FABRICA-browser-session-aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa': '/staged/imported'
    })
  })

  it('clears only the requested partition and unlinks its staged database files', async () => {
    const otherPartition = 'persist:FABRICA-browser-session-bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb'
    const fsState = createFsState()
    seedMeta(fsState, {
      defaultSource: null,
      userAgent: null,
    ...
    const written = JSON.parse(fsState.files.get(META_PATH) ?? '{}')
    expect(written.pendingCookieImports).toEqual({ 'persist:FABRICA-browser': '/staged/default' })
  ...
  it('drops the legacy pointer when the default partition is the one cleared', async () => {
    const otherPartition = 'persist:FABRICA-browser-session-cccccccc-cccc-4ccc-8ccc-cccccccccccc'
    const fsState = createFsState()
    seedMeta(fsState, {
      defaultSource: null,
      userAgent: null,
    ...
    browserSessionRegistry.clearPendingCookieImport('persist:FABRICA-browser')
  ...
  it('is a no-op when the partition has no pending import', async () => {
    ...
    seedMeta(fsState, {
      ...
      userAgentByPartition: {},
      pendingCookieDbPath: '/staged/default',
      pendingCookieImports: { 'persist:FABRICA-browser': '/staged/default' },
      profiles: []
    })
    ...
    browserSessionRegistry.clearPendingCookieImport('persist:FABRICA-browser-session-unknown')
  ...
  it('ignores legacy persisted UAs, valid or broken, and applies the engine UA', async () => {
    const importedPartition = 'persist:FABRICA-browser-session-11111111-1111-4111-8111-111111111111'
    const brokenUa =
    ...
    seedMeta(fsState, {
      ...
      userAgent: brokenUa,
      userAgentByPartition: {
        'persist:FABRICA-browser': brokenUa,
    ...
    // Why: every non-native profile falls to FABRICA's own cleaned engine UA.
    expect(appliedUas.length).toBeGreaterThan(0)
    expect(appliedUas.every((ua) => ua === 'Mozilla/5.0 FABRICA')).toBe(true)
    expect(
  ...
  it('preserves native mode across hydration when no source UA was imported', async () => {
    const importedPartition = 'persist:FABRICA-browser-session-12121212-1212-4121-8121-121212121212'
    const fsState = createFsState()
    seedMeta(fsState, {
      defaultSource: null,
      userAgent: null,
  ...
  it('sets up default-partition policies on restore', async () => {
    ...
    const defaultSessions = sessionFromPartitionMock.mock.results
      .filter((_, idx) => sessionFromPartitionMock.mock.calls[idx]?.[0] === 'persist:FABRICA-browser')
      .map((r) => r.value)
  ...
  it('notifies when default-partition media permission is denied', async () => {
    ...
    const defaultSession = sessionFromPartitionMock.mock.results.find(
      (_, idx) => sessionFromPartitionMock.mock.calls[idx]?.[0] === 'persist:FABRICA-browser'
    )?.value
    const requestHandler = defaultSession.setPermissionRequestHandler.mock.calls[0][0]
  ...
  it('keeps failed partition replay pending and removes unrelated missing entries', async () => {
    const importedPartition = 'persist:FABRICA-browser-session-22222222-2222-4222-8222-222222222222'
    const fsState = createFsState()
    seedMeta(fsState, {
      defaultSource: null,
      userAgent: null,
      ...
      pendingCookieImports: {
        ...
        'persist:FABRICA-browser': '/staged/missing'
      },
      profiles: [
        {
