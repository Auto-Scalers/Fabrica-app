...
const FOREGROUND_FRAME_NAME = '__FABRICA_clicked_link_foreground_test'

type RoutingGlobal = typeof globalThis & {
  __FABRICABrowserClickedLinkRouting?: {
...
function resetRouting(): void {
  ...
  const state = routingGlobal.__FABRICABrowserClickedLinkRouting
  if (state) {
    window.removeEventListener('click', state.listener)
    window.removeEventListener('auxclick', state.listener)
  }
  delete routingGlobal.__FABRICABrowserClickedLinkRouting
...
describe('browser clicked-link routing', () => {
  ...
  it('routes plain target=_blank links back into the current FABRICA tab', () => {
  ...
  it('updates an existing isolated-world installation without adding another listener', () => {
    ...
    link.href = 'https://example.com/reference'
    document.body.append(link)
    installBrowserClickedLinkRouting('__FABRICA_clicked_link_old_fg', true, true)
    installBrowserClickedLinkRouting('__FABRICA_clicked_link_new_fg', false, true)

    expect(addEventListener.mock.calls.filter(([event]) => event === 'click')).toHaveLength(1)
    expect(clickLink(link, { ctrlKey: true }).open).toHaveBeenCalledWith(
      'https://example.com/reference',
      '__FABRICA_clicked_link_new_fg'
    )
  })
