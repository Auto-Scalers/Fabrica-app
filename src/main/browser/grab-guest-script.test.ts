...
describe('buildGuestOverlayScript', () => {
  ...
  it('arm script contains shadow DOM setup', () => {
    ...
    expect(script).toContain('__FABRICA-grab-host')
    expect(script).toContain('__FABRICAGrab')
  })
  ...
  it('teardown script cancels pending awaitClick', () => {
    ...
    expect(buildGuestOverlayScript('awaitClick')).toContain('__FABRICACancelled')
  })
...
describe('awaitClick under a Zone.js-patched global Promise', () => {
  ...
  function armGrabHarness(options?: {
    ...
    window: { __FABRICAGrab: Record<string, unknown> }
    click: () => void
    contextmenu: () => void
    cancel: () => void
  } {
    ...
    const window = { __FABRICAGrab: grab }
    return {
      window,
      click: () => handlers.click?.(noopEvent),
      contextmenu: () => handlers.contextmenu?.(noopEvent),
      // cancelAwait is installed on __FABRICAGrab by the script itself at runtime.
      cancel: () => (window.__FABRICAGrab.cancelAwait as (() => void) | undefined)?.()
    }
  }
  ...
  it('resolves the context-menu marker across the boundary despite ZoneAwarePromise', async () => {
    ...
    expect(received).toHaveProperty('__FABRICAContextMenu', true)
  ...
  it('resolves the teardown cancel marker across the boundary despite ZoneAwarePromise', async () => {
    ...
    expect(received).toEqual({ __FABRICACancelled: true })
  })
