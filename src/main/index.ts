...
import { ensureActiveFABRICAProfile, initFABRICAProfilePaths } from './fabrica-profiles/profile-index-store'
import { getFABRICACloudAuthConfig } from './fabrica-profiles/profile-cloud-auth-config'
...
import { FABRICARuntimeService, type RuntimeWorktreeLifecycleEvent } from './runtime/fabrica-runtime'
...
import { FABRICARuntimeRpcServer } from './runtime/runtime-rpc'
import {
...
import { awaitRuntimeFileWatcherUnsubscribes } from './runtime/fabrica-runtime-files'
...
import {
  ...
  configureFABRICAUserDataPathEnv,
...
import { getFABRICAManagedCodexHomePath, getSystemCodexHomePath } from './codex/codex-home-paths'
...
import { installLinuxBareFABRICADispatcher } from './cli/linux-bare-fabrica-dispatcher'
...
let runtime: FABRICARuntimeService | null = null
let rateLimits: RateLimitService | null = null
let runtimeRpc: FABRICARuntimeRpcServer | null = null
...
function maybeAutoRenameBranchOnFirstWorkFromHook(event: {
  ...
  void maybeAutoRenameBranchOnFirstWork(
    ...
    },
    {
      ...
      canRenameFABRICACreatedBranch: (worktreeId) => {
        const meta = currentStore.getWorktreeMeta(worktreeId)
        // Why: a user branch could coincidentally match a creature name; only FABRICA-stamped worktrees are safe to auto-rename.
        return !!meta?.FABRICACreationSource && meta.preserveBranchOnDelete !== true
      },
...
process.env.FABRICA_APP_VERSION = app.getVersion()
...
if (startupDiagnosticsEnabled) {
  logStartupDiagnostic('before-single-instance-lock', {
    ...
    e2eUserData: Boolean(process.env.FABRICA_E2E_USER_DATA_DIR)
  })
  startEventLoopStallProbe()
}
// Self-gated on FABRICA_MAIN_THREAD_DIAGNOSTICS; runs the whole session to catch steady-state churn (issue #7576).
...
function requestDesktopActivation(argv: readonly string[] = []): void {
  // Why: a duplicate `FABRICA serve` must not drag a headless server into opening a desktop window (#11935).
...
if (hasSingleInstanceLock) {
  // Why: couple to dev-parent only for electron-vite desktop runs; `FABRICA serve`'s parent (CLI shim/background shell) isn't the intended server lifetime.
  ...
  // Why: run after configureDevUserDataPath but before app.setName('Fabrica') (whenReady), which changes the resolved path on case-sensitive filesystems.
  initDataPath()
...
/** A PTY that dies while FABRICA is down never runs the teardown that clears pane
...
function startTerminalRuntimeStartupServices(): Promise<void> {
  ...
  const startupServices = startFirstWindowStartupServices({
    ...
    // Why: PTY spawn env reads FABRICA_AGENT_HOOK_* from live server state, so the renderer awaits this before restored terminals reconnect.
    startAgentHookServer: async () => {
      ...
      await agentHookServer.start({
        ...
        // Why: hooks source this endpoint file at invocation time so old PTY env reaches the current process after restart; dev namespaces it (worktrees share `FABRICA-dev`).
    ...
    onAgentHookServerError: (error) => {
      // Why: hook callbacks are sidebar enrichment only; FABRICA must still boot if the loopback receiver fails.
...
async function prepareCodexSessionResumeForLaunch(args: {
  ...
  const preparation = await prepareCodexSessionResume({
    ...
    sharedRuntimeCodexHomePath: getFABRICAManagedCodexHomePath(),
  ...
  return preparation.outcome === 'resume'
    ? {
        ...
        reconcileSharedRuntimeAuth:
          ...
          normalizeRuntimePathForComparison(getFABRICAManagedCodexHomePath())
      }
    : preparation
}
...
function openMainWindow(): BrowserWindow {
  ...
  registerCoreHandlers(
    ...
    keybindings,
    {
      ...
      onFABRICAProfileAuthMutation: () => desktopRelayService?.authMutated(),
      onBeforeFABRICAProfileSignOut: () => desktopRelayService?.fenceAndCloseNow()
    },
...
async function presentRendererRecoveryPrompt(recentRecoveryCount: number): Promise<void> {
  ...
  const options = {
    ...
    defaultId: 0,
    cancelId: 1,
    title: 'Fabrica keeps failing to load',
    message: 'The app window crashed repeatedly and stopped reloading automatically.',
    detail: `Fabrica tried to recover ${recentRecoveryCount} times in a row without success. This is often a graphics-driver or installation problem. Reload to try again, or quit and relaunch Fabrica.`
  }
...
void app.whenReady().then(async () => {
  ...
  const activeFABRICAProfile = ensureActiveFABRICAProfile()
  store = new Store({ dataFile: activeFABRICAProfile.dataFile })
  ...
  initializeBrowserSessionsForApp({
    FABRICAProfileId: activeFABRICAProfile.profile.id,
    profileDirectory: activeFABRICAProfile.profileDirectory
  })
  ...
  // Honors DO_NOT_TRACK / FABRICA_TELEMETRY_DISABLED / FABRICA_DIAGNOSTICS_DISABLED
  ...
  void startCodexStateDbBackfillRecoveryInBackground(getFABRICAManagedCodexHomePath())
  ...
  const runtimeService = new FABRICARuntimeService(store, stats, {
  ...
  runtimeService.setCommitMessageAgentEnvironmentResolvers({
    // Why: Codex hooks/auth live in FABRICA's managed runtime home even for the default path, so every launch must resolve CODEX_HOME via runtime-home.
    prepareForCodexLaunch: prepareCodexRuntimeHomeForLaunch,
    prepareForClaudeLaunch: (target) => claudeRuntimeAuth!.prepareForClaudeLaunch(target)
  })
  ...
  // Why: headless `FABRICA serve` clients reach plugins through the runtime RPC
  ...
  // Why: externally started serve-sim processes must stay independent — only FABRICA-managed/attached helpers belong to a workspace.
  ...
  const isE2E = Boolean(process.env.FABRICA_E2E_USER_DATA_DIR)
  const requestedE2EWsPort = process.env.FABRICA_E2E_RUNTIME_WS_PORT
  ...
  if (isE2E && (!Number.isInteger(e2eWsPort) || e2eWsPort < 0 || e2eWsPort > 65_535)) {
    throw new Error(`Invalid FABRICA_E2E_RUNTIME_WS_PORT value: ${requestedE2EWsPort}`)
  }
  // Why: pin dev to 6769 so `pnpm dev` doesn't race packaged FABRICA on 6768 and fall back to a random port, breaking deterministic mobile pairing/repro (STA-1511).
  ...
  runtimeRpc = new FABRICARuntimeRpcServer({
    runtime,
    ...
    // `FABRICA serve` is an explicit remote opt-in, and E2E keeps the wide bind its harness connects over.
    ...
    ...(serveOptions?.wsPort !== undefined
      ? {
          ...
          // Why: only explicit `FABRICA serve --port` overrides a stale STA-1511 fallback (issue #8535); default/dev stay fallback-first for pairing stability.
  ...
  if (serveOptions) {
    ...
    if (process.platform === 'darwin' || process.platform === 'linux') {
      try {
        ...
        console.log(
          `[serve] FABRICA CLI install: ${cliStatus.state}${cliStatus.commandPath ? ` (${cliStatus.commandPath})` : ''}`
        )
      } catch (error) {
        console.warn(
          '[serve] FABRICA CLI install skipped:',
          error instanceof Error ? error.message : String(error)
        )
      }
    }
    // Why: Linux CLI installs as `fabrica`, but the Claude Team launcher invokes bare `fabrica`; drop a ~/.local/bin dispatcher (ahead of /usr/bin) so it resolves. Best-effort.
    if (process.platform === 'linux' && app.isPackaged && process.resourcesPath) {
      try {
        const dispatcher = await installLinuxBareFABRICADispatcher({
          resourcesPath: process.resourcesPath
        })
        console.log(
          `[serve] bare FABRICA dispatcher ${dispatcher.state}: ${dispatcher.dispatcherPath}` +
            `${dispatcher.target ? ` -> ${dispatcher.target}` : ''}`
        )
      } catch (error) {
        console.warn(
          '[serve] bare FABRICA dispatcher install skipped:',
          error instanceof Error ? error.message : String(error)
        )
      }
    }
  ...
  const cloudAuth = getFABRICACloudAuthConfig()
