...
const ready: ServeReadiness = {
  ...
  advertisedEndpoint: 'wss://FABRICA.example.test/runtime',
  managedWslCliReconciliation: 'settled',
  pairing: {
    available: true,
    url: 'FABRICA://pair?code=secret',
    endpoint: 'wss://FABRICA.example.test/runtime',
    deviceId: 'device-1',
    webClientUrl: 'https://FABRICA.example.test/runtime/web-index.html#pairing=secret',
    scope: 'runtime',
    qr: null
  }
}
...
describe('ServeReadinessPublisher', () => {
  it('writes one complete human-readable ready block', async () => {
    ...
    expect(write).toHaveBeenCalledWith(
      expect.stringContaining(
        'Fabrica server ready\nBound endpoint: ws://0.0.0.0:6768\nAdvertised endpoint: wss://FABRICA.example.test/runtime'
      )
    )
    expect(write).toHaveBeenCalledWith(
      expect.stringContaining('Pairing URL: FABRICA://pair?code=secret\n')
    )
  })
  ...
  it('preserves the recipe JSON contract', () => {
    expect(renderServeReadiness(ready, { mode: 'recipe-json', projectRoot: '/workspace' })).toBe(
      '{"schemaVersion":1,"pairingCode":"FABRICA://pair?code=secret","projectRoot":"/workspace"}'
    )
  })
