...
describe('startLocalBuildFeed', () => {
  it('serves only tokenized manifest and validated artifact routes', async () => {
    const directory = await mkdtemp(join(tmpdir(), 'FABRICA-local-feed-'))
    const artifactPath = join(directory, 'fabrica-macos-arm64.zip')
    ...
    const candidate = {
      ...
      artifacts: new Map([['fabrica-macos-arm64.zip', { file: artifactFile, size: 3 }]]),
