...
// so FABRICA manages only its own marker-delimited block: install rewrites the
...
// Why: mirror the Claude-compatible events FABRICA normalizes for status. Kimi uses
...
const BLOCK_START = '# >>> FABRICA-managed-kimi-hooks (managed by FABRICA; do not edit) >>>'
const BLOCK_END = '# <<< FABRICA-managed-kimi-hooks <<<'
...
// FABRICA generates; escape both defensively anyway.
...
// FABRICA-managed script (by filename, so a moved userData path is still swept).
