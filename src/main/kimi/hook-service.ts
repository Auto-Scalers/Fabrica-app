...
function getManagedScript(): string {
  return [
    ...
    // Why: refresh PORT/TOKEN/ENV/VERSION from the current FABRICA install so a PTY
    // that survived an FABRICA restart still reaches the live listener. See
    ...
    'printf \'%s\' "$payload" | curl -sS -X POST "http://127.0.0.1:${FABRICA_AGENT_HOOK_PORT}/hook/kimi" \\',
...
export class KimiHookService {
  ...
  // Why: install FABRICA's managed Kimi hooks on a remote box over SFTP, mirroring
