...
function dataFile(): string {
  return join(testState.dir, 'FABRICA-data.json')
}
...
describe('Store', () => {
  beforeEach(() => {
    testState.dir = mkdtempSync(join(tmpdir(), 'FABRICA-test-'))
    trackMock.mockReset()
  ...
  it('loads state from an explicit profile data file path', async () => {
    ...
    const profileDataFile = join(profileDataDirectory, 'FABRICA-data.json')
  ...
  it('snapshots automation contexts onto runs', async () => {
    const store = await createStore()
    store.addRepo(makeRepo({ upstream: { owner: 'Auto-Scalers', repo: 'fabrica' } }))
    const automation = store.createAutomation({
      name: 'Nightly',
      prompt: 'Run checks',
      agentId: 'claude',
  ...
  it('merges rollback commit-message AI writes into existing source-control AI on load', async () => {
    ...
    store.flush()
    const persisted = JSON.parse(readFileSync(join(testState.dir, 'FABRICA-data.json'), 'utf-8'))
  ...
  it('normalizes malformed terminal shortcut policy on load', async () => {
    ...
    const store = await createStore()
    expect(store.getSettings().terminalShortcutPolicy).toBe('FABRICA-first')
  })
  ...
  it('leaves a hostless persisted upstream hostless rather than inventing one', async () => {
    ...
    expect(store.getRepo('r1')!.upstream).toEqual({ owner: 'Auto-Scalers', repo: 'fabrica' })
  ...
  it('drops corrupt worktreeMeta entries while still normalizing valid siblings', async () => {
    ...
    expect(store.getWorktreeMeta('wt-sibling')?.linkedWorkItem?.jiraIdentifier).toBe('FABRICA-123')
  ...
  it('snapshots the cache at flush and seeds the next Store from the sidecar', async () => {
    ...
    store.flush()
    expect(existsSync(join(testState.dir, 'FABRICA-github-cache.json'))).toBe(true)
  ...
  it('keeps GitHub cache sidecars scoped to explicit profile data files', async () => {
    ...
    const profileADataFile = join(profileADir, 'FABRICA-data.json')
    const profileBDataFile = join(profileBDir, 'FABRICA-data.json')
  ...
  it('durably encrypts SSH PTY consumer ownership for process restart recovery', async () => {
    ...
    expect(existsSync(join(testState.dir, 'FABRICA-github-cache.json'))).toBe(false)
