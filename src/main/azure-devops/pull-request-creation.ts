...
function getAuthConfig(): AzureDevOpsCreateAuthConfig {
  return {
    pat: envValue('FABRICA_AZURE_DEVOPS_TOKEN') ?? envValue('FABRICA_AZURE_DEVOPS_PAT'),
    accessToken: envValue('FABRICA_AZURE_DEVOPS_ACCESS_TOKEN'),
    username: envValue('FABRICA_AZURE_DEVOPS_USERNAME')
  }
}
...
function classifyCreateError(error: unknown): CreateHostedReviewResult {
  ...
  if (
    ...
    return {
      ...
      code: 'auth_required',
      error:
        'Create PR failed: Azure DevOps is not authenticated. Next step: set FABRICA_AZURE_DEVOPS_TOKEN in this environment.'
    }
  }
