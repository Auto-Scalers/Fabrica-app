...
export type DevInstanceIdentity = AppIdentity & {
  ...
  // creating a new one per branch and re-prompting. Distinct from prod's 'Fabrica'.
  appName: string
}
...
export function getDevInstanceIdentity(
  ...
  const repoRoot = cleanEnvValue(env.FABRICA_DEV_REPO_ROOT)
  const branch = cleanEnvValue(env.FABRICA_DEV_BRANCH)
  const worktreeName =
    cleanEnvValue(env.FABRICA_DEV_WORKTREE_NAME) ??
    cleanEnvValue(path.basename(repoRoot ?? process.cwd()))
  const devLabel = cleanEnvValue(env.FABRICA_DEV_INSTANCE_LABEL) ?? formatLabel(branch, worktreeName)
  const dockTitle =
    cleanEnvValue(env.FABRICA_DEV_DOCK_TITLE) ?? `${BASE_APP_NAME}: ${branch ?? devLabel ?? 'dev'}`

  return {
    name: dockTitle,
    // Why: one stable Keychain key ('Fabrica Dev Safe Storage') for all dev
