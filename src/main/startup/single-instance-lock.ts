...
export const SINGLE_INSTANCE_LOCK_FAILURE_MESSAGE =
  '[single-instance] Another Fabrica instance is already running for this userData profile; exiting this launch after requesting the existing window. If no Fabrica process is running, this may be an Electron/macOS single-instance lock failure.'
export const SINGLE_INSTANCE_LOCK_BYPASS_ENV = 'FABRICA_BYPASS_SINGLE_INSTANCE_LOCK'
export const SINGLE_INSTANCE_LOCK_E2E_ENFORCE_ENV = 'FABRICA_E2E_ENFORCE_SINGLE_INSTANCE_LOCK'
export const SINGLE_INSTANCE_LOCK_BYPASS_MESSAGE =
  '[single-instance] FABRICA_BYPASS_SINGLE_INSTANCE_LOCK=1 is set; bypassing the packaged macOS single-instance lock for diagnostics. Do not use this with another Fabrica instance running for the same profile.'
...
// Why: a duplicate `FABRICA serve` is a supervisor artifact, not a user asking for a window; fail open when argv is unavailable.
...
/**
 * Why: FABRICA writes two canonical discovery files into `<userData>/`:
 ...
 * instance quits, metadata points at a dead pid and `FABRICA status` reports
 ...
 * way dev (`FABRICA-dev` userData) and packaged (`FABRICA` userData) runs lock in
