...
const MOUNTED_APPIMAGE_ENV = { APPIMAGE: '/opt/FABRICA/FABRICA.AppImage', APPDIR: '/tmp/.mount_ab12' }
...
describe('serve argv rewrite vs AppImage CLI redirect ordering', () => {
  const launchArgv = ['/opt/FABRICA/fabrica', '--no-sandbox', 'serve', '--port', '7777', '--json']
  ...
  it('leaves non-serve CLI commands redirectable either way', () => {
    const argv = ['/opt/FABRICA/fabrica', 'status']
