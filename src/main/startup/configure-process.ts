...
const HTTP1_COMPATIBILITY_ENV_VAR = 'FABRICA_DISABLE_HTTP2'
...
function readPersistedHttp1CompatibilityMode(userDataPath: string): boolean {
  const dataFile = join(userDataPath, 'FABRICA-data.json')
  ...
  if (process.platform !== 'win32') {
    ...
    if (home) {
      extraPaths.push(
        ...
        // Why: some agent CLIs install into ~/.<name>/bin; GUI-launched Electron's minimal PATH misses them (Auto-Scalers/Fabrica#829).
...
export function configureDevUserDataPath(isDev: boolean): void {
  ...
  if (e2eConfig.userDataDir) {
    ...
    const e2eHomeDir = process.env.FABRICA_E2E_HOME_DIR ?? join(e2eConfig.userDataDir, 'home')
  ...
  const overrideUserDataPath = process.env.FABRICA_DEV_USER_DATA_PATH
  ...
  // Why: without a dev-only path, pnpm dev overwrites the packaged app's runtime pointer under userData and breaks the FABRICA CLI.
  app.setPath('userData', join(app.getPath('appData'), 'fabrica-dev'))
}
...
export function configureFABRICAUserDataPathEnv(): void {
  // Why: relaunches can inherit a stale FABRICA_USER_DATA_PATH; canonicalize before CLI-shared modules build runtime-home paths.
  process.env.FABRICA_USER_DATA_PATH = app.getPath('userData')
}

export function shouldInstallManagedHooks(isDev: boolean): boolean {
  void isDev
  // Why: managed hooks now target FABRICA-owned Codex homes, not ~/.codex, so keep install on for all agents until each gets its own seam.
  return true
}
...
export function installDevParentWatchdog(isDev: boolean): void {
  ...
  const timer = setInterval(() => {
    ...
    if (parentPidChanged || parentMissing) {
      clearInterval(timer)
      // Why: the dev runner spawns Electron without IPC, so on macOS Ctrl+C leaves FABRICA open; watch the parent PID to couple shutdown.
      requestDevParentShutdown()
    }
  }, 1000)
