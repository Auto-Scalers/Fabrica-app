...
// duplicate headless `FABRICA serve` kept executing the rest of startup, reached Linux Ozone/X11 init
...
const MARKER_ENV = 'FABRICA_LOCK_FIXTURE_MARKER'
...
function runLockLossGate(termination: string): FixtureRun {
  const root = mkdtempSync(join(tmpdir(), 'FABRICA-lock-loss-'))
  ...
  writeFileSync(
    ...
    '{ "name": "FABRICA-lock-loss-fixture", "main": "main.js" }'
  )
