...
const resourcesPath = 'C:\\Users\\me\\AppData\\Local\\Programs\\FABRICA\\resources'
const execPath = 'C:\\Users\\me\\AppData\\Local\\Programs\\FABRICA\\FABRICA.exe'
...
describe('packaged CLI entry redirect', () => {
  ...
  it('spawns the in-package CLI in Electron node mode before the single-instance lock can win', () => {
    ...
    expect(spawn).toHaveBeenCalledWith(execPath, [cliEntryPath, 'status', '--json'], {
      env: expect.objectContaining({
        ...
        FABRICA_PACKAGED_CLI_ENTRY_REDIRECTED: '1',
        FABRICA_NODE_OPTIONS: '--inspect',
        FABRICA_NODE_REPL_EXTERNAL_MODULE: 'external-loader'
      }),
      stdio: 'inherit'
    })
  ...
  it('does not redirect development launches', () => {
    ...
    const result = maybeRedirectPackagedCliEntryLaunch({
      argv: ['C:\\dev\\FABRICA.exe', cliEntryPath, 'status'],
      platform: 'win32',
      isPackaged: false,
      resourcesPath,
      execPath: 'C:\\dev\\FABRICA.exe',
      exists: () => true,
      spawn: spawn as never
    })
