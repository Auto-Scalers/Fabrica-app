...
describe('serve-mode-argv', () => {
  it('detects --serve and bare serve subcommand', () => {
    expect(argvRequestsServeMode(['FABRICA', '--serve'])).toBe(true)
    ...
    expect(argvRequestsServeMode(['FABRICA'])).toBe(false)
  })
  ...
  it('leaves real GUI launch argv alone', () => {
    ...
    for (const argv of [
      ['/Applications/FABRICA.app/Contents/MacOS/FABRICA', '-psn_0_123456'],
      ['C:\\Program Files\\FABRICA\\FABRICA.exe', '--squirrel-firstrun'],
      ['C:\\Program Files\\FABRICA\\FABRICA.exe', 'FABRICA://worktree/serve'],
      ['/opt/FABRICA/fabrica', '/home/u/serve'],
      // `--pairing-code` takes the next token, so its value is never the subcommand.
      ['/opt/FABRICA/fabrica', '--pairing-code', 'serve'],
      ['/opt/FABRICA/fabrica', '--environment=serve'],
      ['/opt/FABRICA/fabrica', '--', 'serve'],
      ['/opt/FABRICA/fabrica', 'Serve']
    ]) {
      expect(argvRequestsServeMode(argv), argv.join(' ')).toBe(false)
      expect(normalizeServeModeArgv(argv)).toEqual(argv)
  ...
  it('leaves already-normalized argv unchanged', () => {
    // Why every value flag: the CLI's own `FABRICA serve` spawns the app with exactly this shape
    // (serveFABRICAApp), and the rewrite now runs over it too — a bad mapping would drop the port here.
    const argv = [
      'FABRICA',
      '--serve',
      '--serve-json',
  ...
  it('translates serve flags in the mixed `--serve --port` form', () => {
    ...
    expect(normalizeServeModeArgv(['FABRICA', '--serve', '--port', '9090', '--no-pairing'])).toEqual([
      'FABRICA',
      '--serve',
      '--serve-port',
      '9090',
  ...
  it('leaves a non-serve launch untouched', () => {
    const argv = ['FABRICA', '--no-sandbox', '/home/u/project']
