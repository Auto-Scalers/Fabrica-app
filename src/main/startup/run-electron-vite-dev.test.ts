...
function devWrapperTestEnv(extra: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  ...
  for (const key of Object.keys(env)) {
    if (key.startsWith('FABRICA_DEV_')) {
      delete env[key]
    }
  }
...
describe('run-electron-vite-dev', () => {
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      const tempDir = mkdtempSync(join(tmpdir(), 'FABRICA-dev-wrapper-'))
      ...
      const wrapper = spawn(process.execPath, [wrapperPath], {
        ...
        env: devWrapperTestEnv({
          FABRICA_ELECTRON_VITE_CLI: fakeCliPath,
          FABRICA_SKIP_DEV_CLI_PREPARE: '1',
          FABRICA_SKIP_DEV_ELECTRON_APP_PREPARE: '1',
          FABRICA_SKIP_DEV_WEB_PREPARE: '1',
          FABRICA_DEV_WRAPPER_TEST_PID_FILE: pidFile
        }),
        stdio: 'ignore'
      })
  ...
  it('forwards dev instance identity to electron-vite', async () => {
    ...
    const wrapper = spawn(process.execPath, [wrapperPath, '--remote-debugging-port=9444'], {
      ...
      env: devWrapperTestEnv({
        ...
        FABRICA_SKIP_DEV_WEB_PREPARE: '1',
        FABRICA_DEV_WRAPPER_TEST_PID_FILE: pidFile,
        FABRICA_DEV_WRAPPER_TEST_ENV_FILE: envFile,
    ...
    expect(envSnapshot.dockTitle).toBe('Fabrica: feature/billing-shell')
  ...
  it.skipIf(process.platform === 'win32')(
    'prepares userData FABRICA and FABRICA-dev wrappers for dev terminals',
    async () => {
      const tempDir = mkdtempSync(join(tmpdir(), 'FABRICA-dev-wrapper-'))
      ...
      const wrapper = spawn(process.execPath, [wrapperPath], {
        cwd: resolve('.'),
        env: devWrapperTestEnv({
          FABRICA_DEV_USER_DATA_PATH: userDataPath,
          FABRICA_ELECTRON_VITE_CLI: fakeCliPath,
          FABRICA_SKIP_DEV_ELECTRON_APP_PREPARE: '1',
          FABRICA_SKIP_DEV_WEB_PREPARE: '1',
          FABRICA_DEV_WRAPPER_TEST_PID_FILE: pidFile,
      ...
      const trackedPids = trackPidFile(pidFile)
      const devWrapper = readFileSync(join(userDataPath, 'cli', 'bin', 'FABRICA-dev'), 'utf8')
      const publicAliasWrapper = readFileSync(join(userDataPath, 'cli', 'bin', 'FABRICA'), 'utf8')
      expect(publicAliasWrapper).toBe(devWrapper)
      expect(publicAliasWrapper).toContain('FABRICA_USER_DATA_PATH')
    ...
    const wrapper = spawn(
      ...
      [wrapperPath, '--stable-name', '--remote-debugging-port=9445'],
      {
        ...
        env: devWrapperTestEnv({
          ...
          FABRICA_DEV_WRAPPER_TEST_PID_FILE: pidFile,
          FABRICA_DEV_WRAPPER_TEST_ENV_FILE: envFile,
          FABRICA_DEV_BRANCH: 'feature/stable-name',
          FABRICA_DEV_WORKTREE_NAME: 'stable-ui'
        }),
        stdio: 'ignore'
      }
    )
  ...
  it.skipIf(process.platform !== 'darwin')(
    ...
    async () => {
      ...
      const baseEnv = devWrapperTestEnv({
        ...
        FABRICA_SKIP_DEV_CLI_PREPARE: '1',
        FABRICA_SKIP_DEV_WEB_PREPARE: '1',
        FABRICA_DEV_BRANCH: 'feature/rebuild-electron-app',
        FABRICA_DEV_WORKTREE_NAME: 'electron-app-rebuild'
      })
    ...
    async () => {
      ...
      const wrapper = spawn(process.execPath, [wrapperPath, '--remote-debugging-port=9448'], {
        ...
        env: devWrapperTestEnv({
          ...
          FABRICA_DEV_WRAPPER_TEST_PID_FILE: pidFile,
          FABRICA_DEV_WRAPPER_TEST_ENV_FILE: envFile,
          FABRICA_DEV_BRANCH: 'feature/framework-symlinks',
          FABRICA_DEV_WORKTREE_NAME: 'symlink-ui'
        }),
        stdio: 'ignore'
      })
