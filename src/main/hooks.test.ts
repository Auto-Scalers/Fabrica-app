...
import { getDefaultTabsLaunch, parseFABRICAYaml } from './hooks'
...
const TEST_REPO_FABRICA_YAML_PATH = join(TEST_REPO_PATH, 'FABRICA.yaml')
const TEST_WORKTREE_FABRICA_YAML_PATH = join(TEST_WORKTREE_PATH, 'FABRICA.yaml')
const TEST_ISSUE_COMMAND_PATH = join(TEST_REPO_PATH, '.fabrica', 'issue-command')
const TEST_GITIGNORE_PATH = join(TEST_REPO_PATH, '.gitignore')

describe('parseFABRICAYaml', () => {
  it('parses YAML with setup script only', () => {
    const yaml = `scripts:\n  setup: |\n    echo "setting up"\n    npm install\n`
    const result = parseFABRICAYaml(yaml)
  ...
  it('returns null when there is no scripts block', () => {
    const yaml = `other:\n  key: value\n`
    expect(parseFABRICAYaml(yaml)).toBeNull()
  })
  ...
  it('returns null for empty string', () => {
    expect(parseFABRICAYaml('')).toBeNull()
  })
  ...
  it('parses default terminal tabs from fabrica.yaml', () => {
    const yaml = [
      'defaultTabs:',
      '  - title: Claude',
      '    color: "#f97316"',
    ...
    expect(parseFABRICAYaml(yaml)).toEqual({
      scripts: {},
      defaultTabs: [
  ...
  it('parses environmentRecipes from fabrica.yaml', () => {
    const yaml = [
      'environmentRecipes:',
      '  - id: cloud-sandbox',
      '    name: Cloud Sandbox',
      '    description: Starts a per-workspace VM.',
      '    create: ./scripts/FABRICA-vm/start-cloud-sandbox.sh',
      '    suspend: ./scripts/FABRICA-vm/suspend-cloud-sandbox.sh',
      '    resume: ./scripts/FABRICA-vm/resume-cloud-sandbox.sh',
      '    destroy: ./scripts/FABRICA-vm/destroy-cloud-sandbox.sh'
    ].join('\n')
  ...
  it('parses legacy environmentRecipes command and cleanup aliases', () => {
    const yaml = [
      ...
      '  - id: manual-sandbox',
      '    name: Manual Sandbox',
      '    command: ./scripts/FABRICA-vm/start-manual-sandbox.sh',
      '    cleanup: none'
    ].join('\n')
      ...
      environmentRecipes: [
        {
          id: 'manual-sandbox',
          name: 'Manual Sandbox',
          create: './scripts/FABRICA-vm/start-manual-sandbox.sh',
  ...
  it('drops invalid and duplicate environmentRecipes', () => {
    const yaml = [
      ...
      '    create: ./scripts/FABRICA-vm/start-duplicate.sh',
      '  - id: missing-create',
      '    name: Missing Create',
      '  - name: Missing Id',
      '    create: ./scripts/FABRICA-vm/start-missing-id.sh',
      '  - id: "Cloud Sandbox"',
      '    name: Unsafe Id',
      '    create: ./scripts/FABRICA-vm/start-unsafe-id.sh',
      '  - 42'
    ].join('\n')
  ...
  it('parses worktree.sharedDirectories from fabrica.yaml', () => {
    const result = parseFABRICAYaml(
  ...
  it('returns null when sharedDirectories is the only key and holds nothing usable', () => {
    expect(parseFABRICAYaml('worktree:\n  sharedDirectories: []\n')).toBeNull()
    expect(parseFABRICAYaml('worktree:\n  sharedDirectories: node_modules\n')).toBeNull()
  })

  it('keeps sharedDirectories alongside other fabrica.yaml keys', () => {
    const result = parseFABRICAYaml(
      [
        'scripts:',
        '  setup: pnpm install',
...
describe('hasUnrecognizedFABRICAYamlKeys', () => {
  it('returns true when the file contains only keys this version does not handle', async () => {
    ...
    const { hasUnrecognizedFABRICAYamlKeys } = await import('./hooks')
    expect(hasUnrecognizedFABRICAYamlKeys('/test/repo')).toBe(true)
  })
  ...
  it('returns false when the file contains only recognised keys', async () => {
    ...
    const { hasUnrecognizedFABRICAYamlKeys } = await import('./hooks')
    expect(hasUnrecognizedFABRICAYamlKeys('/test/repo')).toBe(false)
  })
...
describe('readIssueCommand', () => {
  it('prefers the local override over the shared fabrica.yaml command', async () => {
    const fs = await import('node:fs')
    vi.mocked(fs.existsSync).mockImplementation(
      (path) => path === TEST_ISSUE_COMMAND_PATH || path === TEST_REPO_FABRICA_YAML_PATH
    )
    vi.mocked(fs.readFileSync).mockImplementation((path) => {
      if (path === TEST_ISSUE_COMMAND_PATH) {
        return 'local command\n'
      }
      if (path === TEST_REPO_FABRICA_YAML_PATH) {
        return 'issueCommand: |\n  shared command\n'
      }
      return ''
    })
  ...
  it('falls back to the shared fabrica.yaml command when no local override exists', async () => {
    const fs = await import('node:fs')
    vi.mocked(fs.existsSync).mockImplementation((path) => path === TEST_REPO_FABRICA_YAML_PATH)
    vi.mocked(fs.readFileSync).mockImplementation((path) => {
      if (path === TEST_REPO_FABRICA_YAML_PATH) {
        return 'issueCommand: |\n  shared command\n'
      }
...
describe('writeIssueCommand', () => {
  it('writes only the local override file and keeps .fabrica ignored locally', async () => {
    const fs = await import('node:fs')
    vi.mocked(fs.existsSync).mockImplementation(
      (path) => path === TEST_GITIGNORE_PATH || path === join(TEST_REPO_PATH, '.fabrica')
    )
    vi.mocked(fs.readFileSync).mockImplementation((path) => {
...
describe('getEffectiveHooks', () => {
  ...
  it('uses hooks from fabrica.yaml when present', async () => {
    const fs = await import('node:fs')
  ...
  it("loads setup hooks from the target worktree's fabrica.yaml when a worktree path is provided", async () => {
    const fs = await import('node:fs')
    vi.mocked(fs.existsSync).mockImplementation(
      (path) => path === TEST_REPO_FABRICA_YAML_PATH || path === TEST_WORKTREE_FABRICA_YAML_PATH
    )
    vi.mocked(fs.readFileSync).mockImplementation((path) => {
      if (path === TEST_REPO_FABRICA_YAML_PATH) {
        return 'scripts:\n  setup: |\n    echo old-version\n'
      }
      if (path === TEST_WORKTREE_FABRICA_YAML_PATH) {
        return 'scripts:\n  setup: |\n    echo new-version\n'
      }
      return ''
    })
  ...
  it('uses local settings by default even when fabrica.yaml defines only one command', async () => {
    const fs = await import('node:fs')
    vi.mocked(fs.existsSync).mockReturnValue(true)
    vi.mocked(fs.readFileSync).mockReturnValue('scripts:\n  archive: |\n    echo "yaml archive"\n')
  ...
  it('treats legacy shared-first policy as fabrica.yaml only', async () => {
    const fs = await import('node:fs')
    vi.mocked(fs.existsSync).mockReturnValue(true)
    vi.mocked(fs.readFileSync).mockReturnValue('scripts:\n  archive: |\n    echo "yaml archive"\n')
...
describe('runHook', () => {
  ...
  it('runs WSL hooks through wsl.exe and translates env paths to Linux', async () => {
    ...
    execFileMock.mockImplementation((_file, _args, options, callback) => {
      ...
      expect(options).toEqual(
        expect.objectContaining({
          env: expect.objectContaining({
            FABRICA_ROOT_PATH: '/mnt/c/Users/jinwo/git/FABRICA',
            FABRICA_WORKTREE_PATH: '/home/jin/feature',
            CONDUCTOR_ROOT_PATH: '/mnt/c/Users/jinwo/git/FABRICA',
            GHOSTX_ROOT_PATH: '/mnt/c/Users/jinwo/git/FABRICA'
          })
        })
      )
      return {} as never
    ...
    try {
      ...
      const result = await runHook('setup', '\\\\wsl.localhost\\Ubuntu\\home\\jin\\feature', {
        ...makeRepo(),
        path: 'C:\\Users\\jinwo\\git\\FABRICA'
      })
  ...
  it('runs Windows-path hooks through WSL when the project runtime targets WSL', async () => {
    ...
    try {
      ...
      const result = await runHook(
        'setup',
        'C:\\Users\\jinwo\\git\\FABRICA-feature',
        {
          ...makeRepo(),
          path: 'C:\\Users\\jinwo\\git\\FABRICA'
        },
      ...
      expect(execFileMock).toHaveBeenCalledWith(
        ...
        [
          ...
          'bash',
          '-c',
          "cd '/mnt/c/Users/jinwo/git/FABRICA-feature' && echo hello"
        ],
        expect.any(Object),
        expect.any(Function)
      )
      expect(capturedOptions).toEqual(
        expect.objectContaining({
          env: expect.objectContaining({
            FABRICA_ROOT_PATH: '/mnt/c/Users/jinwo/git/FABRICA',
            FABRICA_WORKTREE_PATH: '/mnt/c/Users/jinwo/git/FABRICA-feature',
            CONDUCTOR_ROOT_PATH: '/mnt/c/Users/jinwo/git/FABRICA',
            GHOSTX_ROOT_PATH: '/mnt/c/Users/jinwo/git/FABRICA',
            ...
            WSLENV: expect.stringContaining(
              'FABRICA_ROOT_PATH/u:FABRICA_WORKTREE_PATH/u:CONDUCTOR_ROOT_PATH/u:GHOSTX_ROOT_PATH/u:FABRICA_WORKSPACE_NAME/u'
            )
          })
        })
      )
  ...
  it('writes Windows-path setup runners through WSL git when the project runtime targets WSL', async () => {
    ...
    gitExecFileSyncMock.mockReturnValue('/mnt/c/Users/jinwo/git/FABRICA/.git/FABRICA/setup-runner.sh\n')
    ...
    try {
      ...
      expect(gitExecFileSyncMock).toHaveBeenCalledWith(
        ...
        {
          cwd: 'C:\\Users\\jinwo\\git\\FABRICA-feature',
          wslDistro: 'Ubuntu'
        }
      )
...
describe('createSetupRunnerScript', () => {
  ...
  it('writes POSIX setup runners for shebang-declared scripts on native Windows paths', async () => {
    gitExecFileSyncMock.mockReset()
    gitExecFileSyncMock.mockReturnValue('C:\\repo\\.git\\FABRICA\\setup-runner.sh\n')
    const fs = await import('node:fs')
    const writeFileSyncMock = vi.mocked(fs.writeFileSync)
    const chmodSyncMock = vi.mocked(fs.chmodSync)
    ...
    try {
      ...
      expect(writeFileSyncMock).toHaveBeenCalledWith(
        'C:\\repo\\.git\\FABRICA\\setup-runner.sh',
        '#!/usr/bin/env bash\nset -e\npnpm install\nnpm run build\n',
        'utf-8'
      )
      expect(chmodSyncMock).not.toHaveBeenCalled()
      expect(result).toMatchObject({
        runnerScriptPath: 'C:\\repo\\.git\\FABRICA\\setup-runner.sh',
        shell: { family: 'posix' }
      })
    } finally {
  ...
  it('keeps batch setup scripts on cmd.exe when the terminal is Git Bash', async () => {
    ...
    gitExecFileSyncMock.mockReset()
    gitExecFileSyncMock.mockReturnValue('C:\\repo\\.git\\FABRICA\\setup-runner.cmd\n')
    const fs = await import('node:fs')
    const writeFileSyncMock = vi.mocked(fs.writeFileSync)
    writeFileSyncMock.mockClear()
    const originalPlatform = process.platform
    ...
    try {
      ...
      expect(writeFileSyncMock).toHaveBeenCalledWith(
        'C:\\repo\\.git\\FABRICA\\setup-runner.cmd',
        expect.stringContaining('call copy .env.example .env\r\n'),
        'utf-8'
      )
      expect(result).toMatchObject({
        runnerScriptPath: 'C:\\repo\\.git\\FABRICA\\setup-runner.cmd',
  ...
  it('keeps POSIX runner behavior on POSIX platforms', async () => {
    gitExecFileSyncMock.mockReset()
    gitExecFileSyncMock.mockReturnValue('/test/repo/.git/FABRICA/setup-runner.sh\n')
    const fs = await import('node:fs')
    const writeFileSyncMock = vi.mocked(fs.writeFileSync)
    const chmodSyncMock = vi.mocked(fs.chmodSync)
    writeFileSyncMock.mockClear()
    ...
    try {
      ...
      expect(writeFileSyncMock).toHaveBeenCalledWith(
        '/test/repo/.git/FABRICA/setup-runner.sh',
        '#!/usr/bin/env bash\nset -e\npnpm install\n',
        'utf-8'
      )
      expect(chmodSyncMock).toHaveBeenCalledWith('/test/repo/.git/FABRICA/setup-runner.sh', 0o755)
  ...
  it('marks setup-runner terminals for the always-on credential guard', async () => {
    ...
    expect(setup.envVars).toMatchObject({
      FABRICA_ROOT_PATH: '/test/repo',
      FABRICA_WORKTREE_PATH: '/test/worktree',
      FABRICA_INTERNAL_TERMINAL_GIT_CREDENTIAL_GUARD_POLICY: 'guard'
    })
  })
})
...
describe('createIssueCommandRunnerScript', () => {
  ...
  it('writes a POSIX issue-command runner when a shebang declares bash and setup resolves to Git Bash', async () => {
    gitExecFileSyncMock.mockReset()
    gitExecFileSyncMock.mockReturnValue('C:\\repo\\.git\\FABRICA\\issue-command-runner.sh\n')
    const fs = await import('node:fs')
    const writeFileSyncMock = vi.mocked(fs.writeFileSync)
    writeFileSyncMock.mockClear()
    const originalPlatform = process.platform
    ...
    try {
      ...
      expect(gitExecFileSyncMock).toHaveBeenCalledWith(
        ['rev-parse', '--git-path', 'FABRICA/issue-command-runner.sh'],
        { cwd: 'C:\\repo-worktree' }
      )
      expect(writeFileSyncMock).toHaveBeenCalledWith(
        'C:\\repo\\.git\\FABRICA\\issue-command-runner.sh',
        '#!/usr/bin/env bash\nset -e\ngh issue view 42\n',
        'utf-8'
      )
  ...
  it('keeps a plain issue command on the cmd runner under a Git Bash terminal', async () => {
    gitExecFileSyncMock.mockReset()
    gitExecFileSyncMock.mockReturnValue('C:\\repo\\.git\\FABRICA\\issue-command-runner.cmd\n')
    const originalPlatform = process.platform
    Object.defineProperty(process, 'platform', { configurable: true, value: 'win32' })
    ...
    try {
      ...
      expect(gitExecFileSyncMock).toHaveBeenCalledWith(
        ['rev-parse', '--git-path', 'FABRICA/issue-command-runner.cmd'],
        { cwd: 'C:\\repo-worktree' }
      )
