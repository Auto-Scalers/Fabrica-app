...
describe('keybinding-file', () => {
  ...
  beforeEach(() => {
    dir = mkdtempSync(join(tmpdir(), 'FABRICA-keybindings-'))
  ...
  it('resolves the user-facing keybindings path under ~/.fabrica', () => {
    expect(getUserKeybindingsPath('/home/test')).toBe(
      join('/home/test', '.fabrica', 'keybindings.json')
    )
  })
  ...
  it('preserves valid plugin overrides while rejecting malformed plugin action IDs', () => {
    writeFileSync(
      ...
      JSON.stringify({
        keybindings: {
          'plugin:fabrica-samples.tasks/open': 'Mod+Shift+T',
    ...
    writeKeybindingOverride(filePath, 'linux', 'plugin:fabrica-samples.tasks/open', [])
    expect(readKeybindingFile(filePath, 'linux').overrides).toEqual({
      'plugin:fabrica-samples.tasks/open': []
    })
  })
