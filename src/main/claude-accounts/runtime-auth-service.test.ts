...
describe('ClaudeRuntimeAuthService', () => {
  beforeEach(() => {
    ...
    testState.userDataDir = mkdtempSync(join(tmpdir(), 'FABRICA-claude-runtime-'))
    testState.fakeHomeDir = mkdtempSync(join(tmpdir(), 'FABRICA-claude-home-'))
  ...
  it('adopts canonical legacy managed auth paths without existing markers', async () => {
    ...
    const markerPath = join(managedAuthPath, '.FABRICA-managed-claude-auth')
    expect(readFileSync(runtimeCredentialsPath, 'utf-8')).toBe(managedCredentials)
  ...
  it('restores system auth when switching from an owned account to missing credentials', async () => {
    ...
    writeFileSync(join(managedAuthPath2, '.FABRICA-managed-claude-auth'), 'account-2\n', 'utf-8')
    writeFileSync(
  ...
  it('routes refreshed Claude credentials to the matching managed account', async () => {
    ...
    // FABRICA selected account-2. Persist that refresh to account-1, then restore
  ...
  it('uses account WSL runtime for untargeted Claude preparation instead of stale terminal WSL settings', async () => {
    ...
    const settings = createSettings({
      ...
      claudeManagedAccounts: [
        createClaudeAccount('ubuntu-account', ubuntuAuthPath, {
          ...
          wslDistro: 'Ubuntu',
          wslLinuxAuthPath: '/home/alice/.local/share/FABRICA/claude-accounts/ubuntu/auth'
        })
      ],
      activeClaudeManagedAccountId: null,
    ...
    expect(preparation).toMatchObject({
      runtime: 'wsl',
      wslDistro: 'Ubuntu',
      wslLinuxConfigDir: '/home/alice/.local/share/FABRICA/claude-accounts/ubuntu/auth',
    ...
    const settings = createSettings({
      claudeManagedAccounts: [
        createClaudeAccount('account-1', managedAuthPath, {
          managedAuthRuntime: 'wsl',
          wslDistro: 'Ubuntu',
          wslLinuxAuthPath: '/home/alice/.local/share/FABRICA/claude-accounts/account-1/auth'
        })
      ],
      activeClaudeManagedAccountId: null,
    ...
    const settings = createSettings({
      claudeManagedAccounts: [
        ...
        createClaudeAccount('debian-account', debianAuthPath, {
          managedAuthRuntime: 'wsl',
          wslDistro: 'Debian',
          wslLinuxAuthPath: '/home/alice/.local/share/FABRICA/claude-accounts/debian/auth'
        })
      ],
      activeClaudeManagedAccountId: null,
      activeClaudeManagedAccountIdsByRuntime: {
