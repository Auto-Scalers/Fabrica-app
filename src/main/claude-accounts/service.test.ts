...
const CLAUDE_SERVICE_TEST_ROOT = join(tmpdir(), 'FABRICA-claude-service-test')
...
describe('ClaudeAccountService credential capture', () => {
  ...
  it('falls back to captured credentials file on macOS', async () => {
    tempDir = mkdtempSync(join(tmpdir(), 'FABRICA-claude-capture-'))
  ...
  it('restores previous managed auth when reauth materialization fails', async () => {
    ...
    writeFileSync(join(managedAuthPath, '.FABRICA-managed-claude-auth'), 'account-1\n', 'utf-8')
  ...
  it('rejects adding a Claude account whose identity already exists', async () => {
    ...
    const existingMarkerPath = join(existingAuthPath, '.FABRICA-managed-claude-auth')
  ...
  it('adds a Claude account with the same email under a different organization', async () => {
    ...
    mkdirSync(existingAuthPath, { recursive: true })
    writeFileSync(
      join(existingAuthPath, '.FABRICA-managed-claude-auth'),
      'existing-account\n',
      'utf-8'
    )
    let settings = {
    ...
    let settings = {
      claudeManagedAccounts: [
        ...
        },
        {
          ...
          wslDistro: 'Ubuntu',
          wslLinuxAuthPath: '/home/jin/.local/share/FABRICA/claude-accounts/wsl-account/auth',
          authMethod: 'subscription-oauth',
          organizationUuid: null,
          organizationName: null,
          createdAt: 1,
  ...
  it('removes a WSL account without clearing the Windows active account', async () => {
    ...
    mkdirSync(hostAuthPath, { recursive: true })
    mkdirSync(wslAuthPath, { recursive: true })
    writeFileSync(join(wslAuthPath, '.FABRICA-managed-claude-auth'), 'wsl-account\n', 'utf-8')
    let settings = {
      claudeManagedAccounts: [
        {
          id: 'host-account',
    ...
    try {
      ...
      await (
        ...
        ['auth', 'status', '--json'],
        {
          ...
          linuxPath: '/home/user/.config/FABRICA auth',
      ...
      expect(spawnMock).toHaveBeenCalledWith(
        ...
        [
          ...
          'bash',
          '-lc',
          "export CLAUDE_CONFIG_DIR='/home/user/.config/FABRICA auth'; exec claude 'auth' 'status' '--json'"
        ],
...
describe('ClaudeAccountService.addAccountFromConfigDir', () => {
  ...
  it('registers a managed account by capturing an authenticated config dir', async () => {
    sourceDir = mkdtempSync(join(tmpdir(), 'FABRICA-claude-source-'))
    writeFileSync(
  ...
  it('captures only the config-scoped macOS Keychain credential', async () => {
    setPlatform('darwin')
    sourceDir = mkdtempSync(join(tmpdir(), 'FABRICA-claude-source-keychain-'))
  ...
  it('does not mistake an unchanged legacy Keychain credential for the temp login', async () => {
    setPlatform('darwin')
    sourceDir = mkdtempSync(join(tmpdir(), 'FABRICA-claude-source-keychain-empty-'))
    vi.mocked(readActiveClaudeKeychainCredentialsStrict).mockImplementation(async (configDir) =>
  ...
  it('captures a legacy Keychain credential that changed after login began', async () => {
    setPlatform('darwin')
    sourceDir = mkdtempSync(join(tmpdir(), 'FABRICA-claude-source-keychain-legacy-'))
  ...
  it('still registers when the daemon cannot spawn `claude auth status`', async () => {
    ...
    sourceDir = mkdtempSync(join(tmpdir(), 'FABRICA-claude-source-nostatus-'))
    writeFileSync(
      join(sourceDir, '.credentials.json'),
      '{"claudeAiOauth":{"accessToken":"tok"}}\n',
      'utf-8'
  ...
  it('rejects and rolls back when the config dir has no credentials', async () => {
    sourceDir = mkdtempSync(join(tmpdir(), 'FABRICA-claude-source-empty-'))
    const deps = makeDeps()
    const { ClaudeAccountService } = await import('./service')
    const service = new ClaudeAccountService(
      deps.store as never,
