...
export class ClaudeAccountService {
   ...
   * `FABRICA account add` CLI to run `claude login` in the user's own terminal on a
  ...
  private async persistCapturedClaudeAccount(
    ...
    if (!captured.identity.email) {
      throw new Error('Claude login completed, but Fabrica could not resolve the account email.')
    }
  ...
  private createTemporaryClaudeConfigDir(location: ManagedClaudeAuthLocation): {
    ...
    if (location.managedAuthRuntime !== 'wsl') {
      return {
        windowsPath: mkdtempSync(join(tmpdir(), 'FABRICA-claude-login-')),
        linuxPath: null,
        wslDistro: null
      }
    }
    ...
    const linuxPath = execFileSync(
      ...
      [
        ...
        'bash',
        '-lc',
        'mktemp -d "${TMPDIR:-/tmp}/FABRICA-claude-login.XXXXXX"'
      ],
  ...
  private createManagedAuthDir(
    ...
    writeFileSync(join(managedAuthPath, '.fabrica-managed-claude-auth'), `${accountId}\n`, 'utf-8')
    return {
  ...
  private tryCreateWslManagedAuthDir(
    ...
    const wslLinuxAuthPath = `${home.replace(/\/$/, '')}/.local/share/fabrica/claude-accounts/${accountId}/auth`
    const markerPath = `${wslLinuxAuthPath}/.fabrica-managed-claude-auth`
    execFileSync(
      'wsl.exe',
      [
        '-d',
  ...
  private assertManagedAuthPath(candidatePath: string, expectedAccountId?: string): string {
    ...
    if (wslInfo) {
      if (
        !wslInfo.linuxPath.includes('/.local/share/fabrica/claude-accounts/') ||
        !wslInfo.linuxPath.endsWith('/auth')
      ) {
        throw new Error('Managed WSL Claude auth storage is outside Fabrica account storage.')
      }
            ...
            [
              ...
              buildEncodedWslBashCommand(
                [
                  ...
                  'managed_root="${HOME%/}/.local/share/fabrica/claude-accounts"',
                  'candidate_real=$(readlink -f -- "$candidate")',
                  'managed_root_real=$(readlink -f -- "$managed_root")',
                  'test -f "$candidate_real/.fabrica-managed-claude-auth"',
                  expectedAccountId
                    ? `test "$(cat "$candidate_real/.fabrica-managed-claude-auth")" = ${shellQuote(expectedAccountId)}`
                    : 'test -n "$(cat "$candidate_real/.fabrica-managed-claude-auth")"',
      ...
      if (
        ...
        !existsSync(join(candidatePath, '.fabrica-managed-claude-auth'))
      ) {
        throw new Error('Managed Claude auth storage is not owned by Fabrica.')
      }
