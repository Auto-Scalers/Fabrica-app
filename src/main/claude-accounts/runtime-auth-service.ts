...
export class ClaudeRuntimeAuthService {
  ...
  // Why: creds FABRICA last wrote to the shared file; a mismatch on managed→default transition means an external login overwrote it, so adopt it as the new default.
  ...
  private async doSyncForCurrentSelection(target?: ClaudeAccountSelectionTarget): Promise<void> {
    ...
    if (activeAccount.managedAuthRuntime === 'wsl') {
      if (!this.getOwnedManagedAuthPath(activeAccount)) {
        console.warn(
          '[claude-runtime-auth] Active WSL managed account is not owned by Fabrica, restoring system default'
        )
        const nextSelection = setSelectedClaudeAccountIdForTarget(
          normalizeClaudeRuntimeSelection(settings),
          null,
    ...
    if (!this.getOwnedManagedAuthPath(activeAccount)) {
      console.warn(
        '[claude-runtime-auth] Active managed account is not owned by Fabrica, restoring system default'
      )
      if (this.lastSyncedAccountId !== null) {
        if (
          previousAccount &&
            ...
            [
              ...
              buildEncodedWslBashCommand(
                [
                  ...
                  `test "$(cat "$candidate_real/.fabrica-managed-claude-auth")" = ${shellQuote(account.id)}`,
  ...
  private getOwnedRuntimeOauthBaseline(
    ...
    // Why: managed metadata hints identity but isn't proof FABRICA wrote .claude.json; use only after a credential surface proves ownership.
