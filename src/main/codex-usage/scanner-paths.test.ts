...
beforeEach(() => {
  ...
  fakeHomeDir = mkdtempSync(join(tmpdir(), 'FABRICA-codex-usage-home-'))
  userDataDir = mkdtempSync(join(tmpdir(), 'FABRICA-codex-usage-user-data-'))
...
describe('getCodexSessionsDirectory', () => {
  it('defaults to Fabrica-managed Codex runtime sessions', () => {
...
describe('listCodexSessionFiles', () => {
  it('scans both Fabrica-managed and system Codex session homes', async () => {
  ...
  it('scans per-account self-contained Codex homes that hold sessions', async () => {
    ...
    writeFileSync(
      join(userDataDir, 'codex-accounts', 'acct-1', 'home', '.fabrica-managed-home'),
      'acct-1\n',
      'utf-8'
    )
  ...
  it('does not scan an account home redirected outside managed storage', async () => {
    ...
    writeFileSync(join(externalHome, '.fabrica-managed-home'), 'acct-redirected\n', 'utf-8')
  ...
  it('does not scan a sessions root redirected outside an owned account home', async () => {
    ...
    mkdirSync(externalSessionsDir, { recursive: true })
    writeFileSync(join(accountHome, '.fabrica-managed-home'), 'acct-redirected-sessions\n', 'utf-8')
    writeFileSync(join(externalSessionsDir, 'unrelated.jsonl'), '{}\n', 'utf-8')
    symlinkSync(
      externalSessionsDir,
