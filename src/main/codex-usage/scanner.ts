...
export function getCodexSessionsDirectory(): string {
  // Why: FABRICA-launched Codex processes receive an FABRICA-owned CODEX_HOME, so
  // callers that need the primary runtime path should not consult ambient
  // shell CODEX_HOME.
  return join(getFABRICAManagedCodexHomePath(), 'sessions')
}
...
function hasLegacyCopiedSessionBridgeMarkers(): boolean {
  return existsSync(join(getFABRICAManagedCodexHomePath(), '.fabrica-session-copies'))
}
...
export async function attributeCodexUsageEvent(
  ...
  if (event.cwd) {
    ...
    if (worktree) {
      ...
      // Why: all-local mode should still collapse repeated off-FABRICA sessions by
