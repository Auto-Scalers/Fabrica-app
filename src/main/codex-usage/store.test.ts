...
const { getPathMock } = vi.hoisted(() => ({
  getPathMock: vi.fn(() => '/tmp/FABRICA-test-userdata')
}))
...
describe('CodexUsageStore', () => {
  ...
  beforeEach(() => {
    tempUserData = mkdtempSync(join(tmpdir(), 'FABRICA-codex-usage-store-'))
  ...
  it('adapts Codex scans to compact cache persistence', async () => {
    ...
    const persistedJson = readFileSync(join(tempUserData, 'FABRICA-codex-usage.json'), 'utf-8')
    expect(scanCodexUsageFiles).toHaveBeenCalledWith([], [])
    expect(persistedJson).toBe(JSON.stringify(JSON.parse(persistedJson)))
  })

  it('reports no data for FABRICA scope when only non-FABRICA Codex usage exists', async () => {
    const store = createStoreWithState({
      sessions: [
        {
          sessionId: 'session-1',
    ...
    const summary = await store.getSummary('FABRICA', '30d')
  ...
  it('prices current Codex models with current model rates', async () => {
    ...
    const summary = await store.getSummary('FABRICA', '30d')
    const breakdown = await store.getBreakdown('FABRICA', '30d', 'model')
  ...
  it('uses only FABRICA-scoped models when projecting mixed-scope sessions', async () => {
    const store = createStoreWithState({
      sessions: [
        {
          sessionId: 'session-1',
    ...
    const breakdown = await store.getBreakdown('FABRICA', '30d', 'model')
    const recentSessions = await store.getRecentSessions('FABRICA', '30d', 10)
