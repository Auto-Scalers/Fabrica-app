...
function getAuthConfig(): GiteaAuthConfig {
  const apiBaseUrl = envValue('FABRICA_GITEA_API_BASE_URL')
  return {
    apiBaseUrl: apiBaseUrl ? normalizeGiteaApiBaseUrl(apiBaseUrl) : null,
    token: envValue('FABRICA_GITEA_TOKEN')
  }
}
...
/** Invalidate the shared /pulls scan after FABRICA itself creates a PR so the
