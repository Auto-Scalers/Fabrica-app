...
const VERSION = '1.2.3'
const PAYLOAD = 'FABRICA package payload'
...
function capture(overrides: Record<string, unknown> = {}): void {
  const downloadedFile = (overrides.downloadedFile ?? path.join(downloadDir, 'FABRICA.deb')) as string
...
beforeEach(async () => {
  ...
  tempRoot = await fsp.mkdtemp(path.join(os.tmpdir(), 'FABRICA-recovery-'))
  cacheRoot = path.join(tempRoot, 'cache')
  updaterDir = path.join(cacheRoot, 'FABRICA-updater')
...
describe('captureLinuxPackageArtifact', () => {
  it('retains the downloaded package with the digest from the event metadata', () => {
    ...
    expect(recovery.getTrackedLinuxPackageArtifact()).toEqual({
      packageType: 'deb',
      version: VERSION,
      path: path.join(downloadDir, 'FABRICA.deb'),
      sha512: SHA512
    })
  })
  ...
  it('requires an absolute downloaded path', () => {
    capture({ downloadedFile: 'FABRICA.deb' })
    expect(recovery.getTrackedLinuxPackageArtifact()).toBeNull()
  })
  ...
  it('requires a string downloaded path', () => {
    recovery.captureLinuxPackageArtifact({
      downloadedFile: 123,
      version: VERSION,
      files: [{ url: 'FABRICA.deb', sha512: SHA512 }]
    })
    expect(recovery.getTrackedLinuxPackageArtifact()).toBeNull()
  })

  it('requires the package extension to match the installed format', () => {
    capture({ downloadedFile: path.join(downloadDir, 'FABRICA.rpm') })
    expect(recovery.getTrackedLinuxPackageArtifact()).toBeNull()
  })

  it('accepts an uppercase extension', () => {
    capture({ downloadedFile: path.join(downloadDir, 'FABRICA.DEB') })
    expect(recovery.getTrackedLinuxPackageArtifact()?.path).toBe(path.join(downloadDir, 'FABRICA.DEB'))
  })
  ...
  it('matches an absolute file URL by basename', () => {
    capture({
      files: [{ url: 'https://downloads.example.com/releases/1.2.3/FABRICA.deb', sha512: SHA512 }]
    })
  ...
  it('matches a percent-encoded relative URL against the decoded basename', () => {
    const downloadedFile = path.join(downloadDir, 'FABRICA Setup 1.2.3.deb')
    capture({ downloadedFile, files: [{ url: 'FABRICA%20Setup%201.2.3.deb', sha512: SHA512 }] })
  ...
  it('ignores entries for other files and formats', () => {
    capture({
      files: [
        { url: 'FABRICA.AppImage', sha512: 'other-appimage-digest' },
        { url: 'FABRICA-arm64.deb', sha512: 'other-arch-digest' },
        { url: 'FABRICA.deb', sha512: SHA512 }
      ]
    })
    expect(recovery.getTrackedLinuxPackageArtifact()?.sha512).toBe(SHA512)
  })
  ...
  it('accepts duplicate entries that agree on the digest', () => {
    capture({
      files: [
        { url: 'FABRICA.deb', sha512: SHA512 },
        { url: 'https://downloads.example.com/FABRICA.deb', sha512: SHA512 }
      ]
    })
    expect(recovery.getTrackedLinuxPackageArtifact()?.sha512).toBe(SHA512)
  })
  ...
  it('refuses an ambiguous basename with two different digests', () => {
    capture({
      files: [
        { url: 'FABRICA.deb', sha512: SHA512 },
        { url: 'https://mirror.example.com/FABRICA.deb', sha512: 'conflicting-digest' }
      ]
    })
    expect(recovery.getTrackedLinuxPackageArtifact()).toBeNull()
  })
  ...
  it('refuses an entry with a missing or non-string digest', () => {
    capture({ files: [{ url: 'FABRICA.deb' }] })
    expect(recovery.getTrackedLinuxPackageArtifact()).toBeNull()
    capture({ files: [{ url: 'FABRICA.deb', sha512: 42 }] })
    expect(recovery.getTrackedLinuxPackageArtifact()).toBeNull()
  })
  ...
  it('refuses a digest that cannot decode to 64 bytes', () => {
    ...
    capture({
      files: [{ url: 'FABRICA.deb', sha512: createHash('sha256').update(PAYLOAD).digest('base64') }]
    })
    expect(recovery.getTrackedLinuxPackageArtifact()).toBeNull()
  })
  ...
  it('refuses a digest with characters outside the base64 alphabet', () => {
    ...
    capture({ files: [{ url: 'FABRICA.deb', sha512: `${SHA512.slice(0, 4)}!${SHA512.slice(4)}` }] })
    expect(recovery.getTrackedLinuxPackageArtifact()).toBeNull()
  })

  it('refuses a digest that is not base64 at all', () => {
    capture({ files: [{ url: 'FABRICA.deb', sha512: 'not-a-digest' }] })
    expect(recovery.getTrackedLinuxPackageArtifact()).toBeNull()
  })
  ...
  it('refuses a malformed percent-encoded URL', () => {
    capture({ files: [{ url: 'FABRICA%ZZ.deb', sha512: SHA512 }] })
    expect(recovery.getTrackedLinuxPackageArtifact()).toBeNull()
  })
  ...
  it('refuses a missing or non-array file list', () => {
    ...
    expect(recovery.getTrackedLinuxPackageArtifact()).toBeNull()
    capture({ files: { url: 'FABRICA.deb', sha512: SHA512 } })
    expect(recovery.getTrackedLinuxPackageArtifact()).toBeNull()
  })
  ...
  it('replaces the retained artifact with a newer download', () => {
    ...
    capture({
      version: '1.2.4',
      downloadedFile: path.join(downloadDir, 'FABRICA-next.deb'),
      files: [{ url: 'FABRICA-next.deb', sha512: SHA512 }]
    })
    expect(recovery.getTrackedLinuxPackageArtifact()).toMatchObject({
      version: '1.2.4',
      path: path.join(downloadDir, 'FABRICA-next.deb')
    })
  })
  ...
  it('keeps the retained artifact when an unrelated download event arrives', () => {
    capture()
    capture({ downloadedFile: path.join(downloadDir, 'FABRICA.AppImage') })
    expect(recovery.getTrackedLinuxPackageArtifact()?.path).toBe(path.join(downloadDir, 'FABRICA.deb'))
  })
})
...
describePosix('resolveLinuxPackageInstallInstructions', () => {
  it('returns the built command and package file name for a verified package', async () => {
    const filePath = await writePackage('FABRICA.deb')
    capture()
  ...
  it('reports missing when the recovery version does not match', async () => {
    await writePackage('FABRICA.deb')
    capture()
    await expect(
...
describePosix('digest validation', () => {
  ...
  it('rejects a well-formed digest that belongs to another file', async () => {
    ...
    capture({
      files: [
        { url: 'FABRICA.deb', sha512: createHash('sha512').update('other payload').digest('base64') }
      ]
    })
    await expect(recovery.resolveLinuxPackageInstallInstructions(recoveryFor())).resolves.toEqual({
      ok: false,
...
describePosix('cache containment', () => {
  it('rejects a path that traverses out of the cache', async () => {
    const filePath = path.join(outsideDir, 'FABRICA.deb')
    await fsp.writeFile(filePath, PAYLOAD)
    capture({ downloadedFile: path.join(downloadDir, '..', '..', '..', 'outside', 'FABRICA.deb') })
    await expect(recovery.resolveLinuxPackageInstallInstructions(recoveryFor())).resolves.toEqual({
      ok: false,
      reason: 'not-regular'
    })
  ...
  it('rejects a directory in place of the package', async () => {
    await fsp.mkdir(path.join(downloadDir, 'FABRICA.deb'))
    capture()
    await expect(recovery.resolveLinuxPackageInstallInstructions(recoveryFor())).resolves.toEqual({
      ok: false,
      reason: 'not-regular'
  ...
  it('rejects a symlink to a real package inside the cache', async () => {
    ...
    const linkPath = path.join(downloadDir, 'FABRICA.deb')
  ...
  it('rejects a package whose parent symlink escapes the cache', async () => {
    await fsp.writeFile(path.join(outsideDir, 'FABRICA.deb'), PAYLOAD)
    await fsp.symlink(outsideDir, path.join(cacheRoot, 'escape'))
    capture({ downloadedFile: path.join(cacheRoot, 'escape', 'FABRICA.deb') })
    await expect(recovery.resolveLinuxPackageInstallInstructions(recoveryFor())).resolves.toEqual({
      ok: false,
      reason: 'not-regular'
    })
  ...
  it('rejects a subdirectory of the pending directory', async () => {
    ...
    const filePath = path.join(nested, 'FABRICA.deb')
    await fsp.writeFile(filePath, PAYLOAD)
    capture({ downloadedFile: filePath })
    await expect(recovery.resolveLinuxPackageInstallInstructions(recoveryFor())).resolves.toEqual({
      ok: false,
  ...
  it('uses the home cache when XDG_CACHE_HOME is unset', async () => {
    ...
    const homeDownloadDir = path.join(homeCache, '.cache', 'FABRICA-updater', 'pending')
    await fsp.mkdir(homeDownloadDir, { recursive: true })
    const filePath = path.join(homeDownloadDir, 'FABRICA.deb')
    await fsp.writeFile(filePath, PAYLOAD)
...
describePosix('pending-directory anchoring', () => {
  it('rejects a package in an attacker-owned directory under the cache root', async () => {
    ...
    const filePath = path.join(evilDir, 'FABRICA.deb')
    await fsp.writeFile(filePath, PAYLOAD)
    capture({ downloadedFile: filePath })
    await expect(recovery.resolveLinuxPackageInstallInstructions(recoveryFor())).resolves.toEqual({
      ok: false,
  ...
  it('rejects a package in the updater directory itself', async () => {
    const filePath = path.join(updaterDir, 'FABRICA.deb')
    await fsp.writeFile(filePath, PAYLOAD)
    capture({ downloadedFile: filePath })
    await expect(recovery.resolveLinuxPackageInstallInstructions(recoveryFor())).resolves.toEqual({
      ok: false,
  ...
  it('rejects a pending directory buried deeper than one level under the cache root', async () => {
    ...
    const filePath = path.join(deepPending, 'FABRICA.deb')
    await fsp.writeFile(filePath, PAYLOAD)
    capture({ downloadedFile: filePath })
    await expect(recovery.resolveLinuxPackageInstallInstructions(recoveryFor())).resolves.toEqual({
      ok: false,
  ...
  it('rejects a pending directory that is a symlink out of the cache', async () => {
    await fsp.writeFile(path.join(outsideDir, 'FABRICA.deb'), PAYLOAD)
    const fakeUpdaterDir = path.join(cacheRoot, 'FABRICA-updater-2')
    await fsp.mkdir(fakeUpdaterDir)
    await fsp.symlink(outsideDir, path.join(fakeUpdaterDir, 'pending'))
    capture({ downloadedFile: path.join(fakeUpdaterDir, 'pending', 'FABRICA.deb') })
    await expect(recovery.resolveLinuxPackageInstallInstructions(recoveryFor())).resolves.toEqual({
      ok: false,
      reason: 'not-regular'
    })
  ...
  it('accepts any updater directory name that holds the pending directory', async () => {
    ...
    const otherPending = path.join(cacheRoot, 'FABRICA-updater-next', 'pending')
    await fsp.mkdir(otherPending, { recursive: true })
    const filePath = path.join(otherPending, 'FABRICA.deb')
    await fsp.writeFile(filePath, PAYLOAD)
    capture({ downloadedFile: filePath })
    await expect(recovery.resolveLinuxPackageInstallInstructions(recoveryFor())).resolves.toEqual({
      ok: true,
  ...
  it('accepts a pending directory reached through a symlink that stays in the cache', async () => {
    ...
    capture({ downloadedFile: path.join(cacheRoot, 'link-to-updater', 'pending', 'FABRICA.deb') })
    await expect(recovery.resolveLinuxPackageInstallInstructions(recoveryFor())).resolves.toEqual({
      ok: true,
      command: 'installed command',
      packageFileName: 'FABRICA.deb'
...
describePosix('validation coalescing', () => {
  ...
  it('does not reuse an in-flight pass for a different artifact', async () => {
    await writePackage('FABRICA.deb')
    await writePackage('FABRICA-next.deb')
    capture()
...
describePosix('revalidateLinuxPackageForInstall', () => {
  ...
  it('rejects a package swapped after the download was verified', async () => {
    ...
    capture()
    const artifact = recovery.getTrackedLinuxPackageArtifact()
    await writePackage('FABRICA.deb', 'attacker supplied package')
    await expect(recovery.revalidateLinuxPackageForInstall(artifact!)).resolves.toEqual({
      ok: false,
      reason: 'hash-mismatch'
    })
