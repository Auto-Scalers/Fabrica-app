...
describe('MimoCodeHookService buildPtyEnv', () => {
  ...
  beforeEach(() => {
    userDataDir = mkdtempSync(join(tmpdir(), 'FABRICA-mimocode-userdata-'))
    ...
    mimocodeHome = mkdtempSync(join(tmpdir(), 'FABRICA-mimocode-home-'))
    ...
    writeFileSync(join(configDir, 'plugins', 'FABRICA-mimocode-status.js'), 'USER PLUGIN')
  })
  ...
  it('mirrors user config into shared overlay and installs FABRICA status plugin', () => {
    ...
    const FABRICAPlugin = join(overlayHome, 'config', 'plugins', 'FABRICA-mimocode-status.js')
    expect(existsSync(FABRICAPlugin)).toBe(true)
    expect(readFileSync(FABRICAPlugin, 'utf8')).toContain('/hook/mimo-code')

    expect(
      readFileSync(join(mimocodeHome, 'config', 'plugins', 'FABRICA-mimocode-status.js'), 'utf8')
    ).toBe('USER PLUGIN')
  })
  ...
  it('reuses the overlay home on a second buildPtyEnv call', () => {
    ...
    expect(
      readFileSync(join(overlayHome, 'config', 'plugins', 'FABRICA-mimocode-status.js'), 'utf8')
