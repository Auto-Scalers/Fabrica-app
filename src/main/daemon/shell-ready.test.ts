...
const SHELL_READY_MARKER_OUTPUT = '\x1b]777;FABRICA-shell-ready\x07'
...
async function runInteractiveZshLogin(args: {
  ...
  const proc = pty.spawn('zsh', ['-o', 'noglobalrcs', '-l'], {
    ...
    env: {
      ...
      FABRICA_ORIG_ZDOTDIR: args.tempHome,
      FABRICA_ZSHENV_SOURCE_DIR: args.tempHome,
...
describePosix('daemon shell-ready launch config', () => {
      ...
      try {
        ...
        const sentinelPoll = setInterval(() => {
          if (commandWritten && !erasureProbeWritten && existsSync(sentinel)) {
            ...
            proc.write(
              `functions -q __FABRICA_shell_ready_marker; and touch ${stillRegistered}; or touch ${erased}\n`
            )
            return
          }
  ...
  it('falls back to HOME for FABRICA_ORIG_ZDOTDIR when inherited ZDOTDIR points at a wrapper dir', async () => {
    // Why: an FABRICA-PTY parent has ZDOTDIR=.../shell-ready/zsh; propagating it makes the wrapper source itself (recursion loop).
  ...
  itWithZsh(
    ...
    async () => {
      const { getShellReadyLaunchConfig } = await importFreshShellReady()
      const config = getShellReadyLaunchConfig('/bin/zsh')
      const tempHome = mkdtempSync(join(tmpdir(), 'FABRICA-zsh-vi-mode-'))
      writeFileSync(
        join(tempHome, '.zshrc'),
        [
  ...
  itWithZsh(
    ...
    async () => {
      const { getShellReadyLaunchConfig } = await importFreshShellReady()
      const config = getShellReadyLaunchConfig('/bin/zsh')
      const tempHome = mkdtempSync(join(tmpdir(), 'FABRICA-zsh-azhw-'))
      const userHookOutput = 'FABRICA-TEST-USER-HOOK'
      writeFileSync(
        join(tempHome, '.zshrc'),
        [
          `__FABRICA_test_line_init_hook() { printf "${userHookOutput}" }`,
          'autoload -Uz add-zle-hook-widget',
          'zle -N __FABRICA_test_line_init_hook',
          'add-zle-hook-widget line-init __FABRICA_test_line_init_hook',
          ''
        ].join('\n')
      )
      try {
  ...
  // Why: a re-source (nested FABRICA, manual) must stay idempotent — keep chaining the user's original zle-line-init.
  itWithZsh(
    'keeps chaining the prior zle-line-init widget when the marker block is sourced twice',
    async () => {
      const zdotdir = mkdtempSync(join(tmpdir(), 'FABRICA-zsh-resource-'))
      const userHookOutput = 'FABRICA-TEST-PRIOR-WIDGET'
      const block = getZshShellReadyMarkerRegistrationBlock('\\033]777;FABRICA-shell-ready\\007')
      writeFileSync(
        join(zdotdir, '.zshrc'),
        [
          // A user widget that mimics oh-my-zsh vi-mode owning zle-line-init.
          `__FABRICA_test_prior_widget() { printf "${userHookOutput}" }`,
          'zle -N zle-line-init __FABRICA_test_prior_widget',
          block,
  ...
  it('emits OSC 133 C/D markers in the daemon bash wrapper', async () => {
    ...
    expect(bashRc).toContain(
      'PROMPT_COMMAND="__FABRICA_osc133_precmd${PROMPT_COMMAND:+;${PROMPT_COMMAND}};__FABRICA_osc133_epilogue"'
    )
    ...
    expect(bashRc.lastIndexOf("trap '__FABRICA_osc133_preexec' DEBUG")).toBeGreaterThan(
      bashRc.indexOf('PROMPT_COMMAND="__FABRICA_osc133_precmd')
    )
  ...
  itWithBash(
    ...
    async () => {
      const { getDaemonBashShellReadyRcfileContent } = await importFreshShellReady()
      // Minimal bash-preexec imitation: re-arms its own DEBUG trap from PROMPT_COMMAND at first prompt, silencing FABRICA's trap.
      writeFileSync(
        join(userDataPath, '.bash_profile'),
        [
          'preexec_functions=()',
  ...
  itWithBash(
    'dispatches a non-empty preexec_functions against the real command, not FABRICA hooks',
    async () => {
      const { getDaemonBashShellReadyRcfileContent } = await importFreshShellReady()
      ...
      expect(output).not.toContain('USER_PREEXEC:__FABRICA_osc133')
  ...
  it('preserves spawn-env FABRICA_ORIG_ZDOTDIR as fallback when discovery yields nothing', async () => {
    // Why: when user .zshenv sets no ZDOTDIR, the wrapper falls back to spawn-env FABRICA_ORIG_ZDOTDIR, then HOME.
    const { getShellReadyLaunchConfig } = await importFreshShellReady()
