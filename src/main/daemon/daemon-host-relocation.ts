 ...
 * run-as-node FABRICA.exe copy (not node.exe) so there's no console flash and asar still resolves. Fail-open:
...
const LOCAL_HOST_ROOT_NAME = 'Fabrica'

// Copy of FABRICA.exe renamed to a distinct image name so the NSIS updater's `taskkill /IM FABRICA.exe` can't match it.
const DAEMON_HOST_EXE_NAME = 'FABRICA-terminal-daemon.exe'

// V8 snapshots + ICU data the Electron bootstrap reads even under ELECTRON_RUN_AS_NODE; siblings of FABRICA.exe.
