...
import { addFABRICAWslInteropEnv } from '../pty/wsl-fabrica-env'
import {
...
import { FABRICA_HERMES_STARTUP_QUERY_ENV } from '../../shared/hermes-startup-query'
...
function deleteRequestedDaemonEnvKeys(
  ...
  // Compare ownership here so real-home routing neither leaks an FABRICA overlay
  // nor deletes a user-owned CODEX_HOME chosen by the daemon's host context.
  const deleteFABRICAOwnedCodexHome =
    keys?.includes('FABRICA_CODEX_HOME') === true &&
    env.FABRICA_CODEX_HOME !== undefined &&
    env.CODEX_HOME === env.FABRICA_CODEX_HOME
  for (const key of keys ?? []) {
    delete env[key]
  }
  if (deleteFABRICAOwnedCodexHome) {
...
function promoteAgentTeamsShimPath(
  ...
  if (!env.FABRICA_AGENT_TEAMS_TEAM_ID || !requestedPath) {
    return
  }
...
function removeInheritedDevAgentHookEndpoint(
  ...
  if (explicitEnv?.FABRICA_AGENT_HOOK_ENV === 'development' && !explicitEnv.FABRICA_AGENT_HOOK_ENDPOINT) {
    ...
    delete env.FABRICA_AGENT_HOOK_ENDPOINT
  }
}
...
function formatMissingDaemonPathError(kind: 'helper' | 'cwd', path: string): DaemonProtocolError {
  ...
  return new DaemonProtocolError(
    `Daemon's ${kind === 'helper' ? 'node-pty install' : 'working directory'} is gone ` +
      `(worktree deleted?). Restart Fabrica. node-pty: ${step} failed: ENOENT ` +
...
function repairDaemonCwd(): string | null {
  const candidates = [process.env.FABRICA_USER_DATA_PATH]
  try {
...
function runSinglePtySpawnHealthProbe(): Promise<void> {
  const cwd = isExistingDirectory(process.env.FABRICA_USER_DATA_PATH)
    ? process.env.FABRICA_USER_DATA_PATH
    : getDefaultCwd()
...
export function createPtySubprocess(opts: PtySubprocessOptions): SubprocessHandle {
  ...
  const env: Record<string, string> = {
    ...
    // Why: TUIs feature-gate on TERM_PROGRAM_VERSION; FABRICA_APP_VERSION is inherited from the forking main process.
    TERM_PROGRAM_VERSION: process.env.FABRICA_APP_VERSION ?? '0.0.0-dev',
    // Why: `supports-hyperlinks` gates OSC 8 on a TERM_PROGRAM allowlist excluding FABRICA; force it since xterm.js parses OSC 8 for clickable links.
    FORCE_HYPERLINK: '1'
  ...
  if (process.platform === 'win32') {
    ...
    if (pathWin32.basename(shellPath).toLowerCase() === 'wsl.exe') {
      if (codexHomeWslInfo) {
        ...
        if (launchWslDistro && launchWslDistro !== codexHomeWslInfo.distro) {
          delete env.CODEX_HOME
          delete env.FABRICA_CODEX_HOME
        } else {
          env.CODEX_HOME = codexHomeWslInfo.linuxPath
          env.FABRICA_CODEX_HOME = codexHomeWslInfo.linuxPath
          // Why: wsl.exe only imports non-default env vars named in WSLENV.
          addWslEnvKeys(env, ['CODEX_HOME', 'FABRICA_CODEX_HOME'])
          if (!launchWslDistro) {
            const resolved = resolveWindowsShellLaunchArgs(
              shellPath,
              requestedCwd,
      ...
      if (env[FABRICA_HERMES_STARTUP_QUERY_ENV] !== undefined) {
        // Why: wsl.exe drops custom Windows env vars unless named in WSLENV.
        addWslEnvKeys(env, [FABRICA_HERMES_STARTUP_QUERY_ENV])
      }
    } else if (codexHomeWslInfo || isWslCodexHomeForHost(env.CODEX_HOME)) {
      // Why: WSL Codex homes are Linux paths; also drop FABRICA_CODEX_HOME since shell-ready restores CODEX_HOME from it.
      delete env.CODEX_HOME
      delete env.FABRICA_CODEX_HOME
    }
    if (pathWin32.basename(shellPath).toLowerCase() === 'wsl.exe') {
    ...
    if (opts.command && isCodexStartupCommand) {
      ...
      shellLaunch =
        env.FABRICA_ATTRIBUTION_SHIM_DIR ||
        env.FABRICA_OPENCODE_CONFIG_DIR ||
        env.FABRICA_MIMOCODE_HOME ||
        env.FABRICA_OMP_STATUS_EXTENSION ||
        env.FABRICA_CODEX_HOME ||
        env.FABRICA_AGENT_TEAMS_SHIM_DIR
          ? getAttributionShellLaunchConfig(shellPath)
          : null
    }
    if (shellLaunch) {
