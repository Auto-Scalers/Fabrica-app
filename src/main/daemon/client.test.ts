...
describe('DaemonClient', () => {
  ...
  describe('connect', () => {
    ...
    it('captures one matching endpoint identity from both authenticated sockets', async () => {
      const identity = {
        ...
        entryPath: '/Applications/FABRICA.app/Contents/Resources/daemon-entry.js',
        appVersion: '1.2.3',
        spawnerExecPath: '/Applications/FABRICA.app/Contents/MacOS/FABRICA'
      }
  ...
  describe('RPC', () => {
    ...
    it('adds recovery hints to node-pty daemon diagnostics', async () => {
      ...
      await expect(client.request('listSessions', undefined)).rejects.toThrow(
        "Daemon's node-pty install is gone (worktree deleted?). Restart Fabrica. node-pty: posix_spawn failed: ENOENT"
      )
    })
  })
  ...
  describe('events', () => {
    ...
    it('preserves UTF-8 stream events split inside multibyte characters', async () => {
      ...
      const tableRow = 'â”‚OpenCodeâ”‚ðŸ§©â”‚â”¼â”€â”€â”€â”€â”€â”€â”€â”€â”¤'
      const event: DaemonEvent = {
        type: 'event',
        event: 'data',
        sessionId: 'session-1',
      ...
      const [first, second] = splitInsideUtf8Sequence(encodeNdjson(event), 'ðŸ§©')
