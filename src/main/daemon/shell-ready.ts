...
const FABRICA_USER_DATA_PATH_ENV = 'FABRICA_USER_DATA_PATH'
const SHELL_READY_MARKER = '\\033]777;FABRICA-shell-ready\\007'

let didEnsureShellReadyWrappers = false

function getShellReadyWrapperRoot(): string {
  const userDataPath = process.env[FABRICA_USER_DATA_PATH_ENV]
  // Why: older/test launchers may not seed FABRICA_USER_DATA_PATH. Keep a
  // fallback so daemon startup does not fail before the parent can be fixed.
  return join(userDataPath || tmpdir(), userDataPath ? 'shell-ready' : 'FABRICA-shell-ready')
}

// Why: if our own process inherited ZDOTDIR from a parent shell that was
// itself an FABRICA PTY (e.g. the user launched FABRICA from a terminal inside a
// running FABRICA), that ZDOTDIR points at an FABRICA shell-ready wrapper dir.
// Propagating it as the new PTY's FABRICA_ORIG_ZDOTDIR makes the wrapper's
// `source "$FABRICA_ORIG_ZDOTDIR/.zshenv"` line source itself recursively —
// zsh gives "job table full or recursion limit exceeded" and the shell
// never reaches a usable prompt.
//
// Any path component ending in `/shell-ready/zsh` is an FABRICA wrapper dir
// (regardless of whether it came from this daemon's userData, a packaged
// FABRICA, or a different dev build). Treat it as if ZDOTDIR were unset so the
...
export function getDaemonBashShellReadyRcfileContent(): string {
  return `# FABRICA daemon bash shell-ready wrapper
...
__FABRICA_osc133_precmd() {
  ...
  # displaced by one of FABRICA's own hooks.
  [[ "\${FABRICA_SHELL_READY_MARKER:-0}" == "1" ]] && printf "${SHELL_READY_MARKER}"
}
...
__FABRICA_osc133_preexec() {
  ...
  # first prompt; __FABRICA_osc133_epilogue re-takes it each prompt and stores the
  # framework's trap here, so the framework's own preexec still runs while our
  # command-start C survives its re-arm.
  if [[ -n "\${__FABRICA_chained_debug_trap:-}" ]]; then
    eval "$__FABRICA_chained_debug_trap" || true
  fi
  [[ -z "\${__FABRICA_in_prompt_command:-}" ]] || return
  # Why: a chained trap can invoke us more than once for a single command, so
  # emit C only on the first fire (the __FABRICA_in_command gate), and never for a
  # prompt-time hook — ours or bash-preexec's __bp_* helpers.
  [[ -z "\${__FABRICA_in_command:-}" ]] || return
  case "$BASH_COMMAND" in
    *__FABRICA_osc133_*|*__bp_*) return ;;
  esac
...
# while its re-arm can no longer silence FABRICA's command-start signal.
__FABRICA_osc133_epilogue() {
  unset __FABRICA_in_prompt_command
  local __FABRICA_spec="$(trap -p DEBUG)"
  case "$__FABRICA_spec" in
    "" | *__FABRICA_osc133_preexec* ) __FABRICA_chained_debug_trap="" ;;
    * )
      __FABRICA_spec="\${__FABRICA_spec#trap -- }"
      __FABRICA_spec="\${__FABRICA_spec% DEBUG}"
      eval "__FABRICA_chained_debug_trap=$__FABRICA_spec"
      ;;
  esac
...
unset -f __FABRICA_normalize_prompt_command
...
export function getDaemonZshShellReadyRcfileContent(): string {
  return `# FABRICA daemon zsh shell-ready wrapper
${getZshStartupFileSourceBlock({
  fileName: '.zshrc',
...
function ensureShellReadyWrappers(): void {
  ...
  const zshProfile = `# FABRICA daemon zsh shell-ready wrapper
${getZshStartupFileSourceBlock({ fileName: '.zprofile' })}
`
  const zshRc = getDaemonZshShellReadyRcfileContent()
  const zshLogin = `# FABRICA daemon zsh shell-ready wrapper
...
export function resolvePtyShellPath(env: Record<string, string>): string {
  if (process.platform === 'win32') {
    return env.FABRICA_TERMINAL_WINDOWS_SHELL || 'powershell.exe'
  }
