...
const FABRICA_SHELL_WRAPPER_ENV = [
  'FABRICA_ATTRIBUTION_SHIM_DIR',
  'FABRICA_OPENCODE_CONFIG_DIR',
  'FABRICA_MIMOCODE_HOME',
  'FABRICA_PI_CODING_AGENT_DIR',
  'FABRICA_OMP_CODING_AGENT_DIR',
  'FABRICA_CODEX_HOME'
] as const
...
describe('createPtySubprocess', () => {
  const savedWrapperEnv: Partial<Record<(typeof FABRICA_SHELL_WRAPPER_ENV)[number], string>> = {}
  ...
  beforeEach(() => {
    ...
    previousUserDataPath = process.env.FABRICA_USER_DATA_PATH
    ...
    process.env.FABRICA_USER_DATA_PATH = userDataPath
    delete process.env[POWERLEVEL10K_WIZARD_DISABLE_ENV]
    for (const key of FABRICA_SHELL_WRAPPER_ENV) {
  ...
  afterEach(() => {
    if (previousUserDataPath === undefined) {
      ...
      process.env.FABRICA_USER_DATA_PATH = previousUserDataPath
    }
  ...
  it('expands variables in PATH before spawning a Windows shell', () => {
    ...
    try {
      createPtySubprocess({
        ...
        env: {
          ...
          FABRICA_PATH_ROOT: 'C:\\Users\\FABRICA\\AppData\\Local',
          PATH: '%FABRICA_path_root%\\agy\\bin;C:\\Windows'
        }
      })
    } finally {
      if (platform) {
    ...
    expect(spawnMock.mock.calls.at(-1)?.[2].env.PATH).toBe(
      'C:\\Users\\FABRICA\\AppData\\Local\\agy\\bin;C:\\Windows'
    )
  })
  ...
  it('derives shell-ready launch config from the resolved fallback shell', () => {
    ...
    const platform = Object.getOwnPropertyDescriptor(process, 'platform')
    const previousShell = process.env.SHELL
    const previousMarker = process.env.FABRICA_SHELL_READY_MARKER
    const previousZdotdir = process.env.ZDOTDIR
    const warn = vi.spyOn(console, 'warn').mockImplementation(() => {})
    Object.defineProperty(process, 'platform', { configurable: true, value: 'linux' })
    delete process.env.SHELL
    // Why: the test runner itself can execute inside an FABRICA-wrapped shell
    // whose exported wrapper vars would leak through the process.env spread.
    delete process.env.FABRICA_SHELL_READY_MARKER
    ...
    try {
      ...
      // ZDOTDIR and FABRICA_SHELL_READY_MARKER; /bin/sh must spawn without them.
      expect(spawnOptions.env.ZDOTDIR).toBeUndefined()
      expect(spawnOptions.env.FABRICA_SHELL_READY_MARKER).toBeUndefined()
      ...
      if (previousMarker === undefined) {
        ...
        process.env.FABRICA_SHELL_READY_MARKER = previousMarker
      }
  ...
  itOnMacHost('repairs a deleted macOS daemon cwd before spawning node-pty', () => {
    ...
    const deletedDaemonCwd = mkdtempSync(join(tmpdir(), 'FABRICA-deleted-daemon-cwd-'))
  ...
  it('keeps menu startup agent foreground during a slow Windows shell enrichment window', async () => {
    ...
    try {
      const handle = createPtySubprocess({
        sessionId: 'repo::C:\\repo\\FABRICA@@deadbeef',
        cols: 80,
        rows: 24,
        cwd: 'C:\\repo\\FABRICA',
        command: 'codex'
      ...
      expect(resolveAgentForegroundProcessMock).toHaveBeenCalledWith(
        ...
        'powershell.exe',
        expect.objectContaining({
          contextPaths: expect.arrayContaining(['C:\\repo\\FABRICA'])
        })
      )
  ...
  it('does not inherit parent FABRICA pane identity when caller omits pane env', () => {
    const proc = mockPtyProcess()
    spawnMock.mockReturnValue(proc)
    const saved = {
      FABRICA_PANE_KEY: process.env.FABRICA_PANE_KEY,
      FABRICA_TAB_ID: process.env.FABRICA_TAB_ID,
      FABRICA_WORKTREE_ID: process.env.FABRICA_WORKTREE_ID
    }
    process.env.FABRICA_PANE_KEY = 'parent-tab:parent-leaf'
    process.env.FABRICA_TAB_ID = 'parent-tab'
    process.env.FABRICA_WORKTREE_ID = 'parent-worktree'
  ...
  it('preserves explicit child FABRICA pane identity over parent env', () => {
    const proc = mockPtyProcess()
    spawnMock.mockReturnValue(proc)
    const saved = {
      FABRICA_PANE_KEY: process.env.FABRICA_PANE_KEY,
    ...
    try {
      createPtySubprocess({
        ...
        rows: 24,
        env: {
          FABRICA_PANE_KEY: 'child-tab:child-leaf',
          FABRICA_TAB_ID: 'child-tab',
          FABRICA_WORKTREE_ID: 'child-worktree'
        }
      })
    } finally {
      for (const [key, value] of Object.entries(saved)) {
    ...
    const env = spawnMock.mock.calls.at(-1)?.[2].env
    expect(env.FABRICA_PANE_KEY).toBe('child-tab:child-leaf')
    expect(env.FABRICA_TAB_ID).toBe('child-tab')
    expect(env.FABRICA_WORKTREE_ID).toBe('child-worktree')
  })
  ...
  it('does not inherit NODE_ENV from the daemon process env', () => {
    // Why: a dev-mode FABRICA forks the daemon with NODE_ENV=development; leaking
    // FABRICA's build mode into user shells breaks `next build` and Vitest.
    const proc = mockPtyProcess()
    spawnMock.mockReturnValue(proc)
  ...
  it('does not inherit AppImage runtime env into daemon PTY shells', () => {
    ...
    Object.defineProperty(process, 'platform', { value: 'linux' })
    process.env.APPIMAGE = '/data/apps/FABRICA.appimage'
    process.env.APPDIR = '/tmp/.mount_FABRICA123'
    process.env.ARGV0 = '/data/apps/FABRICA.appimage'
    process.env.OWD = '/home/user/project'
    process.env.APPIMAGE_LIBRARY_PATH = '/tmp/.mount_FABRICA123/usr/lib'
    process.env.PATH = ['/tmp/.mount_FABRICA123', '/tmp/.mount_FABRICA123/usr/sbin', '/usr/bin'].join(
      delimiter
    )
    process.env.LD_LIBRARY_PATH = ['/tmp/.mount_FABRICA123/usr/lib', '/opt/audio/lib'].join(delimiter)
  ...
  it('does not inherit parent agent hook endpoint for development hook env', () => {
    const proc = mockPtyProcess()
    spawnMock.mockReturnValue(proc)
    const previousEndpoint = process.env.FABRICA_AGENT_HOOK_ENDPOINT
    process.env.FABRICA_AGENT_HOOK_ENDPOINT = '/tmp/stale-endpoint.env'
    ...
    try {
      createPtySubprocess({
        ...
        rows: 24,
        env: {
          FABRICA_AGENT_HOOK_ENV: 'development',
          FABRICA_AGENT_HOOK_PORT: '1234',
      ...
      if (previousEndpoint === undefined) {
        ...
        process.env.FABRICA_AGENT_HOOK_ENDPOINT = previousEndpoint
      }
    }

    const env = spawnMock.mock.calls.at(-1)?.[2].env
    expect(env.FABRICA_AGENT_HOOK_ENDPOINT).toBeUndefined()
    expect(env.FABRICA_AGENT_HOOK_ENV).toBe('development')
    expect(env.FABRICA_AGENT_HOOK_PORT).toBe('1234')
    expect(env.FABRICA_AGENT_HOOK_TOKEN).toBe('token')
  })
  ...
  it('preserves explicit development agent hook endpoint files', () => {
    ...
    try {
      createPtySubprocess({
        ...
        env: {
          ...
          FABRICA_AGENT_HOOK_TOKEN: 'token',
          FABRICA_AGENT_HOOK_VERSION: '1',
          FABRICA_AGENT_HOOK_ENDPOINT: '/tmp/fresh-endpoint.env'
        }
      })
    } finally {
      if (previousEndpoint === undefined) {
    ...
    const env = spawnMock.mock.calls.at(-1)?.[2].env
    expect(env.FABRICA_AGENT_HOOK_ENDPOINT).toBe('/tmp/fresh-endpoint.env')
    expect(env.FABRICA_AGENT_HOOK_ENV).toBe('development')
    expect(env.FABRICA_AGENT_HOOK_PORT).toBe('1234')
    expect(env.FABRICA_AGENT_HOOK_TOKEN).toBe('token')
  })
    ...
    try {
      expect(() =>
        createPtySubprocess({
          ...
          cols: 80,
          rows: 24,
          cwd: '/definitely-missing-FABRICA-cwd'
        })
      ).toThrow(/definitely-missing-FABRICA-cwd/)
    } finally {
      if (platform) {
        Object.defineProperty(process, 'platform', platform)
      }
  ...
  it('uses shell wrapper when attribution shims must survive shell startup', () => {
    ...
    try {
      createPtySubprocess({
        ...
        env: {
          SHELL: '/bin/zsh',
          FABRICA_ATTRIBUTION_SHIM_DIR: '/tmp/FABRICA-terminal-attribution/posix'
        }
      })
    } finally {
      if (platform) {
    ...
    expect(lastCall[2].env.FABRICA_SHELL_READY_MARKER).toBe('0')
  })
  ...
  it('uses shell wrapper when OpenCode config must survive shell startup', () => {
    ...
    try {
      createPtySubprocess({
        ...
        env: {
          SHELL: '/bin/zsh',
          OPENCODE_CONFIG_DIR: '/tmp/FABRICA-opencode-overlay',
          FABRICA_OPENCODE_CONFIG_DIR: '/tmp/FABRICA-opencode-overlay'
        }
      })
    } finally {
      if (platform) {
  ...
  it('uses shell wrapper when MiMo home must survive shell startup', () => {
    ...
    try {
      createPtySubprocess({
        ...
        env: {
          SHELL: '/bin/zsh',
          MIMOCODE_HOME: '/tmp/FABRICA-mimocode-overlay',
          FABRICA_MIMOCODE_HOME: '/tmp/FABRICA-mimocode-overlay'
        }
      })
    } finally {
      if (platform) {
  ...
  it('uses shell wrapper when typed OMP commands need the status extension', () => {
    ...
    try {
      createPtySubprocess({
        ...
        env: {
          SHELL: '/bin/zsh',
          FABRICA_OMP_STATUS_EXTENSION: '/tmp/.omp/agent/extensions/FABRICA-agent-status.ts'
        }
      })
    } finally {
      if (platform) {
  ...
  it('uses shell wrapper when Codex home must survive shell startup', () => {
    ...
    try {
      createPtySubprocess({
        ...
        env: {
          SHELL: '/bin/zsh',
          CODEX_HOME: '/tmp/FABRICA-codex-home',
          FABRICA_CODEX_HOME: '/tmp/FABRICA-codex-home'
        }
      })
    } finally {
      if (platform) {
  ...
  it('uses shell wrapper when Agent Teams shim path must survive shell startup', () => {
    ...
    try {
      createPtySubprocess({
        ...
        env: {
          SHELL: '/bin/zsh',
          PATH: '/tmp/FABRICA-agent-teams-bin:/usr/bin',
          FABRICA_AGENT_TEAMS_TEAM_ID: 'team-test',
          FABRICA_AGENT_TEAMS_SHIM_DIR: '/tmp/FABRICA-agent-teams-bin'
        }
      })
    } finally {
      if (platform) {
  ...
  it('uses shell-ready wrapper for delivery-hinted Codex startup commands', () => {
    ...
    expect(lastCall[1]).toEqual(['-l'])
    expect(lastCall[2].env.ZDOTDIR).toMatch(ZSH_SHELL_READY_DIR)
    expect(lastCall[2].env.FABRICA_SHELL_READY_MARKER).toBe('1')
  })
  ...
  it('deletes daemon-owned Codex overlay pairs when the private marker is requested', () => {
    ...
    spawnMock.mockReturnValue(proc)
    const previousCodexHome = process.env.CODEX_HOME
    const previousFABRICACodexHome = process.env.FABRICA_CODEX_HOME
    process.env.CODEX_HOME = '/daemon/managed/codex-home'
    process.env.FABRICA_CODEX_HOME = '/daemon/managed/codex-home'
    ...
    try {
      createPtySubprocess({
        ...
        rows: 24,
        env: { SHELL: '/bin/bash' },
        envToDelete: ['FABRICA_CODEX_HOME']
      })
    } finally {
      if (previousCodexHome === undefined) {
        delete process.env.CODEX_HOME
      ...
      if (previousFABRICACodexHome === undefined) {
        delete process.env.FABRICA_CODEX_HOME
      } else {
        process.env.FABRICA_CODEX_HOME = previousFABRICACodexHome
      }
    }
  ...
  it('strips an inherited per-account self-contained CODEX_HOME overlay in a nested FABRICA (#5370)', () => {
    const proc = mockPtyProcess()
    spawnMock.mockReturnValue(proc)
    const previousCodexHome = process.env.CODEX_HOME
    const previousFABRICACodexHome = process.env.FABRICA_CODEX_HOME
    // A per-account home is injected as CODEX_HOME === FABRICA_CODEX_HOME, so the
    // nested-FABRICA strip must clear it exactly as it does the shared mirror.
    const perAccountHome = '/daemon/managed/codex-accounts/019f0000-aaaa/home'
    process.env.CODEX_HOME = perAccountHome
    process.env.FABRICA_CODEX_HOME = perAccountHome
  ...
  it('preserves a daemon-owned custom Codex home while deleting a stale private marker', () => {
    ...
    process.env.FABRICA_CODEX_HOME = '/daemon/stale/managed-home'
  ...
  it('honors explicit terminal env overrides after deleting requested defaults', () => {
    ...
    expect(lastCall[2].env.PATH.split(':')[0]).toBe('/tmp/FABRICA-agent-teams-bin')
    expect(lastCall[2].env.TERM_PROGRAM).toBeUndefined()
    expect(lastCall[2].env.FABRICA_ATTRIBUTION_SHIM_DIR).toBeUndefined()
  })
  ...
  it('collapses its own env merge onto the requested Windows `Path` spelling', () => {
    ...
    expect(env.Path.split(':')[0]).toBe('/tmp/FABRICA-agent-teams-bin')
  })
  ...
  it('combines HOMEDRIVE and HOMEPATH for Windows default cwd', () => {
    ...
    process.env.HOMEPATH = '\\Users\\FABRICA'
    ...
    expect(spawnMock).toHaveBeenCalledWith(
      expect.any(String),
      expect.any(Array),
      expect.objectContaining({ cwd: 'D:\\Users\\FABRICA' })
    )
  })
    ...
    try {
      expect(() =>
        createPtySubprocess({
          ...
          cols: 80,
          rows: 24,
          cwd: 'C:\\definitely-missing-FABRICA-cwd',
          shellOverride: 'powershell.exe'
        })
      ).toThrow(/Working directory "C:\\definitely-missing-FABRICA-cwd" does not exist/)
    } finally {
      if (platform) {
        Object.defineProperty(process, 'platform', platform)
      }
    ...
    try {
      expect(() =>
        createPtySubprocess({
          ...
          cols: 80,
          rows: 24,
          cwd: 'C:\\definitely-missing-FABRICA-wsl-cwd',
          shellOverride: 'wsl.exe'
        })
      ).toThrow(/Working directory "C:\\definitely-missing-FABRICA-wsl-cwd" does not exist/)
    } finally {
      if (platform) {
        Object.defineProperty(process, 'platform', platform)
      }
  ...
  it('does not pass a Windows Codex home into daemon WSL terminals', () => {
    ...
    try {
      createPtySubprocess({
        ...
        rows: 24,
        cwd: '\\\\wsl.localhost\\Ubuntu\\home\\jin\\repo',
        env: { CODEX_HOME: 'C:\\Users\\jin\\.codex', FABRICA_CODEX_HOME: 'C:\\Users\\jin\\.codex' }
      })
    } finally {
      if (platform) {
        Object.defineProperty(process, 'platform', platform)
    ...
    expect(spawnMock).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        env: expect.not.objectContaining({
          ...
          FABRICA_CODEX_HOME: expect.anything()
        })
      })
    )
  })
  ...
  it('does not pass a WSL managed Codex home into daemon Windows terminals', () => {
    ...
    try {
      createPtySubprocess({
        ...
        env: {
          CODEX_HOME:
            '\\\\wsl.localhost\\Ubuntu\\home\\jin\\.local\\share\\FABRICA\\codex-accounts\\a\\home',
          FABRICA_CODEX_HOME:
            '\\\\wsl.localhost\\Ubuntu\\home\\jin\\.local\\share\\FABRICA\\codex-accounts\\a\\home'
        }
      })
    ...
    expect(spawnMock).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        env: expect.objectContaining({
          CODEX_HOME: '/home/jin/.local/share/FABRICA/codex-accounts/a/home',
          FABRICA_CODEX_HOME: '/home/jin/.local/share/FABRICA/codex-accounts/a/home',
  ...
  it('marks FABRICA terminal handles for WSL env import in daemon WSL terminals', () => {
    const proc = mockPtyProcess()
    spawnMock.mockReturnValue(proc)
    const platform = Object.getOwnPropertyDescriptor(process, 'platform')
    ...
    try {
      createPtySubprocess({
        ...
        env: {
          ...
          FABRICA_HERMES_STARTUP_QUERY: 'line one\nline two',
          WSLENV: 'FOO/u'
        }
      })
    } finally {
    ...
    expect(spawnCall[2].env.FABRICA_TERMINAL_HANDLE).toBe('term_wsl')
