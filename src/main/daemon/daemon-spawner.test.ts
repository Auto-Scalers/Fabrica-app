  ...
  describe('ensureRunning', () => {
    ...
    it('uses protocol-scoped socket and token paths', () => {
      ...
      if (process.platform === 'win32') {
        expect(socketPath).toContain(`FABRICA-terminal-host-v${PROTOCOL_VERSION}`)
      } else {
    ...
    it('starts daemon and returns connection info', async () => {
      ...
      if (process.platform === 'win32') {
        expect(info.socketPath).toContain(`FABRICA-terminal-host-v${PROTOCOL_VERSION}`)
      } else {
...
describe('daemon socket publication', () => {
  it('keeps every scratch namespace outside the released sweeper pattern', () => {
    ...
    for (let i = 0; i < 20; i++) {
      const names = [
        basename(getDaemonSocketBindPath(getDaemonSocketPath('/tmp/FABRICA-daemon'))),
        basename(getDaemonPidSwapClaimPath('/tmp/FABRICA-daemon/daemon-v32.pid')),
        basename(getDaemonArtifactHoldClaimPath('/tmp/FABRICA-daemon/daemon-v32.token'))
      ]
  ...
  it('keeps the bind name shorter than the canonical endpoint', () => {
    ...
    const canonicalPath = getDaemonSocketPath('/tmp/FABRICA-daemon-runtime')
