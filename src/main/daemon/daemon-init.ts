...
// Why: pass a log-file arg so field failures are diagnosable, but honor the FABRICA_DIAGNOSTICS_DISABLED privacy switch.
function daemonLogArgs(): string[] {
  const disabled = (process.env.FABRICA_DIAGNOSTICS_DISABLED ?? '').trim().toLowerCase()
 ...
 * Files\...`, `/Applications/FABRICA 2.app/...`) cannot be split back into argv unambiguously.
 */
    ...
    try {
      ...
      const child = fork(
        ...
        ],
        {
          ...
          // Why: run the byte-identical relocated FABRICA.exe so the image path sits outside the updater's kill zone.
...
export async function initDaemonPtyProvider(
  ...
  const e2eInitDelayMs = Number(process.env.FABRICA_E2E_DAEMON_INIT_DELAY_MS)
...
// Leave history sessions marked "unclean" so a daemon crash while FABRICA is closed stays recoverable.
