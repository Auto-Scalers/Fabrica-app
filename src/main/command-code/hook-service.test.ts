...
describe('CommandCodeHookService', () => {
  ...
  beforeEach(() => {
    homeDir = mkdtempSync(join(tmpdir(), 'FABRICA-command-code-home-'))
  ...
  it.skipIf(process.platform !== 'win32')(
    ...
    () => {
      const spaceHome = join(tmpdir(), 'FABRICA command-code home with spaces')
  ...
  it('installs a hook script that can recover the endpoint when Command Code strips token env', () => {
    ...
    const script = readFileSync(join(homeDir, '.FABRICA', 'agent-hooks', scriptFileName), 'utf8')

    if (process.platform === 'win32') {
      expect(script).toContain('sourceEndpointByPort')
      expect(script).toContain('fabrica-dev\\agent-hooks')
      expect(script).toContain('set FABRICA_AGENT_HOOK_PORT=')
    } else {
      ...
      expect(script).toContain('__FABRICA_read_ancestor_var')
      expect(script).toContain('__FABRICA_fill_from_endpoint_file')
      expect(script).toContain('[ "$__FABRICA_endpoint_port" != "$FABRICA_AGENT_HOOK_PORT" ]')
      expect(script).toContain('FABRICA_PANE_KEY')
      expect(script).toContain('FABRICA_AGENT_LAUNCH_TOKEN')
      expect(script).toContain('fabrica-dev/agent-hooks')
  ...
  itPosix('does not let stale endpoint files clobber recovered connection fields', async () => {
    ...
    const server = createServer((req, res) => {
      ...
      req.on('end', () => {
        requests.push({ body, token: req.headers['x-FABRICA-agent-hook-token'] })
        res.statusCode = 204
        res.end()
      })
    })
    ...
    try {
      ...
      const scriptPath = join(homeDir, '.FABRICA', 'agent-hooks', 'command-code-hook.sh')
      const child = spawn('/bin/sh', [scriptPath], {
        env: {
          ...process.env,
          HOME: homeDir,
          FABRICA_AGENT_HOOK_ENDPOINT: staleEndpointPath,
          FABRICA_AGENT_HOOK_PORT: String(address.port),
          FABRICA_AGENT_HOOK_TOKEN: 'current-token',
          FABRICA_PANE_KEY: 'tab:leaf',
          FABRICA_TAB_ID: 'tab',
          FABRICA_WORKTREE_ID: 'worktree',
          FABRICA_AGENT_HOOK_ENV: 'development',
          FABRICA_AGENT_HOOK_VERSION: '1'
        },
