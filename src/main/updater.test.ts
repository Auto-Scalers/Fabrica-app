...
function stageLinuxUpdateCache(): StagedLinuxPackages {
  const cacheRoot = mkdtempSync(join(tmpdir(), 'FABRICA-updater-cache-'))
  const pendingDir = join(cacheRoot, 'FABRICA-updater', 'pending')
  mkdirSync(pendingDir, { recursive: true })
  const stagePackage = (fileName: string): { path: string; sha512: string } => {
    const packagePath = join(pendingDir, fileName)
    const bytes = Buffer.from(`FABRICA test package ${fileName}`)
    writeFileSync(packagePath, bytes)
    return { path: packagePath, sha512: createHash('sha512').update(bytes).digest('base64') }
  }
  const deb = stagePackage('fabrica_1.0.61_amd64.deb')
  const rpm = stagePackage('fabrica-1.0.61.x86_64.rpm')
  return {
    cacheRoot,
    debPath: deb.path,
...
const PRE_COMMIT_INSTALL_FAILURE =
  process.platform === 'darwin'
    ? 'Could not restart to install the update. Quit and reopen Fabrica, then try again.'
    : 'Could not start the update installer. Fabrica remains open.'
...
vi.mock('./updater-prerelease-feed', () => ({
  ...
  getReleaseDownloadUrl: (tag: string) =>
    `https://github.com/Auto-Scalers/Fabrica-app/releases/download/${tag}`
}))
...
describe('updater', () => {
  ...
  it.runIf(process.platform === 'darwin')(
    ...
    async () => {
      ...
      expect(autoUpdaterMock.setFeedURL).toHaveBeenLastCalledWith({
        provider: 'generic',
        url: 'https://github.com/Auto-Scalers/Fabrica-app/releases/latest/download'
      })
    }
  )
  ...
  it('runs a fresh prerelease check when Shift-click promotes an in-flight stable check', async () => {
    ...
    expect(autoUpdaterMock.setFeedURL).toHaveBeenLastCalledWith({
      provider: 'generic',
      url: 'https://github.com/Auto-Scalers/Fabrica-app/releases/download/v1.4.36-rc.5'
    })
    expect(
      sendMock.mock.calls
        .filter(([channel]) => channel === 'updater:status')
  ...
  it('opts into the RC channel when checkForUpdatesFromMenu is called with includePrerelease', async () => {
    ...
    await vi.waitFor(() => {
      ...
      expect(autoUpdaterMock.setFeedURL).toHaveBeenLastCalledWith({
        provider: 'generic',
        url: 'https://github.com/Auto-Scalers/Fabrica-app/releases/download/v1.3.18-rc.1'
      })
      expect(autoUpdaterMock.checkForUpdates).toHaveBeenCalledTimes(1)
    })
  ...
  it('pins the generic feed to a perf-tagged prerelease when requested', async () => {
    ...
    await vi.waitFor(() => {
      ...
      expect(autoUpdaterMock.setFeedURL).toHaveBeenLastCalledWith({
        provider: 'generic',
        url: 'https://github.com/Auto-Scalers/Fabrica-app/releases/download/v1.4.121-rc.6.perf'
      })
      expect(autoUpdaterMock.checkForUpdates).toHaveBeenCalledTimes(1)
    })
    expect(autoUpdaterMock.allowPrerelease).toBe(true)
  ...
  it('keeps background retries on the stable channel after a perf publishing-window miss', async () => {
    ...
    await vi.waitFor(() => {
      ...
      expect(autoUpdaterMock.setFeedURL).toHaveBeenLastCalledWith({
        provider: 'generic',
        url: 'https://github.com/Auto-Scalers/Fabrica-app/releases/download/v1.4.121'
      })
    })
  })
  ...
  it('repins the generic feed to the newest RC tag for a prerelease user', async () => {
    ...
    await vi.waitFor(() => {
      ...
      expect(autoUpdaterMock.setFeedURL).toHaveBeenLastCalledWith({
        provider: 'generic',
        url: 'https://github.com/Auto-Scalers/Fabrica-app/releases/download/v1.3.17-rc.2'
      })
      expect(autoUpdaterMock.checkForUpdates).toHaveBeenCalledTimes(1)
    })
  })
  ...
  it('repins the generic feed to a newer stable tag for a prerelease user', async () => {
    ...
    await vi.waitFor(() => {
      expect(autoUpdaterMock.setFeedURL).toHaveBeenLastCalledWith({
        provider: 'generic',
        url: 'https://github.com/Auto-Scalers/Fabrica-app/releases/download/v1.3.19'
      })
    })
    expect(autoUpdaterMock.allowPrerelease).not.toBe(true)
  })
  ...
  it('uses last-good concrete feed when a user-initiated check lands during publishing', async () => {
    ...
    expect(autoUpdaterMock.setFeedURL).toHaveBeenLastCalledWith({
      provider: 'generic',
      url: 'https://github.com/Auto-Scalers/Fabrica-app/releases/download/v1.4.26'
    })
    expect(autoUpdaterMock.setFeedURL).not.toHaveBeenCalledWith({
      provider: 'generic',
      url: 'https://github.com/Auto-Scalers/Fabrica-app/releases/download/v1.4.27'
    })
    expect(autoUpdaterMock.setFeedURL.mock.calls.slice(feedCallsBeforeCheck)).not.toContainEqual([
      {
        provider: 'generic',
  ...
  it('retries a prerelease check once against the previous feed tag when the manifest is missing', async () => {
    ...
    await vi.waitFor(() => {
      ...
      expect(autoUpdaterMock.setFeedURL).toHaveBeenCalledWith({
        provider: 'generic',
        url: 'https://github.com/Auto-Scalers/Fabrica-app/releases/download/v1.3.51-rc.7'
      })
      expect(autoUpdaterMock.setFeedURL).toHaveBeenLastCalledWith({
        provider: 'generic',
        url: 'https://github.com/Auto-Scalers/Fabrica-app/releases/download/v1.3.51-rc.6'
      })
    })
  ...
  it('pins the generic feed to a concrete stable tag for a stable user', async () => {
    ...
    expect(autoUpdaterMock.setFeedURL).toHaveBeenLastCalledWith({
      provider: 'generic',
      url: 'https://github.com/Auto-Scalers/Fabrica-app/releases/download/v1.3.18'
    })
  })
  ...
  describe('linux root package install recovery', () => {
    ...
    const downloadedEvent = (overrides?: Record<string, unknown>): Record<string, unknown> => ({
      ...
      files: [{ url: 'fabrica_1.0.61_amd64.deb', sha512: staged.debSha512 }],
      ...overrides
    })
    ...
    const rpmDownloadedEvent = (): Record<string, unknown> =>
      downloadedEvent({
        ...
        files: [{ url: 'fabrica-1.0.61.x86_64.rpm', sha512: staged.rpmSha512 }]
      })
    ...
    it('keeps the generic install-failure copy when no artifact was retained', async () => {
      ...
      await reachDownloaded(
        updater,
        downloadedEvent({ files: [{ url: 'fabrica_1.0.61_amd64.deb' }] })
      )
      autoUpdaterMock.quitAndInstall.mockImplementation(() => {
    ...
    it('advises a restart only for a failure before the native invoke', async () => {
      ...
      expect(send).toHaveBeenCalledWith('updater:status', {
        state: 'error',
        message: 'Could not restart to install the update. Quit and reopen Fabrica, then try again.'
      })
      expect(updater.isQuittingForUpdate()).toBe(false)
    })
    ...
    it('aborts the retry when the retained package no longer matches its digest', async () => {
      ...
      expect(send).toHaveBeenCalledWith('updater:status', {
        state: 'error',
        message:
          'The downloaded package no longer matches the verified release, so Fabrica will not hand it to a package manager. Download the update again, or get it from the official release page.'
      })
      expect(recordUpdaterLifecycleMock).toHaveBeenCalledWith(
    ...
    it('keeps the recovery card usable when the re-proof cannot read the package', async () => {
      ...
      expect(lastStatus(send)).toEqual({
        state: 'error',
        message:
          'Fabrica could not read the downloaded package. Download the update again, or get it from the official release page.',
        recovery: {
          kind: 'linux-package-install',
          packageType: 'deb',
          reason: 'package-install-failed',
