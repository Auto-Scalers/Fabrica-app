 ...
 * FABRICA descendant also reads `own`. It bounds the blast radius of a bad
 ...
 * usually belongs to an unrelated process whose chain never passes through FABRICA,
 ...
 * `own`. That is not remote during teardown, when FABRICA is itself the process
...
export function classifyWindowsTreeKillTarget(
  ...
  // `taskkill /T /F` on ourselves would take FABRICA and every pane down with it.
