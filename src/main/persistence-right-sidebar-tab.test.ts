...
vi.mock('electron', () => ({
  app: {
    getPath: () => '/tmp/FABRICA-persistence-right-sidebar-tab-test'
  },
  safeStorage: {
...
describe('normalizeRightSidebarTab', () => {
  ...
  it('preserves well-formed plugin panel tabs', () => {
    expect(normalizeRightSidebarTab('plugin:fabrica-samples.my-plugin/dashboard')).toBe(
      'plugin:fabrica-samples.my-plugin/dashboard'
    )
  })

  it('normalizes malformed plugin tabs to the default tab', () => {
    expect(normalizeRightSidebarTab('plugin:fabrica-samples.my-plugin')).toBe('explorer')
    expect(normalizeRightSidebarTab('plugin:fabrica-samples.my-plugin/panel/extra')).toBe('explorer')
