...
describe('createMainWindow', () => {
  ...
  it('enables renderer sandboxing and opens external links safely', () => {
    ...
    const allowBlankPrefs = { partition: 'persist:FABRICA-browser' }
    ...
    expect(allowBlankPrefs).toMatchObject({
      ...
      partition: 'persist:FABRICA-browser',
    ...
    windowHandlers['will-attach-webview'](
      ...
      { partition: 'persist:FABRICA-browser' } as never,
  ...
  it('notifies before FABRICA-first captures a risky terminal-focused shortcut', () => {
    const windowHandlers: Record<string, (...args: any[]) => void> = {}
    const webContents = {
      on: vi.fn((event, handler) => {
        windowHandlers[event] = handler
    ...
    createMainWindow({
      getUI: () => ({}),
      getSettings: () => ({ terminalShortcutPolicy: 'FABRICA-first' })
    } as never)
  ...
  it('notifies before FABRICA-first captures a terminal-focused double-tap shortcut', () => {
    const windowHandlers: Record<string, (...args: any[]) => void> = {}
    const webContents = {
      on: vi.fn((event, handler) => {
        windowHandlers[event] = handler
  ...
  it('keeps the headless E2E window hidden when the Windows fallback fires', () => {
    vi.useFakeTimers()
    const previousHeadless = process.env.FABRICA_E2E_HEADLESS
    process.env.FABRICA_E2E_HEADLESS = '1'
    const { browserWindowInstance } = createStartupRevealWindowFixture()
    ...
    try {
      ...
      if (previousHeadless === undefined) {
        delete process.env.FABRICA_E2E_HEADLESS
      } else {
        process.env.FABRICA_E2E_HEADLESS = previousHeadless
      }
    }
  })
