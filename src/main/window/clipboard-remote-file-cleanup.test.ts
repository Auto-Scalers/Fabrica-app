...
function* FABRICAStagingDirs(count: number): Generator<{ name: string; isDirectory: () => boolean }> {
  for (let index = 0; index < count; index += 1) {
    yield { name: `FABRICA-clipboard-file-expired-${index}`, isDirectory: () => true }
  }
}
...
describe('cleanupExpiredRemoteClipboardFiles', () => {
  ...
  it('streams all entries with at most eight cleanups in flight', async () => {
    mockTempRoot(FABRICAStagingDirs(257))
    let active = 0
    let peak = 0
  ...
  it('acts on owned directories while the temp root is still being enumerated', async () => {
    ...
    function* enumeration(): Generator<{ name: string; isDirectory: () => boolean }> {
      yield { name: 'FABRICA-clipboard-file-expired', isDirectory: () => true }
  ...
  it('removes only expired staging directories it owns', async () => {
    function* interleaved(): Generator<{ name: string; isDirectory: () => boolean }> {
      ...
      yield { name: 'FABRICA-clipboard-file-expired', isDirectory: () => true }
      yield* unrelatedTempEntries(50_000)
      yield { name: 'FABRICA-clipboard-file-fresh', isDirectory: () => true }
      yield { name: 'FABRICA-clipboard-file-plain-file', isDirectory: () => false }
    }
    ...
    expect(rmMock).toHaveBeenCalledTimes(1)
    expect(rmMock).toHaveBeenCalledWith(join('/tmp', 'FABRICA-clipboard-file-expired'), {
      recursive: true,
      force: true
    })
  })
