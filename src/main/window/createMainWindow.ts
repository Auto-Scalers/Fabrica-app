...
import { FABRICA_BROWSER_GUEST_WEB_PREFERENCES } from '../../shared/browser-guest-web-preferences'
...
type CreateMainWindowOptions = {
  ...
  /** Returns true when FABRICA should reload after renderer loss; update-relaunch/quit tear down children intentionally, so don't fight shutdown. */
...
export function createMainWindow(
  ...
  const mainWindow = new BrowserWindow({
    ...
    show: false,
    // Why: macOS swallows the app-activating click by default, so clicking back into FABRICA needed a second click (Windows/Linux already deliver it).
  ...
  mainWindow.webContents.on('will-attach-webview', (event, webPreferences, params) => {
    ...
    Object.assign(webPreferences, FABRICA_BROWSER_GUEST_WEB_PREFERENCES)
  ...
  const dispatchResolvedWindowShortcutAction = (
    ...
    const capturedTerminalActionId =
      ...
      focusedShortcutContext.terminalShortcutPolicy === 'FABRICA-first' &&
  ...
  mainWindow.webContents.on('before-input-event', (event, input) => {
    ...
    if (isMacAppPasteInput(input)) {
      // Why: chat/terminal panes hold focus without native editable controls, so route Cmd+V through FABRICA's paste ownership.
      event.preventDefault()
      ...
      try {
        new Notification({
          ...
          body: translateMain(
            ...
            'Fabrica is still running in the system tray'
          )
        }).show()
      } catch {
