...
import { RuntimeEmulatorCommands } from '../runtime/fabrica-runtime-emulator'
...
describe('EmulatorBridge helper ownership', () => {
  ...
  it('stops the previous FABRICA-managed helper when a worktree switches devices', async () => {
  ...
  it('shuts down the previous FABRICA-managed device when requested', async () => {
    const bridge = new EmulatorBridge()
    bridge.registerActiveEmulator('wt-1', session('device-old'), { managed: true })
  ...
  it('only kills FABRICA-managed helpers during app shutdown cleanup', async () => {
    const bridge = new EmulatorBridge()
