...
/** True when any record is real user work rather than a shared symlink FABRICA put
 *  in the worktree.
 *
 *  Fails closed on purpose: anything not positively identified as an FABRICA-owned
...
function reviewCopy(provider: HostedReviewProvider): {
  ...
  if (provider === 'azure-devops') {
    return {
      ...
      authInstruction: 'Set FABRICA_AZURE_DEVOPS_TOKEN'
    }
  }
  if (provider === 'gitea') {
    return {
      ...
      reviewLabel: 'pull request',
      providerName: 'Gitea',
      authInstruction: 'Set FABRICA_GITEA_TOKEN'
    }
  }
  return {
    shortLabel: 'PR',
  ...
  try {
    ...
    if (eligibility.reviewLookupOutcome === 'unavailable') {
      return {
        ok: false,
        code: 'validation',
        error: `Create ${copy.shortLabel} failed: Fabrica could not confirm whether this branch already has a ${copy.reviewLabel}. Retry once the ${copy.providerName} lookup succeeds.`
      }
    }
...
export async function getHostedReviewCreationEligibility(
  ...
  try {
    review = await getHostedReviewForBranch({
      ...
      // on, so it earns the fast tier. Without it a review opened outside FABRICA
...
export async function createHostedReview(
  ...
  if (result.ok) {
    ...
    // than a poll interval, so FABRICA's own creation must retire it at once.
