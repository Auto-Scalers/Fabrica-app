...
// cannot commit — it is a symlink FABRICA created.
describe('createHostedReview with shared symlinks', () => {
  ...
  beforeEach(() => {
    worktree = mkdtempSync(join(tmpdir(), 'FABRICA-hosted-shared-'))
  ...
  // Why: only an *untracked* record can be FABRICA's artifact. A tracked change at a
