...
function getFABRICADir(): string {
  return join(homedir(), '.fabrica')
}

function getLegacyTokenPath(): string {
  return join(getFABRICADir(), 'linear-token.enc')
}

function getLegacyViewerPath(): string {
  return join(getFABRICADir(), 'linear-viewer.json')
}

function getWorkspaceFilePath(): string {
  return join(getFABRICADir(), 'linear-workspaces.json')
}

function getWorkspaceTokenDir(): string {
  return join(getFABRICADir(), 'linear-tokens')
}
...
function ensureFABRICADir(): void {
  const dir = getFABRICADir()
  if (!existsSync(dir)) {
