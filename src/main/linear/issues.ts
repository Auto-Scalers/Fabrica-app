...
const SEARCH_ISSUES_QUERY = `
  query FABRICALinearIssueSearch($term: String!, $first: Int) {
...
const VIEWER_ASSIGNED_ISSUES_QUERY = `
  query FABRICALinearViewerAssignedIssues(
    $first: Int,
    $after: String,
    $filter: IssueFilter,
    $orderBy: PaginationOrderBy
...
const VIEWER_CREATED_ISSUES_QUERY = `
  query FABRICALinearViewerCreatedIssues(
    $first: Int,
    $after: String,
    $filter: IssueFilter,
    $orderBy: PaginationOrderBy
...
const ISSUE_BY_UUID_QUERY = `
  query FABRICALinearIssueByUuid($id: String!) {
    issue(id: $id) {
...
const COMMENT_BY_UUID_QUERY = `
  query FABRICALinearCommentByUuid($id: String!) {
    comment(id: $id) {
      id
      url
      body
...
const ATTACHMENT_BY_UUID_QUERY = `
  query FABRICALinearAttachmentByUuid($id: String!) {
    attachment(id: $id) {
      id
      title
      url
...
const ISSUE_COMMENTS_QUERY = `
  query FABRICALinearIssueComments($id: String!) {
    issue(id: $id) {
      comments(first: 50) {
        nodes {
          id
...
async function readIssueConnectionPages(
  ...
  while (items.length < limit) {
    // Why: Linear caps connection pages at 50, so larger FABRICA reads must walk
