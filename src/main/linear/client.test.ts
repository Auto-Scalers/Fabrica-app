...
function writeLegacyLinearToken(token: string | Buffer, viewer: Record<string, unknown>): void {
  const FABRICADir = join(tempHome, '.FABRICA')
  mkdirSync(FABRICADir, { recursive: true })
  writeFileSync(join(FABRICADir, 'linear-token.enc'), token)
  writeFileSync(join(FABRICADir, 'linear-viewer.json'), JSON.stringify(viewer), {
    encoding: 'utf-8'
  })
}
...
function writeMultiWorkspaceFiles(
  ...
  const FABRICADir = join(tempHome, '.FABRICA')
  mkdirSync(join(FABRICADir, 'linear-tokens'), { recursive: true })
  writeFileSync(
    join(FABRICADir, 'linear-workspaces.json'),
    JSON.stringify({
      version: 1,
...
beforeEach(() => {
  tempHome = mkdtempLike('fabrica-linear-client-')
  fixtures = new Map([
    [
      'token-alpha',
      {
...
describe('Linear client workspace storage', () => {
  ...
  it('migrates legacy token storage to a real workspace id when explicitly tested', async () => {
    ...
    expect(existsSync(join(tempHome, '.FABRICA', 'linear-token.enc'))).toBe(false)
    expect(readFileSync(join(tempHome, '.FABRICA', 'linear-workspaces.json'), 'utf-8')).toContain(
      'org-alpha'
    )
  })
  ...
  it('does not pass encrypted safeStorage bytes to the Linear SDK when encryption is unavailable', async () => {
    const tokenPath = join(tempHome, '.FABRICA', 'linear-token.enc')
