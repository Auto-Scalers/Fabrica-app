...
const WSL_COMMAND_NAME = 'fabrica'
const LEGACY_WSL_COMMAND_NAME = 'FABRICA'
...
export class WslCliInstaller {
  ...
  async getStatus(): Promise<CliInstallStatus> {
    ...
    if (content === null) {
      return this.buildStatus({
        ...
        detail: `Register ${ready.commandPath} to use Fabrica from WSL.`
      })
    }
    ...
    if (content === 'not_file') {
      return this.buildStatus({
        ...
        currentTarget: null,
        pathConfigured: ready.pathConfigured,
        detail: `${ready.commandPath} exists but is not a Fabrica launcher script.`
      })
    }
      ...
      return this.buildStatus({
        ...
        detail:
          bridgeContent === null || bridgeManaged
            ...
            : `${ready.bridgePath} exists but is not managed by Fabrica.`
      })
    }
    ...
    return this.buildStatus({
      ...
      pathConfigured: ready.pathConfigured,
      detail: !managed
        ? `${ready.commandPath} exists but is not managed by Fabrica.`
        : bridgeConflict
          ? `${ready.bridgePath} exists but is not managed by Fabrica.`
          : `${ready.commandPath} points to a different Fabrica launcher.`
    })
  }
  ...
  async repairManagedRegistration(): Promise<ManagedWslCliRepairResult> {
    ...
    if (status.state === 'conflict') {
      ...
      // still FABRICA-managed and must remain registered for future reconciliation.
  ...
  async install(precomputedStatus?: CliInstallStatus): Promise<CliInstallStatus> {
    ...
    await this.run(
      ...
      [
        ...
        'committed=1',
        'rm -f "$bridge_backup"',
        // Why: the command was renamed to avoid GNOME FABRICA; remove only the
        // old FABRICA-managed WSL wrapper after the replacement has committed.
  ...
  async remove(): Promise<CliInstallStatus> {
    ...
    if (status.state === 'not_installed') {
      // Why: a managed legacy `FABRICA` left behind would later be re-adopted by
  ...
  > {
    ...
    if (!hostStatus.launcherPath) {
      return {
        status: this.unsupported(
          ...
          hostStatus.detail ?? 'The Windows Fabrica CLI launcher is missing.'
        )
      }
    }
    ...
    if (!interopReady) {
      return {
        status: this.unsupported(
          'launcher_missing',
          'WSL Windows interop is unavailable; Fabrica cannot launch the Windows CLI from WSL.'
        )
      }
    }

    const pathDirectory = `${home}/.local/bin`
    // Why: WSL and Linux share the same `fabrica` command name (avoids GNOME FABRICA conflict).
  ...
  private async readCommandFile(
    ...
    const output = await this.run(
      ...
      [
        ...
        '  printf __FABRICA_NOT_FILE__',
        `elif [ ! -e ${quoteShell(commandPath)} ]; then`,
        '  printf __FABRICA_MISSING__',
    ...
    if (output === '__FABRICA_MISSING__') {
      return null
    }
    if (output === '__FABRICA_NOT_FILE__') {
      return 'not_file'
    }
    return output
  }
