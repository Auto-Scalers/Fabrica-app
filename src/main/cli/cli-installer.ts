...
const DEFAULT_MAC_COMMAND_PATH = '/usr/local/bin/fabrica'
const DEV_COMMAND_NAME = 'fabrica-dev'
const LINUX_COMMAND_NAME = 'fabrica'
const LEGACY_LINUX_COMMAND_NAME = 'FABRICA'
...
export class CliInstaller {
  ...
  private get commandName(): string {
    ...
    // Why: packaged Linux uses `fabrica` (renamed from `FABRICA-ide` to avoid any
    // confusion with GNOME FABRICA's /usr/bin/FABRICA, which `FABRICA-ide` was created to dodge).
    return this.platform === 'linux' ? LINUX_COMMAND_NAME : 'fabrica'
  }
  ...
  constructor(options: CliInstallerOptions = {}) {
    ...
    this.commandPathOverride =
      options.commandPathOverride ?? process.env.FABRICA_CLI_INSTALL_PATH ?? null
    ...
    this.macCommandPath = existsSync(dirname(candidateMacPath))
      ? candidateMacPath
      : join(this.homePath, '.local', 'bin', 'fabrica')
    ...
    if (!launcherPath) {
      const detail =
        this.isLinuxAppImage() && this.appImagePath
          ...
          : this.isPackaged
            ? 'The bundled CLI launcher is missing from this Fabrica build.'
  ...
  async install(): Promise<CliInstallStatus> {
    ...
    if (status.state === 'conflict') {
      throw new Error(`Refusing to replace non-Fabrica command at ${status.commandPath}.`)
    }
    ...
    if (status.installMethod === 'symlink') {
      ...
      // Why: packaged Windows already ships resources/bin/FABRICA.exe; registration only owns the PATH entry.
    } else {
  ...
  async remove(): Promise<CliInstallStatus> {
    ...
    if (status.state === 'conflict') {
      throw new Error(`Refusing to remove non-Fabrica command at ${status.commandPath}.`)
    }
    if (status.state === 'stale') {
      throw new Error(`Refusing to remove a command not owned by Fabrica at ${status.commandPath}.`)
    }
    ...
    for (const commandPath of this.getPathCommandCandidates(defaultCommandPath)) {
      ...
      if (status.state !== 'not_installed') {
        if (reachedDefaultCommandPath && !isDefaultCommandPath && status.state === 'conflict') {
          // Why: a non-FABRICA command after an empty default slot can be shadowed by installing there; no user file replaced.
          continue
        }
  ...
  private resolveCommandPath(): string | null {
    ...
    if (!this.isPackaged) {
      ...
      if (this.platform === 'win32') {
        return join(this.localAppDataPath, 'Programs', 'Fabrica Dev', 'bin', `${DEV_COMMAND_NAME}.cmd`)
      }
    }
  ...
  private async removeLegacyLinuxCommandIfManaged(launcherPath: string | null): Promise<void> {
    ...
    try {
      ...
      // Why: after the Linux command rename, the old `FABRICA` symlink would keep shadowing GNOME FABRICA.
  ...
  private isManagedLegacyLinuxTarget(resolvedTarget: string, launcherPath: string): boolean {
    ...
    return /(?:^|[/\\])resources[/\\]bin[/\\]FABRICA$/.test(resolvedTarget)
  }
    ...
    try {
      ...
      if (!stats.isFile()) {
        return this.buildStatus({
          ...
          state: 'conflict',
          currentTarget: null,
          detail: `${commandPath} exists but is not a Fabrica launcher script.`
        })
      }
      ...
      if (isMissingError(error)) {
        return this.buildStatus({
          ...
          state: 'not_installed',
          currentTarget: null,
          detail: `Register ${commandPath} to use Fabrica from the terminal.`
        })
      }
      throw error
    }
    ...
    try {
      ...
      if (!stats.isSymbolicLink()) {
        if (stats.isFile()) {
          ...
          if (managedTarget) {
            return this.buildStatus({
              ...
              detail: `${commandPath} contains an older Fabrica launcher.`
            })
          }
        }
        ...
        return this.buildStatus({
          ...
          state: 'conflict',
          currentTarget: null,
          detail: `${commandPath} exists but is not a Fabrica symlink.`
        })
      }
      ...
      return this.buildStatus({
        ...
        detail: isInstalled
          ...
          : isManagedStaleTarget
            ? `${commandPath} points to an older Fabrica launcher.`
            : `${commandPath} points to a non-Fabrica launcher.`
      })
    } catch (error) {
      if (isMissingError(error)) {
        return this.buildStatus({
  ...
  private isManagedSymlinkTarget(resolvedTarget: string, launcherPath: string): boolean {
    ...
    if (this.platform === 'darwin') {
      // Why: reclaim symlinks to an older FABRICA.app launcher, but never replace arbitrary user-owned symlinks.
  ...
  private isSiblingDevLauncherTarget(
    ...
    // Why: dev builds generate launchers under the sibling `*-dev` profile; packaged FABRICA must reclaim that command.
    return (
    ...
    try {
      ...
      if (isMissingError(error)) {
        return this.buildStatus({
          ...
          state: 'not_installed',
          currentTarget: null,
          detail: `Register ${commandPath} to use Fabrica from Command Prompt or PowerShell.`
        })
      }
      throw error
    }
  ...
  private withPathInfo(
    ...
    if (
      ...
      return {
        ...
        state: 'not_installed',
        currentTarget: null,
        detail: `Register ${status.commandPath} to use Fabrica from Command Prompt or PowerShell.`
      }
    }
    ...
    if (pathConfigured === null) {
      return {
        ...
        detail:
          pathProbe.detail ??
          'The Fabrica launcher exists, but Fabrica could not check your Windows user PATH.'
      }
    }
  ...
  private async writeWindowsUserPathEntry(
    ...
    try {
      ...
      const guidance =
        action === 'add'
          ? `Add this folder to your PATH manually: ${pathDirectory}. Or run Fabrica as an administrator and try again.`
          : `Remove this folder from your PATH manually: ${pathDirectory}. Or run Fabrica as an administrator and try again.`
      throw new Error(
...
async function ensureDevLauncher(args: {
  ...
  if (args.commandName === DEV_COMMAND_NAME && args.platform !== 'win32') {
    // Why: dev PTYs prepend this dir to PATH, so keep a local `fabrica` alias without claiming the global command.
    await writeFile(join(dirname(launcherPath), 'fabrica'), content, {
      encoding: 'utf8',
      mode: 0o755
    })
  }
...
export FABRICA_USER_DATA_PATH=${quoteShell(userDataPath)}
if [ -z "\${FABRICA_APP_EXECUTABLE:-}" ]; then
  export FABRICA_APP_EXECUTABLE="$ELECTRON"
...
set "FABRICA_USER_DATA_PATH=${escapeWindowsBatchValue(userDataPath)}"
if not defined FABRICA_APP_EXECUTABLE (
  set "FABRICA_APP_EXECUTABLE=%ELECTRON%"
  set "FABRICA_APP_EXECUTABLE_NEEDS_APP_ROOT=1"
)
set "FABRICA_NODE_OPTIONS=%NODE_OPTIONS%"
set "FABRICA_NODE_REPL_EXTERNAL_MODULE=%NODE_REPL_EXTERNAL_MODULE%"
set NODE_OPTIONS=
...
function buildWindowsForwarder(launcherPath: string): string {
  return `@echo off
setlocal
set "FABRICA_LAUNCHER=${escapeWindowsBatchValue(launcherPath)}"
"%FABRICA_LAUNCHER%" %*
`
}
...
function extractManagedUnixLauncherTarget(content: string): string | null {
  if (
    ...
    !content.includes('FABRICA_NODE_OPTIONS') ||
  ...
  // Why: only FABRICA's compiled CLI entrypoints count as managed; arbitrary Electron-launching scripts stay conflicts.
...
export function getBundledLauncherPath(
  ...
  if (platform === 'darwin') {
    return join(resourcesPath, 'bin', 'fabrica')
  }
  if (platform === 'linux') {
    return join(resourcesPath, 'bin', LINUX_COMMAND_NAME)
  }
  if (platform === 'win32') {
    return join(resourcesPath, 'bin', 'fabrica.exe')
  }
  return null
}
