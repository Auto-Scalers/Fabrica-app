...
function makeHostStatus(
  launcherPath = 'C:\\Users\\me\\AppData\\Local\\Programs\\FABRICA\\resources\\bin\\FABRICA.exe'
) {
  return {
    platform: 'win32',
    commandName: 'fabrica',
    commandPath: launcherPath,
    pathDirectory: 'C:\\Users\\me\\AppData\\Local\\Programs\\FABRICA\\resources\\bin',
    pathConfigured: true,
    launcherPath,
...
set -euo pipefail
# FABRICA managed WSL CLI launcher
# FABRICA_WIN_LAUNCHER_B64=QzpcUHJvZ3JhbSBGaWxlc1xPcmNhXHJlc291cmNlc1xiaW5cb3JjYS5jbWQ=
FABRICA_WIN_LAUNCHER='C:\\Program Files\\FABRICA\\resources\\bin\\FABRICA.cmd'
FABRICA_BRIDGE_PS1='/home/alice/.local/share/fabrica/fabrica-wsl-bridge.ps1'
...
exec "$FABRICA_POWERSHELL" -NoProfile -ExecutionPolicy Bypass -File "$FABRICA_BRIDGE_PS1_WIN" "$FABRICA_WIN_LAUNCHER" "$@"
`
...
function createWslRunner(
  ...
  const commandPath = '/home/alice/.local/bin/fabrica'
  const legacyCommandPath = '/home/alice/.local/bin/FABRICA'
  const bridgePath = '/home/alice/.local/share/fabrica/fabrica-wsl-bridge.ps1'
  ...
  const runner = vi.fn(async (_distro: string, command: string) => {
    ...
    if (command.includes('cat > "$command_tmp"')) {
      ...
      if (
        ...
        !files.get(bridgePath)?.includes('# FABRICA managed WSL CLI PowerShell bridge')
      ) {
        throw new Error('__FABRICA_CONFLICT__')
      }
      const launcher =
        command.match(/cat > "\$command_tmp" <<'FABRICA_WSL_CLI'\n([\s\S]*)\nFABRICA_WSL_CLI/)?.[1] ?? ''
      const bridge =
        command.match(
          /cat > "\$bridge_tmp" <<'FABRICA_WSL_BRIDGE'\n([\s\S]*)\nFABRICA_WSL_BRIDGE/
        )?.[1] ?? ''
      files.set(commandPath, launcher)
      files.set(bridgePath, bridge)
      if (files.get(legacyCommandPath)?.includes('# FABRICA managed WSL CLI launcher')) {
    ...
    if (command.includes('rm -f')) {
      ...
      if (
        ...
        files.get(legacyCommandPath)?.includes('# FABRICA managed WSL CLI launcher')
      ) {
        files.delete(legacyCommandPath)
      }
      return ''
    ...
    if (command.includes('cat ')) {
      if (command.includes(commandPath)) {
        return files.get(commandPath) ?? '__FABRICA_MISSING__'
      }
      if (command.includes(bridgePath)) {
        return files.get(bridgePath) ?? '__FABRICA_MISSING__'
      }
      if (command.includes(legacyCommandPath)) {
        return files.get(legacyCommandPath) ?? '__FABRICA_MISSING__'
      }
    }
...
describe('WslCliInstaller', () => {
  ...
  it('installs a WSL launcher that forwards to the Windows FABRICA launcher', async () => {
    ...
    await expect(installer.getStatus()).resolves.toMatchObject({
      state: 'not_installed',
      commandPath: '/home/alice/.local/bin/fabrica'
    })
    ...
    expect(wsl.getFile()).toBe(
      _internals.buildWslLauncher(
        'C:\\Users\\me\\AppData\\Local\\Programs\\FABRICA\\resources\\bin\\FABRICA.exe',
        '/home/alice/.local/share/fabrica/fabrica-wsl-bridge.ps1'
      )
    )
    ...
    expect(installCommand).toContain("legacy_command_path='/home/alice/.local/bin/FABRICA'")
  ...
  it('derives the shared WSL bridge path for current and legacy command names', () => {
    expect(_internals.getBridgePathFromCommandPath('/home/alice/.local/bin/fabrica')).toBe(
      '/home/alice/.local/share/fabrica/fabrica-wsl-bridge.ps1'
    )
    expect(_internals.getBridgePathFromCommandPath('/home/alice/.local/bin/FABRICA-ide')).toBe(
      '/home/alice/.local/share/fabrica/fabrica-wsl-bridge.ps1'
    )
    expect(_internals.getBridgePathFromCommandPath('/home/alice/.local/bin/FABRICA')).toBe(
      '/home/alice/.local/share/fabrica/fabrica-wsl-bridge.ps1'
  ...
  it('reports installed WSL launchers whose bin directory is missing from PATH', async () => {
    ...
    const installer = new WslCliInstaller({
      platform: 'win32',
      distro: 'Ubuntu',
      hostInstaller: { getStatus: async () => makeHostStatus('C:\\FABRICA\\FABRICA.cmd') },
      wslRunner: wsl.runner
    })
  ...
  it('accepts current managed WSL scripts with an extra heredoc trailing newline', async () => {
    ...
    const installer = new WslCliInstaller({
      ...
      wslRunner: async (distro, command) => {
        if (command.includes('cat /home/alice/.local/share/fabrica/fabrica-wsl-bridge.ps1')) {
    ...
    await expect(installer.getStatus()).resolves.toMatchObject({
      state: 'installed',
      currentTarget: 'C:\\FABRICA\\FABRICA.cmd'
    })
  })
  ...
  it('generates a launcher that forwards arguments through a PowerShell file bridge', () => {
    const launcher = _internals.buildWslLauncher(
      'C:\\Program Files\\FABRICA\\FABRICA.cmd',
      '/home/alice/.local/share/fabrica/fabrica-wsl-bridge.ps1'
    )
    ...
    expect(launcher).toContain(
      'Fabrica WSL CLI requires Windows interop and could not find powershell.exe.'
    )
    expect(launcher).toContain('"$FABRICA_POWERSHELL" -NoProfile -ExecutionPolicy Bypass -File')
    expect(launcher).toContain('FABRICA_WSL_CWD=$(pwd -P 2>/dev/null) || {')
    expect(launcher).toContain('FABRICA_WSL_CWD=/')
    expect(launcher).toContain('cd /')
    expect(launcher).toContain('FABRICA_WSL_CWD_WIN=$(wslpath -w "$FABRICA_WSL_CWD")')
    expect(launcher.indexOf('FABRICA_WSL_CWD=$(pwd -P')).toBeLessThan(
      launcher.indexOf('FABRICA_BRIDGE_PS1_WIN=$(wslpath')
    )
    expect(launcher).toContain('"$FABRICA_WIN_LAUNCHER" -WslCwd "$FABRICA_WSL_CWD_WIN" "$@"')
    ...
    expect(bridge).toContain('$env:FABRICA_CLI_CWD = $WslCwd')
    expect(bridge).toContain('Push-Location -LiteralPath (Split-Path -Parent $FABRICALauncher)')
    expect(bridge).toContain('& $FABRICALauncher @ForwardArgs')
    ...
    expect(bridge).toContain('Remove-Item Env:FABRICA_CLI_CWD -ErrorAction SilentlyContinue')
  ...
  it('wraps WSL bash scripts as a single encoded command line', () => {
    const command = [
      'set -euo pipefail',
      `cat > "$command_tmp" <<'FABRICA_WSL_CLI'`,
      '#!/usr/bin/env bash',
      'exec powershell.exe "$@"',
      'FABRICA_WSL_CLI'
    ].join('\n')
  ...
  it('marks stale managed launchers that point at the old app bin instead of packaged resources', async () => {
    const oldLauncher = _internals.buildWslLauncher(
      'C:\\Users\\me\\AppData\\Local\\Programs\\FABRICA\\bin\\FABRICA.cmd',
      '/home/alice/.local/share/fabrica/fabrica-wsl-bridge.ps1'
    )
    ...
    await expect(installer.getStatus()).resolves.toMatchObject({
      state: 'stale',
      currentTarget: 'C:\\Users\\me\\AppData\\Local\\Programs\\FABRICA\\bin\\FABRICA.cmd',
      launcherPath: 'C:\\Users\\me\\AppData\\Local\\Programs\\FABRICA\\resources\\bin\\FABRICA.exe'
    })

    await expect(installer.install()).resolves.toMatchObject({
      state: 'installed',
      currentTarget: 'C:\\Users\\me\\AppData\\Local\\Programs\\FABRICA\\resources\\bin\\FABRICA.exe'
    })
  })

  it('repairs the frozen pre-rc4 registration so orchestration send/reply reach native rc4', async () => {
    const nativeLauncher = 'C:\\Program Files\\FABRICA\\resources\\bin\\FABRICA.exe'
    ...
    const simulateRc4Launch = (args: string[]): number => {
      ...
      return target?.toLowerCase().endsWith('FABRICA.cmd') &&
  ...
  it('repairs a managed launcher whose bridge is missing, but preserves a conflicting bridge', async () => {
    const nativeLauncher = 'C:\\FABRICA\\resources\\bin\\FABRICA.exe'
    ...
    const staleBridge = createWslRunner(PRE_RC4_MANAGED_WSL_LAUNCHER, true, {
      initialBridge: '# FABRICA managed WSL CLI PowerShell bridge\nWrite-Output "stale"\n'
    })
  ...
  it('moves a legacy-only managed registration to fabrica without touching unmanaged names', async () => {
    const nativeLauncher = 'C:\\FABRICA\\resources\\bin\\FABRICA.exe'
  ...
  it('keeps the pre-rc4 files on a transactional replacement failure', async () => {
    ...
    const installer = new WslCliInstaller({
      ...
      distro: 'Ubuntu',
      hostInstaller: {
        getStatus: async () => makeHostStatus('C:\\Program Files\\FABRICA\\resources\\bin\\FABRICA.exe')
      },
      wslRunner: wsl.runner
    })
    ...
    // would leave a window where a concurrent shell finds no fabrica at all.
    expect(installCommand).not.toContain('command_backup')
    expect(installCommand).not.toContain(`mv -f '/home/alice/.local/bin/fabrica'`)
  })

  it.skipIf(process.platform === 'win32')(
    'rolls both files back when the command replacement fails after the bridge move',
    async () => {
      const root = await mkdtemp(join(tmpdir(), 'fabrica-wsl-cli-rollback-'))
      const home = join(root, 'home with spaces')
      const commandPath = join(home, '.local', 'bin', 'fabrica')
      const bridgePath = join(home, '.local', 'share', 'fabrica', 'fabrica-wsl-bridge.ps1')
      const bridge = _internals.buildWslBridgeScript()
      await mkdir(join(home, '.local', 'bin'), { recursive: true })
      await mkdir(join(home, '.local', 'share', 'fabrica'), { recursive: true })
  ...
  it('is idempotent after repairing an old managed registration', async () => {
    const nativeLauncher = 'D:\\Custom FABRICA\\resources\\bin\\FABRICA.exe'
    const wsl = createWslRunner(PRE_RC4_MANAGED_WSL_LAUNCHER)
    const installer = new WslCliInstaller({
      platform: 'win32',
      distro: 'Ubuntu',
    ...
    expect(wsl.getFile()).toContain("FABRICA_WIN_LAUNCHER='D:\\Custom FABRICA\\resources\\bin\\FABRICA.exe'")
  })
  ...
  it('refuses to remove an old managed launcher when the bridge path is user-owned', async () => {
    ...
    await expect(installer.remove()).rejects.toThrow('__FABRICA_CONFLICT__')
  })
})
