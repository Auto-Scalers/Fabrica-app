...
const linuxLauncherAsset = new URL('../../../resources/linux/bin/fabrica', import.meta.url)
...
describe('packaged CLI assets', () => {
  ...
  itRunsUnixShell(
    ...
    async () => {
      const root = await mkdtemp(join(tmpdir(), 'FABRICA-linux-cli-'))
      try {
        const appDir = join(root, 'FABRICA')
        const resourcesDir = join(appDir, 'resources')
        const launcherDir = join(resourcesDir, 'bin')
        const cliDir = join(resourcesDir, 'app.asar.unpacked', 'out', 'cli')
        const launcherPath = join(launcherDir, 'fabrica')
        const electronPath = join(appDir, 'fabrica')
...
`,
        ...
        const commandPath = join(commandDir, 'fabrica')
        await mkdir(commandDir, { recursive: true })
        await mkdir(join(homeDir, 'FABRICA'), { recursive: true })
  ...
  itRunsUnixShell('runs the AppImage CLI wrapper through APPDIR at runtime', async () => {
    const root = await mkdtemp(join(tmpdir(), 'FABRICA-appimage-cli-'))
    try {
      const appDir = join(root, 'FABRICA.AppDir')
      const cliDir = join(appDir, 'resources', 'app.asar.unpacked', 'out', 'cli')
      const cliPath = join(cliDir, 'index.js')
      const appImagePath = join(root, "FABRICA's AppImage.AppImage")
      const commandPath = join(root, 'fabrica')
      await mkdir(cliDir, { recursive: true })
      await writeFile(
        cliPath,
        `exports.main = (argv) => {
  console.log(JSON.stringify({
    ...
    FABRICANodeOptions: process.env.FABRICA_NODE_OPTIONS ?? null,
    nodeReplExternalModule: process.env.NODE_REPL_EXTERNAL_MODULE ?? null,
    FABRICANodeReplExternalModule: process.env.FABRICA_NODE_REPL_EXTERNAL_MODULE ?? null
  }))
}
`,
        'utf8'
...
`,
      ...
      const payload = JSON.parse(result.stdout) as {
        ...
        FABRICANodeOptions: string | null
        nodeReplExternalModule: string | null
        FABRICANodeReplExternalModule: string | null
      }
      ...
      expect(payload.FABRICANodeOptions).toBe('--trace-warnings')
      expect(payload.nodeReplExternalModule).toBeNull()
      expect(payload.FABRICANodeReplExternalModule).toBe('external-loader')
    } finally {
      await rm(root, { recursive: true, force: true })
    }
  })
