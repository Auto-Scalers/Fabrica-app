...
async function createPackagedMacLauncher(root: string): Promise<string> {
  ...
  await writeFile(join(resourcesPath, 'bin', 'FABRICA'), '#!/usr/bin/env bash\necho FABRICA\n', {
    encoding: 'utf8',
    mode: 0o755
  })
...
describe('CliInstaller', () => {
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      const installPath = join(fixture.root, 'bin', 'FABRICA')
      const installer = new CliInstaller({
        platform: 'darwin',
        isPackaged: false,
        userDataPath: fixture.userDataPath,
        execPath: '/Applications/FABRICA.app/Contents/MacOS/FABRICA',
      ...
      expect(initial.launcherPath).toContain(join('userData', 'cli', 'bin', 'FABRICA'))
      ...
      expect(launcherContent).toContain(`export FABRICA_USER_DATA_PATH='${fixture.userDataPath}'`)
      expect(launcherContent).toContain('export FABRICA_APP_EXECUTABLE="$ELECTRON"')
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      const fixture = await makeFixture()
      const installPath = join(fixture.root, '.local', 'bin', 'FABRICA-ide')
      const installer = new CliInstaller({
        platform: 'linux',
        isPackaged: false,
        userDataPath: fixture.userDataPath,
        execPath: '/opt/FABRICA/FABRICA-ide',
        appPath: fixture.appPath,
        commandPathOverride: installPath,
      ...
      const installed = await installer.install()
      expect(installed.state).toBe('installed')
      expect(installed.commandName).toBe('FABRICA-ide')
  ...
  // packaged `FABRICA` / `FABRICA-ide` commands developers rely on day to day.
  it.skipIf(process.platform === 'win32')(
    'uses a separate FABRICA-dev command for default development installs',
    async () => {
      const fixture = await makeFixture()
      ...
      const installed = await installer.install()
      expect(installed.state).toBe('installed')
      expect(installed.commandName).toBe('FABRICA-dev')
      expect(installed.commandPath).toBe(join(commandDir, 'FABRICA-dev'))
      expect(installed.launcherPath).toBe(join(fixture.userDataPath, 'cli', 'bin', 'FABRICA-dev'))
      await expect(readlink(installed.commandPath as string)).resolves.toBe(installed.launcherPath)
      await expect(
        readFile(join(fixture.userDataPath, 'cli', 'bin', 'FABRICA'), 'utf8')
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      const installPath = join(commandDir, 'FABRICA-ide')
      const appImagePath = join(fixture.root, 'FABRICA.AppImage')
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      const commandDir = join(fixture.root, '.local', 'bin')
      const installPath = join(commandDir, 'FABRICA-ide')
      const oldAppImagePath = join(fixture.root, 'Old-FABRICA.AppImage')
      const newAppImagePath = join(fixture.root, 'FABRICA.AppImage')
  ...
  // Why: Linux renamed the public command to avoid shadowing GNOME FABRICA, so
  // upgrading must clean up only the old symlink owned by prior FABRICA installs.
  it.skipIf(process.platform === 'win32')(
    'removes the old managed linux FABRICA symlink when installing FABRICA-ide',
    async () => {
      const fixture = await makeFixture()
      const homePath = join(fixture.root, 'home')
      const commandDir = join(homePath, '.local', 'bin')
      const resourcesPath = join(fixture.root, 'resources')
      const launcherPath = join(resourcesPath, 'bin', 'FABRICA-ide')
      const oldLauncherPath = join(resourcesPath, 'bin', 'FABRICA')
      const legacyCommandPath = join(commandDir, 'FABRICA')
      await mkdir(commandDir, { recursive: true })
      await mkdir(join(resourcesPath, 'bin'), { recursive: true })
      ...
      const installed = await installer.install()
      expect(installed.commandPath).toBe(join(commandDir, 'FABRICA-ide'))
      await expect(lstat(legacyCommandPath)).rejects.toMatchObject({ code: 'ENOENT' })
    }
  )

  it.skipIf(process.platform === 'win32')(
    'removes a legacy linux FABRICA symlink when installing an AppImage wrapper',
    async () => {
      const fixture = await makeFixture()
      const homePath = join(fixture.root, 'home')
      const commandDir = join(homePath, '.local', 'bin')
      ...
      await symlink(join('/tmp', '.mount_FABRICA1234', 'resources', 'bin', 'FABRICA'), legacyCommandPath)
  ...
  it('creates a windows wrapper and updates the user PATH', async () => {
    const fixture = await makeFixture()
    const installPath = join(fixture.root, 'Programs', 'FABRICA', 'bin', 'FABRICA.cmd')
    let userPath = 'C:\\Windows\\System32'
    const installer = new CliInstaller({
      platform: 'win32',
      isPackaged: false,
      userDataPath: fixture.userDataPath,
      execPath: 'C:\\Users\\me\\AppData\\Local\\FABRICA\\FABRICA.exe',
      appPath: fixture.appPath,
      commandPathOverride: installPath,
    ...
    expect(installed.state).toBe('installed')
    expect(installed.pathConfigured).toBe(true)
    expect(userPath).toContain(join(fixture.root, 'Programs', 'FABRICA', 'bin'))

    const wrapperContent = await readFile(installPath, 'utf8')
    expect(wrapperContent).toContain('FABRICA_LAUNCHER=')
    expect(wrapperContent).toContain('FABRICA.cmd')
    const launcherContent = await readFile(installed.launcherPath as string, 'utf8')
    expect(launcherContent).toContain(`set "FABRICA_USER_DATA_PATH=${fixture.userDataPath}"`)
    expect(launcherContent).toContain('set "FABRICA_APP_EXECUTABLE=%ELECTRON%"')

    const removed = await installer.remove()
    expect(removed.state).toBe('not_installed')
    expect(userPath).not.toContain(join(fixture.root, 'Programs', 'FABRICA', 'bin'))
  })
  ...
  it('skips the Windows PATH write when removing an absent entry', async () => {
    ...
    const installer = new CliInstaller({
      ...
      execPath: 'C:\\Users\\me\\AppData\\Local\\FABRICA\\FABRICA.exe',
      appPath: fixture.appPath,
      commandPathOverride: join(fixture.root, 'Programs', 'FABRICA', 'bin', 'FABRICA.cmd'),
      userPathReader: async () => userPathRead('C:\\Windows\\System32'),
      userPathWriter
    })
  ...
  it('matches expandable Windows PATH entries case-insensitively without rewriting them', async () => {
    const fixture = await makeFixture()
    const installPath = join(fixture.root, 'Local App Data', 'FABRICA', 'bin', 'FABRICA.cmd')
    const userPathWriter = vi.fn()
    const installer = new CliInstaller({
      platform: 'win32',
      isPackaged: false,
      ...
      userPathReader: async () => userPathRead('%localappdata%\\FABRICA\\bin\\', true),
      userPathWriter
    })
  ...
  it('does not expand environment variables stored in a REG_SZ Windows PATH', async () => {
    ...
    const installer = new CliInstaller({
      ...
      commandPathOverride: installPath,
      windowsEnvironment: { LOCALAPPDATA: join(fixture.root, 'Local App Data') },
      userPathReader: async () => userPathRead('%LOCALAPPDATA%\\FABRICA\\bin'),
      userPathWriter
    })

    await installer.install()

    expect(userPathWriter).toHaveBeenCalledWith(`%LOCALAPPDATA%\\FABRICA\\bin;${pathDirectory}`)
  })

  // Why: this test creates a Unix symlink to /tmp/not-FABRICA, which only applies on macOS/Linux.
  it.skipIf(process.platform === 'win32')(
    'refuses to replace an unknown symlink at the command path',
    async () => {
      const fixture = await makeFixture()
      const installPath = join(fixture.root, 'bin', 'FABRICA')
      const existingTarget = '/tmp/not-FABRICA'
      ...
      await expect(installer.install()).rejects.toThrow('Refusing to replace non-FABRICA command')
      await expect(readlink(installPath)).resolves.toBe(existingTarget)
    }
  )

  // Why: packaged app moves can leave a symlink to an older FABRICA-owned launcher;
  // those are safe to refresh, unlike arbitrary user symlinks.
  it.skipIf(process.platform === 'win32')(
    'replaces stale packaged FABRICA launcher symlinks',
    async () => {
      const fixture = await makeFixture()
      const commandDir = join(fixture.root, 'bin')
      const installPath = join(commandDir, 'FABRICA')
      const resourcesPath = join(fixture.root, 'Current.app', 'Contents', 'Resources')
      const launcherPath = join(resourcesPath, 'bin', 'FABRICA')
      const oldLauncherPath = join(fixture.root, 'Old.app', 'Contents', 'Resources', 'bin', 'FABRICA')
      await mkdir(commandDir, { recursive: true })
      await mkdir(join(resourcesPath, 'bin'), { recursive: true })
      await writeFile(launcherPath, '#!/usr/bin/env bash\n', 'utf8')
  ...
  // Why: old dev/package experiments wrote a generated FABRICA launcher file
  // directly into /usr/local/bin/FABRICA. That broke profiling because Settings
    ...
    async () => {
      ...
      await writeFile(
        ...
        [
          ...
          `CLI='${oldCliPath}'`,
          'export FABRICA_NODE_OPTIONS="${NODE_OPTIONS-}"',
          'export FABRICA_NODE_REPL_EXTERNAL_MODULE="${NODE_REPL_EXTERNAL_MODULE-}"',
          'unset NODE_OPTIONS',
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      await writeFile(
        installPath,
        '#!/usr/bin/env bash\nELECTRON_RUN_AS_NODE=1 /tmp/not-FABRICA "$@"\n',
        'utf8'
      )
      ...
      await expect(installer.install()).rejects.toThrow('Refusing to replace non-FABRICA command')
      await expect(readFile(installPath, 'utf8')).resolves.toContain('/tmp/not-FABRICA')
    }
  )

  // Why: a dev build can temporarily own the public command on developer
  // machines; packaged FABRICA should treat that as stale, not a hard conflict.
  it.skipIf(process.platform === 'win32')(
    'replaces stale sibling dev launcher symlinks from packaged installs',
    async () => {
      const fixture = await makeFixture()
      for (const devLauncherName of ['FABRICA', 'FABRICA-dev']) {
        const caseRoot = join(fixture.root, devLauncherName)
        const commandDir = join(caseRoot, 'bin')
        const installPath = join(commandDir, 'FABRICA')
        const userDataPath = join(caseRoot, 'FABRICA')
  ...
  it.skipIf(process.platform === 'win32')(
    'falls back to ~/.local/bin/FABRICA on macOS when /usr/local/bin does not exist',
    async () => {
      const fixture = await makeFixture()
      const homePath = join(fixture.root, 'home')
      const resourcesPath = await createPackagedMacLauncher(fixture.root)
      ...
      const absentUsrLocalBin = join(fixture.root, 'usr', 'local', 'bin', 'FABRICA')
      const installer = new CliInstaller({
        platform: 'darwin',
        isPackaged: true,
        resourcesPath,
      ...
      expect(status.commandPath).toBe(join(homePath, '.local', 'bin', 'FABRICA'))
      expect(status.state).toBe('not_installed')
      expect(status.supported).toBe(true)

      const installed = await installer.install()
      expect(installed.state).toBe('installed')
      expect(installed.commandPath).toBe(join(homePath, '.local', 'bin', 'FABRICA'))
      expect(installed.pathConfigured).toBe(true)
    }
  )
  ...
  it.skipIf(process.platform === 'win32')(
    'uses /usr/local/bin/FABRICA on macOS when /usr/local/bin exists',
    async () => {
      const fixture = await makeFixture()
      const resourcesPath = await createPackagedMacLauncher(fixture.root)
      ...
      const installPath = join(usrLocalBin, 'FABRICA')
      const installer = new CliInstaller({
        platform: 'darwin',
        isPackaged: true,
        resourcesPath,
  ...
  // Why: users can have a managed FABRICA command in ~/.local/bin even when
  // /usr/local/bin exists; Settings must follow the shell-visible command.
  it.skipIf(process.platform === 'win32')(
    'uses an existing managed macOS FABRICA command from the shell PATH before /usr/local/bin',
    async () => {
      const fixture = await makeFixture()
      const homePath = join(fixture.root, 'home')
      const resourcesPath = await createPackagedMacLauncher(fixture.root)
      ...
      const defaultInstallPath = join(usrLocalBin, 'FABRICA')
      const userInstallPath = join(userLocalBin, 'FABRICA')
      const launcherPath = join(resourcesPath, 'bin', 'FABRICA')
      await mkdir(usrLocalBin, { recursive: true })
  ...
  it.skipIf(process.platform === 'win32')(
    'skips a broken managed macOS FABRICA symlink before /usr/local/bin',
    async () => {
      const fixture = await makeFixture()
      const homePath = join(fixture.root, 'home')
      const resourcesPath = await createPackagedMacLauncher(fixture.root)
  ...
  // ~/.local/bin/FABRICA must not steal status from /usr/local/bin/FABRICA.
  it.skipIf(process.platform === 'win32')(
    'keeps the default macOS command when a managed FABRICA appears later on PATH',
    async () => {
      const fixture = await makeFixture()
      const homePath = join(fixture.root, 'home')
      const resourcesPath = await createPackagedMacLauncher(fixture.root)
  ...
  // is still the shell-visible FABRICA command until the default path is installed.
  it.skipIf(process.platform === 'win32')(
    'uses a later managed macOS FABRICA command when the default command is missing',
    async () => {
      const fixture = await makeFixture()
      const homePath = join(fixture.root, 'home')
      const resourcesPath = await createPackagedMacLauncher(fixture.root)
  ...
  // Why: bash/zsh skip non-executable PATH entries even at FABRICA's configured
  // install slot, then keep looking for a runnable command later in PATH.
  it.skipIf(process.platform === 'win32')(
    'uses a later managed macOS FABRICA command when the default command is not executable',
    async () => {
      const fixture = await makeFixture()
      const homePath = join(fixture.root, 'home')
      const resourcesPath = await createPackagedMacLauncher(fixture.root)
      ...
      const installed = await installer.install()
      expect(installed.commandPath).toBe(userInstallPath)
      await expect(readFile(defaultInstallPath, 'utf8')).resolves.toContain('other-FABRICA')
    }
  )

  // Why: a non-FABRICA command after an empty default install slot can be shadowed
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      expect(installed.state).toBe('installed')
      await expect(readlink(defaultInstallPath)).resolves.toBe(launcherPath)
      await expect(readFile(userInstallPath, 'utf8')).resolves.toContain('other-FABRICA')
    }
  )

  // Why: an off-PATH ~/.local/bin/FABRICA must not hijack CLI registration and
  // leave the shell-visible /usr/local/bin command missing.
  it.skipIf(process.platform === 'win32')(
    'ignores managed macOS FABRICA commands that are not visible on the shell PATH',
    async () => {
      const fixture = await makeFixture()
      const homePath = join(fixture.root, 'home')
      const resourcesPath = await createPackagedMacLauncher(fixture.root)
  ...
  it.skipIf(process.platform === 'win32')(
    'reports a conflict for an unmanaged macOS FABRICA that shadows the install path',
    async () => {
      const fixture = await makeFixture()
      const homePath = join(fixture.root, 'home')
      const resourcesPath = await createPackagedMacLauncher(fixture.root)
  ...
  it.skipIf(process.platform === 'win32')(
    'skips a non-executable unmanaged macOS FABRICA before the install path',
    async () => {
      const fixture = await makeFixture()
      const homePath = join(fixture.root, 'home')
      const resourcesPath = await createPackagedMacLauncher(fixture.root)
  ...
  // Why: when macCommandPath falls back to ~/.local/bin/FABRICA on arm64, commandName
  // must still be 'FABRICA' (not 'FABRICA-ide' which is Linux-only).
  it.skipIf(process.platform === 'win32')(
    'reports commandName as FABRICA (not FABRICA-ide) when falling back to ~/.local/bin on macOS',
    async () => {
      const fixture = await makeFixture()
      const homePath = join(fixture.root, 'home')
      const resourcesPath = await createPackagedMacLauncher(fixture.root)
      ...
      const status = await installer.getStatus()
      expect(status.commandName).toBe('FABRICA')
    }
  )
  ...
  it.skipIf(process.platform === 'win32' || process.getuid?.() === 0)(
    ...
    async () => {
      ...
      const installPath = join(protectedDir, 'bin', 'FABRICA')
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      expect(s1.commandPath).toBe(join(homePath, '.local', 'bin', 'FABRICA'))
    }
  )
  ...
  it('resolves custom-install packaged Windows command path from resourcesPath', async () => {
    ...
    const resourcesPath = join(fixture.root, 'D Custom FABRICA', 'resources')
    await mkdir(join(resourcesPath, 'bin'), { recursive: true })
    await writeFile(join(resourcesPath, 'bin', 'FABRICA.exe'), 'native launcher', 'utf8')
    ...
    const installer = new CliInstaller({
      ...
      localAppDataPath,
      userDataPath: fixture.userDataPath,
      execPath: join(fixture.root, 'D Custom FABRICA', 'FABRICA.exe'),
      appPath: fixture.appPath,
    ...
    const status = await installer.getStatus()
    expect(status.commandPath).toBe(join(resourcesPath, 'bin', 'FABRICA.exe'))
  })

  it('keeps a bundled Windows launcher installed when the user PATH read is unknown', async () => {
    const fixture = await makeFixture()
    const resourcesPath = join(fixture.root, 'resources')
    const bundledLauncher = join(resourcesPath, 'bin', 'FABRICA.exe')
    ...
    const installer = new CliInstaller({
      ...
      resourcesPath,
      userDataPath: fixture.userDataPath,
      execPath: join(fixture.root, 'FABRICA.exe'),
      appPath: fixture.appPath,
      userPathReader: async () => ({
        state: 'unknown',
        detail: 'FABRICA could not read the Windows user PATH registry value.'
  ...
  it.skipIf(process.platform === 'win32')(
    'resolves to ~/.local/bin/FABRICA on arm64 even when isPackaged is true',
    async () => {
      const fixture = await makeFixture()
      const homePath = join(fixture.root, 'home')
      const absentUsrLocalBin = join(fixture.root, 'usr', 'local', 'bin', 'FABRICA')
      const resourcesPath = join(fixture.root, 'resources')
      const bundledLauncher = join(resourcesPath, 'bin', 'FABRICA')
      await mkdir(join(resourcesPath, 'bin'), { recursive: true })
