...
describe('applyTerminalAttributionEnv PATH key casing', () => {
  ...
  function makeTmpRoot(): string {
    tmpRoot = mkdtempSync(join(tmpdir(), 'FABRICA-attribution-path-casing-'))
    return tmpRoot
  }

  // Why: a Windows child that inherits both spellings crashes the packaged FABRICA.exe launcher
  // with "Item has already been added. Key in dictionary: 'PATH'" (Auto-Scalers/Fabrica#12046).
  ...
  it('keeps a shadowed duplicate when the live spelling strips down to nothing', () => {
    const root = makeTmpRoot()
    const userDataPath = join(root, 'user-data')
    const shimDir = join(userDataPath, 'FABRICA-terminal-attribution', 'win32')
