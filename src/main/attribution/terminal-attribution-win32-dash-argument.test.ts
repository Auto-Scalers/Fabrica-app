...
describe('win32 attribution wrappers with a bare `-` argument', () => {
  ...
  function shimDir(): string {
    tmpRoot = mkdtempSync(join(tmpdir(), 'FABRICA-attribution-dash-'))
    ...
    return join(userDataPath, 'FABRICA-terminal-attribution', 'win32')
  }

  it('guards the git PowerShell dispatch behind a bare-dash check', () => {
    const wrapper = readFileSync(join(shimDir(), 'git.cmd'), 'utf8')
    const guardAt = wrapper.indexOf('call :FABRICA_has_bare_dash %*')
    ...
    expect(wrapper).toContain(':FABRICA_has_bare_dash')
  ...
  itWindows('commits a message piped through `git commit -F -`', () => {
    ...
    const repo = mkdtempSync(join(tmpdir(), 'FABRICA-attribution-dash-repo-'))
    try {
      ...
      const env = {
        ...process.env,
        FABRICA_ENABLE_GIT_ATTRIBUTION: '1',
        FABRICA_REAL_GIT: spawnSync('where', ['git.exe'], { encoding: 'utf8' }).stdout.split(
          /\r?\n/
        )[0]
      }
