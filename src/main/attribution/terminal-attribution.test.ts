...
describe('applyTerminalAttributionEnv', () => {
  ...
  function makeTmpRoot(): string {
    tmpRoot = mkdtempSync(join(tmpdir(), 'FABRICA-attribution-'))
    return tmpRoot
  }
  ...
  function stripInheritedAttributionPath(pathValue: string): string {
    ...
    return pathValue
      .split(pathDelimiter)
      .filter((entry) => !entry.includes('FABRICA-terminal-attribution'))
      .join(pathDelimiter)
  }

  function cleanAttributionEnv(env?: Record<string, string>): Record<string, string> {
    const base = { ...process.env }
    delete base.FABRICA_ENABLE_GIT_ATTRIBUTION
    delete base.FABRICA_GIT_COMMIT_TRAILER
    delete base.FABRICA_GH_PR_FOOTER
    delete base.FABRICA_GH_ISSUE_FOOTER
    delete base.FABRICA_ATTRIBUTION_SHIM_DIR
    delete base.FABRICA_REAL_GIT
    delete base.FABRICA_REAL_GH
  ...
  posixSubprocessIt('does not amend HEAD when git commit --dry-run exits successfully', () => {
    ...
    mkdirSync(repo)
    runGit(repo, ['init'])
    runGit(repo, ['config', 'user.name', 'FABRICA Test'])
    runGit(repo, ['config', 'user.email', 'FABRICA-test@example.com'])
    ...
    expect(runGit(repo, ['log', '-1', '--format=%B'])).not.toContain('Co-authored-by: Fabrica')
...
`,
      ...
      expect(readFileSync(argsPath, 'utf8')).not.toContain('Co-authored-by: Fabrica')
    }
  )
...
grep -Fq 'Co-authored-by: Fabrica <fabrica.studio.contact@gmail.com>' "$1"
`,
      'utf8'
    )
...
`,
    ...
    expect(readFileSync(argsPath, 'utf8')).toContain('Co-authored-by: Fabrica <fabrica.studio.contact@gmail.com>')
  })
...
if [[ "$1 $2 $3 $4" == "pr view --json url" ]]; then
  printf '%s\\n' 'https://github.com/Auto-Scalers/Fabrica-app/pull/123'
  exit 0
fi
if [[ "$1 $2" == "api repos/Auto-Scalers/Fabrica-app/pulls/123" && "\${3:-}" == "--jq" ]]; then
  printf '%s\\n' 'Existing body'
  exit 0
fi
if [[ "$1 $2 $3 $4" == "api -X PATCH repos/Auto-Scalers/Fabrica-app/pulls/123" ]]; then
  touch "${markerPath}"
  exit 0
fi
exit 1
...
if [[ "$1 $2" == "issue create" ]]; then
  printf '%s\\n' 'https://github.com/Auto-Scalers/Fabrica-app/issues/456'
  exit 0
fi
if [[ "$1 $2" == "api repos/Auto-Scalers/Fabrica-app/pulls/123" && "\${3:-}" == "--jq" ]]; then
  printf '%s\\n' 'PR body'
...
if [[ "$1 $2" == "api repos/Auto-Scalers/Fabrica-app/issues/456" && "\${3:-}" == "--jq" ]]; then
  printf '%s\\n' 'Issue body'
  exit 0
fi
if [[ "$1 $2 $3 $4" == "api -X PATCH repos/Auto-Scalers/Fabrica-app/pulls/123" ]]; then
...
if [[ "$1 $2 $3 $4" == "api -X PATCH repos/Auto-Scalers/Fabrica-app/issues/456" ]]; then
...
`,
    ...
    ).toBe('https://github.com/Auto-Scalers/Fabrica-app/pull/123\n')
    expect(
      execFileSync('gh', ['issue', 'create', '--title', 'Issue', '--body', 'Body'], {
        encoding: 'utf8',
        env: cleanAttributionEnv(attributionEnv)
      })
    ).toBe('https://github.com/Auto-Scalers/Fabrica-app/issues/456\n')
...
`,
    ...
    expect(output).toBe('https://github.com/Auto-Scalers/Fabrica-app/pull/123\n')
    expect(existsSync(markerPath)).toBe(false)
  })
...
`,
  ...
  it('fails open when shim files cannot be written', () => {
    ...
    expect(baseEnv.FABRICA_ENABLE_GIT_ATTRIBUTION).toBeUndefined()
  ...
  it('does not duplicate shim directories when applied to an already-injected env', () => {
    ...
    const shimEntries = baseEnv.PATH.split(pathDelimiter).filter((entry) =>
      entry.includes('FABRICA-terminal-attribution')
    )
  ...
  it('puts only Windows shims on PATH for native Windows shells', () => {
    ...
    const posixDir = join(userDataPath, 'FABRICA-terminal-attribution', 'posix')
    const win32Dir = join(userDataPath, 'FABRICA-terminal-attribution', 'win32')
    const pathEntries = baseEnv.PATH.split(';')

    expect(pathEntries[0]).toBe(win32Dir)
    expect(pathEntries).not.toContain(posixDir)
    expect(baseEnv.FABRICA_ATTRIBUTION_SHIM_DIR).toBeUndefined()
  ...
  it('keeps POSIX shims first for Windows Git Bash and WSL shells', () => {
    ...
    expect(baseEnv.FABRICA_ATTRIBUTION_SHIM_DIR).toBe(posixDir)
  ...
  it('writes PowerShell wrappers without raw-template backslash escapes', () => {
    ...
    const shimDir = join(root, 'user-data', 'FABRICA-terminal-attribution', 'win32')
