...
import { FABRICA_GIT_COMMIT_TRAILER } from '../../shared/fabrica-attribution'
...
const ATTRIBUTION_ENV_KEYS = [
  'FABRICA_ENABLE_GIT_ATTRIBUTION',
  'FABRICA_GIT_COMMIT_TRAILER',
  'FABRICA_GH_PR_FOOTER',
  'FABRICA_GH_ISSUE_FOOTER',
...
export function applyTerminalAttributionEnv(
  ...
  // cannot recursively point FABRICA_REAL_* at themselves.
  ...
  // Why: these wrappers should affect only FABRICA-managed PTYs. Prepending the
  // shim directory here keeps the attribution behavior scoped to FABRICA's live
  ...
  baseEnv.FABRICA_ENABLE_GIT_ATTRIBUTION = '1'
  baseEnv.FABRICA_GIT_COMMIT_TRAILER = FABRICA_GIT_COMMIT_TRAILER
  baseEnv.FABRICA_GH_PR_FOOTER = FABRICA_GH_FOOTER
  baseEnv.FABRICA_GH_ISSUE_FOOTER = FABRICA_GH_FOOTER
  if (shellFamily === 'posix') {
    baseEnv.FABRICA_ATTRIBUTION_SHIM_DIR = posixDir
  } else {
  ...
  if (platform === 'win32') {
    if (resolvedGit) {
      baseEnv.FABRICA_REAL_GIT = resolvedGit
    }
    if (resolvedGh) {
      baseEnv.FABRICA_REAL_GH = resolvedGh
    }
  }
}
...
clean_path() {
  ...
  for entry in "${SHELL_DOLLAR}{entries[@]}"; do
    case "$entry" in
      "$script_dir"|*/FABRICA-terminal-attribution/posix|*/FABRICA-terminal-attribution/win32|*\\FABRICA-terminal-attribution\\posix|*\\FABRICA-terminal-attribution\\win32)
        ;;
      *)
        cleaned+=("$entry")
        ;;
...
if [[ -z "$real_git" ]]; then
  echo "Fabrica attribution wrapper could not locate git on PATH." >&2
  exit 127
fi
...
if [[ "\${FABRICA_ENABLE_GIT_ATTRIBUTION:-0}" != "1" || "\${FABRICA_ATTRIBUTION_BYPASS:-0}" == "1" ]] || ! is_commit_command "$@"; then
...
trailer="\${FABRICA_GIT_COMMIT_TRAILER:-Co-authored-by: Fabrica <fabrica.studio.contact@gmail.com>}"
...
FABRICA_ATTRIBUTION_BYPASS=1 PATH="$real_path" exec "$real_git" "${SHELL_DOLLAR}{attributed_args[@]}"
`
...
if [[ -z "$real_gh" ]]; then
  echo "Fabrica attribution wrapper could not locate gh on PATH." >&2
  exit 127
fi
...
if [[ "\${FABRICA_ENABLE_GIT_ATTRIBUTION:-0}" != "1" || "\${FABRICA_ATTRIBUTION_BYPASS:-0}" == "1" ]]; then
...
setlocal
if not "%FABRICA_ENABLE_GIT_ATTRIBUTION%"=="1" goto run
if "%FABRICA_ATTRIBUTION_BYPASS%"=="1" goto run
call :FABRICA_is_git_commit %*
if errorlevel 1 goto run
call :FABRICA_has_bare_dash %*
if not errorlevel 1 goto run
powershell.exe -NoProfile -ExecutionPolicy Bypass -File "%~dp0git-wrapper.ps1" %*
exit /b %ERRORLEVEL%
:run
if defined FABRICA_REAL_GIT (
  "%FABRICA_REAL_GIT%" %*
) else (
  echo Fabrica attribution wrapper could not locate git on PATH. 1>&2
  exit /b 127
)
exit /b %ERRORLEVEL%

:FABRICA_is_git_commit
if "%~1"=="" exit /b 1
if /I "%~1"=="commit" exit /b 0
set "FABRICA_git_arg=%~1"
if /I "%FABRICA_git_arg%"=="-c" goto skip_two
if /I "%FABRICA_git_arg%"=="--config" goto skip_two
if /I "%FABRICA_git_arg%"=="-C" goto skip_two
if /I "%FABRICA_git_arg%"=="--git-dir" goto skip_two
if /I "%FABRICA_git_arg%"=="--work-tree" goto skip_two
if /I "%FABRICA_git_arg%"=="--namespace" goto skip_two
if /I "%FABRICA_git_arg:~0,9%"=="--config=" goto skip_one
if /I "%FABRICA_git_arg:~0,10%"=="--git-dir=" goto skip_one
if /I "%FABRICA_git_arg:~0,12%"=="--work-tree=" goto skip_one
if /I "%FABRICA_git_arg:~0,12%"=="--namespace=" goto skip_one
if "%FABRICA_git_arg:~0,1%"=="-" goto skip_one
exit /b 1
:skip_two
shift
shift
goto FABRICA_is_git_commit
:skip_one
shift
goto FABRICA_is_git_commit
...
:FABRICA_has_bare_dash
if "%~1"=="" exit /b 1
if "%~1"=="-" exit /b 0
shift
goto FABRICA_has_bare_dash
`
...
exit /b %ERRORLEVEL%
:run
if defined FABRICA_REAL_GH (
  "%FABRICA_REAL_GH%" %*
) else (
  echo Fabrica attribution wrapper could not locate gh on PATH. 1>&2
  exit /b 127
)
exit /b %ERRORLEVEL%
...
$realGit = if ($env:FABRICA_REAL_GIT) { $env:FABRICA_REAL_GIT } else { 'git' }
$trailer = if ($env:FABRICA_GIT_COMMIT_TRAILER) { $env:FABRICA_GIT_COMMIT_TRAILER } else { 'Co-authored-by: Fabrica <fabrica.studio.contact@gmail.com>' }
...
$env:FABRICA_ATTRIBUTION_BYPASS = '1'
try {
...
$realGh = if ($env:FABRICA_REAL_GH) { $env:FABRICA_REAL_GH } else { 'gh' }
