...
const POWERSHELL_OSC133_BOOTSTRAP = `# FABRICA OSC 133 shell integration for PowerShell.
...
if ($env:FABRICA_OPENCODE_CONFIG_DIR) { $env:OPENCODE_CONFIG_DIR = $env:FABRICA_OPENCODE_CONFIG_DIR }
if ($env:FABRICA_MIMOCODE_HOME) { $env:MIMOCODE_HOME = $env:FABRICA_MIMOCODE_HOME }
if ($env:FABRICA_CODEX_HOME) { $env:CODEX_HOME = $env:FABRICA_CODEX_HOME }

if ($ExecutionContext.SessionState.LanguageMode -eq "FullLanguage" -and
    ((-not (Test-Path variable:global:__FABRICAOsc133State)) -or
     $null -eq $Global:__FABRICAOsc133State.OriginalPrompt)) {
...
${getPowerShellOmpShellWrapper()}
    ...
    function Global:prompt {
        ...
        if ($Global:__FABRICAOsc133State.HasSeenPrompt) {
            $result += "$($Global:__FABRICAOsc133State.Esc)]133;D;$fakeExitCode$($Global:__FABRICAOsc133State.Bel)"
        }
        $Global:__FABRICAOsc133State.HasSeenPrompt = $true

        $result += "$($Global:__FABRICAOsc133State.Esc)]133;A$($Global:__FABRICAOsc133State.Bel)"
        ...
        $result += $Global:__FABRICAOsc133State.OriginalPrompt.Invoke()
        $result += "$($Global:__FABRICAOsc133State.Esc)]133;B$($Global:__FABRICAOsc133State.Bel)"
        $result
    }

    if ($Global:__FABRICAOsc133State.HasPSReadLine -and
        $null -ne $Global:__FABRICAOsc133State.OriginalReadLine) {
        function Global:PSConsoleHostReadLine {
            $commandLine = $Global:__FABRICAOsc133State.OriginalReadLine.Invoke()
            [Console]::Write("$($Global:__FABRICAOsc133State.Esc)]133;C$($Global:__FABRICAOsc133State.Bel)")
            return $commandLine
        }
    }
}
