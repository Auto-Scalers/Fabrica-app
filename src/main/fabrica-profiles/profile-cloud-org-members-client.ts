...
const ORG_ROLES: readonly FABRICAOrgRole[] = ['owner', 'admin', 'member']

function isOrgRole(value: unknown): value is FABRICAOrgRole {
  return typeof value === 'string' && (ORG_ROLES as readonly string[]).includes(value)
}

function normalizeRole(value: unknown, fallback: FABRICAOrgRole): FABRICAOrgRole {
...
function normalizeMember(value: unknown): FABRICAOrgMember | null {
...
function normalizePendingInvite(value: unknown): FABRICAOrgPendingInvite | null {
  if (!value || typeof value !== 'object') {
    return null
  }
  const record = value as Record<string, unknown>
...
function normalizeRoster(value: unknown): FABRICAOrgMembersRoster {
  if (!value || typeof value !== 'object') {
    throw new Error('invalid_FABRICA_org_members_roster')
  }
  const record = value as Record<string, unknown>
  const members = Array.isArray(record.members)
    ? record.members
        .map(normalizeMember)
        .filter((member): member is FABRICAOrgMember => member !== null)
    : []
  const pendingInvites = Array.isArray(record.pendingInvites)
    ? record.pendingInvites
        .map(normalizePendingInvite)
        .filter((invite): invite is FABRICAOrgPendingInvite => invite !== null)
    : []
  return {
    members,
    pendingInvites,
...
function orgMembersUrl(config: FABRICACloudAuthConfig, orgId: string, path: string): string {
...
async function requestOrgMembers<T>(
  ...
  if (!response.ok) {
    throw new FABRICACloudRequestError(response.status, await extractErrorCode(response))
  }
  return parse((await response.json()) as unknown)
}

export async function listFABRICACloudOrgMembers(
  config: FABRICACloudAuthConfig,
  session: FABRICACloudSession,
  orgId: string
): Promise<FABRICAOrgMembersRoster> {
...
export async function inviteFABRICACloudOrgMember(
  config: FABRICACloudAuthConfig,
  session: FABRICACloudSession,
  args: { orgId: string; email: string; role: FABRICAOrgRole }
): Promise<void> {
...
export async function revokeFABRICACloudOrgInvite(
  config: FABRICACloudAuthConfig,
  session: FABRICACloudSession,
...
export async function changeFABRICACloudOrgMemberRole(
  config: FABRICACloudAuthConfig,
  session: FABRICACloudSession,
  args: { orgId: string; userId: string; role: FABRICAOrgRole }
): Promise<void> {
  await requestOrgMembers(
...
export async function removeFABRICACloudOrgMember(
  config: FABRICACloudAuthConfig,
  session: FABRICACloudSession,
