...
function assertKnownProfiles(args: TransferFABRICAProfileProjectArgs, userDataPath: string): void {
  const profiles = getFABRICAProfileListState(userDataPath).profiles
  const ids = new Set(profiles.map((profile) => profile.id))
  if (!ids.has(args.sourceProfileId)) {
    throw new Error('unknown_source_FABRICA_profile')
  }
  if (!ids.has(args.targetProfileId)) {
    throw new Error('unknown_target_FABRICA_profile')
  }
  if (args.sourceProfileId === args.targetProfileId) {
    throw new Error('matching_FABRICA_profile_transfer')
  }
}

export function transferFABRICAProfileProject(
