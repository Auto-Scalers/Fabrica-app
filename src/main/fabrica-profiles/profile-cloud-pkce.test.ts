...
async function startedFlow(): Promise<{
  authUrl: URL
  flow: ReturnType<typeof beginFABRICACloudPkceFlow>
  nonce: string
  redirectUri: string
  state: string
}> {
  const flow = beginFABRICACloudPkceFlow(config, 'local-default')
...
describe('FABRICA cloud PKCE flow', () => {
  beforeEach(() => {
  ...
  it('keeps the loopback listener alive after an invalid callback', async () => {
    ...
    expect(validResponse.body).toContain('<h1>Signed in to FABRICA</h1>')
  ...
  it('rejects a provider error that matches the flow state', async () => {
    ...
    await expect(observedFlow).resolves.toMatchObject({ message: 'FABRICA_cloud_auth_denied' })
  })
