...
import {
  FABRICA_CLOUD_CALLBACK_RESPONSE_HEADERS,
  FABRICA_CLOUD_CALLBACK_SUCCESS_PAGE
...
export function beginFABRICACloudPkceFlow(
  ...
  return new Promise((resolve, reject) => {
    ...
    function writeInvalidCallback(response: ServerResponse): void {
      ...
      response.end('Invalid Fabrica sign-in response.')
    }
    ...
    const server = createServer((request, response) => {
      try {
        ...
        if (url.searchParams.has('error')) {
          response.writeHead(400)
          response.end('Fabrica sign-in was cancelled.')
    ...
    const timeout = setTimeout(() => {
      rejectFlow(new Error('FABRICA_cloud_auth_timeout'))
    }, AUTH_TIMEOUT_MS)
    ...
    server.listen(0, '127.0.0.1', () => {
      ...
      if (!address || typeof address === 'string') {
        rejectFlow(new Error('FABRICA_cloud_auth_loopback_unavailable'))
        return
      }
      ...
      void shell.openExternal(authorizeUrl.toString()).catch((error) => {
        rejectFlow(
          error instanceof Error ? error : new Error('FABRICA_cloud_auth_browser_open_failed')
        )
      })
    })
  })
