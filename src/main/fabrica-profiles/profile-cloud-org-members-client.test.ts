...
describe('FABRICA cloud org members client', () => {
  beforeEach(() => {
    fetchMock.mockReset()
  ...
  it('normalizes the roster, dropping malformed rows and defaulting the viewer role', async () => {
    ...
    await expect(listFABRICACloudOrgMembers(config, session, 'org-1')).resolves.toEqual({
      members: [
        { userId: 'user-1', email: 'nina@example.com', displayName: 'Nina', role: 'admin' },
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      'https://FABRICA-cloud.example/v1/desktop/orgs/org-1/members',
  ...
  it('percent-encodes the org id in the request URL', async () => {
    ...
    await listFABRICACloudOrgMembers(config, session, 'org/with space')
    expect(fetchMock).toHaveBeenCalledWith(
      'https://FABRICA-cloud.example/v1/desktop/orgs/org%2Fwith%20space/members',
      expect.any(Object)
    )
  })
  ...
  it('posts invites with an email and role body', async () => {
    ...
    await inviteFABRICACloudOrgMember(config, session, {
      orgId: 'org-1',
      email: 'new@example.com',
      role: 'admin'
    })
    expect(fetchMock).toHaveBeenCalledWith(
      'https://FABRICA-cloud.example/v1/desktop/orgs/org-1/invites',
      expect.objectContaining({
        method: 'POST',
  ...
  it('carries the server error code on a 409 conflict', async () => {
    ...
    await expect(
      inviteFABRICACloudOrgMember(config, session, {
        orgId: 'org-1',
  ...
  it('surfaces a 403 as an FABRICACloudRequestError without an error code when the body is empty', async () => {
    fetchMock.mockResolvedValue({
      ok: false,
      status: 403,
      json: async () => {
    ...
    await expect(
      changeFABRICACloudOrgMemberRole(config, session, {
        orgId: 'org-1',
        userId: 'user-2',
        role: 'admin'
      })
    ).rejects.toBeInstanceOf(FABRICACloudRequestError)
    await expect(
      removeFABRICACloudOrgMember(config, session, { orgId: 'org-1', userId: 'user-2' })
    ).rejects.toMatchObject({ statusCode: 403, errorCode: undefined })
  })

  it('posts invite revocations by email', async () => {
    mockJsonResponse({ ok: true })
    await revokeFABRICACloudOrgInvite(config, session, { orgId: 'org-1', email: 'gone@example.com' })
    expect(fetchMock).toHaveBeenCalledWith(
      'https://FABRICA-cloud.example/v1/desktop/orgs/org-1/invites/revoke',
