...
import {
  createDefaultLocalFABRICAProfile,
  DEFAULT_LOCAL_FABRICA_PROFILE_ID,
  DEFAULT_LOCAL_FABRICA_PROFILE_NAME,
  FABRICA_PROFILE_INDEX_SCHEMA_VERSION,
  type CreateLocalFABRICAProfileArgs,
  type CreateLocalFABRICAProfileResult,
  type FABRICAProfileIndex,
  type FABRICAProfileListState,
  type FABRICAProfileSummary
} from '../../shared/fabrica-profiles'
import {
  getFABRICAProfileBrowserSessionMetaFile,
  getFABRICAProfileDataFile,
...
export {
  ...
  getFABRICAProfileDirectory,
  getFABRICAProfileIndexPath,
  getFABRICAProfilesDirectory,
  initFABRICAProfilePaths
} from './profile-storage-paths'

export type ActiveFABRICAProfileState = {
  index: FABRICAProfileIndex
...
function isProfileSummary(value: unknown): value is FABRICAProfileSummary {
  if (!isObject(value)) {
    return false
  }
...
function normalizeProfileIndex(raw: unknown): FABRICAProfileIndex | null {
...
function readProfileIndexFile(indexPath: string): FABRICAProfileIndex | null {
  try {
...
export function readProfileIndex(indexPath: string): FABRICAProfileIndex | null {
...
export function writeProfileIndex(indexPath: string, index: FABRICAProfileIndex): void {
...
function copyLegacyStateToProfile(userDataPath: string, profileId: string): void {
  const profileDataFile = getFABRICAProfileDataFile(profileId, userDataPath)
  copyIfPresent(legacyDataFilePath(userDataPath), profileDataFile)
  copyIfPresent(
    legacyBrowserSessionMetaPath(userDataPath),
    getFABRICAProfileBrowserSessionMetaFile(profileId, userDataPath)
  )
...
export function seedNewFABRICAProfileTelemetryConsent(
  profileId: string,
  ...
  const dataFile = getFABRICAProfileDataFile(profileId, userDataPath)
...
function createInitialProfileIndex(now = Date.now()): FABRICAProfileIndex {
  const profile = createDefaultLocalFABRICAProfile(now)
  return {
    schemaVersion: FABRICA_PROFILE_INDEX_SCHEMA_VERSION,
...
export function loadOrCreateProfileIndex(userDataPath: string): FABRICAProfileIndex {
  const indexPath = getFABRICAProfileIndexPath(userDataPath)
...
function getActiveProfile(index: FABRICAProfileIndex): FABRICAProfileSummary {
  return (
    index.profiles.find((profile) => profile.id === index.activeProfileId) ??
    index.profiles[0] ??
    createDefaultLocalFABRICAProfile(Date.now())
  )
}

export function ensureActiveFABRICAProfile(
  userDataPath = getProfileUserDataPath()
): ActiveFABRICAProfileState {
  const indexPath = getFABRICAProfileIndexPath(userDataPath)
  ...
  const profileDirectory = getFABRICAProfileDirectory(activeProfile.id, userDataPath)
  mkdirSync(profileDirectory, { recursive: true })
  if (activeProfile.id === DEFAULT_LOCAL_FABRICA_PROFILE_ID) {
  ...
  return {
    ...
    dataFile: getFABRICAProfileDataFile(activeProfile.id, userDataPath),
    profileDirectory
  }
}

export function isDefaultLocalFABRICAProfileId(profileId: string): boolean {
  return profileId === DEFAULT_LOCAL_FABRICA_PROFILE_ID
}

export function getFABRICAProfileListState(
  userDataPath = getProfileUserDataPath()
): FABRICAProfileListState {
  const { index } = ensureActiveFABRICAProfile(userDataPath)
  return {
...
export function createLocalFABRICAProfile(
  args: CreateLocalFABRICAProfileArgs = {},
  userDataPath = getProfileUserDataPath()
  ...
  const profile: FABRICAProfileSummary = {
    ...
    avatar: {
      kind: 'initials',
      initials: (
        name.match(/[A-Za-z0-9]/)?.[0] ?? DEFAULT_LOCAL_FABRICA_PROFILE_NAME[0]
      ).toUpperCase(),
      color: 'neutral'
    },
    kind: 'local',
  ...
  const nextIndex: FABRICAProfileIndex = {
    ...index,
...
export function setActiveFABRICAProfile(
  profileId: string,
  userDataPath = getProfileUserDataPath()
): FABRICAProfileListState {
  const index = loadOrCreateProfileIndex(userDataPath)
  ...
  mkdirSync(getFABRICAProfileDirectory(profileId, userDataPath), { recursive: true })
  writeProfileIndex(getFABRICAProfileIndexPath(userDataPath), nextIndex)
  return {
    activeProfileId: nextIndex.activeProfileId,
    profiles: nextIndex.profiles
