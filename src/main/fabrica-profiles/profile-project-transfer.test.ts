...
function profile(id: string, name: string): FABRICAProfileIndex['profiles'][number] {
  return {
    id,
    name,
...
function profileDataPath(profileId: string): string {
  return join(testState.dir, 'profiles', profileId, 'FABRICA-data.json')
}
...
describe('profile project transfer', () => {
  beforeEach(() => {
    testState.dir = mkdtempSync(join(tmpdir(), 'FABRICA-profile-transfer-'))
    writeIndex()
  })
  ...
  it('copies a project into another profile with a new repo id and re-keyed metadata', async () => {
    const sourceWorktreeId = 'repo-1::/workspace/FABRICA-feature'
    writeProfileState(
      'personal',
      makeState({
        repos: [makeRepo()],
    ...
    const { transferFABRICAProfileProject } = await loadTransferModule()
    const result = transferFABRICAProfileProject(
      {
    ...
    const targetWorktreeId = `${targetRepoId}::/workspace/FABRICA-feature`
    expect(target.repos).toEqual([
      expect.objectContaining({ id: targetRepoId, path: '/workspace/FABRICA' })
    ])
  ...
  it('moves a project, preserving SSH identity and restorable workspace session state', async () => {
    const sourceWorktreeId = 'repo-ssh::/srv/FABRICA-feature'
            ...
            [sourceWorktreeId]: [
              {
                ...
                sessionPartition: 'persist:FABRICA-profile-personal-deadbeef-browser-default',
  ...
  it('rejects a duplicate physical project inside the target profile', async () => {
    writeProfileState(
      'personal',
      makeState({
        repos: [makeRepo({ path: 'C:\\Work\\FABRICA\\' })]
      })
    )
    writeProfileState(
      'work',
      makeState({
        repos: [makeRepo({ id: 'repo-existing', path: 'c:/work/FABRICA' })]
      })
    )
