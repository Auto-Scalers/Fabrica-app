...
export class FABRICACloudRequestError extends Error {
...
function assertString(value: unknown, field: string): string {
  if (typeof value !== 'string') {
    throw new Error(`invalid_FABRICA_cloud_${field}`)
  }
...
function normalizeCapabilities(value: unknown): FABRICACloudCapabilities {
...
function normalizeOrganizations(value: unknown): FABRICACloudOrgSummary[] | undefined {
...
function normalizeCloudSummary(value: unknown): FABRICAProfileCloudSummary {
  if (!value || typeof value !== 'object') {
    throw new Error('invalid_FABRICA_cloud_profile')
  }
  const record = value as Record<string, unknown>
  return {
...
function normalizeSessionResponse(value: unknown): FABRICACloudSessionExchangeResponse {
  if (!value || typeof value !== 'object') {
    throw new Error('invalid_FABRICA_cloud_session')
  }
  const record = value as Record<string, unknown>
  return {
...
async function postJson<T>(url: string, body: unknown, accessToken?: string): Promise<T> {
  ...
  if (!response.ok) {
    ...
    throw new FABRICACloudRequestError(response.status)
  }
  return (await response.json()) as T
}

export async function exchangeFABRICACloudAuthCode(
...
export async function refreshFABRICACloudSession(
  config: FABRICACloudAuthConfig,
  session: FABRICACloudSession
): Promise<FABRICACloudSessionExchangeResponse> {
  return normalizeSessionResponse(
...
export async function createFABRICACloudProfile(
  config: FABRICACloudAuthConfig,
  session: FABRICACloudSession,
...
export async function selectFABRICACloudOrg(
  config: FABRICACloudAuthConfig,
  session: FABRICACloudSession,
  orgId: string
...
export async function revokeFABRICACloudSession(
  config: FABRICACloudAuthConfig,
  session: FABRICACloudSession
): Promise<void> {
