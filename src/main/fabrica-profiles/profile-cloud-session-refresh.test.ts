...
vi.mock('./profile-cloud-session-store', () => ({
  readFABRICACloudSession: readMock,
  saveFABRICACloudSessionIfCurrent: saveIfCurrentMock,
  clearFABRICACloudSession: clearMock
}))
...
vi.mock('./profile-cloud-client', async (importOriginal) => {
  ...
  return { ...original, refreshFABRICACloudSession: refreshMock }
})

vi.mock('./profile-cloud-index', () => ({ linkFABRICAProfileToCloud: linkMock }))

import { readFreshFABRICACloudSession } from './profile-cloud-session-refresh'

const config = {} as FABRICACloudAuthConfig
const active = {
  profile: {
    id: 'profile-1',
    cloud: {
...
describe('profile cloud session refresh', () => {
  ...
  it('does not publish a refresh whose persistent mutation snapshot became stale', async () => {
    ...
    const refreshing = readFreshFABRICACloudSession(config, active, '/data')
  ...
  it('single-flights concurrent rotating refresh-token use per profile and store', async () => {
    ...
    const first = readFreshFABRICACloudSession(config, active, '/data')
    const second = readFreshFABRICACloudSession(config, active, '/data')
