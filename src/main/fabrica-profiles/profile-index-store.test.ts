...
import {
  DEFAULT_LOCAL_FABRICA_PROFILE_ID,
  DEFAULT_LOCAL_FABRICA_PROFILE_NAME,
...
describe('profile index store', () => {
  beforeEach(() => {
    testState.dir = mkdtempSync(join(tmpdir(), 'FABRICA-profile-test-'))
  })
  ...
  it('creates the default local profile and copies legacy state without deleting it', async () => {
    ...
    writeFileSync(join(testState.dir, 'FABRICA-data.json'), JSON.stringify(legacyState), 'utf-8')
    writeFileSync(
      join(testState.dir, 'FABRICA-data.json.bak.0'),
    ...
    const { ensureActiveFABRICAProfile, getFABRICAProfileIndexPath } = await loadProfileIndexStore()
    const activeProfile = ensureActiveFABRICAProfile()

    expect(activeProfile.profile.id).toBe(DEFAULT_LOCAL_FABRICA_PROFILE_ID)
    expect(activeProfile.profile.name).toBe(DEFAULT_LOCAL_FABRICA_PROFILE_NAME)
    expect(activeProfile.dataFile).toBe(
      join(testState.dir, 'profiles', DEFAULT_LOCAL_FABRICA_PROFILE_ID, 'FABRICA-data.json')
    )
    ...
    expect(
      readJson(
        join(testState.dir, 'profiles', DEFAULT_LOCAL_FABRICA_PROFILE_ID, 'browser-session-meta.json')
      )
    ).toEqual(legacyBrowserSessionMeta)
    expect(existsSync(join(testState.dir, 'FABRICA-data.json'))).toBe(true)

    expect(readJson(getFABRICAProfileIndexPath())).toMatchObject({
      schemaVersion: FABRICA_PROFILE_INDEX_SCHEMA_VERSION,
      activeProfileId: DEFAULT_LOCAL_FABRICA_PROFILE_ID,
      profiles: [expect.objectContaining({ id: DEFAULT_LOCAL_FABRICA_PROFILE_ID, kind: 'local' })]
    })
  })
  ...
  it('uses an existing active profile data file without overwriting it from legacy state', async () => {
    ...
    writeFileSync(join(profileDirectory, 'FABRICA-data.json'), JSON.stringify(profileData), 'utf-8')
    writeFileSync(
      join(testState.dir, 'FABRICA-data.json'),
    ...
    const { ensureActiveFABRICAProfile } = await loadProfileIndexStore()
    const activeProfile = ensureActiveFABRICAProfile()

    expect(activeProfile.profile.id).toBe(profileId)
    expect(activeProfile.dataFile).toBe(join(profileDirectory, 'FABRICA-data.json'))
  ...
  it('creates an empty local profile without copying legacy state into it', async () => {
    ...
    const { createLocalFABRICAProfile, getFABRICAProfileDataFile, getFABRICAProfileListState } =
      await loadProfileIndexStore()
    const created = createLocalFABRICAProfile({ name: ' Work ' })

    expect(created.profile.name).toBe('Work')
    expect(created.profile.id).toMatch(/^local-/)
    expect(created.activeProfileId).toBe(DEFAULT_LOCAL_FABRICA_PROFILE_ID)
    expect(created.profiles.map((profile) => profile.id)).toContain(created.profile.id)
    expect(existsSync(getFABRICAProfileDataFile(created.profile.id))).toBe(false)
    expect(getFABRICAProfileListState().profiles.map((profile) => profile.id)).toContain(
      created.profile.id
    )
  })

  it('switches the active profile and updates last-opened metadata', async () => {
    const { createLocalFABRICAProfile, setActiveFABRICAProfile } = await loadProfileIndexStore()
    const created = createLocalFABRICAProfile({ name: 'Work' })

    const switched = setActiveFABRICAProfile(created.profile.id)
  ...
  it('rejects switching to an unknown profile', async () => {
    const { setActiveFABRICAProfile } = await loadProfileIndexStore()

    expect(() => setActiveFABRICAProfile('missing-profile')).toThrow('unknown_FABRICA_profile')
  })

  it('recovers a corrupted profile index from the backup copy', async () => {
    const store = await loadProfileIndexStore()
    store.ensureActiveFABRICAProfile()
    const created = store.createLocalFABRICAProfile({ name: 'Work' })
    // Trigger one more write so the backup captures the two-profile index.
    store.setActiveFABRICAProfile(created.profile.id)

    const indexPath = store.getFABRICAProfileIndexPath()
    expect(existsSync(`${indexPath}.bak`)).toBe(true)
    writeFileSync(indexPath, '{ not json', 'utf-8')

    const recovered = store.getFABRICAProfileListState()
  ...
  it('rejects profile ids that are not safe path segments', async () => {
    ...
    const state = store.ensureActiveFABRICAProfile()
    expect(state.profile.id).toBe(DEFAULT_LOCAL_FABRICA_PROFILE_ID)
  })
})
