...
import { FABRICACloudRequestError, refreshFABRICACloudSession } from './profile-cloud-client'
...
export type FreshCloudSessionResult =
  | { status: 'found'; session: FABRICACloudSession }
...
function shouldRefreshCloudSession(session: FABRICACloudSession, now = Date.now()): boolean {
  return session.expiresAt <= now + CLOUD_SESSION_REFRESH_SKEW_MS
}

export function isFABRICACloudAuthFailure(error: unknown): boolean {
  return (
    error instanceof FABRICACloudRequestError && (error.statusCode === 401 || error.statusCode === 403)
  )
}

const inflightCloudSessionRefreshes = new Map<string, Promise<FABRICACloudSession>>()
...
function clearCloudSessionIfUnchanged(
  profileId: string,
  userDataPath: string,
  failed: FABRICACloudSession,
  active: ActiveFABRICAProfileState
): void {
  const current = readFABRICACloudSession(profileId, userDataPath)
  ...
  clearFABRICACloudSession(profileId, userDataPath)
}
...
async function refreshStoredCloudSession(
  ...
  const task = (async () => {
    const current = readFABRICACloudSession(active.profile.id, userDataPath)
    ...
    const refreshed = await refreshFABRICACloudSession(config, session)
    ...
    if (
      saveFABRICACloudSessionIfCurrent(active.profile.id, userDataPath, nextSession, snapshot) === null
    ) {
      throw new StaleCloudSessionMutationError()
    }
    linkFABRICAProfileToCloud(active.profile.id, refreshed.cloud, userDataPath)
    return nextSession
  })()
...
export async function readFreshFABRICACloudSession(
  config: FABRICACloudAuthConfig,
  active: ActiveFABRICAProfileState,
  userDataPath: string
  ...
  try {
    ...
    if (isFABRICACloudAuthFailure(error)) {
...
export async function forceRefreshFABRICACloudSession(
  config: FABRICACloudAuthConfig,
  active: ActiveFABRICAProfileState,
  userDataPath: string,
  session: FABRICACloudSession
...
export async function runWithFreshFABRICACloudSession<T>(
  config: FABRICACloudAuthConfig,
  active: ActiveFABRICAProfileState,
  userDataPath: string,
  operation: (session: FABRICACloudSession) => Promise<T>
): Promise<CloudSessionOperationResult<T>> {
  const session = await readFreshFABRICACloudSession(config, active, userDataPath)
  if (session.status !== 'found') {
    return { status: 'reconnect-required' }
  }
  try {
    ...
    const refreshed = await forceRefreshFABRICACloudSession(
      config,
      active,
      userDataPath,
      session.session
    ...
    try {
      ...
      if (retryError instanceof FABRICACloudRequestError && retryError.statusCode === 401) {
