...
const REMOTE_URLS_BY_REPO: Record<string, Record<string, string>> = {
  [FORK_PATH]: {
    origin: 'https://github.com/fsdwen/FABRICA.git',
    upstream: 'https://github.com/Auto-Scalers/Fabrica-app.git'
  },
  [NON_FORK_PATH]: {
    origin: 'https://github.com/Auto-Scalers/Fabrica-app.git'
  },
  [SSH_FORK_PATH]: {
    origin: 'git@github.com:fsdwen/FABRICA.git',
    upstream: 'git@github.com:Auto-Scalers/Fabrica-app.git'
  }
}
...
describe('issue #7331: fork PR owner/repo resolution', () => {
  it('getOwnerRepo prefers the upstream parent on a fork checkout', async () => {
    ...
    // getIssueOwnerRepo).
    expect(prRepo).toEqual({ owner: 'Auto-Scalers', repo: 'Fabrica-app' })
  })
  ...
  it('getOwnerRepoForRemote(origin) still resolves the fork itself', async () => {
    ...
    expect(origin).toEqual({ owner: 'fsdwen', repo: 'FABRICA' })
  })
  ...
  it('getRepoUpstream still resolves the fork parent offline (regression)', async () => {
    ...
    expect(upstream).toEqual({ owner: 'Auto-Scalers', repo: 'Fabrica-app', host: 'github.com' })
