...
const FABRICA_REPO = 'Auto-Scalers/Fabrica-app'
...
/**
 * Check if the authenticated user has starred the FABRICA repo.
 * Returns true if starred, false if not, null if unable to determine (gh unavailable).
 */
export async function checkFABRICAStarred(): Promise<boolean | null> {
  await acquire()
  try {
    const { stdout, stderr } = await execFileAsync(
      'gh',
      ['api', '--include', `user/starred/${FABRICA_REPO}`],
      { encoding: 'utf-8' }
    )
...
/**
 * Star the FABRICA repo for the authenticated user.
 */
export async function starFABRICA(): Promise<boolean> {
  await acquire()
  try {
    await execFileAsync('gh', ['api', '-X', 'PUT', `user/starred/${FABRICA_REPO}`], {
      encoding: 'utf-8'
    })
    return true
  } catch {
...
export async function createGitHubPullRequest(
  ...
  const tempDir = await mkdtemp(join(tmpdir(), 'FABRICA-pr-body-'))
  await acquire()
...
export async function getPRForBranchOutcome(
  ...
  try {
    ...
    // Why: a visible PR can be merged outside FABRICA; keep a caller-marked fallback fresh even when GitHub no longer reports it by branch (e.g. deleted heads).
