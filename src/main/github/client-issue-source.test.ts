...
describe('GitHub issue source split', () => {
  ...
  it('uses upstream for issues and origin for PRs in mixed recent results', async () => {
    getIssueOwnerRepoMock.mockResolvedValueOnce({ owner: 'Auto-Scalers', repo: 'Fabrica-app' })
    getOwnerRepoMock.mockResolvedValueOnce({ owner: 'fork', repo: 'Fabrica' })
    ghExecFileAsyncMock
        ...
        stdout: JSON.stringify([
          {
            ...
            state: 'open',
            html_url: 'https://github.com/Auto-Scalers/Fabrica-app/issues/923',
            labels: [],
        ...
        stdout: JSON.stringify([
          {
            ...
            title: 'Fork PR',
            state: 'open',
            html_url: 'https://github.com/fork/Fabrica/pull/42',
            labels: [],
    ...
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(1, issueSearchArgs('Auto-Scalers/Fabrica-app'), {
      cwd: '/repo-root'
    })
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(2, prListArgs('fork/Fabrica'), {
      cwd: '/repo-root'
    })
  })
  ...
  it('omits gh api cache args for no-cache recent work-item requests', async () => {
    ...
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      1,
      issueSearchArgs('Auto-Scalers/Fabrica-app', { noCache: true }),
      { cwd: '/repo-root' }
    )
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(2, prListArgs('fork/Fabrica'), {
      cwd: '/repo-root'
  ...
  it('lists SSH repo work items with explicit owner/repo and no local cwd', async () => {
    resolveIssueSourceMock.mockResolvedValueOnce({
      source: { owner: 'Auto-Scalers', repo: 'Fabrica-app' },
      fellBack: false
    })
    getOwnerRepoMock.mockResolvedValueOnce({ owner: 'fork', repo: 'Fabrica' })
    ghExecFileAsyncMock.mockResolvedValueOnce({ stdout: '[]' }).mockResolvedValueOnce({
    ...
    await listWorkItems('/home/jinwoo/FABRICA', 10, undefined, undefined, 'auto', 'openclaw-2')
    ...
    expect(getOwnerRepoMock).toHaveBeenCalledWith('/home/jinwoo/FABRICA', 'openclaw-2', {})
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(1, issueSearchArgs('Auto-Scalers/Fabrica-app'), {})
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(2, prListArgs('fork/Fabrica'), {})
  })
  ...
  it('uses upstream for issue-only queries and origin for PR-only queries', async () => {
    ...
    expect(decodedIssueSearchPath(0)).toContain('q=repo:Auto-Scalers/Fabrica-app is:issue')
    ...
    expect(ghExecFileAsyncMock).toHaveBeenCalledWith(
      expect.arrayContaining(['--repo', 'fork/Fabrica']),
      { cwd: '/repo-root' }
    )
  })
  ...
  it("uses upstream for recent PRs when preference='upstream'", async () => {
    ...
    getOwnerRepoMock.mockResolvedValueOnce({ owner: 'fork', repo: 'Fabrica' })
    mockUpstreamCandidate({ owner: 'Auto-Scalers', repo: 'Fabrica-app' })
    ghExecFileAsyncMock.mockResolvedValueOnce({ stdout: '[]' }).mockResolvedValueOnce({
      stdout: '[]'
    })
    ...
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(2, prListArgs('Auto-Scalers/Fabrica-app'), {
      cwd: '/repo-root'
    })
  })
  ...
  it("uses upstream for queried PRs when preference='upstream'", async () => {
    ...
    expect(ghExecFileAsyncMock).toHaveBeenCalledWith(
      expect.arrayContaining(['--repo', 'Auto-Scalers/Fabrica-app']),
      { cwd: '/repo-root' }
    )
  })
  ...
  it("uses upstream for PR counts when preference='upstream'", async () => {
    ...
    expect(ghExecFileAsyncMock).toHaveBeenCalledWith(
      [
        ...
        '--cache',
        '120s',
        `search/issues?q=${encodeURIComponent('repo:Auto-Scalers/Fabrica-app is:pull-request is:open')}&per_page=1`,
        '--jq',
        '.total_count'
      ],
      { cwd: '/repo-root' }
  ...
  it("falls back to origin for PRs when preference='upstream' and upstream is missing", async () => {
    resolveIssueSourceMock.mockResolvedValueOnce({
      source: { owner: 'fork', repo: 'Fabrica' },
      fellBack: true
    })
    getOwnerRepoMock.mockResolvedValueOnce({ owner: 'fork', repo: 'Fabrica' })
    ...
    expect(result.sources).toEqual({
      issues: { owner: 'fork', repo: 'Fabrica' },
      prs: { owner: 'fork', repo: 'Fabrica' },
      originCandidate: { owner: 'fork', repo: 'Fabrica' },
  ...
  it('counts default work items across upstream issues and origin PRs', async () => {
    ...
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      ...
      [
        ...
        '--cache',
        '120s',
        `search/issues?q=${encodeURIComponent('repo:Auto-Scalers/Fabrica-app is:issue is:open')}&per_page=1`,
        '--jq',
        '.total_count'
      ],
      { cwd: '/repo-root' }
    ...
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      ...
      [
        ...
        '--cache',
        '120s',
        `search/issues?q=${encodeURIComponent('repo:fork/Fabrica is:pull-request is:open')}&per_page=1`,
        '--jq',
        '.total_count'
      ],
      { cwd: '/repo-root' }
  ...
  it('probes the upstream repository for a typed fork PR before origin', async () => {
    const upstream = { owner: 'Auto-Scalers', repo: 'Fabrica-app', host: 'github.com' }
  ...
  it('pins typed PR metadata to explicit origin when upstream has the same number', async () => {
    ...
    expect(
      ghExecFileAsyncMock.mock.calls.some((call) =>
        (call[0] as string[]).some((arg) => arg.includes('upstream/FABRICA'))
      )
    ).toBe(false)
  ...
  it('does not probe a second PR repository after a non-not-found failure', async () => {
    resolvePRRepositoryCandidatesMock.mockResolvedValueOnce({
      candidates: [
        { owner: 'Auto-Scalers', repo: 'Fabrica-app', host: 'github.com' },
        { owner: 'fork', repo: 'Fabrica', host: 'github.com' }
      ],
      headRepo: { owner: 'fork', repo: 'Fabrica', host: 'github.com' }
    })
    ghExecFileAsyncMock
    ...
    expect(
      ghExecFileAsyncMock.mock.calls.some(([args]) =>
        (args as string[]).some((arg) => arg.includes('fork/Fabrica'))
      )
    ).toBe(false)
  })
  ...
  it('raw number lookup tries upstream issue before origin PR', async () => {
    ...
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      1,
      ['api', 'repos/Auto-Scalers/Fabrica-app/issues/42'],
      { cwd: '/repo-root' }
    )
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      2,
  ...
  it('surfaces a 403 from upstream issues through the listWorkItems envelope', async () => {
    ...
    expect(result.sources).toMatchObject({
      issues: { owner: 'Auto-Scalers', repo: 'Fabrica-app' },
      prs: { owner: 'fork', repo: 'Fabrica' }
    })
  ...
  describe('per-repo issue-source preference', () => {
    ...
    it("preference='auto' + no upstream ? queries origin", async () => {
      resolveIssueSourceMock.mockResolvedValueOnce({
        source: { owner: 'solo', repo: 'Fabrica' },
        fellBack: false
      })
      getOwnerRepoMock.mockResolvedValueOnce({ owner: 'solo', repo: 'Fabrica' })
      ghExecFileAsyncMock.mockResolvedValueOnce({ stdout: '[]' }).mockResolvedValueOnce({
        stdout: '[]'
      })
      ...
      expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(1, issueSearchArgs('solo/Fabrica'), {
        cwd: '/repo-root'
      })
    })
    ...
    it("preference='auto' + upstream exists ? PRs query upstream too", async () => {
      ...
      expect(result.sources).toEqual({
        issues: { owner: 'Auto-Scalers', repo: 'Fabrica-app' },
        prs: { owner: 'Auto-Scalers', repo: 'Fabrica-app' },
        originCandidate: { owner: 'fork', repo: 'Fabrica' },
        upstreamCandidate: { owner: 'Auto-Scalers', repo: 'Fabrica-app' }
      })
    })
    ...
    it('collapses the default count to one query when auto resolves both sides to upstream', async () => {
      ...
      expect(ghExecFileAsyncMock).toHaveBeenCalledWith(
        [
          ...
          '--cache',
          '120s',
          `search/issues?q=${encodeURIComponent('repo:Auto-Scalers/Fabrica-app is:open')}&per_page=1`,
          '--jq',
          '.total_count'
        ],
        { cwd: '/repo-root' }
    ...
    it("preference='upstream' + upstream exists ? queries upstream", async () => {
      ...
      expect(decodedIssueSearchPath(0)).toContain('q=repo:Auto-Scalers/Fabrica-app is:issue is:open')
      expect(result.issueSourceFellBack).toBeUndefined()
    })
    ...
    it("preference='upstream' + no upstream ? falls back to origin with fellBack=true", async () => {
      ...
      expect(decodedIssueSearchPath(0)).toContain('q=repo:solo/Fabrica is:issue is:open')
      expect(result.issueSourceFellBack).toBe(true)
    })
    ...
    it("preference='origin' + upstream exists ? queries origin (not upstream)", async () => {
      ...
      expect(decodedIssueSearchPath(0)).toContain('q=repo:fork/Fabrica is:issue is:open')
    })
