...
describe('GitHub PR local runtime routing', () => {
  ...
  it('routes PR details and mutations through the selected WSL distro', async () => {
    ...
    const prRepo = { owner: 'acme', repo: 'Fabrica' }
    ...
    ghExecFileAsyncMock.mockImplementation(async (args: string[]) => {
      const endpoint = args.find((arg) => arg.startsWith('repos/acme/Fabrica/')) ?? ''
  ...
  it('never falls through to the default gh host for an unresolved SSH repository', async () => {
    const legacyRepo = { owner: 'team', repo: 'Fabrica' }
  ...
  it('preserves a ported GHES host in SSH-backed review reads and mutations', async () => {
    ...
    ghExecFileAsyncMock.mockImplementation(async (args: string[]) => {
      const endpoint = args.find((arg) => arg.startsWith('repos/team/Fabrica/')) ?? ''
      const query = args.find((arg) => arg.startsWith('query=')) ?? ''
      ...
      if (args[0] === 'pr' && args[1] === 'view') {
        ...
        return {
          stdout: JSON.stringify({
            ...
            title: 'Enterprise PR',
            state: 'OPEN',
            url: 'https://github.acme-corp.com:8443/team/FABRICA/pull/7',
            labels: [],
            ...
            check_runs: [
              {
                ...
                status: 'completed',
                conclusion: 'failure',
                details_url: 'https://github.acme-corp.com:8443/team/FABRICA/actions/runs/77/job/88'
              }
            ]
          })
        }
    ...
    await expect(
      getWorkItemByOwnerRepo('/remote/repo', { owner: 'team', repo: 'Fabrica' }, 7, 'pr', 'ssh-1')
    ).resolves.toMatchObject({ number: 7, title: 'Enterprise PR' })
    await expect(
    ...
    expect(prViewCall?.[0]).toEqual(expect.arrayContaining(['--repo', 'team/Fabrica']))
    ...
    expect(
      prCalls.every(
        ([args]) => args.includes('--repo') && args[args.indexOf('--repo') + 1] === 'team/Fabrica'
      )
    ).toBe(true)
