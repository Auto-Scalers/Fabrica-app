...
describe('issue source operations', () => {
  ...
  it('gets a single issue from the issue owner/repo', async () => {
    ...
    expect(ghExecFileAsyncMock).toHaveBeenCalledWith(
      ['api', '--cache', '300s', 'repos/Auto-Scalers/Fabrica-app/issues/923'],
  ...
  it('routes local WSL issue operations through repo resolution and gh execution options', async () => {
    ...
    getIssueOwnerRepoMock.mockResolvedValue({ owner: 'Auto-Scalers', repo: 'Fabrica-app' })
    ...
    ghExecFileAsyncMock
      ...
      .mockResolvedValueOnce({
        stdout: JSON.stringify({
          number: 924,
          html_url: 'https://github.com/Auto-Scalers/Fabrica-app/issues/924'
        })
      })
      .mockResolvedValueOnce({ stdout: '' })
      .mockResolvedValueOnce({
        stdout: JSON.stringify({
          ...
          html_url: 'https://github.com/Auto-Scalers/Fabrica-app/issues/923#issuecomment-1'
        })
      })
  ...
  it('routes PR conversation comments to the supplied Enterprise host', async () => {
    ghExecFileAsyncMock.mockResolvedValueOnce({
      stdout: JSON.stringify({
        ...
        html_url: 'https://github.acme-corp.com/team/FABRICA/pull/7#issuecomment-9'
      })
    })
    ...
    expect(ghExecFileAsyncMock).toHaveBeenCalledWith(
      [
        ...
        '-X',
        'POST',
        'repos/team/Fabrica/issues/7/comments',
        '--raw-field',
        'body=Enterprise comment'
      ],
  ...
  it('lists issues from the issue owner/repo', async () => {
    ...
    expect(ghExecFileAsyncMock).toHaveBeenCalledWith(
      [
        ...
        '--cache',
        '120s',
        'repos/Auto-Scalers/Fabrica-app/issues?per_page=5&state=open&sort=updated&direction=desc'
      ],
      { cwd: '/repo-root', host: 'github.com' }
    )
  })
  ...
  it('creates issues in the issue owner/repo', async () => {
    ...
    await expect(createIssue('/repo-root', 'New issue', 'Body')).resolves.toEqual({
      ok: true,
      number: 924,
      url: 'https://github.com/Auto-Scalers/Fabrica-app/issues/924'
    })
    expect(ghExecFileAsyncMock).toHaveBeenCalledWith(
      [
        'api',
        '-X',
        'POST',
        'repos/Auto-Scalers/Fabrica-app/issues',
        '--raw-field',
        'title=New issue',
        '--raw-field',
        'body=Body'
  ...
  it('creates issues with labels and assignees', async () => {
    ...
    ghExecFileAsyncMock.mockResolvedValueOnce({
      stdout: JSON.stringify({
        number: 925,
        html_url: 'https://github.com/Auto-Scalers/Fabrica-app/issues/925'
      })
    })
    ...
    await expect(
      ...
      ok: true,
      number: 925,
      url: 'https://github.com/Auto-Scalers/Fabrica-app/issues/925'
    })
    expect(ghExecFileAsyncMock).toHaveBeenCalledWith(
      [
        'api',
  ...
  it('recovers issue 7704 oversized inline-image creation', async () => {
    ...
    ghExecFileAsyncMock
      ...
      .mockResolvedValueOnce({
        stdout: JSON.stringify({
          number: 926,
          html_url: 'https://github.com/Auto-Scalers/Fabrica-app/issues/926'
        })
      })
      .mockResolvedValueOnce({ stdout: '' })
    ...
    await expect(
      ...
      ok: true,
      number: 926,
      url: 'https://github.com/Auto-Scalers/Fabrica-app/issues/926'
    })
    ...
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      3,
      ['api', '-X', 'PATCH', 'repos/Auto-Scalers/Fabrica-app/issues/926', '--raw-field', `body=${body}`],
      { cwd: '/repo-root', host: 'github.com' }
    )
  })
  ...
  it('recognizes the oversized-body response from structured gh stderr', async () => {
    ...
    ghExecFileAsyncMock
      ...
      .mockResolvedValueOnce({
        stdout: JSON.stringify({
          number: 929,
          html_url: 'https://github.com/Auto-Scalers/Fabrica-app/issues/929'
        })
      })
      .mockResolvedValueOnce({ stdout: '' })
    ...
    await expect(createIssue('/repo-root', 'Image issue', 'data:image')).resolves.toEqual({
      ok: true,
      number: 929,
      url: 'https://github.com/Auto-Scalers/Fabrica-app/issues/929'
    })
  ...
  it('reports partial success when oversized-body patch fails', async () => {
    ...
    ghExecFileAsyncMock
      ...
      .mockResolvedValueOnce({
        stdout: JSON.stringify({
          number: 928,
          html_url: 'https://github.com/Auto-Scalers/Fabrica-app/issues/928'
        })
      })
    ...
    await expect(createIssue('/repo-root', 'Partial issue', 'data:image')).resolves.toEqual({
      ok: true,
      number: 928,
      url: 'https://github.com/Auto-Scalers/Fabrica-app/issues/928',
      bodySaveWarning:
        'Issue https://github.com/Auto-Scalers/Fabrica-app/issues/928 was created, but saving its body failed: HTTP 500: update failed'
    })
  })
  ...
  it('updates issue body through the REST issue endpoint', async () => {
    ...
    expect(ghExecFileAsyncMock).toHaveBeenCalledWith(
      ['api', '-X', 'PATCH', 'repos/Auto-Scalers/Fabrica-app/issues/924', '--raw-field', 'body=Updated body'],
      { cwd: '/repo-root', host: 'github.com' }
    )
  })
  ...
  it('closes issues with completed, not planned, and duplicate reasons', async () => {
    ...
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      1,
      ['issue', 'close', '924', '--repo', 'Auto-Scalers/Fabrica-app', '--reason', 'completed'],
      { cwd: '/repo-root', host: 'github.com' }
    )
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      2,
      ['issue', 'close', '925', '--repo', 'Auto-Scalers/Fabrica-app', '--reason', 'not planned'],
      { cwd: '/repo-root', host: 'github.com' }
    )
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      3,
      ['issue', 'close', '926', '--repo', 'Auto-Scalers/Fabrica-app', '--duplicate-of', '99'],
      { cwd: '/repo-root', host: 'github.com' }
    )
  })
  ...
  it('reopens issues through gh issue reopen', async () => {
    ...
    expect(ghExecFileAsyncMock).toHaveBeenCalledWith(
      ['issue', 'reopen', '924', '--repo', 'Auto-Scalers/Fabrica-app'],
      { cwd: '/repo-root', host: 'github.com' }
    )
  })
})
