...
describe('checkFABRICAStarred', () => {
  ...
  it('returns true only for an included successful GitHub response', async () => {
    ...
    await expect(checkFABRICAStarred()).resolves.toBe(true)

    expect(execFileAsyncMock).toHaveBeenCalledWith(
      'gh',
      ['api', '--include', 'user/starred/Auto-Scalers/Fabrica-app'],
      { encoding: 'utf-8' }
    )
  })
  ...
  it('returns false for GitHub 404 not starred responses', async () => {
    ...
    await expect(checkFABRICAStarred()).resolves.toBe(false)
  })
  ...
  it('returns null when gh exits successfully without response headers', async () => {
    ...
    await expect(checkFABRICAStarred()).resolves.toBe(null)
  })
})
...
describe('getPRForBranch', () => {
  ...
  it('resolves fork PRs from the upstream PR repo with the origin head owner', async () => {
    resolvePRRepositoryCandidatesMock.mockResolvedValueOnce({
      candidates: [
        ...
        { owner: 'fork', repo: 'Fabrica' }
      ],
      headRepo: { owner: 'fork', repo: 'Fabrica' }
    })
    ghExecFileAsyncMock.mockResolvedValueOnce({
      stdout: JSON.stringify([
        {
          ...
          title: 'Fork PR',
          state: 'open',
          html_url: 'https://github.com/Auto-Scalers/Fabrica-app/pull/1738',
          updated_at: '2026-03-28T00:00:00Z',
          draft: false,
    ...
    expect(ghExecFileAsyncMock).toHaveBeenCalledWith(
      ['api', 'repos/Auto-Scalers/Fabrica-app/pulls?head=fork%3Afeature%2Ftest&state=all&per_page=1'],
      { cwd: '/repo-root' }
    )
    expect(pr).toMatchObject({
      number: 1738,
      prRepo: { owner: 'Auto-Scalers', repo: 'Fabrica-app' },
      headRepo: { owner: 'fork', repo: 'Fabrica' }
    })
  })
  ...
  it('looks up a linked PR number across PR repo candidates', async () => {
    ...
    ghExecFileAsyncMock
      ...
      .mockResolvedValueOnce({
        stdout: JSON.stringify({
          ...
          state: 'OPEN',
          url: 'https://github.com/fork/FABRICA/pull/99',
          statusCheckRollup: [],
    ...
    expect(pr?.prRepo).toEqual({ owner: 'fork', repo: 'Fabrica' })
  })
    ...
    ghExecFileAsyncMock
        ...
        stdout: JSON.stringify([
          {
            ...
            html_url: 'https://github.com/Auto-Scalers/Fabrica-app/pull/5511',
      ...
      .mockResolvedValueOnce({
        stdout: JSON.stringify({
          ...
          title: 'Merged branch PR',
          state: 'MERGED',
          url: 'https://github.com/Auto-Scalers/Fabrica-app/pull/5511',
          statusCheckRollup: [],
  ...
  it('reports upstream error when fallback branch discovery fails transiently then retry misses', async () => {
    resolvePRRepositoryCandidatesMock.mockResolvedValueOnce({
      candidates: [{ owner: 'Auto-Scalers', repo: 'Fabrica-app' }],
      headRepo: null
    })
    ghExecFileAsyncMock
    ...
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      2,
      ['api', 'repos/Auto-Scalers/Fabrica-app/pulls?head=Auto-Scalers%3Afeature%2Ftest&state=all&per_page=1'],
      { cwd: '/repo-root' }
    )
  })
    ...
    ghExecFileAsyncMock
        ...
        stdout: JSON.stringify([
          {
            ...
            state: 'open',
            html_url: 'https://github.com/Auto-Scalers/Fabrica-app/pull/42',
            updated_at: '2026-03-28T00:00:00Z',
            draft: false,
            mergeable: true,
            base: { ref: 'main', sha: 'base-oid' },
      ...
      .mockResolvedValueOnce({
        stdout: JSON.stringify({
          ...
          state: 'OPEN',
          url: 'https://github.com/Auto-Scalers/Fabrica-app/pull/42',
          statusCheckRollup: [],
          updatedAt: '2026-03-28T00:00:00Z',
          isDraft: false,
          mergeable: 'MERGEABLE',
  ...
  it('does not carry a merged upstream branch head repo into a fallback PR number', async () => {
    resolvePRRepositoryCandidatesMock.mockResolvedValueOnce({
      candidates: [{ owner: 'Auto-Scalers', repo: 'Fabrica-app' }],
      headRepo: { owner: 'origin-owner', repo: 'Fabrica' }
    })
    getOwnerRepoForRemoteMock.mockResolvedValueOnce({ owner: 'fork-owner', repo: 'Fabrica' })
    gitExecFileAsyncMock.mockResolvedValueOnce({
  ...
  it('uses the tracked upstream remote owner for fork branch lookup', async () => {
    resolvePRRepositoryCandidatesMock.mockResolvedValueOnce({
      candidates: [
        { owner: 'Auto-Scalers', repo: 'Fabrica-app' },
        { owner: 'origin-owner', repo: 'Fabrica' }
      ],
      headRepo: { owner: 'origin-owner', repo: 'Fabrica' }
    })
    getOwnerRepoForRemoteMock.mockResolvedValueOnce({ owner: 'fork-owner', repo: 'Fabrica' })
    ghExecFileAsyncMock
        ...
        stdout: JSON.stringify([
          {
            ...
            state: 'open',
            html_url: 'https://github.com/Auto-Scalers/Fabrica-app/pull/78',
            updated_at: '2026-03-28T00:00:00Z',
            draft: false,
            mergeable: true,
            base: { ref: 'main', sha: 'base-oid' },
      ...
      .mockResolvedValueOnce({
        stdout: JSON.stringify({
          ...
          state: 'OPEN',
          url: 'https://github.com/Auto-Scalers/Fabrica-app/pull/78',
          statusCheckRollup: [],
          updatedAt: '2026-03-28T00:00:00Z',
          isDraft: false,
          mergeable: 'MERGEABLE',
    ...
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      ...
      [
        'api',
        'repos/Auto-Scalers/Fabrica-app/pulls?head=fork-owner%3Acontributor%2Foriginal&state=all&per_page=1'
      ],
      { cwd: '/repo-root' }
    )
    expect(pr).toMatchObject({
      ...
      title: 'Hydrated fork upstream branch PR',
      prRepo: { owner: 'Auto-Scalers', repo: 'Fabrica-app' },
      headRepo: { owner: 'fork-owner', repo: 'Fabrica' }
    })
  })
  ...
  it('uses the tracked upstream remote owner when the fork branch name matches locally', async () => {
    ...
    getOwnerRepoForRemoteMock.mockResolvedValueOnce({ owner: 'brennanb2025', repo: 'Fabrica' })
    ghExecFileAsyncMock
      .mockResolvedValueOnce({ stdout: JSON.stringify([]) })
      .mockResolvedValueOnce({ stdout: JSON.stringify([]) })
      .mockResolvedValueOnce({
        stdout: JSON.stringify([
          {
            ...
            state: 'open',
            html_url: 'https://github.com/Auto-Scalers/Fabrica-app/pull/6433',
      ...
      .mockResolvedValueOnce({
        stdout: JSON.stringify({
          ...
          title: 'Recover Windows worktree deletes from long paths',
          state: 'OPEN',
          url: 'https://github.com/Auto-Scalers/Fabrica-app/pull/6433',
          statusCheckRollup: [],
    ...
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      ...
      [
        'api',
        'repos/Auto-Scalers/Fabrica-app/pulls?head=brennanb2025%3Abrennanb2025%2Fworktree-remove-fix&state=all&per_page=1'
      ],
      { cwd: '/repo-root' }
    )
    expect(pr).toMatchObject({
      number: 6433,
      prRepo: { owner: 'Auto-Scalers', repo: 'Fabrica-app' },
      headRepo: { owner: 'brennanb2025', repo: 'Fabrica' }
    })
  })
  ...
  it('uses the same-name tracked upstream fork owner through the SSH git provider', async () => {
        ...
        stdout: JSON.stringify([
          {
            ...
            state: 'open',
            html_url: 'https://github.com/Auto-Scalers/Fabrica-app/pull/79',
            updated_at: '2026-03-28T00:00:00Z',
            draft: false,
            mergeable: true,
            base: { ref: 'main', sha: 'base-oid' },
    ...
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      3,
      ['api', 'repos/Auto-Scalers/Fabrica-app/pulls?head=fork-owner%3Acontributor%2Ffix&state=all&per_page=1'],
      {}
    )
    expect(pr).toMatchObject({
      number: 79,
  ...
  it('resolves fork PR push target using the origin URL protocol', async () => {
    getOwnerRepoMock.mockResolvedValueOnce({ owner: 'Auto-Scalers', repo: 'Fabrica-app' })
    getOwnerRepoForRemoteMock.mockResolvedValueOnce({ owner: 'Auto-Scalers', repo: 'Fabrica-app' })
    ghExecFileAsyncMock.mockResolvedValueOnce({
      stdout: JSON.stringify({
        head: {
          ref: 'prateek/fix-sidebar-agents-toggle',
          repo: {
            full_name: 'prateek/FABRICA',
            name: 'FABRICA',
            clone_url: 'https://github.com/prateek/FABRICA.git',
            ssh_url: 'git@github.com:prateek/FABRICA.git',
            owner: { login: 'prateek' }
          }
        }
      })
    })
    getRemoteUrlForRepoMock.mockResolvedValueOnce('git@github.com:Auto-Scalers/Fabrica-app.git')

    const target = await getPullRequestPushTarget('/repo-root', 1738)

    expect(ghExecFileAsyncMock).toHaveBeenCalledWith(['api', 'repos/Auto-Scalers/Fabrica-app/pulls/1738'], {
      cwd: '/repo-root'
    })
    expect(target).toEqual({
      pushTarget: {
        remoteName: 'pr-prateek-fabrica',
        branchName: 'prateek/fix-sidebar-agents-toggle',
        remoteUrl: 'git@github.com:prateek/FABRICA.git'
      }
    })
  })

  it('pins explicit origin push-target lookup when upstream has the same PR number', async () => {
    getOwnerRepoMock.mockResolvedValue({ owner: 'fork', repo: 'Fabrica' })
    resolvePRRepositoryCandidatesMock.mockResolvedValue({
      candidates: [
        { owner: 'upstream', repo: 'Fabrica' },
        { owner: 'fork', repo: 'Fabrica' }
      ],
      headRepo: { owner: 'fork', repo: 'Fabrica' }
    })
    ghExecFileAsyncMock.mockResolvedValueOnce({
      stdout: JSON.stringify({
        head: {
          ref: 'contributor/fix',
          repo: {
            full_name: 'contributor/FABRICA',
            name: 'FABRICA',
            clone_url: 'https://github.com/contributor/FABRICA.git',
            ssh_url: 'git@github.com:contributor/FABRICA.git',
            owner: { login: 'contributor' }
          }
        }
      })
    })
    getRemoteUrlForRepoMock.mockResolvedValueOnce('git@github.com:fork/FABRICA.git')

    await getPullRequestPushTarget('/repo-root', 1738, null, {}, 'origin')

    expect(resolvePRRepositoryCandidatesMock).not.toHaveBeenCalled()
    expect(ghExecFileAsyncMock).toHaveBeenCalledWith(['api', 'repos/fork/Fabrica/pulls/1738'], {
      cwd: '/repo-root',
      host: 'github.com'
    })
    expect(ghExecFileAsyncMock).not.toHaveBeenCalledWith(
      ['api', 'repos/upstream/Fabrica/pulls/1738'],
      expect.anything()
    )
  })
    ...
    ghExecFileAsyncMock.mockResolvedValueOnce({
      stdout: JSON.stringify({
        head: {
          ref: 'fix-sidebar',
          repo: {
            full_name: 'Auto-Scalers/Fabrica-app',
            name: 'FABRICA-APP',
            clone_url: 'https://github.com/Auto-Scalers/Fabrica-app.git',
            ssh_url: 'git@github.com:Auto-Scalers/Fabrica-app.git',
            owner: { login: 'Auto-Scalers' }
          }
        }
      })
    })
  ...
  it('keeps getRepoSlug origin-based on a fork checkout (#7331)', async () => {
    ...
    getOwnerRepoMock.mockResolvedValue({ owner: 'fsdwen', repo: 'Fabrica' })
  ...
  it('resolves a distinct upstream remote as the repo upstream', async () => {
    ...
    getOwnerRepoMock.mockResolvedValue({ owner: 'tmchow', repo: 'Fabrica' })
    getOwnerRepoForRemoteMock.mockImplementation(async (_repoPath: string, remoteName: string) =>
      remoteName === 'origin'
        ? { owner: 'tmchow', repo: 'Fabrica' }
        : { owner: 'Auto-Scalers', repo: 'Fabrica-app' }
    )
  ...
  it('falls back to the GitHub parent when no upstream remote is configured', async () => {
    getOwnerRepoForRemoteMock
      .mockResolvedValueOnce({ owner: 'tmchow', repo: 'Fabrica' })
      .mockResolvedValueOnce(null)
    ghExecFileAsyncMock.mockResolvedValueOnce({
      stdout: JSON.stringify({
        isFork: true,
        parent: { name: 'Fabrica', owner: { login: 'Auto-Scalers' } }
      })
    })
  ...
  it('routes GHES push-target probes through the Enterprise host', async () => {
    const ghes = { owner: 'team', repo: 'Fabrica', host: 'github.acme-corp.com' }
    resolvePRRepositoryCandidatesMock.mockResolvedValueOnce({
      candidates: [ghes],
      headRepo: ghes
    })
    ...
    ghExecFileAsyncMock.mockResolvedValueOnce({
      stdout: JSON.stringify({
        head: {
          ref: 'feature',
          repo: {
            full_name: 'team/FABRICA',
            name: 'FABRICA',
            clone_url: 'https://github.acme-corp.com/team/FABRICA.git',
            ssh_url: 'git@github.acme-corp.com:team/FABRICA.git',
            owner: { login: 'team' }
          }
        }
      })
    ...
    expect(ghExecFileAsyncMock).toHaveBeenCalledWith(
      ['api', 'repos/team/Fabrica/pulls/7'],
      expect.objectContaining({ host: 'github.acme-corp.com' })
    )
  })

  it('does not confuse same-slug PR repositories across GitHub hosts', async () => {
    const enterprise = { owner: 'team', repo: 'Fabrica', host: 'github.acme-corp.com' }
    const dotCom = { owner: 'team', repo: 'Fabrica', host: 'github.com' }
    resolvePRRepositoryCandidatesMock.mockResolvedValueOnce({
      candidates: [enterprise, dotCom],
      headRepo: dotCom
    })
    getOwnerRepoForRemoteMock.mockResolvedValueOnce(dotCom)
    getRemoteUrlForRepoMock.mockResolvedValueOnce('git@github.com:team/FABRICA.git')
    ghExecFileAsyncMock.mockResolvedValueOnce({
      stdout: JSON.stringify({
        head: {
          ref: 'feature',
    ...
    await expect(getPullRequestPushTarget('/repo-root', 7)).resolves.toEqual({
      pushTarget: {
        remoteName: 'pr-team-fabrica',
        branchName: 'feature',
        remoteUrl: 'git@github.acme-corp.com:team/FABRICA.git'
      }
    })
  })
  ...
  it('probes additional PR repo candidates when the first lookup is not found', async () => {
    ...
    getOwnerRepoForRemoteMock.mockResolvedValueOnce({ owner: 'fork', repo: 'Fabrica' })
    ghExecFileAsyncMock
      .mockRejectedValueOnce(new Error('HTTP 404: Not Found'))
      .mockResolvedValueOnce({
        stdout: JSON.stringify({
          head: {
            ref: 'feature/test',
            repo: {
              full_name: 'fork/FABRICA',
              name: 'FABRICA',
              clone_url: 'https://github.com/fork/FABRICA.git',
              ssh_url: 'git@github.com:fork/FABRICA.git',
              owner: { login: 'fork' }
            }
          }
        })
    ...
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(1, ['api', 'repos/fork/Fabrica/pulls/1849'], {
      cwd: '/repo-root'
    })
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      2,
      ['api', 'repos/Auto-Scalers/Fabrica-app/pulls/1849'],
      { cwd: '/repo-root' }
    )
  })

  it('probes the next PR work-item candidate after a permission denial', async () => {
    const upstream = { owner: 'upstream', repo: 'Fabrica', host: 'github.com' }
    const origin = { owner: 'fork', repo: 'Fabrica', host: 'github.com' }
    resolvePRRepositoryCandidatesMock.mockResolvedValueOnce({
    ...
    ghExecFileAsyncMock
      ...
      .mockResolvedValueOnce({
        stdout: JSON.stringify({
          ...
          title: 'Origin PR',
          state: 'OPEN',
          url: 'https://github.com/fork/FABRICA/pull/42',
          labels: [],
    ...
    expect(ghExecFileAsyncMock.mock.calls[0][0]).toEqual(
      expect.arrayContaining(['pr', 'view', '--repo', 'upstream/Fabrica'])
    )
    expect(ghExecFileAsyncMock.mock.calls[1][0]).toEqual(['api', 'repos/upstream/Fabrica/pulls/42'])
    expect(ghExecFileAsyncMock.mock.calls[2][0]).toEqual(
      expect.arrayContaining(['pr', 'view', '--repo', 'fork/Fabrica'])
    )
  })
...
describe('updatePRState', () => {
  ...
  it('reopens pull requests through the gh PR command', async () => {
    ...
    expect(ghExecFileAsyncMock).toHaveBeenCalledWith(
      ['pr', 'reopen', '3977', '--repo', 'Auto-Scalers/Fabrica-app'],
  ...
  it('closes pull requests through the gh PR command', async () => {
    ...
    expect(ghExecFileAsyncMock).toHaveBeenCalledWith(
      ['pr', 'close', '3977', '--repo', 'Auto-Scalers/Fabrica-app'],
      { cwd: '/repo-root', host: 'github.com' }
    )
  })
...
describe('GitHub GraphQL rate-limit guard', () => {
  ...
  it('uses explicit PR repo for comments when a fork PR is discovered', async () => {
        ...
        stdout: JSON.stringify([
          {
            ...
            body: 'top-level',
            created_at: '2026-04-01T00:00:00Z',
            html_url: 'https://github.com/Auto-Scalers/Fabrica-app/pull/7#issuecomment-10'
          }
        ])
      })
      .mockResolvedValueOnce({ stdout: '[]' })
    ...
    await getPRComments(
      '/repo-root',
      7,
      { prRepo: { owner: 'Auto-Scalers', repo: 'Fabrica-app', host: 'github.com' } },
      undefined
    )
    ...
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      1,
      ['api', '--cache', '60s', 'repos/Auto-Scalers/Fabrica-app/issues/7/comments?per_page=100'],
      { cwd: '/repo-root', host: 'github.com' }
    )
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      2,
      ['api', '--cache', '60s', 'repos/Auto-Scalers/Fabrica-app/pulls/7/reviews?per_page=100'],
      { cwd: '/repo-root', host: 'github.com' }
    )
  })
  ...
  it('hydrates GitHub-registered stack metadata for exact linked PRs', async () => {
    getOwnerRepoMock.mockResolvedValue({ owner: 'Auto-Scalers', repo: 'Fabrica-app', host: 'github.com' })
    ghExecFileAsyncMock
      .mockResolvedValueOnce({
        stdout: JSON.stringify({
          number: 202,
          title: 'Stack API',
          state: 'OPEN',
          url: 'https://github.com/Auto-Scalers/Fabrica-app/pull/202',
          statusCheckRollup: [],
      ...
      .mockResolvedValueOnce({
        stdout: JSON.stringify({
          ...
          title: 'Stack API',
          state: 'open',
          html_url: 'https://github.com/Auto-Scalers/Fabrica-app/pull/202',
                    ...
                    nodes: [
                      {
                        ...
                        pullRequest: {
                          number: 201,
                          title: 'Stack models',
                          url: 'https://github.com/Auto-Scalers/Fabrica-app/pull/201',
                          state: 'OPEN',
                          isDraft: false,
                          mergeable: 'MERGEABLE',
  ...
  it('uses async merge only for GitHub-registered stacks', async () => {
    ...
    const mergeCall = ghExecFileAsyncMock.mock.calls.find(([args]) =>
      args.includes('repos/Auto-Scalers/Fabrica-app/pulls/202/merge-async')
    )
    expect(mergeCall?.[0]).toEqual(
      expect.arrayContaining([
        'PUT',
        'repos/Auto-Scalers/Fabrica-app/pulls/202/merge-async',
  ...
  it('keeps legacy merge for unregistered dependent PR chains', async () => {
    ...
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      3,
      ['pr', 'merge', '202', '--squash', '--repo', 'Auto-Scalers/Fabrica-app'],
  ...
  it('uses explicit PR repo for merge and title mutations', async () => {
    ghExecFileAsyncMock
      ...
      .mockResolvedValueOnce({
        stdout: JSON.stringify({
          ...
          title: 'PR',
          state: 'OPEN',
          url: 'https://github.com/Auto-Scalers/Fabrica-app/pull/7',
          statusCheckRollup: [],
    ...
    expect(getOwnerRepoMock).not.toHaveBeenCalled()
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(1, ['api', 'repos/Auto-Scalers/Fabrica-app/pulls/7'], {
      cwd: '/repo-root',
      host: 'github.com'
    })
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
    ...
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      3,
      ['pr', 'merge', '7', '--squash', '--repo', 'Auto-Scalers/Fabrica-app'],
      expect.objectContaining({
        cwd: '/repo-root',
    ...
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      4,
      ['pr', 'edit', '7', '--title', 'New title', '--repo', 'Auto-Scalers/Fabrica-app'],
      { cwd: '/repo-root', host: 'github.com' }
    )
  })
  ...
  it('sets and disables PR auto-merge with explicit PR repos and SSH context', async () => {
    ...
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      2,
      ['pr', 'view', '7', '--json', 'id,headRefOid,baseRefName', '--repo', 'Auto-Scalers/Fabrica-app'],
      { host: 'github.com' }
    )
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      3,
    ...
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      4,
      ['pr', 'merge', '7', '--disable-auto', '--repo', 'Auto-Scalers/Fabrica-app'],
      expect.objectContaining({
        env: expect.objectContaining({ GH_PROMPT_DISABLED: '1' }),
        host: 'github.com'
      })
  ...
  it('uses the queue-aware gh merge path when the base branch has a merge queue', async () => {
    ...
    expect(ghExecFileAsyncMock).toHaveBeenNthCalledWith(
      4,
      ['pr', 'merge', '7', '--auto', '--squash', '--repo', 'Auto-Scalers/Fabrica-app'],
      expect.objectContaining({
        cwd: '/repo-root',
        env: expect.objectContaining({ GH_PROMPT_DISABLED: '1' }),
        host: 'github.com'
  ...
  it('detects merge queues once per base branch and blocks direct merges', async () => {
    ...
    await expect(
      mergePR('/repo-root', 7, 'squash', undefined, { owner: 'Auto-Scalers', repo: 'Fabrica-app' })
    ...
    expect(ghExecFileAsyncMock.mock.calls[2]?.[0]).toEqual(
      expect.arrayContaining(['-f', 'owner=Auto-Scalers', '-f', 'repo=Fabrica', '-f', 'branch=true'])
    )
    ...
    ghExecFileAsyncMock.mockImplementation(async (args) => {
      ...
      return {
        stdout: JSON.stringify({
          ...
          title: 'PR',
          state: 'OPEN',
          url: `https://github.com/Auto-Scalers/Fabrica-app/pull/${prViewCount}`,
          statusCheckRollup: [],
          updatedAt: '2026-04-01T00:00:00Z',
          isDraft: false,
          mergeable: 'MERGEABLE',
