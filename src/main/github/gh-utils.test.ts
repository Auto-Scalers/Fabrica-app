...
describe('github owner/repo resolution', () => {
  ...
  it('parses GitHub HTTPS and SSH remotes', () => {
    ...
    expect(parseGitHubOwnerRepo('git@github.com:Auto-Scalers/Fabrica-app.git')).toEqual({
      owner: 'Auto-Scalers',
      repo: 'Fabrica-app'
    })
    ...
    expect(parseGitHubOwnerRepo('ssh://git@github.com/Auto-Scalers/Fabrica-app.git')).toEqual({
      owner: 'Auto-Scalers',
      repo: 'Fabrica-app'
    })
    expect(parseGitHubOwnerRepo('ssh://git@ssh.github.com:443/Auto-Scalers/Fabrica-app.git')).toEqual({
      owner: 'Auto-Scalers',
      repo: 'Fabrica-app'
    })
    expect(parseGitHubOwnerRepo('git@example.com:Auto-Scalers/Fabrica-app.git')).toBeNull()
  })

  it('parses GitHub Enterprise host identity', () => {
    expect(parseGitHubRemoteIdentity('https://ghe.acme.internal/acme/FABRICA.git')).toEqual({
      host: 'ghe.acme.internal',
      owner: 'acme',
      repo: 'FABRICA'
    })
    expect(parseGitHubRemoteIdentity('git@ghe.acme.internal:acme/FABRICA.git')).toEqual({
      host: 'ghe.acme.internal',
      owner: 'acme',
      repo: 'FABRICA'
    })
    expect(parseGitHubOwnerRepo('https://ghe.acme.internal/acme/FABRICA.git')).toBeNull()
  })

  it('prefers upstream for PR owner/repo resolution (#7331)', async () => {
    gitExecFileAsyncMock.mockResolvedValueOnce({
      stdout: 'git@github.com:Auto-Scalers/Fabrica-app.git\n'
    })

    await expect(getOwnerRepo('/repo')).resolves.toEqual({ owner: 'Auto-Scalers', repo: 'Fabrica-app' })
  ...
  it('prefers upstream for issue owner/repo resolution', async () => {
    ...
    await expect(getIssueOwnerRepo('/repo')).resolves.toEqual({ owner: 'Auto-Scalers', repo: 'Fabrica-app' })
    expect(gitExecFileAsyncMock).toHaveBeenCalledWith(['remote', 'get-url', 'upstream'], {
      cwd: '/repo',
      timeout: 30_000
    })
  ...
  it('falls back to origin when upstream is missing or non-GitHub', async () => {
    gitExecFileAsyncMock
      .mockResolvedValueOnce({ stdout: 'git@example.com:Auto-Scalers/Fabrica-app.git\n' })
      .mockResolvedValueOnce({ stdout: 'git@github.com:fork/FABRICA.git\n' })

    await expect(getIssueOwnerRepo('/repo')).resolves.toEqual({ owner: 'fork', repo: 'FABRICA' })
  ...
  it('does not mix origin and upstream cache entries for the same repo path', async () => {
    gitExecFileAsyncMock
      .mockResolvedValueOnce({ stdout: 'git@github.com:fork/FABRICA.git\n' })
      .mockResolvedValueOnce({ stdout: 'git@github.com:Auto-Scalers/Fabrica-app.git\n' })
  ...
  it('resolves SSH repo remotes through the registered SSH git provider', async () => {
    const sshProvider = {
      exec: vi.fn(async (args: string[]) => {
        ...
        return { stdout: 'git@github.com:Auto-Scalers/Fabrica-app.git\n', stderr: '' }
      })
    }
    getSshGitProviderMock.mockReturnValue(sshProvider)

    await expect(getOwnerRepo('/home/user/FABRICA', 'openclaw-2')).resolves.toEqual({
      owner: 'Auto-Scalers',
      repo: 'Fabrica-app'
    })
  ...
  it('keeps local and SSH owner/repo cache entries separate for the same path', async () => {
    const sshProvider = {
      exec: vi.fn().mockResolvedValue({ stdout: 'git@github.com:remote/FABRICA.git\n', stderr: '' })
    }
    gitExecFileAsyncMock.mockResolvedValueOnce({ stdout: 'git@github.com:local/FABRICA.git\n' })
    getSshGitProviderMock.mockReturnValue(sshProvider)

    await expect(getOwnerRepo('/repo')).resolves.toEqual({ owner: 'local', repo: 'FABRICA' })
    await expect(getOwnerRepo('/repo', 'ssh-1')).resolves.toEqual({ owner: 'remote', repo: 'FABRICA' })
  })
      ...
      async (args: string[], options: { wslDistro?: string } = {}) => {
        ...
        return {
          stdout: options.wslDistro
            ? 'git@github.com:wsl/FABRICA.git\n'
            : 'git@github.com:host/FABRICA.git\n'
        }
      }
    )

    await expect(getOwnerRepo('/repo')).resolves.toEqual({ owner: 'host', repo: 'FABRICA' })
  ...
  it('prunes expired distinct owner/repo cache entries on later lookups', async () => {
    ...
    try {
      ...
      await expect(getOwnerRepo('/repo-a')).resolves.toEqual({ owner: 'Auto-Scalers', repo: 'Fabrica-app' })
  ...
  it('resolves PR candidates as upstream then origin and de-dupes matching slugs', async () => {
    gitExecFileAsyncMock
      .mockResolvedValueOnce({ stdout: 'git@github.com:Acme/FABRICA.git\n' })
      .mockResolvedValueOnce({ stdout: 'git@github.com:acme/FABRICA.git\n' })

    await expect(resolvePRRepositoryCandidates('/repo')).resolves.toEqual({
      candidates: [{ owner: 'Acme', repo: 'FABRICA' }],
      headRepo: { owner: 'acme', repo: 'FABRICA' }
    })
  })

  it('ignores non-GitHub upstream while keeping origin as the head repo', async () => {
    gitExecFileAsyncMock
      .mockResolvedValueOnce({ stdout: 'git@example.com:Acme/FABRICA.git\n' })
      .mockResolvedValueOnce({ stdout: 'git@github.com:fork/FABRICA.git\n' })

    await expect(resolvePRRepositoryCandidates('/repo')).resolves.toEqual({
      candidates: [{ owner: 'fork', repo: 'FABRICA' }],
  ...
  it('expires cached remote owner/repo entries after the TTL', async () => {
    ...
    try {
      gitExecFileAsyncMock
        .mockResolvedValueOnce({ stdout: 'git@github.com:old/FABRICA.git\n' })
        .mockResolvedValueOnce({ stdout: 'git@github.com:new/FABRICA.git\n' })
  ...
  it('keeps local missing-remote probes cached beyond the short positive TTL', async () => {
    const repoPath = await mkdtemp(join(tmpdir(), 'FABRICA-gh-utils-'))
...
describe('resolveIssueSource', () => {
  ...
  it("'auto' + no upstream ? origin, fellBack=false", async () => {
    gitExecFileAsyncMock
      .mockResolvedValueOnce({ stdout: 'git@example.com:Auto-Scalers/Fabrica-app.git\n' })
      .mockResolvedValueOnce({ stdout: 'git@github.com:solo/FABRICA.git\n' })
  ...
  it("'origin' + upstream exists ? origin (ignores upstream), fellBack=false", async () => {
    ...
    gitExecFileAsyncMock.mockResolvedValueOnce({
      stdout: 'git@github.com:fork/FABRICA.git\n'
    })
  ...
  it("'origin' + no upstream ? origin, fellBack=false", async () => {
    gitExecFileAsyncMock.mockResolvedValueOnce({
      stdout: 'git@github.com:solo/FABRICA.git\n'
    })
...
describe('gh error classification', () => {
  ...
  it('classifies "has disabled issues" stderr as issues_disabled', () => {
    const stderr =
      "Command failed: gh issue list --limit 36 --json number,title,state --repo brennanb2025/FABRICA --state open\nthe 'brennanb2025/FABRICA' repository has disabled issues"
