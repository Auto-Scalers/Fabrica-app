...
describe('registerAppMenu', () => {
  ...
  it('shows reload shortcuts as policy-routed menu hints', () => {
    ...
    const expectedForceReloadLabel = `Force Reload\t${isMac ? 'âŒ˜â‡§R' : 'Ctrl+Shift+R'}`
  ...
  it('routes Check for Updates modifier clicks to prerelease and perf checks', () => {
    ...
    // either way.
    const parentLabel = isMac ? 'FABRICA' : 'Help'
  ...
  it('shows the worktree palette shortcut as a display-only menu hint', () => {
    ...
    const viewSubmenu = getSubmenu(getTemplate(), 'View')
    const expectedLabel = `Open Worktree Palette\t${isMac ? 'âŒ˜J' : 'Ctrl+Shift+J'}`
  ...
  // Why: pin the platform on every case â€” CI runs this suite on Linux only, so an
  // unpinned test leaves the other platforms' branches entirely uncovered.
  it.each(['darwin', 'linux', 'win32'] as const)(
    'routes Edit > Paste through FABRICA coordinated paste ownership on %s',
    (platform) => {
  ...
  it.each(['darwin', 'linux', 'win32'] as const)(
    'routes Edit selection actions through the focused FABRICA window on %s',
    (platform) => {
      vi.spyOn(process, 'platform', 'get').mockReturnValue(platform)
      const send = vi.fn()
      getFocusedWindowMock.mockReturnValue({ webContents: { send } })
  ...
  it.runIf(!isMac)('puts Settings and Exit under File on Windows/Linux', () => {
    ...
    // Why: no redundant app-named "FABRICA" menu should exist on non-mac â€” the
    // app-menu contents (Settings, Exit, Check for Updates, About) have been
    // redistributed so users see them in File / Help instead.
    expect(template.find((item) => item.label === 'FABRICA')).toBeUndefined()

    const fileLabels = getSubmenu(template, 'File').map((item) => item.label)
    expect(fileLabels).not.toContain(`Export as PDF...\t${isMac ? 'âŒ˜â‡§E' : 'Ctrl+Shift+E'}`)
    expect(fileLabels[0]).toBe(`Settings\t${isMac ? 'âŒ˜,' : 'Ctrl+,'}`)
    expect(fileLabels).toEqual(
      expect.arrayContaining([`Settings\t${isMac ? 'âŒ˜,' : 'Ctrl+,'}`, 'Exit'])
    )
  ...
  it.runIf(isMac)('keeps the macOS app-named menu with Settings and quit roles', () => {
    ...
    const template = getTemplate()
    const appSubmenu = getSubmenu(template, 'FABRICA')
    const appLabels = appSubmenu.map((item) => item.label)
    expect(appLabels).toEqual(
      expect.arrayContaining(['Check for Updates...', `Settings\t${isMac ? 'âŒ˜,' : 'Ctrl+,'}`])
    )
    // Why: on macOS File should NOT duplicate Settings/Exit â€” those live in
  ...
  it('routes Getting Started with Fabrica through its callback', () => {
    const options = buildMenuOptions()
    registerAppMenu(options)

    const setupGuideItem = getSubmenu(getTemplate(), 'Help').find(
      (entry) => entry.label === 'Getting Started with Fabrica'
    )
  ...
  it('routes Feature tour through its callback', () => {
    ...
    const featureTourItem = getSubmenu(getTemplate(), 'Help').find(
      (entry) => entry.label === 'Explore Fabrica'
    )
  ...
  it('exposes an Appearance submenu under View with checkbox items reflecting state', () => {
    ...
    const mobileItem = appearanceSubmenu.find((item) => item.label === 'Show Fabrica Mobile Button')
  ...
  it('routes Appearance checkbox clicks through onToggleAppearance', () => {
    ...
    appearanceSubmenu
      ...
      ?.click?.({} as never, {} as never, {} as never)
    appearanceSubmenu
      .find((item) => item.label === 'Show Fabrica Mobile Button')
      ?.click?.({} as never, {} as never, {} as never)
    appearanceSubmenu
  ...
  it('routes sidebar toggle items through their callbacks', () => {
    ...
    const leftLabel = `Toggle Left Sidebar\t${isMac ? 'âŒ˜B' : 'Ctrl+B'}`
    const rightLabel = `Toggle Right Sidebar\t${isMac ? 'âŒ˜L' : 'Ctrl+L'}`
