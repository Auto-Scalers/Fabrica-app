...
beforeEach(() => {
  tmpHome = mkdtempSync(join(tmpdir(), 'FABRICA-codex-home-'))
  userDataDir = mkdtempSync(join(tmpdir(), 'FABRICA-codex-user-data-'))
...
describe('CodexHookService', () => {
  it('installs PermissionRequest with trust so Codex approval prompts reach FABRICA', () => {
  ...
  it('installs managed hooks + trust into a per-account self-contained home, not the shared mirror', () => {
    ...
    writeFileSync(join(perAccountHome, '.FABRICA-managed-home'), 'account-1\n', 'utf-8')
  ...
  it.skipIf(process.platform !== 'win32')(
    ...
    () => {
      const spaceHome = join(tmpdir(), 'FABRICA home with spaces')
  ...
  it.skipIf(process.platform !== 'win32')(
    ...
    () => {
      const metacharHome = join(tmpdir(), 'FABRICA %FABRICA_TEST% ^ home')
  ...
  it.skipIf(process.platform !== 'win32')(
    ...
    () => {
      ...
      const cmdSafe = /^[A-Za-z0-9_.:\\~-]+$/.test(join(tmpHome, '.FABRICA', 'agent-hooks'))
      if (cmdSafe) {
  ...
  it.skipIf(process.platform !== 'win32')(
    ...
    async () => {
      ...
      const scriptPath = join(homedir(), '.FABRICA', 'agent-hooks', 'codex-hook.cmd')
      ...
      try {
        ...
        // Why: this suite may run inside an FABRICA-launched terminal whose env
        // already carries FABRICA_AGENT_HOOK_ENDPOINT/PORT/TOKEN. The managed
        // script sources that endpoint file, so leave it out or the hook posts
        // to the live FABRICA instead of this test's listener.
        ...
        const child = spawn('cmd.exe', ['/d', '/c', scriptPath], {
          env: {
            ...cleanEnv,
            FABRICA_AGENT_HOOK_PORT: String(port),
            FABRICA_AGENT_HOOK_TOKEN: 'tok123',
            FABRICA_PANE_KEY: '42:leaf-abc',
            FABRICA_TAB_ID: '42',
            FABRICA_WORKTREE_ID: 'C:\\work trees\\my repo & co',
        ...
        expect(received.headers['x-fabrica-agent-hook-token']).toBe('tok123')
  ...
  it('keeps hooks isolated by FABRICA userData instead of mutating system ~/.codex', () => {
    const systemCodexHome = join(tmpHome, '.codex')
    ...
    const devUserDataDir = mkdtempSync(join(tmpdir(), 'FABRICA-dev-codex-user-data-'))
    const prodUserDataDir = mkdtempSync(join(tmpdir(), 'FABRICA-prod-codex-user-data-'))
    try {
      getPathMock.mockImplementation((name: string) => {
        if (name === 'userData') {
      ...
      process.env.FABRICA_USER_DATA_PATH = devUserDataDir
      expect(new CodexHookService().install().state).toBe('installed')
      ...
      process.env.FABRICA_USER_DATA_PATH = prodUserDataDir
      expect(new CodexHookService().install().state).toBe('installed')
  ...
  it('refreshes runtime user hooks without installing FABRICA-managed hooks', () => {
    const systemCodexHome = join(tmpHome, '.codex')
    const systemHooksPath = join(systemCodexHome, 'hooks.json')
    mkdirSync(systemCodexHome, { recursive: true })
    writeFileSync(
  ...
  it('removes legacy FABRICA-managed hooks from system ~/.codex during install', () => {
    const systemCodexHome = join(tmpHome, '.codex')
    const systemHooksPath = join(systemCodexHome, 'hooks.json')
  ...
  it('removes very large legacy FABRICA-managed hook lists from system ~/.codex', () => {
    const systemCodexHome = join(tmpHome, '.codex')
    const systemHooksPath = join(systemCodexHome, 'hooks.json')
    const legacyScriptPath = join(
      tmpHome,
  ...
  it('removes the legacy FABRICA Codex profile file when it only contains managed hooks', () => {
    const systemCodexHome = join(tmpHome, '.codex')
    const profilePath = join(systemCodexHome, 'FABRICA-agent-status.config.toml')
    mkdirSync(systemCodexHome, { recursive: true })
    writeFileSync(
      profilePath,
      [
        '# BEGIN FABRICA AGENT STATUS HOOKS',
        '[[hooks.PermissionRequest]]',
        '[[hooks.PermissionRequest.hooks]]',
        'type = "command"',
        'command = "codex-hook"',
        '# END FABRICA AGENT STATUS HOOKS',
        ''
      ].join('\n'),
      'utf-8'
    )
  ...
  it('removes only the legacy FABRICA block from a user-edited Codex profile file', () => {
    const systemCodexHome = join(tmpHome, '.codex')
    const profilePath = join(systemCodexHome, 'FABRICA-agent-status.config.toml')
    mkdirSync(systemCodexHome, { recursive: true })
    writeFileSync(
    ...
    expect(profileConfig).not.toContain('FABRICA AGENT STATUS HOOKS')
  ...
  it('cleans duplicate Codex hook representations while keeping status hooks in runtime CODEX_HOME', () => {
    ...
    const legacyProfilePath = join(systemCodexHome, 'FABRICA-agent-status.config.toml')
    const legacyScriptPath = join(
      tmpHome,
      '.FABRICA',
      'agent-hooks',
  ...
  it('removes managed trust entries when userData resolves through a symlink', () => {
    ...
    process.env.FABRICA_USER_DATA_PATH = linkedUserDataDir
