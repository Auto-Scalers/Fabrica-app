...
function getProvenancePath(runtimeHomePath: string): string {
  return join(runtimeHomePath, '.fabrica-hook-trust-provenance.json')
}
...
/**
 * Records the runtime config.toml trust state FABRICA leaves behind after an
 * install/refresh, so the next launch can tell "entry FABRICA wrote" apart from
...
/**
 * Promotes hook approvals the user made inside FABRICA-launched Codex (written
...
function promoteCodexRuntimeHookApprovalsToSystemUnsafe(runtimeHomePath: string): void {
  ...
  // Why: without a snapshot of what FABRICA last wrote (first launch after
  ...
  // in-FABRICA approval. Promoting would resurrect trust the user revoked in
  ...
  for (const [key, state] of runtimeTrust) {
    ...
    if (
      ...
      // FABRICA wrote this entry and nothing touched it since — not an approval.
      continue
    }
    ...
    const hook = Array.isArray(definition?.hooks)
      ...
      : undefined
    // Why: never write trust for FABRICA's managed status hook into the user's
    // real config — mutating ~/.codex for FABRICA's own hooks is exactly what the
