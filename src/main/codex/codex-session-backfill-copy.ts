...
const ATOMIC_NO_REPLACE_UNSUPPORTED_CODE = 'FABRICA_ATOMIC_NO_REPLACE_UNSUPPORTED'

export async function copySessionFileWithoutOverwrite(
  sourcePath: string,
  targetPath: string
): Promise<void> {
  const temporaryPath = join(dirname(targetPath), `.fabrica-backfill-${randomUUID()}.tmp`)
