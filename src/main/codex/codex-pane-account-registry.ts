...
import { getFABRICAUserDataPath } from './codex-home-paths'
import type {
 ...
 * across app restarts, so without an on-disk record FABRICA forgets a pane is on
...
// Why: bounds a file that only shrinks when FABRICA observes a PTY exit; a crash
...
function getRegistryPath(): string {
  return join(getFABRICAUserDataPath(), 'codex-pane-accounts.json')
}
