...
const REAL_APPROVED_COMMAND = '/bin/sh "/tmp/FABRICA-case-b-mCmCe6/agent-hooks/codex-hook.sh"'
const REAL_APPROVED_HASH = 'sha256:2bcbb4380db9d78c83a9a661f0ac3e403b27982d85b918e56fcadefb004c1440'

let tmpDir: string
let configPath: string

beforeEach(() => {
  tmpDir = mkdtempSync(join(tmpdir(), 'FABRICA-codex-trust-test-'))
...
describe('computeTrustKey', () => {
  ...
  it('uses native Windows backslashes in the trust key Codex looks up', () => {
    ...
    const winPath = 'C:\\Users\\Rod\\AppData\\Roaming\\FABRICA\\hooks.json'
    ...
    expect(key.startsWith('C:\\Users\\Rod\\AppData\\Roaming\\FABRICA\\hooks.json:')).toBe(true)
  })
...
describe('upsertHookTrustEntries', () => {
  ...
  it('collapses duplicate blocks for the same hook key while preserving unrelated hook state', () => {
    const sourcePath = 'C:\\Users\\me\\AppData\\Roaming\\FABRICA\\codex-runtime-home\\home\\hooks.json'
    const key = `${sourcePath}:session_start:0:0`
    const unrelatedSourcePath =
      'C:\\Users\\me\\AppData\\Roaming\\FABRICA\\codex-runtime-home\\home\\hooks.json'
  ...
  it('finds and replaces a legacy forward-slash block when FABRICA upserts with native backslash key', () => {
    // Why: Codex 0.140 exposes Windows keys with either separator depending on cwd, so replace both.
    const backslashPath = 'C:\\Users\\Rod\\AppData\\Roaming\\FABRICA\\hooks.json'
  ...
  it('produces exactly one Windows separator pair after two consecutive upserts', () => {
    ...
    const entry: CodexTrustEntry = {
      sourcePath: 'C:\\Users\\Rod\\AppData\\Roaming\\FABRICA\\hooks.json',
      eventLabel: 'session_start',
      groupIndex: 0,
      handlerIndex: 0,
      command: 'echo session'
  ...
  it('falls back to TOML basic-string headers when a Windows path contains an apostrophe', () => {
    ...
    const entry: CodexTrustEntry = {
      sourcePath: "C:\\Users\\O'Connor\\AppData\\Roaming\\FABRICA\\hooks.json",
      eventLabel: 'session_start',
      groupIndex: 0,
      handlerIndex: 0,
      command: 'echo session'
    ...
    expect((written.match(/\[hooks\.state\."/g) ?? []).length).toBe(2)
    expect(written).toContain(
      `[hooks.state."C:\\\\Users\\\\O'Connor\\\\AppData\\\\Roaming\\\\FABRICA\\\\hooks.json:session_start:0:0"]`
    )
    expect(written).toContain(
      `[hooks.state."C:/Users/O'Connor/AppData/Roaming/FABRICA/hooks.json:session_start:0:0"]`
    )
  ...
  it('finds a Codex-written block with lowercased username when FABRICA key has mixed-case username', () => {
    // Why: realpathSync.native casing can differ from what Codex wrote, so case-fold to replace not duplicate.
    const lowercasePath = 'C:\\Users\\rod\\AppData\\Roaming\\FABRICA\\hooks.json'
    const mixedCasePath = 'C:\\Users\\Rod\\AppData\\Roaming\\FABRICA\\hooks.json'
...
describe('upsertProjectTrustLevel', () => {
  ...
  it('updates an existing legacy Windows forward-slash project block', () => {
    // Why: older FABRICA builds normalized to forward slashes; backslash fixes must not duplicate them.
    const original = [
...
describe('readHookTrustEntries', () => {
  ...
  it('recognizes and removes a first trust block after a leading BOM', () => {
    ...
    expect(readHookTrustEntriesFromContent(content).get(key)?.trustedHash).toBe('sha256:FABRICA')
  ...
  it('supports case-insensitive lookups for Windows hook trust keys read from config', () => {
    ...
    const rawKey = 'C:\\Users\\rod\\AppData\\Roaming\\FABRICA\\hooks.json:session_start:0:0'
    const lookupKey = 'C:/Users/Rod/AppData/Roaming/FABRICA/hooks.json:session_start:0:0'
    const original = [
      `[hooks.state.'${rawKey}']`,
      'enabled = true',
