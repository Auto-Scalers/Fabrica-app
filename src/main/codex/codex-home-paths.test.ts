...
beforeEach(() => {
  ...
  fakeHomeDir = mkdtempSync(join(tmpdir(), 'FABRICA-codex-resource-home-'))
  userDataDir = mkdtempSync(join(tmpdir(), 'FABRICA-codex-resource-user-data-'))
...
describe('syncSystemCodexResourcesIntoManagedHome', () => {
  it('uses FABRICA_USER_DATA_PATH when Electron cannot be required', async () => {
    vi.resetModules()
    vi.doMock('electron', () => {
      throw new Error('electron unavailable in packaged CLI')
    })
    const previousUserDataPath = process.env.FABRICA_USER_DATA_PATH
    process.env.FABRICA_USER_DATA_PATH = userDataDir
    try {
      const { getFABRICAManagedCodexHomePath: getCliSafeManagedPath } =
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      ...
      mkdirSync(join(managedHomePath, '.FABRICA-resource-copies', 'AGENTS.md.json'), {
        recursive: true
      })
  ...
  it('removes an unowned copy when recording copy ownership fails', () => {
    ...
    mkdirSync(managedHomePath, { recursive: true })
    writeFileSync(join(managedHomePath, '.FABRICA-resource-copies'), 'blocks marker directory\n')
