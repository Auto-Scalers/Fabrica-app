...
export function resolveFABRICAManagedCodexHomePath(): string {
  return join(getFABRICAUserDataPath(), 'codex-runtime-home', 'home')
}

export function getFABRICAManagedCodexHomePath(): string {
  const managedHomePath = resolveFABRICAManagedCodexHomePath()
  mkdirSync(managedHomePath, { recursive: true })
  return managedHomePath
}

export function getCodexSessionBackfillStateDirPath(): string {
  return join(getFABRICAUserDataPath(), 'codex-session-backfill')
}

export function getFABRICAUserDataPath(): string {
  if (process.env.FABRICA_USER_DATA_PATH) {
    return process.env.FABRICA_USER_DATA_PATH
  }
  ...
  if (process.platform === 'darwin') {
    return join(homedir(), 'Library', 'Application Support', 'Fabrica')
  }
  if (process.platform === 'win32') {
    return join(process.env.APPDATA ?? join(homedir(), 'AppData', 'Roaming'), 'Fabrica')
  }
  return join(process.env.XDG_CONFIG_HOME || join(homedir(), '.config'), 'Fabrica')
}
...
export function syncSystemCodexResourcesIntoManagedHome(managedHomePath?: string): void {
  const targetHome = managedHomePath ?? getFABRICAManagedCodexHomePath()
