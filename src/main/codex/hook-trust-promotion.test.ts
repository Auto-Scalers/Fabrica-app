...
describe('codex hook trust write-back promotion', () => {
  ...
  it('keeps an in-FABRICA approval of a user hook across launches and promotes it to ~/.codex', () => {
    writeSystemUserHook()
  ...
  it('never promotes trust for the FABRICA-managed status hook into ~/.codex', () => {
    writeSystemUserHook()
    const service = new CodexHookService()
    service.install()
  ...
  it('promotes an in-FABRICA disable of a mirrored user hook back to the system config', () => {
    writeSystemUserHook()
    const systemTomlPath = join(systemCodexDir(), 'config.toml')
    writeFileSync(systemTomlPath, upsertHookTrustEntriesInContent('', [systemUserStopEntry()]))
    ...
    // User disables the hook via /hooks inside FABRICA-launched Codex.
    const runtimeTomlPath = join(runtimeHomeDir(), 'config.toml')
  ...
  it('carries a Codex-written hash verbatim when it differs from the reproduced hash', () => {
    ...
    const driftedHash = 'sha256:codex-next-gen-hash-FABRICA-cannot-reproduce'
  ...
  it('does not touch ~/.codex on the first launch after upgrading (no provenance yet)', () => {
    ...
    const service = new CodexHookService()
    service.install()
    rmSync(join(runtimeHomeDir(), '.FABRICA-hook-trust-provenance.json'), { force: true })
