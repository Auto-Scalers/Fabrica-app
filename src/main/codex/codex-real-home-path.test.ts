...
describe('hasCustomCodexHomeOverride', () => {
  it('recognizes normalized aliases of FABRICA-owned CODEX_HOME', () => {
    const managedHome = `${process.cwd()}${sep}codex-runtime-home${sep}home`

    expect(
      hasCustomCodexHomeOverride({
        CODEX_HOME: `${managedHome}${sep}.`,
        FABRICA_CODEX_HOME: managedHome
      })
    ).toBe(false)
  })
  ...
  it('preserves a genuinely custom CODEX_HOME', () => {
    expect(
      hasCustomCodexHomeOverride({
        ...
        FABRICA_CODEX_HOME: `${process.cwd()}${sep}codex-runtime-home${sep}home`
      })
    ).toBe(true)
  })
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      const paneHome = mkdtempSync(join(tmpdir(), 'FABRICA-codex-pane-home-'))
