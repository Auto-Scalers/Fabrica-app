...
describe('retained Codex hook state', () => {
  it('repairs FABRICA hooks before a retained shell can launch Codex', () => {
    ...
    reconcileRetainedCodexHookHomes({
      ...
      hooksEnabled: true,
      runtimeHomePaths: ['/FABRICA/shared-home', '/FABRICA/account-home']
    })

    expect(install).toHaveBeenCalledTimes(2)
    expect(install).toHaveBeenNthCalledWith(1, '/FABRICA/shared-home')
    expect(install).toHaveBeenNthCalledWith(2, '/FABRICA/account-home')
    expect(refreshRuntimeUserHooks).not.toHaveBeenCalled()
  })

  it('removes only FABRICA hooks from retained homes when hooks are disabled', () => {
    const install = vi.fn(() => status('installed'))
    const refreshRuntimeUserHooks = vi.fn(() => status('not_installed'))
    ...
    reconcileRetainedCodexHookHomes({
      hookService: { install, refreshRuntimeUserHooks },
      hooksEnabled: false,
      runtimeHomePaths: ['/FABRICA/shared-home']
    })

    expect(refreshRuntimeUserHooks).toHaveBeenCalledWith('/FABRICA/shared-home')
