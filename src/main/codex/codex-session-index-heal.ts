...
// stamped `complete` on first app-server startup), so rollouts that FABRICA's
...
// Codex's DB-driven surfaces. FABRICA never writes Codex's sqlite schema itself.
...
function buildNativeHealInvocation(
  ...
  return {
    command: spawnCmd,
    args: spawnArgs,
    // Why: pin the real home explicitly — nested FABRICA launches can inherit a
