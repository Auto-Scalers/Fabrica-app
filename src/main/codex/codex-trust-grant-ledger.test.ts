...
beforeEach(() => {
  userDataDir = mkdtempSync(join(tmpdir(), 'FABRICA-trust-ledger-'))
...
describe('codex trust grant ledger', () => {
  ...
  it('treats Windows path-case variants as the same home', () => {
    const home = 'C:\\Users\\Alice\\AppData\\Roaming\\FABRICA\\codex-runtime-home\\home'
    writeCodexTrustGrantLedgerHome(home, { binary: null, entries: {} })
    expect(
      readCodexTrustGrantLedgerHome('c:/users/alice/appdata/roaming/FABRICA/codex-runtime-home/home')
    ).not.toBeNull()
  })
