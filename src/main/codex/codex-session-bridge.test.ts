...
function writeLegacyCopyMarker(relativePath: string, sourcePath: string, targetPath: string): void {
  ...
  const markerPath = join(getRuntimeCodexHomePath(), '.FABRICA-session-copies', `${relativePath}.json`)
...
beforeEach(() => {
  ...
  fakeHomeDir = mkdtempSync(join(tmpdir(), 'FABRICA-codex-session-home-'))
  userDataDir = mkdtempSync(join(tmpdir(), 'FABRICA-codex-session-user-data-'))
