...
import { getFABRICAManagedCodexHomePath, getSystemCodexHomePath } from './codex-home-paths'
...
// Why: Pre/PostToolUse feed the live in-flight-tool readout; PermissionRequest exits with no decision so Codex still shows its approval UI while FABRICA flips the pane to waiting.
const CODEX_EVENTS = [
  'SessionStart',
  'UserPromptSubmit',
  'PreToolUse',
...
function getConfigPath(runtimeHomePath: string = getFABRICAManagedCodexHomePath()): string {
...
function writeCodexHooksJson(configPath: string, hooks: Record<string, HookDefinition[]>): void {
  // Why: Codex rejects unknown top-level hooks.json fields, so plugin bookkeeping like `_managed` must not survive FABRICA's rewrite.
  writeHooksJson(configPath, { hooks })
}

function getCodexConfigTomlPath(runtimeHomePath: string = getFABRICAManagedCodexHomePath()): string {
...
const LEGACY_FABRICA_PROFILE_NAME = 'FABRICA-agent-status'
const LEGACY_FABRICA_PROFILE_BLOCK_START = '# BEGIN FABRICA AGENT STATUS HOOKS'
const LEGACY_FABRICA_PROFILE_BLOCK_END = '# END FABRICA AGENT STATUS HOOKS'
...
function getLegacyCodexProfileTomlPath(): string {
  return join(getSystemCodexHomePath(), `${LEGACY_FABRICA_PROFILE_NAME}.config.toml`)
}
...
function getTrustedSystemUserHookSignatures(
  ...
  try {
    ...
    // Why: a hand-broken system config.toml should only disable user-hook trust mirroring, not block FABRICA's managed runtime hooks.
...
function cleanupLegacySystemManagedHooks(): void {
  ...
  // Why: Codex hooks moved to FABRICA's managed CODEX_HOME; stale ~/.codex entries would keep external Codex sessions reporting into FABRICA.
  if (removedManagedHook) {
    // Why: this is the user's system hooks file, not FABRICA's runtime copy.
    // Remove only stale FABRICA hook entries and preserve other managers' metadata.
...
function stripLegacyManagedProfileBlock(content: string): string {
  const start = content.indexOf(LEGACY_FABRICA_PROFILE_BLOCK_START)
  if (start === -1) {
    return content
  }
  const endMarker = content.indexOf(LEGACY_FABRICA_PROFILE_BLOCK_END, start)
  const end = endMarker === -1 ? content.length : endMarker + LEGACY_FABRICA_PROFILE_BLOCK_END.length
...
function cleanupLegacyCodexProfileHooks(): void {
  ...
  // Why: #2778 wrote FABRICA hooks into a Codex profile file; runtime CODEX_HOME supersedes it, so remove only FABRICA's marked block.
...
function removeRuntimeManagedHookTrustEntries(configPath: string): void {
  try {
    removeCodexManagedHookTrustEntries({
      ...
      runtimeHomePath: getFABRICAManagedCodexHomePath(),
...
function removeStaleWslRuntimeManagedHookTrustEntries(
  ...
  removeStaleWslCodexManagedHookTrustEntries({
    ...
    buildManagedCommand: (linuxRuntimeHome) =>
      wrapReadablePosixHookCommand(`${linuxRuntimeHome}/.fabrica/agent-hooks/codex-hook.sh`),
    priorLedgerHomes
  })
}
...
function getManagedScript(target: 'local' | 'posix' = 'local'): string {
  ...
  return [
    '#!/bin/sh',
    ...buildPosixHookPayloadCapture(),
    // Why: sourcing refreshes PORT/TOKEN/ENV/VERSION from the current FABRICA so a surviving PTY keeps reporting after a restart (see claude/hook-service.ts).
    'load_hook_endpoint() {',
    '  endpoint_path="$1"',
    '  case "$endpoint_path" in',
    '    *.cmd)',
    // Why: Windows passes endpoint.cmd into WSL via WSLENV; parse only FABRICA's known assignments since cmd.exe `set` lines aren't shell syntax.
    ...
    '          "set FABRICA_AGENT_HOOK_PORT="*) FABRICA_AGENT_HOOK_PORT=${endpoint_line#*=} ;;',
    '          "set FABRICA_AGENT_HOOK_TOKEN="*) FABRICA_AGENT_HOOK_TOKEN=${endpoint_line#*=} ;;',
    '          "set FABRICA_AGENT_HOOK_ENV="*) FABRICA_AGENT_HOOK_ENV=${endpoint_line#*=} ;;',
    '          "set FABRICA_AGENT_HOOK_VERSION="*) FABRICA_AGENT_HOOK_VERSION=${endpoint_line#*=} ;;',
    '        esac',
    '      done < "$endpoint_path"',
    '      ;;',
    '    *)',
    ...
    '  load_hook_endpoint "$FABRICA_AGENT_HOOK_ENDPOINT"',
    'fi',
    ...
    '  printf \'%s\' "$payload" | "$curl_bin" -sS -X POST "http://127.0.0.1:${FABRICA_AGENT_HOOK_PORT}/hook/codex" \\',
...
function installManagedHooksIntoWslRuntime(
  ...
  try {
    ...
    if (grant.lane === 'fallback') {
      ...
      // here; only upsert FABRICA's entries instead of sweeping the whole source.
...
export class CodexHookService {
  ...
  getStatus(runtimeHomePath: string = getFABRICAManagedCodexHomePath()): AgentHookInstallStatus {
  ...
  private getStatusAfterInstall(
    ...
    runtimeHomePath: string = getFABRICAManagedCodexHomePath()
  ): AgentHookInstallStatus {
  ...
  install(runtimeHomePath: string = getFABRICAManagedCodexHomePath()): AgentHookInstallStatus {
    const configPath = getConfigPath(runtimeHomePath)
    const scriptPath = getManagedScriptPath()
    // Why: must run before this install rewrites hooks.json/config.toml —
    // approvals the user made inside FABRICA-launched Codex are keyed to the
    ...
    try {
      ...
      if (grant.lane === 'rpc') {
        ...
        // all old runtime [hooks.state.*] blocks would keep FABRICA Codex trusted.
  ...
  async installRemote(
    ...
    remoteHome: string,
    options?: {
      /** Explicit CODEX_HOME dir (flat layout). WSL sessions read FABRICA's managed runtime home, not ~/.codex, so the default location leaves them hookless. */
      codexHomeDir?: string
    ...
    try {
      ...
      // Why: SSH remotes use POSIX `.sh` paths even when FABRICA runs on Windows; never derive remote script syntax from local OS.
      await writeManagedScriptRemote(sftp, remoteScriptPath, getManagedScript('posix'))
      // Why: SSH edits the user's remote ~/.codex/hooks.json directly, so preserve non-FABRICA top-level metadata while replacing the hooks tree.
  ...
  refreshRuntimeUserHooks(
    ...
    const configPath = getConfigPath(runtimeHomePath)
    // Why: same as install() — capture in-FABRICA approvals before this refresh
    ...
    try {
      ...
      // Why: this path is used when FABRICA status hooks are disabled. The
      // runtime CODEX_HOME should keep user hooks, but not FABRICA-managed trust.
