...
beforeEach(() => {
  grantMock.mockReset()
  fakeHomeDir = mkdtempSync(join(tmpdir(), 'FABRICA-real-home-hooks-home-'))
  userDataDir = mkdtempSync(join(tmpdir(), 'FABRICA-real-home-hooks-user-data-'))
...
describe('ensureRealHomeCodexHookState (install)', () => {
  it('creates hooks.json with the FABRICA entry in every managed event for a fresh home', () => {
    grantSucceeds()
  ...
  it('keeps the managed lane for unknown top-level fields Codex cannot load', () => {
    ...
    expect(existsSync(join(userDataDir, 'codex-real-home-hooks', 'hooks.json.pre-FABRICA'))).toBe(
      false
    )
  })
  ...
  it('appends LAST and preserves user entries and trust positions', () => {
    ...
    expect(
      readFileSync(join(userDataDir, 'codex-real-home-hooks', 'hooks.json.pre-FABRICA'), 'utf-8')
    ).toBe(original)
  })
  ...
  it("keeps later user handler trust positions stable inside FABRICA's hook group", () => {
    grantSucceeds()
    ensureRealHomeCodexHookState({ hooksEnabled: true, userDataPath: userDataDir })
    const installed = readRealHooksJson()
...
describe('ensureRealHomeCodexHookState (opt-out sweep)', () => {
  it('rebases trust when a user appended hooks after FABRICA installed', () => {
    grantSucceeds()
  ...
  it('aborts without writing when hooks.json changes during the trust inspection', () => {
    ...
    rebaseInternals.setSessionRunnerSync((request) => {
      operations.push(request.operation)
      // A user save (or a second FABRICA instance) lands while the RPC runs.
  ...
  it('removes only FABRICA entries and reports the removed lane', () => {
    grantSucceeds()
    const userStop = {
      matcher: 'deploy-*',
  ...
  it('removes only hash-proven FABRICA trust from a mixed hook group', () => {
    const material = getCodexManagedHookInstallMaterial()
