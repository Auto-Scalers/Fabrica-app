...
const WSL_PATH_MISSING_OUTPUT = '__FABRICA_WSL_PATH_MISSING__'
...
export function createCodexWslRuntimeHookInstallPlan(
  ...
  return {
    ...
    scriptPath: pathWin32.join(runtimeHomePath, '.fabrica', 'agent-hooks', 'codex-hook.sh'),
    commandScriptPath: pathPosix.join(linuxRuntimeHome, '.fabrica', 'agent-hooks', 'codex-hook.sh'),
