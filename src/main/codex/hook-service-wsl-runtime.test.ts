...
function createTestPlan(): CodexWslRuntimeHookInstallPlan {
  const root = mkdtempSync(join(tmpdir(), 'FABRICA-codex-wsl-hooks-'))
  tempRoots.push(root)
  const linuxHome = '/home/alice/.local/share/FABRICA/codex-runtime-home/home'
  return {
    configPath: join(root, 'hooks.json'),
    tomlPath: join(root, 'config.toml'),
    scriptPath: join(root, '.fabrica', 'agent-hooks', 'codex-hook.sh'),
    commandScriptPath: `${linuxHome}/.fabrica/agent-hooks/codex-hook.sh`,
...
describe('Codex WSL runtime hook install', () => {
  it('plans WSL hook files with Linux command and trust paths', () => {
    ...
    expect(
      ...
      scriptPath: pathWin32.join(runtimeHome, '.fabrica', 'agent-hooks', 'codex-hook.sh'),
      commandScriptPath:
        '/home/alice/.local/share/FABRICA/codex-runtime-home/home/.fabrica/agent-hooks/codex-hook.sh',
      trustConfigPath: '/home/alice/.local/share/FABRICA/codex-runtime-home/home/hooks.json',
      wslDistro: 'Ubuntu',
      linuxRuntimeHome: '/home/alice/.local/share/FABRICA/codex-runtime-home/home'
    })
  })

  it('plans WSL hooks when the distro home is mounted on a Windows drive', () => {
    const runtimeHome = 'D:\\wsl-home\\.local\\share\\FABRICA\\codex-runtime-home\\home'
    ...
    expect(
      ...
      scriptPath: pathWin32.join(runtimeHome, '.fabrica', 'agent-hooks', 'codex-hook.sh'),
      commandScriptPath:
        '/mnt/d/wsl-home/.local/share/FABRICA/codex-runtime-home/home/.fabrica/agent-hooks/codex-hook.sh',
      trustConfigPath: '/mnt/d/wsl-home/.local/share/FABRICA/codex-runtime-home/home/hooks.json',
      wslDistro: 'Ubuntu',
      linuxRuntimeHome: '/mnt/d/wsl-home/.local/share/FABRICA/codex-runtime-home/home'
    })
  })

  it('uses WSL-canonical paths for hook commands and trust keys', () => {
    const runtimeHome =
      '\\\\wsl.localhost\\Ubuntu\\home\\alias\\.local\\share\\FABRICA\\codex-runtime-home\\home'
    const canonicalHome = '/home/alice/.local/share/FABRICA/codex-runtime-home/home'
    ...
    const plan = createCodexWslRuntimeHookInstallPlan(
      ...
      (distro, linuxPath) => {
        ...
        expect(linuxPath).toBe('/home/alias/.local/share/FABRICA/codex-runtime-home/home')
        return canonicalHome
      }
    )

    expect(plan?.commandScriptPath).toBe(`${canonicalHome}/.fabrica/agent-hooks/codex-hook.sh`)
  ...
  it('removes managed trust when the WSL canonical path changes', () => {
    ...
    const oldPlan = {
      ...plan,
      commandScriptPath: '/old/home/.fabrica/agent-hooks/codex-hook.sh',
    ...
    const newPlan = {
      ...plan,
      commandScriptPath: '/new/home/.fabrica/agent-hooks/codex-hook.sh',
  ...
  it('generates a POSIX hook that bridges WSL loopback failures through Windows curl', () => {
    ...
    expect(script).toContain('"set FABRICA_AGENT_HOOK_TOKEN="*)')
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      ...
      writeFileSync(
        endpointPath,
        [
          'set FABRICA_AGENT_HOOK_PORT=43210',
          'set FABRICA_AGENT_HOOK_TOKEN=fresh-token',
          'set FABRICA_AGENT_HOOK_ENV=development',
          'set FABRICA_AGENT_HOOK_VERSION=1',
          ''
        ].join('\r\n'),
        'utf-8'
      )
      writeFileSync(
        curlPath,
        '#!/bin/sh\nprintf \'%s\\n\' "$@" > "$FABRICA_TEST_CAPTURE"\ncat >> "$FABRICA_TEST_CAPTURE"\n',
        'utf-8'
      )
      ...
      const result = spawnSync('/bin/sh', [plan.scriptPath], {
        ...
        env: {
          ...
          FABRICA_AGENT_HOOK_ENDPOINT: endpointPath,
          FABRICA_AGENT_HOOK_PORT: '1',
          FABRICA_AGENT_HOOK_TOKEN: 'stale-token',
          FABRICA_PANE_KEY: 'pane-1',
          FABRICA_TEST_CAPTURE: capturePath
        }
      })
      ...
      expect(posted).toContain('X-FABRICA-Agent-Hook-Token: fresh-token')
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    () => {
      ...
      const posted = readFileSync(capturePath, 'utf-8')
      expect(posted).toContain('http://127.0.0.1:43210/hook/codex')
      expect(posted).toContain('X-FABRICA-Agent-Hook-Token: token')
    }
  )

  it('installs trusted WSL hooks and removes only FABRICA entries when disabled', () => {
    const plan = createTestPlan()
              ...
              hooks: [
                {
                  type: 'command',
                  command:
                    "if [ -x '/old/.fabrica/agent-hooks/codex-hook.sh' ]; then /bin/sh '/old/.fabrica/agent-hooks/codex-hook.sh'; fi"
                }
              ]
            }
          ]
...
describe('Codex WSL runtime hook install app-server grant lane', () => {
  ...
  beforeEach(() => {
    userDataDir = mkdtempSync(join(tmpdir(), 'FABRICA-wsl-grant-userdata-'))
    tempRoots.push(userDataDir)
    previousUserDataPath = process.env.FABRICA_USER_DATA_PATH
    process.env.FABRICA_USER_DATA_PATH = userDataDir
