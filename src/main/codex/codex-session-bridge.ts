...
export function syncSystemCodexSessionsIntoManagedHome(sourceCodexHomePath?: string): void {
  ...
  const managedSessionsRoot = join(getFABRICAManagedCodexHomePath(), 'sessions')
...
function replaceSymlinkSessionBridgeWithHardlink(
  ...
  try {
    ...
    replacementPath = `${targetPath}.fabrica-link-${process.pid}-${Date.now()}`
...
function getLegacySessionCopyMarkerPath(relativePath: string): string {
  return join(getFABRICAManagedCodexHomePath(), '.fabrica-session-copies', `${relativePath}.json`)
}
