...
export function syncSystemConfigIntoManagedCodexHome(
  ...
  // user made inside FABRICA-launched Codex (/model, /approvals) must be written
  ...
  // unpromoted runtime change as FABRICA-written would strand it forever.
  snapshotCodexRuntimeSettingsBaseline(
...
function mergeSystemCodexConfigIntoRuntime(runtimeConfig: string, systemConfig: string): string {
  ...
  // trust and project trust are written under FABRICA's managed CODEX_HOME and
