...
describe('canCleanupUnregisteredFABRICALeftoverDirectory', () => {
  const repo = { path: '/repos/main' }
  const ownedMeta = { FABRICACreatedAt: 1, FABRICACreationSource: 'runtime' as const }
  const baseArgs = {
    meta: ownedMeta,
    worktreePath: '/workspaces/FABRICA-owned',
    runtimeWorktreePath: '/workspaces/FABRICA-owned',
    repo,
  ...
  it('rejects unregistered existing targets that are files or symlinks', async () => {
    ...
    await expect(
      canCleanupUnregisteredFABRICALeftoverDirectory({
        ...baseArgs,
        statPath: makeStatPath(['/workspaces/FABRICA-owned']),
        isGitRepository
      })
    ).resolves.toBe(false)
    await expect(
      canCleanupUnregisteredFABRICALeftoverDirectory({
        ...baseArgs,
        statPath: async (path) => {
          if (path === '/workspaces/FABRICA-owned') {
            return { type: 'symlink' }
          }
          throw missingPath(path)
        },
  ...
  it('rejects unregistered leftover directories with a .git marker', async () => {
    ...
    await expect(
      canCleanupUnregisteredFABRICALeftoverDirectory({
        ...baseArgs,
        statPath: makeStatPath(['/workspaces/FABRICA-owned/.git'], ['/workspaces/FABRICA-owned']),
        isGitRepository
      })
    ).resolves.toBe(false)
  ...
  it('rejects no-marker cleanup when only the FABRICA path shape matches', async () => {
    const isGitRepository = vi.fn().mockResolvedValue(false)
  ...
  it('rejects unregistered leftover directories that still answer git status', async () => {
    ...
    expect(isGitRepository).toHaveBeenCalledWith('/workspaces/FABRICA-owned')
  })
    ...
    await expect(
      canCleanupUnregisteredFABRICALeftoverDirectory({
        ...
        registeredWorktrees: [
          ...
          makeGitWorktree('/workspaces/FABRICA-owned/child')
        ],
        statPath: makeStatPath([], ['/workspaces/FABRICA-owned']),
        isGitRepository: vi.fn().mockResolvedValue(false)
      })
    ).rejects.toThrow(
      'Refusing to delete worktree because it contains another registered worktree: /workspaces/FABRICA-owned/child'
    )
  })
  ...
  it('uses runtime paths for filesystem proof and original paths for nested worktree checks', async () => {
    const statPath = vi.fn(async (path: string) => {
      if (path === '/mnt/c/workspaces/FABRICA-owned') {
        return { type: 'directory' }
      }
      throw missingPath(path)
    })
    ...
    await expect(
      canCleanupUnregisteredFABRICALeftoverDirectory({
        ...baseArgs,
        worktreePath: 'C:\\workspaces\\FABRICA-owned',
        runtimeWorktreePath: '/mnt/c/workspaces/FABRICA-owned',
        repo: { path: 'C:\\repos\\main' },
        runtimeRepoPath: '/mnt/c/repos/main',
        registeredWorktrees: [
          makeGitWorktree('C:\\repos\\main', true),
          makeGitWorktree('C:\\workspaces\\FABRICA-owned-sibling')
        ],
        statPath,
        isGitRepository
      })
    ...
    expect(statPath).toHaveBeenCalledWith('/mnt/c/workspaces/FABRICA-owned')
    expect(statPath).toHaveBeenCalledWith('/mnt/c/workspaces/FABRICA-owned/.git')
    expect(statPath).not.toHaveBeenCalledWith('C:\\workspaces\\FABRICA-owned')
    expect(isGitRepository).toHaveBeenCalledWith('/mnt/c/workspaces/FABRICA-owned')
  })
  ...
  it('rejects translated-runtime cleanup when original path contains a registered child', async () => {
    await expect(
      canCleanupUnregisteredFABRICALeftoverDirectory({
        ...
        registeredWorktrees: [
          makeGitWorktree('C:\\repos\\main', true),
          makeGitWorktree('C:\\workspaces\\FABRICA-owned\\child')
        ],
        statPath: makeStatPath([], ['/mnt/c/workspaces/FABRICA-owned']),
        isGitRepository: vi.fn().mockResolvedValue(false)
      })
    ).rejects.toThrow(
      'Refusing to delete worktree because it contains another registered worktree: C:\\workspaces\\FABRICA-owned\\child'
    )
  })
})
