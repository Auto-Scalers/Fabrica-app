...
describe('skill discovery', () => {
  it('discovers home and repo SKILL.md packages with provider metadata', async () => {
    const root = await mkdtemp(join(tmpdir(), 'FABRICA-skills-'))
  ...
  it('discovers skill packages through symlinked skill directories', async () => {
    const root = await mkdtemp(join(tmpdir(), 'FABRICA-skills-'))
    const home = join(root, 'home')
    const realSkill = join(root, 'central-skills', 'FABRICA-cli')
    const linkedSkill = join(home, '.agents', 'skills', 'FABRICA-cli')
    await mkdir(realSkill, { recursive: true })
    await mkdir(join(home, '.agents', 'skills'), { recursive: true })
    await writeFile(join(realSkill, 'SKILL.md'), '# FABRICA CLI\n\nUse the FABRICA CLI.')
    ...
    const skill = result.skills.find((entry) => entry.name === 'FABRICA CLI')
  ...
  it('keeps home classification when cwd points at the same directory as home', async () => {
    const root = await mkdtemp(join(tmpdir(), 'FABRICA-skills-'))
    const home = join(root, 'home')
    const skillDir = join(home, '.agents', 'skills', 'FABRICA-cli')
    await mkdir(skillDir, { recursive: true })
    await writeFile(
      join(skillDir, 'SKILL.md'),
      ['---', 'name: FABRICA-cli', 'description: Use the FABRICA CLI.', '---', ''].join('\n')
    )
    ...
    expect(result.skills.filter((entry) => entry.name === 'FABRICA-cli')).toMatchObject([
      {
        sourceKind: 'home',
