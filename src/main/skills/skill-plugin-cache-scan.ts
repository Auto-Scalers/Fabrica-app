...
export async function scanKnownPluginSkillCandidates(
  ...
  function recordIssue(
    ...
    // date", so evicting one for display budget makes FABRICA report all-clear over a read
    ...
    // FABRICA's own bounds no longer raise attention, it would do so silently.
  ...
  async function visit(
    ...
    // silent — recording it would put FABRICA's own traversal rules in the user's dialog.
