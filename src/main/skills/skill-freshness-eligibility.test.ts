...
const globallyUpdatableNames = new Set(['computer-use', 'FABRICA-cli', 'orchestration'])
...
describe('skill freshness name-scoped update eligibility', () => {
  ...
  it.each([
    ...
    (status, topology) => {
      ...
      expect(
        eligible([
          placement('FABRICA-cli'),
          placement('FABRICA-cli', { id: `outlier-${status}-${topology}`, status, topology })
        ])
      ).toEqual(['FABRICA-cli'])
    }
  )
  ...
  it.each(['unrecognized', 'inaccessible', 'newer-known'] as const)(
    ...
    (status) => {
      ...
      expect(
        eligible([
          placement('FABRICA-cli', { id: 'blocked-canonical', status }),
          placement('FABRICA-cli', {
            id: 'FABRICA-cli-claude',
            rootId: 'home-claude',
            topology: 'provider-alias',
  ...
  it('still updates the canonical copy when a clean standalone duplicate exists', () => {
    ...
    expect(
      eligible([
        placement('FABRICA-cli'),
        placement('FABRICA-cli', {
          id: 'FABRICA-cli-gemini',
          rootId: 'home-gemini',
          unresolvedPath: '/home/.gemini/skills/FABRICA-cli',
          resolvedPath: '/home/.gemini/skills/FABRICA-cli',
  ...
  it('does not offer an official canonical copy missing from the updater lock (#10791)', () => {
    expect(eligibleSkillUpdateNames([placement('FABRICA-cli')], new Set())).toEqual([])
  })
