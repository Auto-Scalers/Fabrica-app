...
async function fixture() {
  const root = await mkdtemp(join(tmpdir(), 'FABRICA-skill-inventory-'))
  ...
  await Promise.all([
        ...
        `${JSON.stringify({
          ...
          skills: {
            'FABRICA-cli': {
              ...
              skillPath: 'skills/FABRICA-cli/SKILL.md',
              source: 'Auto-Scalers/Fabrica-app'
            }
          }
        })}\n`
      )
    ...
    writeFile(
      ...
      `${JSON.stringify({ schemaVersion: 1, skills: { 'FABRICA-cli': snapshots } }, null, 2)}\n`
    ),
    writeFile(
      ...
      `${JSON.stringify(
        {
          ...
          releases: [
            ...
            { appVersion: '2.0.0', skills: { 'FABRICA-cli': 2 } },
            { appVersion: '3.0.0', skills: { 'FABRICA-cli': 3 } }
          ]
        },
        null,
        2
  ...
  const writeSkill = async (rootPath: string, markdown: string): Promise<string> => {
    const directory = join(rootPath, 'FABRICA-cli')
...
describe('read-only skill freshness inventory', () => {
  it('offers an exact older official name only when all global placements are safe', async () => {
    ...
    expect(inventory.eligibleUpdateNames).toEqual(['FABRICA-cli'])
  })
  ...
  it('scopes the lock hash to the current bundle, not every path a revision ever shipped', async () => {
    ...
    registry.skills['FABRICA-cli'][0].files.push(legacy)
  ...
  it('still flags canonical bytes that do not match what the lock says was installed', async () => {
    ...
    expect(getSkillFreshnessDisplayStatus(inventory, 'FABRICA-cli')).toBe('needs-attention')
  })
  ...
  it.runIf(process.platform !== 'win32')(
    ...
    async () => {
      ...
      await symlink(canonical, join(claudeRoot, 'FABRICA-cli'))
    ...
    async () => {
      ...
      const repos = await Promise.all(
        ['one', 'two'].map(async (id) => {
          ...
          await symlink(shared, join(root, 'FABRICA-cli'))
  ...
  it('keeps an unreadable foreign-home placement visible without withholding the update', async () => {
    const test = await fixture()
    await test.writeSkill(join(test.homeDir, '.agents', 'skills'), test.oldMarkdown)
    const inaccessiblePath = join(test.homeDir, '.codex', 'skills', 'FABRICA-cli')
  ...
  it('does not lose an inaccessible known repository placement', async () => {
    ...
    const inaccessiblePath = join(repoPath, '.agents', 'skills', 'FABRICA-cli')
  ...
  it('reads a plugin-cache copy with untouched official files as current', async () => {
    ...
    await writeFile(join(withSidecarRoot, 'README.md'), 'Neighbouring file FABRICA never shipped\n')
  ...
  it('does not classify an empty plugin-cache directory as a skill', async () => {
    const test = await fixture()
    await test.writeSkill(join(test.homeDir, '.agents', 'skills'), test.currentMarkdown)
    const emptyRoot = join(test.homeDir, '.codex', 'plugins', 'cache', 'vendor', 'FABRICA-cli')
  ...
  it('classifies exact current bytes as current when a later snapshot reuses them', async () => {
    ...
    const registry = JSON.parse(await readFile(registryPath, 'utf8'))
    registry.skills['FABRICA-cli'].push(snapshot(4, test.currentMarkdown))
    await writeFile(registryPath, `${JSON.stringify(registry, null, 2)}\n`)
    await test.writeSkill(join(test.homeDir, '.agents', 'skills'), test.currentMarkdown)
