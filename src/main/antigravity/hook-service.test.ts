...
describe('AntigravityHookService', () => {
  ...
  beforeEach(() => {
    homeDir = mkdtempSync(join(tmpdir(), 'FABRICA-antigravity-home-'))
  ...
  it('installs Antigravity global hooks.json bundle and managed script', () => {
    ...
    expect(Object.keys(config['FABRICA-status']).sort()).toEqual(
      ['PostInvocation', 'PostToolUse', 'PreInvocation', 'Stop'].sort()
    )
    expect(config['FABRICA-status'].PreToolUse).toBeUndefined()
    expect(config['FABRICA-status'].PostToolUse[0].matcher).toBe('*')
    ...
    if (process.platform === 'win32') {
      ...
      expect(config['FABRICA-status'].Stop[0].command).toContain("FABRICA_ANTIGRAVITY_EVENT='Stop'")
    }
    ...
    if (process.platform === 'win32') {
      ...
      expect(script).toContain('hook_event_name=$env:FABRICA_ANTIGRAVITY_EVENT')
      ...
      expect(script).toContain('hook_event_name=${FABRICA_ANTIGRAVITY_EVENT}')
  ...
  it('installs Windows event wrappers without nested cmd quoting and removes stale PreToolUse hooks', () => {
    withPlatform('win32', () => {
              ...
              PreToolUse: [
                {
                  ...
                  hooks: [
                    {
                      type: 'command',
                      command: `cmd /d /s /c "set "FABRICA_ANTIGRAVITY_EVENT=PreToolUse" && call "${staleScriptPath}""`
                    }
                  ]
                }
              ]
      ...
      for (const [eventName, wrapperFileName] of Object.entries(expectedWrappers)) {
        const definition = config['FABRICA-status'][eventName][0]
        const command =
  ...
  it('preserves user-authored hook bundles and entries in FABRICA bundle', () => {
    const configPath = join(homeDir, '.gemini', 'config', 'hooks.json')
    mkdirSync(dirname(configPath), { recursive: true })
    writeFileSync(
      configPath,
      `${JSON.stringify(
        {
          ...
          'FABRICA-status': {
            PreInvocation: [{ type: 'command', command: '/usr/local/bin/FABRICA-extra' }]
          }
        },
        null,
        2
    ...
    const config = JSON.parse(readFileSync(configPath, 'utf8')) as {
      ...
      'FABRICA-status': { PreInvocation: { command: string }[] }
    }
    expect(config['user-hook'].PreInvocation[0].command).toBe('/usr/local/bin/user-hook')
    const commands = config['FABRICA-status'].PreInvocation.map((entry) => entry.command)
    expect(commands).toContain('/usr/local/bin/FABRICA-extra')
  ...
  it('removes stale managed Antigravity hook entries from retired events', () => {
    ...
    const config = JSON.parse(readFileSync(configPath, 'utf8')) as {
      'FABRICA-status': Record<string, { command?: string; hooks?: { command: string }[] }[]>
    }
    expect(config['FABRICA-status'].OldEvent).toBeUndefined()
    expect(config['FABRICA-status'].PreToolUse).toBeUndefined()
    const commands = config['FABRICA-status'].PostToolUse.flatMap((definition) =>
