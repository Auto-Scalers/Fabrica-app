...
export class WslHookRelayManager {
  ...
  getInstanceKey(): string | null {
    ...
    const port = Number(this.deps.hookCoordsEnv().FABRICA_AGENT_HOOK_PORT ?? '')
  ...
  private async ensureInternal(requestedDistro: string | null): Promise<void> {
    ...
    const port = Number(coords.FABRICA_AGENT_HOOK_PORT ?? '')
    if (!Number.isInteger(port) || port <= 0 || !coords.FABRICA_AGENT_HOOK_TOKEN) {
      return
    }
