...
beforeEach(() => {
  tmpDir = mkdtempSync(join(tmpdir(), 'FABRICA-installer-utils-test-'))
...
describe('createManagedCommandMatcher', () => {
  ...
  it('matches commands containing the agent-hooks/<scriptFileName> path', () => {
    expect(
      match('/bin/sh "/Users/alice/Library/Application Support/FABRICA/agent-hooks/claude-hook.sh"')
    ).toBe(true)
  ...
  it('normalizes Windows backslashes so cmd-style paths still match', () => {
    expect(match('C:\\Users\\alice\\AppData\\Roaming\\FABRICA\\agent-hooks\\claude-hook.sh')).toBe(
      true
    )
  })
  ...
  it('matches the guarded launcher form so wrapped commands sweep correctly', () => {
    ...
    expect(
      match(
        'if [ -x "/Users/alice/Library/Application Support/FABRICA/agent-hooks/claude-hook.sh" ]; then /bin/sh "/Users/alice/Library/Application Support/FABRICA/agent-hooks/claude-hook.sh"; fi'
      )
    ).toBe(true)
  })

  it('matches encoded Windows launcher commands by decoding their script path', () => {
    const command = wrapWindowsHookCommand('C:\\Users\\alice\\.fabrica\\agent-hooks\\claude-hook.cmd')
  ...
  it('matches PowerShell and POSIX variants across Copilot platform switches', () => {
    ...
    expect(matchPosix("& 'C:\\Users\\alice\\.fabrica\\agent-hooks\\copilot-hook.ps1'")).toBe(true)
    expect(
      matchPosix(wrapWindowsHookCommand('C:\\Users\\alice\\.fabrica\\agent-hooks\\copilot-hook.ps1'))
    ).toBe(true)
    expect(matchPowerShell("/bin/sh '/home/alice/.fabrica/agent-hooks/copilot-hook.sh'")).toBe(true)
  })

  it('matches the legacy per-userData script path AND the new shared ~/.fabrica path', () => {
    // Why: install() must sweep old per-userData commands when migrating to
    // the shared ~/.fabrica script path, or stale launchers keep failing.
    expect(
      match("/bin/sh '/Users/alice/Library/Application Support/FABRICA/agent-hooks/claude-hook.sh'")
    ).toBe(true)
    expect(match("/bin/sh '/Users/alice/.fabrica/agent-hooks/claude-hook.sh'")).toBe(true)
  })
})
  ...
  it('removes managed direct bash/powershell/command fields', () => {
    const cleaned = removeManagedCommands(
      [
        {
          type: 'command',
          bash: '/bin/sh "/Users/alice/FABRICA/agent-hooks/copilot-hook.sh"',
          timeoutSec: 5
        },
        {
          type: 'command',
          powershell: "& 'C:\\Users\\alice\\FABRICA\\agent-hooks\\copilot-hook.sh'",
          timeoutSec: 5
        },
        {
          type: 'command',
...
describe('getSharedManagedScriptPath', () => {
  it("returns ~/.fabrica/agent-hooks/<scriptFileName> rooted at the user's home", () => {
    expect(getSharedManagedScriptPath('claude-hook.sh')).toBe(
      join(homedir(), '.fabrica', 'agent-hooks', 'claude-hook.sh')
    )
  })

  it('does not depend on Electron app.getPath, so two FABRICA instances resolve to the same path', () => {
...
describe('wrapPosixHookCommand', () => {
  ...
  it('preserves spaces in the script path (Library/Application Support case)', () => {
    ...
    const cmd = wrapPosixHookCommand('/Users/a/Library/Application Support/FABRICA/agent-hooks/x.sh')
    expect(cmd).toContain("'/Users/a/Library/Application Support/FABRICA/agent-hooks/x.sh'")
  })
  ...
  it('can scope environment variables to the guarded script invocation', () => {
    const cmd = wrapPosixHookCommand('/does/not/exist.sh', {
      FABRICA_COPILOT_HOOK_EVENT: 'UserPromptSubmit'
    })
    expect(cmd).toBe(
      `if [ -f '/does/not/exist.sh' ] && [ -r '/does/not/exist.sh' ] && [ -x '/does/not/exist.sh' ]; then FABRICA_COPILOT_HOOK_EVENT='UserPromptSubmit' /bin/sh '/does/not/exist.sh'; else ${POSIX_HOOK_STDIN_DRAIN_COMMAND}; fi`
    )
  })
...
describe('wrapWindowsHookCommand', () => {
  it('invokes the .cmd through an encoded PowerShell command', () => {
    const command = wrapWindowsHookCommand('C:\\Users\\alice\\.fabrica\\agent-hooks\\codex-hook.cmd')
    ...
    expect(decodeWindowsHookCommand(command)).toBe(
      expectedDecodedWindowsHookCommand('C:\\Users\\alice\\.fabrica\\agent-hooks\\codex-hook.cmd')
    )
  })
  ...
  it('scopes environment variables inside the encoded launcher', () => {
    ...
    expect(decodeWindowsHookCommand(command)).toContain(
      "$env:FABRICA_COPILOT_HOOK_EVENT = 'UserPromptSubmit'; if (Test-Path"
    )
  })
  ...
  it('preserves spaces in the script path (user profile with space case)', () => {
    const cmd = wrapWindowsHookCommand('C:\\Users\\Jorge Silva\\.fabrica\\agent-hooks\\codex-hook.cmd')
    ...
    expect(decodeWindowsHookCommand(cmd)).toBe(
      expectedDecodedWindowsHookCommand(
        'C:\\Users\\Jorge Silva\\.fabrica\\agent-hooks\\codex-hook.cmd'
      )
    )
  })

  it('keeps cmd.exe percent expansion and caret escapes out of the command line', () => {
    const cmd = wrapWindowsHookCommand('C:\\Users\\%FABRICA_TEST%\\a^b\\codex-hook.cmd')
    expect(cmd).not.toContain('%FABRICA_TEST%')
    expect(cmd).not.toContain('^')
    expect(decodeWindowsHookCommand(cmd)).toBe(
      expectedDecodedWindowsHookCommand('C:\\Users\\%FABRICA_TEST%\\a^b\\codex-hook.cmd')
    )
  })
  ...
  it.skipIf(process.platform !== 'win32')(
    ...
    () => {
      const scriptDir = join(tmpDir, 'home with ^ caret', '.fabrica', 'agent-hooks')
      ...
      const result = spawnSync('cmd.exe', ['/d', '/c', wrapWindowsHookCommand(scriptPath)], {
        env: { ...process.env, FABRICA_WRAP_TEST: 'expanded' }
      })
...
describe('wrapWindowsCmdHookCommand', () => {
  it('returns the bare, directly-spawnable path for a cmd-safe managed script', () => {
    ...
    const scriptPath = 'C:\\Users\\alice\\.fabrica\\agent-hooks\\codex-hook.cmd'
  ...
  it('falls back to the encoded launcher when cmd.exe would split or expand the path', () => {
    const scriptPath = 'C:\\Users\\Jane Doe\\%FABRICA_TEST%\\codex-hook.cmd'
    const command = wrapWindowsCmdHookCommand(scriptPath)
    expect(command).toMatch(qualifiedWindowsPowerShellCommand)
...
describe('wrapWindowsGitBashHookCommand', () => {
  it('guards the forward-slash fast path and drains when missing', () => {
    expect(
      wrapWindowsGitBashHookCommand('C:\\Users\\alice\\.fabrica\\agent-hooks\\claude-hook.cmd')
    ).toBe(
      `if [ -f 'C:/Users/alice/.fabrica/agent-hooks/claude-hook.cmd' ]; then 'C:/Users/alice/.fabrica/agent-hooks/claude-hook.cmd'; else ${POSIX_HOOK_STDIN_DRAIN_COMMAND}; fi`
    )
  })

  it('falls back to the encoded launcher when bash would split the path', () => {
    const scriptPath = 'C:\\Users\\Jane Doe\\.fabrica\\agent-hooks\\claude-hook.cmd'
  ...
  it('falls back to the encoded launcher when bash metacharacters are present', () => {
    const scriptPath = 'C:\\Users\\alice & bob\\.fabrica\\agent-hooks\\claude-hook.cmd'
    const command = wrapWindowsGitBashHookCommand(scriptPath)
    expect(command).toMatch(qualifiedWindowsPowerShellCommand)
...
describe('buildWindowsAgentHookPostCommand', () => {
  it('posts hook stdin through bounded curl without spawning PowerShell', () => {
    ...
    expect(command).toContain('-H "X-FABRICA-Agent-Hook-Token: %FABRICA_AGENT_HOOK_TOKEN%"')
    expect(command).toContain('--data-urlencode "paneKey=%FABRICA_PANE_KEY%"')
...
describe('buildWindowsAgentHookCurlPostCommand', () => {
  it('posts form fields via curl.exe and reads the payload from stdin', () => {
    ...
    expect(command).toContain('http://127.0.0.1:%FABRICA_AGENT_HOOK_PORT%/hook/codex')
    expect(command).toContain('-H "Content-Type: application/x-www-form-urlencoded"')
    expect(command).toContain('-H "X-FABRICA-Agent-Hook-Token: %FABRICA_AGENT_HOOK_TOKEN%"')
    expect(command).toContain('--data-urlencode "paneKey=%FABRICA_PANE_KEY%"')
    expect(command).toContain('--data-urlencode "worktreeId=%FABRICA_WORKTREE_ID%"')
