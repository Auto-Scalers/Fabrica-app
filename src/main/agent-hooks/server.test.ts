...
describe('AgentHookServer listener replay', () => {
  ...
  it('blocks local HTTP interrupt inference for provider-owned work without exposing transport metadata', async () => {
    ...
    try {
      ...
      const postHook = (payload: Record<string, unknown>): Promise<Response> =>
        fetch(`http://127.0.0.1:${env.FABRICA_AGENT_HOOK_PORT}/hook/claude`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'X-FABRICA-Agent-Hook-Token': env.FABRICA_AGENT_HOOK_TOKEN
          },
  ...
  it('hydrates cached statuses as not observed in the current runtime', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'FABRICA-agent-hooks-'))
  ...
  it('replays the latest retained pane status when a listener attaches after windowless events', async () => {
    ...
    try {
      ...
      const response = await fetch(`http://127.0.0.1:${env.FABRICA_AGENT_HOOK_PORT}/hook/claude`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'X-FABRICA-Agent-Hook-Token': env.FABRICA_AGENT_HOOK_TOKEN
    ...
    try {
      ...
      await expect(
        postClaudeHook({
          ...
          tool_name: 'Bash',
          tool_input: { command: 'rm -rf /tmp/FABRICA-subagent-repro' }
        })
      ).resolves.toMatchObject({ status: 204 })
      await expect(
        postClaudeHook({
      ...
      expect(server.getStatusSnapshot()).toEqual([
        expect.objectContaining({
          ...
          agentType: 'claude',
          toolName: 'Bash',
          toolInput: 'rm -rf /tmp/FABRICA-subagent-repro'
        })
      ])
    } finally {
      server.stop()
  ...
  it('lets Claude permission clear when approved PostToolUse matches the preceding tool use id', async () => {
    ...
    try {
      ...
      await postClaudeHook({
        hook_event_name: 'PreToolUse',
        tool_name: 'Bash',
        tool_input: { command: 'rm -rf /tmp/FABRICA-2824-permission-target' },
      ...
      expect(server.getStatusSnapshot()).toEqual([
        expect.objectContaining({
          ...
          agentType: 'claude',
          toolName: 'Bash',
          toolInput: 'rm -rf /tmp/FABRICA-2824-permission-target'
        })
      ])
    } finally {
      server.stop()
  ...
  it('does not replay cleared pane state to a newly attached listener', async () => {
    ...
    try {
      const env = server.buildPtyEnv()
      await fetch(`http://127.0.0.1:${env.FABRICA_AGENT_HOOK_PORT}/hook/codex`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'X-FABRICA-Agent-Hook-Token': env.FABRICA_AGENT_HOOK_TOKEN
    ...
    try {
      ...
      const postHook = async (
        ...
        const response = await fetch(
          `http://127.0.0.1:${env.FABRICA_AGENT_HOOK_PORT}/hook/${source}`,
          {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
  ...
  it('tracks Codex agent statuses from form-encoded managed hook posts', async () => {
    ...
    try {
      ...
      const postCodexHook = async (payload: Record<string, unknown>): Promise<void> => {
        ...
        const response = await fetch(`http://127.0.0.1:${env.FABRICA_AGENT_HOOK_PORT}/hook/codex`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/x-www-form-urlencoded',
            'X-FABRICA-Agent-Hook-Token': env.FABRICA_AGENT_HOOK_TOKEN
  ...
  it('accepts Hermes plugin hook posts on /hook/hermes', async () => {
    ...
    try {
      const env = server.buildPtyEnv()
      const response = await fetch(`http://127.0.0.1:${env.FABRICA_AGENT_HOOK_PORT}/hook/hermes`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'X-FABRICA-Agent-Hook-Token': env.FABRICA_AGENT_HOOK_TOKEN
  ...
  it('accepts Amp plugin hook posts on /hook/amp', async () => {
    ...
    try {
      ...
      const response = await fetch(`http://127.0.0.1:${env.FABRICA_AGENT_HOOK_PORT}/hook/amp`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'X-FABRICA-Agent-Hook-Token': env.FABRICA_AGENT_HOOK_TOKEN
...
describe('AgentHookServer prompt-sent telemetry', () => {
  ...
  it('tracks OpenCode user MessagePart hooks once per message id', async () => {
    ...
    try {
      const env = server.buildPtyEnv()
      const response = await fetch(`http://127.0.0.1:${env.FABRICA_AGENT_HOOK_PORT}/hook/opencode`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'X-FABRICA-Agent-Hook-Token': env.FABRICA_AGENT_HOOK_TOKEN
      ...
      const updatedResponse = await fetch(
        `http://127.0.0.1:${env.FABRICA_AGENT_HOOK_PORT}/hook/opencode`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
...
describe('Claude hook normalization', () => {
  ...
  describe('Stop transcript scan', () => {
    ...
    beforeEach(() => {
      tmpDir = mkdtempSync(join(tmpdir(), 'FABRICA-hook-test-'))
...
describe('Copilot hook normalization', () => {
  ...
  it('PermissionRequest stays working and preserves tool context', () => {
    const result = _internals.normalizeHookPayload(
      ...
      buildBody({
        hook_event_name: 'PermissionRequest',
        tool_name: 'bash',
        tool_input: { command: 'rm -rf /tmp/FABRICA-test' }
      }),
      'production'
    )
    expect(result?.payload.state).toBe('working')
    expect(result?.payload.toolName).toBe('bash')
    expect(result?.payload.toolInput).toBe('rm -rf /tmp/FABRICA-test')
  })
  ...
  it('Stop reads the final assistant message from Copilot transcript events', () => {
    const tmpDir = mkdtempSync(join(tmpdir(), 'FABRICA-copilot-transcript-'))
  ...
  it('accepts authenticated HTTP posts on /hook/copilot', async () => {
    ...
    try {
      ...
      const listener = vi.fn()
      server.setListener(listener)
      const response = await fetch(`http://127.0.0.1:${env.FABRICA_AGENT_HOOK_PORT}/hook/copilot`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'X-FABRICA-Agent-Hook-Token': env.FABRICA_AGENT_HOOK_TOKEN
  ...
  it('updates Copilot Stop with final transcript text after a non-blocking retry', async () => {
    const server = new AgentHookServer()
    const tmpDir = mkdtempSync(join(tmpdir(), 'FABRICA-copilot-transcript-retry-'))
    const transcriptPath = join(tmpDir, 'events.jsonl')
    ...
    try {
      ...
      await fetch(`http://127.0.0.1:${env.FABRICA_AGENT_HOOK_PORT}/hook/copilot`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'X-FABRICA-Agent-Hook-Token': env.FABRICA_AGENT_HOOK_TOKEN
  ...
  it('updates Grok Stop with final chat-history text after a non-blocking retry', async () => {
    const server = new AgentHookServer()
    const tmpDir = mkdtempSync(join(tmpdir(), 'FABRICA-grok-chat-history-retry-'))
    ...
    try {
      ...
      await fetch(`http://127.0.0.1:${env.FABRICA_AGENT_HOOK_PORT}/hook/grok`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'X-FABRICA-Agent-Hook-Token': env.FABRICA_AGENT_HOOK_TOKEN
      ...
      const response = await fetch(`http://127.0.0.1:${env.FABRICA_AGENT_HOOK_PORT}/hook/grok`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'X-FABRICA-Agent-Hook-Token': env.FABRICA_AGENT_HOOK_TOKEN
...
describe('Endpoint file lifecycle', () => {
  ...
  beforeEach(() => {
    userDataPath = mkdtempSync(join(tmpdir(), 'FABRICA-endpoint-'))
  })
  ...
  it('writes the endpoint file with the expected shell-sourceable shape', async () => {
    ...
    try {
      ...
      const expectedPort = server.buildPtyEnv().FABRICA_AGENT_HOOK_PORT
      const expectedToken = server.buildPtyEnv().FABRICA_AGENT_HOOK_TOKEN
      const prefix = process.platform === 'win32' ? 'set ' : ''
      expect(contents).toContain(`${prefix}FABRICA_AGENT_HOOK_PORT=${expectedPort}`)
      expect(contents).toContain(`${prefix}FABRICA_AGENT_HOOK_TOKEN=${expectedToken}`)
      expect(contents).toContain(`${prefix}FABRICA_AGENT_HOOK_ENV=development`)
      expect(contents).toContain(`${prefix}FABRICA_AGENT_HOOK_VERSION=1`)
    } finally {
      server.stop()
    }
  })
  ...
  it('rewrites the endpoint file with a new port after restart on the same path', async () => {
    ...
    const firstToken = server.buildPtyEnv().FABRICA_AGENT_HOOK_TOKEN
    server.stop()

    await server.start({ env: 'production', userDataPath })
    try {
      const secondPath = server.endpointFilePath
      const secondPort = server.buildPtyEnv().FABRICA_AGENT_HOOK_PORT
      const secondToken = server.buildPtyEnv().FABRICA_AGENT_HOOK_TOKEN
      ...
      expect(contents).toContain(`FABRICA_AGENT_HOOK_PORT=${secondPort}`)
      expect(contents).toContain(`FABRICA_AGENT_HOOK_TOKEN=${secondToken}`)
      expect(contents).not.toContain(`FABRICA_AGENT_HOOK_TOKEN=${firstToken}`)
    } finally {
      server.stop()
    }
  })
  ...
  it('leaves the endpoint file in place on stop()', async () => {
    // Why: stop() leaves the file (stale = fail-open); unlinking would race a concurrent FABRICA instance rewriting it between token-check and unlink (TOCTOU).
    const server = new AgentHookServer()
    await server.start({ env: 'production', userDataPath })
    const filePath = server.endpointFilePath!
    expect(existsSync(filePath)).toBe(true)
  ...
  it('buildPtyEnv includes FABRICA_AGENT_HOOK_ENDPOINT when the server is running', async () => {
    const server = new AgentHookServer()
    await server.start({ env: 'production', userDataPath })
    try {
      const env = server.buildPtyEnv()
  ...
  it('buildPtyEnv includes namespaced FABRICA_AGENT_HOOK_ENDPOINT for development servers', async () => {
    const server = new AgentHookServer()
    await server.start({
      env: 'development',
      userDataPath,
  ...
  it('keeps endpoint files separate for parallel dev namespaces', async () => {
    ...
    try {
      ...
      expect(firstServer.buildPtyEnv().FABRICA_AGENT_HOOK_ENDPOINT).toBe(firstServer.endpointFilePath)
      expect(secondServer.buildPtyEnv().FABRICA_AGENT_HOOK_ENDPOINT).toBe(
  ...
  it('buildPtyEnv omits FABRICA_AGENT_HOOK_ENDPOINT when no userDataPath was provided', async () => {
  ...
  it('refuses to write the endpoint file when a value contains shell metacharacters', async () => {
    ...
    try {
      ...
      expect(server.buildPtyEnv().FABRICA_AGENT_HOOK_ENDPOINT).toBeUndefined()
      // PORT/TOKEN still flow via PTY env — fail-open to v1 behavior.
      expect(server.buildPtyEnv().FABRICA_AGENT_HOOK_PORT).toBeTruthy()
      expect(server.buildPtyEnv().FABRICA_AGENT_HOOK_TOKEN).toBeTruthy()
    } finally {
      server.stop()
    }
  })
  ...
  it('endpoint file contents are re-parseable by /bin/sh', async () => {
    ...
    try {
      ...
      const out = execFileSync('/bin/sh', ['-c', `. "${filePath}" && echo "$FABRICA_AGENT_HOOK_PORT"`])
        .toString()
        .trim()
...
describe('Last-status persistence', () => {
  ...
  beforeEach(() => {
    userDataPath = mkdtempSync(join(tmpdir(), 'FABRICA-laststatus-'))
  })
  ...
  async function postHookEvent(
    ...
    const env = server.buildPtyEnv()
    return fetch(`http://127.0.0.1:${env.FABRICA_AGENT_HOOK_PORT}${path}`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'X-FABRICA-Agent-Hook-Token': env.FABRICA_AGENT_HOOK_TOKEN
...
describe('AgentHookServer ingestTerminalStatus', () => {
  ...
  // erased the session id from persisted rows and from headless `FABRICA serve` — which serves these
