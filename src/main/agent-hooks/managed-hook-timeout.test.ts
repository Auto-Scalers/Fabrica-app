...
const MANAGED_HOOKS_DIR_NEEDLE = '/.fabrica/agent-hooks/'
...
describe('managed agent hook timeouts', () => {
  ...
  it('bounds every generated POSIX curl wrapper with --connect-timeout and --max-time', async () => {
    ...
    const kimiWrapper = kimi.fs.files.get(`${REMOTE_HOME}/.fabrica/agent-hooks/kimi-hook.sh`)!
  ...
  describe('dead-endpoint transport budget', () => {
    ...
    it.skipIf(!hasPosixCurl)(
      ...
      async () => {
        ...
        const wrapperBody = fs.files.get(`${REMOTE_HOME}/.fabrica/agent-hooks/codex-hook.sh`)!
