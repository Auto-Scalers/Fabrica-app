...
describe('remote hook service installers', () => {
  it('always writes POSIX scripts for SSH remotes even from a Windows host', async () => {
    ...
    try {
      const installers = [
        ...
        {
          path: '/home/dev/.config/amp/plugins/FABRICA-agent-status.ts',
  ...
  it('installs Codex hooks into an explicit redirected CODEX_HOME (WSL managed runtime home)', async () => {
    const runtimeHome = '/home/dev/.local/share/FABRICA/codex-runtime-home/home'
    const { sftp, fs } = createFakeSftp({
...
`
    ...
    const ampPlugin = amp.fs.files.get('/home/dev/.config/amp/plugins/FABRICA-agent-status.ts')
    ...
    const grokConfig = JSON.parse(grok.fs.files.get('/home/dev/.grok/hooks/FABRICA-status.json')!) as {
      hooks: Record<string, { matcher?: string; hooks?: { command: string }[] }[]>
    }
    for (const eventName of [
      'SessionStart',
  ...
  it.each(['relative/grok', '/bad\\grok', `/${'x'.repeat(4096)}`])(
    ...
    async (remoteGrokHome) => {
      ...
      expect(status.configPath).toBe('/home/dev/.grok/hooks/FABRICA-status.json')
      expect(fs.files.has('/home/dev/.grok/hooks/FABRICA-status.json')).toBe(true)
    }
  )
  ...
  it('removes stale remote Antigravity PreToolUse hooks while installing SSH hooks', async () => {
    ...
    const config = JSON.parse(fs.files.get('/home/dev/.gemini/config/hooks.json')!) as {
      'FABRICA-status': Record<string, { hooks?: { command: string }[] }[]>
    }
    expect(config['FABRICA-status'].PreToolUse).toBeUndefined()
    const postToolCommands = config['FABRICA-status'].PostToolUse.flatMap((definition) =>
  ...
  it('installs remote Copilot hooks under the user-level hooks directory', async () => {
    ...
    fs.files.set(
      '/home/dev/.copilot/hooks/FABRICA.json',
      JSON.stringify({
        version: 99,
    ...
    expect(status.state).toBe('installed')
    expect(status.configPath).toBe('/home/dev/.copilot/hooks/FABRICA.json')
    const config = JSON.parse(fs.files.get('/home/dev/.copilot/hooks/FABRICA.json')!) as {
      version: number
  ...
  it('installs remote Hermes plugin files and enables the plugin', async () => {
    ...
    expect(fs.files.get('/home/dev/.hermes/plugins/FABRICA-status/plugin.yaml')).toContain(
      'pre_llm_call'
    )
    expect(fs.files.get('/home/dev/.hermes/plugins/FABRICA-status/__init__.py')).toContain(
      '/hook/hermes'
    )
    expect(fs.files.get('/home/dev/.hermes/config.yaml')).toContain('FABRICA-status')
  })

  it('does not overwrite a remote user-authored Amp plugin file', async () => {
    const { sftp, fs } = createFakeSftp({
      '/home/dev/.config/amp/plugins/FABRICA-agent-status.ts':
    ...
    expect(fs.files.get('/home/dev/.config/amp/plugins/FABRICA-agent-status.ts')).toBe(
      'export default function userPlugin() {}\n'
    )
  })
})
