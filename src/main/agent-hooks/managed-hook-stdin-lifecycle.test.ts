...
// missing-FABRICA-env path, so their writer may break there.
...
beforeEach(() => {
  ...
  isolatedUserDataDir = mkdtempSync(join(tmpdir(), 'FABRICA-hook-stdin-user-data-'))
  // Why: FABRICA-managed Codex hooks resolve through FABRICA_USER_DATA_PATH before
  // the mocked home; an inherited live path would let this test rewrite them.
  process.env.FABRICA_USER_DATA_PATH = isolatedUserDataDir
})
...
function hookEnvironment(extraEnv: NodeJS.ProcessEnv = {}): NodeJS.ProcessEnv {
  const env = Object.fromEntries(
    Object.entries(process.env).filter(([key]) => !key.startsWith('FABRICA_'))
  )
  return {
    ...env,
    HOME: REMOTE_HOME,
...
describe('Windows managed hook stdin structure', () => {
  it('exits immediately when FABRICA env is missing and keeps drain for other failures', () => {
    const home = mkdtempSync(join(tmpdir(), 'FABRICA-hook-stdin-windows-'))
    ...
    try {
      ...
      const hooksDir = join(home, '.fabrica', 'agent-hooks')
      ...
      for (const fileName of mainBatchScripts) {
        ...
        expect(script, `${fileName} port guard`).toContain(
          'if "%FABRICA_AGENT_HOOK_PORT%"=="" exit /b 0'
        )
        expect(script, `${fileName} token guard`).toContain(
          'if "%FABRICA_AGENT_HOOK_TOKEN%"=="" exit /b 0'
        )
        expect(script, `${fileName} pane guard`).toContain('if "%FABRICA_PANE_KEY%"=="" exit /b 0')
        // Why: pin the rule, not today's three guards — a fourth FABRICA_* guard routed to the
        ...
        // FABRICA_* var, so it stays exempt.
        expect(script, `${fileName} no FABRICA_* guard may route to the more.com drain`).not.toMatch(
          /FABRICA_[A-Z_]+.*goto :?FABRICA_agent_hook_drain_stdin/
        )
        ...
        expect(script, `${fileName} drain epilogue`).toContain(
          [
            ':FABRICA_agent_hook_drain_stdin',
      ...
      // must sit below the env guards — otherwise a Devin session outside an FABRICA pane still
      ...
      expect(claude, 'claude devin guard present').toContain(
        'if not "%DEVIN_PROJECT_DIR%"=="" goto :FABRICA_agent_hook_drain_stdin'
      )
      expect(claude.indexOf('if "%FABRICA_PANE_KEY%"=="" exit /b 0')).toBeLessThan(
        claude.indexOf('if not "%DEVIN_PROJECT_DIR%"=="" goto :FABRICA_agent_hook_drain_stdin')
      )

      const copilot = readFileSync(join(hooksDir, 'copilot-hook.ps1'), 'utf8')
      expect(copilot.indexOf('[Console]::In.ReadToEnd()')).toBeLessThan(
        copilot.indexOf('if (-not $env:FABRICA_AGENT_HOOK_PORT')
      )
  ...
  it.skipIf(process.platform !== 'win32')(
    ...
    async () => {
      const home = mkdtempSync(join(tmpdir(), 'FABRICA-hook-stdin-windows-live-'))
      homedirMock.mockReturnValue(home)
      try {
        ...
        for (const fileName of mainScripts) {
          ...
          if (fileName.endsWith('.cmd')) {
            ...
            // strips every FABRICA_* var, so the relaxation only ever covers the missing-env
        ...
        const missingScript = 'C:\\missing\\FABRICA-hook.cmd'
...
describe.skipIf(process.platform === 'win32')('managed hook stdin lifecycle', () => {
  ...
  it('accepts a large payload without FABRICA environment or a broken writer', async () => {
    const scripts = await generatePosixScripts()
    for (const [agent, script] of scripts) {
      const extraEnv = agent.startsWith('command-code')
        ? {
            FABRICA_AGENT_HOOK_PORT: '1',
            FABRICA_AGENT_HOOK_TOKEN: 'test-token',
            FABRICA_PANE_KEY: 'test-pane'
          }
        : {}
  ...
  it('does not need PATH to capture or drain POSIX hook stdin', async () => {
    ...
    const missing = await runPosixHook(wrapPosixHookCommand('/missing/FABRICA-hook.sh'), { PATH: '' })
  ...
  it.each([
    ...
    const decoyDir = mkdtempSync(join(tmpdir(), 'FABRICA-hook-stdin-decoy-'))
    try {
  ...
  it('drains a large payload when the configured script is missing', async () => {
    const result = await runPosixHook(wrapPosixHookCommand('/missing/FABRICA-hook.sh'))
    expect(result.exitCode).toBe(0)
    expect(result.stdinErrors).toHaveLength(0)
  })
})
