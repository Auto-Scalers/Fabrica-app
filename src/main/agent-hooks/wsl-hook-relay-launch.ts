...
export function resolveWslHookRelayBundle(): WslHookRelayBundle | null {
  ...
  if (process.env.FABRICA_RELAY_PATH) {
    candidates.push(join(process.env.FABRICA_RELAY_PATH, 'wsl'))
  }
...
// Why: the install dir is namespaced by bundle version so concurrent FABRICA
...
export function buildGuestInstallScript(bundleJs: Buffer, version: string): string {
  ...
  return [
    ...
    'mkdir -p "$d"',
    `base64 -d > "$d/bundle.$$.tmp" << 'FABRICA_EOF_BUNDLE'`,
    b64.trimEnd(),
    'FABRICA_EOF_BUNDLE',
    `mv "$d/bundle.$$.tmp" "$d/${WSL_HOOK_RELAY_BUNDLE_NAME}"`,
    `cat > "$d/launch.$$.tmp" << 'FABRICA_EOF_LAUNCH'`,
...
export function buildWslRelaySpawnEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...process.env,
    WSL_UTF8: '1',
    FABRICA_AGENT_HOOK_PORT: coords.FABRICA_AGENT_HOOK_PORT,
    FABRICA_AGENT_HOOK_TOKEN: coords.FABRICA_AGENT_HOOK_TOKEN,
    FABRICA_AGENT_HOOK_ENV: coords.FABRICA_AGENT_HOOK_ENV,
    FABRICA_AGENT_HOOK_VERSION: coords.FABRICA_AGENT_HOOK_VERSION,
  ...
  addFABRICAWslInteropEnv(env as Record<string, string>)
  return env
}
