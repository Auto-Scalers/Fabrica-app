...
describe.skipIf(process.platform === 'win32')(
  ...
  () => {
    ...
    let FABRICAServer: AgentHookServer | null
    ...
    it('delivers a Claude hook POST from the live relay into ingestRemote and installs guest hooks', async () => {
      ...
      manager = new WslHookRelayManager({
        ...
        hookCoordsEnv: () => ({
          ...
          FABRICA_AGENT_HOOK_TOKEN: 'live-token',
      ...
      const claudeScript = readFileSync(
        join(fakeHome, '.FABRICA', 'agent-hooks', 'claude-hook.sh'),
        'utf8'
      )
      ...
      const port = Number(/FABRICA_AGENT_HOOK_PORT=['"]?(\d+)/.exec(endpointText)?.[1])
      const token = /FABRICA_AGENT_HOOK_TOKEN=['"]?([A-Za-z0-9-]+)/.exec(endpointText)?.[1]
