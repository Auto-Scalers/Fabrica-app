...
import { FABRICA_HOOK_PROTOCOL_VERSION } from '../../shared/agent-hook-types'
import {
...
export class AgentHookServer {
  ...
  private port = 0
  private token = ''
  // Why: identifies this FABRICA instance so the server can detect dev vs. prod cross-talk; set at start() from packaged-build knowledge.
  ...
  private scheduleCodexSubagentPoll(
    ...
    // Why: a nested non-codex CLI inherits FABRICA_PANE_KEY, so clearing here would silently end a live codex poll.
    if (source !== 'codex') {
      return
    }
  ...
  ingestTerminalStatus(event: {
    ...
    // That erased it from persisted rows (lost across restart) and from headless `FABRICA serve`, which
  ...
  stop(): void {
    ...
    // Why: don't unlink the endpoint file — a stale file matches fail-open and avoids a TOCTOU race with a concurrent FABRICA.
   ...
   *  has no live local agent process behind it any more. A PTY that dies while FABRICA
  ...
  buildPtyEnv(): Record<string, string> {
    ...
    const env: Record<string, string> = {
      FABRICA_AGENT_HOOK_PORT: String(this.port),
      FABRICA_AGENT_HOOK_TOKEN: this.token,
      FABRICA_AGENT_HOOK_ENV: this.env,
      FABRICA_AGENT_HOOK_VERSION: FABRICA_HOOK_PROTOCOL_VERSION
    }
    ...
    if (this.endpointFileWritten && this.endpointFilePathCache) {
      env.FABRICA_AGENT_HOOK_ENDPOINT = this.endpointFilePathCache
    }
    return env
  }
