...
export type FirstWorkBranchRenameDeps = {
  ...
  /** True only for FABRICA-created worktrees whose branch FABRICA is allowed to rename. */
  canRenameFABRICACreatedBranch: (worktreeId: string) => boolean
...
async function runAutoRename(
  ...
  if (!deps.canRenameFABRICACreatedBranch(worktreeId)) {
    return stop(`worktree is not eligible for auto-rename`, true)
  }
  // Only rename a branch FABRICA auto-named — never one the user chose.
