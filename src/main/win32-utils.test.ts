...
describe('getSpawnArgsForWindows', () => {
  ...
  it('keeps the waiting form for batch launches without detachedGui', () => {
    withPlatform('win32', () => {
      const { spawnArgs } = getSpawnArgsForWindows('C:\\Tools\\idea.cmd', ['C:\\workspaces\\FABRICA'])
      expect(spawnArgs).toEqual(['/d', '/c', 'C:\\Tools\\idea.cmd', 'C:\\workspaces\\FABRICA'])
    })
  })
  ...
  it('leaves .exe GUI launches alone even when detachedGui is requested', () => {
    withPlatform('win32', () => {
      ...
      expect(spawnArgs).toEqual(['C:\\workspaces\\FABRICA'])
    })
  })
...
describe('resolveWindowsCommand', () => {
  it('finds package-manager .cmd shims on PATH before spawning fixed commands', () => {
    const tempDir = mkdtempSync(join(tmpdir(), 'FABRICA-win-command-'))
    try {
