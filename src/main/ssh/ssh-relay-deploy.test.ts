...
vi.mock('./ssh-relay-deploy-helpers', () => ({
  ...
  execCommand: vi.fn().mockResolvedValue('__FABRICA_REMOTE_PLATFORM__ Linux x86_64')
}))
...
describe('deployAndLaunchRelay', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    vi.mocked(execCommand).mockReset().mockResolvedValue('__FABRICA_REMOTE_PLATFORM__ Linux x86_64')
  ...
  it('calls exec to detect remote platform', async () => {
    ...
    mockExecCommand.mockResolvedValueOnce('__FABRICA_REMOTE_PLATFORM__ Linux x86_64') // tagged POSIX platform probe
    mockExecCommand.mockResolvedValueOnce('/home/user') // echo $HOME
    mockExecCommand.mockResolvedValueOnce('FABRICA-NATIVE-DEPS-OK') // native deps probe
    ...
    expect(mockExecCommand).toHaveBeenCalledWith(
      conn,
      "printf '\\n%s ' '__FABRICA_REMOTE_PLATFORM__'; uname -sm",
  ...
  it('reports progress via callback', async () => {
    const conn = makeMockConnection()
    const mockExecCommand = vi.mocked(execCommand)
    mockExecCommand.mockResolvedValueOnce('__FABRICA_REMOTE_PLATFORM__ Linux x86_64')
  ...
  it('does not launch fresh after unconfirmed stale-socket cleanup', async () => {
    ...
    vi.mocked(execCommand)
      .mockResolvedValueOnce('__FABRICA_REMOTE_PLATFORM__ Linux x86_64')
      .mockResolvedValueOnce('/home/user')
      .mockResolvedValueOnce('FABRICA-NATIVE-DEPS-OK')
  ...
  it('resolves the remote node path once per deploy', async () => {
    ...
    mockExecCommand.mockResolvedValueOnce('__FABRICA_REMOTE_PLATFORM__ Linux x86_64')
    mockExecCommand.mockResolvedValueOnce('/home/user')
    mockExecCommand.mockResolvedValueOnce('FABRICA-NATIVE-DEPS-OK')
    queueLaunchNamespaceAndDeadSocketProbe()
    ...
    try {
      ...
      vi.mocked(execCommand).mockImplementation((_conn, command) => {
        if (command.includes('uname')) {
          return Promise.resolve('__FABRICA_REMOTE_PLATFORM__ Linux x86_64')
        }
        if (command === 'echo $HOME') {
          return Promise.resolve('/home/user')
        }
        const marker = command.match(/\.sftp-namespace-[0-9a-f]{32}/u)?.[0]
        if (command.includes('__FABRICA_UPLOAD_STAGE_SLOT__') && marker) {
          return Promise.resolve(`__FABRICA_UPLOAD_STAGE_SLOT__${marker}:slot-0`)
        }
    ...
    try {
      ...
      mockExecCommand
        ...
        .mockImplementationOnce(
          () =>
            new Promise<string>((resolve) =>
              setTimeout(() => resolve('FABRICA-NATIVE-DEPS-OK'), 899_900)
            )
        )
        .mockResolvedValueOnce('') // launch namespace marker
  ...
  it('uses distinct target-specific relay socket paths', async () => {
    ...
    mockExecCommand.mockImplementation((_conn, command) => {
      if (command.includes('__FABRICA_REMOTE_PLATFORM__')) {
        return Promise.resolve('__FABRICA_REMOTE_PLATFORM__ Linux x86_64')
      }
      if (command === 'echo $HOME') {
        return Promise.resolve('/home/user')
      }
      if (command.includes('FABRICA-NATIVE')) {
        return Promise.resolve('FABRICA-NATIVE-DEPS-OK')
      }
  ...
  it('launches Windows remotes via a named pipe endpoint', async () => {
    ...
    mockExecCommand
      ...
      .mockResolvedValueOnce('__FABRICA_REMOTE_PLATFORM__ Windows X64') // tagged PowerShell platform probe
      .mockResolvedValueOnce('C:\\Users\\me user') // remote home
      .mockResolvedValueOnce('FABRICA-NATIVE-DEPS-OK') // native deps probe
    ...
    expect(result.sockPath).toMatch(/^\\\\\.\\pipe\\FABRICA-relay-[0-9a-f]{20}$/)
  ...
  it('relaunches Windows remotes on a fallback pipe when reconnecting the occupied pipe fails', async () => {
    ...
    expect(primaryPipe).toMatch(/^\\\\\.\\pipe\\FABRICA-relay-[0-9a-f]{20}$/)
    expect(fallbackPipe).toMatch(/^\\\\\.\\pipe\\FABRICA-relay-[0-9a-f]{20}$/)
  ...
  it('prefers a persisted Windows fallback pipe on later reconnects', async () => {
    const conn = makeMockConnection()
    const mockExecCommand = vi.mocked(execCommand)
    const persistedPipe = '\\\\.\\pipe\\FABRICA-relay-1234567890abcdef1234'
    vi.mocked(resolveRemoteNodePath).mockResolvedValue('C:/Program Files/nodejs/node.exe')
    mockExecCommand
      .mockRejectedValueOnce(new Error('uname not found')) // tagged POSIX platform probe
      .mockResolvedValueOnce('__FABRICA_REMOTE_PLATFORM__ Windows X64') // tagged PowerShell platform probe
  ...
  it('scopes persisted Windows active pipe markers by relay target', async () => {
    ...
    mockExecCommand
      ...
      .mockResolvedValueOnce('__FABRICA_REMOTE_PLATFORM__ Windows X64')
