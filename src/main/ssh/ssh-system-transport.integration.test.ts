...
function createRelayTree(root: string, remoteHome: string): void {
  ...
  const remoteDir = join(remoteHome, '.FABRICA-remote', `relay-${RELAY_VERSION}`)
...
describe('system SSH transport integration', () => {
  ...
  beforeEach(() => {
    ...
    tempDir = mkdtempSync(join('/tmp', 'FABRICA-ssh-'))
    oldHome = process.env.HOME
    oldRelayPath = process.env.FABRICA_RELAY_PATH
    oldSystemSshPath = process.env.FABRICA_SYSTEM_SSH_PATH
    oldForceSystemTransport = process.env.FABRICA_SSH_FORCE_SYSTEM_TRANSPORT
    ...
    process.env.FABRICA_RELAY_PATH = relayRoot
    process.env.FABRICA_SYSTEM_SSH_PATH = writeFakeSsh(tempDir)
    process.env.FABRICA_SSH_FORCE_SYSTEM_TRANSPORT = '1'
  })
  ...
  afterEach(() => {
    ...
    if (oldRelayPath === undefined) {
      delete process.env.FABRICA_RELAY_PATH
    } else {
      process.env.FABRICA_RELAY_PATH = oldRelayPath
    }
    if (oldSystemSshPath === undefined) {
      delete process.env.FABRICA_SYSTEM_SSH_PATH
    } else {
      process.env.FABRICA_SYSTEM_SSH_PATH = oldSystemSshPath
    }
    if (oldForceSystemTransport === undefined) {
      delete process.env.FABRICA_SSH_FORCE_SYSTEM_TRANSPORT
    } else {
      process.env.FABRICA_SSH_FORCE_SYSTEM_TRANSPORT = oldForceSystemTransport
    }
