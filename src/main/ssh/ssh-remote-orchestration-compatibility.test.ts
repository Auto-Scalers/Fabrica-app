...
import { runRemoteFABRICACli } from './ssh-remote-fabrica-cli'
import { acknowledgeRemoteFABRICACliPostOutput } from './ssh-remote-orchestration-post-output'
...
const WORKER_ENV = {
  FABRICA_TERMINAL_HANDLE: WORKER_HANDLE,
  FABRICA_PANE_KEY: WORKER_PANE,
  FABRICA_AGENT_LAUNCH_TOKEN: 'legacy-ssh-token'
}
const COORDINATOR_ENV = {
  FABRICA_TERMINAL_HANDLE: COORDINATOR_HANDLE,
  FABRICA_PANE_KEY: COORDINATOR_PANE,
  FABRICA_AGENT_LAUNCH_TOKEN: 'legacy-ssh-coordinator-token'
}
...
function createLegacyRuntime() {
  ...
  vi.spyOn(runtime, 'verifyOrchestrationCompatibilityCaller').mockImplementation((evidence) => {
    const worker =
      ...
      evidence.launchToken === WORKER_ENV.FABRICA_AGENT_LAUNCH_TOKEN
    const coordinator =
      evidence?.terminalHandle === COORDINATOR_HANDLE &&
      evidence.paneKey === COORDINATOR_PANE &&
      evidence.launchToken === COORDINATOR_ENV.FABRICA_AGENT_LAUNCH_TOKEN
    if (
      (!worker && !coordinator) ||
    ...
    const identity = worker
      ? {
          ...
          paneKey: WORKER_PANE,
          launchToken: WORKER_ENV.FABRICA_AGENT_LAUNCH_TOKEN
        }
      : {
          terminalHandle: COORDINATOR_HANDLE,
          paneKey: COORDINATOR_PANE,
          launchToken: COORDINATOR_ENV.FABRICA_AGENT_LAUNCH_TOKEN
        }
    return {
...
describe('legacy SSH orchestration fallback', () => {
  it('acknowledges a consuming check only after remote output', async () => {
    ...
    try {
      const first = await runRemoteFABRICACli(runtime, request, LEGACY_FALLBACK_OPTIONS)
      const replay = await runRemoteFABRICACli(runtime, request, LEGACY_FALLBACK_OPTIONS)
      ...
      await acknowledgeRemoteFABRICACliPostOutput(runtime, {
        postOutput: first.postOutput!,
        env: WORKER_ENV,
        runtimeAuthority: RUNTIME_AUTHORITY
      })
      const afterAck = await runRemoteFABRICACli(runtime, request, LEGACY_FALLBACK_OPTIONS)
  ...
  it('keeps peek formatted and non-consuming', async () => {
    ...
    try {
      const peek = await runRemoteFABRICACli(
        runtime,
        {
  ...
  it('reads and acknowledges current Run delivery', async () => {
    ...
    try {
      const checked = await runRemoteFABRICACli(
        runtime,
        {
          ...baseRequest,
      ...
      const checkedJson = await runRemoteFABRICACli(
        runtime,
        {
          ...baseRequest,
      ...
      const acknowledged = await runRemoteFABRICACli(
        runtime,
        {
          ...baseRequest,
          argv: [
  ...
  it('resumes and acknowledges a question answer', async () => {
    ...
    try {
      const result = await runRemoteFABRICACli(
        runtime,
        {
