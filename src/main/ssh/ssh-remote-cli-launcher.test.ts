...
describe('SSH remote FABRICA CLI launcher', () => {
  function windowsInstallPlan(): ReturnType<typeof createRemoteCliInstallPlan> {
    return createRemoteCliInstallPlan({
      binDir: 'C:/Users/me user/.FABRICA-relay/bin',
      relayDir: 'C:/Users/me user/.FABRICA-remote/relay-v1',
      nodePath: 'C:/Program Files/nodejs/node.exe',
      sockPath: '\\\\.\\pipe\\FABRICA-relay-123',
      credentialFile: 'C:/Users/me user/.FABRICA-remote/relay-v1/relay.sock.credential',
  ...
  it('compiles a native Windows launcher without a cmd.exe argument bridge', () => {
    ...
    expect(plan.launcherPath).toBe('C:/Users/me user/.FABRICA-relay/bin/FABRICA.exe')
    expect(plan.files).toHaveLength(1)
    expect(plan.files[0]?.path).toBe('C:/Users/me user/.FABRICA-relay/bin/FABRICA-launcher.cs')
    expect(plan.files[0]?.contents).toContain('ProcessStartInfo')
    expect(plan.files[0]?.contents).toContain('"--fabrica-cli"')
    ...
    expect(compileScript).toContain(
      "Set-Location -ErrorAction Stop -LiteralPath 'C:/Users/me user/.FABRICA-relay/bin'"
    )
    expect(compileScript).toContain('/out:FABRICA.exe')
    expect(compileScript).toContain('C:/Users/me user/.FABRICA-relay/bin/FABRICA-launcher.cs')
    expect(compileScript).toContain('C:/Users/me user/.FABRICA-relay/bin/FABRICA.cmd')
  })

  it('removes the legacy FABRICA.cmd only after every compile guard has passed', () => {
    const script = decodePowerShellCommand(windowsInstallPlan().postWriteCommands[0] ?? '')
    const legacyShimRemoval =
      "Remove-Item -LiteralPath 'C:/Users/me user/.FABRICA-relay/bin/FABRICA.cmd' -Force -ErrorAction SilentlyContinue"
    ...
    const guards = [
      ...
      "if (-not (Test-Path -LiteralPath 'C:/Users/me user/.FABRICA-relay/bin/FABRICA.exe' -PathType Leaf))"
    ]
  ...
  itWindows('preserves a multiline argument through the compiled remote launcher', () => {
    const root = mkdtempSync(join(tmpdir(), 'FABRICA remote cli '))
    try {
      const binDir = join(root, 'bin').replaceAll('\\', '/')
      const relayDir = join(root, 'relay').replaceAll('\\', '/')
      const sockPath = '\\\\.\\pipe\\FABRICA-relay-test'
      ...
      const launched = spawnSync(
        ...
        ['orchestration', 'send', '--body', body, '--json'],
        {
          ...
          env: {
            ...process.env,
            FABRICA_RELAY_NODE_PATH: process.execPath,
            FABRICA_RELAY_DIR: relayDir,
            FABRICA_RELAY_SOCKET_PATH: sockPath,
            FABRICA_RELAY_CREDENTIAL_FILE: credentialFile
          }
        }
      )
  ...
  itWindows('preserves the existing FABRICA.cmd when the compiler is missing', () => {
    const root = mkdtempSync(join(tmpdir(), 'FABRICA remote cli '))
    try {
      const binDir = join(root, 'bin').replaceAll('\\', '/')
      mkdirSync(binDir, { recursive: true })
      const legacyShimPath = join(binDir, 'FABRICA.cmd')
      writeFileSync(legacyShimPath, '@echo legacy FABRICA cli\r\n', 'utf8')

      const plan = createRemoteCliInstallPlan({
        binDir,
        relayDir: join(root, 'relay').replaceAll('\\', '/'),
        nodePath: process.execPath,
        sockPath: '\\\\.\\pipe\\FABRICA-relay-test',
      ...
      expect(existsSync(legacyShimPath), 'existing FABRICA.cmd must survive a failed install').toBe(
        true
      )
    } finally {
      rmSync(root, { recursive: true, force: true })
  ...
  it('keeps the POSIX launcher as an argv-preserving shell exec', () => {
    const plan = createRemoteCliInstallPlan({
      binDir: '/home/me/.FABRICA-relay/bin',
      relayDir: '/home/me/.FABRICA-remote/relay-v1',
      nodePath: '/usr/bin/node',
      sockPath: '/home/me/.FABRICA-remote/relay-v1/relay.sock',
      hostPlatform: getRemoteHostPlatform('linux-x64')
    })

    expect(plan.launcherPath).toBe('/home/me/.FABRICA-relay/bin/FABRICA')
    expect(plan.files).toEqual([
      expect.objectContaining({
        path: '/home/me/.FABRICA-relay/bin/FABRICA',
        contents: expect.stringContaining('--fabrica-cli "$@"')
      })
    ])
  })
})
