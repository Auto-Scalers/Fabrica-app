...
function expectNoFABRICAControlMasterArgs(args: string[]): void {
...
function expectFABRICAControlMasterArgs(args: string[]): void {
  if (process.platform === 'win32') {
    expectNoFABRICAControlMasterArgs(args)
    return
  }
...
describe('spawnSystemSsh', () => {
  ...
  it('passes explicit options for manual targets with implicit configHost', () => {
    const args = buildSshArgs(
      createTarget({
        ...
        host: '127.0.0.1',
        port: 2222,
        identityFile: '/tmp/FABRICA-docker-key',
        identitiesOnly: true
      })
    )

    expect(args).toEqual(expect.arrayContaining(['-p', '2222', '-i', '/tmp/FABRICA-docker-key']))
    expect(args).toContain('IdentitiesOnly=yes')
  ...
  it('does not inject FABRICA ControlMaster flags when ssh config already owns muxing', () => {
  ...
  it('injects FABRICA ControlMaster flags when ssh config only sets ControlPersist', () => {
    const args = buildSshArgs(createTarget({ configHost: 'workbox', source: 'ssh-config' }), {
      resolvedConfig: createResolvedConfig({
        controlMaster: 'no',
        controlPersist: '10m'
    ...
    expectFABRICAControlMasterArgs(args)
    expect(args).not.toContain('-S')
  })

  it('injects FABRICA ControlMaster flags when ssh config only sets ControlPath', () => {
    const args = buildSshArgs(createTarget({ configHost: 'workbox', source: 'ssh-config' }), {
      resolvedConfig: createResolvedConfig({
        controlMaster: 'no',
        controlPath: '/Users/me/.ssh/cm/%r@%h:%p'
  ...
  it('injects FABRICA ControlMaster flags when ssh config omits ControlPath', () => {
    const args = buildSshArgs(createTarget({ configHost: 'workbox', source: 'ssh-config' }), {
      resolvedConfig: createResolvedConfig({
        controlMaster: 'auto'
      })
  ...
  it('does not inject FABRICA ControlMaster flags for unresolved ssh-config targets', () => {
    const args = buildSshArgs(createTarget({ configHost: 'workbox', source: 'ssh-config' }))
  ...
  it('does not inject FABRICA ControlMaster flags for unresolved legacy config aliases', () => {
  ...
  it('can inject FABRICA ControlMaster flags for ssh-config targets with resolved config', () => {
    const args = buildSshArgs(createTarget({ configHost: 'workbox', source: 'ssh-config' }), {
      resolvedConfig: createResolvedConfig()
    })
  ...
  it('adds keepalive options to FABRICA-owned ControlMaster connections', () => {
  ...
  it('streams a local file through one POSIX system SSH command', async () => {
    ...
    spawnMock.mockReturnValue(proc)
    const dir = mkdtempSync(join(tmpdir(), 'FABRICA-system-ssh-upload-'))
  ...
  it('downloads files from POSIX system SSH targets', async () => {
    const proc = createEventedProcess()
    spawnMock.mockReturnValue(proc)
    const dir = mkdtempSync(join(tmpdir(), 'FABRICA-system-ssh-download-'))
  ...
  it('uploads directories to Windows system SSH targets in one PowerShell batch', async () => {
    const localDir = mkdtempSync(join(tmpdir(), 'FABRICA-system-ssh-upload-'))
    ...
    expect(payload).toEqual(
      expect.arrayContaining([
        { kind: 'directory', path: 'C:/Users/me/.FABRICA-remote/relay' },
        {
          kind: 'file',
          path: 'C:/Users/me/.FABRICA-remote/relay/relay.js',
