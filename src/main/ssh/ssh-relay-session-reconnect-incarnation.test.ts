...
vi.mock('./ssh-remote-fabrica-cli', () => ({
  runRemoteFABRICACli: vi.fn().mockResolvedValue({ exitCode: 0, stdout: '', stderr: '' })
}))
...
describe('SshRelaySession reconnect incarnation ordering', () => {
  ...
  it('keeps the winning reconnect incarnation when a stale health check resolves last', async () => {
    ...
    const winningCliHandler = muxInstances[2]?.requestHandlers.get('FABRICA.cli')
    ...
    expect(vi.mocked(runRemoteFABRICACli)).toHaveBeenCalledWith(
      runtime,
