...
async function deployAndLaunchRelayAttempt(
  ...
  if (!hostPlatform) {
    throw new Error(
      'Unsupported remote platform. FABRICA relay supports: linux-x64, linux-arm64, darwin-x64, darwin-arm64, win32-x64, win32-arm64.'
    )
  }
  ...
  if (!localRelayDir) {
    throw new Error(
      `Relay package for ${platform} not found locally. ` +
        `This may be a packaging issue — try reinstalling FABRICA.`
    )
  }
...
/**
 * A marker is only meaningful where a split namespace can occur and where FABRICA
...
const NATIVE_DEPS_MISSING_PREFIX = 'FABRICA-NATIVE-DEPS-MISSING:'
...
async function probeRequiredNativeDeps(
  ...
  const probeJs = nativeDepsProbeJs('FABRICA-NATIVE-DEPS-OK')
  try {
    ...
    const available = probe.includes('FABRICA-NATIVE-DEPS-OK')
...
async function probeInstalledNativeDeps(
  ...
  const PROBE_OK = 'FABRICA-NPTY-PROBE-OK'
...
export function getLocalRelayCandidates(platform: RelayPlatform): string[] {
  ...
  if (process.env.FABRICA_RELAY_PATH) {
    candidates.push(join(process.env.FABRICA_RELAY_PATH, platform))
  }
...
async function launchRelay(
  ...
  const escapedNode = shellEscape(nodePath)
  // Why: remoteRelayDir is shared across FABRICA targets for one account; hashing the target ID into the socket name stops cross-target attach.
