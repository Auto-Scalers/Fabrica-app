...
internal static class FABRICARemoteCliLauncher
{
    private static int Main(string[] args)
    {
        try
        {
            string nodePath = RequireEnvironmentVariable("FABRICA_RELAY_NODE_PATH");
            string relayDirectory = RequireEnvironmentVariable("FABRICA_RELAY_DIR");
            string socketPath = RequireEnvironmentVariable("FABRICA_RELAY_SOCKET_PATH");
            string credentialFile = Environment.GetEnvironmentVariable("FABRICA_RELAY_CREDENTIAL_FILE");
            ...
            {
                Console.Error.WriteLine("FABRICA SSH CLI bridge cannot find Node.js at \"{0}\"", nodePath);
                return 1;
            }
            if (!File.Exists(relayPath))
            {
                Console.Error.WriteLine("FABRICA SSH CLI bridge cannot find the relay at \"{0}\"", relayPath);
                return 1;
            }
        ...
        {
            Console.Error.WriteLine("Unable to start the Fabrica SSH CLI bridge: {0}", error.Message);
            return 1;
        }
    }
    ...
    private static string BuildArguments(string relayPath, string socketPath, string credentialFile, string[] args)
    {
        ...
        AppendArgument(commandLine, "--fabrica-cli");
...
function createWindowsLauncherCompileCommand(
  ...
  return powerShellCommand(
    [
      ...
      // host missing csc.exe keeps its existing CLI (FABRICA.exe shadows FABRICA.cmd).
...
export function createRemoteCliInstallPlan(env: RemoteCliInstallEnv): RemoteCliInstallPlan {
  if (isWindowsRemoteHost(env.hostPlatform)) {
    const launcherFileName = 'FABRICA.exe'
    const sourceFileName = 'FABRICA-launcher.cs'
    ...
    const legacyShimPath = joinRemotePath(env.hostPlatform, env.binDir, 'FABRICA.cmd')
  ...
  const launcherPath = joinRemotePath(env.hostPlatform, env.binDir, 'FABRICA')
  return {
    launcherPath,
    files: [
      {
        ...
        contents: [
          '#!/usr/bin/env sh',
          'set -eu',
          `FABRICA_RELAY_NODE_PATH=\${FABRICA_RELAY_NODE_PATH:-${quoteSh(env.nodePath)}}`,
          `FABRICA_RELAY_DIR=\${FABRICA_RELAY_DIR:-${quoteSh(env.relayDir)}}`,
          `FABRICA_RELAY_SOCKET_PATH=\${FABRICA_RELAY_SOCKET_PATH:-${quoteSh(env.sockPath)}}`,
          `FABRICA_RELAY_CREDENTIAL_FILE=\${FABRICA_RELAY_CREDENTIAL_FILE:-${quoteSh(env.credentialFile ?? `${env.sockPath}.credential`)}}`,
          'if [ ! -S "$FABRICA_RELAY_SOCKET_PATH" ]; then',
          '  echo "Fabrica SSH CLI bridge cannot find the relay socket: $FABRICA_RELAY_SOCKET_PATH" >&2',
          '  exit 1',
          'fi',
          'exec "$FABRICA_RELAY_NODE_PATH" "$FABRICA_RELAY_DIR/relay.js" --sock-path "$FABRICA_RELAY_SOCKET_PATH" --credential-file "$FABRICA_RELAY_CREDENTIAL_FILE" --fabrica-cli "$@"',
          ''
        ].join('\n')
      }
    ],
