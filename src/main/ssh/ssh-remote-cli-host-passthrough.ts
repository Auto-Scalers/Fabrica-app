// Why: the SSH relay shim (`~/.fabrica-relay/bin/FABRICA`) forwards CLI invocations
// to the host app. Instead of re-implementing every command in a hand-rolled
// switch (the cause of "Unsupported SSH FABRICA CLI command", #7716), the host
// runs the real bundled `FABRICA` CLI entry in Electron node mode — the same
...
export type RemoteFABRICACliRequest = {
  argv: string[]
  cwd: string
...
export type RemoteFABRICACliResult = {
  stdout: string
  stderr: string
  exitCode: number
  postOutput?: RemoteFABRICACliPostOutput
}

export type RemoteFABRICACliPostOutput =
  | {
...
// Why: only FABRICA terminal-context vars may cross from the remote shell into
// the host CLI process. Remote PATH / FABRICA_USER_DATA_PATH are paths on the
...
export function buildHostCliEnv(args: {
  ...
  env.FABRICA_USER_DATA_PATH = args.userDataPath
  // Why: the caller's working directory lives on the remote machine, so the
  // subprocess cwd cannot be chdir'd there; FABRICA_CLI_CWD carries it for
  // cwd-based selectors like `--worktree active`.
  env.FABRICA_CLI_CWD = args.remoteCwd
  ...
  env.FABRICA_NODE_OPTIONS = args.hostEnv.NODE_OPTIONS ?? ''
  env.FABRICA_NODE_REPL_EXTERNAL_MODULE = args.hostEnv.NODE_REPL_EXTERNAL_MODULE ?? ''
...
export async function runHostFABRICACliPassthrough(
  ...
  try {
    ...
    // to (see index.ts FABRICARuntimeRpcServer wiring), or the CLI subprocess
    // reports "FABRICA is not running" against a healthy app.
  ...
  if (!entryExists(cliEntryPath)) {
    throw new HostCliUnavailableError(`Fabrica CLI entry not found at ${cliEntryPath}`)
  }
  ...
  return await new Promise<RemoteFABRICACliResult>((resolve, reject) => {
    let settled = false
    ...
    const killTimer = setTimeout(() => {
      ...
      resolve({
        ...
        stderr: `${stderr.toString()}Fabrica CLI bridge timed out after ${killTimeoutMs}ms on the host.\n`,
        exitCode: 1
      })
    }, killTimeoutMs)
    killTimer.unref?.()
    ...
    child.on('error', (err) => {
      ...
      reject(
        new HostCliUnavailableError(`Failed to launch the Fabrica CLI on the host: ${err.message}`)
      )
    })
...
class CappedOutputCollector {
  ...
  toString(): string {
    ...
    return this.truncated ? `${text}\n[FABRICA ssh cli] output truncated\n` : text
  }
}
