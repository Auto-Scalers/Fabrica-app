...
vi.mock('./ssh-system-fallback', () => ({
  getFABRICAControlSocketPath: getFABRICAControlSocketPathMock,
...
function createSystemCommandChannel(): EventEmitter & {
  ...
  channel.close = vi.fn()
  queueMicrotask(() => {
    channel.emit('data', Buffer.from('FABRICA-SYSTEM-SSH-OK'))
...
function createSystemSshProcess() {
  ...
  queueMicrotask(() => {
    proc.stdout.emit('data', Buffer.from('FABRICA-SYSTEM-SSH-READY'))
  })
  return proc
}
...
describe('SshConnection', () => {
  beforeEach(() => {
    ...
    notifyClientCreated = undefined
    clientInstances = []
    getFABRICAControlSocketPathMock.mockReset()
    getFABRICAControlSocketPathMock.mockReturnValue(null)
  ...
  it('falls back to direct private key auth when agent auth fails', async () => {
    vi.stubEnv('SSH_AUTH_SOCK', '/tmp/agent.sock')
    const tempDir = mkdtempSync(join(tmpdir(), 'FABRICA-ssh-key-'))
  ...
  it('wraps exec commands as a single line that csh/tcsh login shells cannot break', async () => {
    ...
    expect(wrapped).toMatch(/^exec \/bin\/sh -c '.*printf %b .*' FABRICA-command /)
  ...
  it('allows concurrent exec commands for system SSH with an FABRICA ControlMaster socket', async () => {
    getFABRICAControlSocketPathMock.mockReturnValue('/tmp/FABRICA-ssh-501/live-socket')
    vi.mocked(resolveWithSshG).mockResolvedValueOnce(createResolvedConfig())
    const conn = new SshConnection(createTarget({ configHost: 'fdpass-host' }), createCallbacks())
  ...
  it('accepts GitHub restricted-shell SSH probes with resolved user fallback', async () => {
    ...
    spawnSystemSshCommandMock.mockImplementation(() =>
      createFailingSystemCommandChannel(1, 'Invalid command: echo FABRICA-SYSTEM-SSH-OK')
    )
    const conn = new SshConnection(
      createTarget({
  ...
  it('accepts GitHub restricted-shell SSH probes with the real git:// advisory transcript', async () => {
    ...
    spawnSystemSshCommandMock.mockImplementation(() =>
      createFailingSystemCommandChannel(
        1,
        'Invalid command: echo FABRICA-SYSTEM-SSH-OK\n' +
  ...
  it('accepts GitHub restricted-shell SSH probes when OpenSSH config resolution fails', async () => {
    vi.stubEnv('FABRICA_SSH_FORCE_SYSTEM_TRANSPORT', '1')
  ...
  it('retries a failed system SSH probe without ControlMaster and disables mux for the session', async () => {
    getFABRICAControlSocketPathMock.mockImplementation(
      (_target: SshTarget, options?: { disableControlMaster?: boolean }) =>
        options?.disableControlMaster ? null : '/tmp/FABRICA-ssh-501/stale-socket'
    )
    spawnSystemSshCommandMock
    ...
    expect(removeControlSocketPathMock).toHaveBeenCalledWith('/tmp/FABRICA-ssh-501/stale-socket')
  ...
  it('skips a second probe for a definite host failure', async () => {
    getFABRICAControlSocketPathMock.mockReturnValue('/tmp/FABRICA-ssh-501/stale-socket')
    spawnSystemSshCommandMock.mockImplementation(() =>
  ...
  it('uses system SSH before ssh2 parses a security-key private key', async () => {
    ...
    const directory = mkdtempSync(join(tmpdir(), 'FABRICA-security-key-connect-'))
  ...
  it('uses system SSH for an agent-backed security-key public identity', async () => {
    findSystemSshMock.mockReturnValue('/usr/bin/ssh')
    const directory = mkdtempSync(join(tmpdir(), 'FABRICA-security-key-agent-connect-'))
  ...
  it('passes the detected host platform to system SSH file operations', async () => {
    ...
    await conn.connect()
    await conn.uploadDirectory('/tmp/local-relay', 'C:/Users/me/.FABRICA-remote/relay', {
      hostPlatform
    })
    await conn.writeFile('C:/Users/me/.FABRICA-remote/relay/.version', '0.1.0', {
      hostPlatform
    })
    await conn.writeBuffer('C:/Users/me/.FABRICA-remote/relay/logo.png', Buffer.from('png'), {
      hostPlatform,
      exclusive: true
    })
    await conn.downloadFile('C:/Users/me/.FABRICA-remote/relay/logo.png', '/tmp/logo.png', {
      hostPlatform
    })
    ...
    expect(uploadDirectoryViaSystemSsh).toHaveBeenCalledWith(
      expect.objectContaining({ configHost: 'fdpass-host' }),
      '/tmp/local-relay',
      'C:/Users/me/.FABRICA-remote/relay',
      expect.objectContaining({
        hostPlatform,
        resolvedConfig: expect.objectContaining({ proxyUseFdpass: true })
      })
    ...
    expect(writeFileViaSystemSsh).toHaveBeenCalledWith(
      expect.objectContaining({ configHost: 'fdpass-host' }),
      'C:/Users/me/.FABRICA-remote/relay/.version',
      '0.1.0',
      expect.objectContaining({
        hostPlatform,
        resolvedConfig: expect.objectContaining({ proxyUseFdpass: true })
    ...
    expect(writeBufferViaSystemSsh).toHaveBeenCalledWith(
      expect.objectContaining({ configHost: 'fdpass-host' }),
      'C:/Users/me/.FABRICA-remote/relay/logo.png',
      Buffer.from('png'),
      expect.objectContaining({
        hostPlatform,
        exclusive: true,
