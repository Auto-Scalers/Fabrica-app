...
describe('buildHostCliEnv', () => {
  it('forwards only FABRICA terminal-context vars from the remote env', () => {
    const env = buildHostCliEnv({
      hostEnv: { PATH: '/host/bin', NODE_OPTIONS: '--inspect' },
      remoteEnv: {
        FABRICA_TERMINAL_HANDLE: 'term_remote',
        FABRICA_WORKTREE_ID: 'repo::/home/alice/wt',
        FABRICA_PANE_KEY: 'pane-9',
        FABRICA_AGENT_LAUNCH_TOKEN: 'launch-secret',
        FABRICA_WORKSPACE_ID: 'ws-1',
        ...
        PATH: '/remote/bin',
        FABRICA_USER_DATA_PATH: '/remote/user-data'
      },
      userDataPath: '/host/user-data',
    ...
    expect(env.FABRICA_TERMINAL_HANDLE).toBe('term_remote')
    expect(env.FABRICA_WORKTREE_ID).toBe('repo::/home/alice/wt')
    expect(env.FABRICA_PANE_KEY).toBe('pane-9')
    expect(env.FABRICA_AGENT_LAUNCH_TOKEN).toBe('launch-secret')
    expect(env.FABRICA_WORKSPACE_ID).toBe('ws-1')
    ...
    expect(env.FABRICA_USER_DATA_PATH).toBe('/host/user-data')
    expect(env.FABRICA_CLI_CWD).toBe('/home/alice/wt/sub')
    expect(env.ELECTRON_RUN_AS_NODE).toBe('1')
    expect(env.NODE_OPTIONS).toBeUndefined()
    expect(env.FABRICA_NODE_OPTIONS).toBe('--inspect')
  })
...
describe('runHostFABRICACliPassthrough', () => {
  it('spawns the bundled CLI entry with the remote argv and returns captured output', async () => {
    ...
    expect(options.env.FABRICA_CLI_CWD).toBe('/home/alice/wt')
    expect(options.env.FABRICA_TERMINAL_HANDLE).toBe('term_remote')
