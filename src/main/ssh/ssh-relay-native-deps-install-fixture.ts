...
export function makeStagedFirstInstallExecPrefix(): ExecResponse[] {
  return [
    ...
    `__FABRICA_UPLOAD_STAGE_SLOT__${STAGE_OWNER}:slot-0`,
    '', // chmod staged node
    '', // final install namespace marker
    `__FABRICA_UPLOAD_STAGE_PROMOTION__${STAGE_OWNER}:PROMOTED`
  ]
}
...
export function makeRepairToolchainSkipExecResponses(): ExecResponse[] {
  const bothMissing = 'FABRICA-NATIVE-DEPS-MISSING:node-pty,@parcel/watcher\nMISSING'
  return [
    '__FABRICA_REMOTE_PLATFORM__ Linux x86_64',
    '/home/u',
...
export function makeExecResponses(opts: {
  ...
  if (opts.npmInstall !== 'ok') {
    ...
    return [
      ...
      opts.nodePtySkipWatcher === 'missing'
        ? 'FABRICA-NATIVE-DEPS-MISSING:node-pty,@parcel/watcher\nMISSING\n'
        : 'FABRICA-NATIVE-DEPS-MISSING:node-pty\nMISSING\n',
      '', // cat probe stderr
      '', // rm -f probe stderr
      '', // clean stage root
  ...
  if (probeResolved) {
    const probeOk = probeSlot.includes('FABRICA-NPTY-PROBE-OK')
    if (!probeOk) {
    ...
    if (!probeOk) {
      ...
      const repairProbe = opts.repairProbe === 'ok' ? 'FABRICA-NPTY-PROBE-OK\n' : 'MISSING\n'
      slots.push(repairProbe)
      if (!repairProbe.includes('FABRICA-NPTY-PROBE-OK')) {
