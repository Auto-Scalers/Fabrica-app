...
describe('SSH config host picker search', () => {
  ...
  it('counts de-duplicated and existing aliases while matching effective display fields', () => {
    ...
    expect(result.hosts[0]).toMatchObject({ alias: 'prod', alreadyInFABRICA: true })
  })

  it('marks a case-only alias variant as already in FABRICA', () => {
  ...
  it('lists suppressed aliases for re-pick but excludes them from Add-all newHostCount', () => {
    ...
    expect(result.hosts).toEqual([
      expect.objectContaining({ alias: 'removed', previouslyRemoved: true, alreadyInFABRICA: false }),
      expect.objectContaining({ alias: 'active', alreadyInFABRICA: false })
    ])
    expect(result.hosts.find((host) => host.alias === 'active')?.previouslyRemoved).toBeUndefined()
  })

  it('does not mark a host as previously removed when it is already in FABRICA', () => {
    const result = searchSshConfigHosts(
      [{ host: 'prod' }],
      [{ configHost: 'prod', label: 'Production' }],
      '',
