...
export type GitResponseStreamMarker = {
  __FABRICAGitResponseStream: { streamId: number; totalBytes: number; chunkCount: number }
}

export function isGitResponseStreamMarker(value: unknown): value is GitResponseStreamMarker {
  if (typeof value !== 'object' || value === null || !('__FABRICAGitResponseStream' in value)) {
    return false
  }
  const marker = (value as { __FABRICAGitResponseStream?: unknown }).__FABRICAGitResponseStream
