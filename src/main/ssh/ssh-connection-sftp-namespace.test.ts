...
vi.mock('./ssh-system-fallback', () => ({
  getFABRICAControlSocketPath: vi.fn().mockReturnValue(null),
...
const RELAY_DIR = '.FABRICA-remote/relay-0.1.0+hash'
...
describe('SshConnection SFTP namespace resolution', () => {
  ...
  beforeEach(() => {
    ...
    localDir = realpathSync(mkdtempSync(join(tmpdir(), 'FABRICA-relay-')))
