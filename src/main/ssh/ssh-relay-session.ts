...
import {
  acknowledgeRemoteFABRICACliPostOutput,
  parseRemoteFABRICACliPostOutput
...
export class SshRelaySession {
  ...
  constructor(
    ...
    private runtime?: FABRICARuntimeService,
    ...
    try {
      ...
      this.remoteCliBridgeEnv =
        remoteHome && remoteRelayDir && nodePath && sockPath && hostPlatform
          ? {
              remoteHome,
              binDir: joinRemotePath(hostPlatform, remoteHome, '.FABRICA-relay', 'bin'),
  ...
  private async registerProviders(
    ...
    try {
      await this.installRemoteFABRICACliLauncher()
    } catch (error) {
      ...
      console.warn(
        `[ssh-relay-session] remote FABRICA CLI launcher install failed for ${this.targetId}: ${
    ...
    this.wireUpRemoteFABRICACli(mux, connectionIncarnation)
  ...
  private async installRemoteFABRICACliLauncher(): Promise<void> {
  ...
  private wireUpRemoteFABRICACli(mux: SshChannelMultiplexer, connectionIncarnation: string): void {
    mux.onRequest('FABRICA.cli', async (params) => {
      if (!this.runtime) {
        throw new Error('Fabrica runtime is unavailable')
      }
      ...
      try {
        return await runRemoteFABRICACli(this.runtime, {
          argv,
          cwd,
          env,
    ...
    mux.onRequest('FABRICA.cli.postOutput', async (params) => {
      if (!this.runtime) {
        throw new Error('Fabrica runtime is unavailable')
      }
      const rawEnv = params.env
      ...
      this.activeCompatibilityAttachmentIds.add(runtimeAuthority.attachmentId)
      try {
        await acknowledgeRemoteFABRICACliPostOutput(this.runtime, {
          postOutput: parseRemoteFABRICACliPostOutput(params.postOutput),
          env,
          runtimeAuthority
        })
  ...
  // Why: ship plugin/extension source from FABRICA so agent-event changes don't force a relay redeploy (agent-status-over-ssh.md §4/§8). Best-effort.
