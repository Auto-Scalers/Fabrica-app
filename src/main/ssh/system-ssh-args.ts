...
export type SystemSshBuildArgsOptions = {
  ...
  suppressFABRICAControlMaster?: boolean
  gssapiOnly?: boolean
}
...
export function buildSshArgs(target: SshTarget, options?: SystemSshBuildArgsOptions): string[] {
  ...
  if (options?.gssapiOnly) {
    ...
    // credential prompt; failure belongs to FABRICA's existing ssh2 prompt path.
  ...
  const controlPath = getFABRICAControlSocketPath(target, options)
...
export function getFABRICAControlSocketPath(
  target: SshTarget,
  options?: SystemSshBuildArgsOptions
): string | null {
  if (shouldDisableFABRICAControlMaster(target, options)) {
    return null
  }
...
export function getSystemSshBuildArgsFromOperationOptions(
  ...
  if (options?.suppressFABRICAControlMaster === true) {
    buildArgsOptions.suppressFABRICAControlMaster = true
  }
...
function shouldDisableFABRICAControlMaster(
  target: SshTarget,
  options?: SystemSshBuildArgsOptions
): boolean {
  // Why: unresolved ssh_config aliases could otherwise share one FABRICA socket
  ...
  return (
    options?.disableControlMaster === true ||
    options?.suppressFABRICAControlMaster === true ||
    target.systemSshConnectionReuse === false ||
...
function hasUserConfiguredControlMaster(
  ...
  // created, but they do not create the setup-burst master FABRICA needs.
  return (
