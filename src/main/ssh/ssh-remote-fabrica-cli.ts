...
import {
  ...
  runHostFABRICACliPassthrough,
  type HostCliPassthroughOptions,
  type RemoteFABRICACliRequest,
  type RemoteFABRICACliResult
...
export type { RemoteFABRICACliRequest, RemoteFABRICACliResult } from './ssh-remote-cli-host-passthrough'
...
const HOST_INTERACTIVE_COMMANDS: Record<string, string> = {
  serve:
    'FABRICA serve starts a foreground headless Fabrica server and cannot run through the SSH relay bridge. Run it directly on the machine that should host Fabrica.',
  'claude-teams':
    'FABRICA claude-teams starts an interactive Claude Code session and cannot run through the SSH relay bridge. Run it in a terminal on the Fabrica host machine.',
  'agent-teams-tmux':
    'FABRICA agent-teams-tmux is a tmux pane shim for the Fabrica host machine and cannot run through the SSH relay bridge.',
  'account add':
    'FABRICA account add runs an interactive agent login and cannot run through the buffered SSH relay bridge. Run it directly in a terminal on the Fabrica host machine.'
}

export async function runRemoteFABRICACli(
  runtime: FABRICARuntimeService,
  request: RemoteFABRICACliRequest,
  passthroughOptions?: HostCliPassthroughOptions
): Promise<RemoteFABRICACliResult> {
  ...
  if (command === 'orchestration check' || command === 'orchestration ask') {
    ...
    return await runLegacyRemoteFABRICACli(
      runtime,
      request,
      parsed,
      json,
  ...
  try {
    return await runHostFABRICACliPassthrough(request, passthroughOptions)
  } catch (err) {
  ...
  return await runLegacyRemoteFABRICACli(runtime, request, parsed, json, passthroughFailure)
}

async function runLegacyRemoteFABRICACli(
  runtime: FABRICARuntimeService,
  request: RemoteFABRICACliRequest,
...
async function dispatchRemoteCli(
  ...
  runtimeAuthority: RemoteFABRICACliRequest['runtimeAuthority']
): Promise<RpcResponse> {
  const command = parsed.commandPath.join(' ')
  ...
  switch (command) {
    ...
    case 'orchestration send': {
      ...
      return await call(
        ...
        'orchestration.send',
        {
          ...
          senderPaneKey: env.FABRICA_PANE_KEY || undefined
        },
        {
      ...
      return await call(
        ...
        'orchestration.check',
        {
          ...
          terminalPaneKey: parsed.flags.has('terminal')
            ? undefined
            : env.FABRICA_PANE_KEY || undefined,
          ...
          compatibilityCliCommand: 'FABRICA',
    ...
    default:
      ...
      throw new Error(
        `Unsupported SSH FABRICA CLI command: ${command} (full FABRICA CLI bridge unavailable: ${passthroughFailureReason})`
      )
  }
}
