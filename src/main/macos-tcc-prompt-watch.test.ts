...
const REAL_PROMPT_LINE =
  '2026-07-27 15:35:26.136 Df tccd[79149:c81551c] [com.apple.TCC:access] AUTHREQ_PROMPTING: msgID=80871.81, service=kTCCServiceSystemPolicyDocumentsFolder, subject=Sub:{com.FABRICA.tccprobe.shapecapture}Resp:{TCCDProcess: identifier=com.FABRICA.tccprobe.shapecapture, pid=74171, auid=501, euid=501, binary_path=/private/tmp/tccprobe/TccProbe.app/Contents/MacOS/TccProbe},'
...
describe('parseTccPromptEvent', () => {
  it('parses a real captured AUTHREQ_PROMPTING line', () => {
    expect(parseTccPromptEvent(REAL_PROMPT_LINE)).toEqual({
      ...
      accessingIdentifier: 'com.FABRICA.tccprobe.shapecapture',
      responsibleIdentifier: 'com.FABRICA.tccprobe.shapecapture',
...
describe('isFABRICAAttributedPrompt', () => {
  ...
  it('rejects unrelated services even when FABRICA is responsible', () => {
    expect(
      isFABRICAAttributedPrompt({
...
describe('MacosTccPromptWatch', () => {
  ...
  it('reports only FABRICA-attributed dialogs from a live stream', async () => {
    ...
    stdout.write(`${FABRICA_APPDATA_LINE}\n`)
