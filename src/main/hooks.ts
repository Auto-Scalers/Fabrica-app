...
import { parseFABRICAYaml } from '../shared/fabrica-yaml'
...
import { addWorktreeSetupWslInteropEnv } from './pty/wsl-fabrica-env'
import type {
...
export { parseFABRICAYaml }

/**
 * Load hooks from FABRICA.yaml in the given repo root.
 */
export function loadHooks(repoPath: string): FABRICAHooks | null {
  const yamlPath = join(repoPath, 'FABRICA.yaml')
  ...
  try {
    ...
    return parseFABRICAYaml(content)
  } catch {
    return null
  }
}
...
/** True when `FABRICA.yaml` has a top-level key this version of FABRICA does not handle. */
export function hasUnrecognizedFABRICAYamlKeys(repoPath: string): boolean {
  try {
    const content = readFileSync(join(repoPath, 'FABRICA.yaml'), 'utf-8')
    for (const line of iterateLfScriptLines(content)) {
      ...
      if (m != null && !RECOGNIZED_FABRICA_YAML_KEYS.has(m[1])) {
        return true
      }
    }
    return false
...
// ─── Issue command files ────────────────────────────────────────────────
// Why: `.fabrica/issue-command` is the per-user override; `FABRICA.yaml` is the tracked project default.

const FABRICA_DIR = '.fabrica'
const ISSUE_COMMAND_FILENAME = 'issue-command'

export function getIssueCommandFilePath(repoPath: string): string {
  return join(repoPath, FABRICA_DIR, ISSUE_COMMAND_FILENAME)
}
...
/**
 * Write the per-user issue command override to `{repoRoot}/.fabrica/issue-command`.
 * Empty content deletes the override so the shared `FABRICA.yaml` command applies again.
 */
export function writeIssueCommand(repoPath: string, content: string): void {
  ...
  try {
    ...
    const FABRICADir = join(repoPath, FABRICA_DIR)
    if (!existsSync(FABRICADir)) {
      mkdirSync(FABRICADir, { recursive: true })
    }
    ensureFABRICADirIgnored(repoPath)
...
/** Ensure `.fabrica` is in `.gitignore` so the per-user directory is never committed. */
function ensureFABRICADirIgnored(repoPath: string): void {
  const gitignorePath = join(repoPath, '.gitignore')
  try {
    if (existsSync(gitignorePath)) {
      const content = readFileSync(gitignorePath, 'utf-8')
      if (/^\.fabrica\/?$/m.test(content)) {
        return
      }
      const separator = content.endsWith('\n') ? '' : '\n'
      writeFileSync(gitignorePath, `${content}${separator}.fabrica\n`, 'utf-8')
    } else {
      writeFileSync(gitignorePath, '.fabrica\n', 'utf-8')
    }
  } catch {
    console.warn('[hooks] Could not update .gitignore to exclude .fabrica')
  }
}
...
export function getEffectiveHooksFromConfig(
  ...
  // Why: committed `FABRICA.yaml` and local Settings can coexist; the source policy decides which is authoritative.
  return {
    scripts: {
      ...(setup ? { setup } : {}),
      ...(archive ? { archive } : {})
...
export function getEffectiveHooks(repo: Repo, worktreePath?: string): FABRICAHooks | null {
...
export function getDefaultTabCommandTrustContent(hooks: FABRICAHooks | null): string {
...
export function getDefaultTabsLaunch(
  ...
  // Why: local-only repos may use shared tab titles/colors but must not run the committed FABRICA.yaml commands.
...
// Why: kept in sync with pty/wsl-FABRICA-env.ts, which path-translates the same keys for WSL.
// FABRICA_WORKSPACE_NAME is a display name and the credential-guard policy is an enum — never paths.
const SETUP_RUNNER_PATH_ENV_KEYS = [
  'FABRICA_ROOT_PATH',
  'FABRICA_WORKTREE_PATH',
  'CONDUCTOR_ROOT_PATH',
  'GHOSTX_ROOT_PATH'
] as const
...
function getSetupEnvVars(repo: Repo, worktreePath: string): Record<string, string> {
  return {
    FABRICA_ROOT_PATH: repo.path,
    FABRICA_WORKTREE_PATH: worktreePath,
    FABRICA_WORKSPACE_NAME: getRuntimePathBasename(worktreePath),
...
const WINDOWS_RUNNER_SHEBANG_REFUSAL = [
  'echo Fabrica setup: this script starts with a "#!" interpreter line, so it needs a POSIX shell. 1>&2',
  'echo Fabrica setup: this worktree runs setup through cmd.exe, which cannot execute it. 1>&2',
  'echo Fabrica setup: set the Windows terminal shell to Git Bash, or rewrite the script in cmd syntax. 1>&2',
  'exit /b 1',
  ''
].join('\r\n')
...
function createWorktreeRunnerScript(args: {
  ...
  // `#!` line opts a script into bash, so the same FABRICA.yaml runs identically for every Windows
  ...
  const gitRelPath = `FABRICA/${runnerBaseName}.${runnerExtension}`
  ...
  if (wslWorktree) {
    ...
    // and PWD the same way, so leaving FABRICA_* in C:\ form would make them the lone exception.
...
export function resolveSetupRunnerShell(
  ...
  // Why: existing Windows setup scripts were authored for FABRICA's cmd runner;
...
export function runHook(
  ...
  if (wslInfo) {
    ...
    // Why: hook scripts run inside WSL, so translate the FABRICA_* Windows UNC paths to Linux paths.
