...
export function registerGitHubHandlers(store: Store, stats: StatsCollector): void {
  ...
  ipcMain.handle(
    ...
    async (
      ...
      // This captures PRs opened from any workflow (FABRICA UI, gh CLI, github.com).
  ...
  // Star operations target the FABRICA repo itself — no repoPath validation needed
  ipcMain.handle('gh:viewer', () => getAuthenticatedViewer())
  ipcMain.handle('gh:checkFABRICAStarred', () => checkFABRICAStarred())
  ipcMain.handle('gh:starFABRICA', async (_event, source: unknown) => {
    const sourceParse = appStarSourceSchema.safeParse(source)
    const starred = await starFABRICA()
