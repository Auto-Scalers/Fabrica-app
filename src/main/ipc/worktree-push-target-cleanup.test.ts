...
const FORK_URL = 'git@github.com:contributor/FABRICA.git'
const FORK_REMOTE = 'pr-contributor-FABRICA'
...
describe('cleanupUnusedWorktreePushTargetRemoteWithExec', () => {
  it('removes an FABRICA-created fork remote that nothing else uses', async () => {
    const exec = makeExec()
  ...
  it('keeps a remote FABRICA did not create (remoteCreated falsy)', async () => {
    const exec = makeExec()
    await cleanupUnusedWorktreePushTargetRemoteWithExec(
      REPO_PATH,
      'repo-1::/wt/a',
  ...
  it('keeps the remote when its URL no longer matches the fork (repurposed by the user)', async () => {
    const exec = makeExec({ getUrl: 'git@github.com:someone-else/FABRICA.git' })
    await cleanupUnusedWorktreePushTargetRemoteWithExec(
      REPO_PATH,
      'repo-1::/wt/a',
      forkTarget(),
...
describe('sameGitHubRemoteUrl', () => {
  it('matches SSH and HTTPS forms of the same GitHub fork', () => {
    expect(
      sameGitHubRemoteUrl(
        'git@github.com:contributor/FABRICA.git',
        'https://github.com/contributor/FABRICA.git'
      )
    ).toBe(true)
  })
  ...
  it('does not match different forks', () => {
    expect(
      sameGitHubRemoteUrl(
        'git@github.com:contributor/FABRICA.git',
        'git@github.com:someone-else/FABRICA.git'
      )
    ).toBe(false)
  })
  ...
  it('falls back to exact equality for non-GitHub hosts', () => {
    expect(
      sameGitHubRemoteUrl(
        'git@gitlab.com:contributor/FABRICA.git',
        'git@gitlab.com:contributor/FABRICA.git'
      )
    ).toBe(true)
    expect(
      sameGitHubRemoteUrl(
        'git@gitlab.com:contributor/FABRICA.git',
        'https://gitlab.com/contributor/FABRICA.git'
      )
    ).toBe(false)
  })
})
