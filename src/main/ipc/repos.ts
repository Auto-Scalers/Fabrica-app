...
type RepoRemoteClientNotifier = Pick<FABRICARuntimeService, 'notifyReposChangedForRemoteClients'>
...
function getDefaultCreateProjectParent(): string {
  return join(homedir(), 'FABRICA', 'projects')
}
...
export function registerRepoHandlers(mainWindow: BrowserWindow, store: Store): void {
  ...
  // Create a repo/folder from scratch (FABRICA#763); git repos need an empty initial commit so HEAD has a branch ref for worktrees.
  ipcMain.handle(
    'repos:create',
    async (
      _event,
      ...
      let targetExists = false
      try {
        // Why: the default parent (~/FABRICA/projects) may not exist on a fresh install; create only the parent before probing the target.
