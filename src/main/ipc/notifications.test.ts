...
describe('registerNotificationHandlers', () => {
  ...
  beforeEach(() => {
    ...
    tempDir = mkdtempSync(join(tmpdir(), 'FABRICA-notification-test-'))
  ...
  it('opens the current macOS app notification settings entry', async () => {
    ...
    const originalBundleId = process.env.FABRICA_DEV_MACOS_BUNDLE_ID
    Object.defineProperty(process, 'platform', { value: 'darwin', configurable: true })
    process.env.FABRICA_DEV_MACOS_BUNDLE_ID = 'com.autoscalers.fabrica.dev.fb5a47066f08'
    try {
      registerNotificationHandlers({
        getSettings: () => ({
          notifications: {
      ...
      expect(shellOpenExternalMock).toHaveBeenCalledWith(
        'x-apple.systempreferences:com.apple.Notifications-Settings.extension?id=com.autoscalers.fabrica.dev.fb5a47066f08'
      )
    } finally {
      Object.defineProperty(process, 'platform', { value: originalPlatform, configurable: true })
      if (originalBundleId === undefined) {
        delete process.env.FABRICA_DEV_MACOS_BUNDLE_ID
      } else {
        process.env.FABRICA_DEV_MACOS_BUNDLE_ID = originalBundleId
      }
    }
  })
  ...
  it('suppresses active-worktree notifications while FABRICA is focused', async () => {
    getAllWindowsMock.mockReturnValue([
      {
        isDestroyed: () => false,
        isFocused: () => true
  ...
  it('delivers a notification when the event is allowed', async () => {
    ...
    expect(
      await handler(
        {},
        { source: 'agent-task-complete', repoLabel: 'FABRICA', worktreeLabel: 'feat/notis' }
      )
  ...
  it('uses the macOS default notification sound when no custom sound is configured', async () => {
    ...
    try {
      ...
      expect(notificationCtorMock).toHaveBeenCalledWith({
        title: 'Fabrica notifications are on',
        body: 'This is a test notification from Fabrica.',
        sound: 'default'
      })
    } finally {
      Object.defineProperty(process, 'platform', { value: originalPlatform, configurable: true })
  ...
  it('includes the repo name when multiple repos are active', async () => {
    ...
    expect(notificationCtorMock).toHaveBeenCalledWith(
      expectedNativeNotificationOptions({
        title: 'FABRICA / feat/notis - Codex finished',
        body: 'Updated the notification body.'
      })
    )
  })
  ...
  it('skips native delivery and reports blocked-by-system when macOS would swallow it', async () => {
    ...
    try {
      ...
      // Notification Center delivered list â€” skip creating it entirely.
      expect(notificationCtorMock).not.toHaveBeenCalled()
    } finally {
      Object.defineProperty(process, 'platform', { value: originalPlatform, configurable: true })
    }
...
describe('notifications:probeDelivery', () => {
  ...
  function getProbeOnceEventHandler(eventName: string): (...args: unknown[]) => void {
    // Why: findLast â€” a test may run several probes, and only the newest
...
describe('triggerStartupNotificationRegistration', () => {
  ...
  it('shows welcome notification when not yet requested', async () => {
    ...
    expect(store.updateUI).toHaveBeenCalledWith({ notificationPermissionRequested: true })
    expect(notificationCtorMock).toHaveBeenCalledWith({
      title: 'Fabrica is ready to notify you',
      body: 'Allow notifications so Fabrica can alert you when agents finish or terminals need attention.'
    })
    expect(notificationShowMock).toHaveBeenCalledTimes(1)
  })
