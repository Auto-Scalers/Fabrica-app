...
describe('tui agent detection commands', () => {
  it('requires Claude before reporting Claude Agent Teams', () => {
    ...
    expect(resolveDetectedTuiAgentIds(commands, new Set(['FABRICA']), 'linux')).toEqual([])
    expect(resolveDetectedTuiAgentIds(commands, new Set(['FABRICA', 'claude']), 'linux')).toEqual([
      'claude-agent-teams'
    ])
    expect(getTuiAgentDetectionProbeCommands(commands, 'win32')).toEqual([])
    expect(resolveDetectedTuiAgentIds(commands, new Set(['FABRICA', 'claude']), 'win32')).toEqual([])
    expect(getTuiAgentDetectionProbeCommands(commands, 'wsl')).toEqual([])
    expect(resolveDetectedTuiAgentIds(commands, new Set(['FABRICA-ide', 'claude']), 'wsl')).toEqual([])
  })
})
