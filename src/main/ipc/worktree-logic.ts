...
import type { GlobalSettings, FABRICAWorkspaceLayout, Repo } from '../../shared/types'
 ...
 * Windows workspace layout inside ~/FABRICA/workspaces on the WSL filesystem
 * (e.g. \\wsl.localhost\Ubuntu\home\user\FABRICA\workspaces\repo\feature).
 */
...
export function computeWorkspaceRoot(repoPath: string, settings: { workspaceDir: string }): string {
  ...
  if (wsl && shouldMirrorWorkspaceDirInsideWsl(repoPath, settings.workspaceDir)) {
    ...
    if (wslHome) {
      ...
      return win32.join(wslHome, 'FABRICA', 'workspaces')
    }
  }
