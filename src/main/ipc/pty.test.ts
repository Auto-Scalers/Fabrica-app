...
const expectedOmpStatusExtension = posix.join(
  ...
  'FABRICA-agent-status.ts'
)
function expectedAttributionShimDir(): string {
  return join(
    '/tmp/FABRICA-user-data',
    'FABRICA-terminal-attribution',
...
vi.mock('electron', () => ({
  // Why defined-but-undefined: the real FABRICARuntimeService guards BrowserWindow with `?.`; vitest throws on reading exports the mock omits.
...
vi.mock('../cli/linux-terminal-fabrica-cli-shim', () => ({
  ensureLinuxTerminalFABRICACliShimDir: (options: { userDataPath: string }) =>
    join(options.userDataPath, 'linux-FABRICA-cli-shim')
}))
...
import { FABRICARuntimeService } from '../runtime/fabrica-runtime'
...
describe('registerPtyHandlers', () => {
  ...
  const savedFABRICAOpenCodeConfigDir = process.env.FABRICA_OPENCODE_CONFIG_DIR
  const savedFABRICAOpenCodeSourceConfigDir = process.env.FABRICA_OPENCODE_SOURCE_CONFIG_DIR
  const savedPiAgentDir = process.env.PI_CODING_AGENT_DIR
  const savedFABRICAPiAgentDir = process.env.FABRICA_PI_CODING_AGENT_DIR
  const savedFABRICAPiSourceAgentDir = process.env.FABRICA_PI_SOURCE_AGENT_DIR
  const savedFABRICACodexHome = process.env.FABRICA_CODEX_HOME
  const savedFABRICAOmpAgentDir = process.env.FABRICA_OMP_CODING_AGENT_DIR
  const savedFABRICAOmpSourceAgentDir = process.env.FABRICA_OMP_SOURCE_AGENT_DIR
  const savedFABRICAOmpStatusExtension = process.env.FABRICA_OMP_STATUS_EXTENSION
  const savedPrimeAgentDir = process.env.PRIME_AGENT_CODING_AGENT_DIR
  const savedFABRICAPrimeAgentSourceDir = process.env.FABRICA_PRIME_AGENT_SOURCE_AGENT_DIR
  const savedFABRICAPrimeAgentStatusExtension = process.env.FABRICA_PRIME_AGENT_STATUS_EXTENSION
  const savedFABRICAClaudeAgentStatusSettings = process.env.FABRICA_CLAUDE_AGENT_STATUS_SETTINGS
  const savedProcessPlatform = Object.getOwnPropertyDescriptor(process, 'platform')
  const savedDisableMacosLoginShell = process.env.FABRICA_DISABLE_MACOS_LOGIN_SHELL
  const savedFABRICAUserDataPath = process.env.FABRICA_USER_DATA_PATH
  ...
  beforeEach(() => {
    ...
    process.env.FABRICA_DISABLE_MACOS_LOGIN_SHELL = '1'
    delete process.env.OPENCODE_CONFIG_DIR
    delete process.env.FABRICA_OPENCODE_SOURCE_CONFIG_DIR
    delete process.env.FABRICA_OPENCODE_CONFIG_DIR
    delete process.env.FABRICA_AGENT_HOOK_ENDPOINT
    delete process.env.FABRICA_CLAUDE_AGENT_STATUS_SETTINGS
    delete process.env.PI_CODING_AGENT_DIR
    delete process.env.FABRICA_PI_SOURCE_AGENT_DIR
    delete process.env.FABRICA_PI_CODING_AGENT_DIR
    delete process.env.FABRICA_CODEX_HOME
    delete process.env.FABRICA_OMP_SOURCE_AGENT_DIR
    delete process.env.FABRICA_OMP_CODING_AGENT_DIR
    delete process.env.FABRICA_OMP_STATUS_EXTENSION
    delete process.env.PRIME_AGENT_CODING_AGENT_DIR
    delete process.env.FABRICA_PRIME_AGENT_SOURCE_AGENT_DIR
    delete process.env.FABRICA_PRIME_AGENT_STATUS_EXTENSION
    handlers.clear()
    ...
    getPathMock.mockReturnValue('/tmp/FABRICA-user-data')
    // Why: wrapper roots resolve from FABRICA_USER_DATA_PATH; mirror the mocked userData so ZDOTDIR/wrapper assertions match.
    process.env.FABRICA_USER_DATA_PATH = '/tmp/FABRICA-user-data'
    existsSyncMock.mockReturnValue(true)
    statSyncMock.mockReturnValue({ isDirectory: () => true, mode: 0o755 })
    readFileSyncMock.mockReturnValue('')
    openCodeBuildPtyEnvMock.mockImplementation((_ptyId: string, existingConfigDir?: string) => ({
      FABRICA_OPENCODE_HOOK_PORT: '4567',
      FABRICA_OPENCODE_HOOK_TOKEN: 'opencode-token',
      FABRICA_OPENCODE_PTY_ID: 'test-pty',
      OPENCODE_CONFIG_DIR: existingConfigDir
        ? '/tmp/FABRICA-opencode-overlay'
        : '/tmp/FABRICA-opencode-config'
    }))
    mimoCodeBuildPtyEnvMock.mockImplementation((_ptyId: string, existingHome?: string) => ({
      MIMOCODE_HOME: existingHome ? '/tmp/FABRICA-mimocode-overlay' : '/tmp/FABRICA-mimocode-shared'
    }))
    buildAgentHookEnvMock.mockReturnValue({
      FABRICA_AGENT_HOOK_PORT: '5678',
      FABRICA_AGENT_HOOK_TOKEN: 'agent-token'
    })
      ...
      (
        ...
        if (kind === 'omp') {
          ...
          if (!existingAgentDir && !materializeDefaultHome) {
            return {
              FABRICA_OMP_STATUS_EXTENSION:
                '/tmp/FABRICA-user-data/omp-managed-status-extension/FABRICA-agent-status.ts'
            }
          }
          return {
            FABRICA_OMP_SOURCE_AGENT_DIR: existingAgentDir ?? '/tmp/default-omp-agent',
            FABRICA_OMP_STATUS_EXTENSION: `${existingAgentDir ?? '/tmp/default-omp-agent'}/extensions/FABRICA-agent-status.ts`
          }
        }
        if (kind === 'prime-agent') {
          if (!existingAgentDir && !materializeDefaultHome) {
          ...
          return {
            FABRICA_PRIME_AGENT_SOURCE_AGENT_DIR: existingAgentDir ?? '/tmp/default-prime-agent'
          }
        }
        if (!existingAgentDir && !materializeDefaultHome) {
          return {}
        ...
        return {
          FABRICA_PI_SOURCE_AGENT_DIR: existingAgentDir ?? '/tmp/default-pi-agent'
        }
      }
    )
    piBuildStatusOnlyPtyEnvMock.mockReturnValue({
      FABRICA_PRIME_AGENT_STATUS_EXTENSION:
        'C:\\Users\\test\\AppData\\Roaming\\FABRICA\\prime-agent-managed-status-extension\\FABRICA-agent-status.ts'
    })
  ...
  afterEach(() => {
    ...
    if (savedDisableMacosLoginShell !== undefined) {
      process.env.FABRICA_DISABLE_MACOS_LOGIN_SHELL = savedDisableMacosLoginShell
    } else {
      delete process.env.FABRICA_DISABLE_MACOS_LOGIN_SHELL
    }
    if (savedFABRICAUserDataPath !== undefined) {
      process.env.FABRICA_USER_DATA_PATH = savedFABRICAUserDataPath
    } else {
      delete process.env.FABRICA_USER_DATA_PATH
    }
    ...
    if (savedFABRICAOpenCodeConfigDir !== undefined) {
      process.env.FABRICA_OPENCODE_CONFIG_DIR = savedFABRICAOpenCodeConfigDir
    } else {
      delete process.env.FABRICA_OPENCODE_CONFIG_DIR
    }
    if (savedFABRICAOpenCodeSourceConfigDir !== undefined) {
      process.env.FABRICA_OPENCODE_SOURCE_CONFIG_DIR = savedFABRICAOpenCodeSourceConfigDir
    } else {
      delete process.env.FABRICA_OPENCODE_SOURCE_CONFIG_DIR
    }
    ...
    if (savedFABRICAPiAgentDir !== undefined) {
      process.env.FABRICA_PI_CODING_AGENT_DIR = savedFABRICAPiAgentDir
    } else {
      delete process.env.FABRICA_PI_CODING_AGENT_DIR
    }
    if (savedFABRICAPiSourceAgentDir === undefined) {
      delete process.env.FABRICA_PI_SOURCE_AGENT_DIR
    } else {
      process.env.FABRICA_PI_SOURCE_AGENT_DIR = savedFABRICAPiSourceAgentDir
    }
    if (savedFABRICACodexHome === undefined) {
      delete process.env.FABRICA_CODEX_HOME
    } else {
      process.env.FABRICA_CODEX_HOME = savedFABRICACodexHome
    }
    if (savedFABRICAOmpAgentDir !== undefined) {
      process.env.FABRICA_OMP_CODING_AGENT_DIR = savedFABRICAOmpAgentDir
    } else {
      delete process.env.FABRICA_OMP_CODING_AGENT_DIR
    }
    if (savedFABRICAOmpSourceAgentDir !== undefined) {
      process.env.FABRICA_OMP_SOURCE_AGENT_DIR = savedFABRICAOmpSourceAgentDir
    } else {
      delete process.env.FABRICA_OMP_SOURCE_AGENT_DIR
    }
    if (savedFABRICAOmpStatusExtension !== undefined) {
      process.env.FABRICA_OMP_STATUS_EXTENSION = savedFABRICAOmpStatusExtension
    } else {
      delete process.env.FABRICA_OMP_STATUS_EXTENSION
    }
    ...
    if (savedFABRICAPrimeAgentSourceDir !== undefined) {
      process.env.FABRICA_PRIME_AGENT_SOURCE_AGENT_DIR = savedFABRICAPrimeAgentSourceDir
    } else {
      delete process.env.FABRICA_PRIME_AGENT_SOURCE_AGENT_DIR
    }
    if (savedFABRICAPrimeAgentStatusExtension !== undefined) {
      process.env.FABRICA_PRIME_AGENT_STATUS_EXTENSION = savedFABRICAPrimeAgentStatusExtension
    } else {
      delete process.env.FABRICA_PRIME_AGENT_STATUS_EXTENSION
    }
    if (savedFABRICAClaudeAgentStatusSettings === undefined) {
      delete process.env.FABRICA_CLAUDE_AGENT_STATUS_SETTINGS
    } else {
      process.env.FABRICA_CLAUDE_AGENT_STATUS_SETTINGS = savedFABRICAClaudeAgentStatusSettings
    }
  })
  ...
  describe('controller probePtyLiveness routing', () => {
    ...
    it('answers unknown for remote-scoped ids without consulting local providers', async () => {
      ...
      // for a PTY that lives on a remote FABRICA host.
      setLocalPtyProvider(new LocalPtyProvider())
      const controller = registerAgentClaimController()
  ...
  describe('spawn environment', () => {
    ...
    it('always sets TERM and COLORTERM regardless of env', async () => {
      ...
      expect(env.TERM_PROGRAM).toBe('Fabrica')
    })
    ...
    it('advertises OSC 8 hyperlink support via FORCE_HYPERLINK', async () => {
      // Why: supports-hyperlinks allowlists TERM_PROGRAM and reports false for FABRICA, so FORCE_HYPERLINK=1 forces detection on (xterm.js handles OSC 8 natively).
      const env = await spawnAndGetEnv()
      expect(env.FORCE_HYPERLINK).toBe('1')
    })

    it('surfaces FABRICA_APP_VERSION as TERM_PROGRAM_VERSION for TUI feature gating', async () => {
      const env = await spawnAndGetEnv(undefined, { FABRICA_APP_VERSION: '1.2.3-test' })
      expect(env.TERM_PROGRAM_VERSION).toBe('1.2.3-test')
    })

    it('falls back to a placeholder version when FABRICA_APP_VERSION is unset', async () => {
      const env = await spawnAndGetEnv(undefined, { FABRICA_APP_VERSION: undefined })
      expect(env.TERM_PROGRAM_VERSION).toBe('0.0.0-dev')
    })

    it('injects the selected Codex home into FABRICA terminal PTYs', async () => {
      const env = await spawnAndGetEnv(undefined, undefined, () => TEST_CODEX_HOME)
      expect(env.CODEX_HOME).toBe(TEST_CODEX_HOME)
      expect(env.FABRICA_CODEX_HOME).toBe(TEST_CODEX_HOME)
    })
    ...
    it('resumes an automatic Codex session from its prepared originating home', async () => {
      ...
      expect(env.FABRICA_CODEX_HOME).toBe('/managed/origin/home')
    })
    ...
    it('overrides an unmarked custom home when the resumed session originated in real home', async () => {
      ...
      await handlers.get('pty:spawn')!(null, {
        ...
        envToDelete: ['CODEX_HOME', 'FABRICA_CODEX_HOME', 'REMOVE_ME'],
        launchAgent: 'codex',
        resumeProviderSession: {
          key: 'session_id',
          id: 'session-a',
      ...
      expect(env.FABRICA_CODEX_HOME).toBe(systemHome)
    ...
    describe('unverifiable Codex resume provenance', () => {
      ...
      posixOnlyIt(
        'launches plain codex when a REAL rollout sits under a home FABRICA no longer trusts',
        async () => {
      ...
      it('strips the resume argv from the sequenced startup command too', async () => {
        ...
        await spawnCodexResume(ORIGIN_ROLLOUT, {
          env: { FABRICA_SEQUENCED_STARTUP_COMMAND: `codex 'resume' '${RESUME_SESSION_ID}'` }
        })

        const spawnOptions = daemonSpawn.mock.calls.at(-1)![0]
        expect(spawnOptions.env.FABRICA_SEQUENCED_STARTUP_COMMAND).toBe('codex')
      ...
      posixOnlyIt(
        ...
        async () => {
          ...
          const env = spawnMock.mock.calls.at(-1)![2].env as Record<string, string>
          expect(env.FABRICA_SEQUENCED_STARTUP_COMMAND).toBe('codex')
        }
      )
      ...
      posixOnlyIt(
        ...
        async () => {
          ...
          const env = spawnMock.mock.calls.at(-1)![2].env as Record<string, string>
          expect(env.FABRICA_SEQUENCED_STARTUP_COMMAND).toBe(sequenced)
        }
      )
    ...
    it('injects the OpenCode hook env into FABRICA terminal PTYs', async () => {
      ...
      expect(env.FABRICA_OPENCODE_HOOK_PORT).toBe('4567')
      expect(env.FABRICA_OPENCODE_HOOK_TOKEN).toBe('opencode-token')
      expect(env.FABRICA_OPENCODE_PTY_ID).toBe('test-pty')
      expect(env.OPENCODE_CONFIG_DIR).toEqual(expect.any(String))
      expect(env.FABRICA_OPENCODE_CONFIG_DIR).toBe(env.OPENCODE_CONFIG_DIR)
    })

    it('mirrors the original OpenCode source dir when launched from an FABRICA overlay shell', async () => {
      const env = await spawnAndGetEnv({
        OPENCODE_CONFIG_DIR: '/tmp/parent-FABRICA-opencode-overlay',
        FABRICA_OPENCODE_SOURCE_CONFIG_DIR: '/tmp/user-opencode-config'
      })
      expect(openCodeBuildPtyEnvMock).toHaveBeenCalledWith(
        expect.any(String),
        '/tmp/user-opencode-config'
      )
      expect(env.OPENCODE_CONFIG_DIR).toBe('/tmp/FABRICA-opencode-overlay')
      expect(env.FABRICA_OPENCODE_CONFIG_DIR).toBe('/tmp/FABRICA-opencode-overlay')
      expect(env.FABRICA_OPENCODE_SOURCE_CONFIG_DIR).toBe('/tmp/user-opencode-config')
    })

    it('does not treat inherited FABRICA OpenCode config as user config without a source dir', async () => {
      const env = await spawnAndGetEnv({
        OPENCODE_CONFIG_DIR: '/tmp/parent-FABRICA-opencode-overlay',
        FABRICA_OPENCODE_CONFIG_DIR: '/tmp/parent-FABRICA-opencode-overlay'
      })

      expect(openCodeBuildPtyEnvMock).toHaveBeenCalledWith(expect.any(String), undefined)
      expect(env.OPENCODE_CONFIG_DIR).toBe('/tmp/FABRICA-opencode-config')
      expect(env.FABRICA_OPENCODE_CONFIG_DIR).toBe('/tmp/FABRICA-opencode-config')
      expect(env.FABRICA_OPENCODE_SOURCE_CONFIG_DIR).toBeUndefined()
    })

    it('restores user OpenCode config when agent status hooks are disabled in a nested FABRICA shell', async () => {
      const env = await spawnAndGetEnv(
        {
          OPENCODE_CONFIG_DIR: '/tmp/parent-FABRICA-opencode-overlay',
          FABRICA_OPENCODE_CONFIG_DIR: '/tmp/parent-FABRICA-opencode-overlay',
      ...
      expect(env.FABRICA_OPENCODE_CONFIG_DIR).toBeUndefined()
      expect(env.FABRICA_OPENCODE_SOURCE_CONFIG_DIR).toBeUndefined()
    })
    ...
    it('injects MiMo overlay env only when launch command is mimo', async () => {
      ...
      expect(env.MIMOCODE_HOME).toBe('/tmp/FABRICA-mimocode-shared')
      expect(env.FABRICA_MIMOCODE_HOME).toBe('/tmp/FABRICA-mimocode-shared')
      expect(env.FABRICA_MIMOCODE_SOURCE_HOME).toBeUndefined()
    })
    ...
    it('restores user MiMo home when agent status hooks are disabled in a nested FABRICA shell', async () => {
      const env = await spawnAndGetEnv(
        {
          MIMOCODE_HOME: '/tmp/parent-FABRICA-mimocode-overlay',
          FABRICA_MIMOCODE_HOME: '/tmp/parent-FABRICA-mimocode-overlay',
          FABRICA_MIMOCODE_SOURCE_HOME: '/tmp/user-mimocode-home'
        },
        undefined,
        undefined,
        () => ({ agentStatusHooksEnabled: false }),
      ...
      expect(env.FABRICA_MIMOCODE_HOME).toBeUndefined()
      expect(env.FABRICA_MIMOCODE_SOURCE_HOME).toBeUndefined()
    })

    posixOnlyIt(
      'reproduces issue #1534: GUI-launched FABRICA mirrors zshrc-only OpenCode config',
      async () => {
        ...
        const env = await spawnAndGetEnv(undefined, {
          ...
          FABRICA_OPENCODE_SOURCE_CONFIG_DIR: undefined
        })
        ...
        expect(env.OPENCODE_CONFIG_DIR).toBe('/tmp/FABRICA-opencode-overlay')
        expect(env.FABRICA_OPENCODE_CONFIG_DIR).toBe('/tmp/FABRICA-opencode-overlay')
        expect(env.FABRICA_OPENCODE_SOURCE_CONFIG_DIR).toBe('/home/pim/company/opencode-config')
        expect(env.OPENCODE_CONFIG_DIR).not.toBe(env.FABRICA_OPENCODE_SOURCE_CONFIG_DIR)
      }
    )

    it('installs Pi managed extensions without redirecting FABRICA terminal PTY homes', async () => {
      ...
      expect(env.FABRICA_PI_CODING_AGENT_DIR).toBeUndefined()
      expect(env.FABRICA_PI_SOURCE_AGENT_DIR).toBe('/tmp/user-pi-agent')
      expect(env.FABRICA_OMP_CODING_AGENT_DIR).toBeUndefined()
      expect(env.FABRICA_OMP_STATUS_EXTENSION).toBe(
        '/tmp/FABRICA-user-data/omp-managed-status-extension/FABRICA-agent-status.ts'
      )
      expect(env.FABRICA_OMP_SOURCE_AGENT_DIR).toBeUndefined()
    })
    ...
    it('does not materialize a missing Pi home when another agent mentions Pi', async () => {
      ...
      expect(env.FABRICA_PI_SOURCE_AGENT_DIR).toBeUndefined()
    })
    ...
    it('materializes Pi home for an explicit Pi launch through a custom command', async () => {
      ...
      expect(env.FABRICA_PI_SOURCE_AGENT_DIR).toBe('/tmp/default-pi-agent')
    })

    it('threads command: "omp" through to piBuildPtyEnv and emits OMP status metadata', async () => {
      // Why: OMP launches emit FABRICA_OMP_* shadow vars, not Pi-named ones; only PI_CODING_AGENT_DIR stays (OMP's own binary reads it).
      const env = await spawnAndGetEnv(
        undefined,
      ...
      expect(env.FABRICA_OMP_CODING_AGENT_DIR).toBeUndefined()
      expect(env.FABRICA_OMP_STATUS_EXTENSION).toBe(
        '/tmp/user-omp-agent/extensions/FABRICA-agent-status.ts'
      )
      expect(env.FABRICA_OMP_SOURCE_AGENT_DIR).toBe('/tmp/user-omp-agent')
    ...
    it('installs Prime status into its independent agent dir on explicit launch', async () => {
      ...
      expect(env.PI_CODING_AGENT_DIR).toBe('/tmp/user-pi-agent')
      expect(env.FABRICA_PRIME_AGENT_SOURCE_AGENT_DIR).toBe('/tmp/user-prime-agent')
      expect(env.FABRICA_PI_SOURCE_AGENT_DIR).toBeUndefined()
      expect(env.FABRICA_OMP_SOURCE_AGENT_DIR).toBeUndefined()
    })
    ...
    it('mirrors the original Pi source dir when launched from an FABRICA overlay shell', async () => {
      const env = await spawnAndGetEnv({
        PI_CODING_AGENT_DIR: '/tmp/parent-FABRICA-pi-overlay',
        FABRICA_PI_SOURCE_AGENT_DIR: '/tmp/user-pi-agent'
      })
      expect(piBuildPtyEnvMock).toHaveBeenCalledWith(
        expect.any(String),
        '/tmp/user-pi-agent',
      ...
      expect(env.PI_CODING_AGENT_DIR).toBe('/tmp/parent-FABRICA-pi-overlay')
      expect(env.FABRICA_PI_CODING_AGENT_DIR).toBeUndefined()
      expect(env.FABRICA_PI_SOURCE_AGENT_DIR).toBe('/tmp/user-pi-agent')
    })
    ...
    it('does not use an inherited Pi overlay source for an OMP launch', async () => {
      const env = await spawnAndGetEnv(
        {
          PI_CODING_AGENT_DIR: '/tmp/parent-FABRICA-pi-overlay',
          FABRICA_PI_CODING_AGENT_DIR: '/tmp/parent-FABRICA-pi-overlay',
          FABRICA_PI_SOURCE_AGENT_DIR: '/tmp/user-pi-agent'
        },
        undefined,
        undefined,
      ...
      expect(env.FABRICA_OMP_CODING_AGENT_DIR).toBeUndefined()
      expect(env.FABRICA_OMP_SOURCE_AGENT_DIR).toBe('/tmp/default-omp-agent')
      expect(env.FABRICA_PI_CODING_AGENT_DIR).toBeUndefined()
      expect(env.FABRICA_PI_SOURCE_AGENT_DIR).toBeUndefined()
    })
    ...
    it('does not use an inherited OMP overlay source for an explicit Pi launch', async () => {
      const env = await spawnAndGetEnv(
        {
          PI_CODING_AGENT_DIR: '/tmp/parent-FABRICA-omp-overlay',
          FABRICA_OMP_CODING_AGENT_DIR: '/tmp/parent-FABRICA-omp-overlay',
          FABRICA_OMP_SOURCE_AGENT_DIR: '/tmp/user-omp-agent'
        },
        undefined,
        undefined,
        undefined,
      ...
      expect(env.FABRICA_OMP_CODING_AGENT_DIR).toBeUndefined()
      expect(env.FABRICA_OMP_SOURCE_AGENT_DIR).toBeUndefined()
      expect(env.FABRICA_OMP_STATUS_EXTENSION).toBeUndefined()
    })

    it('restores user Pi config when agent status hooks are disabled in a nested FABRICA shell', async () => {
      const env = await spawnAndGetEnv(
        {
          PI_CODING_AGENT_DIR: '/tmp/parent-FABRICA-pi-overlay',
          FABRICA_PI_CODING_AGENT_DIR: '/tmp/parent-FABRICA-pi-overlay',
    ...
    it('strips only the Prime source shadow when hooks are disabled', async () => {
      const env = await spawnAndGetEnv(
        {
          PRIME_AGENT_CODING_AGENT_DIR: '/tmp/user-prime-agent',
          FABRICA_PRIME_AGENT_SOURCE_AGENT_DIR: '/tmp/user-prime-agent'
        },
        undefined,
        undefined,
        () => ({ agentStatusHooksEnabled: false })
      ...
      expect(env.PRIME_AGENT_CODING_AGENT_DIR).toBe('/tmp/user-prime-agent')
      expect(env.FABRICA_PRIME_AGENT_SOURCE_AGENT_DIR).toBeUndefined()
    })
    ...
    posixOnlyIt(
      ...
      async () => {
        ...
        expect(env.FABRICA_PI_CODING_AGENT_DIR).toBeUndefined()
        expect(env.FABRICA_PI_SOURCE_AGENT_DIR).toBe('/home/tester/.config/pi-agent')
      }
    )

    it('injects the agent hook receiver env into FABRICA terminal PTYs', async () => {
      const env = await spawnAndGetEnv()
      ...
      expect(env.FABRICA_AGENT_HOOK_TOKEN).toBe('agent-token')
    })

    it('strips stale inherited hook receiver env before injecting this runtime', async () => {
      const env = await spawnAndGetEnv({
        FABRICA_AGENT_HOOK_PORT: '1111',
        FABRICA_AGENT_HOOK_TOKEN: 'stale-token',
      ...
      expect(env.FABRICA_AGENT_HOOK_PORT).toBe('5678')
      expect(env.FABRICA_AGENT_HOOK_TOKEN).toBe('agent-token')
      expect(env.FABRICA_AGENT_HOOK_ENV).toBeUndefined()
      expect(env.FABRICA_AGENT_HOOK_VERSION).toBeUndefined()
      expect(env.FABRICA_AGENT_HOOK_ENDPOINT).toBeUndefined()
      expect(env.FABRICA_CLAUDE_AGENT_STATUS_SETTINGS).toBeUndefined()
    })
    ...
    it('does not leak inherited hook receiver env if the hook server is unavailable', async () => {
      ...
      expect(env.FABRICA_AGENT_HOOK_TOKEN).toBeUndefined()
      expect(env.FABRICA_AGENT_HOOK_ENV).toBeUndefined()
      expect(env.FABRICA_AGENT_HOOK_VERSION).toBeUndefined()
      expect(env.FABRICA_AGENT_HOOK_ENDPOINT).toBeUndefined()
      expect(env.FABRICA_CLAUDE_AGENT_STATUS_SETTINGS).toBeUndefined()
    ...
    it('skips git/gh attribution shims when attribution is disabled', async () => {
      ...
      expect(env.FABRICA_ENABLE_GIT_ATTRIBUTION).toBeUndefined()
      expect(env.FABRICA_GIT_COMMIT_TRAILER).toBeUndefined()
      expect(env.FABRICA_GH_PR_FOOTER).toBeUndefined()
      expect(env.FABRICA_GH_ISSUE_FOOTER).toBeUndefined()
    ...
    it('overrides ambient CODEX_HOME with the FABRICA-managed home for system default', async () => {
      const env = await spawnAndGetEnv(
        undefined,
    ...
    it('waits for managed Codex auth before spawning a local PTY', async () => {
      ...
      expect(spawnMock.mock.calls.at(-1)?.[2].env).toMatchObject({
        ...
        FABRICA_CODEX_HOME: TEST_CODEX_HOME
      })
    })
    ...
    it('strips a nested-FABRICA override for system default when the real-home flag is ON', async () => {
      const env = await spawnAndGetEnv(
        { CODEX_HOME: '/managed/home', FABRICA_CODEX_HOME: '/managed/home' },
        undefined,
        () => null,
        () => ({ codexSystemDefaultRealHomeEnabled: true }) as never
      )
      expect(env.CODEX_HOME).toBeUndefined()
      expect(env.FABRICA_CODEX_HOME).toBeUndefined()
    })
    ...
    it('lets the resolver keep a per-spawn custom CODEX_HOME on the managed lane', async () => {
      ...
      const env = await spawnAndGetEnv(
        ...
        { CODEX_HOME: undefined, FABRICA_CODEX_HOME: undefined },
        resolveHome,
        () => ({ codexSystemDefaultRealHomeEnabled: true }) as never
      )
    ...
    describe('daemon-active provider (parity with LocalPtyProvider)', () => {
      ...
      it('mirrors a user-provided OPENCODE_CONFIG_DIR into a source-scoped overlay on the daemon path', async () => {
        ...
        expect(env.OPENCODE_CONFIG_DIR).toBe('/tmp/FABRICA-opencode-overlay')
        expect(env.FABRICA_OPENCODE_CONFIG_DIR).toBe('/tmp/FABRICA-opencode-overlay')
        expect(env.FABRICA_OPENCODE_SOURCE_CONFIG_DIR).toBe('/user/custom/opencode')
      })

      it('uses source OpenCode config env instead of remirroring a parent overlay', async () => {
        const env = await daemonSpawnAndGetEnv({
          OPENCODE_CONFIG_DIR: '/tmp/parent-FABRICA-opencode-overlay',
          FABRICA_OPENCODE_SOURCE_CONFIG_DIR: '/user/custom/opencode'
        })
        expect(openCodeBuildPtyEnvMock).toHaveBeenCalledWith(
          expect.any(String),
          '/user/custom/opencode'
      ...
      it('installs Pi managed extensions without redirecting homes on the daemon path', async () => {
        ...
        expect(env.FABRICA_PI_CODING_AGENT_DIR).toBeUndefined()
        expect(env.FABRICA_PI_SOURCE_AGENT_DIR).toBe('/user/.pi/agent')
        expect(env.FABRICA_OMP_CODING_AGENT_DIR).toBeUndefined()
        expect(env.FABRICA_OMP_STATUS_EXTENSION).toBe(expectedOmpStatusExtension)
      })
      ...
      it('threads command: "omp" through to piBuildPtyEnv on the daemon path with OMP status metadata', async () => {
        ...
        expect(env.FABRICA_OMP_CODING_AGENT_DIR).toBeUndefined()
        expect(env.FABRICA_OMP_STATUS_EXTENSION).toBe(
          '/user/.omp/agent/extensions/FABRICA-agent-status.ts'
        )
        expect(env.FABRICA_OMP_SOURCE_AGENT_DIR).toBe('/user/.omp/agent')
        expect(env.FABRICA_PI_CODING_AGENT_DIR).toBeUndefined()
        expect(env.FABRICA_PI_SOURCE_AGENT_DIR).toBeUndefined()
      })
      ...
      it('keeps the authoritative home for runtime-created daemon resumes', async () => {
        ...
        expect(spawnOptions.env.FABRICA_CODEX_HOME).toBe(systemHome)
        expect(spawnOptions.env.REMOVE_ME).toBeUndefined()
        expect(spawnOptions.envToDelete ?? []).not.toContain('CODEX_HOME')
        expect(spawnOptions.envToDelete ?? []).not.toContain('FABRICA_CODEX_HOME')
      ...
      it('skips host Codex home when a daemon-backed Windows spawn targets a WSL cwd', async () => {
        ...
        try {
          ...
          expect(spawnOptions.envToDelete).toEqual(
            expect.arrayContaining(['CODEX_HOME', 'FABRICA_CODEX_HOME'])
          )
        } finally {
          Object.defineProperty(process, 'platform', {
            configurable: true,
      ...
      it('skips host Codex home when a daemon-backed Windows spawn uses a WSL shell override', async () => {
        ...
        try {
          ...
          expect(spawnOptions.env.FABRICA_CODEX_HOME).toBeUndefined()
          expect(spawnOptions.envToDelete).toEqual(
            expect.arrayContaining(['CODEX_HOME', 'FABRICA_CODEX_HOME'])
          )
        } finally {
      ...
      it('bridges a host-managed Prime status extension into WSL without redirecting Prime home', async () => {
        await withWin32Platform(async () => {
          ...
          expect(env.FABRICA_PRIME_AGENT_SOURCE_AGENT_DIR).toBeUndefined()
          expect(env.FABRICA_PRIME_AGENT_STATUS_EXTENSION).toBe(
            'C:\\Users\\test\\AppData\\Roaming\\FABRICA\\prime-agent-managed-status-extension\\FABRICA-agent-status.ts'
          )
          expect(env.FABRICA_WSL_HOOK_INSTANCE).toBe('port5678')
      ...
      it('prepares the Prime bridge for a typed launch in a bare WSL shell', async () => {
        await withWin32Platform(async () => {
          ...
          expect(piBuildStatusOnlyPtyEnvMock).toHaveBeenCalledWith('prime-agent')
          expect(env.FABRICA_PRIME_AGENT_STATUS_EXTENSION).toContain(
      ...
      it('points OPENCODE_CONFIG_DIR at the guest overlay when the WSL relay reports it', async () => {
        const guestDir = '/home/jin/.FABRICA-relay/opencode-overlays/abc'
        const spy = vi.spyOn(wslHookRelayManager, 'getOpenCodeOverlayDir').mockReturnValue(guestDir)
        try {
          await withWin32Platform(async () => {
            const env = await daemonSpawnAndGetEnv(
              { FABRICA_OPENCODE_SOURCE_CONFIG_DIR: '/home/jin/.config/opencode' },
              undefined,
              undefined,
              undefined,
              { shellOverride: 'wsl.exe' }
            ...
            expect(env.FABRICA_OPENCODE_CONFIG_DIR).toBe(guestDir)
      ...
      it('strips the daemon-inherited FABRICA-owned CODEX_HOME for real-home routing', async () => {
        const spawnOptions = await daemonSpawnAndGetOptions(
          {},
          () => null,
          () => ({ codexSystemDefaultRealHomeEnabled: true }) as never,
        ...
        expect(spawnOptions.env.CODEX_HOME).toBeUndefined()
        expect(spawnOptions.env.FABRICA_CODEX_HOME).toBeUndefined()
        expect(spawnOptions.envToDelete).toEqual(expect.arrayContaining(['FABRICA_CODEX_HOME']))
      ...
      it('preserves a daemon-inherited user CODEX_HOME for real-home routing', async () => {
        const spawnOptions = await daemonSpawnAndGetOptions(
          ...
          () => null,
          () => ({ codexSystemDefaultRealHomeEnabled: true }) as never,
          { CODEX_HOME: '/home/me/.config/codex', FABRICA_CODEX_HOME: undefined }
        )
        expect(spawnOptions.envToDelete).toEqual(expect.arrayContaining(['FABRICA_CODEX_HOME']))
      ...
      it('does not strip the daemon-inherited CODEX_HOME when the flag is OFF', async () => {
        const spawnOptions = await daemonSpawnAndGetOptions({}, () => null, undefined, {
          ...
          FABRICA_CODEX_HOME: '/managed/home'
        })
        expect(spawnOptions.envToDelete ?? []).not.toEqual(expect.arrayContaining(['CODEX_HOME']))
      })
      ...
      it('prepends the bare-FABRICA CLI shim dir to PATH for packaged Linux spawns', async () => {
        const originalPlatform = process.platform
        Object.defineProperty(process, 'platform', {
          configurable: true,
          value: 'linux'
        ...
        try {
          ...
          const shimDir = join('/tmp/FABRICA-user-data', 'linux-FABRICA-cli-shim')
          // Why: bare `FABRICA` must resolve to the FABRICA CLI before /usr/bin/FABRICA (the GNOME screen reader) in FABRICA terminals (#7904).
          ...
          expect(env.FABRICA_CLI_COMMAND).toBeUndefined()
        } finally {
          Object.defineProperty(process, 'platform', {
            configurable: true,
            value: originalPlatform
      ...
      it('prepends the bundled CLI dir to PATH for packaged macOS spawns', async () => {
        ...
        Object.defineProperty(process, 'resourcesPath', {
          configurable: true,
          value: '/tmp/FABRICA-resources'
        })
        try {
          const env = await daemonSpawnAndGetEnv({ PATH: '/usr/bin' })
          expect(env.PATH.split(delimiter)[0]).toBe(join('/tmp/FABRICA-resources', 'bin'))
        } finally {
      ...
      it('deletes stale Claude scoped settings env from daemon-hosted PTYs', async () => {
        ...
        expect(spawnOptions.env.FABRICA_CLAUDE_AGENT_STATUS_SETTINGS).toBeUndefined()
        expect(spawnOptions.envToDelete).toEqual(
          expect.arrayContaining(['FABRICA_CLAUDE_AGENT_STATUS_SETTINGS'])
        )
        expect(spawnOptions.env.FABRICA_AGENT_HOOK_PORT).toBe('5678')
        expect(spawnOptions.env.FABRICA_AGENT_HOOK_TOKEN).toBe('agent-token')
      })
      ...
      it('deletes stale Claude scoped settings env from runtime-created daemon PTYs', async () => {
        ...
        process.env.FABRICA_CLAUDE_AGENT_STATUS_SETTINGS =
          '/tmp/FABRICA/agent-hooks/claude-agent-status-settings.json'
        handlers.clear()
        registerPtyHandlers(mainWindow as never, runtime as never)
        const controller = runtime.setPtyController.mock.calls[0]?.[0] as RuntimeSpawnController
      ...
      it('strips inherited Claude child-session stamps from runtime-created PTYs', async () => {
        // Why: the runtime controller is the `FABRICA` CLI / automation spawn path and
      ...
      it('resolves default WSL authority before daemon host env and spawn metadata', async () => {
        await withWin32Platform(async () => {
          ...
          expect(spawnOptions.env).toMatchObject({
            ...
            FABRICA_ORCHESTRATION_COMPATIBILITY_HOST_ID: 'compat-host',
      ...
      it('keeps the Agent Teams tmux shim ahead of host PATH shims for runtime-created daemon PTYs', async () => {
        ...
        await controller.spawn({
          ...
          command: 'claude',
          env: {
            PATH: `/tmp/FABRICA-agent-teams-bin${delimiter}/usr/bin`,
            FABRICA_AGENT_TEAMS_TEAM_ID: 'team-test',
            TERM_PROGRAM: 'FABRICA',
            FABRICA_ATTRIBUTION_SHIM_DIR: '/tmp/stale-attribution'
          },
          envToDelete: ['TERM_PROGRAM', 'FABRICA_ATTRIBUTION_SHIM_DIR']
        })

        const spawnOptions = daemonSpawn.mock.calls.at(-1)?.[0] as DaemonSpawnCall
        expect(spawnOptions.env.PATH.split(delimiter)[0]).toBe('/tmp/FABRICA-agent-teams-bin')
        expect(spawnOptions.env.PATH).toContain(expectedAttributionShimDir())
        expect(spawnOptions.env.TERM_PROGRAM).toBeUndefined()
        expect(spawnOptions.env.FABRICA_ATTRIBUTION_SHIM_DIR).toBeUndefined()
        expect(spawnOptions.envToDelete).toEqual(
          expect.arrayContaining(['TERM_PROGRAM', 'FABRICA_ATTRIBUTION_SHIM_DIR'])
        )
      })
      ...
      it('injects dev-mode FABRICA_USER_DATA_PATH + dev CLI PATH on the daemon path', async () => {
        ...
        try {
          const env = await daemonSpawnAndGetEnv({ PATH: '/usr/bin' })
          expect(env.FABRICA_USER_DATA_PATH).toBe('/tmp/FABRICA-user-data')
          expect(env.PATH).toContain(join('/tmp/FABRICA-user-data', 'cli', 'bin'))
        } finally {
          mockedApp.isPackaged = prev
        }
      })
      ...
      it('preserves the inherited PATH when dev-mode daemon env omits PATH', async () => {
        ...
        try {
          ...
          expect(env.PATH).toContain(
            `${join('/tmp/FABRICA-user-data', 'cli', 'bin')}${delimiter}/system/bin`
          )
        } finally {
          mockedApp.isPackaged = prev
        }
      ...
      it('falls back to process.env.PI_CODING_AGENT_DIR when baseEnv lacks it on the daemon path', async () => {
        ...
        expect(env.PI_CODING_AGENT_DIR).toBeUndefined()
        expect(env.FABRICA_PI_CODING_AGENT_DIR).toBeUndefined()
        expect(env.FABRICA_PI_SOURCE_AGENT_DIR).toBe('/ambient/pi/agent')
      })
      ...
      it('does not mutate the caller-provided args.env on the daemon path', async () => {
        // Why: the handler clones baseEnv so IPC-provided env stays pristine; a regression would leak FABRICA host env back into the renderer's reused copy.
        const daemonSpawn = setupDaemonAdapter()
        ...
        expect(spawnEnv.FABRICA_AGENT_HOOK_PORT).toBe('5678')
      ...
      it('does NOT inject host-local env on SSH spawns (connectionId set)', async () => {
        ...
        await handlers.get('pty:spawn')!(null, {
          cols: 80,
          rows: 24,
          env: { FOO: 'bar', FABRICA_PANE_KEY: makePaneKey('tab-1', leafId) },
          connectionId: 'ssh-1',
          worktreeId: 'wt-1',
          tabId: 'tab-1',
          leafId
        ...
        await handlers.get('pty:spawn')!(null, {
          cols: 80,
          rows: 24,
          env: { FABRICA_PANE_KEY: 'tab-1:pane:1' },
          connectionId: 'ssh-1',
          worktreeId: 'wt-1',
          tabId: 'tab-1',
          leafId: 'pane:1'
        ...
        expect(legacySpawnOptions?.env.FABRICA_PANE_KEY).toBeUndefined()
      ...
      it('strips FABRICA_PANE_KEY/TAB_ID/WORKTREE_ID from SSH spawn env when remote agent hooks are disabled', async () => {
        ...
        handlers.clear()
        registerPtyHandlers(mainWindow as never)
        const prevFlag = process.env.FABRICA_FEATURE_REMOTE_AGENT_HOOKS
        process.env.FABRICA_FEATURE_REMOTE_AGENT_HOOKS = '0'
        try {
          await handlers.get('pty:spawn')!(null, {
            cols: 80,
            rows: 24,
            env: {
              FOO: 'bar',
              FABRICA_PANE_KEY: 'tab-1:0',
              FABRICA_TAB_ID: 'tab-1',
              FABRICA_WORKTREE_ID: 'wt-1'
            },
            connectionId: 'ssh-1'
          })
          const env = sshSpawn.mock.calls.at(-1)![0].env
          expect(env.FOO).toBe('bar')
          expect(env.FABRICA_PANE_KEY).toBeUndefined()
          expect(env.FABRICA_TAB_ID).toBeUndefined()
          expect(env.FABRICA_WORKTREE_ID).toBeUndefined()
          expect(env.FABRICA_AGENT_HOOK_TOKEN).toBeUndefined()
          // Why: the local hook server's userData-relative endpoint path is meaningless on the remote box; assert no leak.
          expect(env.FABRICA_AGENT_HOOK_ENDPOINT).toBeUndefined()
        } finally {
          if (prevFlag === undefined) {
            delete process.env.FABRICA_FEATURE_REMOTE_AGENT_HOOKS
          } else {
            process.env.FABRICA_FEATURE_REMOTE_AGENT_HOOKS = prevFlag
          }
        }
      })

      it('forwards FABRICA_PANE_KEY/TAB_ID/WORKTREE_ID over SSH by default', async () => {
        const sshSpawn = vi.fn(async (_opts: { env: Record<string, string> }) => ({
          id: 'ssh-pty'
        }))
        registerSshPtyProvider('ssh-1', {
        ...
        try {
          ...
          await handlers.get('pty:spawn')!(null, {
            ...
            env: {
              FOO: 'bar',
              FABRICA_PANE_KEY: paneKey,
              FABRICA_TAB_ID: 'tab-2',
              FABRICA_WORKTREE_ID: 'wt-2'
            },
            connectionId: 'ssh-1',
            tabId: 'tab-2',
            leafId
          ...
          const env = sshSpawn.mock.calls.at(-1)![0].env
          expect(env.FABRICA_PANE_KEY).toBe(paneKey)
          expect(env.FABRICA_TAB_ID).toBe('tab-2')
          expect(env.FABRICA_WORKTREE_ID).toBe('wt-2')
  ...
  it('injects FABRICA_TERMINAL_HANDLE for non-local PTY providers', async () => {
    ...
    expect(spawn).toHaveBeenCalledWith(
      expect.objectContaining({
        env: expect.objectContaining({
          EXISTING: '1',
          FABRICA_TERMINAL_HANDLE: 'term_remote'
        })
      })
    )
  ...
  it('refreshes captured native Agent Teams env for renderer PTY spawns', async () => {
    ...
    const runtime = {
      ...
      prepareClaudeAgentTeamsLeaderForHandle: vi.fn(async () => ({
        env: {
          ...
          TMUX: '/tmp/FABRICA-claude-agent-teams/team-fresh,0,1',
          TMUX_PANE: '%1',
          FABRICA_AGENT_TEAMS_TEAM_ID: 'team-fresh',
          FABRICA_AGENT_TEAMS_TOKEN: 'fresh-token'
        }
      })),
      registerPreAllocatedHandleForPty: vi.fn(),
      registerPty: vi.fn(),
    ...
    const result = (await handlers.get('pty:spawn')!(mainWindowIpcEvent, {
      ...
      worktreeId: 'wt-1',
      env: {
        FABRICA_PANE_KEY: `tab-1:${leafId}`,
        FABRICA_TAB_ID: 'tab-1',
        FABRICA_WORKTREE_ID: 'wt-1',
        CLAUDE_PROFILE: 'captured',
        PATH: `/tmp/stale-agent-teams${delimiter}/usr/bin`,
        TMUX: '/tmp/FABRICA-claude-agent-teams/team-stale,0,1',
        FABRICA_AGENT_TEAMS_TEAM_ID: 'team-stale',
        FABRICA_AGENT_TEAMS_TOKEN: 'stale-token',
        TERM_PROGRAM: 'FABRICA',
        FABRICA_ATTRIBUTION_SHIM_DIR: '/tmp/stale-attribution'
      },
      launchConfig: {
    ...
    expect(spawnOptions.env).toMatchObject({
      CLAUDE_PROFILE: 'captured',
      CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS: '1',
      FABRICA_TERMINAL_HANDLE: 'term_agent_teams',
      FABRICA_AGENT_TEAMS_TEAM_ID: 'team-fresh',
      FABRICA_AGENT_TEAMS_TOKEN: 'fresh-token',
      TMUX: '/tmp/FABRICA-claude-agent-teams/team-fresh,0,1',
      TMUX_PANE: '%1'
  ...
  it.each([
    ...
    const result = (await handlers.get('pty:spawn')!(mainWindowIpcEvent, {
      ...
      env: {
        FABRICA_PANE_KEY: paneKey,
        FABRICA_TAB_ID: tabId,
        FABRICA_WORKTREE_ID: worktreeId,
        FABRICA_AGENT_LAUNCH_TOKEN: testCase.envLaunchToken
      },
  ...
  it('reuses the runtime background handle in local PTY spawn env', async () => {
    ...
    expect(env.FABRICA_TERMINAL_HANDLE).toBe('term_expected')
  ...
  it('persists runtime-owned headless session bindings when explicitly requested', async () => {
    ...
    await spawnController.spawn({
      ...
      tabId: 'tab-headless',
      leafId,
      env: { FABRICA_PANE_KEY: makePaneKey('tab-headless', leafId) },
  ...
  it('reuses runtime materialization when renderer focuses the same pane during spawn', async () => {
    ...
    const runtimeSpawn = spawnController.spawn({
      ...
      tabId: 'tab-race',
      leafId,
      env: { FABRICA_PANE_KEY: paneKey },
      persistHostSessionBinding: true
    })
    await Promise.resolve()

    // Why: SSH can strip FABRICA_PANE_KEY before spawn; tab/leaf metadata must still dedupe against runtime materialization.
    const rendererSpawn = handlers.get('pty:spawn')!(null, {
      ...
      leafId,
      env: {
        FABRICA_TAB_ID: 'tab-race',
        FABRICA_WORKTREE_ID: 'repo-1::/tmp'
      }
    }) as Promise<{ id: string }>
    await Promise.resolve()
  ...
  it('waits for an early runtime pane claim before renderer creation', async () => {
    ...
    const mounted = handlers.get('pty:spawn')!(null, {
      ...
      leafId,
      env: {
        FABRICA_PANE_KEY: makePaneKey(tabId, leafId),
        FABRICA_TAB_ID: tabId,
        FABRICA_WORKTREE_ID: worktreeId
      }
    })
  ...
  it('strips runtime-owned SSH pane env when remote agent hooks are disabled', async () => {
    ...
    const savedRemoteHooks = process.env.FABRICA_FEATURE_REMOTE_AGENT_HOOKS
    process.env.FABRICA_FEATURE_REMOTE_AGENT_HOOKS = '0'
    ...
    try {
      ...
      await spawnController.spawn({
        ...
        env: {
          FOO: 'bar',
          FABRICA_PANE_KEY: makePaneKey('tab-remote', leafId),
          FABRICA_TAB_ID: 'tab-remote',
          FABRICA_WORKTREE_ID: 'wt-remote'
        },
      ...
      if (savedRemoteHooks === undefined) {
        ...
        process.env.FABRICA_FEATURE_REMOTE_AGENT_HOOKS = savedRemoteHooks
      }
  ...
  it('preserves adopted SSH ownership when runtime binding persistence fails', async () => {
    ...
    ).rejects.toThrow(/FABRICA_TERMINAL_SESSION_STATE_SAVE_FAILED/)
  ...
  it('ignores renderer-provided FABRICA_TERMINAL_HANDLE for local PTY spawns', async () => {
    const runtime = {
      setPtyController: vi.fn(),
      noteTerminalSpawnCommand: vi.fn(),
      preAllocateHandleForPty: vi.fn(() => 'term_trusted'),
    ...
    await handlers.get('pty:spawn')!(null, {
      cols: 80,
      rows: 24,
      env: { FABRICA_TERMINAL_HANDLE: 'term_untrusted' }
    })

    const spawnCall = spawnMock.mock.calls.at(-1)!
    const env = spawnCall[2].env as Record<string, string>
    expect(env.FABRICA_TERMINAL_HANDLE).toBe('term_trusted')
    expect(runtime.preAllocateHandleForPty).toHaveBeenCalledWith(expect.any(String))
  })

  it('forwards the trusted FABRICA terminal handle into managed WSL terminals', async () => {
    const platform = Object.getOwnPropertyDescriptor(process, 'platform')
    Object.defineProperty(process, 'platform', {
      configurable: true,
      value: 'win32'
    ...
    expect(env.FABRICA_TERMINAL_HANDLE).toBe('term_wsl')
    expect(env.FABRICA_USER_DATA_PATH).toBe('/tmp/FABRICA-user-data')
    expect(env.FABRICA_CLI_COMMAND).toBe('fabrica')
    ...
    expect(env.WSLENV?.split(':')).not.toEqual(
      expect.arrayContaining(['FABRICA_OMP_SOURCE_AGENT_DIR/p'])
    )
  })

  it('forces managed FABRICA_USER_DATA_PATH for WSL spawns even when the caller provides a stale root', async () => {
    const platform = Object.getOwnPropertyDescriptor(process, 'platform')
    Object.defineProperty(process, 'platform', {
      configurable: true,
      value: 'win32'
    ...
    try {
      ...
      await handlers.get('pty:spawn')!(null, {
        ...
        shellOverride: 'wsl.exe',
        env: {
          FABRICA_USER_DATA_PATH: '/tmp/stale-FABRICA-user-data'
        }
      })
    } finally {
      if (platform) {
  ...
  it('falls back to the worktree root when a saved local cwd no longer exists', async () => {
    ...
    const worktreePath = '/Users/motoki/FABRICA/workspaces/nakamuramotoki/Fableと議論'
  ...
  it('spawns a plain POSIX login shell and queues startup commands for the live session', async () => {
    ...
    const originalFABRICAOrigZdotdir = process.env.FABRICA_ORIG_ZDOTDIR
    ...
    delete process.env.FABRICA_ORIG_ZDOTDIR
    ...
    try {
      ...
      expect(options.env.ZDOTDIR).toBe('/tmp/FABRICA-user-data/shell-ready/zsh')
      expect(options.env.FABRICA_ORIG_ZDOTDIR).toBe(process.env.HOME)
    } finally {
      Object.defineProperty(process, 'platform', {
        configurable: true,
        value: originalPlatform
      ...
      if (originalFABRICAOrigZdotdir === undefined) {
        delete process.env.FABRICA_ORIG_ZDOTDIR
      } else {
        process.env.FABRICA_ORIG_ZDOTDIR = originalFABRICAOrigZdotdir
      }
  ...
  posixOnlyIt('wraps macOS spawns in login(1) with SHELL restored by the trampoline', async () => {
    ...
    loginPreflightExecFileMock.mockImplementation(
      (
        ...
        callback(null, 'FABRICA_LOGIN_PREFLIGHT_OK', '')
  ...
  it('uses the POSIX shell wrapper so OpenCode config survives shell startup files', async () => {
    ...
    try {
      ...
      expect(shell).toBe('/bin/zsh')
      expect(args).toEqual(['-l'])
      expect(options.env.OPENCODE_CONFIG_DIR).toBe('/tmp/FABRICA-opencode-config')
      expect(options.env.FABRICA_OPENCODE_CONFIG_DIR).toBe('/tmp/FABRICA-opencode-config')
      expect(options.env.ZDOTDIR).toBe('/tmp/FABRICA-user-data/shell-ready/zsh')
      expect(options.env.FABRICA_SHELL_READY_MARKER).toBe('0')
    } finally {
      Object.defineProperty(process, 'platform', {
        configurable: true,
        value: originalPlatform
  ...
  it('uses the POSIX shell wrapper so Pi config survives shell startup files', async () => {
    ...
    try {
      ...
      expect(options.env.FABRICA_OPENCODE_CONFIG_DIR).toBeUndefined()
      expect(options.env.PI_CODING_AGENT_DIR).toBe('/tmp/user-pi-agent')
      expect(options.env.FABRICA_PI_CODING_AGENT_DIR).toBeUndefined()
      expect(options.env.FABRICA_PI_SOURCE_AGENT_DIR).toBe('/tmp/user-pi-agent')
      expect(options.env.ZDOTDIR).toBe('/tmp/FABRICA-user-data/shell-ready/zsh')
      expect(options.env.FABRICA_SHELL_READY_MARKER).toBe('0')
    } finally {
      Object.defineProperty(process, 'platform', {
  ...
  posixOnlyIt('waits for shell-ready before writing delivery-hinted Codex startup', async () => {
    ...
    try {
      ...
      const [, , options] = spawnMock.mock.calls[0]!
      expect(options.env.FABRICA_SHELL_READY_MARKER).toBe('1')
      expect(mockProc.proc.write).not.toHaveBeenCalled()
      ...
      mockProc.emitData('\x1b]777;FABRICA-shell-ready\x07')
      await Promise.resolve()
    ...
    try {
      ...
      expect(spawnMock).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          cwd: '/tmp',
          env: expect.objectContaining({
            FABRICA_OPENCODE_CONFIG_DIR: '/tmp/FABRICA-opencode-config',
            FABRICA_SHELL_READY_MARKER: '0',
            ZDOTDIR: '/tmp/FABRICA-user-data/shell-ready/zsh'
          })
        })
      )
      expect(warnSpy).toHaveBeenCalledWith(
  ...
  it('records the launch Codex account for a fresh spawn but not for a reattach', async () => {
    ...
    const nativeCodexEnv = { CODEX_HOME: '', FABRICA_CODEX_HOME: '' }
  ...
  it('does not resume under another account when the origin auth stays unavailable', async () => {
    ...
    const launch = handlers.get('pty:spawn')!(null, {
      ...
      rows: 24,
      command: 'codex resume session-a',
      envToDelete: ['CODEX_HOME', 'FABRICA_CODEX_HOME'],
      launchAgent: 'codex',
      resumeProviderSession: {
        key: 'session_id',
        id: 'session-a',
  ...
  // Why windowless: `FABRICA serve`/CLI runtime creation is the topology that most
  ...
  it('upgrades legacy numeric pane keys when the spawn metadata proves the stable leaf', async () => {
    ...
    await handlers.get('pty:spawn')!(null, {
      ...
      tabId: 'tab-1',
      leafId,
      env: { FABRICA_PANE_KEY: stablePaneKey }
    })
    ...
    await handlers.get('pty:spawn')!(null, {
      ...
      tabId: 'tab-1',
      leafId,
      env: { FABRICA_PANE_KEY: makePaneKey('tab-2', leafId) }
    })
