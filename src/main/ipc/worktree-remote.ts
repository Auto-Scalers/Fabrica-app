...
async function getOrStartSshWorktreeCreateFetch(
  ...
  const promise = enqueueSshWorktreeCreateFetch(queueKey, async () => {
    ...
    await fetch()
    // Why: SSH creation has no FABRICARuntimeService to share; still reuse recent fetches for repeated creates on the same target.
...
async function prepareWorktreePushTargetSsh(
  ...
  if (target.remoteUrl) {
    ...
    if (existingRemote) {
      ...
      // Why: a reused FABRICA-created fork remote must inherit ownership so deleting the final user can remove it.
      remoteCreated = store
        ? isPushTargetRemoteCreatedByKnownWorktree(
            store,
            {
...
async function readRemoteEffectiveHooks(
  ...
  return getEffectiveHooksFromConfig(repo, await readRemoteFABRICAYaml(fsProvider, hooksRootPath))
}

async function readRemoteFABRICAYaml(
  fsProvider: IFilesystemProvider,
  hooksRootPath: string
): Promise<ReturnType<typeof parseFABRICAYaml>> {
  try {
    const result = await fsProvider.readFile(joinWorktreeRelativePath(hooksRootPath, 'FABRICA.yaml'))
    return result.isBinary ? null : parseFABRICAYaml(result.content)
  } catch {
    return null
  }
}
...
async function createRemoteSetupRunnerScript(
  ...
  const runnerRelativePath = useWindowsFormat ? 'FABRICA/setup-runner.cmd' : 'FABRICA/setup-runner.sh'
...
export async function createRemoteWorktree(
  ...
  const metaUpdates: Partial<WorktreeMeta> = {
    ...
    FABRICACreationSource: 'ssh',
    creatorProvenance: { kind: 'host' },
    FABRICACreationWorkspaceLayout: getWorktreeCreationLayout(repo, settings),
  ...
  // Why: shared/symlink paths, `FABRICA.yaml` shared directories, and `.worktreeinclude` copies are local-only; remote (SSH) support needs a new relay method + auth surface, so all are skipped here.

  let setup: CreateWorktreeResult['setup']
  let defaultTabs: CreateWorktreeResult['defaultTabs']
  if (fsProvider) {
    await timing.time('prepare_setup', async () => {
      const yamlHooks = await readRemoteFABRICAYaml(fsProvider, created.path)
...
export async function createLocalWorktree(
  ...
  // Why: project-level `FABRICA.yaml` shared directories add to (never replace) the per-user
  ...
  // Why: the worktree's base-branch `FABRICA.yaml` is authoritative; we don't re-gate on content parity with the primary checkout since benign divergence silently disabled setup (#1280).
  let setup: CreateWorktreeResult['setup']
  let defaultTabs: CreateWorktreeResult['defaultTabs']
  await timing.time('prepare_setup', async () => {
