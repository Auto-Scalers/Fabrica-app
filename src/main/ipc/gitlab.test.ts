...
describe('GitLab IPC handlers', () => {
    ...
    await expect(
      handler?.(null, {
        ...
        sourceContext: {
          kind: 'task-source',
          provider: 'gitlab',
          projectId: 'gitlab:Auto-Scalers/Fabrica-app',
  ...
  it('rejects source context for a different host', async () => {
    registerGitLabHandlers(
      storeWithRepos([repo({ id: 'repo-local', path: '/local/FABRICA' })]) as Store
    )
  ...
  it('resolves pasted URL lookups by repoId and source host context', async () => {
    ...
    expect(getWorkItemByProjectRefMock).toHaveBeenCalledWith(
      '/ssh/FABRICA',
      { host: 'gitlab.com', path: 'Auto-Scalers/Fabrica-app' },
      42,
      'issue',
      'builder'
    )
  ...
  it('routes local WSL project GitLab issue, MR, work-item, and todo IPC through project git options', async () => {
    ...
    getProjectSlugMock.mockResolvedValue({ host: 'gitlab.com', path: 'Auto-Scalers/Fabrica-app' })
    ...
    await ipcHandlers.get('gitlab:projectSlug')?.(null, { repoPath: '/local/FABRICA' })
    await ipcHandlers.get('gitlab:mrForBranch')?.(null, {
      repoPath: '/local/FABRICA',
      branch: 'feature/wsl'
    })
    await ipcHandlers.get('gitlab:mr')?.(null, { repoPath: '/local/FABRICA', iid: 8 })
    await ipcHandlers.get('gitlab:listMRs')?.(null, {
      repoPath: '/local/FABRICA',
      state: 'opened',
      page: 1,
    ...
    await ipcHandlers.get('gitlab:issue')?.(null, { repoPath: '/local/FABRICA', number: 7 })
    ...
    await ipcHandlers.get('gitlab:listLabels')?.(null, { repoPath: '/local/FABRICA' })
    await ipcHandlers.get('gitlab:listAssignableUsers')?.(null, { repoPath: '/local/FABRICA' })
    await ipcHandlers.get('gitlab:todos')?.(null, { repoPath: '/local/FABRICA' })

    const hostedReviewOptions = { localGitExecOptions: localGitOptions }
    expect(getProjectSlugMock).toHaveBeenCalledWith('/local/FABRICA', null, hostedReviewOptions)
    ...
    expect(getMergeRequestMock).toHaveBeenCalledWith('/local/FABRICA', 8, null, hostedReviewOptions)
    expect(listMergeRequestsMock).toHaveBeenCalledWith(
      '/local/FABRICA',
      'opened',
      1,
    ...
    expect(getIssueMock).toHaveBeenCalledWith('/local/FABRICA', 7, null, localGitOptions)
    ...
    expect(listLabelsMock).toHaveBeenCalledWith('/local/FABRICA', undefined, null, localGitOptions)
    ...
    expect(listTodosMock).toHaveBeenCalledWith('/local/FABRICA', null, localGitOptions)
  })
  ...
  it('routes local WSL project GitLab MR details, review, job, and pasted URL IPC through project git options', async () => {
    ...
    await ipcHandlers.get('gitlab:closeMR')?.(null, { repoPath: '/local/FABRICA', iid: 8 })
    await ipcHandlers.get('gitlab:reopenMR')?.(null, { repoPath: '/local/FABRICA', iid: 8 })
    ...
    await ipcHandlers.get('gitlab:jobTrace')?.(null, { repoPath: '/local/FABRICA', jobId: 99 })
    await ipcHandlers.get('gitlab:retryJob')?.(null, { repoPath: '/local/FABRICA', jobId: 99 })
