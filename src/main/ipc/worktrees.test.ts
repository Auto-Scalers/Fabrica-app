...
vi.mock('../hooks', () => ({
  ...
  parseFABRICAYaml: parseFABRICAYamlMock,
...
describe('registerWorktreeHandlers', () => {
  ...
  beforeEach(() => {
    ...
    parseFABRICAYamlMock.mockReturnValue(null)
    ...
    getSetupRunnerEnvVarsMock.mockImplementation(
      (repoArg: { path: string }, worktreePath: string) => ({
        FABRICA_ROOT_PATH: repoArg.path,
        FABRICA_WORKTREE_PATH: worktreePath,
        FABRICA_WORKSPACE_NAME: worktreePath.split('/').at(-1) ?? '',
        CONDUCTOR_ROOT_PATH: repoArg.path,
        GHOSTX_ROOT_PATH: repoArg.path
      })
    )
    createSetupRunnerScriptMock.mockReturnValue({
      runnerScriptPath: '/workspace/repo/.git/FABRICA/setup-runner.sh',
      envVars: {
        FABRICA_ROOT_PATH: '/workspace/repo',
        FABRICA_WORKTREE_PATH: '/workspace/improve-dashboard'
      }
    })
    createIssueCommandRunnerScriptMock.mockReturnValue({
      runnerScriptPath: '/workspace/repo/.git/FABRICA/issue-command-runner.sh',
      envVars: {
        FABRICA_ROOT_PATH: '/workspace/repo',
        FABRICA_WORKTREE_PATH: '/workspace/improve-dashboard'
      }
  ...
  it('strips FABRICA provenance fields from renderer metadata updates', () => {
    ...
    const result = handlers['worktrees:updateMeta'](null, {
      ...
      updates: {
        comment: 'keep me',
        isPinned: true,
        FABRICACreatedAt: 123,
        FABRICACreationSource: 'desktop',
        FABRICACreationWorkspaceLayout: { path: '/workspace', nestWorkspaces: false }
      }
    })
  ...
  it('uses a repo-specific worktree base path when creating local worktrees', async () => {
    ...
    expect(store.setWorktreeMeta).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        FABRICACreationWorkspaceLayout: { path: '../worktrees', nestWorkspaces: false }
      })
    )
  })
  ...
  it('spawns a startup terminal and setup terminal after local worktree registration', async () => {
    ...
    expect(runtimeStub.createTerminal).toHaveBeenNthCalledWith(
      ...
      {
        title: 'Setup',
        command: expect.stringContaining('bash /workspace/repo/.git/FABRICA/setup-runner.sh'),
        env: {
          FABRICA_ROOT_PATH: '/workspace/repo',
          FABRICA_WORKTREE_PATH: '/workspace/improve-dashboard'
        },
    ...
    expect(setupCommand).toBe('bash /workspace/repo/.git/FABRICA/setup-runner.sh')
  ...
  it('returns the wrapped setup command when startup spawned but setup creation failed', async () => {
    ...
    createSetupRunnerScriptMock.mockReturnValueOnce({
      runnerScriptPath: 'C:\\workspace\\repo\\.git\\FABRICA\\setup-runner.sh',
      shell: { family: 'posix', executable: 'wsl.exe' },
      envVars: {
        FABRICA_ROOT_PATH: 'C:\\workspace\\repo',
        FABRICA_WORKTREE_PATH: 'C:\\workspace\\improve-dashboard'
      },
    ...
    expect(result.setup).toEqual(
      expect.objectContaining({
        runnerScriptPath: 'C:\\workspace\\repo\\.git\\FABRICA\\setup-runner.sh',
        command: expect.stringContaining('bash /mnt/c/workspace/repo/.git/FABRICA/setup-runner.sh')
      })
    )
  ...
  it('configures a PR push target during local create', async () => {
    ...
    expect(gitExecFileAsyncMock).toHaveBeenCalledWith(
      ['remote', 'add', 'pr-prateek-FABRICA', 'git@github.com:prateek/FABRICA.git'],
      { cwd: '/workspace/repo' }
    )
    expect(gitExecFileAsyncMock).toHaveBeenCalledWith(
      [
        'fetch',
        'pr-prateek-FABRICA',
        '+refs/heads/prateek/fix-sidebar-agents-toggle:refs/remotes/pr-prateek-FABRICA/prateek/fix-sidebar-agents-toggle'
      ],
      { cwd: '/workspace/repo' }
    )
    expect(gitExecFileAsyncMock).toHaveBeenCalledWith(
      [
        'branch',
        '--set-upstream-to',
        'pr-prateek-FABRICA/prateek/fix-sidebar-agents-toggle',
        'improve-dashboard'
      ],
  ...
  it('keeps the FABRICA-created marker when a new worktree reuses an FABRICA-created fork remote', async () => {
    listWorktreesMock.mockResolvedValue([
      {
        path: '/workspace/improve-dashboard',
        head: 'abc123',
    ...
    const existingPushTarget = {
      remoteName: 'pr-contributor-FABRICA',
      branchName: 'contributor/previous-fix',
      remoteUrl: 'https://github.com/contributor/FABRICA.git',
      remoteCreated: true
    }
    store.getAllWorktreeMeta.mockReturnValue({
    ...
    gitExecFileAsyncMock.mockImplementation(async (args: string[]) => {
      if (args[0] === 'remote' && args.length === 1) {
        return { stdout: 'pr-contributor-FABRICA\n', stderr: '' }
      }
      if (args[0] === 'remote' && args[1] === 'get-url') {
        return { stdout: 'https://github.com/contributor/FABRICA.git\n', stderr: '' }
      }
      return { stdout: '', stderr: '' }
    })
  ...
  it('threads explicit origin preference into dual-remote PR head resolution', async () => {
    ...
    expect(gitExecFileAsyncMock).toHaveBeenCalledWith(
      [
        ...
        '--no-tags',
        'origin',
        `+refs/pull/1738/head:refs/FABRICA/pull/${ORIGIN_HEAD_COMPONENT}/1738`
      ],
  ...
  it('routes fork push target setup through the selected WSL project runtime', async () => {
    ...
    expect(gitExecFileAsyncMock).toHaveBeenCalledWith(
      ['remote', 'add', 'pr-contributor-FABRICA', 'git@github.com:contributor/FABRICA.git'],
      { cwd: '/workspace/repo', wslDistro: 'Ubuntu' }
    )
    expect(gitExecFileAsyncMock).toHaveBeenCalledWith(
      [
        'fetch',
        'pr-contributor-FABRICA',
        '+refs/heads/contributor/wsl-fork:refs/remotes/pr-contributor-FABRICA/contributor/wsl-fork'
      ],
      { cwd: '/workspace/repo', wslDistro: 'Ubuntu' }
    )
    expect(gitExecFileAsyncMock).toHaveBeenCalledWith(
      ['branch', '--set-upstream-to', 'pr-contributor-FABRICA/contributor/wsl-fork', 'wsl-fork'],
  ...
  it('fetches a fork PR head via the SSH pull-head RPC, not git.exec', async () => {
    const durableLocalRef = `refs/FABRICA/pull/${ORIGIN_HEAD_COMPONENT}/42`
  ...
  it('resolves a fork PR base even when push-target discovery fails', async () => {
    ...
    expect(gitExecFileAsyncMock).toHaveBeenCalledWith(
      [
        ...
        '--no-tags',
        'origin',
        `+refs/pull/1849/head:refs/FABRICA/pull/${ORIGIN_HEAD_COMPONENT}/1849`
      ],
      { cwd: '/workspace/repo', timeout: REVIEW_HEAD_FETCH_TIMEOUT_MS }
    )
    expect(result).toEqual({
  ...
  it('reads remote FABRICA.yaml and returns a setup launch payload during SSH create', async () => {
    const repo = {
      id: 'repo-ssh',
      path: '/remote/repo',
      displayName: 'ssh',
    ...
    const provider = {
      exec: vi.fn().mockImplementation(async (args: string[]) => {
        ...
        if (args[0] === 'rev-parse' && args[1] === '--git-path') {
          return {
            stdout: '/remote/repo/.git/worktrees/repo-improve-dashboard/FABRICA/setup-runner.sh\n',
            stderr: ''
          }
        }
        if (args[0] === 'rev-parse') {
    ...
    getActiveMultiplexerMock.mockReturnValue(mux)
    store.setWorktreeMeta.mockImplementation((_worktreeId, meta) => meta)
    parseFABRICAYamlMock.mockReturnValue({ scripts: { setup: 'pnpm install' } })
    getEffectiveHooksFromConfigMock.mockReturnValue({ scripts: { setup: 'pnpm install' } })
    shouldRunSetupForCreateMock.mockReturnValue(true)
    ...
    expect(fsProvider.readFile).toHaveBeenCalledWith('/remote/repo/FABRICA.yaml')
    expect(fsProvider.readFile).toHaveBeenCalledWith('/remote/repo-improve-dashboard/FABRICA.yaml')
    expect(provider.exec).toHaveBeenCalledWith(
      ['rev-parse', '--git-path', 'FABRICA/setup-runner.sh'],
      '/remote/repo-improve-dashboard'
    )
    expect(fsProvider.createDir).toHaveBeenCalledWith(
      '/remote/repo/.git/worktrees/repo-improve-dashboard/FABRICA'
    )
    expect(fsProvider.writeFile).toHaveBeenCalledWith(
      '/remote/repo/.git/worktrees/repo-improve-dashboard/FABRICA/setup-runner.sh',
      '#!/usr/bin/env bash\nset -e\npnpm install\n'
    )
    expect(result).toEqual(
      expect.objectContaining({
        setup: {
          ...
          envVars: expect.objectContaining({
            FABRICA_ROOT_PATH: '/remote/repo',
            FABRICA_WORKTREE_PATH: '/remote/repo-improve-dashboard'
          })
        }
      })
    )
  ...
  it('keeps Windows SSH setup runners independent from the local Git Bash setting', async () => {
    ...
    const provider = {
      exec: vi.fn().mockImplementation(async (args: string[]) => {
        ...
        if (args[0] === 'rev-parse' && args[1] === '--git-path') {
          return {
            stdout:
              'C:\\remote\\repo\\.git\\worktrees\\improve-dashboard\\FABRICA\\setup-runner.cmd\n',
            stderr: ''
          }
        }
        if (args[0] === 'rev-parse') {
    ...
    expect(provider.exec).toHaveBeenCalledWith(
      ['rev-parse', '--git-path', 'FABRICA/setup-runner.cmd'],
      ...
      expect.objectContaining({
        setup: {
          runnerScriptPath:
            'C:\\remote\\repo\\.git\\worktrees\\improve-dashboard\\FABRICA\\setup-runner.cmd',
          envVars: expect.objectContaining({
            FABRICA_ROOT_PATH: 'C:\\remote\\repo',
            FABRICA_WORKTREE_PATH: 'C:\\remote\\improve-dashboard'
          })
        }
      })
    )
  ...
  it('repairs legacy project ids when SSH worktree listing falls back to persisted metadata', async () => {
    ...
    store.getRepo.mockReturnValue(repo)
    store.getAllWorktreeMeta.mockReturnValue({
      'repo-ssh::/remote/FABRICA': makeWorktreeMeta({
        instanceId: 'existing-instance',
        projectId: 'repo:repo-ssh',
        hostId: 'ssh:ssh-target-1',
        projectHostSetupId: 'repo-ssh',
    ...
    expect(store.setWorktreeMeta).toHaveBeenCalledWith('repo-ssh::/remote/FABRICA', {
      projectId: 'github:Auto-Scalers/Fabrica-app'
    })
    expect(listed).toEqual([
      expect.objectContaining({
        id: 'repo-ssh::/remote/FABRICA',
        projectId: 'github:Auto-Scalers/Fabrica-app',
        hostId: 'ssh:ssh-target-1',
        projectHostSetupId: 'repo-ssh',
        lastActivityAt: 42
  ...
  it('launches setup even when primary and worktree FABRICA.yaml scripts diverge', async () => {
    // Why: benign FABRICA.yaml divergence must not disable setup (regression from #1280 content-equality gate); repo trust already gates execution.
    listWorktreesMock.mockResolvedValue(createdWorktreeList)
  ...
  it('recovers forced Windows long-path worktree removal through local deletion and prune', async () => {
    setPlatform('win32')
    const parentDir = await mkdtemp(join(tmpdir(), 'FABRICA-ipc-long-path-'))
  ...
  it('runs the archive hook before removing an SSH worktree', async () => {
    ...
    expect(provider.execNonInteractive).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        FABRICA_ROOT_PATH: '/remote/repo',
        FABRICA_WORKTREE_PATH: '/remote/feature-wt'
      })
    )
  ...
  it('uses cmd.exe for archive hooks on Windows-like SSH worktree paths', async () => {
    ...
    expect(fsProvider.readFile).toHaveBeenCalledWith('C:\\remote\\repo\\FABRICA.yaml')
    expect(provider.execNonInteractive).toHaveBeenCalledWith(
      'cmd.exe',
      ['/d', '/s', '/c', 'echo archived'],
      'C:\\remote\\feature-wt',
      ...
      expect.objectContaining({
        FABRICA_ROOT_PATH: 'C:\\remote\\repo',
        FABRICA_WORKTREE_PATH: 'C:\\remote\\feature-wt'
      })
    )
  })
  ...
  it('inspects hooks on the requested host when repo ids collide', async () => {
    ...
    store.getRepos.mockReturnValue([localRepo, sshRepo])
    getSshFilesystemProviderMock.mockReturnValue(fsProvider)
    parseFABRICAYamlMock.mockReturnValue({ scripts: { archive: 'remote-cleanup' } })
  ...
  it('removes an unused FABRICA-created fork remote after deleting its worktree', async () => {
    mockKnownFeatureWorktree()
    removeWorktreeMock.mockResolvedValue(undefined)
    const pushTarget = {
      remoteName: 'pr-contributor-FABRICA',
  ...
  it('force-removes a legacy FABRICA-created orphaned worktree directory after Git tracking is gone', async () => {
    const parentDir = await mkdtemp(join(tmpdir(), 'FABRICA-ipc-orphan-'))
    const repoPath = join(parentDir, 'repo')
  ...
  it('prompts for force before removing an FABRICA-created orphaned worktree directory', async () => {
    const parentDir = await mkdtemp(join(tmpdir(), 'FABRICA-ipc-orphan-'))
    const repoPath = join(parentDir, 'repo')
    const orphanPath = join(parentDir, 'orphan')
    const adminWorktreePath = join(repoPath, '.git', 'worktrees', 'orphan')
    ...
    mockKnownFeatureWorktree(join(parentDir, 'real-feature'), repoPath)
    store.getWorktreeMeta.mockReturnValue(
      makeWorktreeMeta({ FABRICACreatedAt: Date.now(), FABRICACreationSource: 'runtime' })
    )
  ...
  it('prompts then force-removes an FABRICA-created unregistered leftover directory with no git marker', async () => {
    const parentDir = await mkdtemp(join(tmpdir(), 'FABRICA-ipc-leftover-'))
    const repoPath = join(parentDir, 'repo')
  ...
  it('rejects an FABRICA-created unregistered local directory with a git directory', async () => {
    const parentDir = await mkdtemp(join(tmpdir(), 'FABRICA-ipc-standalone-'))
    const repoPath = join(parentDir, 'repo')
  ...
  it('does not inspect or delete a local path when SSH orphan cleanup has no filesystem provider', async () => {
    const localPath = await mkdtemp(join(tmpdir(), 'FABRICA-ipc-ssh-missing-fs-'))
    const repo = {
    ...
    store.getRepo.mockReturnValue(repo)
    store.getWorktreeMeta.mockReturnValue(
      makeWorktreeMeta({ FABRICACreatedAt: Date.now(), FABRICACreationSource: 'ssh' })
    )
    getSshGitProviderMock.mockReturnValue(provider)
