...
import type { FABRICARuntimeService } from '../runtime/fabrica-runtime'
...
import { ensureLinuxTerminalFABRICACliShimDir } from '../cli/linux-terminal-fabrica-cli-shim'
...
import { addFABRICAWslInteropEnv, stampWslOrchestrationCompatibilityHost } from '../pty/wsl-fabrica-env'
...
const AGENT_HOOK_RUNTIME_ENV_KEYS = [
  'FABRICA_AGENT_HOOK_PORT',
  'FABRICA_AGENT_HOOK_TOKEN',
  'FABRICA_AGENT_HOOK_ENV',
  'FABRICA_AGENT_HOOK_VERSION',
  'FABRICA_AGENT_HOOK_ENDPOINT',
...
// Why: FABRICA never sets these, so an inherited value means a pty host launched from inside a Claude session — Claude reads it as a nested child and silently stops persisting the transcript.
...
function seedTerminalRestoreRecordsFromSpawnResult(
  runtime: FABRICARuntimeService | undefined,
...
function admitRendererAgentLaunchAuthority(args: {
  ...
  if (
    ...
    args.spawnEnv?.FABRICA_AGENT_LAUNCH_TOKEN !== args.launchToken
  ) {
    return null
  }
...
function resolveStablePaneOwner(
  ...
  let resolved: ReturnType<FABRICARuntimeService['resolveTerminalPane']> | null = null
  let resolvedHandleCandidate: ReturnType<FABRICARuntimeService['resolveTerminalPane']> | null = null
...
function stripRemotePaneEnvWhenHooksDisabled(
  ...
  if (
    !env ||
    (!('FABRICA_PANE_KEY' in env) &&
      !('FABRICA_TAB_ID' in env) &&
      !('FABRICA_WORKTREE_ID' in env) &&
      !('FABRICA_AGENT_LAUNCH_TOKEN' in env))
  ) {
    return env
  }
  const stripped = { ...env }
  delete stripped.FABRICA_PANE_KEY
  delete stripped.FABRICA_TAB_ID
  delete stripped.FABRICA_WORKTREE_ID
  delete stripped.FABRICA_AGENT_LAUNCH_TOKEN
  return stripped
}
...
export type BuildPtyHostEnvOptions = {
   ...
   *  and strip only an inherited FABRICA-owned override so nested FABRICA panes do not
   *  leak the parent's managed home. A user-set CODEX_HOME is preserved. */
  stripInheritedFABRICACodexHome?: boolean
...
function promoteAgentTeamsShimPath(
  ...
  if (!env?.FABRICA_AGENT_TEAMS_TEAM_ID) {
    return
  }
  ...
  // Why: host env injection prepends FABRICA's shims; Claude Agent Teams must still resolve our fake tmux before any real tmux.
...
// null managed home. Signal the env builder to strip a nested-FABRICA-inherited
// override instead of injecting one, so Codex runs on the user's own ~/.codex.
function shouldStripInheritedFABRICACodexHome(args: {
...
const CODEX_HOME_ENV_KEYS = ['CODEX_HOME', 'FABRICA_CODEX_HOME'] as const

// Why: system-default real-home routing runs Codex on the user's own ~/.codex.
// Nested FABRICA panes inherit the parent's FABRICA-owned override; strip only that
// (CODEX_HOME matching FABRICA's private FABRICA_CODEX_HOME marker), and always drop
// the marker so a shell-ready wrapper cannot restore the managed home. A
// user-set CODEX_HOME with no FABRICA marker is preserved untouched (see #8606).
function stripInheritedFABRICACodexHomeOverride(baseEnv: Record<string, string>): void {
  for (const key of getLocalFABRICACodexHomeEnvKeysToDelete(baseEnv)) {
    delete baseEnv[key]
  }
}
...
function getLocalFABRICACodexHomeEnvKeysToDelete(env: Record<string, string>): string[] {
  const inheritedFABRICAOverride = env.FABRICA_CODEX_HOME ?? process.env.FABRICA_CODEX_HOME
  const inheritedCodexHome = env.CODEX_HOME ?? process.env.CODEX_HOME
  const keysToDelete = ['FABRICA_CODEX_HOME']
  if (inheritedFABRICAOverride && inheritedCodexHome === inheritedFABRICAOverride) {
...
function resolvePiAgentSourceDir(
  ...
  const overlayKey = kind === 'omp' ? 'FABRICA_OMP_CODING_AGENT_DIR' : 'FABRICA_PI_CODING_AGENT_DIR'
  const otherOverlayKey = kind === 'omp' ? 'FABRICA_PI_CODING_AGENT_DIR' : 'FABRICA_OMP_CODING_AGENT_DIR'
  ...
  // Why: if PI_CODING_AGENT_DIR is a restored FABRICA overlay with no source shadow, remirroring leaks another agent's overlay tree; fall through to defaults.
...
function clearPiAgentShadowEnv(baseEnv: Record<string, string>, kind: PiAgentKind): void {
  if (kind === 'omp') {
    delete baseEnv.FABRICA_OMP_CODING_AGENT_DIR
    delete baseEnv.FABRICA_OMP_SOURCE_AGENT_DIR
    delete baseEnv.FABRICA_OMP_STATUS_EXTENSION
    return
  }
  if (kind === 'prime-agent') {
    delete baseEnv.FABRICA_PRIME_AGENT_SOURCE_AGENT_DIR
    delete baseEnv.FABRICA_PRIME_AGENT_STATUS_EXTENSION
    return
  }
  delete baseEnv.FABRICA_PI_CODING_AGENT_DIR
  delete baseEnv.FABRICA_PI_SOURCE_AGENT_DIR
}
...
function exposePiManagedExtensionEnv(
  ...
  if (kind === 'omp') {
    delete baseEnv.FABRICA_OMP_CODING_AGENT_DIR
    if (managedEnv.FABRICA_OMP_SOURCE_AGENT_DIR) {
      baseEnv.FABRICA_OMP_SOURCE_AGENT_DIR = managedEnv.FABRICA_OMP_SOURCE_AGENT_DIR
    } else {
      delete baseEnv.FABRICA_OMP_SOURCE_AGENT_DIR
    }
    if (managedEnv.FABRICA_OMP_STATUS_EXTENSION) {
      baseEnv.FABRICA_OMP_STATUS_EXTENSION = managedEnv.FABRICA_OMP_STATUS_EXTENSION
    } else {
      delete baseEnv.FABRICA_OMP_STATUS_EXTENSION
    }
    return
  ...
  if (kind === 'prime-agent') {
    if (managedEnv.FABRICA_PRIME_AGENT_SOURCE_AGENT_DIR) {
      baseEnv.FABRICA_PRIME_AGENT_SOURCE_AGENT_DIR = managedEnv.FABRICA_PRIME_AGENT_SOURCE_AGENT_DIR
    } else {
      delete baseEnv.FABRICA_PRIME_AGENT_SOURCE_AGENT_DIR
    }
    if (managedEnv.FABRICA_PRIME_AGENT_STATUS_EXTENSION) {
      baseEnv.FABRICA_PRIME_AGENT_STATUS_EXTENSION = managedEnv.FABRICA_PRIME_AGENT_STATUS_EXTENSION
    } else {
      delete baseEnv.FABRICA_PRIME_AGENT_STATUS_EXTENSION
    }
    return
  ...
  delete baseEnv.FABRICA_PI_CODING_AGENT_DIR
  if (managedEnv.FABRICA_PI_SOURCE_AGENT_DIR) {
    baseEnv.FABRICA_PI_SOURCE_AGENT_DIR = managedEnv.FABRICA_PI_SOURCE_AGENT_DIR
  } else {
    delete baseEnv.FABRICA_PI_SOURCE_AGENT_DIR
  }
}
...
function removeCodexHomeDeletionRequests(keys: string[] | undefined): string[] | undefined {
  ...
  const filtered = keys?.filter((key) => key !== 'CODEX_HOME' && key !== 'FABRICA_CODEX_HOME')
...
// Why: a nested terminal can inherit prior OpenCode/Pi/OMP overlay env; restore the user's recorded source dir, else strip only FABRICA-owned values.
...
function resolveMimocodeSourceHome(baseEnv: Record<string, string>): string | undefined {
  const sourceHome = baseEnv.FABRICA_MIMOCODE_SOURCE_HOME ?? process.env.FABRICA_MIMOCODE_SOURCE_HOME
  if (sourceHome) {
    return sourceHome
  }
  const configHome = baseEnv.MIMOCODE_HOME ?? process.env.MIMOCODE_HOME
  const FABRICAHome = baseEnv.FABRICA_MIMOCODE_HOME ?? process.env.FABRICA_MIMOCODE_HOME
  if (configHome && FABRICAHome && configHome === FABRICAHome) {
    return undefined
  }
  return configHome
}
...
function resolveOpenCodeSourceConfigDir(baseEnv: Record<string, string>): string | undefined {
  const sourceDir =
    baseEnv.FABRICA_OPENCODE_SOURCE_CONFIG_DIR ?? process.env.FABRICA_OPENCODE_SOURCE_CONFIG_DIR
  if (sourceDir) {
    return sourceDir
  }

  const configDir = baseEnv.OPENCODE_CONFIG_DIR ?? process.env.OPENCODE_CONFIG_DIR
  const FABRICAConfigDir = baseEnv.FABRICA_OPENCODE_CONFIG_DIR ?? process.env.FABRICA_OPENCODE_CONFIG_DIR
  // Why: with no recorded source dir, an inherited OPENCODE_CONFIG_DIR is FABRICA-owned, not user config; treating it as user config makes child FABRICAs mirror the hook dir.
  if (configDir && FABRICAConfigDir && configDir === FABRICAConfigDir) {
    return undefined
  }
...
export function buildPtyHostEnv(
  ...
  if (opts.agentStatusHooksEnabled) {
    // Why: OPENCODE_CONFIG_DIR is a single path, not a colon-list; mirror the user's value into an overlay so their plugins and FABRICA's status plugin coexist. See docs/opencode-config-dir-collision.md.
    ...
    if (baseEnv.OPENCODE_CONFIG_DIR) {
      ...
      baseEnv.FABRICA_OPENCODE_CONFIG_DIR = baseEnv.OPENCODE_CONFIG_DIR
      if (preexistingOpenCodeConfigDir) {
        // Why: nested FABRICA terminals inherit the overlay as OPENCODE_CONFIG_DIR; keep the real source so overlays don't mirror overlays.
        baseEnv.FABRICA_OPENCODE_SOURCE_CONFIG_DIR = preexistingOpenCodeConfigDir
      } else {
        delete baseEnv.FABRICA_OPENCODE_SOURCE_CONFIG_DIR
      }
    }
    if (isMimoLaunchCommand(launchCommandHint)) {
      ...
      if (baseEnv.MIMOCODE_HOME) {
        baseEnv.FABRICA_MIMOCODE_HOME = baseEnv.MIMOCODE_HOME
        if (preexistingMimocodeHome) {
          baseEnv.FABRICA_MIMOCODE_SOURCE_HOME = preexistingMimocodeHome
        } else {
          delete baseEnv.FABRICA_MIMOCODE_SOURCE_HOME
        }
      }
    }
  } else {
    restoreOrStripOverlayEnv(baseEnv, {
      ...
      overlay: 'FABRICA_OPENCODE_CONFIG_DIR',
      source: 'FABRICA_OPENCODE_SOURCE_CONFIG_DIR'
    })
    restoreOrStripOverlayEnv(baseEnv, {
      primary: 'MIMOCODE_HOME',
      overlay: 'FABRICA_MIMOCODE_HOME',
      source: 'FABRICA_MIMOCODE_SOURCE_HOME'
    })
  }
  ...
  if (opts.agentStatusHooksEnabled) {
    ...
    if (opts.isWsl === true) {
      ...
      if (hookInstance) {
        baseEnv.FABRICA_WSL_HOOK_INSTANCE = hookInstance
      } else {
        delete baseEnv.FABRICA_WSL_HOOK_INSTANCE
      }
      wslHookRelayManager.ensureForDistro(distro)
      const guestEndpoint = wslHookRelayManager.getGuestEndpointFilePath(distro)
      if (guestEndpoint) {
        baseEnv.FABRICA_AGENT_HOOK_ENDPOINT = guestEndpoint
      }
      ...
      if (opencodeOverlayDir) {
        ...
        baseEnv.FABRICA_OPENCODE_CONFIG_DIR = opencodeOverlayDir
        delete baseEnv.FABRICA_OPENCODE_SOURCE_CONFIG_DIR
      } else {
        ...
        delete baseEnv.FABRICA_OPENCODE_CONFIG_DIR
        delete baseEnv.FABRICA_OPENCODE_SOURCE_CONFIG_DIR
      }
    }
  }

  // Why: PI_CODING_AGENT_DIR is the user's config/session root; install only FABRICA-owned extension files, don't override it.
  if (opts.agentStatusHooksEnabled) {
    ...
    restoreOrStripOverlayEnv(baseEnv, {
      ...
      overlay: 'FABRICA_PI_CODING_AGENT_DIR',
      source: 'FABRICA_PI_SOURCE_AGENT_DIR'
    })
    restoreOrStripOverlayEnv(baseEnv, {
      primary: 'PI_CODING_AGENT_DIR',
      overlay: 'FABRICA_OMP_CODING_AGENT_DIR',
      source: 'FABRICA_OMP_SOURCE_AGENT_DIR'
    })
    delete baseEnv.FABRICA_OMP_STATUS_EXTENSION
    delete baseEnv.FABRICA_PRIME_AGENT_SOURCE_AGENT_DIR
    delete baseEnv.FABRICA_PRIME_AGENT_STATUS_EXTENSION
  ...
  // Why: keep the Codex home override PTY-scoped so dev/prod FABRICAs don't share hooks through ~/.codex.
  if (opts.skipCodexHomeEnv) {
    delete baseEnv.CODEX_HOME
    delete baseEnv.FABRICA_CODEX_HOME
    ...
    baseEnv.FABRICA_CODEX_HOME = opts.selectedCodexHomePath
  } else if (opts.stripInheritedFABRICACodexHome) {
    stripInheritedFABRICACodexHomeOverride(baseEnv)
  }

  // Why: WSL shells need the managed userData root for shell-ready wrappers; dev-mode terminals need the same export so `FABRICA` targets the live dev instance.
  if (opts.isWsl) {
    baseEnv.FABRICA_USER_DATA_PATH = opts.userDataPath
    // Why: managed WSL registration uses `fabrica`; exposing that literal scopes agent guidance to WSL without a bare-fabrica shim.
    baseEnv.FABRICA_CLI_COMMAND = opts.isPackaged ? 'fabrica' : 'fabrica-dev'
  } else {
    if (!opts.isPackaged) {
      baseEnv.FABRICA_USER_DATA_PATH ??= opts.userDataPath
    }
    delete baseEnv.FABRICA_CLI_COMMAND
  }
  // Why: dev mode needs the launcher PATH override so `FABRICA` resolves to the dev build instead of the production binary at /usr/local/bin/FABRICA.
  if (!opts.isPackaged) {
    ...
    // Why: bare-`fabrica` shim scoped to Fabrica PTYs — Linux CLI installs as `fabrica` to avoid shadowing GNOME's /usr/bin/FABRICA screen reader (Auto-Scalers/Fabrica#7904).
    const shimDir = ensureLinuxTerminalFABRICACliShimDir({ userDataPath: opts.userDataPath })
    if (shimDir) {
    ...
    // Why: global CLI registration is optional, but agents in FABRICA-managed PTYs must always reach this app's bundled CLI.
  ...
  // Why: PATH shims keep GitHub attribution scoped to FABRICA's own PTYs without rewriting user git config.
  if (!opts.githubAttributionEnabled) {
    delete baseEnv.FABRICA_ENABLE_GIT_ATTRIBUTION
    delete baseEnv.FABRICA_GIT_COMMIT_TRAILER
    delete baseEnv.FABRICA_GH_PR_FOOTER
    delete baseEnv.FABRICA_GH_ISSUE_FOOTER
    delete baseEnv.FABRICA_ATTRIBUTION_SHIM_DIR
  }
...
export function registerPtyHandlers(
    ...
    localProvider.configure({
      ...
      buildSpawnEnv: (id, baseEnv, ctx) => {
        ...
        const env = buildPtyHostEnv(id, baseEnv, {
          ...
          selectedCodexHomePath,
          skipCodexHomeEnv,
          stripInheritedFABRICACodexHome: shouldStripInheritedFABRICACodexHome({
        ...
        const requestedHandle = baseEnv.FABRICA_TERMINAL_HANDLE
        ...
        if (requestedHandle && requestedHandle !== preAllocatedHandle) {
          delete env.FABRICA_TERMINAL_HANDLE
        }
        if (preAllocatedHandle) {
          env.FABRICA_TERMINAL_HANDLE = preAllocatedHandle
        }
  ...
  /** Why: buildPtyHostEnv prefers FABRICA_SEQUENCED_STARTUP_COMMAND over the launch command
  ...
  runtime?.setPtyController({
    ...
    spawn: async (args) => {
      ...
      let env: Record<string, string> | undefined = claudeAuth
        ...
        : sshScopedEnv
      const requestedAgentTeamsPath = env?.FABRICA_AGENT_TEAMS_TEAM_ID
        ? env[resolvePathEnvKey(env, process.platform)]
        : undefined
      env = stripSequencedStartupResumeArgv(env, codexResumeLaunch)
      if (args.preAllocatedHandle) {
        env = { ...env, FABRICA_TERMINAL_HANDLE: args.preAllocatedHandle }
      }
      ...
      const skipCodexHomeEnv =
        ...
        !selectedCodexHomePath
      const stripInheritedFABRICACodexHome =
        isDaemonHostSpawn &&
        shouldStripInheritedFABRICACodexHome({
          target: codexSelectionTarget,
          selectedCodexHomePath,
          skipCodexHomeEnv,
          settings: getSettings?.()
      ...
      if (isDaemonHostSpawn && sessionId && !preAdoptedStablePane) {
        ...
        env = buildPtyHostEnv(sessionId, env ?? {}, {
          ...
          selectedCodexHomePath,
          skipCodexHomeEnv,
          stripInheritedFABRICACodexHome,
          githubAttributionEnabled: getSettings?.()?.enableGitHubAttribution ?? false,
          launchCommand,
          launchAgent: isTuiAgent(args.launchAgent) ? args.launchAgent : undefined,
      ...
      } else if (stripInheritedFABRICACodexHome) {
        // Why: the daemon owns a persistent inherited environment that may
        // differ from main. FABRICA_CODEX_HOME asks it to compare/delete the pair.
      ...
      try {
        ...
        const paneKey = rememberPaneKeyForPty(result.id, env?.FABRICA_PANE_KEY)
    ...
    async (
      ...
      try {
        ...
        const baseEnvWithAuth = claudeAuth
          ...
          : sshSourceEnv
        const spawnPaneKey = baseEnvWithAuth?.FABRICA_PANE_KEY
        ...
        const requestedAgentTeamsPath = baseEnv?.FABRICA_AGENT_TEAMS_TEAM_ID
          ? baseEnv[resolvePathEnvKey(baseEnv, process.platform)]
          : undefined
        const agentTeamsEnvToDelete = shouldRefreshAgentTeamsEnv
          ? ['TERM_PROGRAM', 'FABRICA_ATTRIBUTION_SHIM_DIR']
          : undefined
        if (baseEnv && stablePaneKey) {
          baseEnv.FABRICA_PANE_KEY = stablePaneKey
          if (typeof args.tabId === 'string') {
            baseEnv.FABRICA_TAB_ID = args.tabId
          } else if (!args.connectionId) {
            delete baseEnv.FABRICA_TAB_ID
          }
          if (typeof args.worktreeId === 'string') {
            baseEnv.FABRICA_WORKTREE_ID = args.worktreeId
          } else if (!args.connectionId) {
            delete baseEnv.FABRICA_WORKTREE_ID
          }
        } else if (baseEnv) {
          // Why: FABRICA_PANE_KEY crosses into shells/hook registries; only a key proven to match this spawn's tab+leaf may cross the IPC boundary.
          delete baseEnv.FABRICA_PANE_KEY
          delete baseEnv.FABRICA_TAB_ID
          delete baseEnv.FABRICA_WORKTREE_ID
          delete baseEnv.FABRICA_AGENT_LAUNCH_TOKEN
        }
        const validatedPaneKey = stablePaneKey
        // Why: SSH can strip FABRICA_PANE_KEY when remote hooks are off; IPC tab/leaf metadata still names the pane.
        ...
        let combinedEnvToDelete = mergePtyEnvDeletions(
          ...
          stripInheritedFABRICACodexHome ? ['FABRICA_CODEX_HOME'] : []
        )
        if (codexResumeHomeSelected) {
        ...
        const relayResultId = getRelayPtyId(args.connectionId, result.id)
        if (store && args.connectionId) {
          // Why: remote PTYs live in the SSH relay grace window after FABRICA detaches; persist IDs immediately so reconnect reattaches instead of spawning a fresh shell.
          store.upsertSshRemotePtyLease({
            targetId: args.connectionId,
            ptyId: relayResultId,
            ...(typeof args.worktreeId === 'string' ? { worktreeId: args.worktreeId } : {}),
...
export function registerHeadlessPtyRuntime(
  ...
  // Why: headless `FABRICA serve` has no renderer window but still needs the same PTY handlers so remote clients can drive terminals.
