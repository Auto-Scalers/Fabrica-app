...
describe('registerEphemeralVmHandlers', () => {
  ...
  beforeEach(() => {
    ...
    connectRuntimeOwnedSshTargetMock.mockResolvedValue({
      targetId: 'runtime-ssh-FABRICA-instance-1',
      target: {
        id: 'runtime-ssh-FABRICA-instance-1',
        label: 'Sandbox',
  ...
  it('lists recipes from local repo FABRICA.yaml', async () => {
    const repoPath = makeDir('FABRICA-ephemeral-vm-ipc-repo-')
    writeFileSync(
      join(repoPath, 'FABRICA.yaml'),
      [
        'environmentRecipes:',
        '  - id: cloud-sandbox',
        '    name: Cloud Sandbox',
  ...
  it('merges approved plugin recipes while repository recipes shadow matching ids', async () => {
    ...
    const pluginService = pluginServiceWithRecipes([
      {
        pluginKey: 'fabrica-samples.recipes',
  ...
  it('uses an immutable plugin recipe snapshot after the plugin is removed', async () => {
    const userDataPath = makeDir('FABRICA-ephemeral-vm-ipc-user-data-')
    const repoPath = makeDir('FABRICA-ephemeral-vm-ipc-repo-')
  ...
  it('provisions an ssh recipe without creating a runtime environment', async () => {
    ...
    expect(result).toMatchObject({
      ok: true,
      connectionType: 'ssh',
      sshTargetId: 'runtime-ssh-FABRICA-instance-1',
      runtime: {
        repoId: 'repo-1',
        status: 'running',
        connectionMode: 'ssh',
  ...
  it('removes the runtime-owned SSH target on cleanup even when destroy fails', async () => {
    ...
    expect(removeRuntimeOwnedSshTargetMock).toHaveBeenCalledWith('runtime-ssh-FABRICA-instance-1')
