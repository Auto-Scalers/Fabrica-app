...
import {
  ...
  buildKnownFABRICAWorkspaceLayouts,
...
import {
  ...
  runHook,
  hasHooksFile,
  hasUnrecognizedFABRICAYamlKeys,
  writeIssueCommand
} from '../hooks'
import {
  mergeWorktree,
...
import type { FABRICARuntimeService, RuntimeWorktreeLifecycleEvent } from '../runtime/fabrica-runtime'
...
import {
  ...
  canCleanupUnregisteredFABRICALeftoverDirectory,
  canCleanupUnregisteredFABRICAWorktreeDirectory,
  canSafelyRemoveOrphanedWorktreeDirectory,
  findRegisteredDeletableWorktree,
  isDangerousWorktreeRemovalPath,
  isWorktreePathMissing,
  ORPHANED_WORKTREE_DIRECTORY_MESSAGE,
  stripFABRICAProvenanceMetaUpdates,
...
async function getArchiveHooksForRemoval(repo: Repo): Promise<FABRICAHooks | null> {
  if (!repo.connectionId) {
  ...
  try {
    const result = await fsProvider.readFile(joinWorktreeRelativePath(repo.path, 'FABRICA.yaml'))
    const yamlHooks = result.isBinary ? null : parseFABRICAYaml(result.content)
...
function buildDetectedGitWorktrees(
  ...
  const knownFABRICALayouts = buildKnownFABRICAWorkspaceLayouts(settings, repo)
...
async function listDetectedWorktreesFFABRICApturedRepo(
  store: Store,
  repo: Repo,
  isCurrent: () => boolean,
...
async function hydrateLineageWithinDeadline(runtime: FABRICARuntimeService): Promise<boolean> {
...
async function listHostQualifiedDetectedWorktrees(
  ...
  const result = await listDetectedWorktreesFFABRICApturedRepo(
    store,
    repo,
    isCurrent,
    provider,
...
export function registerWorktreeHandlers(
  ...
  // above re-lists a worktree deleted outside FABRICA on every launch. An authoritative scan is the only proof of
        ...
        return withWorktreeSpan({ stage: 'remove', path: worktreePath }, async () => {
          ...
          if (!registeredWorktree) {
            ...
            if (!repo.connectionId) {
              ...
              if (
                await canCleanupUnregisteredFABRICALeftoverDirectory({
                  meta: removedMeta,
                  worktreePath,
                  runtimeWorktreePath,
                  repo,
            ...
            if (await isAlreadyRemovedWorktreePath(repo, worktreePath, localWorktreeGitOptions)) {
              if (!args.force && !removedMeta) {
                // Why: without persisted metadata, require the renderer recovery path before deleting FABRICA-only state for an unregistered path.
                throw new Error(UNREGISTERED_MISSING_WORKTREE_MESSAGE)
              }
              // Why: a manually deleted worktree is already gone; persisted metadata proves it was an FABRICA-known row, so no force is needed.
              if (repo.connectionId) {
                await cleanupUnusedWorktreePushTargetRemoteSsh(
                  provider!,
                  repo.path,
          ...
          // Why: `FABRICA.yaml` shared directories are symlinked in too, and a
    ...
    (_event, args: { worktreeId: string; updates: Partial<WorktreeMeta> }) => {
      ...
      const updates =
        validatedUpdates.displayName !== undefined
          ...
          : validatedUpdates
      const meta = store.setWorktreeMeta(args.worktreeId, stripFABRICAProvenanceMetaUpdates(updates))
  ...
  ipcMain.handle(
    ...
    async (_event, args: { repoId: string; hostId?: ExecutionHostId }) => {
      ...
      if (repo.connectionId) {
        ...
        try {
          ...
          return {
            ...
            hooks: result.isBinary ? null : parseFABRICAYaml(result.content),
            mayNeedUpdate: false
          }
        } catch (error) {
          return {
      ...
      // Why: unrecognised top-level keys mean the file is well-formed but from a newer FABRICA; suggest updating rather than "could not be parsed".
      const mayNeedUpdate = has && !hooks && hasUnrecognizedFABRICAYamlKeys(repo.path)
      return {
        status: 'ok',
        hasHooks: has,
        hooks,
  ...
  ipcMain.handle(
    ...
    async (_event, args: { repoId: string; hostId?: ExecutionHostId }) => {
      ...
      if (repo.connectionId) {
        const issueCommandPath = joinWorktreeRelativePath(repo.path, '.fabrica/issue-command')
        const fsProvider = getSshFilesystemProvider(repo.connectionId)
        if (!fsProvider) {
          return {
            status: 'error',
        ...
        try {
          ...
          sharedContent = result.isBinary
            ? null
            : parseFABRICAYaml(result.content)?.issueCommand?.trim() || null
        } catch (error) {
          if (!isENOENT(error)) {
            status = 'error'
          }
  ...
  ipcMain.handle(
    ...
    async (_event, args: { repoId: string; content: string; hostId?: ExecutionHostId }) => {
      ...
      if (repo.connectionId) {
        ...
        await fsProvider.createDir(joinWorktreeRelativePath(repo.path, '.fabrica'))
        const gitignorePath = joinWorktreeRelativePath(repo.path, '.gitignore')
        try {
          const result = await fsProvider.readFile(gitignorePath)
          if (!result.isBinary && !/^\.fabrica\/?$/m.test(result.content)) {
            const separator = result.content.endsWith('\n') ? '' : '\n'
            await fsProvider.writeFile(gitignorePath, `${result.content}${separator}.fabrica\n`)
          }
        } catch (error) {
          if (!isENOENT(error)) {
            throw error
          }
          await fsProvider.writeFile(gitignorePath, '.fabrica\n')
        }
