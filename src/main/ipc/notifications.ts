...
const MACOS_PACKAGED_BUNDLE_ID = 'com.autoscalers.fabrica'
...
function probeNotificationDelivery(): Promise<NotificationDeliveryProbeResult> {
  ...
  const probe = new Notification({
    ...
    body: 'Fabrica will alert you when agents finish or terminals need attention.',
    silent: true
  })
...
function getMacNotificationSettingsUrl(): string {
  const bundleId = process.env.FABRICA_DEV_MACOS_BUNDLE_ID ?? MACOS_PACKAGED_BUNDLE_ID
...
export function registerNotificationHandlers(store: Store, runtime?: FABRICARuntimeService): void {
