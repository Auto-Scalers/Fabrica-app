...
const {
  ...
  registerFABRICAProfileHandlersMock,
  ...
  registerFABRICAProfileHandlersMock: vi.fn(),
...
vi.mock('./fabrica-profiles', () => ({
  registerFABRICAProfileHandlers: registerFABRICAProfileHandlersMock
}))
...
describe('registerCoreHandlers', () => {
  beforeEach(() => {
    ...
    registerFABRICAProfileHandlersMock.mockReset()
  ...
  it('passes the store through to handler registrars that need it', async () => {
    ...
    expect(registerFABRICAProfileHandlersMock).toHaveBeenCalledWith(store, { onBeforeRelaunch })
