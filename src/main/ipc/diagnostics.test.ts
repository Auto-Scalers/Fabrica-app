...
describe('diagnostics IPC handlers', () => {
  beforeEach(() => {
    ...
    delete (globalThis as { FABRICA_BUILD_IDENTITY?: unknown }).FABRICA_BUILD_IDENTITY
    delete (globalThis as { FABRICA_DIAGNOSTICS_TOKEN_URL?: unknown }).FABRICA_DIAGNOSTICS_TOKEN_URL
    process.env.FABRICA_DIAGNOSTICS_TOKEN_URL = 'https://diagnostics.example.com/diagnostics/token'
  ...
  it('pins official builds to the compile-time diagnostics endpoint', async () => {
    ...
    const globalOverrides = globalThis as {
      FABRICA_BUILD_IDENTITY?: 'stable'
      FABRICA_DIAGNOSTICS_TOKEN_URL?: string
    }
    globalOverrides.FABRICA_BUILD_IDENTITY = 'stable'
    globalOverrides.FABRICA_DIAGNOSTICS_TOKEN_URL = 'https://official.example.com/diagnostics/token'
    process.env.FABRICA_DIAGNOSTICS_TOKEN_URL = 'https://attacker.example.com/diagnostics/token'
    collectDiagnosticBundleMock.mockReturnValue(bundle)
    readFileSyncMock.mockReturnValue(bundle.payload)
    const collect = handlers.get('diagnostics:collectBundle')!
    const openPreview = handlers.get('diagnostics:openBundlePreview')!
