...
describe('registerGitHubHandlers', () => {
  ...
  it('routes local WSL project GitHub PR detail and action IPC through project git options', async () => {
    ...
    const prRepo = { owner: 'acme', repo: 'fabrica', host: 'github.acme-corp.com' }
    const localGitOptions = { wslDistro: 'Ubuntu' }
    getWorkItemMock.mockResolvedValue(null)
  ...
  it('threads SSH connectionId through pull request merge', async () => {
    ...
    await handlers['gh:mergePR'](
      { sender: { id: 1 } },
      {
        ...
        prNumber: 42,
        method: 'squash',
        prRepo: { owner: 'acme', repo: 'fabrica' }
      }
    )
  ...
  it('emits app_starred_FABRICA once after a successful star with cohort context', async () => {
    starFABRICAMock.mockResolvedValue(true)
    getCohortAtEmitMock.mockReturnValue({ nth_repo_added: 3 })

    registerGitHubHandlers(store as never, stats as never)

    await expect(handlers['gh:starFABRICA'](null, 'settings')).resolves.toBe(true)

    expect(starFABRICAMock).toHaveBeenCalledTimes(1)
  ...
  it('accepts every app star source for success telemetry', async () => {
    ...
    for (const source of [
      ...
      await expect(handlers['gh:starFABRICA'](null, source)).resolves.toBe(true)
    }
  ...
  it('does not emit app_starred_FABRICA when the star action returns false', async () => {
    starFABRICAMock.mockResolvedValue(false)

    registerGitHubHandlers(store as never, stats as never)

    await expect(handlers['gh:starFABRICA'](null, 'landing')).resolves.toBe(false)
  ...
  it('does not emit app_starred_FABRICA when the star action throws', async () => {
    starFABRICAMock.mockRejectedValue(new Error('gh failed'))

    registerGitHubHandlers(store as never, stats as never)

    await expect(handlers['gh:starFABRICA'](null, 'star_nag')).rejects.toThrow('gh failed')
  ...
  it('preserves star result but skips telemetry for an invalid IPC source', async () => {
    ...
    await expect(handlers['gh:starFABRICA'](null, 'github_website')).resolves.toBe(true)
