...
describe('plugin consent IPC schema', () => {
  it('requires the fingerprint reviewed by the caller', () => {
    expect(() =>
      parsePluginConsentArgs({ pluginKey: 'fabrica-samples.demo', decision: 'approve' })
    ).toThrow()
  })
...
describe('plugin removal authority', () => {
  it('allows installed rows but refuses dev overrides and unknown keys', () => {
    const service = {
      getDiscovered: () => [
        { pluginKey: 'fabrica-samples.installed', isDev: false },
        { pluginKey: 'fabrica-samples.dev', isDev: true }
      ]
    } as unknown as PluginService

    expect(canRemoveInstalledPlugin(service, 'fabrica-samples.installed')).toBe(true)
    expect(canRemoveInstalledPlugin(service, 'fabrica-samples.dev')).toBe(false)
    expect(canRemoveInstalledPlugin(service, 'fabrica-samples.unknown')).toBe(false)
  })

  it('refuses bundled installs because startup would restore them', () => {
    const service = {
      getDiscovered: () => [{ pluginKey: 'autoscalers.fabrica-theme', isDev: false }]
    } as unknown as PluginService
    const lock = {
      version: 1,
      plugins: {
        'autoscalers.fabrica-theme': {
          pluginKey: 'autoscalers.fabrica-theme',
          version: '1.0.0',
          source: { kind: 'bundled', bundleId: 'autoscalers.fabrica-theme' },
          resolvedCommit: null,
    ...
    expect(canRemoveInstalledPlugin(service, 'autoscalers.fabrica-theme', lock)).toBe(false)
  })
})
