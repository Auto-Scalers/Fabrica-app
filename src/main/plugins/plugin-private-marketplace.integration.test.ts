...
const savedEnvironment = {
  ...
  FABRICA_TEST_SSH_REPOSITORIES: process.env.FABRICA_TEST_SSH_REPOSITORIES
}
...
describe('private Git marketplace integration', () => {
  it('uses the caller SSH environment for marketplace preview and install', async () => {
    const root = await mkdtemp(join(tmpdir(), 'FABRICA-private-marketplace-'))
    ...
    const pluginRepository = await createGitRepository(root, 'locale-source', {
      'fabrica-plugin.json': JSON.stringify({
        manifestVersion: 1,
        id: 'private-locale',
        publisher: 'private',
        name: 'Private Locale',
    ...
    const marketplaceRepository = await createGitRepository(root, 'marketplace-source', {
      'FABRICA-marketplace.json': JSON.stringify({
    ...
    process.env.FABRICA_TEST_SSH_REPOSITORIES = JSON.stringify({
