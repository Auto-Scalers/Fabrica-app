...
describe('Phase 1 launch plugin content', () => {
  it('lists and validates the launch plugin packs', async () => {
    const marketplace = pluginMarketplaceSchema.parse(
      await readJson(join(launchRoot, 'fabrica-marketplace.json'))
    )
  ...
  it('publishes every bundled pack only when its release hash matches exact bytes', async () => {
    const userDataPath = await mkdtemp(join(tmpdir(), 'FABRICA-launch-content-'))
  ...
  it('boots release-indexed content from the packaged resources layout', async () => {
    const resourcesPath = await mkdtemp(join(tmpdir(), 'FABRICA-packaged-resources-'))
    const userDataPath = await mkdtemp(join(tmpdir(), 'FABRICA-packaged-user-data-'))
    ...
    expect(result.errors).toEqual([])
    expect(result.installed).toEqual(['autoscalers.fabrica-navigation-shortcuts'])
  })
})
