...
async function pluginWithCatalog(catalog: unknown): Promise<ValidDiscoveredPlugin> {
  const rootDir = await mkdtemp(join(tmpdir(), 'FABRICA-plugin-language-registry-'))
  roots.push(rootDir)
  ...
  return {
    pluginKey: 'fabrica-samples.portuguese',
    rootDir,
    manifest,
...
describe('PluginLanguagePackRegistry', () => {
  it('loads approved catalogs under an isolated plugin language id', async () => {
    ...
    expect(registry.list()).toEqual([
      {
        id: 'plugin:fabrica-samples.portuguese/pt-BR',
        resourceLanguage:
          'plugin0070006c007500670069006e003a0066006100620072006900630061002d00730061006d0070006c00650073002e0070006f00720074007500670075006500730065002f00700074002d00420052',
        pluginKey: 'fabrica-samples.portuguese',
        locale: 'pt-BR',
