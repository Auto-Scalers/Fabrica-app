...
async function readInstallManifest(
  ...
  if (!satisfiesFabricaEngineRange(hostVersion, parsed.manifest.engines.fabrica)) {
    return {
      ok: false,
      error: `plugin requires Fabrica ${parsed.manifest.engines.fabrica} (this is ${hostVersion})`
    }
  }
...
export async function installStagedPluginTree(input: {
  ...
  if (blockedReason) {
    return { ok: false, error: `plugin is blocked by FABRICA's safety list: ${blockedReason}` }
  }
