...
/** API surface handed to a plugin's `activate(FABRICA)` export. Everything is
 *  EXPERIMENTAL until pluginApi v1 freezes. */
export type PluginWorkerFABRICAApi = {
...
export function createPluginWorkerRuntime(
  ...
  async function handleInit(input: {
    ...
    const FABRICA: PluginWorkerFABRICAApi = {
      commands: {
