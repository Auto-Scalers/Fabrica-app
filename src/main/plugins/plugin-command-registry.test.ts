...
describe('PluginCommandRegistry', () => {
  ...
  it('uses saved effective bindings to recover conflicting plugins', () => {
    ...
    registry.reconcile(
      [first, second],
      () => true,
      { 'plugin:fabrica-samples.first/tasks': ['Mod+Shift+T'] },
      'linux'
    )
  ...
  it('rejects conflicting saved bindings within one plugin', () => {
    ...
    registry.reconcile(
      ...
      () => true,
      {
        'plugin:fabrica-samples.aliases/tasks': ['Mod+Alt+T'],
        'plugin:fabrica-samples.aliases/sidebar': ['Mod+Alt+T']
      },
      'linux'
    )
