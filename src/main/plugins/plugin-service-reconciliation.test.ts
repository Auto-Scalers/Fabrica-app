...
async function pluginRoot(pluginManifest = manifest()): Promise<string> {
  const root = await mkdtemp(join(tmpdir(), 'FABRICA-plugin-reconcile-'))
  roots.push(root)
  await writeFile(join(root, 'fabrica-plugin.json'), JSON.stringify(pluginManifest))
  ...
  const service = new PluginService({
    ...
    getPluginKillListEntry: (key) =>
      killed && key === pluginKey
        ? { pluginKey, reason: 'Security incident', advisoryUrl: 'https://FABRICA.example/advisory' }
        : null,
...
describe('PluginService worker reconciliation', () => {
  it('blocks every runtime surface until a saved override resolves a content conflict', async () => {
    ...
    const service = new PluginService({
      ...
      getDisabledPlugins: () => [],
      getPluginConsents: () => ({
        'fabrica-samples.first': fingerprintPluginConsent(firstManifest, firstHash.hash),
        'fabrica-samples.second': fingerprintPluginConsent(secondManifest, secondHash.hash)
      }),
    ...
    expect(service.activationError('fabrica-samples.first')).toContain('conflicts')
    expect(service.getGrantedCapabilities('fabrica-samples.first')).toBeNull()
    await expect(service.invokeCommand('fabrica-samples.first', 'run')).rejects.toThrow('not enabled')
    await expect(service.panels.readEntry('fabrica-samples.first', 'panel')).resolves.toBeNull()
    ...
    keybindings = { 'plugin:fabrica-samples.first/run': ['Mod+Shift+T'] }
    await service.reconcileActivationState()

    expect(service.activationError('fabrica-samples.first')).toBeNull()
    await expect(service.panels.readEntry('fabrica-samples.first', 'panel')).resolves.toMatchObject({
      html: expect.stringContaining('<h1>Panel</h1>')
    })
    await expect(service.invokeCommand('fabrica-samples.first', 'run')).resolves.toBeNull()
