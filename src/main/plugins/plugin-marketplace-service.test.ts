...
async function tempRoot(): Promise<string> {
  const root = await mkdtemp(join(tmpdir(), 'FABRICA-marketplace-service-'))
  roots.push(root)
  return root
}
...
describe('PluginMarketplaceService', () => {
  ...
  it('rejects reserved identities outside the official organization', async () => {
    const service = new PluginMarketplaceService({
      ...
      fetcher: async () =>
        fetched(
          marketplace('Attack', 'community.fabrica-secrets', 'https://github.com/attacker/x.git')
        )
    })

    await expect(service.addSource(source())).rejects.toThrow(
      'reserved plugin identity community.fabrica-secrets'
    )
    await expect(service.listSources()).resolves.toEqual([])
  })
  ...
  it('derives the Official badge only from the canonical marketplace and source organization', async () => {
    const officialMarketplace: PluginMarketplace = {
      ...
      plugins: [
        {
          id: 'autoscalers.fabrica-shortcuts',
          source: {
            kind: 'git',
            url: 'git@github.com:Auto-Scalers/fabrica-shortcuts.git',
            ref: 'main'
          },
    ...
    await service.addSource(source('https://github.com/Auto-Scalers/fabrica-plugins.git'))

    await expect(service.listPlugins()).resolves.toEqual([
      expect.objectContaining({ pluginKey: 'autoscalers.fabrica-shortcuts', official: true })
    ])
  })
  ...
  it('seeds the official marketplace once and keeps it configured across restarts', async () => {
    ...
    const officialMarketplace = marketplace(
      'Fabrica Plugins',
      'autoscalers.fabrica-notes',
      'https://github.com/Auto-Scalers/fabrica-notes.git'
    )
    officialMarketplace.owner = 'Auto-Scalers'
    ...
    await expect(first.seedOfficialSource()).resolves.toMatchObject({
      official: true,
      marketplace: { name: 'Fabrica Plugins' }
    })
