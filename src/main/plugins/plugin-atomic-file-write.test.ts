...
describe('writePluginFileAtomically', () => {
  ...
  beforeEach(async () => {
    dir = await mkdtemp(join(tmpdir(), 'FABRICA-plugin-atomic-'))
  })
...
describe('renamePluginFileWithWindowsRetry', () => {
  it('renames when the source exists', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'FABRICA-plugin-rename-'))
    try {
  ...
  describe('on Windows', () => {
    ...
    beforeEach(async () => {
      dir = await mkdtemp(join(tmpdir(), 'FABRICA-plugin-rename-win-'))
  ...
  it('does not retry off Windows', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'FABRICA-plugin-rename-posix-'))
