...
async function createInstalledPlugin(options: { worker: boolean }): Promise<{
  ...
  const userDataPath = await mkdtemp(join(tmpdir(), 'FABRICA-plugin-service-integrity-'))
  ...
  await writeFile(join(stagingDir, 'fabrica-plugin.json'), JSON.stringify(manifest))
