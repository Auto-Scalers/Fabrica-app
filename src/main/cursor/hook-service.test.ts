...
describe('CursorHookService', () => {
  ...
  beforeEach(() => {
    homeDir = mkdtempSync(join(tmpdir(), 'FABRICA-cursor-home-'))
  ...
  it('installs Cursor Agent hooks with the documented top-level command schema', () => {
    ...
    for (const eventName of CURSOR_EVENTS) {
      ...
      if (process.platform !== 'win32') {
        expect(definition?.command).toContain(join(homeDir, '.FABRICA'))
      }
      expect(definition?.hooks).toBeUndefined()
    }

    const script = readFileSync(
      join(homeDir, '.FABRICA', 'agent-hooks', CURSOR_SCRIPT_FILE_NAME),
      'utf8'
    )
  ...
  it.skipIf(process.platform !== 'win32')(
    ...
    () => {
      const spaceHome = join(tmpdir(), 'FABRICA cursor home with spaces')
      ...
      `${JSON.stringify(
        {
          ...
          hooks: {
            beforeSubmitPrompt: [
              ...
              { command: '/old/path/.FABRICA/agent-hooks/cursor-hook.sh' }
            ],
            retiredEvent: [
              { command: '/old/path/.FABRICA/agent-hooks/cursor-hook.sh' },
