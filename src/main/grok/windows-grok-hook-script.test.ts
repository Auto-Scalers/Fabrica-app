...
describe('buildWindowsGrokHookScript', () => {
  ...
  it('re-checks the envelope after appending the trailing-backslash sentinel', () => {
    ...
    expect(lines[appended + 1]).toBe('if not "%FABRICA_GROK_HOME:~4096,1%"=="" set "FABRICA_GROK_HOME="')
  })
})
...
describe.skipIf(process.platform !== 'win32')('buildWindowsGrokHookScript (win32)', () => {
  ...
  function writeScript(): string {
    const dir = mkdtempSync(join(tmpdir(), 'FABRICA-grok-hook-'))
    dirs.push(dir)
    ...
    return new Promise((resolve, reject) => {
      ...
      server.listen(0, '127.0.0.1', () => {
        ...
        const env: NodeJS.ProcessEnv = {
          ...process.env,
          FABRICA_AGENT_HOOK_PORT: String(typeof address === 'object' && address ? address.port : 0),
          FABRICA_AGENT_HOOK_TOKEN: 'test-token',
          FABRICA_PANE_KEY: PANE_KEY,
          FABRICA_WORKTREE_ID: WORKTREE_ID
        }
