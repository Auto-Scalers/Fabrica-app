...
function createWindowsGrokHookEnvironment(grokHome?: string): NodeJS.ProcessEnv {
  ...
  env.FABRICA_AGENT_HOOK_TOKEN = 'test-token'
  env.FABRICA_PANE_KEY = 'pane-test'
  return env
}
...
async function runWindowsGrokHook(
  ...
  env.FABRICA_AGENT_HOOK_PORT = String(address.port)
  try {
...
describe('GrokHookService', () => {
  ...
  beforeEach(() => {
    homeDir = mkdtempSync(join(tmpdir(), 'FABRICA-grok-home-'))
  ...
  // every SessionStart/UserPromptSubmit on Windows outside FABRICA terminals.
  it('guards Windows GROK_HOME substring checks when empty (#9358)', () => {
    const script = buildWindowsGrokHookScript()
    expect(script).toContain('set "FABRICA_GROK_HOME="')
    expect(script).toContain('if not defined GROK_HOME goto :FABRICA_grok_home_ready')
    expect(script).toContain('%GROK_HOME:~4096,1%')
    expect(script).toContain('set "FABRICA_GROK_HOME=%GROK_HOME%"')
    expect(script).toContain('%FABRICA_GROK_HOME:~4096,1%')
    expect(script).toContain(':FABRICA_grok_home_ready')
    expect(script).toContain('if not defined FABRICA_GROK_HOME goto :FABRICA_grok_home_ready')
    expect(script).toContain('if "%FABRICA_GROK_HOME:~-1%"=="\\"')
    expect(script).toContain('if not "%GROK_HOME:~4096,1%"=="" goto :FABRICA_grok_home_ready')
  ...
  it('installs a dedicated global Grok hook config and managed script', () => {
    ...
    expect(status.configPath).toBe(join(homeDir, '.grok', 'hooks', 'FABRICA-status.json'))
    expect(status.managedHooksPresent).toBe(true)

    const config = JSON.parse(
      readFileSync(join(homeDir, '.grok', 'hooks', 'FABRICA-status.json'), 'utf8')
    ) as {
    ...
    if (process.platform !== 'win32') {
      expect(config.hooks.PreToolUse[0].hooks[0].command).toContain(join(homeDir, '.FABRICA'))
    }

    const script = readFileSync(
      join(homeDir, '.FABRICA', 'agent-hooks', GROK_SCRIPT_FILE_NAME),
      'utf8'
    )
    ...
    if (process.platform === 'win32') {
      ...
      expect(script).toContain('set "FABRICA_GROK_HOME=%GROK_HOME%"')
      expect(script).toContain('--data-urlencode "grokHome=%FABRICA_GROK_HOME%"')
    } else {
  ...
  it.skipIf(process.platform !== 'win32')(
    ...
    () => {
      const spaceHome = join(tmpdir(), 'FABRICA grok home with spaces')
      ...
      try {
        ...
        const config = JSON.parse(
          readFileSync(join(spaceHome, '.grok', 'hooks', 'FABRICA-status.json'), 'utf8')
  ...
  it('installs hooks under GROK_HOME when set', () => {
    const grokHome = mkdtempSync(join(tmpdir(), 'FABRICA-grok-home-env-'))
    const previous = process.env.GROK_HOME
    process.env.GROK_HOME = grokHome
    try {
      const status = new GrokHookService().install()
      expect(status.state).toBe('installed')
      expect(status.configPath).toBe(join(grokHome, 'hooks', 'FABRICA-status.json'))
      expect(readFileSync(join(grokHome, 'hooks', 'FABRICA-status.json'), 'utf8')).toContain(
        'SessionStart'
      )
  ...
  it('preserves user-authored hook entries in the FABRICA Grok config file', () => {
    const configPath = join(homeDir, '.grok', 'hooks', 'FABRICA-status.json')
