...
function getConfigPath(): string {
  ...
  // home Grok and transcript lookup use; keep FABRICA entries in a dedicated file
  // so user-authored hook files stay untouched.
  return join(resolveGrokHomeDir(), 'hooks', 'FABRICA-status.json')
}
...
function getManagedScript(target: 'local' | 'posix' = 'local'): string {
  ...
  return [
    ...
    'printf \'%s\' "$payload" | curl -sS -X POST "http://127.0.0.1:${FABRICA_AGENT_HOOK_PORT}/hook/grok" \\',
...
function buildInstalledConfig(
  ...
  // Why: FABRICA owns only grok-hook.* entries. Sweep stale managed commands out
