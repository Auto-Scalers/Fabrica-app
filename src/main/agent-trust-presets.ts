...
import { getFABRICAManagedCodexHomePath } from './codex/codex-home-paths'
 ...
 * first launch.
 *
 * Why: FABRICA's "drop URL into agent input as a draft" flow injects the URL
...
export function markCodexProjectTrusted(workspacePath: string): void {
  ...
  // Why: FABRICA-launched Codex runs with an FABRICA-owned CODEX_HOME, so the trust
  // preset must also update the runtime config Codex will actually read.
  upsertProjectTrustLevel(join(getFABRICAManagedCodexHomePath(), 'config.toml'), absPath, 'trusted')
}
...
function canonicalize(p: string): string {
  ...
  // (FABRICA caches realpath()'d worktree paths) matches the agent's lookup.
  try {
    if (existsSync(p)) {
