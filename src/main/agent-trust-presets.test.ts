...
beforeEach(() => {
  testState.fakeHomeDir = mkdtempSync(join(tmpdir(), 'FABRICA-trust-presets-'))
  testState.userDataDir = mkdtempSync(join(tmpdir(), 'FABRICA-trust-presets-user-data-'))
...
describe('markCursorWorkspaceTrusted', () => {
  it('writes ~/.cursor/projects/<slug>/.workspace-trusted with the cwd payload', () => {
    const workspace = mkdtempSync(join(tmpdir(), 'FABRICA-cursor-ws-'))
    try {
...
describe('markCopilotFolderTrusted', () => {
  it('appends the workspace to trustedFolders in ~/.copilot/config.json', () => {
    const workspace = mkdtempSync(join(tmpdir(), 'FABRICA-copilot-ws-'))
    try {
...
describe('markCodexProjectTrusted', () => {
  it('trusts the main repository root for a linked worktree without reading commondir', () => {
    const fixtureRoot = mkdtempSync(join(tmpdir(), 'FABRICA-codex-linked-ws-'))
  ...
  it('does not broaden trust through arbitrary or adversarial Git metadata', () => {
    const fixtureRoot = mkdtempSync(join(tmpdir(), 'FABRICA-codex-untrusted-gitdir-'))
  ...
  it('writes ~/.codex/config.toml with the project marked trusted', () => {
    const workspace = mkdtempSync(join(tmpdir(), 'FABRICA-codex-ws-'))
    try {
