...
/**
 * Counts the macOS TCC consent dialogs that name FABRICA as the responsible
 ...
 * FABRICA and puts FABRICA's name on the dialog, so users read it as FABRICA snooping.
 *
...
/** True when this dialog is one macOS raised in FABRICA's name for a watched file-access service. */
export function isFABRICAAttributedPrompt(event: TccPromptEvent): boolean {
  return (
    FABRICA_RESPONSIBLE_IDENTIFIERS.has(event.responsibleIdentifier) &&
...
/**
 * Watches for FABRICA-attributed TCC dialogs. macOS-only; `start()` is a no-op
...
export class MacosTccPromptWatch {
  ...
  private handleLine(line: string): void {
    ...
    if (!event || !isFABRICAAttributedPrompt(event)) {
      return
    }
