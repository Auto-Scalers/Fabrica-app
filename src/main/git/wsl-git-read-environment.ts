...
const PROBE_MARKER = 'FABRICA_WSL_GIT_READ_ENV_V1'
...
function probeWslGitReadEnvironment(distro: string): Promise<ProbeOutcome> {
  const probeCommand = [
    '_FABRICA_git_path=$(command -v git 2>/dev/null || true)',
    'case "$_FABRICA_git_path" in /*) [ -x "$_FABRICA_git_path" ] || exit 127 ;; *) exit 127 ;; esac',
    'if [ -n "${XDG_CONFIG_HOME:-}" ] || [ -n "${LD_LIBRARY_PATH:-}" ] || env | grep -q \'^GIT_\'; then exit 78; fi',
    `printf '\\0${PROBE_MARKER}\\0%s\\0%s\\0%s\\0' "$PATH" "$_FABRICA_git_path" "$HOME"`
  ].join('\n')
