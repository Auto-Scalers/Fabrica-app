...
describe('WSL direct Git reads', () => {
  ...
  it('coalesces the login-shell environment probe per distro', async () => {
    ...
    completeProbe?.(
      null,
      'profile banner\n\0FABRICA_WSL_GIT_READ_ENV_V1\0/home/user/bin:/usr/bin\0/home/user/bin/git\0/home/user\0',
      ''
    )
    ...
    try {
      ...
      execFileMock.mockImplementationOnce((_command, _args, _options, callback) => {
        ...
        queueMicrotask(() =>
          callback?.(
            null,
            `\0FABRICA_WSL_GIT_READ_ENV_V1\0${LOGIN_ENVIRONMENT.path}\0${LOGIN_ENVIRONMENT.gitPath}\0${LOGIN_ENVIRONMENT.home}\0`,
            ''
          )
        )
        return child
  ...
  it('keeps the first read on the login shell while priming the fast path', async () => {
    await withPlatform('win32', async () => {
      ...
      execFileMock.mockImplementation((_command, args, _options, callback) => {
        const child = createMockChild()
        if ((args as string[])[5]?.includes('FABRICA_WSL_GIT_READ_ENV_V1')) {
          completeProbe = callback
        } else {
