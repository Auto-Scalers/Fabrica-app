...
async function createRepoWithNewlineWorktree(): Promise<{
  ...
  const root = await mkdtemp(path.join(tmpdir(), 'FABRICA-worktree-paths-'))
  tempRoots.push(root)
...
async function createRepoWithLockedDeletedWorktree(): Promise<{
  ...
  const root = await mkdtemp(path.join(tmpdir(), 'FABRICA-worktree-locked-delete-'))
  tempRoots.push(root)
  const repoPath = path.join(root, 'repo')
  ...
  git(repoPath, ['worktree', 'lock', '--reason', 'FABRICA locked stale repro', requestedWorktreePath])
...
async function createRepoWithPrunableWorktree(): Promise<{
  ...
  const root = await mkdtemp(path.join(tmpdir(), 'FABRICA-worktree-prunable-'))
  tempRoots.push(root)
  const repoPath = path.join(root, 'repo')
