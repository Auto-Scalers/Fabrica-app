...
describe('resolveWorktreeSharedDirectories', () => {
  ...
  const writeFABRICAYaml = (body: string): void => {
    writeFileSync(join(repo, 'FABRICA.yaml'), body)
  }

  beforeEach(() => {
    clearConfiguredWorktreeSharedDirectoriesCacheForTests()
    repo = mkdtempSync(join(tmpdir(), 'FABRICA-shared-dirs-'))
    git(['init', '-q'], repo)
  ...
  it('returns gitignored directories listed under worktree.sharedDirectories', async () => {
    ...
    writeFABRICAYaml('worktree:\n  sharedDirectories:\n    - node_modules\n    - .cache\n')

    expect(await resolveWorktreeSharedDirectories(repo)).toEqual(['.cache', 'node_modules'])
  })

  it('returns [] when FABRICA.yaml is absent', async () => {
    mkdirSync(join(repo, 'node_modules'))

    expect(await resolveWorktreeSharedDirectories(repo)).toEqual([])
  })

  it('returns [] when FABRICA.yaml has no worktree key', async () => {
    mkdirSync(join(repo, 'node_modules'))
    writeFABRICAYaml('scripts:\n  setup: pnpm install\n')
...
describe('getConfiguredWorktreeSharedDirectories', () => {
  ...
  beforeEach(() => {
    clearConfiguredWorktreeSharedDirectoriesCacheForTests()
    repo = mkdtempSync(join(tmpdir(), 'FABRICA-shared-dirs-config-'))
  })
  ...
  it('combines live per-user paths with cached repo configuration', () => {
    writeFileSync(join(repo, 'FABRICA.yaml'), 'worktree:\n  sharedDirectories:\n    - node_modules\n')
  ...
  it('returns [] when FABRICA.yaml is absent or has no worktree key', () => {
    expect(getConfiguredWorktreeSharedDirectories(repo)).toEqual([])

    writeFileSync(join(repo, 'FABRICA.yaml'), 'scripts:\n  setup: pnpm install\n')
    clearConfiguredWorktreeSharedDirectoriesCacheForTests()
    expect(getConfiguredWorktreeSharedDirectories(repo)).toEqual([])
  })
...
describe('shared directories and worktree removal', () => {
  ...
  beforeEach(() => {
    root = mkdtempSync(join(tmpdir(), 'FABRICA-shared-dirs-removal-'))
