...
/**
 * Pin FABRICA-spawned git to untranslated English output so stderr/progress parsers
...
function defaultGhExecTimeoutMs(env: NodeJS.ProcessEnv = process.env): number {
  const raw = env.FABRICA_GH_EXEC_TIMEOUT_MS
  if (!raw) {
 ...
 * Why: git-in-WSL emits Linux-native paths, but FABRICA reads files via Node fs, which needs Windows UNC.
 */
