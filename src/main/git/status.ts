...
export type GetStatusOptions = GitRuntimeOptions & {
  ...
  /** Paths FABRICA may have symlinked into this worktree (per-user shared paths
   *  plus `FABRICA.yaml` shared directories). Untracked entries that are one of
   *  these *and* really symlinks are dropped: Git cannot ignore them when the
   *  repo's rule is directory-only (`node_modules/`), but they are FABRICA's own
...
/** Remove untracked entries that are shared symlinks FABRICA created.
 *
