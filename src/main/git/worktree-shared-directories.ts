...
// duplicates disk; `FABRICA.yaml` names the ones every worktree should share instead.
...
/** Every path FABRICA may have symlinked into a worktree: the per-user Worktree
 *  Shared Paths setting plus the repo's `FABRICA.yaml` shared directories.
 *
...
/** Resolve `worktree.sharedDirectories` from the repo-root `FABRICA.yaml` to
