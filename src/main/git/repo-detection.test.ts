...
describe('isGitRepo', () => {
  ...
  beforeEach(() => {
    tmpDir = mkdtempSync(path.join(tmpdir(), 'FABRICA-repo-detect-'))
  })
  ...
  it('accepts a linked worktree when git itself cannot be run', () => {
    ...
    git(realRepo, [
      ...
      '-c',
      'user.email=FABRICA@example.com',
      'commit',
      '--allow-empty',
      '--message',
      'initial'
  ...
  it('rejects a .git file that points at a missing gitdir when git cannot be run', () => {
    ...
    mkdirSync(fakeRepo)
    writeFileSync(path.join(fakeRepo, '.git'), 'gitdir: /missing/FABRICA/gitdir')
...
describe('getLinkedWorktreeMainRepoRoot', () => {
  ...
  beforeEach(() => {
    tmpDir = mkdtempSync(path.join(tmpdir(), 'FABRICA-linked-worktree-'))
  })
  ...
  function initRepoWithCommit(repoRoot: string): void {
    ...
    git(repoRoot, ['config', 'user.email', 'test@FABRICA.test'])
    git(repoRoot, ['config', 'user.name', 'FABRICA Test'])
