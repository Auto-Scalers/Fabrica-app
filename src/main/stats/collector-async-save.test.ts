...
const statsPath = (): string => join(userDataDir, 'FABRICA-stats.json')
...
describe('StatsCollector async debounced save', () => {
  beforeEach(() => {
    userDataDir = mkdtempSync(join(tmpdir(), 'FABRICA-stats-test-'))
    gate.blocked = false
  ...
  it('writes valid JSON via the async fs/promises path on the debounced timer, no stray temp files', async () => {
    ...
    await vi.waitFor(() => {
      expect(readdirSync(userDataDir)).toContain('FABRICA-stats.json')
    })
