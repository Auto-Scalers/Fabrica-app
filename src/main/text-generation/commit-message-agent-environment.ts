...
// must use the user's own ~/.codex. If FABRICA itself was launched from a nested
// FABRICA terminal it can inherit an FABRICA-owned CODEX_HOME override; strip only
// that (CODEX_HOME matching the private FABRICA_CODEX_HOME marker), preserving a
// user-set CODEX_HOME.
function cloneProcessEnvWithoutFABRICACodexHomeOverride(): Record<string, string> {
...
function prepareShellConfigDirEnv(agentId: string): { ok: true; env?: NodeJS.ProcessEnv } | null {
  ...
  // Why: each kind owns a distinct FABRICA_*_SOURCE_* shadow so a headless commit
  ...
  const sourceVar =
    agentId === 'opencode'
      ? 'FABRICA_OPENCODE_SOURCE_CONFIG_DIR'
      : agentId === 'pi'
        ? 'FABRICA_PI_SOURCE_AGENT_DIR'
        : agentId === 'omp'
          ? 'FABRICA_OMP_SOURCE_AGENT_DIR'
          : undefined
  ...
  // Why: GUI-launched FABRICA may not inherit shell startup exports, but these
  // vars point the headless CLI at the user's auth/config root. Nested FABRICA
  // launches inherit PTY overlays, so prefer FABRICA_*_SOURCE_* when present.
  ...
  try {
    if (agentId === 'codex' && resolvers.prepareForCodexLaunch) {
      ...
      if (target?.runtime === 'wsl') {
        ...
        // Why: the fallback must still strip FABRICA-owned overrides, or a
        // system-default WSL run inherits the managed CODEX_HOME.
        return {
          ok: true,
          env: codexHomeForTarget
            ? { ...cloneProcessEnvWithoutFABRICACodexHomeOverride(), CODEX_HOME: codexHomeForTarget }
            : cloneProcessEnvWithoutFABRICACodexHomeOverride()
        }
      }
