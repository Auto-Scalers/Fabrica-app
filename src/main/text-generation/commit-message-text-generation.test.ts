...
describe('discoverCommitMessageModelsLocal', () => {
    ...
    listeners.get('stdout:data')?.(
      Buffer.from(
        `${JSON.stringify({
          ...
          response: {
            subtype: 'success',
            request_id: 'FABRICA-model-discovery',
            response: {
              models: [
  ...
  it('falls back to the Claude seed models when the CLI lacks list_models', async () => {
    ...
    listeners.get('stdout:data')?.(
      Buffer.from(
        '{"type":"control_response","response":{"subtype":"error","request_id":"FABRICA-model-discovery","error":"Unsupported control request subtype: list_models"}}\n'
      )
    )
    listeners.get('close')?.(0)
...
describe('generateCommitMessageFromContext', () => {
  ...
  it('passes prepared provider environment to local agent subprocesses', async () => {
    ...
    const pending = generateCommitMessageFromContext(
      ...
      },
      {
        agentId: 'custom',
        model: '',
        customAgentCommand: 'FABRICA-test-agent-nope'
      },
      {
        kind: 'local',
        cwd: '/repo',
  ...
  it('routes WSL local commit generation through the selected distro login shell', async () => {
    await withPlatform('win32', async () => {
      process.env.FABRICA_HOST_ONLY_SECRET = 'do-not-leak'
      const listeners = new Map<string, (value: unknown) => void>()
      const child = {
        pid: 123,
        kill: vi.fn(),
      ...
      expect(spawnEnv.FABRICA_HOST_ONLY_SECRET).toBeUndefined()
      const shellCommand = spawnMock.mock.calls[0]?.[1]?.[5] as string
      expect(shellCommand).toContain('getent passwd')
      expect(shellCommand).toContain('exec "\\$_FABRICA_wsl_shell" -ilc')
      expect(shellCommand).toContain('/mnt/c/repo')
  ...
  it('rejects unsafe argv prompts for Windows batch-script agent commands', async () => {
    await withPlatform('win32', async () => {
      ...
      expect(result).toEqual({
        success: false,
        error:
          'C:/tools/agent.cmd cannot be run as a Windows batch command with the prompt in argv. Remove {prompt} so Fabrica sends the prompt on stdin.'
      })
      expect(spawnMock).not.toHaveBeenCalled()
    })
  })
