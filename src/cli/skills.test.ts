...
vi.mock('./runtime-client', async () => {
  ...
  return {
    ...
    serveFABRICAApp: vi.fn(),
    getDefaultUserDataPath: vi.fn(() => '/tmp/FABRICA-user-data')
  }
})
...
describe('FABRICA skills CLI', () => {
  ...
  it('lists installable skills when no --skill/--all is given', async () => {
    ...
    expect(stdoutText(stdoutSpy)).toBe(
      [
        ...
        '  zeta',
        '',
        'Usage: fabrica skills install --skill <name> [--skill <name> ...]',
        '   or: fabrica skills install --all',
        ''
      ].join('\n')
    )
      ...
      JSON.stringify(
        {
          ...
          error: {
            ...
            message:
              "fabrica skills install --json only supports --dry-run. Real installs stream npx's " +
  ...
  it('prints the resolved install command without running it for --dry-run', async () => {
    ...
    expect(stdoutText(stdoutSpy)).toBe(
      'npx --yes skills add https://github.com/Auto-Scalers/Fabrica-app --skill alpha --global --agent claude-code --agent universal -y\n\n' +
  ...
  it('drops --global for --local in the dry-run command and JSON', async () => {
    ...
    expect(stdoutText(stdoutSpy)).toBe(
      'npx --yes skills add https://github.com/Auto-Scalers/Fabrica-app --skill alpha --agent claude-code --agent universal -y\n\n' +
        'Rerun without --dry-run to install now.\n'
    )
  ...
  it('lists updatable skills when no --skill/--all is given', async () => {
    ...
    expect(stdoutText(stdoutSpy)).toBe(
      [
        ...
        '  zeta',
        '',
        'Usage: fabrica skills update --skill <name> [--skill <name> ...]',
        '   or: fabrica skills update --all',
        ''
      ].join('\n')
    )
    expect(spawnMock).not.toHaveBeenCalled()
  ...
  it('refuses a real run when the shell forwards FABRICA to the FABRICA host', async () => {
    vi.stubEnv('FABRICA_CLI_CWD', '/home/alice/wt')
    const errorSpy = vi.spyOn(console, 'error').mockImplementation(() => {})

    await main(['skills', 'install', '--skill', 'alpha'], '/tmp/repo')

    // Why: the SSH relay and WSL bridge run argv on the FABRICA host, so a real
  ...
  it('refuses to install when FABRICA detects no agent, instead of targeting them all', async () => {
  ...
  it('maps detected agents onto the skills CLI namespace, not FABRICA ids', async () => {
    const stdoutSpy = vi.spyOn(process.stdout, 'write').mockImplementation(() => true)
  ...
  it('reports the command it is about to run on stderr', async () => {
    ...
    expect(stderrSpy).toHaveBeenCalledWith(
      'Running: npx --yes skills add https://github.com/Auto-Scalers/Fabrica-app --skill alpha --global --agent claude-code --agent universal -y\n'
    )
  })
      ...
      JSON.stringify(
        {
          ...
          error: {
            code: 'invalid_argument',
            message:
              "fabrica skills update --json only supports --dry-run. Real updates stream npx's " +
              "own output, which isn't JSON."
          },
          _meta: { runtimeId: null }
        },
