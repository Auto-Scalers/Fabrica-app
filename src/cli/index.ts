...
// Why: the SSH relay bridge executes this CLI on the FABRICA host while the
// caller's shell cwd lives on the remote machine (which cannot be chdir'd
// into). FABRICA_CLI_CWD carries that remote cwd so cwd-based selectors like
// `--worktree active` resolve against the caller's directory.
function resolveInvocationCwd(): string {
  const override = process.env.FABRICA_CLI_CWD
...
export async function main(
  ...
  try {
    ...
    // lookup so users do not get misleading "FABRICA is not running" failures for
    ...
    // FABRICA_PAIRING_CODE / FABRICA_ENVIRONMENT env-var fallback for commands
...
async function runClaudeTeams(argv: string[], cwd: string): Promise<void> {
  try {
    // Why: everything after `fabrica claude-teams` belongs to Claude Code, not
    // FABRICA's own flag parser, so new Claude flags work without FABRICA changes.
...
async function runAgentTeamsTmuxShim(argv: string[]): Promise<void> {
  try {
    ...
    const response = await client.call<{
      ...
      {
        teamId: process.env.FABRICA_AGENT_TEAMS_TEAM_ID,
        token: process.env.FABRICA_AGENT_TEAMS_TOKEN,
