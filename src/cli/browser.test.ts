...
describe('FABRICA cli browser page targeting', () => {
  beforeEach(() => {
    callMock.mockReset()
  })
  ...
  it('passes explicit page ids to snapshot without resolving the current worktree', async () => {
    ...
    await main(['snapshot', '--page', 'page-1', '--json'], '/tmp/not-an-FABRICA-worktree')
  ...
  it('shows tab profile labels in text mode when requested', async () => {
    ...
    await main(['tab', 'list', '--show-profile', '--worktree', 'all'], '/tmp/not-an-FABRICA-worktree')
  ...
  it('shows a single tab by page id', async () => {
    ...
    await main(['tab', 'show', '--page', 'page-1', '--json'], '/tmp/not-an-FABRICA-worktree')
  ...
  it('resolves the current tab in a worktree', async () => {
    ...
    await main(['tab', 'current', '--worktree', 'all', '--json'], '/tmp/not-an-FABRICA-worktree')
...
describe('FABRICA cli browser profile management', () => {
  beforeEach(() => {
    callMock.mockReset()
  })
...
describe('FABRICA cli browser tab profiles', () => {
  beforeEach(() => {
    callMock.mockReset()
  })
  ...
  it('lists browser tab profiles', async () => {
    queueFixtures(
      ...
      okFixture('req_profiles', {
        profiles: [
          { id: 'default', scope: 'default', label: 'Default', partition: 'persist:FABRICA-browser' },
          {
            id: 'work',
            scope: 'isolated',
            label: 'Work',
            partition: 'persist:FABRICA-browser-session-work'
          }
        ]
      })
    )
    ...
    await main(['tab', 'profile', 'list', '--json'], '/tmp/not-an-FABRICA-worktree')
  ...
  it('reports an empty browser tab profile list with a friendly message', async () => {
    ...
    await main(['tab', 'profile', 'list'], '/tmp/not-an-FABRICA-worktree')
        ...
        profiles: [
          {
            ...
            scope: 'isolated',
            label: 'Google',
            partition: 'persist:FABRICA-browser-session-google',
            source: null,
    ...
    queueFixtures(
      ...
      okFixture('req_profile_create', {
        profile: {
          ...
          scope: 'imported',
          label: 'From Chrome',
          partition: 'persist:FABRICA-browser-session-imp'
        }
      })
    )
    vi.spyOn(console, 'log').mockImplementation(() => {})
  ...
  it('surfaces a runtime error if the registry refuses to create a profile', async () => {
    ...
    await main(['tab', 'profile', 'create', '--label', 'Bogus'], '/tmp/not-an-FABRICA-worktree')
  ...
  it('reports a not-deleted profile in text mode without throwing', async () => {
    ...
    await main(['tab', 'profile', 'delete', '--profile', 'default'], '/tmp/not-an-FABRICA-worktree')
...
describe('FABRICA cli browser cookies', () => {
  beforeEach(() => {
    callMock.mockReset()
...
describe('FABRICA cli browser waits and viewport flags', () => {
  beforeEach(() => {
    callMock.mockReset()
    process.exitCode = undefined
  })
  ...
  it('does not tell users FABRICA is down for a generic runtime timeout', async () => {
    ...
    await main(['wait', '--selector', '#ready', '--worktree', 'all'], '/tmp/not-an-FABRICA-worktree')

    expect(errorSpy).toHaveBeenCalledWith('Timed out waiting for the FABRICA runtime to respond.')
  })
