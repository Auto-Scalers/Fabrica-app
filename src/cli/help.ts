...
Common Commands:
  ...
  fabrica agent-context [--json]
  fabrica account add [--agent claude|codex] [--json]
  fabrica account list [--json]
  fabrica environment add --name <name> --pairing-code <code> [--json]
  fabrica environment list [--json]
  fabrica environment show --environment <selector> [--json]
  fabrica environment rm --environment <selector> [--json]
  fabrica worktree list [--repo <selector>] [--limit <n>] [--json]
  ...
  fabrica worktree rm --worktree <selector> [--force] [--run-hooks] [--json]
  fabrica worktree ps [--limit <n>] [--json]
  fabrica file open <path> [--worktree <selector>] [--json]
  fabrica file diff <path> [--staged] [--worktree <selector>] [--json]
  fabrica file open-changed [--mode edit|diff|both] [--worktree <selector>] [--json]
  fabrica terminal list [--worktree <selector>] [--limit <n>] [--include-visual-layouts] [--json]
  fabrica terminal show [--terminal <handle>] [--json]
  fabrica terminal read [--terminal <handle>] [--cursor <n>] [--limit <n>] [--json]
  fabrica terminal send [--terminal <handle>] [--text <text>] [--enter] [--interrupt] [--json]
  fabrica terminal wait [--terminal <handle>] --for exit|tui-idle [--timeout-ms <ms>] [--json]
  fabrica terminal stop --worktree <selector> [--json]
  fabrica terminal create [--worktree <selector>] [--title <name>] [--command <text>] [--focus] [--json]
  fabrica terminal split [--terminal <handle>] [--direction horizontal|vertical] [--json]
  fabrica terminal switch [--terminal <handle>] [--json]
  fabrica terminal close [--terminal <handle>] [--tab] [--json]
  fabrica project list [--json]
  fabrica project setups [--project <id>] [--host <host-id>] [--json]
  fabrica project setup-existing-folder --project <id> --host <host-id> --path <path> [--kind git|folder] [--display-name <name>] [--json]
  fabrica project setup-clone --project <id> --host <host-id> --url <clone-url> --destination <path> [--display-name <name>] [--json]
  fabrica project setup-create --project <id> --host <host-id> [--setup-id <id>] [--path <path>] [--kind git|folder] [--display-name <name>] [--worktree-base-path <path>] [--git-username <name>] [--state ready|not-set-up|setting-up|error|unsupported] [--method imported-existing-folder|cloned|provisioned] [--json]
  fabrica project setup-update --setup <setup-id> [--display-name <name>] [--path <path>] [--worktree-base-path <path>] [--git-username <name>] [--kind git|folder] [--state ready|not-set-up|setting-up|error|unsupported] [--method legacy-repo|imported-existing-folder|cloned|provisioned] [--json]
  fabrica project setup-delete --setup <setup-id> [--json]
  fabrica repo list [--json]
  fabrica repo add --path <path> [--json]
  fabrica repo show --repo <selector> [--json]
  fabrica repo set-base-ref --repo <selector> --ref <ref> [--json]
  fabrica repo search-refs --repo <selector> --query <text> [--limit <n>] [--json]
...
Selectors:
  ...
  --terminal <handle>       Runtime-issued terminal handle returned by \`fabrica terminal list --json\`
...
Browser Workflow:
  1. Create or navigate:  fabrica tab create --url https://example.com
                          fabrica goto --url https://example.com
  2. Inspect the page:    fabrica snapshot
     (Returns an accessibility tree with element refs like e1, e2, e3)
     For concurrent workflows, prefer: fabrica tab list --json
     then reuse tabs[].browserPageId with --page <id> on later commands.
  3. Interact:            fabrica click --element e2
                          fabrica fill --element e5 --value "search query"
                          fabrica keypress --key Enter
  4. Re-inspect:          fabrica snapshot
...
Examples:
  ...
  $ fabrica worktree set --worktree active --comment "waiting on review"
  $ fabrica worktree set --worktree active --linear-issue null
  $ fabrica worktree ps --limit 10
  ...
  $ fabrica tab show --page page_123 --json
  $ fabrica tab create --url https://example.com --profile work
  $ fabrica tab profile clone --page page_123 --profile work --json
  $ fabrica snapshot
  $ fabrica click --element e3
  $ fabrica fill --element e5 --value "hello"
  $ fabrica goto --url https://example.com/login
  $ fabrica keypress --key Enter
  $ fabrica eval --expression "document.title"
...
export function formatCommandHelp(spec: CommandSpec): string {
  const lines = [`fabrica ${spec.path.join(' ')}`, '', `Usage: ${spec.usage}`, '', spec.summary]
...
export function formatGroupHelp(specs: CommandSpec[], group: string): string {
  ...
  lines.push('', `Run \`fabrica ${group} <command> --help\` for command-specific usage.`)
  return lines.join('\n')
}
...
export function formatFlagHelp(flag: string): string {
  const helpByFlag: Record<string, string> = {
    ...
    page: '--page <id>            Stable browser page id from `fabrica tab list --json`',
