 ...
 * GUI flow drives this via a browser FABRICA can't reach on a headless host.
 */
...
async function addClaudeAccount({ client, json }: HandlerContext): Promise<void> {
  const configDir = mkdtempSync(join(tmpdir(), 'FABRICA-account-add-claude-'))
...
async function addCodexAccount({ client, json }: HandlerContext): Promise<void> {
  const codexHome = mkdtempSync(join(tmpdir(), 'FABRICA-account-add-codex-'))
  const session: InteractiveLoginSession = {
    child: null,
    registering: false,
    terminationPromise: null
...
/** CLI handlers for `fabrica account add [--agent claude|codex]` and `fabrica account list`. */
export const ACCOUNT_HANDLERS: Record<string, CommandHandler> = {
  'account add': async (ctx) => {
    ...
    rejectRemoteSelectionFlags(ctx, 'fabrica account add')
  ...
  'account list': async (ctx) => {
    rejectRemoteSelectionFlags(ctx, 'fabrica account list')
