...
vi.mock('../runtime-client', async () => {
  class RuntimeClient {
    ...
    constructor(
      ...
      remotePairingCode = process.env.FABRICA_PAIRING_CODE ?? null,
      environmentSelector = process.env.FABRICA_ENVIRONMENT ?? null
    ) {
...
describe('FABRICA linear CLI handlers', () => {
  ...
  beforeEach(() => {
    ...
    // Why: these tests can run inside an FABRICA-managed terminal, which exports
  ...
  it('passes verified current-context hints without resolving cwd for remote runtimes', async () => {
    process.env.FABRICA_TERMINAL_HANDLE = 'term_123'
    process.env.FABRICA_WORKTREE_ID = 'repo::/srv/app'
    process.env.FABRICA_PAIRING_CODE = 'FABRICA://pair?payload=bad'
    queueFixtures(callMock, okFixture('req_linear', issueResult()))
