...
function resolveKeepaliveIntervalMs(): number {
  const raw = process.env.FABRICA_KEEPALIVE_INTERVAL_MS ?? process.env.FABRICA_HEARTBEAT_INTERVAL_MS
  if (!raw) {
...
function resolveCompatibilityCliCommand(): 'fabrica' | 'FABRICA' | 'FABRICA-ide' | 'FABRICA-dev' {
  const configured = process.env.FABRICA_CLI_COMMAND
  if (configured === 'fabrica' || configured === 'FABRICA' || configured === 'FABRICA-ide' || configured === 'FABRICA-dev') {
    return configured
  }
  return 'fabrica'
}

function resolvePackagedWindowsCompatibilityCommand(): 'fabrica' | 'FABRICA' | 'FABRICA-ide' | undefined {
  if (process.env.FABRICA_WINDOWS_PACKAGED_CLI_LAUNCHER !== '1') {
    return undefined
  }
  const command = process.env.FABRICA_CLI_COMMAND
  if (command === 'fabrica' || command === 'FABRICA' || command === 'FABRICA-ide') {
    return command
  }
  throw new RuntimeClientError(
    'invalid_argument',
    'The packaged Fabrica launcher did not provide a valid resume command. No question was created.'
  )
}
...
async function resolveOrchestrationTerminalHandle(
  ...
  const envHandle = process.env.FABRICA_TERMINAL_HANDLE
  if (envHandle && envHandle.length > 0) {
    if (flagName === 'from' && options.validateEnvHandle) {
      // Why: long-lived shells can retain a stale FABRICA_TERMINAL_HANDLE after remint; don't bake it into coordinator preambles.
...
async function resolveOrchestrationPaneTerminalHandle(
  ...
  const paneKey = process.env.FABRICA_PANE_KEY
...
function throwNoActiveSenderTerminal(): never {
  throw new RuntimeClientError(
    ...
    'Could not determine the sender terminal for this orchestration command. ' +
      'Pass --from <terminal-handle> or run the command inside a live FABRICA terminal with FABRICA_TERMINAL_HANDLE set.'
  )
}
...
function isDevCliInvocation(): boolean {
  return (
    ...
    (process.env.FABRICA_USER_DATA_PATH?.includes('FABRICA-dev') ?? false)
  )
}
...
export const ORCHESTRATION_HANDLERS: Record<string, CommandHandler> = {
  ...
  'orchestration send': async ({ flags, client, cwd, json }) => {
    ...
    if (
      ...
      !process.env.FABRICA_TERMINAL_HANDLE
    ) {
      // Why: focus isn't lifecycle authority — an identity-less subprocess must fail closed rather than guess the worker.
      throwNoActiveSenderTerminal()
    }

    // Why: lifecycle senders preserve FABRICA_TERMINAL_HANDLE across restarts for older runtimes.
    ...
    const sendParams = {
      ...
      senderPaneKey: process.env.FABRICA_PANE_KEY || undefined,
  ...
  'orchestration check': async ({ flags, client, cwd, json }) => {
    ...
    try {
      result = await callMutation<CheckResult>(client, flags, 'orchestration.check', {
        terminal,
        terminalPaneKey: explicitTerminal ? undefined : process.env.FABRICA_PANE_KEY || undefined,
    ...
    if (model || effort) {
      ...
      if (
        ...
        throw new RuntimeClientError(
          ...
          'The connected FABRICA runtime does not support worker model or effort overrides. Update or restart FABRICA and try again.'
        )
      }
    }
    const result = await callMutation<{
