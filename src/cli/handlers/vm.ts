...
function doctorRecipe(repoPath: string, recipeId: string): DoctorResult {
  ...
  if (!existsSync(yamlPath)) {
    return {
      ...
      checks: [
        {
          id: 'FABRICA_yaml.exists',
          status: 'fail',
          message: `No FABRICA.yaml found at ${yamlPath}`,
          remediation: 'Add environmentRecipes to the repo FABRICA.yaml.'
        }
      ]
    }
  }

  const hooks = parseFABRICAYaml(readTextFile(yamlPath))
...
function loadRecipe(repoPath: string, recipeId: string): FABRICAVmRecipe | null {
  const hooks = parseFABRICAYaml(readTextFile(join(repoPath, 'FABRICA.yaml')))
