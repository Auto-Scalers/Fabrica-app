...
/** Agents FABRICA can see on this host, as `skills --agent` keys. */
...
function formatSkillSelectionHelp(verb: SkillMutationVerb, skillNames: string[]): string {
  return [
    ...
    '',
    `Usage: fabrica skills ${verb} --skill <name> [--skill <name> ...]`,
    `   or: fabrica skills ${verb} --all`
  ].join('\n')
}
...
function createSkillMutationHandler(verb: SkillMutationVerb): CommandHandler {
  return async ({ flags, json }) => {
    ...
    if (process.env.FABRICA_CLI_CWD) {
      throw new RuntimeClientError(
        'invalid_environment',
        `fabrica skills ${verb} writes to the machine that runs it, but this shell forwards ` +
          `fabrica to the Fabrica host. Run the same fabrica skills ${verb} command on the machine ` +
    ...
    if (json) {
      ...
      throw new RuntimeClientError(
        'invalid_argument',
        `fabrica skills ${verb} --json only supports --dry-run. Real ${verb}s stream ` +
