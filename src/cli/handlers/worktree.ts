...
function getEnvParentWorkspace(): string | undefined {
  const workspaceId = process.env.FABRICA_WORKSPACE_ID
  if (typeof workspaceId === 'string' && isWorkspaceKey(workspaceId)) {
    return workspaceId
  }
  const worktreeId = process.env.FABRICA_WORKTREE_ID
...
export const WORKTREE_HANDLERS: Record<string, CommandHandler> = {
  ...
  'worktree create': async ({ flags, client, cwd, json }) => {
    ...
    const callerTerminalHandle =
      typeof process.env.FABRICA_TERMINAL_HANDLE === 'string' &&
      process.env.FABRICA_TERMINAL_HANDLE.length > 0
    ...
    if (
      ...
      try {
        // Why: agent shells can lose FABRICA_TERMINAL_HANDLE while still running
        // inside an FABRICA worktree. Cwd keeps CLI-created children nestable and
