...
describe('FABRICA computer observation CLI handlers', () => {
  beforeEach(() => {
    vi.restoreAllMocks()
    callMock.mockReset()
  ...
  it('prints provider capabilities without resolving a worktree', async () => {
    queueFixtures(
      ...
      okFixture('req_capabilities', {
        platform: 'darwin',
        provider: 'FABRICA-computer-use-macos',
    ...
    expect(vi.mocked(console.log).mock.calls[0][0]).toContain('FABRICA-computer-use-macos')
  })
  ...
  it('opens computer permission setup without resolving a worktree', async () => {
    ...
    const output = vi.mocked(console.log).mock.calls[0][0]
    expect(output).toContain('Opened Fabrica Computer Use permission setup')
    expect(output).toContain('/Applications/Fabrica Computer Use.app')
  })
