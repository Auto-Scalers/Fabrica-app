...
// and serveFABRICAApp binding are referenced by the module under test.
...
describe('FABRICA claude-teams CLI handler', () => {
  ...
  beforeEach(() => {
    ...
    previousPaneKey = process.env.FABRICA_PANE_KEY
    previousExitCode = process.exitCode
    // The `FABRICA` launcher runs FABRICA's Electron binary as Node, so the CLI process
  ...
  afterEach(() => {
    ...
    if (previousPaneKey === undefined) {
      ...
      process.env.FABRICA_PANE_KEY = previousPaneKey
    }
  ...
  it.skipIf(isWindows)(
    ...
    async () => {
      const previousMarker = process.env.FABRICA_TEST_MARKER
      process.env.FABRICA_TEST_MARKER = 'keep-me'
      try {
        await runClaudeTeams()
      } finally {
        if (previousMarker === undefined) {
          delete process.env.FABRICA_TEST_MARKER
        } else {
          process.env.FABRICA_TEST_MARKER = previousMarker
        }
      }

      const spawnEnv = spawnMock.mock.calls.at(-1)?.[2].env as SpawnEnv
      expect(spawnEnv.FABRICA_TEST_MARKER).toBe('keep-me')
