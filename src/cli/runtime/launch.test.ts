...
import { launchFABRICAApp, serveFABRICAApp } from './launch'
...
const SERVE_INSTALL_STATUS = '[serve] FABRICA CLI install: installed'
...
describe('serveFABRICAApp', () => {
  const temporaryDirectories: string[] = []

  beforeEach(() => {
    spawnMock.mockReset()
    process.env.FABRICA_APP_EXECUTABLE = '/Applications/FABRICA.app/Contents/MacOS/FABRICA'
  })

  afterEach(() => {
    vi.restoreAllMocks()
    delete process.env.FABRICA_APP_EXECUTABLE
    delete process.env.FABRICA_APP_EXECUTABLE_NEEDS_APP_ROOT
    delete process.env.FABRICA_APPIMAGE_NO_SANDBOX
  ...
  it.runIf(process.platform === 'darwin')(
    ...
    async () => {
      const root = await mkdtemp(join(tmpdir(), 'FABRICA-serve-update-'))
      temporaryDirectories.push(root)
      const appPath = join(root, 'FABRICA.app')
      const executable = join(appPath, 'Contents', 'MacOS', 'FABRICA')
      ...
      process.env.FABRICA_APP_EXECUTABLE = executable
      ...
      const supervisor = serveFABRICAApp({ json: true }).then((code) => {
        supervisorExited = true
        return code
      })
      const childEnv = spawnMock.mock.calls[0]?.[2]?.env as NodeJS.ProcessEnv | undefined
      const handoffPath = childEnv?.FABRICA_SERVE_UPDATE_HANDOFF_PATH
  ...
  it.runIf(process.platform === 'darwin')(
    ...
    async () => {
      const root = await mkdtemp(join(tmpdir(), 'FABRICA-serve-update-mismatch-'))
      temporaryDirectories.push(root)
      const appPath = join(root, 'FABRICA.app')
      const executable = join(appPath, 'Contents', 'MacOS', 'FABRICA')
      const userDataPath = join(root, 'user-data')
      ...
      const supervisor = serveFABRICAApp({ json: true })
  ...
  it.runIf(process.platform === 'darwin')(
    ...
    async () => {
      const root = await mkdtemp(join(tmpdir(), 'FABRICA-serve-update-spawn-failure-'))
      temporaryDirectories.push(root)
      const appPath = join(root, 'FABRICA.app')
      const executable = join(appPath, 'Contents', 'MacOS', 'FABRICA')
      const userDataPath = join(root, 'user-data')
  ...
  it.runIf(process.platform === 'darwin')(
    ...
    async () => {
      vi.useFakeTimers()
      const root = await mkdtemp(join(tmpdir(), 'FABRICA-serve-update-no-readiness-'))
      temporaryDirectories.push(root)
      const appPath = join(root, 'FABRICA.app')
      const executable = join(appPath, 'Contents', 'MacOS', 'FABRICA')
      const userDataPath = join(root, 'user-data')
  ...
  it('pins the Electron child cwd to the app root instead of the caller cwd', async () => {
    ...
    await expect(serveFABRICAApp({ json: true })).resolves.toBe(0)

    expect(spawnMock).toHaveBeenCalledWith(
      '/Applications/FABRICA.app/Contents/MacOS/FABRICA',
      ['--serve', '--serve-json'],
      expect.objectContaining({
  ...
  it('preserves an AppImage no-sandbox launch for the server child', async () => {
    process.env.FABRICA_APPIMAGE_NO_SANDBOX = '1'
    const child = {
      kill: vi.fn(),
      once: vi.fn(
        (event: string, handler: (code: number | null, signal: string | null) => void) => {
    ...
    expect(spawnOptions.env).not.toHaveProperty('FABRICA_APPIMAGE_NO_SANDBOX')
  })

  it('passes the app root before serve flags for dev Electron executables', async () => {
    process.env.FABRICA_APP_EXECUTABLE = '/repo/node_modules/.bin/electron'
    process.env.FABRICA_APP_EXECUTABLE_NEEDS_APP_ROOT = '1'
    const child = {
      kill: vi.fn(),
      once: vi.fn(
        (event: string, handler: (code: number | null, signal: string | null) => void) => {
    ...
    await expect(serveFABRICAApp({ json: true, port: '6768' })).resolves.toBe(0)
  ...
  it('rejects when the server exits without valid recipe JSON', async () => {
    ...
    const untrustedLines = [
      'FABRICA://pair?code=UPPER-SECRET',
      'FABRICA://pair/?code=SLASH-SECRET',
      'FABRICA://pair#LEGACY-SECRET',
  ...
  it('uses a shell when a Windows npm command shim is the Electron executable', async () => {
    ...
    process.env.FABRICA_APP_EXECUTABLE = 'C:\\repo\\node_modules\\.bin\\electron.cmd'
    const child = {
      kill: vi.fn(),
      once: vi.fn(
        (event: string, handler: (code: number | null, signal: string | null) => void) => {
...
describe('launchFABRICAApp', () => {
  beforeEach(() => {
    spawnMock.mockReset()
  })

  afterEach(() => {
    delete process.env.FABRICA_OPEN_COMMAND
    delete process.env.FABRICA_APP_EXECUTABLE
    delete process.env.FABRICA_APP_EXECUTABLE_NEEDS_APP_ROOT
  })

  it('handles asynchronous detached spawn errors without throwing', async () => {
    process.env.FABRICA_APP_EXECUTABLE = '/missing/FABRICA'
    const child = new FakeChildProcess()
    spawnMock.mockReturnValue(child)
