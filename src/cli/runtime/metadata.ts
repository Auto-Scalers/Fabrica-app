...
export function getDefaultUserDataPath(
  ...
  // runtime metadata to a separate userData directory (e.g. `FABRICA-dev`) to avoid
  ...
  if (platform === 'win32') {
    ...
    if (!appData) {
      throw new RuntimeClientError(
        'runtime_unavailable',
        'APPDATA is not set, so the Fabrica runtime metadata path cannot be resolved.'
      )
    }
    return join(appData, 'Fabrica')
  }
