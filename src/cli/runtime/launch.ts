...
export function launchFABRICAApp(): void {
  const overrideCommand = process.env.FABRICA_OPEN_COMMAND
  if (typeof overrideCommand === 'string' && overrideCommand.trim().length > 0) {
    spawnDetached(overrideCommand, [], { shell: true })
    return
  }

  const overrideExecutable = process.env.FABRICA_APP_EXECUTABLE
...
function spawnDetached(command: string, args: string[], options: SpawnOptions): void {
  ...
  // returns; openFABRICA already reports the user-facing timeout if startup fails.
  child.once('error', () => {})
  child.unref()
}

export function serveFABRICAApp(
  args: {
    json?: boolean
    port?: string | null
  ...
  const executable = resolveForegroundFABRICAExecutable()
  const childArgs = [...getExecutableAppArgs()]
  if (process.env.FABRICA_APPIMAGE_NO_SANDBOX === '1') {
  ...
  delete childEnv.FABRICA_APPIMAGE_NO_SANDBOX
  if (handoffPath) {
...
function waitForRecipeJson(child: ReturnType<typeof spawnProcess>): Promise<number> {
  return new Promise((resolve, reject) => {
    ...
    const processRecipeOutputLine = (line: string): void => {
      ...
      if (getEphemeralVmRecipeResultConnection(parsed.result).type !== 'FABRICA-server') {
        writeIgnoredRecipeStdout()
        return
      }
...
function getExecutableAppArgs(): string[] {
  return process.env.FABRICA_APP_EXECUTABLE_NEEDS_APP_ROOT === '1' ? [resolveAppRoot()] : []
}
...
function resolveAppRoot(): string {
  ...
  // process.cwd(). Pin it to the app root so `fabrica serve` behaves the same
  // regardless of the shell directory it was launched from.
  return resolve(__dirname, '../../..')
}

function resolveForegroundFABRICAExecutable(): string {
  const overrideExecutable = process.env.FABRICA_APP_EXECUTABLE
  if (typeof overrideExecutable === 'string' && overrideExecutable.trim().length > 0) {
