...
const {
  ...
  serveFABRICAAppMock: vi.fn(),
  getDefaultUserDataPathMock: vi.fn(() => '/tmp/FABRICA-user-data'),
...
vi.mock('./runtime-client', async () => {
    ...
    constructor(
      ...
      const effectivePairingCode =
        remotePairingCode === undefined
          ? (process.env.FABRICA_PAIRING_CODE ?? process.env.FABRICA_REMOTE_PAIRING)
          : remotePairingCode
      const effectiveEnvironment =
        environmentSelector === undefined ? process.env.FABRICA_ENVIRONMENT : environmentSelector
  ...
  return {
    ...
    serveFABRICAApp: serveFABRICAAppMock,
...
describe('command aliases dispatch to the canonical handler', () => {
  ...
  it('keeps `agent-context` local when remote environment variables are set', async () => {
    vi.stubEnv('FABRICA_PAIRING_CODE', 'pairing-code')
    vi.stubEnv('FABRICA_ENVIRONMENT', 'stale-environment')
    try {
      await main(['agent-context', '--json'], '/tmp/repo')
...
describe('artifact runtime routing', () => {
  ...
  it('uses the desktop runtime despite remote-selection environment fallbacks', async () => {
    vi.stubEnv('FABRICA_ENVIRONMENT', 'remote-environment')
    vi.stubEnv('FABRICA_PAIRING_CODE', 'remote-pairing-code')
...
describe('unknown command surfaces a suggestion', () => {
  ...
  it('prints did-you-mean for a near-miss command and exits non-zero', async () => {
    ...
    expect(stderr).toContain('fabrica worktree')
  })
...
describe('unknown help command surfaces a suggestion', () => {
  it.each([
    ...
    expect(process.exitCode).toBe(1)
    expect(logSpy.mock.calls.flat().join('\n')).toContain('Did you mean: fabrica worktree')
    logSpy.mockRestore()
    process.exitCode = 0
  })
})

describe('FABRICA root help', () => {
  ...
  it('progressively discloses Linear commands', async () => {
    ...
    expect(groupHelp).toContain('fabrica linear')
    ...
    expect(issueHelp).toContain('fabrica linear issue [<id>]')
    ...
    expect(searchHelp).toContain('fabrica linear search <query>')
  ...
  it('hides removed parent-workspace help and scopes create parent selectors', async () => {
    ...
    expect(createHelp).not.toContain('active FABRICA workspace')
...
describe('FABRICA cli worktree awareness', () => {
  const originalTerminalHandle = process.env.FABRICA_TERMINAL_HANDLE
  const originalUserDataPath = process.env.FABRICA_USER_DATA_PATH
  const originalDevCliInvocation = process.env.FABRICA_DEV_CLI_INVOCATION
  const originalPairingCode = process.env.FABRICA_PAIRING_CODE
  const originalRemotePairing = process.env.FABRICA_REMOTE_PAIRING
  const originalEnvironment = process.env.FABRICA_ENVIRONMENT
  const originalWorkspaceId = process.env.FABRICA_WORKSPACE_ID
  const originalWorktreeId = process.env.FABRICA_WORKTREE_ID

  beforeEach(() => {
    callMock.mockReset()
    delete process.env.FABRICA_TERMINAL_HANDLE
    delete process.env.FABRICA_USER_DATA_PATH
    delete process.env.FABRICA_DEV_CLI_INVOCATION
    delete process.env.FABRICA_WORKSPACE_ID
    delete process.env.FABRICA_WORKTREE_ID
    ...
    delete process.env.FABRICA_PANE_KEY
    serveFABRICAAppMock.mockReset()
  ...
  afterEach(() => {
    ...
    if (originalTerminalHandle === undefined) {
      ...
      process.env.FABRICA_TERMINAL_HANDLE = originalTerminalHandle
    }
    if (originalUserDataPath === undefined) {
      delete process.env.FABRICA_USER_DATA_PATH
    } else {
      process.env.FABRICA_USER_DATA_PATH = originalUserDataPath
    }
    if (originalDevCliInvocation === undefined) {
      delete process.env.FABRICA_DEV_CLI_INVOCATION
    } else {
      process.env.FABRICA_DEV_CLI_INVOCATION = originalDevCliInvocation
    }
    if (originalPairingCode === undefined) {
      delete process.env.FABRICA_PAIRING_CODE
    } else {
      process.env.FABRICA_PAIRING_CODE = originalPairingCode
    }
    if (originalRemotePairing === undefined) {
      delete process.env.FABRICA_REMOTE_PAIRING
    } else {
      process.env.FABRICA_REMOTE_PAIRING = originalRemotePairing
    }
    if (originalEnvironment === undefined) {
      delete process.env.FABRICA_ENVIRONMENT
    } else {
      process.env.FABRICA_ENVIRONMENT = originalEnvironment
    }
    if (originalWorkspaceId === undefined) {
      delete process.env.FABRICA_WORKSPACE_ID
    } else {
      process.env.FABRICA_WORKSPACE_ID = originalWorkspaceId
    }
    if (originalWorktreeId === undefined) {
      delete process.env.FABRICA_WORKTREE_ID
    } else {
      process.env.FABRICA_WORKTREE_ID = originalWorktreeId
    }
  })
  ...
  it('resolves the invocation cwd from FABRICA_CLI_CWD when no cwd is passed', async () => {
    // Why: the SSH relay bridge runs the CLI on the FABRICA host with the remote
    // shell's cwd carried in FABRICA_CLI_CWD (#7716); cwd-based selectors must
    // resolve against it, not the host process cwd.
    process.env.FABRICA_CLI_CWD = '/tmp/repo/feature/src'
    try {
      queueFixtures(
        callMock,
        worktreeListFixture([
      ...
      delete process.env.FABRICA_CLI_CWD
    }
  })

  it.skipIf(process.platform === 'win32')(
    'prepares and starts Claude Agent Teams in the current FABRICA terminal',
    async () => {
      process.env.FABRICA_PANE_KEY = 'tab-1:11111111-1111-4111-8111-111111111111'
      queueFixtures(
        callMock,
        okFixture('req_agent_teams_prepare', {
          launch: {
            env: {
              ...
              TMUX: '/tmp/FABRICA-claude-agent-teams/team-1,0,1',
              TMUX_PANE: '%1',
              PATH: '/tmp/FABRICA-shim:/usr/bin'
            }
          }
        })
      )
      ...
      expect(callMock).toHaveBeenCalledWith('agentTeams.prepareLaunch', {
        ...
        env: expect.objectContaining({
          FABRICA_PANE_KEY: 'tab-1:11111111-1111-4111-8111-111111111111'
        })
      })
  ...
  it('resolves project and host flags to the matching repo for worktree.create', async () => {
    queueFixtures(
      ...
      okFixture('req_create', {
        worktree: buildWorktree('/srv/FABRICA/feature', 'feature', 'abc', 'repo-gpu'),
        lineage: null,
        warnings: []
      })
    )
  ...
  it('passes folder workspace environment lineage through worktree.create', async () => {
    process.env.FABRICA_WORKSPACE_ID = 'folder:folder-1'
    queueFixtures(
      callMock,
      worktreeListFixture([buildWorktree('/tmp/repo', 'main', 'abc', 'repo-1')]),
      okFixture('req_create', {
  ...
  it('passes caller terminal handle through worktree.create with cwd fallback', async () => {
    process.env.FABRICA_TERMINAL_HANDLE = 'term_parent'
    queueFixtures(
      callMock,
      worktreeListFixture([buildWorktree('/tmp/repo', 'main', 'abc', 'repo-1')]),
      okFixture('req_create', {
  ...
  it('starts a foreground headless server through `serve`', async () => {
    serveFABRICAAppMock.mockResolvedValue(0)
    process.env.FABRICA_ENVIRONMENT = 'stale-env'
    ...
    expect(serveFABRICAAppMock).toHaveBeenCalledWith({
      json: true,
      port: '6768',
  ...
  it('runs vm recipe doctor locally without contacting the app runtime', async () => {
    const repoPath = mkdtempSync(path.join(tmpdir(), 'FABRICA-vm-doctor-'))
    try {
      mkdirSync(path.join(repoPath, 'scripts', 'FABRICA-vm'), { recursive: true })
      const startScript = path.join(repoPath, 'scripts', 'FABRICA-vm', 'start.sh')
      const cleanupScript = path.join(repoPath, 'scripts', 'FABRICA-vm', 'cleanup.sh')
      ...
      writeFileSync(
        path.join(repoPath, 'FABRICA.yaml'),
        [
          'environmentRecipes:',
          '  - id: cloud-sandbox',
          '    name: Cloud Sandbox',
          '    create: ./scripts/FABRICA-vm/start.sh',
          '    destroy: ./scripts/FABRICA-vm/cleanup.sh'
        ].join('\n')
      )
      const logSpy = vi.spyOn(console, 'log').mockImplementation(() => {})
      ...
      expect(output.checks).toEqual(
        expect.arrayContaining([
          expect.objectContaining({ id: 'FABRICA_yaml.parse', status: 'pass' }),
  ...
  it('warns when vm recipe doctor finds no cleanup hook', async () => {
    ...
    try {
      mkdirSync(path.join(repoPath, 'scripts', 'FABRICA-vm'), { recursive: true })
      writeFileSync(path.join(repoPath, 'scripts', 'FABRICA-vm', 'start.sh'), '#!/bin/sh\n')
      writeFileSync(
        path.join(repoPath, 'FABRICA.yaml'),
        [
          'environmentRecipes:',
  ...
  it('runs vm recipe doctor provision mode and invokes cleanup', async () => {
    const repoPath = mkdtempSync(path.join(tmpdir(), 'FABRICA-vm-doctor-provision-'))
    ...
    try {
      mkdirSync(path.join(repoPath, 'scripts', 'FABRICA-vm'), { recursive: true })
      writeFileSync(
        path.join(repoPath, 'scripts', 'FABRICA-vm', 'start.js'),
        [
      ...
      writeFileSync(
        path.join(repoPath, 'scripts', 'FABRICA-vm', 'cleanup.js'),
        [
          "const fs = require('fs')",
      ...
      writeFileSync(
        ...
        [
          ...
          '  - id: cloud-sandbox',
          '    name: Cloud Sandbox',
          `    create: ${JSON.stringify(`${process.execPath} ./scripts/FABRICA-vm/start.js`)}`,
          `    destroy: ${JSON.stringify(`${process.execPath} ./scripts/FABRICA-vm/cleanup.js`)}`
        ].join('\n')
      )
      const { EventEmitter } = await import('node:events')
  ...
  it('returns the full create transcript when provision fails so the agent can self-diagnose', async () => {
    const repoPath = mkdtempSync(path.join(tmpdir(), 'FABRICA-vm-doctor-provision-fail-'))
    try {
      mkdirSync(path.join(repoPath, 'scripts', 'FABRICA-vm'), { recursive: true })
      writeFileSync(path.join(repoPath, 'scripts', 'FABRICA-vm', 'start.js'), 'process.exit(0)')
      writeFileSync(
        path.join(repoPath, 'FABRICA.yaml'),
        [
          'environmentRecipes:',
  ...
  it('rejects recipe JSON output without a project root', async () => {
    ...
    expect(serveFABRICAAppMock).not.toHaveBeenCalled()
    expect([...logSpy.mock.calls, ...errSpy.mock.calls].flat().join('\n')).toContain(
  ...
  it('lists saved environments even when FABRICA_ENVIRONMENT is set', async () => {
    process.env.FABRICA_ENVIRONMENT = 'stale-env'
    ...
    expect(listEnvironmentsMock).toHaveBeenCalledWith('/tmp/FABRICA-user-data')
    expect(callMock).not.toHaveBeenCalled()
    expect(logSpy.mock.calls[0]?.[0]).not.toContain('token')
    expect(logSpy.mock.calls[0]?.[0]).not.toContain('publicKeyB64')
  })

  it('adds saved environments even when FABRICA_ENVIRONMENT is set', async () => {
    process.env.FABRICA_ENVIRONMENT = 'stale-env'
    const logSpy = vi.spyOn(console, 'log').mockImplementation(() => {})

    await main(
      ['environment', 'add', '--name', 'desk', '--pairing-code', 'FABRICA://pair#abc', '--json'],
      '/tmp/repo'
    )

    expect(addEnvironmentFromPairingCodeMock).toHaveBeenCalledWith('/tmp/FABRICA-user-data', {
      name: 'desk',
      pairingCode: 'FABRICA://pair#abc'
    })
    expect(callMock).not.toHaveBeenCalled()
    expect(logSpy.mock.calls[0]?.[0]).not.toContain('token')
    expect(logSpy.mock.calls[0]?.[0]).not.toContain('publicKeyB64')
  ...
  it('sends remote repo.add absolute paths unchanged', async () => {
    ...
    await main(
      ['repo', 'add', '--path', '/srv/FABRICA/web', '--pairing-code', 'remote-runtime', '--json'],
      '/tmp/repo'
    )
  ...
  it('formats group orchestration sends in text mode', async () => {
    process.env.FABRICA_TERMINAL_HANDLE = 'term_sender'
  ...
  it('rejects unknown task-update status with an enum-aware error', async () => {
    process.env.FABRICA_TERMINAL_HANDLE = 'term_coord'
    const logSpy = vi.spyOn(console, 'log').mockImplementation(() => {})
    const errSpy = vi.spyOn(console, 'error').mockImplementation(() => {})
    const priorExitCode = process.exitCode
  ...
  it('passes the caller terminal handle through orchestration task-create', async () => {
    process.env.FABRICA_TERMINAL_HANDLE = 'term_creator'
    callMock.mockResolvedValueOnce({
      id: 'req_task_create',
      ok: true,
      result: {
  ...
  it('passes dev mode to injected orchestration dispatches', async () => {
    process.env.FABRICA_TERMINAL_HANDLE = 'term_sender'
    process.env.FABRICA_USER_DATA_PATH = '/tmp/FABRICA-dev'
    callMock.mockResolvedValueOnce({
      id: 'req_dispatch',
      ok: true,
      result: {
  ...
  it('passes dev mode from an explicit dev CLI marker with a custom profile path', async () => {
    process.env.FABRICA_TERMINAL_HANDLE = 'term_sender'
    process.env.FABRICA_USER_DATA_PATH = '/tmp/federation-acceptance-profile'
    process.env.FABRICA_DEV_CLI_INVOCATION = '1'
    callMock.mockResolvedValueOnce({
      id: 'req_dispatch',
      ok: true,
      result: {
  ...
  it('sends explicit remote terminal create worktree selectors unchanged', async () => {
    queueFixtures(
      ...
      okFixture('req_terminal_create', {
        terminal: {
          handle: 'term_1',
          worktreeId: 'repo-1::/srv/FABRICA/feature',
    ...
    expect(callMock).toHaveBeenCalledWith('terminal.create', {
      worktree: 'id:repo-1::/srv/FABRICA/feature',
      command: undefined,
      title: undefined,
      focus: false
    })
  ...
  it('exits nonzero when terminal wait returns an unsatisfied blocked result', async () => {
    process.env.FABRICA_TERMINAL_HANDLE = 'term_worker'
    callMock.mockResolvedValueOnce({
  ...
  it('updates project host setup metadata through the project-first runtime API', async () => {
    ...
    expect(callMock).toHaveBeenCalledWith('projectHostSetup.update', {
      ...
      updates: {
        displayName: 'GPU VM',
        path: path.resolve('/tmp/repo', '/srv/FABRICA'),
