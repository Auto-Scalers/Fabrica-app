...
import { launchFABRICAApp } from './runtime/launch'

vi.mock('./runtime/launch', () => ({
  launchFABRICAApp: vi.fn()
}))
...
afterEach(async () => {
  vi.mocked(launchFABRICAApp).mockClear()
...
function writeMetadata(
  ...
  writeFileSync(
    join(userDataPath, 'FABRICA-runtime.json'),
    JSON.stringify({
      runtimeId: 'runtime-1',
      pid,
      transports: [
...
describe.skipIf(process.platform === 'win32')('RuntimeClient', () => {
  it('adds an opaque durable request ID only to orchestration mutations', async () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'FABRICA-runtime-client-'))
    ...
    const priorLaunchToken = process.env.FABRICA_AGENT_LAUNCH_TOKEN
    process.env.FABRICA_AGENT_LAUNCH_TOKEN = 'launch-secret'
    ...
    try {
      ...
      if (priorLaunchToken === undefined) {
        delete process.env.FABRICA_AGENT_LAUNCH_TOKEN
      } else {
        process.env.FABRICA_AGENT_LAUNCH_TOKEN = priorLaunchToken
      }
    }
  ...
  it('openFABRICA activates the app even when a desktop runtime is already reachable', async () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'FABRICA-runtime-client-'))
    const endpoint = join(userDataPath, 'runtime.sock')
    const server = createServer((socket) => {
      sockets.add(socket)
    ...
    const client = new RuntimeClient(userDataPath, 100)
    const status = await client.openFABRICA(100)

    expect(status.result.runtime.state).toBe('ready')
    expect(status.result.runtime.reachable).toBe(true)
    expect(launchFABRICAApp).toHaveBeenCalledOnce()
  })

  it('openFABRICA waits for a reachable headless runtime to expose a desktop window', async () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'FABRICA-runtime-client-'))
    const endpoint = join(userDataPath, 'runtime.sock')
    ...
    const client = new RuntimeClient(userDataPath, 100)
    const status = await client.openFABRICA(1_000)
  ...
  it('openFABRICA fails explicitly when the serve owner cannot promote safely', async () => {
    const userDataPath = mkdtempSync(join(tmpdir(), 'FABRICA-runtime-client-'))
    const endpoint = join(userDataPath, 'runtime.sock')
    const server = createServer((socket) => {
      sockets.add(socket)
    ...
    await expect(client.openFABRICA(100)).rejects.toMatchObject({
      code: 'desktop_activation_blocked'
    })
    // A blocked runtime can't promote, so we bail before spawning the app.
    expect(launchFABRICAApp).not.toHaveBeenCalled()
  })
