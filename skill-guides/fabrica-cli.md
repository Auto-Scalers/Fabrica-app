...
Use `fabrica` when Orca's running editor/runtime is the source of truth. Inside Orca-managed terminals, `fabrica` always resolves to the Fabrica CLI on every platform. In any other shell on Linux, use `fabrica` too — outside Orca's terminals, bare `orca` on Linux is the GNOME Orca screen reader (`/usr/bin/orca`), and running it starts speech on the user's machine, so never invoke it. The legacy `orca` shim still exists on macOS and Windows as insurance against stale references.

**Dev builds (`pnpm dev`):** after `pnpm build:cli`, the dev CLI is exposed as `fabrica-dev` (the global shim points at this checkout's wrapper + out/cli). Inside a dev Orca's terminals use `fabrica-dev emulator ...` (or `./config/scripts/orca-dev.mjs emulator ...` for worktree-local invocation that does not depend on the /usr/local/bin symlink). Plain `fabrica` targets any installed production Orca. The app's own agent preambles use `fabrica-dev` automatically in dev mode.
...
- Otherwise, use `fabrica`. Never use bare `orca` on Linux because it normally resolves to the
  GNOME screen reader.
...
Think of its id as a two-part address: `<repoId>::<worktreePath>`. For example, `repo-123::/Users/me/orca/fix-login` means “the `fix-login` checkout inside repo `repo-123`.” Always copy the complete `id` field from `fabrica worktree create --json` or `fabrica worktree list --json`; `repo-123` alone identifies only the repo.
...
These commands control only Orca's embedded browser tabs. For external Chrome/Safari/webviews or Orca app chrome/settings, use the Computer Use skill/tool. If the user explicitly asks for Orca CLI desktop control, use `fabrica computer ...`; do not use browser commands for desktop UI.
...
- Use typed tab commands (`fabrica tab list/create/close/switch`), not `fabrica exec --command "tab ..."`, so Orca keeps UI state synchronized.
...
The mobile emulator surface is workspace-scoped like browser tabs (active per worktree for unqualified; explicit --worktree/--device/--emulator for targeting). Always prefer `fabrica emulator ...` over raw `npx serve-sim` or simctl when inside Orca (the bridge owns lifecycle, scoping, and registration with the live pane).
...
- No raw serve-sim in agent prompts/skills (use fabrica wrappers; see orca-emulator skill).
