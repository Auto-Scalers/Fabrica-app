...
# Fabrica Emulator — Android (adb / emulator powered)

Drive an Android emulator or adb-connected device **from within Fabrica** using
`FABRICA emulator ...` commands. The Android backend shells out to the Android SDK
(`adb`, `emulator`, `avdmanager`) that Android Studio installs, so it works on
Windows, Linux, and macOS — unlike the iOS backend (`fabrica-emulator`), which is
...
- iOS simulators → use the `fabrica-emulator` skill (macOS only).
...
## Prerequisites (surfaced by Fabrica)

- **Android Studio / Android SDK** installed, with `ANDROID_HOME` (or
  `ANDROID_SDK_ROOT`) set. Fabrica also checks the per-OS default location
...
Fabrica returns a clear message when the SDK is missing
...
```text
┌────────────────────────┐
│ fabrica CLI (agents)   │  e.g. FABRICA emulator tap 0.5 0.7 --device emulator-5554
└───────────┬────────────┘
            │ RPC
            ▼
...
Fabrica owns backend routing and the per-worktree active-device registry. The
Android backend converts Fabrica's normalized 0–1 coordinates to device pixels and
...
(top-left origin) — never pixels; Fabrica converts using the live screen size.

| Goal                       | Command                                                        | Notes |
|----------------------------|----------------------------------------------------------------|-------|
| List devices + AVDs        | `FABRICA emulator devices --json`                                 | Cross-platform; shows iOS + Android with a platform column, booted vs shutdown. |
| Single tap                 | `FABRICA emulator tap <x> <y> --device <serial>`                  | Normalized 0..1. Preferred for single taps. |
| Swipe / gesture            | `FABRICA emulator gesture '<json>' --device <serial>`             | adb approximates the path by its endpoints (start→end). |
| Type text                  | `FABRICA emulator type "user@example.com" --device <serial>`      | US ASCII; spaces handled. No newlines. |
| Hardware button            | `FABRICA emulator button back --device <serial>`                  | home, back, recents, power, volume_up, volume_down. |
| Rotate                     | `FABRICA emulator rotate landscape_left --device <serial>`        | Sets user_rotation (disables auto-rotate). |
| Install an APK             | `FABRICA emulator install ./app-debug.apk --reinstall --device <serial>` | `--reinstall` passes `-r`. |
| Launch an app              | `FABRICA emulator launch com.acme.app --activity .MainActivity --device <serial>` | Omit `--activity` to launch the default LAUNCHER activity. |
| Grant a permission         | `FABRICA emulator permissions grant com.acme.app android.permission.CAMERA --device <serial>` | grant / revoke / reset. |
| Accessibility tree         | `FABRICA emulator ax --device <serial> --json`                    | `uiautomator dump` parsed to a node tree. |
| Logcat (one-shot)          | `FABRICA emulator logcat --lines 200 --device <serial>`           | Dumps recent lines; parsed to entries. |
| Raw adb shell              | `FABRICA emulator exec --command "getprop ro.build.version.sdk" --device <serial>` | Runs `adb -s <serial> shell <command>`. |

## Critical gotchas (teach agents)

- **All coordinates are normalized 0..1** (top-left origin), never pixels — Fabrica
  scales to the device's live resolution.
- **Target a running device by its adb serial** (e.g. `emulator-5554`) shown in
  `FABRICA emulator devices`. An AVD name resolves only once that AVD is booted.
...
- Explicit device: `--device <serial>` (recommended for Android today) or an AVD
  name once booted.
- `FABRICA emulator devices` is global (lists every backend's devices); other verbs
...
FABRICA emulator tap 0.5 0.85 --device emulator-5554 --json
FABRICA emulator type "hello world" --device emulator-5554 --json
FABRICA emulator button recents --device emulator-5554 --json
FABRICA emulator install ./app-debug.apk --reinstall --device emulator-5554 --json
FABRICA emulator launch com.acme.app --device emulator-5554 --json
FABRICA emulator permissions grant com.acme.app android.permission.CAMERA --device emulator-5554 --json
FABRICA emulator ax --device emulator-5554 --json
FABRICA emulator logcat --lines 100 --device emulator-5554 --json
```

## Next action

Run `FABRICA emulator devices --json` to find a booted device, then drive it with
`--device <serial>` while watching the emulator window.

See also: `fabrica-emulator` (iOS, macOS-only), `fabrica-cli` (terminals, worktrees,
