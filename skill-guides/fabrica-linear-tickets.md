...
If the installed CLI help disagrees with this skill, trust `fabrica linear --help` for the available command surface and tell the user the skill guidance may be stale.
...
```bash
fabrica linear search "auth bug" --workspace all --limit 10 --json
...
```bash
fabrica linear save-issue [<id>] [--current] [--team <key|id>] [--title <title>] [--description <text> | --body-file <path|->] [--state <state>] [--assignee me|<user>|null] [--priority none|low|medium|high|urgent] [--estimate <number>|null] [--due-date <yyyy-mm-dd>|null] [--label <label>]... [--project <project>|null] [--parent-id <issue>|null] [--write-id <uuid>] [--workspace <id>] [--json]
fabrica linear issue [<id>] [--current] [--comments] [--children] [--depth <n>] [--attachments] [--relations] [--activity] [--full] [--workspace <id>] [--json]
fabrica linear list-issues [--team <team>] [--cycle <cycle>] [--label <label>] [--limit <n>] [--query <text>] [--state <state>] [--cursor <cursor>] [--order-by createdAt|updatedAt] [--project <project>] [--release <release>] [--assignee <user|me|null>] [--delegate <user|me|null>] [--parent-id <issue|null>] [--priority <0-4>] [--created-at <datetime|duration>] [--updated-at <datetime|duration>] [--include-archived] [--workspace <id>|all] [--json]
fabrica linear relation add [<id>] [--current] --related <issue> --type blocks|blocked-by|related|duplicate-of [--workspace <id>] [--json]
fabrica linear relation remove [<id>] [--current] --related <issue> --type blocks|blocked-by|related|duplicate-of [--workspace <id>] [--json]
fabrica linear search <query> [--limit <n>] [--workspace <id>|all] [--json]
fabrica linear team list [--workspace <id>|all] [--json]
fabrica linear team members --team <key|id> [--workspace <id>] [--json]
fabrica linear team states --team <key|id> [--workspace <id>] [--json]
fabrica linear team labels --team <key|id> [--workspace <id>] [--json]
fabrica linear project list [--query <text>] [--limit <n>] [--workspace <id>|all] [--json]
fabrica linear list [--filter assigned|created|all|completed|open] [--team <key|id>] [--limit <n>] [--workspace <id>|all] [--json]
fabrica linear status set [<id>] [--current] --to <state> [--workspace <id>] [--json]
fabrica linear assignee set [<id>] [--current] (--me | --to-id <userId>) [--workspace <id>] [--json]
fabrica linear assignee clear [<id>] [--current] [--workspace <id>] [--json]
fabrica linear priority set [<id>] [--current] --to none|low|medium|high|urgent [--workspace <id>] [--json]
fabrica linear priority clear [<id>] [--current] [--workspace <id>] [--json]
fabrica linear estimate set [<id>] [--current] --to <number> [--workspace <id>] [--json]
fabrica linear estimate clear [<id>] [--current] [--workspace <id>] [--json]
fabrica linear due-date set [<id>] [--current] --to <yyyy-mm-dd> [--workspace <id>] [--json]
fabrica linear due-date clear [<id>] [--current] [--workspace <id>] [--json]
fabrica linear label add [<id>] [--current] --label <labelId-or-exact-name>... [--workspace <id>] [--json]
fabrica linear label remove [<id>] [--current] --label <labelId-or-exact-name>... [--workspace <id>] [--json]
fabrica linear label set [<id>] [--current] --label <labelId-or-exact-name>... [--workspace <id>] [--json]
fabrica linear comment add [<id>] [--current] (--body <text> | --body-file <path|->) [--reply-to <commentId>] [--write-id <uuid>] [--workspace <id>] [--json]
fabrica linear attach [<id>] [--current] --url <url> [--title <title>] [--write-id <uuid>] [--workspace <id>] [--json]
fabrica linear create --title <title> [--body <text> | --body-file <path|->] [--team <key|id>] [--project <projectId-or-exact-name>] [--state <stateId|exact-name>] [--assignee me|<userId>] [--priority none|low|medium|high|urgent] [--estimate <number>] [--due-date <yyyy-mm-dd>] [--label <labelId-or-exact-name>]... [--parent <id> | --parent-current] [--write-id <uuid>] [--workspace <id>] [--json]
```
...
```bash
fabrica linear team list --workspace all --json
fabrica linear team states --team <key-or-id> --workspace <workspaceId> --json
fabrica linear team labels --team <key-or-id> --workspace <workspaceId> --json
fabrica linear team members --team <key-or-id> --workspace <workspaceId> --json
fabrica linear project list --query <project-name> --workspace <workspaceId> --json
```
...
```bash
fabrica linear list --filter assigned --limit 10 --workspace all --json
fabrica linear list --filter open --team <key-or-id> --workspace <workspaceId> --json
```
...
The PR/MR command is `fabrica linear attach`; there is no `attach-pr` command.

Attach the PR/MR link:

```bash
fabrica linear attach --current --url <pr-or-mr-url> --title "PR/MR link" --json
```

Use stdin for multiline comments:

```bash
fabrica linear comment add --current --body-file - --json
```
...
2. Otherwise try `fabrica linear status set --current --to "In Review" --json`.
...
```bash
fabrica linear create --title <title> --parent-current --body-file - --json
```
...
```bash
fabrica linear issue <id> --workspace <workspaceId> --json
```
...
Confirm `fabrica status --json` unless already checked this turn, then read the current issue with `fabrica linear issue --current --full --json`. For completion, attach the PR/MR link, add one completion comment, and move status only when the target state is deterministic and non-regressive.
