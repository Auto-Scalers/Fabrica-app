...
9. **[CHECKPOINT] Live self-test** — get the user's OK once, then run
   `fabrica vm recipe doctor <recipe-id> --provision --json` as a loop: it runs create → validates →
...
# 3. remote exec: start fabrica serve in the background and read the recipe JSON it writes (see below)
...
**The exact `fabrica serve` invocation and its output (verified — do not improvise the flags).** Inside the
VM, run:
...
**Binary name:** in a VM built from source (the Phase-2 flow), run it as `pnpm exec fabrica-dev serve …`
from the repo root — `fabrica-dev` is the in-repo entrypoint and is what the §7f example uses. Plain
`fabrica serve …` is the same command when the built CLI is installed on the VM's PATH. The flags/output
...
vercel sandbox exec "$name" "${vercel_args[@]}" --timeout 20m \
  --env "GH_TOKEN=$gh_token" --env "FABRICA_PROJECT_ROOT=$project_root" \
  --env "FABRICA_REPO_URL=$repo_url" --env "FABRICA_REPO_REF=$repo_ref" \
  -- bash -lc 'set -euo pipefail; cd "$FABRICA_PROJECT_ROOT"; \
...
# 1. ensure the repo is present and at the right commit on the host (NO fabrica serve here)
...
`connection.type:"ssh"` block above instead of starting `fabrica serve`.
...
docker run -d --name "$name" -p 127.0.0.1::22 -e "FABRICA_SSH_PUBLIC_KEY=$pubkey" "$auth_image"
...
Here `pairingCode` (from `fabrica serve --recipe-json`) and `projectRoot` are required; `schemaVersion` (`1`)
and `userData` are optional.

**SSH mode** — do **not** run `fabrica serve`; print the `connection.type:"ssh"` block instead (full shape +
...
`fabrica vm recipe doctor <recipe-id> --repo-path <repo> --json` validates **static wiring only** — it does
...
`fabrica vm recipe doctor <recipe-id> --repo-path <repo> --provision --json` actually runs the recipe end
