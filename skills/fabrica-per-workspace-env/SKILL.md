...
environment reference is served by the `fabrica` binary itself — kept out of this file on
purpose so it can never drift from the binary that will actually run your commands.

Engage Fabrica whenever you set up, review, debug, or validate a per-workspace environment
...
`environmentRecipes` entry in `fabrica.yaml`, scaffold provider lifecycle scripts, or resolve
an `fabrica vm recipe doctor` failure. Fabrica is a thin wrapper: you guide, detect, and scaffold;
...
```text
FABRICA skills get fabrica-per-workspace-env
```

That prints the complete, version-matched guide for the exact binary that will handle your
next commands — provider setup, base and auth snapshots, `environmentRecipes` in
`fabrica.yaml`, lifecycle scripts, and `fabrica vm recipe doctor`. Read it first, then run the
...
```text
FABRICA status --json
FABRICA vm recipe doctor <recipe-id> --repo-path <repo> --json
```
...
`FABRICA skills get fabrica-per-workspace-env`. Beyond these commands, ask the user rather than
