...
_Last recount: 2026-08-31 (Group H expanded: added macOS+Linux rebuilds H7-H8; Group G confirmed all done. 63 tasks total: 49 DONE, 14 TODO — 7 Group H physical testing + 7 Group I PM-feedback items.)_
...
> Started 2026-08-30. PM-directed: physically test the live app, then rebuild installers for all platforms. Gate for marketing trigger.
...
| APP-H7 | Rebuild macOS installer (.dmg) | ⬜ | After H1-H5 pass: `pnpm build` + electron-builder --mac, publish .dmg to GitHub release |
| APP-H8 | Rebuild Linux packages (.AppImage + .deb) | ⬜ | After H1-H5 pass: `pnpm build` + electron-builder --linux, publish AppImage + deb |
| APP-H9 | Rebuild Android APK (EAS) | ⬜ | After H1-H5 pass: `eas build -p android --profile preview`, publish artifact |

### Group G — Follow-up Fixes (ALL DONE)

| # | Task | Status | Notes |
|---|------|--------|-------|
| APP-G1 | UI color contrast audit | ✅ | Token-level WCAG AA fixes |
| APP-G2 | Orca/Fabrica isolation audit | ✅ | Diagnosis done, namespace isolation fixed |
| APP-G3 | Enhance icon/logo | ✅ | Colored icon wired, DEFAULT_APP_ICON_ID='light' |
| APP-G4 | Investigate unavailable features | ✅ | Root cause diagnosed, G4-FIX deployed |
| APP-G5 | Investigate plugins not listing | ✅ | Root cause diagnosed, G5-FIX pushed |
| APP-G6 | Android APK handling | ✅ | APK built via EAS |
| APP-G7 | UI rebrand to non-technical | ✅ | Copy reworded, font/zoom defaults landed |
| APP-G2-FIX | Isolate shared namespaces from Orca | ✅ | fabrica- prefix applied, remote hooks namespaced |
| APP-G4-FIX | Deploy /v1/desktop/* backend | ✅ | Deployed to Vercel |
| APP-G5-FIX | Fix marketplace owner mismatch | ✅ | Committed bca1b85, pushed |
| APP-G6-DEC | Build+sign Fabrica APK | ✅ | Prep done |
| APP-G4-ENV | Set Vercel env | ✅ | PM confirmed |
| APP-G6-BUILD | Run `eas build` | ✅ | PM ran |
| APP-G6-RUN | Capture APK artifact | ✅ | URL reported |
...
| Field | Value |
|---|---|
| **Current Group** | Group H — Physical Testing & Rebuild (APP-H1–H9) |
| **Current Task** | APP-H1: Physical test: desktop login — ⬜ awaiting PM |
| **Last Action** | Group H expanded (macOS + Linux rebuilds added); Group G confirmed all done; REVIEW.md removed |
| **Next Action** | PM physical tests H1–H5 (desktop login, relay pairing, phone control, plugins, general UI). After pass → H6-H9 rebuild (Windows, macOS, Linux, Android). After rebuild → trigger marketing. |
...
_Last updated: 2026-08-31 (Group H expanded: added macOS+Linux rebuilds H7-H8; Group G confirmed all done; REVIEW.md removed. Rollup 63 tasks, 49 DONE, 14 TODO (78%).)_
